...
namespace RO.Web
{
	public partial class AdmMenuOptModule : RO.Web.ModuleBase
	{
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_111";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_111";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_111";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_111";
		private const string KEY_dtLabel = "Cache:dtLabel3_111";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_111";
		private const string KEY_dtScrCri = "Cache:dtScrCri3_111";
		private const string KEY_dsScrCriVal = "Cache:dsScrCriVal3_111";

		private const string KEY_dtProjectId248 = "Cache:dtProjectId248";
		private const string KEY_dtInputBy248 = "Cache:dtInputBy248";

		private const string KEY_dtSystems = "Cache:dtSystems3_111";
		private const string KEY_sysConnectionString = "Cache:sysConnectionString3_111";
		private const string KEY_dtAdmMenuOpt111List = "Cache:dtAdmMenuOpt111List";
		private const string KEY_bNewVisible = "Cache:bNewVisible3_111";
		private const string KEY_bNewSaveVisible = "Cache:bNewSaveVisible3_111";
		private const string KEY_bCopyVisible = "Cache:bCopyVisible3_111";
		private const string KEY_bCopySaveVisible = "Cache:bCopySaveVisible3_111";
		private const string KEY_bDeleteVisible = "Cache:bDeleteVisible3_111";
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_111";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_111";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_111";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				DataTable dtMenuAccess = (new MenuSystem()).GetMenu(base.LUser.CultureId, 3, base.LImpr, LcSysConnString, LcAppPw,111, null, null);
				...
				Session.Remove(KEY_dtAdmMenuOpt111List);
				...
				Session.Remove(KEY_dtProjectId248);
				Session.Remove(KEY_dtInputBy248);
				SetButtonHlp();
				GetSystems();
				SetColumnAuthority();
				GetGlobalFilter();
				...
				cTab72.InnerText = dt.Rows[0]["TabFolderName"].ToString();
				cTab73.InnerText = dt.Rows[1]["TabFolderName"].ToString();
				cTab74.InnerText = dt.Rows[2]["TabFolderName"].ToString();
				cTab75.InnerText = dt.Rows[3]["TabFolderName"].ToString();
				SetClientRule(null,false);
				IgnoreConfirm(); InitPreserve();
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 111, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmMenuOpt111List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=111&typ=N&sys=3','','',''); return false;"; }
		...
		private void SetSystem(byte SystemId)
		{
			...
			try
			{
				...
				if ((Config.DeployType == "DEV" || row["dbAppDatabase"].ToString() == base.CPrj.EntityCode + "View") && !(base.CPrj.EntityCode != "RO" && row["SysProgram"].ToString() == "Y") && (new AdminSystem()).IsRegenNeeded(string.Empty,111,0,0,LcSysConnString,LcAppPw))
				{
					(new GenScreensSystem()).CreateProgram(111, "Menu Option", row["dbAppDatabase"].ToString(), base.CPrj, base.CSrc, base.CTar, LcAppConnString, LcAppPw);
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(111,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(111,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(111,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(111,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,111,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(111,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(111,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(111,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		protected DataTable GetAuthRow()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthRow(111,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				try
				{
					try {dv = new DataView((new AdminSystem()).GetExp(111,"GetExpAdmMenuOpt111","Y",null,null,filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
					catch {dv = new DataView((new AdminSystem()).GetExp(111,"GetExpAdmMenuOpt111","N",null,null,filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(111,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					{
						if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["MenuOptId248"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["MenuOptName248"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["MenuOptDesc248"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["MenuOptIco248"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["MenuOptImg248"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append(drv["ProjectId248"].ToString() + (char)9 + drv["ProjectId248Text"].ToString() + (char)9);}
						if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["OvaRating248"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[7]["ColExport"].ToString() == "Y") {sb.Append(drv["InputBy248"].ToString() + (char)9 + drv["InputBy248Text"].ToString() + (char)9);}
						if (dtAu.Rows[8]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmShortDateTimeUTC(drv["InputOn248"].ToString(),base.LUser.Culture,CurrTimeZoneInfo()) + (char)9);}
						if (dtAu.Rows[9]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["TopMenuCss248"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[10]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["SidMenuCss248"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[11]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["TopMenuJs248"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[12]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["SidMenuJs248"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[13]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["TopMenuIvk248"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[14]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["SidMenuIvk248"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						sb.Append(Environment.NewLine);
					}
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmMenuOpt.csv"; Session["ExportStr"] = (Config.ExportExcelCSV ? "sep=\t\n": "") + sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmMenuOpt.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private string GetRtfGrid(DataView dv)
		{
			...
			try
			{
				...
				foreach (DataRowView drv in dv)
				{
					...
					sb.Append("\r\n");
					sb.Append(@"{");
					if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["MenuOptId248"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["MenuOptName248"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(drv["MenuOptDesc248"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(drv["MenuOptIco248"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append(drv["MenuOptImg248"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append(drv["ProjectId248Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["OvaRating248"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[7]["ColExport"].ToString() == "Y") {sb.Append(drv["InputBy248Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[8]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmShortDateTimeUTC(drv["InputOn248"].ToString(),base.LUser.Culture,CurrTimeZoneInfo()) + @"\cell ");}
					if (dtAu.Rows[9]["ColExport"].ToString() == "Y") {sb.Append(drv["TopMenuCss248"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[10]["ColExport"].ToString() == "Y") {sb.Append(drv["SidMenuCss248"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[11]["ColExport"].ToString() == "Y") {sb.Append(drv["TopMenuJs248"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[12]["ColExport"].ToString() == "Y") {sb.Append(drv["SidMenuJs248"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[13]["ColExport"].ToString() == "Y") {sb.Append(drv["TopMenuIvk248"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[14]["ColExport"].ToString() == "Y") {sb.Append(drv["SidMenuIvk248"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					sb.Append(@"}");
					sb.Append("\r\n");
					sb.Append(@"\pard \ql \li0\ri0\widctlpar\intbl\aspalpha\aspnum\adjustright\rin0\lin0 {");
					sb.Append(@"\insrsid2981395 \trowd \irow0\irowband0\lastrow \ts15\trgaph108\trleft-108\trbrdrt\brdrs\brdrw10 \trbrdrl\brdrs\brdrw10 \trbrdrb\brdrs\brdrw10 \trbrdrr\brdrs\brdrw10 \trbrdrh\brdrs\brdrw10 \trbrdrv\brdrs\brdrw10 ");
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 111, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			...
			UpdCriteria(true);
			cAdmMenuOpt111List.ClearSearch(); Session.Remove(KEY_dtAdmMenuOpt111List);
			PopAdmMenuOpt111List(sender, e, false, null);
		}
			...
			try
			{
				foreach (DataRowView drv in dvCri)
				{
					...
					if (drv["DisplayName"].ToString() == "ComboBox")
					{
						...
						(new AdminSystem()).MkGetScreenIn("111", drv["ScreenCriId"].ToString(), "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), LcAppDb, LcDesDb, drv["MultiDesignDb"].ToString(), LcSysConnString, LcAppPw);
						DataView dv = new DataView((new AdminSystem()).GetScreenIn("111", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), (new AdminSystem()).CountScrCri(drv["ScreenCriId"].ToString(), drv["MultiDesignDb"].ToString(), drv["MultiDesignDb"].ToString() == "N" ? LcSysConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)), drv["RequiredValid"].ToString(), 0, string.Empty, drv["DisplayMode"].ToString() != "AutoListBox", val, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)));
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("111", LcSysConnString, LcAppPw);
				}
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmMenuOpt111" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmMenuOpt111" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			foreach (DataRowView drv in dvCri)
			{
			    if (drv["DisplayName"].ToString() == "ListBox")
			    {
					...
					if (cListBox != null)
					{
						...
						int TotalChoiceCnt = new DataView((new AdminSystem()).GetScreenIn("111", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), CriCnt, drv["RequiredValid"].ToString(), 0, string.Empty, true, string.Empty, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId))).Count;
						string selectedValues = string.Join(",", cListBox.Items.Cast<ListItem>().Where(x => x.Selected).Select(x => "'" + x.Value + "'").ToArray());
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("111", "AdmMenuOpt111", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		private DataTable GetScreenTab()
		{
			...
			try
			{
				dtScreenTab = (new AdminSystem()).GetScreenTab(111,base.LUser.CultureId,LcSysConnString,LcAppPw);
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			return dtScreenTab;
		}

		private void SetProjectId248(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtProjectId248];
			...
			if (ddl != null)
			{
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(111,"GetDdlProjectId3S1926",true,bAll,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					if (dv.Table.Columns.Contains("MenuOptId"))
					{
						ss = "(MenuOptId is null";
						if (string.IsNullOrEmpty(cAdmMenuOpt111List.SelectedValue)) {ss = ss + ")";} else {ss = ss + " OR MenuOptId = " + cAdmMenuOpt111List.SelectedValue + ")";}
					}
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "ProjectId248 is not NULL"; }
					dv.RowFilter = ss;
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(111,"GetDdlProjectId3S1926",true,true,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtProjectId248] = dv.Table;
				}
			}
		}

		private void SetInputBy248(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtInputBy248];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (ddl != null)
			{
				string ss = string.Empty;
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(111,"GetDdlInputBy3S1924",true,bAll,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					...
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "InputBy248 is not NULL"; }
					dv.RowFilter = ss;
					ddl.DataSource = dv; ddl.DataBind();
					li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					else if (!bFirst)
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(111,"GetDdlInputBy3S1924",true,true,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtInputBy248] = dv.Table;
				}
			}
		}

		private DataView GetAdmMenuOpt111List()
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmMenuOpt111List];
			if (dt == null)
			{
				CheckAuthentication(false);
				int filterId = 0;
				...
				try
				{
					try {dt = (new AdminSystem()).GetLis(111,"GetLisAdmMenuOpt111",true,"Y",0,null,null,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
					catch {dt = (new AdminSystem()).GetLis(111,"GetLisAdmMenuOpt111",true,"N",0,null,null,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				}
		...
		private void PopAdmMenuOpt111List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			DataView dv = GetAdmMenuOpt111List();
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			cAdmMenuOpt111List.AutoCompleteUrl = "AutoComplete.aspx/LisSuggests";
			cAdmMenuOpt111List.DataContext = context;
			if (dv.Table == null) return;
			cAdmMenuOpt111List.DataSource = dv;
			cAdmMenuOpt111List.Visible = true;
			if (cAdmMenuOpt111List.Items.Count <= 0) {cAdmMenuOpt111List.Visible = false; cAdmMenuOpt111List_SelectedIndexChanged(sender,e);}
			else
			{
				if (bPageLoad && !string.IsNullOrEmpty(Request.QueryString["key"]) && bUseCri.Value == string.Empty)
				{
					try {cAdmMenuOpt111List.SelectByValue(Request.QueryString["key"],string.Empty,true);} catch {cAdmMenuOpt111List.Items[0].Selected = true; cAdmMenuOpt111List_SelectedIndexChanged(sender, e);}
				    cScreenSearch.Visible = false; cSystem.Visible = false; cEditButton.Visible = true; cSaveCloseButton.Visible = cSaveButton.Visible;
				}
				else
				{
					if (ID != null) {if (!cAdmMenuOpt111List.SelectByValue(ID,string.Empty,true)) {ID = null;}}
					if (ID == null)
					{
						cAdmMenuOpt111List_SelectedIndexChanged(sender, e);
					}
				}
			}
		}
		...
		private void SetColumnAuthority()
		{
			...
			{
				base.SetFoldBehavior(cMenuOptId248, dtAuth.Rows[0], cMenuOptId248P1, cMenuOptId248Label, cMenuOptId248P2, null, dtLabel.Rows[0], null, null, null);
				base.SetFoldBehavior(cMenuOptName248, dtAuth.Rows[1], cMenuOptName248P1, cMenuOptName248Label, cMenuOptName248P2, null, dtLabel.Rows[1], cRFVMenuOptName248, null, null);
				base.SetFoldBehavior(cMenuOptDesc248, dtAuth.Rows[2], cMenuOptDesc248P1, cMenuOptDesc248Label, cMenuOptDesc248P2, cMenuOptDesc248E, null, dtLabel.Rows[2], null, null, null);
				cMenuOptDesc248E.Attributes["label_id"] = cMenuOptDesc248Label.ClientID; cMenuOptDesc248E.Attributes["target_id"] = cMenuOptDesc248.ClientID;
				base.SetFoldBehavior(cMenuOptIco248, dtAuth.Rows[3], cMenuOptIco248P1, cMenuOptIco248Label, cMenuOptIco248P2, null, dtLabel.Rows[3], null, null, null);
				base.SetFoldBehavior(cMenuOptImg248, dtAuth.Rows[4], cMenuOptImg248P1, cMenuOptImg248Label, cMenuOptImg248P2, null, dtLabel.Rows[4], null, null, null);
				base.SetFoldBehavior(cProjectId248, dtAuth.Rows[5], cProjectId248P1, cProjectId248Label, cProjectId248P2, null, dtLabel.Rows[5], null, null, null);
				base.SetFoldBehavior(cOvaRating248, dtAuth.Rows[6], cOvaRating248P1, cOvaRating248Label, cOvaRating248P2, null, dtLabel.Rows[6], null, null, null);
				base.SetFoldBehavior(cInputBy248, dtAuth.Rows[7], cInputBy248P1, cInputBy248Label, cInputBy248P2, null, dtLabel.Rows[7], null, null, null);
				base.SetFoldBehavior(cInputOn248, dtAuth.Rows[8], cInputOn248P1, cInputOn248Label, cInputOn248P2, null, dtLabel.Rows[8], null, null, null);
				base.SetFoldBehavior(cTopMenuCss248, dtAuth.Rows[9], cTopMenuCss248P1, cTopMenuCss248Label, cTopMenuCss248P2, cTopMenuCss248E, null, dtLabel.Rows[9], null, null, null);
				cTopMenuCss248E.Attributes["label_id"] = cTopMenuCss248Label.ClientID; cTopMenuCss248E.Attributes["target_id"] = cTopMenuCss248.ClientID;
				base.SetFoldBehavior(cSidMenuCss248, dtAuth.Rows[10], cSidMenuCss248P1, cSidMenuCss248Label, cSidMenuCss248P2, cSidMenuCss248E, null, dtLabel.Rows[10], null, null, null);
				cSidMenuCss248E.Attributes["label_id"] = cSidMenuCss248Label.ClientID; cSidMenuCss248E.Attributes["target_id"] = cSidMenuCss248.ClientID;
				base.SetFoldBehavior(cTopMenuJs248, dtAuth.Rows[11], cTopMenuJs248P1, cTopMenuJs248Label, cTopMenuJs248P2, cTopMenuJs248E, null, dtLabel.Rows[11], null, null, null);
				cTopMenuJs248E.Attributes["label_id"] = cTopMenuJs248Label.ClientID; cTopMenuJs248E.Attributes["target_id"] = cTopMenuJs248.ClientID;
				base.SetFoldBehavior(cSidMenuJs248, dtAuth.Rows[12], cSidMenuJs248P1, cSidMenuJs248Label, cSidMenuJs248P2, cSidMenuJs248E, null, dtLabel.Rows[12], null, null, null);
				cSidMenuJs248E.Attributes["label_id"] = cSidMenuJs248Label.ClientID; cSidMenuJs248E.Attributes["target_id"] = cSidMenuJs248.ClientID;
				base.SetFoldBehavior(cTopMenuIvk248, dtAuth.Rows[13], cTopMenuIvk248P1, cTopMenuIvk248Label, cTopMenuIvk248P2, cTopMenuIvk248E, null, dtLabel.Rows[13], null, null, null);
				cTopMenuIvk248E.Attributes["label_id"] = cTopMenuIvk248Label.ClientID; cTopMenuIvk248E.Attributes["target_id"] = cTopMenuIvk248.ClientID;
				base.SetFoldBehavior(cSidMenuIvk248, dtAuth.Rows[14], cSidMenuIvk248P1, cSidMenuIvk248Label, cSidMenuIvk248P2, cSidMenuIvk248E, null, dtLabel.Rows[14], null, null, null);
				cSidMenuIvk248E.Attributes["label_id"] = cSidMenuIvk248Label.ClientID; cSidMenuIvk248E.Attributes["target_id"] = cSidMenuIvk248.ClientID;
			}
			if ((cMenuOptName248.Attributes["OnChange"] == null || cMenuOptName248.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cMenuOptName248.Visible && !cMenuOptName248.ReadOnly) {cMenuOptName248.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cMenuOptDesc248.Attributes["OnChange"] == null || cMenuOptDesc248.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cMenuOptDesc248.Visible && !cMenuOptDesc248.ReadOnly) {cMenuOptDesc248.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cMenuOptIco248.Attributes["OnChange"] == null || cMenuOptIco248.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cMenuOptIco248.Visible && !cMenuOptIco248.ReadOnly) {cMenuOptIco248.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cMenuOptImg248.Attributes["OnChange"] == null || cMenuOptImg248.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cMenuOptImg248.Visible && !cMenuOptImg248.ReadOnly) {cMenuOptImg248.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cProjectId248.Attributes["OnChange"] == null || cProjectId248.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cProjectId248.Visible && cProjectId248.Enabled) {cProjectId248.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cOvaRating248.Attributes["OnChange"] == null || cOvaRating248.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cOvaRating248.Visible && !cOvaRating248.ReadOnly) {cOvaRating248.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cInputBy248.Attributes["OnChange"] == null || cInputBy248.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cInputBy248.Visible && cInputBy248.Enabled) {cInputBy248.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cInputOn248.Attributes["OnChange"] == null || cInputOn248.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cInputOn248.Visible && !cInputOn248.ReadOnly) {cInputOn248.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cTopMenuCss248.Attributes["OnChange"] == null || cTopMenuCss248.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cTopMenuCss248.Visible && !cTopMenuCss248.ReadOnly) {cTopMenuCss248.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cSidMenuCss248.Attributes["OnChange"] == null || cSidMenuCss248.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cSidMenuCss248.Visible && !cSidMenuCss248.ReadOnly) {cSidMenuCss248.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cTopMenuJs248.Attributes["OnChange"] == null || cTopMenuJs248.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cTopMenuJs248.Visible && !cTopMenuJs248.ReadOnly) {cTopMenuJs248.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cSidMenuJs248.Attributes["OnChange"] == null || cSidMenuJs248.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cSidMenuJs248.Visible && !cSidMenuJs248.ReadOnly) {cSidMenuJs248.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cTopMenuIvk248.Attributes["OnChange"] == null || cTopMenuIvk248.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cTopMenuIvk248.Visible && !cTopMenuIvk248.ReadOnly) {cTopMenuIvk248.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cSidMenuIvk248.Attributes["OnChange"] == null || cSidMenuIvk248.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cSidMenuIvk248.Visible && !cSidMenuIvk248.ReadOnly) {cSidMenuIvk248.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
		}
		...
		private void ClearMaster(object sender, System.EventArgs e)
		{
			...
			if (dt.Rows[1]["ColVisible"].ToString() == "Y" && dt.Rows[1]["ColReadOnly"].ToString() != "Y") {cMenuOptName248.Text = string.Empty;}
			if (dt.Rows[2]["ColVisible"].ToString() == "Y" && dt.Rows[2]["ColReadOnly"].ToString() != "Y") {cMenuOptDesc248.Text = string.Empty;}
			if (dt.Rows[3]["ColVisible"].ToString() == "Y" && dt.Rows[3]["ColReadOnly"].ToString() != "Y") {cMenuOptIco248.Text = string.Empty;}
			if (dt.Rows[4]["ColVisible"].ToString() == "Y" && dt.Rows[4]["ColReadOnly"].ToString() != "Y") {cMenuOptImg248.Text = string.Empty;}
			if (dt.Rows[5]["ColVisible"].ToString() == "Y" && dt.Rows[5]["ColReadOnly"].ToString() != "Y") {SetProjectId248(cProjectId248,string.Empty);}
			if (dt.Rows[6]["ColVisible"].ToString() == "Y" && dt.Rows[6]["ColReadOnly"].ToString() != "Y") {cOvaRating248.Text = string.Empty;}
			if (dt.Rows[7]["ColVisible"].ToString() == "Y" && dt.Rows[7]["ColReadOnly"].ToString() != "Y") {SetInputBy248(cInputBy248,string.Empty);}
			if (dt.Rows[8]["ColVisible"].ToString() == "Y" && dt.Rows[8]["ColReadOnly"].ToString() != "Y") {cInputOn248.Text = string.Empty;}
			if (dt.Rows[9]["ColVisible"].ToString() == "Y" && dt.Rows[9]["ColReadOnly"].ToString() != "Y") {cTopMenuCss248.Text = string.Empty;}
			if (dt.Rows[10]["ColVisible"].ToString() == "Y" && dt.Rows[10]["ColReadOnly"].ToString() != "Y") {cSidMenuCss248.Text = string.Empty;}
			if (dt.Rows[11]["ColVisible"].ToString() == "Y" && dt.Rows[11]["ColReadOnly"].ToString() != "Y") {cTopMenuJs248.Text = string.Empty;}
			if (dt.Rows[12]["ColVisible"].ToString() == "Y" && dt.Rows[12]["ColReadOnly"].ToString() != "Y") {cSidMenuJs248.Text = string.Empty;}
			if (dt.Rows[13]["ColVisible"].ToString() == "Y" && dt.Rows[13]["ColReadOnly"].ToString() != "Y") {cTopMenuIvk248.Text = string.Empty;}
			if (dt.Rows[14]["ColVisible"].ToString() == "Y" && dt.Rows[14]["ColReadOnly"].ToString() != "Y") {cSidMenuIvk248.Text = string.Empty;}
			// *** Default Value (Folder) Web Rule starts here *** //
		}

		private void InitMaster(object sender, System.EventArgs e)
		{
			cMenuOptId248.Text = string.Empty;
			cMenuOptName248.Text = string.Empty;
			cMenuOptDesc248.Text = string.Empty;
			cMenuOptIco248.Text = string.Empty;
			cMenuOptImg248.Text = string.Empty;
			SetProjectId248(cProjectId248,string.Empty);
			cOvaRating248.Text = string.Empty;
			SetInputBy248(cInputBy248,string.Empty);
			cInputOn248.Text = string.Empty;
			cTopMenuCss248.Text = string.Empty;
			cSidMenuCss248.Text = string.Empty;
			cTopMenuJs248.Text = string.Empty;
			cSidMenuJs248.Text = string.Empty;
			cTopMenuIvk248.Text = string.Empty;
			cSidMenuIvk248.Text = string.Empty;
			// *** Default Value (Folder) Web Rule starts here *** //
		}
		protected void cAdmMenuOpt111List_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmMenuOpt111List_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmMenuOpt111List_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			InitMaster(sender, e);      // Do this first to enable defaults for non-database columns.
			DataTable dt = null;
			try
			{
				dt = (new AdminSystem()).GetMstById("GetAdmMenuOpt111ById",cAdmMenuOpt111List.SelectedValue,null,null);
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				if (dt.Rows.Count > 0)
				{
					cAdmMenuOpt111List.SetDdVisible();
					DataRow dr = dt.Rows[0];
					try {cMenuOptId248.Text = RO.Common3.Utils.fmNumeric("0",dr["MenuOptId248"].ToString(),base.LUser.Culture);} catch {cMenuOptId248.Text = string.Empty;}
					try {cMenuOptName248.Text = dr["MenuOptName248"].ToString();} catch {cMenuOptName248.Text = string.Empty;}
					try {cMenuOptDesc248.Text = dr["MenuOptDesc248"].ToString();} catch {cMenuOptDesc248.Text = string.Empty;}
					try {cMenuOptIco248.Text = dr["MenuOptIco248"].ToString();} catch {cMenuOptIco248.Text = string.Empty;}
					try {cMenuOptImg248.Text = dr["MenuOptImg248"].ToString();} catch {cMenuOptImg248.Text = string.Empty;}
					SetProjectId248(cProjectId248,dr["ProjectId248"].ToString());
					try {cOvaRating248.Text = RO.Common3.Utils.fmNumeric("0",dr["OvaRating248"].ToString(),base.LUser.Culture);} catch {cOvaRating248.Text = string.Empty;}
					SetInputBy248(cInputBy248,dr["InputBy248"].ToString());
					try {cInputOn248.Text = RO.Common3.Utils.fmShortDateTimeUTC(dr["InputOn248"].ToString(),base.LUser.Culture,CurrTimeZoneInfo());} catch {cInputOn248.Text = string.Empty;}
					try {cTopMenuCss248.Text = dr["TopMenuCss248"].ToString();} catch {cTopMenuCss248.Text = string.Empty;}
					try {cSidMenuCss248.Text = dr["SidMenuCss248"].ToString();} catch {cSidMenuCss248.Text = string.Empty;}
					try {cTopMenuJs248.Text = dr["TopMenuJs248"].ToString();} catch {cTopMenuJs248.Text = string.Empty;}
					try {cSidMenuJs248.Text = dr["SidMenuJs248"].ToString();} catch {cSidMenuJs248.Text = string.Empty;}
					try {cTopMenuIvk248.Text = dr["TopMenuIvk248"].ToString();} catch {cTopMenuIvk248.Text = string.Empty;}
					try {cSidMenuIvk248.Text = dr["SidMenuIvk248"].ToString();} catch {cSidMenuIvk248.Text = string.Empty;}
				}
			}
			cButPanel.DataBind();
			ScriptManager.GetCurrent(Parent.Page).SetFocus(cAdmMenuOpt111List.FocusID);
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cMenuOptId248.Text == string.Empty) { cClearButton_Click(sender, new EventArgs()); ShowDirty(false); PanelTop.Update();}
			else { PopAdmMenuOpt111List(sender, e, false, cMenuOptId248.Text); }
			// *** System Button Click (After) Web Rule starts here *** //
		}
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmMenuOpt111List); PopAdmMenuOpt111List(sender, e, false, null);
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			if (ValidPage())
			{
				AdmMenuOpt111 ds = PrepAdmMenuOptData(null,cMenuOptId248.Text == string.Empty);
				if (!string.IsNullOrEmpty(cAdmMenuOpt111List.SelectedValue) && cMenuOptId248.Text != cAdmMenuOpt111List.SelectedValue)
				{
					PreMsgPopup("Primary key cannot be changed");
					return rtn;
				}
				if (string.IsNullOrEmpty(cAdmMenuOpt111List.SelectedValue))	// Add
				{
					if (ds != null)
					{
						pid = (new AdminSystem()).AddData(111,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc,noTrans,commandTimeOut);
					}
					if (!string.IsNullOrEmpty(pid))
					{
						cAdmMenuOpt111List.ClearSearch(); Session.Remove(KEY_dtAdmMenuOpt111List);
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmMenuOpt111List(sender, e, false, pid);
						rtn = GetScreenHlp().Rows[0]["AddMsg"].ToString();
					}
				}
				else {
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(111,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc,noTrans,commandTimeOut)) {bValid7 = true;}
					if (bValid7)
					{
						cAdmMenuOpt111List.ClearSearch(); Session.Remove(KEY_dtAdmMenuOpt111List);
						ShowDirty(false); PopAdmMenuOpt111List(sender, e, false, ds.Tables["AdmMenuOpt"].Rows[0]["MenuOptId248"]);
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cMenuOptId248.Text != string.Empty)
			{
				AdmMenuOpt111 ds = PrepAdmMenuOptData(null,false);
				try
				{
					if (ds != null && (new AdminSystem()).DelData(111,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc))
					{
						cAdmMenuOpt111List.ClearSearch(); Session.Remove(KEY_dtAdmMenuOpt111List);
						ShowDirty(false); PopAdmMenuOpt111List(sender, e, false, null);
		...
		private AdmMenuOpt111 PrepAdmMenuOptData(DataView dv, bool bAdd)
		{
			...
			if (bAdd) { dr["MenuOptId248"] = string.Empty; } else { dr["MenuOptId248"] = cMenuOptId248.Text; }
			drType["MenuOptId248"] = "Numeric"; drDisp["MenuOptId248"] = "TextBox";
			try {dr["MenuOptName248"] = cMenuOptName248.Text.Trim();} catch {}
			drType["MenuOptName248"] = "VarWChar"; drDisp["MenuOptName248"] = "TextBox";
			try {dr["MenuOptDesc248"] = cMenuOptDesc248.Text;} catch {}
			drType["MenuOptDesc248"] = "VarWChar"; drDisp["MenuOptDesc248"] = "MultiLine";
			try {dr["MenuOptIco248"] = cMenuOptIco248.Text.Trim();} catch {}
			drType["MenuOptIco248"] = "VarWChar"; drDisp["MenuOptIco248"] = "TextBox";
			try {dr["MenuOptImg248"] = cMenuOptImg248.Text.Trim();} catch {}
			drType["MenuOptImg248"] = "VarWChar"; drDisp["MenuOptImg248"] = "TextBox";
			try {dr["ProjectId248"] = cProjectId248.SelectedValue;} catch {}
			drType["ProjectId248"] = "Numeric"; drDisp["ProjectId248"] = "DropDownList";
			try {dr["OvaRating248"] = cOvaRating248.Text;} catch {}
			drType["OvaRating248"] = "Numeric"; drDisp["OvaRating248"] = "StarRating";
			try { dr["InputBy248"] = cInputBy248.SelectedValue == string.Empty && bAdd ? base.LUser.UsrId.ToString().ToString(): cInputBy248.SelectedValue;} catch {};
			drType["InputBy248"] = "Numeric"; drDisp["InputBy248"] = "DropDownList";
			try {dr["InputOn248"] = base.ToIntDateTime(cInputOn248.Text,true,true);} catch {}
			drType["InputOn248"] = "DBTimeStamp"; drDisp["InputOn248"] = "ShortDateTimeUTC";
			try {dr["TopMenuCss248"] = cTopMenuCss248.Text;} catch {}
			drType["TopMenuCss248"] = "VarChar"; drDisp["TopMenuCss248"] = "MultiLine";
			try {dr["SidMenuCss248"] = cSidMenuCss248.Text;} catch {}
			drType["SidMenuCss248"] = "VarChar"; drDisp["SidMenuCss248"] = "MultiLine";
			try {dr["TopMenuJs248"] = cTopMenuJs248.Text;} catch {}
			drType["TopMenuJs248"] = "VarChar"; drDisp["TopMenuJs248"] = "MultiLine";
			try {dr["SidMenuJs248"] = cSidMenuJs248.Text;} catch {}
			drType["SidMenuJs248"] = "VarChar"; drDisp["SidMenuJs248"] = "MultiLine";
			try {dr["TopMenuIvk248"] = cTopMenuIvk248.Text;} catch {}
			drType["TopMenuIvk248"] = "VarChar"; drDisp["TopMenuIvk248"] = "MultiLine";
			try {dr["SidMenuIvk248"] = cSidMenuIvk248.Text;} catch {}
		...
		public bool CanAct(char typ) { return CanAct(typ, GetAuthRow(), cAdmMenuOpt111List.SelectedValue.ToString()); }
		...
		private void PreMsgPopup(string msg) { PreMsgPopup(msg,cAdmMenuOpt111List,null); }

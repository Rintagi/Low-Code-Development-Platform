<%@ Control Language="c#" Inherits="RO.Web.AdmMenuHlpModule" CodeFile="AdmMenuHlpModule.ascx.cs" CodeFileBaseClass="RO.Web.ModuleBase" %>
        ...
        <div id="cScreenSearch" class="screen-search" runat="server">
            <div class="r-table">
                <div class="r-tr">
                    ...
                    <div class="r-td r-content"><rcasp:ComboBox autocomplete="off" id="cAdmMenuHlp36List" CssClass="inp-ddl" runat="server" AutoPostBack="true" OnSelectedIndexChanged="cAdmMenuHlp36List_SelectedIndexChanged" OnTextChanged="cAdmMenuHlp36List_TextChanged" OnDDFindClick="cAdmMenuHlp36List_DDFindClick" Mode="A" OnPostBack="cbPostBack" DataValueField="MenuId39" DataTextField="MenuId39Text" /></div>
                </div>
            </div>
        </div>
    </div>
...
<div id="GridDiv" class="grid-container rg-1-8">
...
<div class="ListView">
	<asp:ListView id="cAdmMenuHlpGrid" DataKeyNames="MenuHlpId40" OnItemCommand="cAdmMenuHlpGrid_OnItemCommand" OnSorting="cAdmMenuHlpGrid_OnSorting" OnPreRender="cAdmMenuHlpGrid_OnPreRender" OnPagePropertiesChanging="cAdmMenuHlpGrid_OnPagePropertiesChanging" OnItemEditing="cAdmMenuHlpGrid_OnItemEditing" OnItemCanceling="cAdmMenuHlpGrid_OnItemCanceling" OnItemDeleting="cAdmMenuHlpGrid_OnItemDeleting" OnItemDataBound="cAdmMenuHlpGrid_OnItemDataBound" OnLayoutCreated="cAdmMenuHlpGrid_OnLayoutCreated" OnItemUpdating="cAdmMenuHlpGrid_OnItemUpdating" runat="server">
	<LayoutTemplate>
	<table cellpadding="0" cellspacing="0" border="0" width="100%">
	<tr id="cAdmMenuHlpGridHeader" class="GrdHead" visible="true" runat="server">
    <td>
		<div class='GrdOuter' runat="server"><div class='GrdInner' style='max-width:100px;text-align:right;' runat="server">
			<asp:LinkButton id="cMenuHlpId40hl" CssClass="GrdHead" onClick="cMenuHlpId40hl_Click" runat="server" /><asp:Image id="cMenuHlpId40hi" runat="server" />
		</div></div>
    </td>
    <td>
		<div class='GrdOuter' runat="server"><div class='GrdInner' style='max-width:300px;text-align:left;' runat="server">
			<asp:LinkButton id="cCultureId40hl" CssClass="GrdHead" onClick="cCultureId40hl_Click" runat="server" /><asp:Image id="cCultureId40hi" runat="server" />
		</div></div>
    </td>
    <td>
		<div class='GrdOuter' runat="server"><div class='GrdInner' style='text-align:left;' runat="server">
			<asp:LinkButton id="cMenuText40hl" CssClass="GrdHead" onClick="cMenuText40hl_Click" runat="server" /><asp:Image id="cMenuText40hi" runat="server" />
		</div></div>
    </td>
    <td>&nbsp;</td>
	</tr>
	<tr id="itemPlaceholder" runat="server"></tr>
	<tr id="cAdmMenuHlpGridFooter" class="GrdFoot" visible="false" runat="server">
    <td>
		<div class='GrdOuter' runat="server"><div class='GrdInner' style='max-width:100px;text-align:right;' runat="server">
		    <asp:Label id="cMenuHlpId40fl" class='GrdFoot' runat="server" />
		</div></div>
    </td>
    <td>
		<div class='GrdOuter' runat="server"><div class='GrdInner' style='max-width:300px;text-align:left;' runat="server">
		    <asp:Label id="cCultureId40fl" class='GrdFoot' runat="server" />
		</div></div>
    </td>
    <td>
		<div class='GrdOuter' runat="server"><div class='GrdInner' style='text-align:left;' runat="server">
		    <asp:Label id="cMenuText40fl" class='GrdFoot' runat="server" />
		</div></div>
    </td>
    <td>&nbsp;</td>
	</tr></table></LayoutTemplate>
	<ItemTemplate>
	<tr id="cAdmMenuHlpGridRow" class='<%# Container.DisplayIndex % 2 == 0 ? "GrdItm" : "GrdAlt" %>' runat="server">
    <td>
		<div class='GrdOuter' runat="server"><div class='GrdInner' style='max-width:100px;text-align:right;' visible="<%# GridColumnVisible(1) %>" onclick='GridEdit("MenuHlpId40")' runat="server">
			<asp:Label id="cMenuHlpId40l" Text='<%# RO.Common3.Utils.fmNumeric("0",DataBinder.Eval(Container.DataItem,"MenuHlpId40").ToString(),base.LUser.Culture) %>' CssClass="GrdTxtLb" runat="server" />
		</div></div>
    </td>
    <td>
		<div class='GrdOuter' runat="server"><div class='GrdInner' style='max-width:300px;text-align:left;' visible="<%# GridColumnVisible(2) %>" onclick='GridEdit("CultureId40")' runat="server">
			<asp:Label Text='<%# DataBinder.Eval(Container.DataItem,"CultureId40").ToString() %>' Visible="false" runat="server" />
			<asp:Label id="cCultureId40l" text='<%# DataBinder.Eval(Container.DataItem,"CultureId40Text") %>' CssClass="GrdTxtLb" runat="server" />
		</div></div>
    </td>
    <td>
		<div class='GrdOuter' runat="server"><div class='GrdInner' style='text-align:left;' visible="<%# GridColumnVisible(3) %>" onclick='GridEdit("MenuText40")' runat="server">
			<asp:Label id="cMenuText40l" Text='<%# DataBinder.Eval(Container.DataItem,"MenuText40").ToString().Replace("\r\n","<br />").Replace("\r","<br />").Replace("\n","<br />").Replace("  ",HtmlSpace()) %>' CssClass="GrdTxtLb" runat="server" />
		</div></div>
    </td>
	<td>
       <asp:LinkButton ID="cAdmMenuHlpGridEdit" style="display:none;" CausesValidation="true" CommandName="Edit" runat="server" />
       <asp:LinkButton ID="cAdmMenuHlpGridDelete" CssClass="GrdDel" tooltip="DELETE" CommandName="Delete" onclientclick='GridDelete()' runat="server" />
	</td>
	</tr>
	</ItemTemplate>
	<EditItemTemplate>
    ...
    <td>
		<div class='GrdOuter' runat="server"><div class='GrdInner' style='max-width:100px;text-align:right;' visible="<%# GridColumnVisible(1) %>" runat="server"><div class="GrdEdtLabelText"><asp:label id="cMenuHlpId40ml" runat="server" /></div>
		    <asp:TextBox id="cMenuHlpId40" CssClass="GrdNum" Text='<%# RO.Common3.Utils.fmNumeric("0",DataBinder.Eval(Container.DataItem,"MenuHlpId40").ToString(),base.LUser.Culture) %>' runat="server" />
		</div></div>
    </td>
    <td>
		<div class='GrdOuter' runat="server"><div class='GrdInner' style='max-width:300px;text-align:left;' visible="<%# GridColumnVisible(2) %>" runat="server"><div class="GrdEdtLabelText"><asp:label id="cCultureId40ml" runat="server" /></div>
		    <rcasp:ComboBox autocomplete="off" id="cCultureId40" CssClass="GrdDdl" DataValueField="CultureId40" DataTextField="CultureId40Text" Mode="A" OnPostBack="cbPostBack" OnSearch="cbCultureId40" runat="server" /><asp:RequiredFieldValidator id="cRFVCultureId40" ControlToValidate="cCultureId40" display="none" runat="server" />
		</div></div>
    </td>
    <td>
		<div class='GrdOuter' runat="server"><div class='GrdInner' style='text-align:left;' visible="<%# GridColumnVisible(3) %>" runat="server"><div class="GrdEdtLabelText"><asp:label id="cMenuText40ml" runat="server" /></div>
		    <asp:TextBox id="cMenuText40" CssClass="GrdTxt" Text='<%# DataBinder.Eval(Container.DataItem,"MenuText40").ToString() %>' MaxLength="50" runat="server" /><asp:RequiredFieldValidator id="cRFVMenuText40" ControlToValidate="cMenuText40" display="none" runat="server" />
		</div></div>
    </td>
	<td>
       <asp:LinkButton ID="cAdmMenuHlpGridCancel" CssClass="GrdCan" tooltip="CANCEL" OnClientClick="GridCancel();" CausesValidation="true" CommandName="Cancel" runat="server" />
       <asp:LinkButton ID="cAdmMenuHlpGridUpdate" style="display:none;" CommandName="Update" runat="server" />
	</td>
	</tr>
	</EditItemTemplate>
	...
	<asp:DataPager ID="cAdmMenuHlpGridDataPager" runat="server" Visible="false" PagedControlID="cAdmMenuHlpGrid"></asp:DataPager>
</div></div></div>
</asp:Panel>

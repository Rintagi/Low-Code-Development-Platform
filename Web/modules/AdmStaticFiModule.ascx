<%@ Control Language="c#" Inherits="RO.Web.AdmStaticFiModule" CodeFile="AdmStaticFiModule.ascx.cs" CodeFileBaseClass="RO.Web.ModuleBase" %>
...
<div class="ListView">
	<asp:ListView id="cAdmStaticFiGrid" DataKeyNames="StaticFiId262" OnItemCommand="cAdmStaticFiGrid_OnItemCommand" OnSorting="cAdmStaticFiGrid_OnSorting" OnPreRender="cAdmStaticFiGrid_OnPreRender" OnPagePropertiesChanging="cAdmStaticFiGrid_OnPagePropertiesChanging" OnItemEditing="cAdmStaticFiGrid_OnItemEditing" OnItemCanceling="cAdmStaticFiGrid_OnItemCanceling" OnItemDeleting="cAdmStaticFiGrid_OnItemDeleting" OnItemDataBound="cAdmStaticFiGrid_OnItemDataBound" OnLayoutCreated="cAdmStaticFiGrid_OnLayoutCreated" OnItemUpdating="cAdmStaticFiGrid_OnItemUpdating" runat="server">
	<LayoutTemplate>
	<table cellpadding="0" cellspacing="0" border="0" width="100%">
	<tr id="cAdmStaticFiGridHeader" class="GrdHead" visible="true" runat="server">
    <td>
		<div class='GrdOuter' runat="server"><div class='GrdInner' style='max-width:80px;text-align:right;' runat="server">
			<asp:LinkButton id="cStaticFiId262hl" CssClass="GrdHead" onClick="cStaticFiId262hl_Click" runat="server" /><asp:Image id="cStaticFiId262hi" runat="server" />
		</div></div>
    </td>
    <td>
		<div class='GrdOuter' runat="server"><div class='GrdInner' style='max-width:350px;text-align:left;' runat="server">
			<asp:LinkButton id="cStaticFiUrl262hl" CssClass="GrdHead" onClick="cStaticFiUrl262hl_Click" runat="server" /><asp:Image id="cStaticFiUrl262hi" runat="server" />
		</div></div>
    </td>
    <td>&nbsp;</td>
	</tr>
	<tr id="itemPlaceholder" runat="server"></tr>
	<tr id="cAdmStaticFiGridFooter" class="GrdFoot" visible="false" runat="server">
    <td>
		<div class='GrdOuter' runat="server"><div class='GrdInner' style='max-width:80px;text-align:right;' runat="server">
		    <asp:Label id="cStaticFiId262fl" class='GrdFoot' runat="server" />
		</div></div>
    </td>
    <td>
		<div class='GrdOuter' runat="server"><div class='GrdInner' style='max-width:350px;text-align:left;' runat="server">
		    <asp:Label id="cStaticFiUrl262fl" class='GrdFoot' runat="server" />
		</div></div>
    </td>
    <td>&nbsp;</td>
	</tr></table></LayoutTemplate>
	<ItemTemplate>
	<tr id="cAdmStaticFiGridRow" class='<%# Container.DisplayIndex % 2 == 0 ? "GrdItm" : "GrdAlt" %>' runat="server">
    <td>
		<div class='GrdOuter' runat="server"><div class='GrdInner' style='max-width:80px;text-align:right;' visible="<%# GridColumnVisible(0) %>" onclick='GridEdit("StaticFiId262")' runat="server">
			<asp:Label id="cStaticFiId262l" Text='<%# RO.Common3.Utils.fmNumeric("0",DataBinder.Eval(Container.DataItem,"StaticFiId262").ToString(),base.LUser.Culture) %>' CssClass="GrdTxtLb" runat="server" />
		</div></div>
    </td>
    <td>
		<div class='GrdOuter' runat="server"><div class='GrdInner' style='max-width:350px;text-align:left;' visible="<%# GridColumnVisible(1) %>" onclick='GridEdit("StaticFiUrl262")' runat="server">
			<asp:Label id="cStaticFiUrl262l" Text='<%# DataBinder.Eval(Container.DataItem,"StaticFiUrl262").ToString().Replace("\r\n","<br />").Replace("\r","<br />").Replace("\n","<br />").Replace("  ",HtmlSpace()) %>' CssClass="GrdTxtLb" runat="server" />
		</div></div>
    </td>
	<td>
       <asp:LinkButton ID="cAdmStaticFiGridEdit" style="display:none;" CausesValidation="true" CommandName="Edit" runat="server" />
       <asp:LinkButton ID="cAdmStaticFiGridDelete" CssClass="GrdDel" tooltip="DELETE" CommandName="Delete" onclientclick='GridDelete()' runat="server" />
	</td>
	</tr>
	</ItemTemplate>
	<EditItemTemplate>
    ...
    <td>
		<div class='GrdOuter' runat="server"><div class='GrdInner' style='max-width:80px;text-align:right;' visible="<%# GridColumnVisible(0) %>" runat="server"><div class="GrdEdtLabelText"><asp:label id="cStaticFiId262ml" runat="server" /></div>
		    <asp:TextBox id="cStaticFiId262" CssClass="GrdNum" Text='<%# RO.Common3.Utils.fmNumeric("0",DataBinder.Eval(Container.DataItem,"StaticFiId262").ToString(),base.LUser.Culture) %>' runat="server" />
		</div></div>
    </td>
    <td>
		<div class='GrdOuter' runat="server"><div class='GrdInner' style='max-width:350px;text-align:left;' visible="<%# GridColumnVisible(1) %>" runat="server"><div class="GrdEdtLabelText"><asp:label id="cStaticFiUrl262ml" runat="server" /></div>
		<table cellspacing="0" cellpadding="0"><tr>
		    <td><asp:Panel id="cStaticFiUrl262Pan" CssClass="DocPanel" Visible="false" runat="server">
		    <table width="100%"><tr>
		    <td style="display:none;" ><asp:Button id="cStaticFiUrl262Upl" CssClass="small blue button DocUpload" OnClientClick="GridUpload(); return false;" text="Upload" runat="server" /></td>
		    <td><asp:FileUpload id="cStaticFiUrl262Fi" runat="server"  onchange="AutoUpload(this,event);" /></td>
		    </tr></table>
		    </asp:Panel>
		    <asp:TextBox id="cStaticFiUrl262" CssClass="GrdTxt" MaxLength="100" Text='<%# DataBinder.Eval(Container.DataItem,"StaticFiUrl262").ToString() %>' AutoPostBack="true" runat="server" /></td><td><asp:imagebutton id="cStaticFiUrl262Tgo" OnClientClick='NoConfirm()' onclick="cStaticFiUrl262Tgo_Click" runat="server" ImageUrl="~/Images/UpLoad.png" CausesValidation="true" /></td>
		</tr></table>
		</div></div>
    </td>
	<td>
       <asp:LinkButton ID="cAdmStaticFiGridCancel" CssClass="GrdCan" tooltip="CANCEL" OnClientClick="GridCancel();" CausesValidation="true" CommandName="Cancel" runat="server" />
       <asp:LinkButton ID="cAdmStaticFiGridUpdate" style="display:none;" CommandName="Update" runat="server" />
	</td>
	</tr>
	</EditItemTemplate>
	...
	<asp:DataPager ID="cAdmStaticFiGridDataPager" runat="server" Visible="false" PagedControlID="cAdmStaticFiGrid"></asp:DataPager>
</div></div></div>

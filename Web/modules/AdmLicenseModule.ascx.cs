...
namespace RO.Web
{
	public partial class AdmLicenseModule : RO.Web.ModuleBase
	{
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_1022";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_1022";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_1022";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_1022";
		private const string KEY_dtLabel = "Cache:dtLabel3_1022";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_1022";
		private const string KEY_dtScrCri = "Cache:dtScrCri3_1022";
		private const string KEY_dsScrCriVal = "Cache:dsScrCriVal3_1022";


		private const string KEY_dtSystems = "Cache:dtSystems3_1022";
		private const string KEY_sysConnectionString = "Cache:sysConnectionString3_1022";
		private const string KEY_dtAdmLicense1022List = "Cache:dtAdmLicense1022List";
		private const string KEY_bNewVisible = "Cache:bNewVisible3_1022";
		private const string KEY_bNewSaveVisible = "Cache:bNewSaveVisible3_1022";
		private const string KEY_bCopyVisible = "Cache:bCopyVisible3_1022";
		private const string KEY_bCopySaveVisible = "Cache:bCopySaveVisible3_1022";
		private const string KEY_bDeleteVisible = "Cache:bDeleteVisible3_1022";
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_1022";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_1022";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_1022";
		...
		//WebRule: ShowLicenseDetail
        private void ShowLicenseDetail(string LicenseServer, string licenseString)
        {
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				DataTable dtMenuAccess = (new MenuSystem()).GetMenu(base.LUser.CultureId, 3, base.LImpr, LcSysConnString, LcAppPw,1022, null, null);
				...
				Session.Remove(KEY_dtAdmLicense1022List);
				...
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 1022, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmLicense1022List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=1022&typ=N&sys=3','','',''); return false;"; }
				//WebRule: modify button behavior
		...
		private void SetSystem(byte SystemId)
		{
			...
			try
			{
				...
				if ((Config.DeployType == "DEV" || row["dbAppDatabase"].ToString() == base.CPrj.EntityCode + "View") && !(base.CPrj.EntityCode != "RO" && row["SysProgram"].ToString() == "Y") && (new AdminSystem()).IsRegenNeeded(string.Empty,1022,0,0,LcSysConnString,LcAppPw))
				{
					(new GenScreensSystem()).CreateProgram(1022, "Rintagi Installation", row["dbAppDatabase"].ToString(), base.CPrj, base.CSrc, base.CTar, LcAppConnString, LcAppPw);
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(1022,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(1022,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(1022,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(1022,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,1022,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(1022,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(1022,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(1022,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		protected DataTable GetAuthRow()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthRow(1022,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				try
				{
					try {dv = new DataView((new AdminSystem()).GetExp(1022,"GetExpAdmLicense1022","Y",null,null,filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
					catch {dv = new DataView((new AdminSystem()).GetExp(1022,"GetExpAdmLicense1022","N",null,null,filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(1022,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmLicense.csv"; Session["ExportStr"] = (Config.ExportExcelCSV ? "sep=\t\n": "") + sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmLicense.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 1022, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			...
			UpdCriteria(true);
			cAdmLicense1022List.ClearSearch(); Session.Remove(KEY_dtAdmLicense1022List);
			PopAdmLicense1022List(sender, e, false, null);
		}
			...
			try
			{
				foreach (DataRowView drv in dvCri)
				{
					...
					if (drv["DisplayName"].ToString() == "ComboBox")
					{
						...
						(new AdminSystem()).MkGetScreenIn("1022", drv["ScreenCriId"].ToString(), "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), LcAppDb, LcDesDb, drv["MultiDesignDb"].ToString(), LcSysConnString, LcAppPw);
						DataView dv = new DataView((new AdminSystem()).GetScreenIn("1022", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), (new AdminSystem()).CountScrCri(drv["ScreenCriId"].ToString(), drv["MultiDesignDb"].ToString(), drv["MultiDesignDb"].ToString() == "N" ? LcSysConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)), drv["RequiredValid"].ToString(), 0, string.Empty, drv["DisplayMode"].ToString() != "AutoListBox", val, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)));
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("1022", LcSysConnString, LcAppPw);
				}
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmLicense1022" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmLicense1022" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			foreach (DataRowView drv in dvCri)
			{
			    if (drv["DisplayName"].ToString() == "ListBox")
			    {
					...
					if (cListBox != null)
					{
						...
						int TotalChoiceCnt = new DataView((new AdminSystem()).GetScreenIn("1022", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), CriCnt, drv["RequiredValid"].ToString(), 0, string.Empty, true, string.Empty, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId))).Count;
						string selectedValues = string.Join(",", cListBox.Items.Cast<ListItem>().Where(x => x.Selected).Select(x => "'" + x.Value + "'").ToArray());
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("1022", "AdmLicense1022", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		private DataView GetAdmLicense1022List()
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmLicense1022List];
			if (dt == null)
			{
				CheckAuthentication(false);
				int filterId = 0;
				...
				try
				{
					try {dt = (new AdminSystem()).GetLis(1022,"GetLisAdmLicense1022",true,"Y",0,null,null,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
					catch {dt = (new AdminSystem()).GetLis(1022,"GetLisAdmLicense1022",true,"N",0,null,null,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				}
		...
		private void PopAdmLicense1022List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			DataView dv = GetAdmLicense1022List();
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			cAdmLicense1022List.AutoCompleteUrl = "AutoComplete.aspx/LisSuggests";
			cAdmLicense1022List.DataContext = context;
			if (dv.Table == null) return;
			cAdmLicense1022List.DataSource = dv;
			cAdmLicense1022List.Visible = true;
			if (cAdmLicense1022List.Items.Count <= 0) {cAdmLicense1022List.Visible = false; cAdmLicense1022List_SelectedIndexChanged(sender,e);}
			else
			{
				if (bPageLoad && !string.IsNullOrEmpty(Request.QueryString["key"]) && bUseCri.Value == string.Empty)
				{
					try {cAdmLicense1022List.SelectByValue(Request.QueryString["key"],string.Empty,true);} catch {cAdmLicense1022List.Items[0].Selected = true; cAdmLicense1022List_SelectedIndexChanged(sender, e);}
				    cScreenSearch.Visible = false; cSystem.Visible = false; cEditButton.Visible = true; cSaveCloseButton.Visible = cSaveButton.Visible;
				}
				else
				{
					if (ID != null) {if (!cAdmLicense1022List.SelectByValue(ID,string.Empty,true)) {ID = null;}}
					if (ID == null)
					{
						cAdmLicense1022List_SelectedIndexChanged(sender, e);
					}
				}
			}
		}
		...
		private void SetColumnAuthority()
		{
			...
			if ((cFeatureIncluded.Attributes["OnChange"] == null || cFeatureIncluded.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cFeatureIncluded.Visible && !cFeatureIncluded.ReadOnly) {cFeatureIncluded.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cFeatureExcluded.Attributes["OnChange"] == null || cFeatureExcluded.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cFeatureExcluded.Visible && !cFeatureExcluded.ReadOnly) {cFeatureExcluded.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cCompanyCount.Attributes["OnChange"] == null || cCompanyCount.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cCompanyCount.Visible && !cCompanyCount.ReadOnly) {cCompanyCount.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cProjectCount.Attributes["OnChange"] == null || cProjectCount.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cProjectCount.Visible && !cProjectCount.ReadOnly) {cProjectCount.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cUserCount.Attributes["OnChange"] == null || cUserCount.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cUserCount.Visible && !cUserCount.ReadOnly) {cUserCount.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cLicenseServerUrl.Attributes["OnChange"] == null || cLicenseServerUrl.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cLicenseServerUrl.Visible && !cLicenseServerUrl.ReadOnly) {cLicenseServerUrl.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cLicenseString.Attributes["OnChange"] == null || cLicenseString.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cLicenseString.Visible && !cLicenseString.ReadOnly) {cLicenseString.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
		}
		...
		private void InitMaster(object sender, System.EventArgs e)
		{
			...
			cFeatureIncluded.Text = string.Empty;
			cFeatureExcluded.Text = string.Empty;
			cCompanyCount.Text = string.Empty;
			cProjectCount.Text = string.Empty;
			cUserCount.Text = string.Empty;
			cLicenseServerUrl.Text = string.Empty;
			cLicenseString.Text = string.Empty;
			// *** Default Value (Folder) Web Rule starts here *** //
		}
		protected void cAdmLicense1022List_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmLicense1022List_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmLicense1022List_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			InitMaster(sender, e);      // Do this first to enable defaults for non-database columns.
			DataTable dt = null;
			try
			{
				dt = (new AdminSystem()).GetMstById("GetAdmLicense1022ById",cAdmLicense1022List.SelectedValue,null,null);
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				if (dt.Rows.Count > 0)
				{
					cAdmLicense1022List.SetDdVisible();
					...
					if (string.IsNullOrEmpty("http://www.rintagi.com")) {cRegisterInsall.Visible = false;}
					else
					{
						cRegisterInsall.Visible = true;
						cRegisterInsall.Style.Value = "cursor:pointer;"; cRegisterInsall.Attributes.Add("OnClick","SearchLink('http://www.rintagi.com','','',''); return stopEvent(this,event);");
					    try {cRegisterInsall.Text = "http://www.rintagi.com";} catch {cRegisterInsall.Text = string.Empty;}
					}
				}
			}
			cButPanel.DataBind();
		...
		protected void cRenewLicense_Click(object sender, System.EventArgs e)
		{
			//WebRule: UpdateLicense
            if (!string.IsNullOrEmpty(cLicenseString.Text))
            {
		...
		public void cUndoAllButton_Click(object sender, System.EventArgs e)
		{
			...
			else { PopAdmLicense1022List(sender, e, false, cSystemId1317.Text); }
			// *** System Button Click (After) Web Rule starts here *** //
		}
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmLicense1022List); PopAdmLicense1022List(sender, e, false, null);
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			if (ValidPage())
			{
				AdmLicense1022 ds = PrepAdmLicenseData(null,cSystemId1317.Text == string.Empty);
				if (!string.IsNullOrEmpty(cAdmLicense1022List.SelectedValue) && cSystemId1317.Text != cAdmLicense1022List.SelectedValue)
				{
					PreMsgPopup("Primary key cannot be changed");
					return rtn;
				}
				if (string.IsNullOrEmpty(cAdmLicense1022List.SelectedValue))	// Add
				{
					if (ds != null)
					{
						pid = (new AdminSystem()).AddData(1022,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc,noTrans,commandTimeOut);
					}
					if (!string.IsNullOrEmpty(pid))
					{
						cAdmLicense1022List.ClearSearch(); Session.Remove(KEY_dtAdmLicense1022List);
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmLicense1022List(sender, e, false, pid);
						rtn = GetScreenHlp().Rows[0]["AddMsg"].ToString();
					}
				}
				else {
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(1022,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc,noTrans,commandTimeOut)) {bValid7 = true;}
					if (bValid7)
					{
						cAdmLicense1022List.ClearSearch(); Session.Remove(KEY_dtAdmLicense1022List);
						ShowDirty(false); PopAdmLicense1022List(sender, e, false, ds.Tables["AdmLicense"].Rows[0]["SystemId1317"]);
		...
		public void cDeleteButton_Click(object sender, System.EventArgs e)
		{
			...
			{
				AdmLicense1022 ds = PrepAdmLicenseData(null,false);
				try
				{
					if (ds != null && (new AdminSystem()).DelData(1022,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc))
					{
						cAdmLicense1022List.ClearSearch(); Session.Remove(KEY_dtAdmLicense1022List);
						ShowDirty(false); PopAdmLicense1022List(sender, e, false, null);
		...
		private AdmLicense1022 PrepAdmLicenseData(DataView dv, bool bAdd)
		{
			...
			try {dr["InstallID"] = cInstallID.Text.Trim();} catch {}
		...
		public bool CanAct(char typ) { return CanAct(typ, GetAuthRow(), cAdmLicense1022List.SelectedValue.ToString()); }
		...
		private void PreMsgPopup(string msg) { PreMsgPopup(msg,cAdmLicense1022List,null); }

	...
	public partial class AdmMenuDrgModule : RO.Web.ModuleBase
	{
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				try
				{
					try {dv = new DataView((new AdminSystem()).GetExp(121,"GetExpAdmMenuDrg121","Y",(string)Session[KEY_sysConnectionString],LcAppPw,filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
					catch {dv = new DataView((new AdminSystem()).GetExp(121,"GetExpAdmMenuDrg121","N",(string)Session[KEY_sysConnectionString],LcAppPw,filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
...
\par }}");
		...
		private DataView GetAdmMenuDrg121List()
		{
			...
			if (dt == null)
			{
				...
				try
				{
					try {dt = (new AdminSystem()).GetLis(121,"GetLisAdmMenuDrg121",true,"Y",0,(string)Session[KEY_sysConnectionString],LcAppPw,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
					catch {dt = (new AdminSystem()).GetLis(121,"GetLisAdmMenuDrg121",true,"N",0,(string)Session[KEY_sysConnectionString],LcAppPw,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				}
		...
		protected void cAdmMenuDrg121List_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			...
			try
			{
				dt = (new AdminSystem()).GetMstById("GetAdmMenuDrg121ById",cAdmMenuDrg121List.SelectedValue,(string)Session[KEY_sysConnectionString],LcAppPw);
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			if (ValidPage())
			{
				...
				if (string.IsNullOrEmpty(cAdmMenuDrg121List.SelectedValue))	// Add
				{
					if (ds != null)
					{
						pid = (new AdminSystem()).AddData(121,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],LcAppPw,base.CPrj,base.CSrc);
					}
					...
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(121,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],LcAppPw,base.CPrj,base.CSrc)) {bValid7 = true;}
					if (bValid7)
					{
						cAdmMenuDrg121List.ClearSearch(); Session.Remove(KEY_dtAdmMenuDrg121List);
		...
		public void cDeleteButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cMenuId39.Text != string.Empty)
			{
				...
				try
				{
					if (ds != null && (new AdminSystem()).DelData(121,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],LcAppPw,base.CPrj,base.CSrc))
					{
						cAdmMenuDrg121List.ClearSearch(); Session.Remove(KEY_dtAdmMenuDrg121List);

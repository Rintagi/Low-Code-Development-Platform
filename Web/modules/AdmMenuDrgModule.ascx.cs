...
namespace RO.Web
{
	public partial class AdmMenuDrgModule : RO.Web.ModuleBase
	{
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_121";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_121";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_121";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_121";
		private const string KEY_dtLabel = "Cache:dtLabel3_121";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_121";
		private const string KEY_dtScrCri = "Cache:dtScrCri3_121";
		private const string KEY_dsScrCriVal = "Cache:dsScrCriVal3_121";


		private const string KEY_dtSystems = "Cache:dtSystems3_121";
		private const string KEY_sysConnectionString = "Cache:sysConnectionString3_121";
		private const string KEY_dtAdmMenuDrg121List = "Cache:dtAdmMenuDrg121List";
		private const string KEY_bNewVisible = "Cache:bNewVisible3_121";
		private const string KEY_bNewSaveVisible = "Cache:bNewSaveVisible3_121";
		private const string KEY_bCopyVisible = "Cache:bCopyVisible3_121";
		private const string KEY_bCopySaveVisible = "Cache:bCopySaveVisible3_121";
		private const string KEY_bDeleteVisible = "Cache:bDeleteVisible3_121";
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_121";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_121";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_121";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				DataTable dtMenuAccess = (new MenuSystem()).GetMenu(base.LUser.CultureId, 3, base.LImpr, LcSysConnString, LcAppPw,121, null, null);
				...
				Session.Remove(KEY_dtAdmMenuDrg121List);
				...
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 121, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmMenuDrg121List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=121&typ=N&sys=3','','',''); return false;"; }
		...
		private void SetSystem(byte SystemId)
		{
			...
			try
			{
				...
				if ((Config.DeployType == "DEV" || row["dbAppDatabase"].ToString() == base.CPrj.EntityCode + "View") && !(base.CPrj.EntityCode != "RO" && row["SysProgram"].ToString() == "Y") && (new AdminSystem()).IsRegenNeeded(string.Empty,121,0,0,LcSysConnString,LcAppPw))
				{
					(new GenScreensSystem()).CreateProgram(121, "Menu IDE", row["dbAppDatabase"].ToString(), base.CPrj, base.CSrc, base.CTar, LcAppConnString, LcAppPw);
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(121,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(121,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(121,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(121,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,121,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(121,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(121,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(121,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		protected DataTable GetAuthRow()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthRow(121,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				try
				{
					try {dv = new DataView((new AdminSystem()).GetExp(121,"GetExpAdmMenuDrg121","Y",(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
					catch {dv = new DataView((new AdminSystem()).GetExp(121,"GetExpAdmMenuDrg121","N",(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(121,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					foreach (DataRowView drv in dv)
					{
						...
						if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["ParentId39"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["MenuIndex39"].ToString(),base.LUser.Culture) + (char)9);}
						sb.Append(Environment.NewLine);
					}
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmMenuDrg.csv"; Session["ExportStr"] = (Config.ExportExcelCSV ? "sep=\t\n": "") + sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmMenuDrg.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private string GetRtfGrid(DataView dv)
		{
			...
			try
			{
				...
				foreach (DataRowView drv in dv)
				{
					...
					if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["ParentId39"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["MenuIndex39"].ToString(),base.LUser.Culture) + @"\cell ");}
					sb.Append(@"}");
					sb.Append("\r\n");
					sb.Append(@"\pard \ql \li0\ri0\widctlpar\intbl\aspalpha\aspnum\adjustright\rin0\lin0 {");
					sb.Append(@"\insrsid2981395 \trowd \irow0\irowband0\lastrow \ts15\trgaph108\trleft-108\trbrdrt\brdrs\brdrw10 \trbrdrl\brdrs\brdrw10 \trbrdrb\brdrs\brdrw10 \trbrdrr\brdrs\brdrw10 \trbrdrh\brdrs\brdrw10 \trbrdrv\brdrs\brdrw10 ");
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 121, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			...
			UpdCriteria(true);
			cAdmMenuDrg121List.ClearSearch(); Session.Remove(KEY_dtAdmMenuDrg121List);
			PopAdmMenuDrg121List(sender, e, false, null);
		}
			...
			try
			{
				foreach (DataRowView drv in dvCri)
				{
					...
					if (drv["DisplayName"].ToString() == "ComboBox")
					{
						...
						(new AdminSystem()).MkGetScreenIn("121", drv["ScreenCriId"].ToString(), "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), LcAppDb, LcDesDb, drv["MultiDesignDb"].ToString(), LcSysConnString, LcAppPw);
						DataView dv = new DataView((new AdminSystem()).GetScreenIn("121", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), (new AdminSystem()).CountScrCri(drv["ScreenCriId"].ToString(), drv["MultiDesignDb"].ToString(), drv["MultiDesignDb"].ToString() == "N" ? LcSysConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)), drv["RequiredValid"].ToString(), 0, string.Empty, drv["DisplayMode"].ToString() != "AutoListBox", val, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)));
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("121", LcSysConnString, LcAppPw);
				}
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmMenuDrg121" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmMenuDrg121" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			foreach (DataRowView drv in dvCri)
			{
			    if (drv["DisplayName"].ToString() == "ListBox")
			    {
					...
					if (cListBox != null)
					{
						...
						int TotalChoiceCnt = new DataView((new AdminSystem()).GetScreenIn("121", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), CriCnt, drv["RequiredValid"].ToString(), 0, string.Empty, true, string.Empty, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId))).Count;
						string selectedValues = string.Join(",", cListBox.Items.Cast<ListItem>().Where(x => x.Selected).Select(x => "'" + x.Value + "'").ToArray());
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("121", "AdmMenuDrg121", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		private DataView GetAdmMenuDrg121List()
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmMenuDrg121List];
			if (dt == null)
			{
				CheckAuthentication(false);
				int filterId = 0;
				...
				try
				{
					try {dt = (new AdminSystem()).GetLis(121,"GetLisAdmMenuDrg121",true,"Y",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
					catch {dt = (new AdminSystem()).GetLis(121,"GetLisAdmMenuDrg121",true,"N",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				}
		...
		private void PopAdmMenuDrg121List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			DataView dv = GetAdmMenuDrg121List();
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			cAdmMenuDrg121List.AutoCompleteUrl = "AutoComplete.aspx/LisSuggests";
			cAdmMenuDrg121List.DataContext = context;
			if (dv.Table == null) return;
			cAdmMenuDrg121List.DataSource = dv;
			cAdmMenuDrg121List.Visible = true;
			if (cAdmMenuDrg121List.Items.Count <= 0) {cAdmMenuDrg121List.Visible = false; cAdmMenuDrg121List_SelectedIndexChanged(sender,e);}
			else
			{
				if (bPageLoad && !string.IsNullOrEmpty(Request.QueryString["key"]) && bUseCri.Value == string.Empty)
				{
					try {cAdmMenuDrg121List.SelectByValue(Request.QueryString["key"],string.Empty,true);} catch {cAdmMenuDrg121List.Items[0].Selected = true; cAdmMenuDrg121List_SelectedIndexChanged(sender, e);}
				    cScreenSearch.Visible = false; cSystem.Visible = false; cEditButton.Visible = true; cSaveCloseButton.Visible = cSaveButton.Visible;
				}
				else
				{
					if (ID != null) {if (!cAdmMenuDrg121List.SelectByValue(ID,string.Empty,true)) {ID = null;}}
					if (ID == null)
					{
						cAdmMenuDrg121List_SelectedIndexChanged(sender, e);
					}
				}
			}
		}
		...
		private void SetColumnAuthority()
		{
			...
			if (dtAuth != null && dtLabel != null)
			{
				...
				base.SetFoldBehavior(cParentId39, dtAuth.Rows[1], cParentId39P1, cParentId39Label, cParentId39P2, null, dtLabel.Rows[1], null, null, null);
				base.SetFoldBehavior(cMenuIndex39, dtAuth.Rows[2], cMenuIndex39P1, cMenuIndex39Label, cMenuIndex39P2, null, dtLabel.Rows[2], cRFVMenuIndex39, null, null);
				base.SetFoldBehavior(cMenuList, dtAuth.Rows[3], cMenuListP1, cMenuListLabel, cMenuListP2, null, dtLabel.Rows[3], null, null, null);
				base.SetFoldBehavior(cNewMenuList, dtAuth.Rows[4], cNewMenuListP1, cNewMenuListLabel, cNewMenuListP2, null, dtLabel.Rows[4], null, null, null);
				base.SetFoldBehavior(cCurrParentId, dtAuth.Rows[5], cCurrParentIdP1, cCurrParentIdLabel, cCurrParentIdP2, null, dtLabel.Rows[5], null, null, null);
				base.SetFoldBehavior(cDelItemId, dtAuth.Rows[6], cDelItemIdP1, cDelItemIdLabel, cDelItemIdP2, null, dtLabel.Rows[6], null, null, null);
			}
			...
			if ((cMenuList.Attributes["OnChange"] == null || cMenuList.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cMenuList.Visible && !cMenuList.ReadOnly) {cMenuList.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cNewMenuList.Attributes["OnChange"] == null || cNewMenuList.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cNewMenuList.Visible && !cNewMenuList.ReadOnly) {cNewMenuList.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cCurrParentId.Attributes["OnChange"] == null || cCurrParentId.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cCurrParentId.Visible && !cCurrParentId.ReadOnly) {cCurrParentId.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cDelItemId.Attributes["OnChange"] == null || cDelItemId.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cDelItemId.Visible && !cDelItemId.ReadOnly) {cDelItemId.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
		}
		...
		private void ClearMaster(object sender, System.EventArgs e)
		{
			...
			if (dt.Rows[1]["ColVisible"].ToString() == "Y" && dt.Rows[1]["ColReadOnly"].ToString() != "Y") {cParentId39.Text = string.Empty;}
			if (dt.Rows[2]["ColVisible"].ToString() == "Y" && dt.Rows[2]["ColReadOnly"].ToString() != "Y") {cMenuIndex39.Text = string.Empty;}
			if (dt.Rows[3]["ColVisible"].ToString() == "Y" && dt.Rows[3]["ColReadOnly"].ToString() != "Y") {cMenuList.Text = string.Empty;}
			if (dt.Rows[4]["ColVisible"].ToString() == "Y" && dt.Rows[4]["ColReadOnly"].ToString() != "Y") {cNewMenuList.Text = string.Empty;}
			if (dt.Rows[5]["ColVisible"].ToString() == "Y" && dt.Rows[5]["ColReadOnly"].ToString() != "Y") {cCurrParentId.Text = string.Empty;}
			if (dt.Rows[6]["ColVisible"].ToString() == "Y" && dt.Rows[6]["ColReadOnly"].ToString() != "Y") {cDelItemId.Text = string.Empty;}
		...
		private void InitMaster(object sender, System.EventArgs e)
		{
			...
			cMenuList.Text = string.Empty;
			cNewMenuList.Text = string.Empty;
			cCurrParentId.Text = string.Empty;
			cDelItemId.Text = string.Empty;
			// *** Default Value (Folder) Web Rule starts here *** //
		}
		protected void cAdmMenuDrg121List_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmMenuDrg121List_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmMenuDrg121List_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			InitMaster(sender, e);      // Do this first to enable defaults for non-database columns.
			DataTable dt = null;
			try
			{
				dt = (new AdminSystem()).GetMstById("GetAdmMenuDrg121ById",cAdmMenuDrg121List.SelectedValue,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId));
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				if (dt.Rows.Count > 0)
				{
					cAdmMenuDrg121List.SetDdVisible();
			...
			cButPanel.DataBind();
			ScriptManager.GetCurrent(Parent.Page).SetFocus(cAdmMenuDrg121List.FocusID);
			ShowDirty(false); PanelTop.Update();
			//WebRule: Replace the screen with the entire menu tree.
            ScriptManager.RegisterStartupScript(this, this.GetType(), "CustomCss", @"<link rel='stylesheet' type='text/css' media='screen' href='css/ide.css' />", false);
		...
		public void cUndoAllButton_Click(object sender, System.EventArgs e)
		{
			...
			else { PopAdmMenuDrg121List(sender, e, false, cMenuId39.Text); }
			// *** System Button Click (After) Web Rule starts here *** //
		}
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmMenuDrg121List); PopAdmMenuDrg121List(sender, e, false, null);
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			if (ValidPage())
			{
				AdmMenuDrg121 ds = PrepAdmMenuDrgData(null,cMenuId39.Text == string.Empty);
				if (!string.IsNullOrEmpty(cAdmMenuDrg121List.SelectedValue) && cMenuId39.Text != cAdmMenuDrg121List.SelectedValue)
				{
					PreMsgPopup("Primary key cannot be changed");
					return rtn;
				}
				if (string.IsNullOrEmpty(cAdmMenuDrg121List.SelectedValue))	// Add
				{
					if (ds != null)
					{
						pid = (new AdminSystem()).AddData(121,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc,noTrans,commandTimeOut);
					}
					if (!string.IsNullOrEmpty(pid))
					{
						cAdmMenuDrg121List.ClearSearch(); Session.Remove(KEY_dtAdmMenuDrg121List);
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmMenuDrg121List(sender, e, false, pid);
						rtn = GetScreenHlp().Rows[0]["AddMsg"].ToString();
					}
				}
				else {
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(121,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc,noTrans,commandTimeOut)) {bValid7 = true;}
					if (bValid7)
					{
						cAdmMenuDrg121List.ClearSearch(); Session.Remove(KEY_dtAdmMenuDrg121List);
						ShowDirty(false); PopAdmMenuDrg121List(sender, e, false, ds.Tables["AdmMenuDrg"].Rows[0]["MenuId39"]);
		...
		public void cDeleteButton_Click(object sender, System.EventArgs e)
		{
			...
			{
				AdmMenuDrg121 ds = PrepAdmMenuDrgData(null,false);
				try
				{
					if (ds != null && (new AdminSystem()).DelData(121,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc))
					{
						cAdmMenuDrg121List.ClearSearch(); Session.Remove(KEY_dtAdmMenuDrg121List);
						ShowDirty(false); PopAdmMenuDrg121List(sender, e, false, null);
		...
		private AdmMenuDrg121 PrepAdmMenuDrgData(DataView dv, bool bAdd)
		{
			...
			try {dr["MenuList"] = cMenuList.Text.Trim();} catch {}
			drType["MenuList"] = string.Empty; drDisp["MenuList"] = "TextBox";
			try {dr["NewMenuList"] = cNewMenuList.Text.Trim();} catch {}
			drType["NewMenuList"] = string.Empty; drDisp["NewMenuList"] = "TextBox";
			try {dr["CurrParentId"] = cCurrParentId.Text.Trim();} catch {}
			drType["CurrParentId"] = string.Empty; drDisp["CurrParentId"] = "TextBox";
			try {dr["DelItemId"] = cDelItemId.Text.Trim();} catch {}
		...
		public bool CanAct(char typ) { return CanAct(typ, GetAuthRow(), cAdmMenuDrg121List.SelectedValue.ToString()); }
		...
		private void PreMsgPopup(string msg) { PreMsgPopup(msg,cAdmMenuDrg121List,null); }

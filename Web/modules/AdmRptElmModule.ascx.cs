...
\par }}");
		...
		private void SetReportId160(RoboCoder.WebControls.ComboBox ddl, string keyId, ListViewItem li)
		{
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(91,"GetDdlReportId3S1599",true,false,0,keyId,(string)Session[KEY_sysConnectionString],LcAppPw,string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
		...
		private void SetRptElmTypeCd160(DropDownList ddl, string keyId)
		{
			...
			if (ddl != null)
			{
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(91,"GetDdlRptElmTypeCd3S1600",true,bAll,0,keyId,(string)Session[KEY_sysConnectionString],LcAppPw,string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(91,"GetDdlRptElmTypeCd3S1600",true,true,0,keyId,(string)Session[KEY_sysConnectionString],LcAppPw,string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
		...
		private void SetRptStyleId160(RoboCoder.WebControls.ComboBox ddl, string keyId, ListViewItem li)
		{
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(91,"GetDdlRptStyleId3S1601",true,false,0,keyId,(string)Session[KEY_sysConnectionString],LcAppPw,string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
		...
		private void PopAdmRptElm91List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			...
			try
			{
				try {dt = (new AdminSystem()).GetLis(91,"GetLisAdmRptElm91",false,"Y",0,(string)Session[KEY_sysConnectionString],LcAppPw,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				catch {dt = (new AdminSystem()).GetLis(91,"GetLisAdmRptElm91",false,"N",0,(string)Session[KEY_sysConnectionString],LcAppPw,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				foreach (DataRowView drv in dti.DefaultView)
				{
					if ((drv.Row.RowState == System.Data.DataRowState.Added || drv.Row.RowState == System.Data.DataRowState.Detached) && jj >= iExist)
					{
						...
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(91,"GetDdlReportId3S1599",true,true,0,string.Empty,(string)Session[KEY_sysConnectionString],LcAppPw,string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						...
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(91,"GetDdlRptElmTypeCd3S1600",true,true,0,string.Empty,(string)Session[KEY_sysConnectionString],LcAppPw,string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						...
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(91,"GetDdlRptStyleId3S1601",true,true,0,string.Empty,(string)Session[KEY_sysConnectionString],LcAppPw,string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			if (ValidPage() && UpdateGridRow(cAdmRptElmGrid, new CommandEventArgs("Save", "")))
			{
				...
				if (ftr != string.Empty) {dv.RowFilter = ftr;}
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(91,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],LcAppPw,base.CPrj,base.CSrc)) {bValid7 = true;}
					if (bValid7)
					{
		...
		private bool UpdateGridRow(object sender, CommandEventArgs e)
		{
			...
			{
				TextBox pwd = null;				cAdmRptElmGrid_OnItemUpdating(sender, new ListViewUpdateEventArgs(cAdmRptElmGrid.EditIndex));
			}
			return true;
		}

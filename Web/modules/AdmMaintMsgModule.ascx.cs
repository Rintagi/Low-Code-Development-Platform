...
namespace RO.Web
{
	public partial class AdmMaintMsgModule : RO.Web.ModuleBase
	{
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_109";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_109";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_109";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_109";
		private const string KEY_dtLabel = "Cache:dtLabel3_109";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_109";
		private const string KEY_dtScrCri = "Cache:dtScrCri3_109";
		private const string KEY_dsScrCriVal = "Cache:dsScrCriVal3_109";


		private const string KEY_dtSystems = "Cache:dtSystems3_109";
		private const string KEY_sysConnectionString = "Cache:sysConnectionString3_109";
		private const string KEY_dtAdmMaintMsg109List = "Cache:dtAdmMaintMsg109List";
		private const string KEY_bNewVisible = "Cache:bNewVisible3_109";
		private const string KEY_bNewSaveVisible = "Cache:bNewSaveVisible3_109";
		private const string KEY_bCopyVisible = "Cache:bCopyVisible3_109";
		private const string KEY_bCopySaveVisible = "Cache:bCopySaveVisible3_109";
		private const string KEY_bDeleteVisible = "Cache:bDeleteVisible3_109";
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_109";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_109";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_109";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				DataTable dtMenuAccess = (new MenuSystem()).GetMenu(base.LUser.CultureId, 3, base.LImpr, LcSysConnString, LcAppPw,109, null, null);
				...
				Session.Remove(KEY_dtAdmMaintMsg109List);
				...
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 109, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmMaintMsg109List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=109&typ=N&sys=3','','',''); return false;"; }
		...
		private void SetSystem(byte SystemId)
		{
			...
			try
			{
				...
				if ((Config.DeployType == "DEV" || row["dbAppDatabase"].ToString() == base.CPrj.EntityCode + "View") && !(base.CPrj.EntityCode != "RO" && row["SysProgram"].ToString() == "Y") && (new AdminSystem()).IsRegenNeeded(string.Empty,109,0,0,LcSysConnString,LcAppPw))
				{
					(new GenScreensSystem()).CreateProgram(109, "Maintenance Message", row["dbAppDatabase"].ToString(), base.CPrj, base.CSrc, base.CTar, LcAppConnString, LcAppPw);
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(109,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(109,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(109,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(109,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,109,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(109,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(109,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(109,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		protected DataTable GetAuthRow()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthRow(109,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				try
				{
					try {dv = new DataView((new AdminSystem()).GetExp(109,"GetExpAdmMaintMsg109","Y",null,null,filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
					catch {dv = new DataView((new AdminSystem()).GetExp(109,"GetExpAdmMaintMsg109","N",null,null,filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(109,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					{
						if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["MaintMsgId233"].ToString(),base.LUser.Culture) + (char)9);}
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmMaintMsg.xls"; Session["ExportStr"] = sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmMaintMsg.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private string GetRtfGrid(DataView dv)
		{
			...
			try
			{
				...
				foreach (DataRowView drv in dv)
				{
					...
					sb.Append("\r\n");
					sb.Append(@"{");
					if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["MaintMsgId233"].ToString(),base.LUser.Culture) + @"\cell ");}
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 109, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			...
			UpdCriteria(true);
			cAdmMaintMsg109List.ClearSearch(); Session.Remove(KEY_dtAdmMaintMsg109List);
			PopAdmMaintMsg109List(sender, e, false, null);
		}
			...
			try
			{
				foreach (DataRowView drv in dvCri)
				{
					...
					if (drv["DisplayName"].ToString() == "ComboBox")
					{
						...
						(new AdminSystem()).MkGetScreenIn("109", drv["ScreenCriId"].ToString(), "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), LcAppDb, LcDesDb, drv["MultiDesignDb"].ToString(), LcSysConnString, LcAppPw);
						DataView dv = new DataView((new AdminSystem()).GetScreenIn("109", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), (new AdminSystem()).CountScrCri(drv["ScreenCriId"].ToString(), drv["MultiDesignDb"].ToString(), drv["MultiDesignDb"].ToString() == "N" ? LcSysConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)), drv["RequiredValid"].ToString(), 0, string.Empty, drv["DisplayMode"].ToString() != "AutoListBox", val, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)));
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("109", LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmMaintMsg109" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmMaintMsg109" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			foreach (DataRowView drv in dvCri)
			{
			    if (drv["DisplayName"].ToString() == "ListBox")
			    {
					...
					if (cListBox != null)
					{
						...
						int TotalChoiceCnt = new DataView((new AdminSystem()).GetScreenIn("109", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), CriCnt, drv["RequiredValid"].ToString(), 0, string.Empty, true, string.Empty, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId))).Count;
						string selectedValues = string.Join(",", cListBox.Items.Cast<ListItem>().Where(x => x.Selected).Select(x => "'" + x.Value + "'").ToArray());
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("109", "AdmMaintMsg109", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		private DataView GetAdmMaintMsg109List()
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmMaintMsg109List];
			if (dt == null)
			{
				CheckAuthentication(false);
				int filterId = 0;
				...
				try
				{
					try {dt = (new AdminSystem()).GetLis(109,"GetLisAdmMaintMsg109",true,"Y",0,null,null,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
					catch {dt = (new AdminSystem()).GetLis(109,"GetLisAdmMaintMsg109",true,"N",0,null,null,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				}
		...
		private void PopAdmMaintMsg109List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			DataView dv = GetAdmMaintMsg109List();
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			cAdmMaintMsg109List.AutoCompleteUrl = "AutoComplete.aspx/LisSuggests";
			cAdmMaintMsg109List.DataContext = context;
			if (dv.Table == null) return;
			cAdmMaintMsg109List.DataSource = dv;
			cAdmMaintMsg109List.Visible = true;
			if (cAdmMaintMsg109List.Items.Count <= 0) {cAdmMaintMsg109List.Visible = false; cAdmMaintMsg109List_SelectedIndexChanged(sender,e);}
			else
			{
				if (bPageLoad && !string.IsNullOrEmpty(Request.QueryString["key"]) && bUseCri.Value == string.Empty)
				{
					try {cAdmMaintMsg109List.SelectByValue(Request.QueryString["key"],string.Empty,true);} catch {cAdmMaintMsg109List.Items[0].Selected = true; cAdmMaintMsg109List_SelectedIndexChanged(sender, e);}
				    cScreenSearch.Visible = false; cSystem.Visible = false; cEditButton.Visible = true; cSaveCloseButton.Visible = cSaveButton.Visible;
				}
				else
				{
					if (ID != null) {if (!cAdmMaintMsg109List.SelectByValue(ID,string.Empty,true)) {ID = null;}}
					if (ID == null)
					{
						cAdmMaintMsg109List_SelectedIndexChanged(sender, e);
					}
				}
			}
		}
		...
		private void SetColumnAuthority()
		{
			...
			{
				base.SetFoldBehavior(cMaintMsgId233, dtAuth.Rows[0], cMaintMsgId233P1, cMaintMsgId233Label, cMaintMsgId233P2, null, dtLabel.Rows[0], null, null, null);
				base.SetFoldBehavior(cMaintMsgName233, dtAuth.Rows[1], cMaintMsgName233P1, cMaintMsgName233Label, cMaintMsgName233P2, null, dtLabel.Rows[1], cRFVMaintMsgName233, null, null);
				base.SetFoldBehavior(cMaintMessage233, dtAuth.Rows[2], cMaintMessage233P1, cMaintMessage233Label, cMaintMessage233P2, cMaintMessage233E, null, dtLabel.Rows[2], cRFVMaintMessage233, null, null);
				cMaintMessage233E.Attributes["label_id"] = cMaintMessage233Label.ClientID; cMaintMessage233E.Attributes["target_id"] = cMaintMessage233.ClientID;
				base.SetFoldBehavior(cShowOnLogin233, dtAuth.Rows[3], cShowOnLogin233P1, cShowOnLogin233Label, cShowOnLogin233P2, null, dtLabel.Rows[3], null, null, null);
				base.SetFoldBehavior(cLastEmailDt233, dtAuth.Rows[4], cLastEmailDt233P1, cLastEmailDt233Label, cLastEmailDt233P2, null, dtLabel.Rows[4], null, null, null);
				base.SetFoldBehavior(cEmailUsers, dtAuth.Rows[5], null, null, null, dtLabel.Rows[5], null, null, null);
			}
			if ((cMaintMsgName233.Attributes["OnChange"] == null || cMaintMsgName233.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cMaintMsgName233.Visible && !cMaintMsgName233.ReadOnly) {cMaintMsgName233.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cMaintMessage233.Attributes["OnChange"] == null || cMaintMessage233.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cMaintMessage233.Visible && !cMaintMessage233.ReadOnly) {cMaintMessage233.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cShowOnLogin233.Attributes["OnClick"] == null || cShowOnLogin233.Attributes["OnClick"].IndexOf("ChkPgDirty") < 0) && cShowOnLogin233.Visible && cShowOnLogin233.Enabled) {cShowOnLogin233.Attributes["OnClick"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty(); this.focus();";}
			if ((cLastEmailDt233.Attributes["OnChange"] == null || cLastEmailDt233.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cLastEmailDt233.Visible && !cLastEmailDt233.ReadOnly) {cLastEmailDt233.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
		}
		...
		private void ClearMaster(object sender, System.EventArgs e)
		{
			...
			if (dt.Rows[1]["ColVisible"].ToString() == "Y" && dt.Rows[1]["ColReadOnly"].ToString() != "Y") {cMaintMsgName233.Text = string.Empty;}
			if (dt.Rows[2]["ColVisible"].ToString() == "Y" && dt.Rows[2]["ColReadOnly"].ToString() != "Y") {cMaintMessage233.Text = string.Empty;}
			if (dt.Rows[3]["ColVisible"].ToString() == "Y" && dt.Rows[3]["ColReadOnly"].ToString() != "Y") {cShowOnLogin233.Checked = base.GetBool("N");}
			if (dt.Rows[4]["ColVisible"].ToString() == "Y" && dt.Rows[4]["ColReadOnly"].ToString() != "Y") {cLastEmailDt233.Text = string.Empty;}
			// *** Default Value (Folder) Web Rule starts here *** //
		}

		private void InitMaster(object sender, System.EventArgs e)
		{
			cMaintMsgId233.Text = string.Empty;
			cMaintMsgName233.Text = string.Empty;
			cMaintMessage233.Text = string.Empty;
			cShowOnLogin233.Checked = base.GetBool("N");
			cLastEmailDt233.Text = string.Empty;
			// *** Default Value (Folder) Web Rule starts here *** //
		}
		protected void cAdmMaintMsg109List_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmMaintMsg109List_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmMaintMsg109List_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			InitMaster(sender, e);      // Do this first to enable defaults for non-database columns.
			DataTable dt = null;
			try
			{
				dt = (new AdminSystem()).GetMstById("GetAdmMaintMsg109ById",cAdmMaintMsg109List.SelectedValue,null,null);
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				if (dt.Rows.Count > 0)
				{
					cAdmMaintMsg109List.SetDdVisible();
					DataRow dr = dt.Rows[0];
					try {cMaintMsgId233.Text = RO.Common3.Utils.fmNumeric("0",dr["MaintMsgId233"].ToString(),base.LUser.Culture);} catch {cMaintMsgId233.Text = string.Empty;}
			...
			cButPanel.DataBind();
			ScriptManager.GetCurrent(Parent.Page).SetFocus(cAdmMaintMsg109List.FocusID);
			ShowDirty(false); PanelTop.Update();
			// *** List Selection (End of) Web Rule starts here *** //
		}

		protected void cEmailUsers_Click(object sender, System.EventArgs e)
		{
			//WebRule: Email all active users
            if (cMaintMsgId233.Text != string.Empty && cMaintMessage233.Text != string.Empty)
            {
                try
                {
                    StringBuilder sb = new StringBuilder();
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cMaintMsgId233.Text == string.Empty) { cClearButton_Click(sender, new EventArgs()); ShowDirty(false); PanelTop.Update();}
			else { PopAdmMaintMsg109List(sender, e, false, cMaintMsgId233.Text); }
			// *** System Button Click (After) Web Rule starts here *** //
		}
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmMaintMsg109List); PopAdmMaintMsg109List(sender, e, false, null);
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			if (ValidPage())
			{
				AdmMaintMsg109 ds = PrepAdmMaintMsgData(null,cMaintMsgId233.Text == string.Empty);
				if (!string.IsNullOrEmpty(cAdmMaintMsg109List.SelectedValue) && cMaintMsgId233.Text != cAdmMaintMsg109List.SelectedValue)
				{
					PreMsgPopup("Primary key cannot be changed");
					return rtn;
				}
				if (string.IsNullOrEmpty(cAdmMaintMsg109List.SelectedValue))	// Add
				{
					if (ds != null)
					{
						pid = (new AdminSystem()).AddData(109,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc,noTrans,commandTimeOut);
					}
					if (!string.IsNullOrEmpty(pid))
					{
						cAdmMaintMsg109List.ClearSearch(); Session.Remove(KEY_dtAdmMaintMsg109List);
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmMaintMsg109List(sender, e, false, pid);
						rtn = GetScreenHlp().Rows[0]["AddMsg"].ToString();
					}
				}
				else {
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(109,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc,noTrans,commandTimeOut)) {bValid7 = true;}
					if (bValid7)
					{
						cAdmMaintMsg109List.ClearSearch(); Session.Remove(KEY_dtAdmMaintMsg109List);
						ShowDirty(false); PopAdmMaintMsg109List(sender, e, false, ds.Tables["AdmMaintMsg"].Rows[0]["MaintMsgId233"]);
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cMaintMsgId233.Text != string.Empty)
			{
				AdmMaintMsg109 ds = PrepAdmMaintMsgData(null,false);
				try
				{
					if (ds != null && (new AdminSystem()).DelData(109,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc))
					{
						cAdmMaintMsg109List.ClearSearch(); Session.Remove(KEY_dtAdmMaintMsg109List);
						ShowDirty(false); PopAdmMaintMsg109List(sender, e, false, null);
		...
		private AdmMaintMsg109 PrepAdmMaintMsgData(DataView dv, bool bAdd)
		{
			...
			if (bAdd) { dr["MaintMsgId233"] = string.Empty; } else { dr["MaintMsgId233"] = cMaintMsgId233.Text; }
			drType["MaintMsgId233"] = "Numeric"; drDisp["MaintMsgId233"] = "TextBox";
			try {dr["MaintMsgName233"] = cMaintMsgName233.Text.Trim();} catch {}
			drType["MaintMsgName233"] = "VarWChar"; drDisp["MaintMsgName233"] = "TextBox";
			try {dr["MaintMessage233"] = cMaintMessage233.Text;} catch {}
		...
		public bool CanAct(char typ) { return CanAct(typ, GetAuthRow(), cAdmMaintMsg109List.SelectedValue.ToString()); }
		...
		private void PreMsgPopup(string msg) { PreMsgPopup(msg,cAdmMaintMsg109List,null); }

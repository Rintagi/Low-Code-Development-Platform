...
namespace RO.Web
{
	public partial class AdmColHlpModule : RO.Web.ModuleBase
	{

		private const string KEY_lastAddedRow = "Cache:lastAddedRow3_1006";
		private const string KEY_lastSortOrd = "Cache:lastSortOrd3_1006";
		private const string KEY_lastSortImg = "Cache:lastSortImg3_1006";
		private const string KEY_lastSortCol = "Cache:lastSortCol3_1006";
		private const string KEY_lastSortExp = "Cache:lastSortExp3_1006";
		private const string KEY_lastSortUrl = "Cache:lastSortUrl3_1006";
		private const string KEY_lastSortTog = "Cache:lastSortTog3_1006";
		private const string KEY_lastImpPwdOvride = "Cache:lastImpPwdOvride3_1006";
		private const string KEY_cntImpPwdOvride = "Cache:cntImpPwdOvride3_1006";
		private const string KEY_currPageIndex = "Cache:currPageIndex3_1006";
		private const string KEY_bHiImpVisible = "Cache:bHiImpVisible3_1006";
		private const string KEY_bShImpVisible = "Cache:bShImpVisible3_1006";
		private const string KEY_dtAdmColHlpGrid = "Cache:dtAdmColHlpGrid";
		private const string KEY_scrImport = "Cache:scrImport3_1006";
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_1006";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_1006";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_1006";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_1006";
		private const string KEY_dtLabel = "Cache:dtLabel3_1006";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_1006";
		private const string KEY_dtScrCri = "Cache:dtScrCri3_1006";
		private const string KEY_dsScrCriVal = "Cache:dsScrCriVal3_1006";

		private const string KEY_dtCultureId21 = "Cache:dtCultureId21";

		private const string KEY_dtSystems = "Cache:dtSystems3_1006";
		private const string KEY_sysConnectionString = "Cache:sysConnectionString3_1006";
		private const string KEY_dtAdmColHlp1006List = "Cache:dtAdmColHlp1006List";
		private const string KEY_bNewVisible = "Cache:bNewVisible3_1006";
		private const string KEY_bNewSaveVisible = "Cache:bNewSaveVisible3_1006";
		private const string KEY_bCopyVisible = "Cache:bCopyVisible3_1006";
		private const string KEY_bCopySaveVisible = "Cache:bCopySaveVisible3_1006";
		private const string KEY_bDeleteVisible = "Cache:bDeleteVisible3_1006";
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_1006";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_1006";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_1006";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				DataTable dtMenuAccess = (new MenuSystem()).GetMenu(base.LUser.CultureId, 3, base.LImpr, LcSysConnString, LcAppPw,1006, null, null);
				...
				Session.Remove(KEY_dtCultureId21);
				SetButtonHlp();
				GetSystems();
				SetColumnAuthority();
				GetGlobalFilter();
				...
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 1006, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmColHlp1006List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=1006&typ=N&sys=3','','',''); return false;"; }
			...
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmColHlpGrid];
				if (dt != null)
				{
		...
		private void InitializeComponent()
		{
			...
			if (LcSysConnString == null)
			{
				...
				{
					DataTable dt;
					dt = (new AdminSystem()).GetAuthRow(1006,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
					...
					{
						cAdmColHlpGrid.PreRender += new EventHandler(cAdmColHlpGrid_OnPreRender);
					}
				}
		...
		private void SetSystem(byte SystemId)
		{
			...
			try
			{
				...
				if ((Config.DeployType == "DEV" || row["dbAppDatabase"].ToString() == base.CPrj.EntityCode + "View") && !(base.CPrj.EntityCode != "RO" && row["SysProgram"].ToString() == "Y") && (new AdminSystem()).IsRegenNeeded(string.Empty,1006,0,0,LcSysConnString,LcAppPw))
				{
					(new GenScreensSystem()).CreateProgram(1006, "Screen Object Label", row["dbAppDatabase"].ToString(), base.CPrj, base.CSrc, base.CTar, LcAppConnString, LcAppPw);
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(1006,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(1006,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void SetClientRule(ListViewDataItem ee, bool isEditItem)
		{
			...
			if (dvRul.Count > 0)
			{
				...
				{
					if (ee == null || drv["ScriptEvent"].ToString().Substring(0,2).ToLower() != "on" || (cAdmColHlpGrid.EditIndex > -1 && GetDataItemIndex(cAdmColHlpGrid.EditIndex) == ee.DataItemIndex))
					{
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(1006,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(1006,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,1006,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(1006,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(1006,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(1006,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				try
				{
					try {dv = new DataView((new AdminSystem()).GetExp(1006,"GetExpAdmColHlp1006","Y",(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
					catch {dv = new DataView((new AdminSystem()).GetExp(1006,"GetExpAdmColHlp1006","N",(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(1006,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					foreach (DataRowView drv in dv)
					{
						...
						if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["ScreenObjHlpId21"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(drv["CultureId21"].ToString() + (char)9 + drv["CultureId21Text"].ToString() + (char)9);}
						if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["ColumnHeader21"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["ToolTip21"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["ErrMessage21"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["TbHint21"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						sb.Append(Environment.NewLine);
					}
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmColHlp.csv"; Session["ExportStr"] = (Config.ExportExcelCSV ? "sep=\t\n": "") + sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmColHlp.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private string GetRtfGrid(DataView dv)
		{
			...
			try
			{
				...
				foreach (DataRowView drv in dv)
				{
					...
					if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["ScreenObjHlpId21"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(drv["CultureId21Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(drv["ColumnHeader21"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append(drv["ToolTip21"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append(drv["ErrMessage21"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append(drv["TbHint21"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					sb.Append(@"}");
					sb.Append("\r\n");
					sb.Append(@"\pard \ql \li0\ri0\widctlpar\intbl\aspalpha\aspnum\adjustright\rin0\lin0 {");
					sb.Append(@"\insrsid2981395 \trowd \irow0\irowband0\lastrow \ts15\trgaph108\trleft-108\trbrdrt\brdrs\brdrw10 \trbrdrl\brdrs\brdrw10 \trbrdrb\brdrs\brdrw10 \trbrdrr\brdrs\brdrw10 \trbrdrh\brdrs\brdrw10 \trbrdrv\brdrs\brdrw10 ");
		...
		private void GetPageInfo()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastPageInfo(1006, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 1006, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			// *** Criteria Trigger (before) Web Rule starts here *** //
			DataTable dt = (DataTable)Session[KEY_dtAdmColHlpGrid];
			if (cAdmColHlpGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
			if (IsPostBack && cCriteria.Visible) ReBindCriteria(GetScrCriteria());
			UpdCriteria(true);
			cAdmColHlp1006List.ClearSearch(); Session.Remove(KEY_dtAdmColHlp1006List);
			PopAdmColHlp1006List(sender, e, false, null);
			}
		}
			...
			try
			{
				foreach (DataRowView drv in dvCri)
				{
					...
					if (drv["DisplayName"].ToString() == "ComboBox")
					{
						...
						(new AdminSystem()).MkGetScreenIn("1006", drv["ScreenCriId"].ToString(), "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), LcAppDb, LcDesDb, drv["MultiDesignDb"].ToString(), LcSysConnString, LcAppPw);
						DataView dv = new DataView((new AdminSystem()).GetScreenIn("1006", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), (new AdminSystem()).CountScrCri(drv["ScreenCriId"].ToString(), drv["MultiDesignDb"].ToString(), drv["MultiDesignDb"].ToString() == "N" ? LcSysConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)), drv["RequiredValid"].ToString(), 0, string.Empty, drv["DisplayMode"].ToString() != "AutoListBox", val, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)));
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("1006", LcSysConnString, LcAppPw);
				}
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmColHlp1006" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmColHlp1006" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			foreach (DataRowView drv in dvCri)
			{
			    if (drv["DisplayName"].ToString() == "ListBox")
			    {
					...
					if (cListBox != null)
					{
						...
						int TotalChoiceCnt = new DataView((new AdminSystem()).GetScreenIn("1006", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), CriCnt, drv["RequiredValid"].ToString(), 0, string.Empty, true, string.Empty, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId))).Count;
						string selectedValues = string.Join(",", cListBox.Items.Cast<ListItem>().Where(x => x.Selected).Select(x => "'" + x.Value + "'").ToArray());
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("1006", "AdmColHlp1006", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		protected void cbCultureId21(object sender, System.EventArgs e)
		{
			SetCultureId21((RoboCoder.WebControls.ComboBox)sender,string.Empty,cAdmColHlpGrid.EditItem);
		}

		private void SetCultureId21(RoboCoder.WebControls.ComboBox ddl, string keyId, ListViewItem li)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(1006,"GetDdlCultureId3S3255",true,false,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
					...
					{
						context["pMKeyColID"] = cAdmColHlp1006List.ClientID;
						context["pMKeyCol"] = "ScreenObjId";
						string ss = "(ScreenObjId is null";
						if (string.IsNullOrEmpty(cAdmColHlp1006List.SelectedValue)) {ss = ss + ")";} else {ss = ss + " OR ScreenObjId = " + cAdmColHlp1006List.SelectedValue + ")";}
						dv.RowFilter = ss;
					}
					ddl.DataSource = dv; Session[KEY_dtCultureId21] = dv.Table;
				    try { ddl.SelectByValue(keyId,string.Empty,true); } catch { try { ddl.SelectedIndex = 0; } catch { } }
				}
			}
		}

		private DataView GetAdmColHlp1006List()
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmColHlp1006List];
			if (dt == null)
			{
				CheckAuthentication(false);
				int filterId = 0;
				...
				try
				{
					try {dt = (new AdminSystem()).GetLis(1006,"GetLisAdmColHlp1006",true,"Y",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
					catch {dt = (new AdminSystem()).GetLis(1006,"GetLisAdmColHlp1006",true,"N",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				}
		...
		private void PopAdmColHlp1006List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			DataView dv = GetAdmColHlp1006List();
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			context["isSys"] = "N";
			context["conn"] = KEY_sysConnectionString;
			cAdmColHlp1006List.AutoCompleteUrl = "AutoComplete.aspx/LisSuggests";
			cAdmColHlp1006List.DataContext = context;
			if (dv.Table == null) return;
			cAdmColHlp1006List.DataSource = dv;
			cAdmColHlp1006List.Visible = true;
			if (cAdmColHlp1006List.Items.Count <= 0) {cAdmColHlp1006List.Visible = false; cAdmColHlp1006List_SelectedIndexChanged(sender,e);}
			else
			{
				if (bPageLoad && !string.IsNullOrEmpty(Request.QueryString["key"]) && bUseCri.Value == string.Empty)
				{
					try {cAdmColHlp1006List.SelectByValue(Request.QueryString["key"],string.Empty,true);} catch {cAdmColHlp1006List.Items[0].Selected = true; cAdmColHlp1006List_SelectedIndexChanged(sender, e);}
				    cScreenSearch.Visible = false; cSystem.Visible = false; cEditButton.Visible = true; cSaveCloseButton.Visible = cSaveButton.Visible;
				}
				else
				{
					if (ID != null) {if (!cAdmColHlp1006List.SelectByValue(ID,string.Empty,true)) {ID = null;}}
					if (ID == null)
					{
						cAdmColHlp1006List_SelectedIndexChanged(sender, e);
					}
				}
			}
		}
		...
		protected void cAdmColHlp1006List_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmColHlp1006List_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmColHlp1006List_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			InitMaster(sender, e);      // Do this first to enable defaults for non-database columns.
			DataTable dt = null;
			try
			{
				dt = (new AdminSystem()).GetMstById("GetAdmColHlp1006ById",cAdmColHlp1006List.SelectedValue,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId));
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				if (dt.Rows.Count > 0)
				{
					cAdmColHlp1006List.SetDdVisible();
					DataRow dr = dt.Rows[0];
			...
			DataTable dtAdmColHlpGrid = null;
			int filterId = 0; if (Utils.IsInt(cFilterId.SelectedValue)) { filterId = int.Parse(cFilterId.SelectedValue); }
			try
			{
				dtAdmColHlpGrid = (new AdminSystem()).GetDtlById(1006,"GetAdmColHlp1006DtlById",cAdmColHlp1006List.SelectedValue,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,base.LImpr,base.LCurr);
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (!dtAdmColHlpGrid.Columns.Contains("_NewRow")) { dtAdmColHlpGrid.Columns.Add("_NewRow"); }
			Session[KEY_dtAdmColHlpGrid] = dtAdmColHlpGrid;
			cAdmColHlpGrid.EditIndex = -1;
			cAdmColHlpGridDataPager.PageSize = Int16.Parse(cPgSize.Text); GotoPage(0);
			if (Session[KEY_lastSortCol] != null)
			{
				cAdmColHlpGrid_OnSorting(sender, new ListViewSortEventArgs((string)Session[KEY_lastSortExp], SortDirection.Ascending));
			}
			else if (dtAdmColHlpGrid.Rows.Count <= 0 || (!((GetAuthRow().Rows[0]["AllowUpd"].ToString() == "N" || GetAuthRow().Rows[0]["ViewOnly"].ToString() == "G") && GetAuthRow().Rows[0]["AllowAdd"].ToString() == "N") && !(Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N") && dtAdmColHlpGrid.Rows.Count == 1))
			{
				cAdmColHlpGrid_DataBind(dtAdmColHlpGrid.DefaultView);
			}
			cFindButton_Click(sender, e);
			cNaviPanel.Visible = true; cImportPwdPanel.Visible = false; Session.Remove(KEY_scrImport);
			ScriptManager.GetCurrent(Parent.Page).SetFocus(cAdmColHlp1006List.FocusID);
		...
		public void cFindButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dv != null)
			{
				...
				if ((cc != null && cc.ID.Equals("cAdmColHlp1006List")) || cAdmColHlpGrid.EditIndex < 0 || UpdateGridRow(cAdmColHlpGrid, new CommandEventArgs("Save", "")))
				{
					...
					GotoPage(0); cAdmColHlpGrid_DataBind(dv);
					if (GetCurrPageIndex() != (int)Session[KEY_currPageIndex] && (int)Session[KEY_currPageIndex] < GetTotalPages()) { GotoPage((int)Session[KEY_currPageIndex]); cAdmColHlpGrid_DataBind(dv);}
		...
		public void cInsRowButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dt != null && (ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")))
			{
				...
				Session[KEY_lastAddedRow] = 0; Session[KEY_dtAdmColHlpGrid] = dt; Session[KEY_currPageIndex] = 0; GotoPage(0);
				cAdmColHlpGrid_DataBind(dt.DefaultView);
				cAdmColHlpGrid_OnItemEditing(cAdmColHlpGrid, new ListViewEditEventArgs(0));
		...
		public void cPgSizeButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cAdmColHlpGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				...
				cAdmColHlpGridDataPager.PageSize = int.Parse(cPgSize.Text); GotoPage(0);
				if (dt.Rows.Count <= 0 || (!((GetAuthRow().Rows[0]["AllowUpd"].ToString() == "N" || GetAuthRow().Rows[0]["ViewOnly"].ToString() == "G") && GetAuthRow().Rows[0]["AllowAdd"].ToString() == "N") && !(Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N") && dt.Rows.Count == 1)) {cAdmColHlpGrid_DataBind(dt.DefaultView);} else {cFindButton_Click(sender, e);}
				try
				{
					(new AdminSystem()).UpdLastPageInfo(1006, base.LUser.UsrId, cPgSize.Text, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		}
		...
		public void cShowImpButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cSchemaImage.Visible)
			{
				try
				{
					Session["ImportSchema"] = (new AdminSystem()).GetSchemaScrImp(1006,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (cSchemaImage.Attributes["OnClick"] == null || cSchemaImage.Attributes["OnClick"].IndexOf("ImportSchema.aspx") < 0) {cSchemaImage.Attributes["OnClick"] += "SearchLink('ImportSchema.aspx?scm=S&key=1006&csy=3','','',''); return false;";}
			}
			// *** System Button Click (after) Web Rule starts here *** //
		}
		...
		private void ScrImportDdl(DataView dvd, DataRowView drv, bool bComboBox, string PKey, string CKey, string CNam, int MaxLen, string MatchCd, bool bAllowNulls)
		{
			if (dvd != null)
			{
				...
				{
					if (string.IsNullOrEmpty(cAdmColHlp1006List.SelectedValue))
					{
						dvd.RowFilter = "(" + PKey + " is null)";
					}
					else
					{
						dvd.RowFilter = "(" + PKey + " is null OR " + PKey + " = " + cAdmColHlp1006List.SelectedValue + ")";
					}
				}
		...
		protected void ScrImport(bool bClear)
		{
			...
			DataTable dtAdmColHlpGrid = (DataTable)Session[KEY_dtAdmColHlpGrid];
			if (dti != null && dtAdmColHlpGrid != null)
			{
				if (bClear)
				{
					dtAdmColHlpGrid.Rows.Clear();
				}
				cFind.Text = string.Empty;
				int iExist = dtAdmColHlpGrid.Rows.Count;
				...
				foreach (DataRowView drv in dti.DefaultView)
				{
					...
					{
						DataTable dtd;
						dtd = jj==iExist ? null : (DataTable)Session[KEY_dtCultureId21];
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(1006,"GetDdlCultureId3S3255",true,true,0,string.Empty,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
							Session[KEY_dtCultureId21] = dtd;
						}
						ScrImportDdl(dtd.DefaultView, drv, true, "ScreenObjId", "CultureId21", "CultureId21Text", 50, "", false);
					}
					jj = jj + 1;
				}
				iRow = dti.DefaultView.Count - iExist;
				if (!dti.Columns.Contains("_NewRow")) { dti.Columns.Add("_NewRow"); }
				Session[KEY_dtAdmColHlpGrid] = dti;
				cAdmColHlpGrid_DataBind(dti.DefaultView);
		...
		public void cImportButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cFNameO.Text != string.Empty && cWorkSheet.Items.Count > 0 && cWorkSheet.SelectedItem.Text != string.Empty && cStartRow.Text != string.Empty)
			{
				...
				try
				{
					if (cAdmColHlpGrid.EditIndex < 0 || UpdateGridRow(cAdmColHlpGrid, new CommandEventArgs("Save", "")))
					{
						Session[KEY_scrImport] = ImportFile(dtAdmColHlpGrid.Copy(),cFNameO.Text,cWorkSheet.SelectedItem.Text,cStartRow.Text,Config.PathTmpImport + cFName.Text);
					}
		...
		private void GotoPage(int pageNo)
		{
			if (pageNo >= 0 && pageNo < GetTotalPages())
			{
				if (cAdmColHlpGrid.EditIndex < 0 || UpdateGridRow(cAdmColHlpGrid, new CommandEventArgs("Save", "")))
				{
					try { cAdmColHlpGridDataPager.SetPageProperties(cAdmColHlpGridDataPager.PageSize * pageNo, cAdmColHlpGridDataPager.MaximumRows, false); cAdmColHlpGrid_DataBind(null); }
					catch
					{
						try { cAdmColHlpGridDataPager.SetPageProperties(0, cAdmColHlpGridDataPager.MaximumRows, false); cAdmColHlpGrid_DataBind(null); } catch {}
					}
				}
			}
		}

		protected void cAdmColHlpGrid_OnItemCommand(object sender, ListViewCommandEventArgs e)
		{
		}

		private void cAdmColHlpGrid_DataBind(DataView dvAdmColHlpGrid)
		{
			if (dvAdmColHlpGrid == null)
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmColHlpGrid];
				dvAdmColHlpGrid = dt != null ? dt.DefaultView : null;
			}
			if (dvAdmColHlpGrid != null)
			{
				LcAuth = GetAuthCol().AsEnumerable().ToDictionary<DataRow,string>(dr=>dr["ColName"].ToString());
				cAdmColHlpGrid.DataSource = dvAdmColHlpGrid;
				cAdmColHlpGrid.DataBind();
		...
		protected void cAdmColHlpGrid_OnSorting(object sender, ListViewSortEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmColHlpGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null)
			{
				if (cAdmColHlpGrid.EditIndex < 0 || UpdateGridRow(cAdmColHlpGrid, new CommandEventArgs("Save", "")))
				{
					...
					Session[KEY_dtAdmColHlpGrid] = dt;
					cAdmColHlpGrid_DataBind(dv);
				}
			}
		}

		protected void cAdmColHlpGrid_OnPagePropertiesChanging(object sender, PagePropertiesChangingEventArgs e)
		{
		    cAdmColHlpGridDataPager.SetPageProperties(e.StartRowIndex, e.MaximumRows, false);
		    DataTable dt = (DataTable)Session[KEY_dtAdmColHlpGrid];
		    DataView dv = dt != null ? dt.DefaultView : null;
			cAdmColHlpGrid_DataBind(dv); cAdmColHlpGrid.EditIndex = -1;
		}

		private void GridChkPgDirty(ListViewItem lvi)
		{
				WebControl cc = null; System.Web.UI.WebControls.Image ib = null;
				cc = ((WebControl)lvi.FindControl("cCultureId21"));
				...
				cc = ((WebControl)lvi.FindControl("cColumnHeader21"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				ib = ((System.Web.UI.WebControls.Image)lvi.FindControl("cColumnHeader21E"));
				ib.Attributes["target_id"] = cc.ClientID;
				cc = ((WebControl)lvi.FindControl("cToolTip21"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cErrMessage21"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cTbHint21"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				ib = ((System.Web.UI.WebControls.Image)lvi.FindControl("cTbHint21E"));
				ib.Attributes["target_id"] = cc.ClientID;
		}

		protected void cAdmColHlpGrid_OnItemDataBound(object sender, ListViewItemEventArgs e)
		{
			// *** GridItemDataBound (before) Web Rule End *** //
			DataTable dt = (DataTable)Session[KEY_dtAdmColHlpGrid];
			bool isEditItem = false;
			DataView dvAdmColHlpGrid = dt != null ? dt.DefaultView : null;
			if (cAdmColHlpGrid.EditIndex > -1 && GetDataItemIndex(cAdmColHlpGrid.EditIndex) == e.Item.DataItemIndex)
			{
				isEditItem = true;
				base.SetGridEnabled(e.Item, GetAuthCol(), GetLabel(), 1);
				if (dvAdmColHlpGrid[e.Item.DataItemIndex]["CultureId21"].ToString() != string.Empty) {SetCultureId21((RoboCoder.WebControls.ComboBox)e.Item.FindControl("cCultureId21"),dvAdmColHlpGrid[e.Item.DataItemIndex]["CultureId21"].ToString(), e.Item);} else {SetCultureId21((RoboCoder.WebControls.ComboBox)e.Item.FindControl("cCultureId21"),"1", e.Item);}
			...
			if (dtAuthRow != null)
			{
				...
				{
					cc = e.Item.FindControl("cAdmColHlpGridDelete"); if (cc != null) { cc.Visible = false; }
					cc = e.Item.FindControl("cAdmColHlpGridEdit"); if (cc != null) { cc.Visible = false; }
				}
				else
				{
			        HtmlTableRow tr = e.Item.FindControl("cAdmColHlpGridRow") as HtmlTableRow;
			        LinkButton lb = e.Item.FindControl("cAdmColHlpGridEdit") as LinkButton;
		...
		protected void cScreenObjHlpId21hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "0") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "0"; Session[KEY_lastSortExp] = "ScreenObjHlpId21";Session[KEY_lastSortImg] = "cScreenObjHlpId21hi";
			cAdmColHlpGrid_OnSorting(sender, new ListViewSortEventArgs("ScreenObjHlpId21", SortDirection.Ascending));
		}

		protected void cCultureId21hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "1") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "1"; Session[KEY_lastSortExp] = "CultureId21Text";Session[KEY_lastSortImg] = "cCultureId21hi";
			cAdmColHlpGrid_OnSorting(sender, new ListViewSortEventArgs("CultureId21Text", SortDirection.Ascending));
		}

		protected void cColumnHeader21hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "2") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "2"; Session[KEY_lastSortExp] = "ColumnHeader21";Session[KEY_lastSortImg] = "cColumnHeader21hi";
			cAdmColHlpGrid_OnSorting(sender, new ListViewSortEventArgs("ColumnHeader21", SortDirection.Ascending));
		}

		protected void cToolTip21hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "3") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "3"; Session[KEY_lastSortExp] = "ToolTip21";Session[KEY_lastSortImg] = "cToolTip21hi";
			cAdmColHlpGrid_OnSorting(sender, new ListViewSortEventArgs("ToolTip21", SortDirection.Ascending));
		}

		protected void cErrMessage21hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "4") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "4"; Session[KEY_lastSortExp] = "ErrMessage21";Session[KEY_lastSortImg] = "cErrMessage21hi";
			cAdmColHlpGrid_OnSorting(sender, new ListViewSortEventArgs("ErrMessage21", SortDirection.Ascending));
		}

		protected void cTbHint21hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "5") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "5"; Session[KEY_lastSortExp] = "TbHint21";Session[KEY_lastSortImg] = "cTbHint21hi";
			cAdmColHlpGrid_OnSorting(sender, new ListViewSortEventArgs("TbHint21", SortDirection.Ascending));
		}

		protected void cAdmColHlpGrid_OnItemEditing(object sender, ListViewEditEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmColHlpGrid];
			if ((ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")) && dt != null && dt.DefaultView.Count > GetDataItemIndex(e.NewEditIndex))
			{
				cAdmColHlpGrid.EditIndex = e.NewEditIndex;
				cAdmColHlpGrid_DataBind(null);
				Control wc = null;
				string idx = Page.Request["__EVENTARGUMENT"];
				if (!string.IsNullOrEmpty(idx))
				{
				    Control ctrlToFocus = cAdmColHlpGrid.EditItem.FindControl("c" + idx);
				    if (((WebControl)ctrlToFocus).Enabled) { wc = ctrlToFocus; }
				}
				if (wc == null) { wc = FindEditableControl(cAdmColHlpGrid.EditItem); }
				if (wc != null)
             {
             ...
             MkMobileLabel(cAdmColHlpGrid.EditItem);
		    }
		    else { e.Cancel = true; }
		}
		...
		{
		    Label ml = null;
		    ml = lvi.FindControl("cScreenObjHlpId21ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(1); }
		    ml = lvi.FindControl("cCultureId21ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(2); }
		    ml = lvi.FindControl("cColumnHeader21ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(3); }
		    ml = lvi.FindControl("cToolTip21ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(4); }
		    ml = lvi.FindControl("cErrMessage21ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(5); }
		    ml = lvi.FindControl("cTbHint21ml") as Label;
		...
		protected void GridFill(ListViewItem lvi, DataTable dt, DataRow dr, bool bInsert)
		{
			...
			cbb = (RoboCoder.WebControls.ComboBox)lvi.FindControl("cCultureId21");
			if (cbb != null)
			{
				if (cbb.SelectedValue != string.Empty)
				{
					dr["CultureId21"] = cbb.SelectedValue;
					dr["CultureId21Text"] = cbb.SelectedText;
				}
				else
				{
					dr["CultureId21"] = System.DBNull.Value;
					dr["CultureId21Text"] = System.DBNull.Value;
				}
			}
			TextBox tb = null;
			tb = (TextBox)lvi.FindControl("cColumnHeader21");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["ColumnHeader21"] = tb.Text;} else {dr["ColumnHeader21"] = System.DBNull.Value;}
			}
			tb = (TextBox)lvi.FindControl("cToolTip21");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["ToolTip21"] = tb.Text;} else {dr["ToolTip21"] = System.DBNull.Value;}
			}
			tb = (TextBox)lvi.FindControl("cErrMessage21");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["ErrMessage21"] = tb.Text;} else {dr["ErrMessage21"] = System.DBNull.Value;}
			}
			tb = (TextBox)lvi.FindControl("cTbHint21");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["TbHint21"] = tb.Text;} else {dr["TbHint21"] = System.DBNull.Value;}
			}
		...
		protected void cAdmColHlpGrid_OnItemUpdating(object sender, ListViewUpdateEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmColHlpGrid];
			if (dt != null && dt.Rows.Count > 0)
			{
				DataRow dr = dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row;
			    GridFill(cAdmColHlpGrid.EditItem, dt, dr, false);
			    dr["_NewRow"] = "N";
			    if (dt.Columns.Contains("_SortOrder")) dr["_SortOrder"] = null;
			}
		}

		protected void cAdmColHlpGrid_OnItemCanceling(object sender, ListViewCancelEventArgs e)
		{
			PanelTop.Update();
			DataTable dt = (DataTable)Session[KEY_dtAdmColHlpGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			...
			{
				cAdmColHlpGrid_OnItemDeleting(sender, new ListViewDeleteEventArgs(e.ItemIndex));
			}
			if (dt.Columns.Contains("_SortOrder")) dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row["_SortOrder"] = null;
			cAdmColHlpGrid.EditIndex = -1; cAdmColHlpGrid_DataBind(null);
		}
		...
		protected void cDeleteAllButton_Click(object sender, System.EventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmColHlpGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null && (cAdmColHlpGrid.EditIndex < 0 || UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				GotoPage(0);
				while (dv.Count > 0) {dv.Delete(0);}
				Session[KEY_dtAdmColHlpGrid] = dt;
				cAdmColHlpGrid.EditIndex = -1; cAdmColHlpGrid_DataBind(dv);
			}
		}

		protected void cAdmColHlpGrid_OnItemDeleting(object sender, ListViewDeleteEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmColHlpGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			DataRow dr = dv != null ? dv[GetDataItemIndex(e.ItemIndex)].Row : null;
			if (dv != null && (cAdmColHlpGrid.EditIndex == e.ItemIndex || UpdateGridRow(sender, new CommandEventArgs("Save",""))))
			{
		...
		public void cUndoAllButton_Click(object sender, System.EventArgs e)
		{
			...
			else { PopAdmColHlp1006List(sender, e, false, cScreenObjId14.Text); }
			// *** System Button Click (After) Web Rule starts here *** //
		}
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmColHlp1006List); PopAdmColHlp1006List(sender, e, false, null);
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			if (ValidPage() && (cAdmColHlpGrid.EditIndex < 0 || UpdateGridRow(cAdmColHlpGrid, new CommandEventArgs("Save", ""))))
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmColHlpGrid];
				DataView dv = dt != null ? dt.DefaultView : null;
				string ftr = string.Empty;
				if (dv.RowFilter != string.Empty) { ftr = dv.RowFilter; dv.RowFilter = string.Empty; }
				AdmColHlp1006 ds = PrepAdmColHlpData(dv,cScreenObjId14.Text == string.Empty);
				if (ftr != string.Empty) {dv.RowFilter = ftr;}
				if (!string.IsNullOrEmpty(cAdmColHlp1006List.SelectedValue) && cScreenObjId14.Text != cAdmColHlp1006List.SelectedValue)
				{
					PreMsgPopup("Primary key cannot be changed");
					return rtn;
				}
				if (string.IsNullOrEmpty(cAdmColHlp1006List.SelectedValue))	// Add
				{
					if (ds != null)
					{
						pid = (new AdminSystem()).AddData(1006,true,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc,noTrans,commandTimeOut);
					}
					if (!string.IsNullOrEmpty(pid))
					{
						cAdmColHlp1006List.ClearSearch(); Session.Remove(KEY_dtAdmColHlp1006List);
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmColHlp1006List(sender, e, false, pid);
						rtn = GetScreenHlp().Rows[0]["AddMsg"].ToString();
					}
				}
				else {
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(1006,true,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc,noTrans,commandTimeOut)) {bValid7 = true;}
					if (bValid7)
					{
						cAdmColHlp1006List.ClearSearch(); Session.Remove(KEY_dtAdmColHlp1006List);
						Session[KEY_currPageIndex] = GetCurrPageIndex();
						ShowDirty(false); PopAdmColHlp1006List(sender, e, false, ds.Tables["AdmColHlp"].Rows[0]["ScreenObjId14"]);
						rtn = GetScreenHlp().Rows[0]["UpdMsg"].ToString();
					}
				}
			}
			//WebRule: Synchronize the label with the controlling object
            DataTable dtHlp = (new AdminSystem()).GetDtlById(10, "WrGetScreenObjById", cScreenObjId14.Text, (string)Session[KEY_sysConnectionString], LcAppPw, 0, base.LImpr, base.LCurr);
		...
		public void cDeleteButton_Click(object sender, System.EventArgs e)
		{
			...
			{
				AdmColHlp1006 ds = PrepAdmColHlpData(null,false);
				try
				{
					if (ds != null && (new AdminSystem()).DelData(1006,true,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc))
					{
						cAdmColHlp1006List.ClearSearch(); Session.Remove(KEY_dtAdmColHlp1006List);
						ShowDirty(false); PopAdmColHlp1006List(sender, e, false, null);
		...
		private AdmColHlp1006 PrepAdmColHlpData(DataView dv, bool bAdd)
		{
			...
			if (dv != null)
			{
				...
				if (bAdd)
				{
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmColHlpAdd"].Rows.Add(MakeColRow(ds.Tables["AdmColHlpAdd"].NewRow(), drv, true));
					}
				}
				else
				{
					...
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmColHlpUpd"].Rows.Add(MakeColRow(ds.Tables["AdmColHlpUpd"].NewRow(), drv, false));
					}
					...
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmColHlpDel"].Rows.Add(MakeColRow(ds.Tables["AdmColHlpDel"].NewRow(), drv, false));
					}
		...
		public bool CanAct(char typ) { return CanAct(typ, GetAuthRow(), cAdmColHlp1006List.SelectedValue.ToString()); }
		...
		private bool ValidPage()
		{
			...
			DataTable dt = null;
			dt = (DataTable)Session[KEY_dtCultureId21];
			if (dt != null && dt.Rows.Count <= 0)
			{
		...
		private DataRow MakeColRow(DataRow dr, DataRowView drv, bool bAdd)
		{
			...
			if (dtAuth != null)
			{
				dr["ScreenObjHlpId21"] = drv["ScreenObjHlpId21"].ToString().Trim();
				dr["CultureId21"] = drv["CultureId21"];
				dr["ColumnHeader21"] = drv["ColumnHeader21"];
				if (bAdd && dtAuth.Rows[3]["ColReadOnly"].ToString() == "Y" && dr["ColumnHeader21"].ToString() == string.Empty) {dr["ColumnHeader21"] = System.DBNull.Value;}
				dr["ToolTip21"] = drv["ToolTip21"].ToString().Trim();
				if (bAdd && dtAuth.Rows[4]["ColReadOnly"].ToString() == "Y" && dr["ToolTip21"].ToString() == string.Empty) {dr["ToolTip21"] = System.DBNull.Value;}
				dr["ErrMessage21"] = drv["ErrMessage21"].ToString().Trim();
				if (bAdd && dtAuth.Rows[5]["ColReadOnly"].ToString() == "Y" && dr["ErrMessage21"].ToString() == string.Empty) {dr["ErrMessage21"] = System.DBNull.Value;}
				dr["TbHint21"] = drv["TbHint21"];
				if (bAdd && dtAuth.Rows[6]["ColReadOnly"].ToString() == "Y" && dr["TbHint21"].ToString() == string.Empty) {dr["TbHint21"] = System.DBNull.Value;}
			}
			return dr;
		}
		...
		{
			if (cAdmColHlpGrid.EditIndex > -1)
			{
				cAdmColHlpGrid_OnItemUpdating(sender, new ListViewUpdateEventArgs(cAdmColHlpGrid.EditIndex));
			}
			return true;
		}

		protected void cAdmColHlpGrid_OnPreRender(object sender, System.EventArgs e)
		{
			System.Web.UI.WebControls.Image hi = null;
			hi = (System.Web.UI.WebControls.Image)cAdmColHlpGrid.FindControl("cScreenObjHlpId21hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmColHlpGrid.FindControl("cCultureId21hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmColHlpGrid.FindControl("cColumnHeader21hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmColHlpGrid.FindControl("cToolTip21hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmColHlpGrid.FindControl("cErrMessage21hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmColHlpGrid.FindControl("cTbHint21hi"); if (hi != null) { hi.Visible = false; }
			if (Session[KEY_lastSortImg] != null)
			{
				hi = (System.Web.UI.WebControls.Image)cAdmColHlpGrid.FindControl((string)Session[KEY_lastSortImg]);
				if (hi != null) { hi.ImageUrl = Utils.AddTilde((string)Session[KEY_lastSortUrl]); hi.Visible = true; }
			}
			IgnoreHeaderConfirm((LinkButton)cAdmColHlpGrid.FindControl("cScreenObjHlpId21hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmColHlpGrid.FindControl("cCultureId21hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmColHlpGrid.FindControl("cColumnHeader21hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmColHlpGrid.FindControl("cToolTip21hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmColHlpGrid.FindControl("cErrMessage21hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmColHlpGrid.FindControl("cTbHint21hl"));
		}

		private string GetButtonId(ListViewItem lvi)
		{
			string ButtonID = String.Empty;
			Control c = lvi.FindControl("cAdmColHlpGridEdit");
		...
		{
		    return cAdmColHlpGridDataPager.StartRowIndex / cAdmColHlpGridDataPager.PageSize;
		}

		private int GetTotalPages()
		{
		    return (int)Math.Ceiling((double)cAdmColHlpGridDataPager.TotalRowCount / cAdmColHlpGridDataPager.PageSize);
		}

		private int GetDataItemIndex(int editIndex)
		{
		    return cAdmColHlpGridDataPager.StartRowIndex + editIndex;
		}

		protected void cAdmColHlpGrid_OnLayoutCreated(object sender, EventArgs e)
		{
		    // Header:
		    LinkButton lb = null;
		    lb = cAdmColHlpGrid.FindControl("cScreenObjHlpId21hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(1); lb.ToolTip = ColumnToolTip(1); lb.Parent.Visible = GridColumnVisible(1); }
		    lb = cAdmColHlpGrid.FindControl("cCultureId21hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(2); lb.ToolTip = ColumnToolTip(2); lb.Parent.Visible = GridColumnVisible(2); }
		    lb = cAdmColHlpGrid.FindControl("cColumnHeader21hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(3); lb.ToolTip = ColumnToolTip(3); lb.Parent.Visible = GridColumnVisible(3); }
		    lb = cAdmColHlpGrid.FindControl("cToolTip21hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(4); lb.ToolTip = ColumnToolTip(4); lb.Parent.Visible = GridColumnVisible(4); }
		    lb = cAdmColHlpGrid.FindControl("cErrMessage21hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(5); lb.ToolTip = ColumnToolTip(5); lb.Parent.Visible = GridColumnVisible(5); }
		    lb = cAdmColHlpGrid.FindControl("cTbHint21hl") as LinkButton;
			...
			if (dtAuthRow != null)
			{
				...
				{
		            lb = cAdmColHlpGrid.FindControl("cDeleteAllButton") as LinkButton; if (lb != null) { lb.Visible = false; }
				}
			}
		    // footer:
		    Label gc = null;
		    gc = cAdmColHlpGrid.FindControl("cScreenObjHlpId21fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(1); }
		    gc = cAdmColHlpGrid.FindControl("cCultureId21fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(2); }
		    gc = cAdmColHlpGrid.FindControl("cColumnHeader21fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(3); }
		    gc = cAdmColHlpGrid.FindControl("cToolTip21fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(4); }
		    gc = cAdmColHlpGrid.FindControl("cErrMessage21fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(5); }
		    gc = cAdmColHlpGrid.FindControl("cTbHint21fl") as Label;
		...
		private void PreMsgPopup(string msg) { PreMsgPopup(msg,cAdmColHlp1006List,null); }

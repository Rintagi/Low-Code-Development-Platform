	...
	public partial class AdmColHlpModule : RO.Web.ModuleBase
	{
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				try
				{
					try {dv = new DataView((new AdminSystem()).GetExp(1006,"GetExpAdmColHlp1006","Y",(string)Session[KEY_sysConnectionString],LcAppPw,filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
					catch {dv = new DataView((new AdminSystem()).GetExp(1006,"GetExpAdmColHlp1006","N",(string)Session[KEY_sysConnectionString],LcAppPw,filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
...
\par }}");
		...
		private void SetCultureId21(RoboCoder.WebControls.ComboBox ddl, string keyId, ListViewItem li)
		{
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(1006,"GetDdlCultureId3S3255",true,false,0,keyId,(string)Session[KEY_sysConnectionString],LcAppPw,string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
		...
		private DataView GetAdmColHlp1006List()
		{
			...
			if (dt == null)
			{
				...
				try
				{
					try {dt = (new AdminSystem()).GetLis(1006,"GetLisAdmColHlp1006",true,"Y",0,(string)Session[KEY_sysConnectionString],LcAppPw,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
					catch {dt = (new AdminSystem()).GetLis(1006,"GetLisAdmColHlp1006",true,"N",0,(string)Session[KEY_sysConnectionString],LcAppPw,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				}
		...
		protected void cAdmColHlp1006List_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			...
			try
			{
				dt = (new AdminSystem()).GetMstById("GetAdmColHlp1006ById",cAdmColHlp1006List.SelectedValue,(string)Session[KEY_sysConnectionString],LcAppPw);
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
			...
			try
			{
				dtAdmColHlpGrid = (new AdminSystem()).GetDtlById(1006,"GetAdmColHlp1006DtlById",cAdmColHlp1006List.SelectedValue,(string)Session[KEY_sysConnectionString],LcAppPw,filterId,base.LImpr,base.LCurr);
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				...
				foreach (DataRowView drv in dti.DefaultView)
				{
					if ((drv.Row.RowState == System.Data.DataRowState.Added || drv.Row.RowState == System.Data.DataRowState.Detached) && jj >= iExist)
					{
						...
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(1006,"GetDdlCultureId3S3255",true,true,0,string.Empty,(string)Session[KEY_sysConnectionString],LcAppPw,string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			if (ValidPage() && (cAdmColHlpGrid.EditIndex < 0 || UpdateGridRow(cAdmColHlpGrid, new CommandEventArgs("Save", ""))))
			{
				...
				if (string.IsNullOrEmpty(cAdmColHlp1006List.SelectedValue))	// Add
				{
					if (ds != null)
					{
						pid = (new AdminSystem()).AddData(1006,true,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],LcAppPw,base.CPrj,base.CSrc);
					}
					...
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(1006,true,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],LcAppPw,base.CPrj,base.CSrc)) {bValid7 = true;}
					if (bValid7)
					{
						cAdmColHlp1006List.ClearSearch(); Session.Remove(KEY_dtAdmColHlp1006List);
		...
		public void cDeleteButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cScreenObjId14.Text != string.Empty)
			{
				...
				try
				{
					if (ds != null && (new AdminSystem()).DelData(1006,true,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],LcAppPw,base.CPrj,base.CSrc))
					{
						cAdmColHlp1006List.ClearSearch(); Session.Remove(KEY_dtAdmColHlp1006List);
		...
		private bool UpdateGridRow(object sender, CommandEventArgs e)
		{
			...
			{
				TextBox pwd = null;				cAdmColHlpGrid_OnItemUpdating(sender, new ListViewUpdateEventArgs(cAdmColHlpGrid.EditIndex));
			}
			return true;
		}

...
namespace RO.Web
{
	public partial class AdmRulTierModule : RO.Web.ModuleBase
	{
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_106";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_106";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_106";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_106";
		private const string KEY_dtLabel = "Cache:dtLabel3_106";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_106";
		private const string KEY_dtScrCri = "Cache:dtScrCri3_106";
		private const string KEY_dsScrCriVal = "Cache:dsScrCriVal3_106";

		private const string KEY_dtEntityId196 = "Cache:dtEntityId196";
		private const string KEY_dtLanguageCd196 = "Cache:dtLanguageCd196";
		private const string KEY_dtFrameworkCd196 = "Cache:dtFrameworkCd196";

		private const string KEY_dtSystems = "Cache:dtSystems3_106";
		private const string KEY_sysConnectionString = "Cache:sysConnectionString3_106";
		private const string KEY_dtAdmRulTier106List = "Cache:dtAdmRulTier106List";
		private const string KEY_bNewVisible = "Cache:bNewVisible3_106";
		private const string KEY_bNewSaveVisible = "Cache:bNewSaveVisible3_106";
		private const string KEY_bCopyVisible = "Cache:bCopyVisible3_106";
		private const string KEY_bCopySaveVisible = "Cache:bCopySaveVisible3_106";
		private const string KEY_bDeleteVisible = "Cache:bDeleteVisible3_106";
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_106";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_106";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_106";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				DataTable dtMenuAccess = (new MenuSystem()).GetMenu(base.LUser.CultureId, 3, base.LImpr, LcSysConnString, LcAppPw,106, null, null);
				...
				Session.Remove(KEY_dtAdmRulTier106List);
				...
				Session.Remove(KEY_dtEntityId196);
				Session.Remove(KEY_dtLanguageCd196);
				Session.Remove(KEY_dtFrameworkCd196);
				SetButtonHlp();
				GetSystems();
				SetColumnAuthority();
				GetGlobalFilter();
				...
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 106, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmRulTier106List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=106&typ=N&sys=3','','',''); return false;"; }
		...
		private void SetSystem(byte SystemId)
		{
			...
			try
			{
				...
				if ((Config.DeployType == "DEV" || row["dbAppDatabase"].ToString() == base.CPrj.EntityCode + "View") && !(base.CPrj.EntityCode != "RO" && row["SysProgram"].ToString() == "Y") && (new AdminSystem()).IsRegenNeeded(string.Empty,106,0,0,LcSysConnString,LcAppPw))
				{
					(new GenScreensSystem()).CreateProgram(106, "Rule Tier", row["dbAppDatabase"].ToString(), base.CPrj, base.CSrc, base.CTar, LcAppConnString, LcAppPw);
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(106,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(106,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(106,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(106,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,106,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(106,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(106,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(106,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		protected DataTable GetAuthRow()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthRow(106,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				try
				{
					try {dv = new DataView((new AdminSystem()).GetExp(106,"GetExpAdmRulTier106","Y",null,null,filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
					catch {dv = new DataView((new AdminSystem()).GetExp(106,"GetExpAdmRulTier106","N",null,null,filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(106,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					{
						if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["RuleTierId196"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["RuleTierName196"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(drv["EntityId196"].ToString() + (char)9 + drv["EntityId196Text"].ToString() + (char)9);}
						if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["LanguageCd196"].ToString().Replace("\"","\"\"") + "\"" + (char)9 + "\"" + drv["LanguageCd196Text"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["FrameworkCd196"].ToString().Replace("\"","\"\"") + "\"" + (char)9 + "\"" + drv["FrameworkCd196Text"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["DevProgramPath196"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["IsDefault196"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						sb.Append(Environment.NewLine);
					}
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmRulTier.csv"; Session["ExportStr"] = (Config.ExportExcelCSV ? "sep=\t\n": "") + sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmRulTier.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private string GetRtfGrid(DataView dv)
		{
			...
			try
			{
				...
				foreach (DataRowView drv in dv)
				{
					...
					sb.Append("\r\n");
					sb.Append(@"{");
					if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["RuleTierId196"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["RuleTierName196"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(drv["EntityId196Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(drv["LanguageCd196Text"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append(drv["FrameworkCd196Text"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append(drv["DevProgramPath196"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append(drv["IsDefault196"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					sb.Append(@"}");
					sb.Append("\r\n");
					sb.Append(@"\pard \ql \li0\ri0\widctlpar\intbl\aspalpha\aspnum\adjustright\rin0\lin0 {");
					sb.Append(@"\insrsid2981395 \trowd \irow0\irowband0\lastrow \ts15\trgaph108\trleft-108\trbrdrt\brdrs\brdrw10 \trbrdrl\brdrs\brdrw10 \trbrdrb\brdrs\brdrw10 \trbrdrr\brdrs\brdrw10 \trbrdrh\brdrs\brdrw10 \trbrdrv\brdrs\brdrw10 ");
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 106, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			...
			UpdCriteria(true);
			cAdmRulTier106List.ClearSearch(); Session.Remove(KEY_dtAdmRulTier106List);
			PopAdmRulTier106List(sender, e, false, null);
		}
			...
			try
			{
				foreach (DataRowView drv in dvCri)
				{
					...
					if (drv["DisplayName"].ToString() == "ComboBox")
					{
						...
						(new AdminSystem()).MkGetScreenIn("106", drv["ScreenCriId"].ToString(), "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), LcAppDb, LcDesDb, drv["MultiDesignDb"].ToString(), LcSysConnString, LcAppPw);
						DataView dv = new DataView((new AdminSystem()).GetScreenIn("106", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), (new AdminSystem()).CountScrCri(drv["ScreenCriId"].ToString(), drv["MultiDesignDb"].ToString(), drv["MultiDesignDb"].ToString() == "N" ? LcSysConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)), drv["RequiredValid"].ToString(), 0, string.Empty, drv["DisplayMode"].ToString() != "AutoListBox", val, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)));
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("106", LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmRulTier106" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmRulTier106" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			foreach (DataRowView drv in dvCri)
			{
			    if (drv["DisplayName"].ToString() == "ListBox")
			    {
					...
					if (cListBox != null)
					{
						...
						int TotalChoiceCnt = new DataView((new AdminSystem()).GetScreenIn("106", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), CriCnt, drv["RequiredValid"].ToString(), 0, string.Empty, true, string.Empty, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId))).Count;
						string selectedValues = string.Join(",", cListBox.Items.Cast<ListItem>().Where(x => x.Selected).Select(x => "'" + x.Value + "'").ToArray());
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("106", "AdmRulTier106", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		private void SetEntityId196(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtEntityId196];
			...
			if (ddl != null)
			{
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(106,"GetDdlEntityId3S1822",true,bAll,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					if (dv.Table.Columns.Contains("RuleTierId"))
					{
						ss = "(RuleTierId is null";
						if (string.IsNullOrEmpty(cAdmRulTier106List.SelectedValue)) {ss = ss + ")";} else {ss = ss + " OR RuleTierId = " + cAdmRulTier106List.SelectedValue + ")";}
					}
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "EntityId196 is not NULL"; }
					dv.RowFilter = ss;
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(106,"GetDdlEntityId3S1822",true,true,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtEntityId196] = dv.Table;
				}
			}
		}

		private void SetLanguageCd196(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtLanguageCd196];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (ddl != null)
			{
				string ss = string.Empty;
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(106,"GetDdlLanguageCd3S1823",true,bAll,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					...
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "LanguageCd196 is not NULL"; }
					dv.RowFilter = ss;
					ddl.DataSource = dv; ddl.DataBind();
					li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					else if (!bFirst)
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(106,"GetDdlLanguageCd3S1823",true,true,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtLanguageCd196] = dv.Table;
				}
			}
		}

		private void SetFrameworkCd196(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtFrameworkCd196];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (ddl != null)
			{
				string ss = string.Empty;
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(106,"GetDdlFrameworkCd3S1824",true,bAll,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					...
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "FrameworkCd196 is not NULL"; }
					dv.RowFilter = ss;
					ddl.DataSource = dv; ddl.DataBind();
					li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					else if (!bFirst)
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(106,"GetDdlFrameworkCd3S1824",true,true,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtFrameworkCd196] = dv.Table;
				}
			}
		}

		private DataView GetAdmRulTier106List()
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmRulTier106List];
			if (dt == null)
			{
				CheckAuthentication(false);
				int filterId = 0;
				...
				try
				{
					try {dt = (new AdminSystem()).GetLis(106,"GetLisAdmRulTier106",true,"Y",0,null,null,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
					catch {dt = (new AdminSystem()).GetLis(106,"GetLisAdmRulTier106",true,"N",0,null,null,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				}
		...
		private void PopAdmRulTier106List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			DataView dv = GetAdmRulTier106List();
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			cAdmRulTier106List.AutoCompleteUrl = "AutoComplete.aspx/LisSuggests";
			cAdmRulTier106List.DataContext = context;
			if (dv.Table == null) return;
			cAdmRulTier106List.DataSource = dv;
			cAdmRulTier106List.Visible = true;
			if (cAdmRulTier106List.Items.Count <= 0) {cAdmRulTier106List.Visible = false; cAdmRulTier106List_SelectedIndexChanged(sender,e);}
			else
			{
				if (bPageLoad && !string.IsNullOrEmpty(Request.QueryString["key"]) && bUseCri.Value == string.Empty)
				{
					try {cAdmRulTier106List.SelectByValue(Request.QueryString["key"],string.Empty,true);} catch {cAdmRulTier106List.Items[0].Selected = true; cAdmRulTier106List_SelectedIndexChanged(sender, e);}
				    cScreenSearch.Visible = false; cSystem.Visible = false; cEditButton.Visible = true; cSaveCloseButton.Visible = cSaveButton.Visible;
				}
				else
				{
					if (ID != null) {if (!cAdmRulTier106List.SelectByValue(ID,string.Empty,true)) {ID = null;}}
					if (ID == null)
					{
						cAdmRulTier106List_SelectedIndexChanged(sender, e);
					}
				}
			}
		}
		...
		private void SetColumnAuthority()
		{
			...
			{
				base.SetFoldBehavior(cRuleTierId196, dtAuth.Rows[0], cRuleTierId196P1, cRuleTierId196Label, cRuleTierId196P2, null, dtLabel.Rows[0], null, null, null);
				base.SetFoldBehavior(cRuleTierName196, dtAuth.Rows[1], cRuleTierName196P1, cRuleTierName196Label, cRuleTierName196P2, null, dtLabel.Rows[1], cRFVRuleTierName196, null, null);
				base.SetFoldBehavior(cEntityId196, dtAuth.Rows[2], cEntityId196P1, cEntityId196Label, cEntityId196P2, null, dtLabel.Rows[2], cRFVEntityId196, null, null);
				base.SetFoldBehavior(cLanguageCd196, dtAuth.Rows[3], cLanguageCd196P1, cLanguageCd196Label, cLanguageCd196P2, null, dtLabel.Rows[3], cRFVLanguageCd196, null, null);
				base.SetFoldBehavior(cFrameworkCd196, dtAuth.Rows[4], cFrameworkCd196P1, cFrameworkCd196Label, cFrameworkCd196P2, null, dtLabel.Rows[4], cRFVFrameworkCd196, null, null);
				base.SetFoldBehavior(cDevProgramPath196, dtAuth.Rows[5], cDevProgramPath196P1, cDevProgramPath196Label, cDevProgramPath196P2, null, dtLabel.Rows[5], cRFVDevProgramPath196, null, null);
				base.SetFoldBehavior(cIsDefault196, dtAuth.Rows[6], cIsDefault196P1, cIsDefault196Label, cIsDefault196P2, null, dtLabel.Rows[6], null, null, null);
			}
			if ((cRuleTierName196.Attributes["OnChange"] == null || cRuleTierName196.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cRuleTierName196.Visible && !cRuleTierName196.ReadOnly) {cRuleTierName196.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cEntityId196.Attributes["OnChange"] == null || cEntityId196.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cEntityId196.Visible && cEntityId196.Enabled) {cEntityId196.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cLanguageCd196.Attributes["OnChange"] == null || cLanguageCd196.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cLanguageCd196.Visible && cLanguageCd196.Enabled) {cLanguageCd196.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cFrameworkCd196.Attributes["OnChange"] == null || cFrameworkCd196.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cFrameworkCd196.Visible && cFrameworkCd196.Enabled) {cFrameworkCd196.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cDevProgramPath196.Attributes["OnChange"] == null || cDevProgramPath196.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cDevProgramPath196.Visible && !cDevProgramPath196.ReadOnly) {cDevProgramPath196.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cIsDefault196.Attributes["OnClick"] == null || cIsDefault196.Attributes["OnClick"].IndexOf("ChkPgDirty") < 0) && cIsDefault196.Visible && cIsDefault196.Enabled) {cIsDefault196.Attributes["OnClick"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty(); this.focus();";}
		}
		...
		private void ClearMaster(object sender, System.EventArgs e)
		{
			...
			if (dt.Rows[1]["ColVisible"].ToString() == "Y" && dt.Rows[1]["ColReadOnly"].ToString() != "Y") {cRuleTierName196.Text = string.Empty;}
			if (dt.Rows[2]["ColVisible"].ToString() == "Y" && dt.Rows[2]["ColReadOnly"].ToString() != "Y") {SetEntityId196(cEntityId196,string.Empty);}
			if (dt.Rows[3]["ColVisible"].ToString() == "Y" && dt.Rows[3]["ColReadOnly"].ToString() != "Y") {SetLanguageCd196(cLanguageCd196,string.Empty);}
			if (dt.Rows[4]["ColVisible"].ToString() == "Y" && dt.Rows[4]["ColReadOnly"].ToString() != "Y") {SetFrameworkCd196(cFrameworkCd196,string.Empty);}
			if (dt.Rows[5]["ColVisible"].ToString() == "Y" && dt.Rows[5]["ColReadOnly"].ToString() != "Y") {cDevProgramPath196.Text = string.Empty;}
			if (dt.Rows[6]["ColVisible"].ToString() == "Y" && dt.Rows[6]["ColReadOnly"].ToString() != "Y") {cIsDefault196.Checked = base.GetBool("N");}
			// *** Default Value (Folder) Web Rule starts here *** //
		}

		private void InitMaster(object sender, System.EventArgs e)
		{
			cRuleTierId196.Text = string.Empty;
			cRuleTierName196.Text = string.Empty;
			SetEntityId196(cEntityId196,string.Empty);
			SetLanguageCd196(cLanguageCd196,string.Empty);
			SetFrameworkCd196(cFrameworkCd196,string.Empty);
			cDevProgramPath196.Text = string.Empty;
			cIsDefault196.Checked = base.GetBool("N");
			// *** Default Value (Folder) Web Rule starts here *** //
		}
		protected void cAdmRulTier106List_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmRulTier106List_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmRulTier106List_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			InitMaster(sender, e);      // Do this first to enable defaults for non-database columns.
			DataTable dt = null;
			try
			{
				dt = (new AdminSystem()).GetMstById("GetAdmRulTier106ById",cAdmRulTier106List.SelectedValue,null,null);
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				if (dt.Rows.Count > 0)
				{
					cAdmRulTier106List.SetDdVisible();
					DataRow dr = dt.Rows[0];
					try {cRuleTierId196.Text = RO.Common3.Utils.fmNumeric("0",dr["RuleTierId196"].ToString(),base.LUser.Culture);} catch {cRuleTierId196.Text = string.Empty;}
					try {cRuleTierName196.Text = dr["RuleTierName196"].ToString();} catch {cRuleTierName196.Text = string.Empty;}
					SetEntityId196(cEntityId196,dr["EntityId196"].ToString());
					SetLanguageCd196(cLanguageCd196,dr["LanguageCd196"].ToString());
					SetFrameworkCd196(cFrameworkCd196,dr["FrameworkCd196"].ToString());
					try {cDevProgramPath196.Text = dr["DevProgramPath196"].ToString();} catch {cDevProgramPath196.Text = string.Empty;}
					try {cIsDefault196.Checked = base.GetBool(dr["IsDefault196"].ToString());} catch {cIsDefault196.Checked = false;}
				}
			}
			cButPanel.DataBind();
			ScriptManager.GetCurrent(Parent.Page).SetFocus(cAdmRulTier106List.FocusID);
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cRuleTierId196.Text == string.Empty) { cClearButton_Click(sender, new EventArgs()); ShowDirty(false); PanelTop.Update();}
			else { PopAdmRulTier106List(sender, e, false, cRuleTierId196.Text); }
			// *** System Button Click (After) Web Rule starts here *** //
		}
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmRulTier106List); PopAdmRulTier106List(sender, e, false, null);
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			if (ValidPage())
			{
				AdmRulTier106 ds = PrepAdmRulTierData(null,cRuleTierId196.Text == string.Empty);
				if (!string.IsNullOrEmpty(cAdmRulTier106List.SelectedValue) && cRuleTierId196.Text != cAdmRulTier106List.SelectedValue)
				{
					PreMsgPopup("Primary key cannot be changed");
					return rtn;
				}
				if (string.IsNullOrEmpty(cAdmRulTier106List.SelectedValue))	// Add
				{
					if (ds != null)
					{
						pid = (new AdminSystem()).AddData(106,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc,noTrans,commandTimeOut);
					}
					if (!string.IsNullOrEmpty(pid))
					{
						cAdmRulTier106List.ClearSearch(); Session.Remove(KEY_dtAdmRulTier106List);
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmRulTier106List(sender, e, false, pid);
						rtn = GetScreenHlp().Rows[0]["AddMsg"].ToString();
					}
				}
				else {
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(106,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc,noTrans,commandTimeOut)) {bValid7 = true;}
					if (bValid7)
					{
						cAdmRulTier106List.ClearSearch(); Session.Remove(KEY_dtAdmRulTier106List);
						ShowDirty(false); PopAdmRulTier106List(sender, e, false, ds.Tables["AdmRulTier"].Rows[0]["RuleTierId196"]);
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cRuleTierId196.Text != string.Empty)
			{
				AdmRulTier106 ds = PrepAdmRulTierData(null,false);
				try
				{
					if (ds != null && (new AdminSystem()).DelData(106,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc))
					{
						cAdmRulTier106List.ClearSearch(); Session.Remove(KEY_dtAdmRulTier106List);
						ShowDirty(false); PopAdmRulTier106List(sender, e, false, null);
		...
		private AdmRulTier106 PrepAdmRulTierData(DataView dv, bool bAdd)
		{
			...
			if (bAdd) { dr["RuleTierId196"] = string.Empty; } else { dr["RuleTierId196"] = cRuleTierId196.Text; }
			drType["RuleTierId196"] = "Numeric"; drDisp["RuleTierId196"] = "TextBox";
			try {dr["RuleTierName196"] = cRuleTierName196.Text.Trim();} catch {}
			drType["RuleTierName196"] = "VarWChar"; drDisp["RuleTierName196"] = "TextBox";
			try {dr["EntityId196"] = cEntityId196.SelectedValue;} catch {}
			drType["EntityId196"] = "Numeric"; drDisp["EntityId196"] = "DropDownList";
			try {dr["LanguageCd196"] = cLanguageCd196.SelectedValue;} catch {}
			drType["LanguageCd196"] = "Char"; drDisp["LanguageCd196"] = "DropDownList";
			try {dr["FrameworkCd196"] = cFrameworkCd196.SelectedValue;} catch {}
			drType["FrameworkCd196"] = "Char"; drDisp["FrameworkCd196"] = "DropDownList";
			try {dr["DevProgramPath196"] = cDevProgramPath196.Text.Trim();} catch {}
			drType["DevProgramPath196"] = "VarChar"; drDisp["DevProgramPath196"] = "TextBox";
			try {dr["IsDefault196"] = base.SetBool(cIsDefault196.Checked);} catch {}
		...
		public bool CanAct(char typ) { return CanAct(typ, GetAuthRow(), cAdmRulTier106List.SelectedValue.ToString()); }
		...
		private void PreMsgPopup(string msg) { PreMsgPopup(msg,cAdmRulTier106List,null); }

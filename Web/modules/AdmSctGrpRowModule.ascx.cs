...
{
	public class AdmSctGrpRow1004 : DataSet
	{
		public AdmSctGrpRow1004()
		{
			this.Tables.Add(MakeColumns(new DataTable("AdmSctGrpRow")));
			this.DataSetName = "AdmSctGrpRow1004";
			this.Namespace = "http://Rintagi.com/DataSet/AdmSctGrpRow1004";
		}

		private DataTable MakeColumns(DataTable dt)
		{
			DataColumnCollection columns = dt.Columns;
			columns.Add("SctGrpRowId1283", typeof(string));
			columns.Add("SectionCd1283", typeof(string));
			columns.Add("GroupRowId1283", typeof(string));
			columns.Add("SctGrpRowCss1283", typeof(string));
			return dt;
		}
	}
}
...
namespace RO.Web
{
	public partial class AdmSctGrpRowModule : RO.Web.ModuleBase
	{
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_1004";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_1004";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_1004";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_1004";
		private const string KEY_dtLabel = "Cache:dtLabel3_1004";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_1004";
		private const string KEY_dtScrCri = "Cache:dtScrCri3_1004";
		private const string KEY_dsScrCriVal = "Cache:dsScrCriVal3_1004";

		private const string KEY_dtSectionCd1283 = "Cache:dtSectionCd1283";
		private const string KEY_dtGroupRowId1283 = "Cache:dtGroupRowId1283";

		private const string KEY_dtSystems = "Cache:dtSystems3_1004";
		private const string KEY_sysConnectionString = "Cache:sysConnectionString3_1004";
		private const string KEY_dtAdmSctGrpRow1004List = "Cache:dtAdmSctGrpRow1004List";
		private const string KEY_bNewVisible = "Cache:bNewVisible3_1004";
		private const string KEY_bNewSaveVisible = "Cache:bNewSaveVisible3_1004";
		private const string KEY_bCopyVisible = "Cache:bCopyVisible3_1004";
		private const string KEY_bCopySaveVisible = "Cache:bCopySaveVisible3_1004";
		private const string KEY_bDeleteVisible = "Cache:bDeleteVisible3_1004";
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_1004";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_1004";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_1004";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				DataTable dtMenuAccess = (new MenuSystem()).GetMenu(base.LUser.CultureId, 3, base.LImpr, LcSysConnString, LcAppPw,1004, null, null);
				...
				Session.Remove(KEY_dtAdmSctGrpRow1004List);
				...
				Session.Remove(KEY_dtSectionCd1283);
				Session.Remove(KEY_dtGroupRowId1283);
				SetButtonHlp();
				GetSystems();
				SetColumnAuthority();
				GetGlobalFilter();
				...
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 1004, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmSctGrpRow1004List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=1004&typ=N&sys=3','','',''); return false;"; }
		...
		private void SetSystem(byte SystemId)
		{
			...
			try
			{
				...
				if ((Config.DeployType == "DEV" || row["dbAppDatabase"].ToString() == base.CPrj.EntityCode + "View") && !(base.CPrj.EntityCode != "RO" && row["SysProgram"].ToString() == "Y") && (new AdminSystem()).IsRegenNeeded(string.Empty,1004,0,0,LcSysConnString,LcAppPw))
				{
					(new GenScreensSystem()).CreateProgram(1004, "Section Row Style", row["dbAppDatabase"].ToString(), base.CPrj, base.CSrc, base.CTar, LcAppConnString, LcAppPw);
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(1004,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(1004,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(1004,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(1004,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,1004,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(1004,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(1004,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(1004,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		protected DataTable GetAuthRow()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthRow(1004,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				try
				{
					try {dv = new DataView((new AdminSystem()).GetExp(1004,"GetExpAdmSctGrpRow1004","Y",null,null,filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
					catch {dv = new DataView((new AdminSystem()).GetExp(1004,"GetExpAdmSctGrpRow1004","N",null,null,filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(1004,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					{
						if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["SctGrpRowId1283"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["SectionCd1283"].ToString().Replace("\"","\"\"") + "\"" + (char)9 + "\"" + drv["SectionCd1283Text"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(drv["GroupRowId1283"].ToString() + (char)9 + drv["GroupRowId1283Text"].ToString() + (char)9);}
						if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["SctGrpRowCss1283"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						sb.Append(Environment.NewLine);
					}
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmSctGrpRow.csv"; Session["ExportStr"] = (Config.ExportExcelCSV ? "sep=\t\n": "") + sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmSctGrpRow.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private string GetRtfGrid(DataView dv)
		{
			...
			try
			{
				...
				foreach (DataRowView drv in dv)
				{
					...
					sb.Append("\r\n");
					sb.Append(@"{");
					if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["SctGrpRowId1283"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["SectionCd1283Text"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(drv["GroupRowId1283Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(drv["SctGrpRowCss1283"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					sb.Append(@"}");
					sb.Append("\r\n");
					sb.Append(@"\pard \ql \li0\ri0\widctlpar\intbl\aspalpha\aspnum\adjustright\rin0\lin0 {");
					sb.Append(@"\insrsid2981395 \trowd \irow0\irowband0\lastrow \ts15\trgaph108\trleft-108\trbrdrt\brdrs\brdrw10 \trbrdrl\brdrs\brdrw10 \trbrdrb\brdrs\brdrw10 \trbrdrr\brdrs\brdrw10 \trbrdrh\brdrs\brdrw10 \trbrdrv\brdrs\brdrw10 ");
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 1004, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			...
			UpdCriteria(true);
			cAdmSctGrpRow1004List.ClearSearch(); Session.Remove(KEY_dtAdmSctGrpRow1004List);
			PopAdmSctGrpRow1004List(sender, e, false, null);
		}
			...
			try
			{
				foreach (DataRowView drv in dvCri)
				{
					...
					if (drv["DisplayName"].ToString() == "ComboBox")
					{
						...
						(new AdminSystem()).MkGetScreenIn("1004", drv["ScreenCriId"].ToString(), "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), LcAppDb, LcDesDb, drv["MultiDesignDb"].ToString(), LcSysConnString, LcAppPw);
						DataView dv = new DataView((new AdminSystem()).GetScreenIn("1004", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), (new AdminSystem()).CountScrCri(drv["ScreenCriId"].ToString(), drv["MultiDesignDb"].ToString(), drv["MultiDesignDb"].ToString() == "N" ? LcSysConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)), drv["RequiredValid"].ToString(), 0, string.Empty, drv["DisplayMode"].ToString() != "AutoListBox", val, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)));
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("1004", LcSysConnString, LcAppPw);
				}
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmSctGrpRow1004" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmSctGrpRow1004" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			foreach (DataRowView drv in dvCri)
			{
			    if (drv["DisplayName"].ToString() == "ListBox")
			    {
					...
					if (cListBox != null)
					{
						...
						int TotalChoiceCnt = new DataView((new AdminSystem()).GetScreenIn("1004", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), CriCnt, drv["RequiredValid"].ToString(), 0, string.Empty, true, string.Empty, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId))).Count;
						string selectedValues = string.Join(",", cListBox.Items.Cast<ListItem>().Where(x => x.Selected).Select(x => "'" + x.Value + "'").ToArray());
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("1004", "AdmSctGrpRow1004", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		private void SetSectionCd1283(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtSectionCd1283];
			...
			if (ddl != null)
			{
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(1004,"GetDdlSectionCd3S3146",true,bAll,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					if (dv.Table.Columns.Contains("SctGrpRowId"))
					{
						ss = "(SctGrpRowId is null";
						if (string.IsNullOrEmpty(cAdmSctGrpRow1004List.SelectedValue)) {ss = ss + ")";} else {ss = ss + " OR SctGrpRowId = " + cAdmSctGrpRow1004List.SelectedValue + ")";}
					}
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "SectionCd1283 is not NULL"; }
					dv.RowFilter = ss;
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(1004,"GetDdlSectionCd3S3146",true,true,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtSectionCd1283] = dv.Table;
				}
			}
		}

		private void SetGroupRowId1283(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtGroupRowId1283];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (ddl != null)
			{
				string ss = string.Empty;
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(1004,"GetDdlGroupRowId3S3147",true,bAll,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					...
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "GroupRowId1283 is not NULL"; }
					dv.RowFilter = ss;
					ddl.DataSource = dv; ddl.DataBind();
					li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					else if (!bFirst)
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(1004,"GetDdlGroupRowId3S3147",true,true,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtGroupRowId1283] = dv.Table;
				}
			}
		}

		private DataView GetAdmSctGrpRow1004List()
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmSctGrpRow1004List];
			if (dt == null)
			{
				CheckAuthentication(false);
				int filterId = 0;
				...
				try
				{
					try {dt = (new AdminSystem()).GetLis(1004,"GetLisAdmSctGrpRow1004",true,"Y",0,null,null,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
					catch {dt = (new AdminSystem()).GetLis(1004,"GetLisAdmSctGrpRow1004",true,"N",0,null,null,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				}
		...
		private void PopAdmSctGrpRow1004List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			DataView dv = GetAdmSctGrpRow1004List();
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			context["method"] = "GetLisAdmSctGrpRow1004";
			context["mKey"] = "SctGrpRowId1283";
			context["mVal"] = "SctGrpRowId1283Text";
			context["mTip"] = "SctGrpRowId1283Text";
			context["mImg"] = "SctGrpRowId1283Text";
			context["ssd"] = Request.QueryString["ssd"];
			context["scr"] = "1004";
			context["csy"] = "3";
			...
			cAdmSctGrpRow1004List.AutoCompleteUrl = "AutoComplete.aspx/LisSuggests";
			cAdmSctGrpRow1004List.DataContext = context;
			if (dv.Table == null) return;
			cAdmSctGrpRow1004List.DataSource = dv;
			cAdmSctGrpRow1004List.Visible = true;
			if (cAdmSctGrpRow1004List.Items.Count <= 0) {cAdmSctGrpRow1004List.Visible = false; cAdmSctGrpRow1004List_SelectedIndexChanged(sender,e);}
			else
			{
				if (bPageLoad && !string.IsNullOrEmpty(Request.QueryString["key"]) && bUseCri.Value == string.Empty)
				{
					try {cAdmSctGrpRow1004List.SelectByValue(Request.QueryString["key"],string.Empty,true);} catch {cAdmSctGrpRow1004List.Items[0].Selected = true; cAdmSctGrpRow1004List_SelectedIndexChanged(sender, e);}
				    cScreenSearch.Visible = false; cSystem.Visible = false; cEditButton.Visible = true; cSaveCloseButton.Visible = cSaveButton.Visible;
				}
				else
				{
					if (ID != null) {if (!cAdmSctGrpRow1004List.SelectByValue(ID,string.Empty,true)) {ID = null;}}
					if (ID == null)
					{
						cAdmSctGrpRow1004List_SelectedIndexChanged(sender, e);
					}
				}
			}
		}
		...
		private void SetColumnAuthority()
		{
			...
			{
				base.SetFoldBehavior(cSctGrpRowId1283, dtAuth.Rows[0], cSctGrpRowId1283P1, cSctGrpRowId1283Label, cSctGrpRowId1283P2, null, dtLabel.Rows[0], null, null, null);
				base.SetFoldBehavior(cSectionCd1283, dtAuth.Rows[1], cSectionCd1283P1, cSectionCd1283Label, cSectionCd1283P2, null, dtLabel.Rows[1], null, null, null);
				base.SetFoldBehavior(cGroupRowId1283, dtAuth.Rows[2], cGroupRowId1283P1, cGroupRowId1283Label, cGroupRowId1283P2, null, dtLabel.Rows[2], null, null, null);
				base.SetFoldBehavior(cSctGrpRowCss1283, dtAuth.Rows[3], cSctGrpRowCss1283P1, cSctGrpRowCss1283Label, cSctGrpRowCss1283P2, cSctGrpRowCss1283E, null, dtLabel.Rows[3], null, null, null);
				cSctGrpRowCss1283E.Attributes["label_id"] = cSctGrpRowCss1283Label.ClientID; cSctGrpRowCss1283E.Attributes["target_id"] = cSctGrpRowCss1283.ClientID;
			}
			if ((cSectionCd1283.Attributes["OnChange"] == null || cSectionCd1283.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cSectionCd1283.Visible && cSectionCd1283.Enabled) {cSectionCd1283.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cGroupRowId1283.Attributes["OnChange"] == null || cGroupRowId1283.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cGroupRowId1283.Visible && cGroupRowId1283.Enabled) {cGroupRowId1283.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cSctGrpRowCss1283.Attributes["OnChange"] == null || cSctGrpRowCss1283.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cSctGrpRowCss1283.Visible && !cSctGrpRowCss1283.ReadOnly) {cSctGrpRowCss1283.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
		}
		...
		private void ClearMaster(object sender, System.EventArgs e)
		{
			...
			if (dt.Rows[1]["ColVisible"].ToString() == "Y" && dt.Rows[1]["ColReadOnly"].ToString() != "Y") {SetSectionCd1283(cSectionCd1283,string.Empty);}
			if (dt.Rows[2]["ColVisible"].ToString() == "Y" && dt.Rows[2]["ColReadOnly"].ToString() != "Y") {SetGroupRowId1283(cGroupRowId1283,string.Empty);}
			if (dt.Rows[3]["ColVisible"].ToString() == "Y" && dt.Rows[3]["ColReadOnly"].ToString() != "Y") {cSctGrpRowCss1283.Text = string.Empty;}
			// *** Default Value (Folder) Web Rule starts here *** //
		}

		private void InitMaster(object sender, System.EventArgs e)
		{
			cSctGrpRowId1283.Text = string.Empty;
			SetSectionCd1283(cSectionCd1283,string.Empty);
			SetGroupRowId1283(cGroupRowId1283,string.Empty);
			cSctGrpRowCss1283.Text = string.Empty;
			// *** Default Value (Folder) Web Rule starts here *** //
		}
		protected void cAdmSctGrpRow1004List_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmSctGrpRow1004List_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmSctGrpRow1004List_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			InitMaster(sender, e);      // Do this first to enable defaults for non-database columns.
			DataTable dt = null;
			try
			{
				dt = (new AdminSystem()).GetMstById("GetAdmSctGrpRow1004ById",cAdmSctGrpRow1004List.SelectedValue,null,null);
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				if (dt.Rows.Count > 0)
				{
					cAdmSctGrpRow1004List.SetDdVisible();
					DataRow dr = dt.Rows[0];
					try {cSctGrpRowId1283.Text = RO.Common3.Utils.fmNumeric("0",dr["SctGrpRowId1283"].ToString(),base.LUser.Culture);} catch {cSctGrpRowId1283.Text = string.Empty;}
					SetSectionCd1283(cSectionCd1283,dr["SectionCd1283"].ToString());
					SetGroupRowId1283(cGroupRowId1283,dr["GroupRowId1283"].ToString());
					try {cSctGrpRowCss1283.Text = dr["SctGrpRowCss1283"].ToString();} catch {cSctGrpRowCss1283.Text = string.Empty;}
				}
			}
			cButPanel.DataBind();
			ScriptManager.GetCurrent(Parent.Page).SetFocus(cAdmSctGrpRow1004List.FocusID);
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cSctGrpRowId1283.Text == string.Empty) { cClearButton_Click(sender, new EventArgs()); ShowDirty(false); PanelTop.Update();}
			else { PopAdmSctGrpRow1004List(sender, e, false, cSctGrpRowId1283.Text); }
			// *** System Button Click (After) Web Rule starts here *** //
		}
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmSctGrpRow1004List); PopAdmSctGrpRow1004List(sender, e, false, null);
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			if (ValidPage())
			{
				AdmSctGrpRow1004 ds = PrepAdmSctGrpRowData(null,cSctGrpRowId1283.Text == string.Empty);
				if (!string.IsNullOrEmpty(cAdmSctGrpRow1004List.SelectedValue) && cSctGrpRowId1283.Text != cAdmSctGrpRow1004List.SelectedValue)
				{
					PreMsgPopup("Primary key cannot be changed");
					return rtn;
				}
				if (string.IsNullOrEmpty(cAdmSctGrpRow1004List.SelectedValue))	// Add
				{
					if (ds != null)
					{
						pid = (new AdminSystem()).AddData(1004,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc,noTrans,commandTimeOut);
					}
					if (!string.IsNullOrEmpty(pid))
					{
						cAdmSctGrpRow1004List.ClearSearch(); Session.Remove(KEY_dtAdmSctGrpRow1004List);
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmSctGrpRow1004List(sender, e, false, pid);
						rtn = GetScreenHlp().Rows[0]["AddMsg"].ToString();
					}
				}
				else {
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(1004,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc,noTrans,commandTimeOut)) {bValid7 = true;}
					if (bValid7)
					{
						cAdmSctGrpRow1004List.ClearSearch(); Session.Remove(KEY_dtAdmSctGrpRow1004List);
						ShowDirty(false); PopAdmSctGrpRow1004List(sender, e, false, ds.Tables["AdmSctGrpRow"].Rows[0]["SctGrpRowId1283"]);
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cSctGrpRowId1283.Text != string.Empty)
			{
				AdmSctGrpRow1004 ds = PrepAdmSctGrpRowData(null,false);
				try
				{
					if (ds != null && (new AdminSystem()).DelData(1004,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc))
					{
						cAdmSctGrpRow1004List.ClearSearch(); Session.Remove(KEY_dtAdmSctGrpRow1004List);
						ShowDirty(false); PopAdmSctGrpRow1004List(sender, e, false, null);
		...
		private AdmSctGrpRow1004 PrepAdmSctGrpRowData(DataView dv, bool bAdd)
		{
			AdmSctGrpRow1004 ds = new AdmSctGrpRow1004();
			DataRow dr = ds.Tables["AdmSctGrpRow"].NewRow();
			DataRow drType = ds.Tables["AdmSctGrpRow"].NewRow();
			DataRow drDisp = ds.Tables["AdmSctGrpRow"].NewRow();
			if (bAdd) { dr["SctGrpRowId1283"] = string.Empty; } else { dr["SctGrpRowId1283"] = cSctGrpRowId1283.Text; }
			drType["SctGrpRowId1283"] = "Numeric"; drDisp["SctGrpRowId1283"] = "TextBox";
			try {dr["SectionCd1283"] = cSectionCd1283.SelectedValue;} catch {}
			drType["SectionCd1283"] = "Char"; drDisp["SectionCd1283"] = "DropDownList";
			try {dr["GroupRowId1283"] = cGroupRowId1283.SelectedValue;} catch {}
			drType["GroupRowId1283"] = "Numeric"; drDisp["GroupRowId1283"] = "DropDownList";
			try {dr["SctGrpRowCss1283"] = cSctGrpRowCss1283.Text;} catch {}
			drType["SctGrpRowCss1283"] = "VarChar"; drDisp["SctGrpRowCss1283"] = "MultiLine";
			if (bAdd)
			{
			}
			ds.Tables["AdmSctGrpRow"].Rows.Add(dr); ds.Tables["AdmSctGrpRow"].Rows.Add(drType); ds.Tables["AdmSctGrpRow"].Rows.Add(drDisp);
			return ds;
		}

		public bool CanAct(char typ) { return CanAct(typ, GetAuthRow(), cAdmSctGrpRow1004List.SelectedValue.ToString()); }
		...
		private void PreMsgPopup(string msg) { PreMsgPopup(msg,cAdmSctGrpRow1004List,null); }

...
{
	public class AdmTemplate53 : DataSet
	{
		public AdmTemplate53()
		{
			this.Tables.Add(MakeDtlColumns(new DataTable("AdmTemplateDef")));
			this.Tables.Add(MakeDtlColumns(new DataTable("AdmTemplateAdd")));
			this.Tables.Add(MakeDtlColumns(new DataTable("AdmTemplateUpd")));
			this.Tables.Add(MakeDtlColumns(new DataTable("AdmTemplateDel")));
			this.DataSetName = "AdmTemplate53";
			this.Namespace = "http://Rintagi.com/DataSet/AdmTemplate53";
		}
...
namespace RO.Web
{
	public partial class AdmTemplateModule : RO.Web.ModuleBase
	{

		private const string KEY_lastAddedRow = "Cache:lastAddedRow3_53";
		private const string KEY_lastSortOrd = "Cache:lastSortOrd3_53";
		private const string KEY_lastSortImg = "Cache:lastSortImg3_53";
		private const string KEY_lastSortCol = "Cache:lastSortCol3_53";
		private const string KEY_lastSortExp = "Cache:lastSortExp3_53";
		private const string KEY_lastSortUrl = "Cache:lastSortUrl3_53";
		private const string KEY_lastSortTog = "Cache:lastSortTog3_53";
		private const string KEY_lastImpPwdOvride = "Cache:lastImpPwdOvride3_53";
		private const string KEY_cntImpPwdOvride = "Cache:cntImpPwdOvride3_53";
		private const string KEY_currPageIndex = "Cache:currPageIndex3_53";
		private const string KEY_bHiImpVisible = "Cache:bHiImpVisible3_53";
		private const string KEY_bShImpVisible = "Cache:bShImpVisible3_53";
		private const string KEY_dtAdmTemplateGrid = "Cache:dtAdmTemplateGrid";
		private const string KEY_scrImport = "Cache:scrImport3_53";
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_53";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_53";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_53";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_53";
		private const string KEY_dtLabel = "Cache:dtLabel3_53";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_53";
		private const string KEY_dtScrCri = "Cache:dtScrCri3_53";
		private const string KEY_dsScrCriVal = "Cache:dsScrCriVal3_53";


		private const string KEY_dtSystems = "Cache:dtSystems3_53";
		private const string KEY_sysConnectionString = "Cache:sysConnectionString3_53";
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_53";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_53";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_53";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				DataTable dtMenuAccess = (new MenuSystem()).GetMenu(base.LUser.CultureId, 3, base.LImpr, LcSysConnString, LcAppPw,53, null, null);
				...
				Session.Remove(KEY_dtAdmTemplateGrid);
				...
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 53, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmTemplate53List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=53&typ=N&sys=3','','',''); return false;"; }
		...
		private void InitializeComponent()
		{
			...
			if (LcSysConnString == null)
			{
				...
				{
					DataTable dt;
					dt = (new AdminSystem()).GetAuthRow(53,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
					...
					{
						cAdmTemplateGrid.PreRender += new EventHandler(cAdmTemplateGrid_OnPreRender);
					}
				}
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(53,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(53,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void SetClientRule(ListViewDataItem ee, bool isEditItem)
		{
			...
			if (dvRul.Count > 0)
			{
				...
				{
					if (ee == null || drv["ScriptEvent"].ToString().Substring(0,2).ToLower() != "on" || (cAdmTemplateGrid.EditIndex > -1 && GetDataItemIndex(cAdmTemplateGrid.EditIndex) == ee.DataItemIndex))
					{
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(53,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(53,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,53,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(53,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(53,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(53,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				CheckAuthentication(false);
				DataView dv = null;
				try {dv = ((DataTable)Session[KEY_dtAdmTemplateGrid]).DefaultView;}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(53,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmTemplate.csv"; Session["ExportStr"] = (Config.ExportExcelCSV ? "sep=\t\n": "") + sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmTemplate.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private void GetPageInfo()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastPageInfo(53, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 53, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			// *** Criteria Trigger (before) Web Rule starts here *** //
			DataTable dt = (DataTable)Session[KEY_dtAdmTemplateGrid];
			if (cAdmTemplateGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
			if (IsPostBack && cCriteria.Visible) ReBindCriteria(GetScrCriteria());
			UpdCriteria(true);
			PopAdmTemplate53List(sender, e, false, null);
			}
		}
			...
			try
			{
				foreach (DataRowView drv in dvCri)
				{
					...
					if (drv["DisplayName"].ToString() == "ComboBox")
					{
						...
						(new AdminSystem()).MkGetScreenIn("53", drv["ScreenCriId"].ToString(), "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), LcAppDb, LcDesDb, drv["MultiDesignDb"].ToString(), LcSysConnString, LcAppPw);
						DataView dv = new DataView((new AdminSystem()).GetScreenIn("53", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), (new AdminSystem()).CountScrCri(drv["ScreenCriId"].ToString(), drv["MultiDesignDb"].ToString(), drv["MultiDesignDb"].ToString() == "N" ? LcSysConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)), drv["RequiredValid"].ToString(), 0, string.Empty, drv["DisplayMode"].ToString() != "AutoListBox", val, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)));
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("53", LcSysConnString, LcAppPw);
				}
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmTemplate53" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmTemplate53" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			foreach (DataRowView drv in dvCri)
			{
			    if (drv["DisplayName"].ToString() == "ListBox")
			    {
					...
					if (cListBox != null)
					{
						...
						int TotalChoiceCnt = new DataView((new AdminSystem()).GetScreenIn("53", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), CriCnt, drv["RequiredValid"].ToString(), 0, string.Empty, true, string.Empty, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId))).Count;
						string selectedValues = string.Join(",", cListBox.Items.Cast<ListItem>().Where(x => x.Selected).Select(x => "'" + x.Value + "'").ToArray());
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("53", "AdmTemplate53", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		private void PopAdmTemplate53List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			DataTable dt = null;
			CheckAuthentication(false);
			...
			try
			{
				try {dt = (new AdminSystem()).GetLis(53,"GetLisAdmTemplate53",false,"Y",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				catch {dt = (new AdminSystem()).GetLis(53,"GetLisAdmTemplate53",false,"N",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				Session[KEY_dtAdmTemplateGrid] = dt;
				cAdmTemplateGridDataPager.PageSize = Int16.Parse(cPgSize.Text); GotoPage(0);
			    if (Session[KEY_lastSortCol] != null)
			    {
					cAdmTemplateGrid_OnSorting(sender, new ListViewSortEventArgs((string)Session[KEY_lastSortExp], SortDirection.Ascending));
			    }
				else
			    {
					cAdmTemplateGrid_DataBind(dt.DefaultView);
			    }
		...
		public void cFindButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dv != null)
			{
				if (cAdmTemplateGrid.EditIndex < 0 || UpdateGridRow(cAdmTemplateGrid, new CommandEventArgs("Save", "")))
				{
					...
					if (GetCurrPageIndex() != (int)Session[KEY_currPageIndex] && (int)Session[KEY_currPageIndex] < GetTotalPages()) { GotoPage((int)Session[KEY_currPageIndex]); cAdmTemplateGrid_DataBind(dv);}
		...
		public void cInsRowButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dt != null && (ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")))
			{
				...
				Session[KEY_lastAddedRow] = 0; Session[KEY_dtAdmTemplateGrid] = dt; Session[KEY_currPageIndex] = 0; GotoPage(0);
				cAdmTemplateGrid_DataBind(dt.DefaultView);
				cAdmTemplateGrid_OnItemEditing(cAdmTemplateGrid, new ListViewEditEventArgs(0));
		...
		public void cPgSizeButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cAdmTemplateGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				...
				cAdmTemplateGridDataPager.PageSize = int.Parse(cPgSize.Text); GotoPage(0);
				if (dt.Rows.Count <= 0 || (GetAuthRow().Rows[0]["AllowAdd"].ToString() == "Y" && dt.Rows.Count == 1)) {cAdmTemplateGrid_DataBind(dt.DefaultView);} else {cFindButton_Click(sender, e);}
				try
				{
					(new AdminSystem()).UpdLastPageInfo(53, base.LUser.UsrId, cPgSize.Text, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		}
		...
		public void cShowImpButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cSchemaImage.Visible)
			{
				try
				{
					Session["ImportSchema"] = (new AdminSystem()).GetSchemaScrImp(53,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (cSchemaImage.Attributes["OnClick"] == null || cSchemaImage.Attributes["OnClick"].IndexOf("ImportSchema.aspx") < 0) {cSchemaImage.Attributes["OnClick"] += "SearchLink('ImportSchema.aspx?scm=S&key=53&csy=3','','',''); return false;";}
			}
			// *** System Button Click (after) Web Rule starts here *** //
		}
		...
		protected void ScrImport(bool bClear)
		{
			...
			DataTable dtAdmTemplateGrid = (DataTable)Session[KEY_dtAdmTemplateGrid];
			if (dti != null && dtAdmTemplateGrid != null)
			{
				if (bClear)
				{
					dtAdmTemplateGrid.Rows.Clear();
				}
				cFind.Text = string.Empty;
				int iExist = dtAdmTemplateGrid.Rows.Count;
				iRow = dti.DefaultView.Count - iExist;
				if (!dti.Columns.Contains("_NewRow")) { dti.Columns.Add("_NewRow"); }
				Session[KEY_dtAdmTemplateGrid] = dti;
				cAdmTemplateGrid_DataBind(dti.DefaultView);
		...
		public void cImportButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cFNameO.Text != string.Empty && cWorkSheet.Items.Count > 0 && cWorkSheet.SelectedItem.Text != string.Empty && cStartRow.Text != string.Empty)
			{
				...
				try
				{
					if (cAdmTemplateGrid.EditIndex < 0 || UpdateGridRow(cAdmTemplateGrid, new CommandEventArgs("Save", "")))
					{
						Session[KEY_scrImport] = ImportFile(dtAdmTemplateGrid.Copy(),cFNameO.Text,cWorkSheet.SelectedItem.Text,cStartRow.Text,Config.PathTmpImport + cFName.Text);
					}
		...
		private void GotoPage(int pageNo)
		{
			if (pageNo >= 0 && pageNo < GetTotalPages())
			{
				if (cAdmTemplateGrid.EditIndex < 0 || UpdateGridRow(cAdmTemplateGrid, new CommandEventArgs("Save", "")))
				{
					try { cAdmTemplateGridDataPager.SetPageProperties(cAdmTemplateGridDataPager.PageSize * pageNo, cAdmTemplateGridDataPager.MaximumRows, false); cAdmTemplateGrid_DataBind(null); }
					catch
					{
						try { cAdmTemplateGridDataPager.SetPageProperties(0, cAdmTemplateGridDataPager.MaximumRows, false); cAdmTemplateGrid_DataBind(null); } catch {}
					}
				}
			}
		}

		protected void cAdmTemplateGrid_OnItemCommand(object sender, ListViewCommandEventArgs e)
		{
		}

		private void cAdmTemplateGrid_DataBind(DataView dvAdmTemplateGrid)
		{
			if (dvAdmTemplateGrid == null)
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmTemplateGrid];
				dvAdmTemplateGrid = dt != null ? dt.DefaultView : null;
			}
			if (dvAdmTemplateGrid != null)
			{
				LcAuth = GetAuthCol().AsEnumerable().ToDictionary<DataRow,string>(dr=>dr["ColName"].ToString());
				cAdmTemplateGrid.DataSource = dvAdmTemplateGrid;
				cAdmTemplateGrid.DataBind();
		...
		protected void cAdmTemplateGrid_OnSorting(object sender, ListViewSortEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmTemplateGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null)
			{
				if (cAdmTemplateGrid.EditIndex < 0 || UpdateGridRow(cAdmTemplateGrid, new CommandEventArgs("Save", "")))
				{
					...
					Session[KEY_dtAdmTemplateGrid] = dt;
					cAdmTemplateGrid_DataBind(dv);
				}
			}
		}

		protected void cAdmTemplateGrid_OnPagePropertiesChanging(object sender, PagePropertiesChangingEventArgs e)
		{
		    cAdmTemplateGridDataPager.SetPageProperties(e.StartRowIndex, e.MaximumRows, false);
		    DataTable dt = (DataTable)Session[KEY_dtAdmTemplateGrid];
		    DataView dv = dt != null ? dt.DefaultView : null;
			cAdmTemplateGrid_DataBind(dv); cAdmTemplateGrid.EditIndex = -1;
		}

		private void GridChkPgDirty(ListViewItem lvi)
		{
				WebControl cc = null;
				cc = ((WebControl)lvi.FindControl("cTemplateName79"));
				...
				cc = ((WebControl)lvi.FindControl("cTmplPrefix79"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cTmplDefault79"));
		...
		protected void cAdmTemplateGrid_OnItemDataBound(object sender, ListViewItemEventArgs e)
		{
			...
			if (cAdmTemplateGrid.EditIndex > -1 && GetDataItemIndex(cAdmTemplateGrid.EditIndex) == e.Item.DataItemIndex)
			{
				...
				CheckBox cb = null;
				cb = (CheckBox)e.Item.FindControl("cTmplDefault79");
				if (cb != null)
				{
					cb.Checked = base.GetBool(dvAdmTemplateGrid[e.Item.DataItemIndex]["TmplDefault79"].ToString());
				}
			...
			if (dtAuthRow != null)
			{
				...
				{
					cc = e.Item.FindControl("cAdmTemplateGridDelete"); if (cc != null) { cc.Visible = false; }
				}
				if (!AllowRowEdit(GetAuthRow(),dvAdmTemplateGrid[e.Item.DataItemIndex]["TemplateId79"].ToString()) || (Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N"))
				{
					cc = e.Item.FindControl("cAdmTemplateGridEdit"); if (cc != null) { cc.Visible = false; }
				}
				else
				{
			        HtmlTableRow tr = e.Item.FindControl("cAdmTemplateGridRow") as HtmlTableRow;
			        LinkButton lb = e.Item.FindControl("cAdmTemplateGridEdit") as LinkButton;
		...
		protected void cTemplateId79hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "0") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "0"; Session[KEY_lastSortExp] = "TemplateId79";Session[KEY_lastSortImg] = "cTemplateId79hi";
			cAdmTemplateGrid_OnSorting(sender, new ListViewSortEventArgs("TemplateId79", SortDirection.Ascending));
		}

		protected void cTemplateName79hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "1") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "1"; Session[KEY_lastSortExp] = "TemplateName79";Session[KEY_lastSortImg] = "cTemplateName79hi";
			cAdmTemplateGrid_OnSorting(sender, new ListViewSortEventArgs("TemplateName79", SortDirection.Ascending));
		}

		protected void cTmplPrefix79hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "2") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "2"; Session[KEY_lastSortExp] = "TmplPrefix79";Session[KEY_lastSortImg] = "cTmplPrefix79hi";
			cAdmTemplateGrid_OnSorting(sender, new ListViewSortEventArgs("TmplPrefix79", SortDirection.Ascending));
		}

		protected void cTmplDefault79hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "3") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "3"; Session[KEY_lastSortExp] = "TmplDefault79";Session[KEY_lastSortImg] = "cTmplDefault79hi";
			cAdmTemplateGrid_OnSorting(sender, new ListViewSortEventArgs("TmplDefault79", SortDirection.Ascending));
		}

		protected void cAdmTemplateGrid_OnItemEditing(object sender, ListViewEditEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmTemplateGrid];
			if ((ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")) && dt != null && dt.DefaultView.Count > GetDataItemIndex(e.NewEditIndex))
			{
				cAdmTemplateGrid.EditIndex = e.NewEditIndex;
				cAdmTemplateGrid_DataBind(null);
				Control wc = null;
				string idx = Page.Request["__EVENTARGUMENT"];
				if (!string.IsNullOrEmpty(idx))
				{
				    Control ctrlToFocus = cAdmTemplateGrid.EditItem.FindControl("c" + idx);
				    if (((WebControl)ctrlToFocus).Enabled) { wc = ctrlToFocus; }
				}
				if (wc == null) { wc = FindEditableControl(cAdmTemplateGrid.EditItem); }
				if (wc != null)
             {
             ...
             MkMobileLabel(cAdmTemplateGrid.EditItem);
		    }
		    else { e.Cancel = true; }
		}
		...
		{
		    Label ml = null;
		    ml = lvi.FindControl("cTemplateId79ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(0); }
		    ml = lvi.FindControl("cTemplateName79ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(1); }
		    ml = lvi.FindControl("cTmplPrefix79ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(2); }
		    ml = lvi.FindControl("cTmplDefault79ml") as Label;
		...
		{
			TextBox tb = null;
			tb = (TextBox)lvi.FindControl("cTemplateName79");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["TemplateName79"] = tb.Text;} else {dr["TemplateName79"] = System.DBNull.Value;}
			}
			tb = (TextBox)lvi.FindControl("cTmplPrefix79");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["TmplPrefix79"] = tb.Text;} else {dr["TmplPrefix79"] = System.DBNull.Value;}
			}
			CheckBox cb = null;
			cb = (CheckBox)lvi.FindControl("cTmplDefault79");
			if (cb != null)
			{
				dr["TmplDefault79"] = base.SetBool(cb.Checked);
			}
		    if (bInsert) { dt.Rows.InsertAt(dr, 0); }
			Session[KEY_dtAdmTemplateGrid] = dt;
			cAdmTemplateGrid.EditIndex = -1;
			cAdmTemplateGrid_DataBind(dt.DefaultView);
		}

		protected void cAdmTemplateGrid_OnItemUpdating(object sender, ListViewUpdateEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmTemplateGrid];
			if (dt != null && dt.Rows.Count > 0)
			{
				DataRow dr = dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row;
			    GridFill(cAdmTemplateGrid.EditItem, dt, dr, false);
			    dr["_NewRow"] = "N";
			    if (dt.Columns.Contains("_SortOrder")) dr["_SortOrder"] = null;
			}
		}

		protected void cAdmTemplateGrid_OnItemCanceling(object sender, ListViewCancelEventArgs e)
		{
			PanelTop.Update();
			DataTable dt = (DataTable)Session[KEY_dtAdmTemplateGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			...
			{
				cAdmTemplateGrid_OnItemDeleting(sender, new ListViewDeleteEventArgs(e.ItemIndex));
			}
			if (dt.Columns.Contains("_SortOrder")) dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row["_SortOrder"] = null;
			cAdmTemplateGrid.EditIndex = -1; cAdmTemplateGrid_DataBind(null);
		}
		...
		protected void cDeleteAllButton_Click(object sender, System.EventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmTemplateGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null && (cAdmTemplateGrid.EditIndex < 0 || UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				GotoPage(0);
				while (dv.Count > 0) {dv.Delete(0);}
				Session[KEY_dtAdmTemplateGrid] = dt;
				cAdmTemplateGrid.EditIndex = -1; cAdmTemplateGrid_DataBind(dv);
			}
		}

		protected void cAdmTemplateGrid_OnItemDeleting(object sender, ListViewDeleteEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmTemplateGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			DataRow dr = dv != null ? dv[GetDataItemIndex(e.ItemIndex)].Row : null;
			if (dv != null && (cAdmTemplateGrid.EditIndex == e.ItemIndex || UpdateGridRow(sender, new CommandEventArgs("Save",""))))
			{
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmTemplateGrid); PopAdmTemplate53List(sender, e, false, null);
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			// *** System Button Click (Before) Web Rule starts here *** //
			if (ValidPage() && UpdateGridRow(cAdmTemplateGrid, new CommandEventArgs("Save", "")))
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmTemplateGrid];
				DataView dv = dt != null ? dt.DefaultView : null;
				string ftr = string.Empty;
				if (dv.RowFilter != string.Empty) { ftr = dv.RowFilter; dv.RowFilter = string.Empty; }
				AdmTemplate53 ds = PrepAdmTemplateData(dv,false);
				if (ftr != string.Empty) {dv.RowFilter = ftr;}
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(53,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc,noTrans,commandTimeOut)) {bValid7 = true;}
					if (bValid7)
					{
						Session[KEY_currPageIndex] = GetCurrPageIndex();
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmTemplate53List(sender, e, false, null);
		...
		private AdmTemplate53 PrepAdmTemplateData(DataView dv, bool bAdd)
		{
			AdmTemplate53 ds = new AdmTemplate53();
			if (dv != null)
			{
				ds.Tables["AdmTemplateDef"].Rows.Add(MakeTypRow(ds.Tables["AdmTemplateDef"].NewRow()));
				ds.Tables["AdmTemplateDef"].Rows.Add(MakeDisRow(ds.Tables["AdmTemplateDef"].NewRow()));
					dv.RowStateFilter = DataViewRowState.ModifiedCurrent;
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmTemplateUpd"].Rows.Add(MakeColRow(ds.Tables["AdmTemplateUpd"].NewRow(), drv, false));
					}
					dv.RowStateFilter = DataViewRowState.Added;
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmTemplateAdd"].Rows.Add(MakeColRow(ds.Tables["AdmTemplateAdd"].NewRow(), drv, true));
					}
					dv.RowStateFilter = DataViewRowState.Deleted;
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmTemplateDel"].Rows.Add(MakeColRow(ds.Tables["AdmTemplateDel"].NewRow(), drv, false));
					}
		...
		private DataRow MakeTypRow(DataRow dr)
		{
			...
			dr["TemplateName79"] = System.Data.OleDb.OleDbType.VarWChar.ToString();
			dr["TmplPrefix79"] = System.Data.OleDb.OleDbType.VarChar.ToString();
			dr["TmplDefault79"] = System.Data.OleDb.OleDbType.Char.ToString();
			return dr;
		}
		...
		private DataRow MakeDisRow(DataRow dr)
		{
			...
			dr["TemplateName79"] = "TextBox";
			dr["TmplPrefix79"] = "TextBox";
			dr["TmplDefault79"] = "CheckBox";
			return dr;
		}
		...
		private DataRow MakeColRow(DataRow dr, DataRowView drv, bool bAdd)
		{
			...
			if (dtAuth != null)
			{
				dr["TemplateId79"] = drv["TemplateId79"].ToString().Trim();
				dr["TemplateName79"] = drv["TemplateName79"].ToString().Trim();
				dr["TmplPrefix79"] = drv["TmplPrefix79"].ToString().Trim();
				dr["TmplDefault79"] = drv["TmplDefault79"];
			}
			return dr;
		}
		...
		{
			if (cAdmTemplateGrid.EditIndex > -1)
			{
				cAdmTemplateGrid_OnItemUpdating(sender, new ListViewUpdateEventArgs(cAdmTemplateGrid.EditIndex));
			}
			return true;
		}

		protected void cAdmTemplateGrid_OnPreRender(object sender, System.EventArgs e)
		{
			System.Web.UI.WebControls.Image hi = null;
			hi = (System.Web.UI.WebControls.Image)cAdmTemplateGrid.FindControl("cTemplateId79hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmTemplateGrid.FindControl("cTemplateName79hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmTemplateGrid.FindControl("cTmplPrefix79hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmTemplateGrid.FindControl("cTmplDefault79hi"); if (hi != null) { hi.Visible = false; }
			if (Session[KEY_lastSortImg] != null)
			{
				hi = (System.Web.UI.WebControls.Image)cAdmTemplateGrid.FindControl((string)Session[KEY_lastSortImg]);
				if (hi != null) { hi.ImageUrl = Utils.AddTilde((string)Session[KEY_lastSortUrl]); hi.Visible = true; }
			}
			IgnoreHeaderConfirm((LinkButton)cAdmTemplateGrid.FindControl("cTemplateId79hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmTemplateGrid.FindControl("cTemplateName79hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmTemplateGrid.FindControl("cTmplPrefix79hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmTemplateGrid.FindControl("cTmplDefault79hl"));
		}

		private string GetButtonId(ListViewItem lvi)
		{
			string ButtonID = String.Empty;
			Control c = lvi.FindControl("cAdmTemplateGridEdit");
		...
		{
		    return cAdmTemplateGridDataPager.StartRowIndex / cAdmTemplateGridDataPager.PageSize;
		}

		private int GetTotalPages()
		{
		    return (int)Math.Ceiling((double)cAdmTemplateGridDataPager.TotalRowCount / cAdmTemplateGridDataPager.PageSize);
		}

		private int GetDataItemIndex(int editIndex)
		{
		    return cAdmTemplateGridDataPager.StartRowIndex + editIndex;
		}

		protected void cAdmTemplateGrid_OnLayoutCreated(object sender, EventArgs e)
		{
		    // Header:
		    LinkButton lb = null;
		    lb = cAdmTemplateGrid.FindControl("cTemplateId79hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(0); lb.ToolTip = ColumnToolTip(0); lb.Parent.Visible = GridColumnVisible(0); }
		    lb = cAdmTemplateGrid.FindControl("cTemplateName79hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(1); lb.ToolTip = ColumnToolTip(1); lb.Parent.Visible = GridColumnVisible(1); }
		    lb = cAdmTemplateGrid.FindControl("cTmplPrefix79hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(2); lb.ToolTip = ColumnToolTip(2); lb.Parent.Visible = GridColumnVisible(2); }
		    lb = cAdmTemplateGrid.FindControl("cTmplDefault79hl") as LinkButton;
			...
			if (dtAuthRow != null)
			{
				...
				if (dr["AllowDel"].ToString() == "N" || (Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N"))
				{
		            lb = cAdmTemplateGrid.FindControl("cDeleteAllButton") as LinkButton; if (lb != null) { lb.Visible = false; }
				}
			}
		    // footer:
		    Label gc = null;
		    gc = cAdmTemplateGrid.FindControl("cTemplateId79fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(0); }
		    gc = cAdmTemplateGrid.FindControl("cTemplateName79fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(1); }
		    gc = cAdmTemplateGrid.FindControl("cTmplPrefix79fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(2); }
		    gc = cAdmTemplateGrid.FindControl("cTmplDefault79fl") as Label;

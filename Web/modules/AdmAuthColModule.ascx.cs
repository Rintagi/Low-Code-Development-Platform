...
namespace RO.Web
{
	public partial class AdmAuthColModule : RO.Web.ModuleBase
	{

		private const string KEY_lastAddedRow = "Cache:lastAddedRow3_16";
		private const string KEY_lastSortOrd = "Cache:lastSortOrd3_16";
		private const string KEY_lastSortImg = "Cache:lastSortImg3_16";
		private const string KEY_lastSortCol = "Cache:lastSortCol3_16";
		private const string KEY_lastSortExp = "Cache:lastSortExp3_16";
		private const string KEY_lastSortUrl = "Cache:lastSortUrl3_16";
		private const string KEY_lastSortTog = "Cache:lastSortTog3_16";
		private const string KEY_lastImpPwdOvride = "Cache:lastImpPwdOvride3_16";
		private const string KEY_cntImpPwdOvride = "Cache:cntImpPwdOvride3_16";
		private const string KEY_currPageIndex = "Cache:currPageIndex3_16";
		private const string KEY_bHiImpVisible = "Cache:bHiImpVisible3_16";
		private const string KEY_bShImpVisible = "Cache:bShImpVisible3_16";
		private const string KEY_dtAdmAuthColGrid = "Cache:dtAdmAuthColGrid";
		private const string KEY_scrImport = "Cache:scrImport3_16";
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_16";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_16";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_16";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_16";
		private const string KEY_dtLabel = "Cache:dtLabel3_16";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_16";
		private const string KEY_dtScrCri = "Cache:dtScrCri3_16";
		private const string KEY_dsScrCriVal = "Cache:dsScrCriVal3_16";

		private const string KEY_dtPermKeyId241 = "Cache:dtPermKeyId241";
		private const string KEY_dtPermKeyRowId241 = "Cache:dtPermKeyRowId241";

		private const string KEY_dtSystems = "Cache:dtSystems3_16";
		private const string KEY_sysConnectionString = "Cache:sysConnectionString3_16";
		private const string KEY_dtAdmAuthCol16List = "Cache:dtAdmAuthCol16List";
		private const string KEY_bNewVisible = "Cache:bNewVisible3_16";
		private const string KEY_bNewSaveVisible = "Cache:bNewSaveVisible3_16";
		private const string KEY_bCopyVisible = "Cache:bCopyVisible3_16";
		private const string KEY_bCopySaveVisible = "Cache:bCopySaveVisible3_16";
		private const string KEY_bDeleteVisible = "Cache:bDeleteVisible3_16";
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_16";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_16";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_16";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				DataTable dtMenuAccess = (new MenuSystem()).GetMenu(base.LUser.CultureId, 3, base.LImpr, LcSysConnString, LcAppPw,16, null, null);
				...
				Session.Remove(KEY_dtPermKeyId241);
				Session.Remove(KEY_dtPermKeyRowId241);
				SetButtonHlp();
				GetSystems();
				SetColumnAuthority();
				GetGlobalFilter();
				...
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 16, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmAuthCol16List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=16&typ=N&sys=3','','',''); return false;"; }
			...
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmAuthColGrid];
				if (dt != null)
				{
		...
		private void InitializeComponent()
		{
			...
			if (LcSysConnString == null)
			{
				...
				{
					DataTable dt;
					dt = (new AdminSystem()).GetAuthRow(16,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
					...
					{
						cAdmAuthColGrid.PreRender += new EventHandler(cAdmAuthColGrid_OnPreRender);
					}
				}
		...
		private void SetSystem(byte SystemId)
		{
			...
			try
			{
				...
				if ((Config.DeployType == "DEV" || row["dbAppDatabase"].ToString() == base.CPrj.EntityCode + "View") && !(base.CPrj.EntityCode != "RO" && row["SysProgram"].ToString() == "Y") && (new AdminSystem()).IsRegenNeeded(string.Empty,16,0,0,LcSysConnString,LcAppPw))
				{
					(new GenScreensSystem()).CreateProgram(16, "Object Authorization", row["dbAppDatabase"].ToString(), base.CPrj, base.CSrc, base.CTar, LcAppConnString, LcAppPw);
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(16,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(16,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void SetClientRule(ListViewDataItem ee, bool isEditItem)
		{
			...
			if (dvRul.Count > 0)
			{
				...
				{
					if (ee == null || drv["ScriptEvent"].ToString().Substring(0,2).ToLower() != "on" || (cAdmAuthColGrid.EditIndex > -1 && GetDataItemIndex(cAdmAuthColGrid.EditIndex) == ee.DataItemIndex))
					{
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(16,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(16,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,16,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(16,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(16,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(16,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				try
				{
					try {dv = new DataView((new AdminSystem()).GetExp(16,"GetExpAdmAuthCol16","Y",(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
					catch {dv = new DataView((new AdminSystem()).GetExp(16,"GetExpAdmAuthCol16","N",(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(16,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					foreach (DataRowView drv in dv)
					{
						...
						if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["ColOvrdId241"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["ColVisible241"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["ColReadOnly241"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["ColExport241"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append(drv["PermKeyId241"].ToString() + (char)9 + drv["PermKeyId241Text"].ToString() + (char)9);}
						if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append(drv["PermKeyRowId241"].ToString() + (char)9 + drv["PermKeyRowId241Text"].ToString() + (char)9);}
						if (dtAu.Rows[7]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["Priority241"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[8]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["ToolTip241"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[9]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["ColumnHeader241"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[10]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["ErrMessage241"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						sb.Append(Environment.NewLine);
					}
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmAuthCol.csv"; Session["ExportStr"] = (Config.ExportExcelCSV ? "sep=\t\n": "") + sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmAuthCol.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private string GetRtfGrid(DataView dv)
		{
			...
			try
			{
				...
				foreach (DataRowView drv in dv)
				{
					...
					if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["ColOvrdId241"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(drv["ColVisible241"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(drv["ColReadOnly241"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append(drv["ColExport241"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append(drv["PermKeyId241Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append(drv["PermKeyRowId241Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[7]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["Priority241"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[8]["ColExport"].ToString() == "Y") {sb.Append(drv["ToolTip241"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[9]["ColExport"].ToString() == "Y") {sb.Append(drv["ColumnHeader241"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[10]["ColExport"].ToString() == "Y") {sb.Append(drv["ErrMessage241"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					sb.Append(@"}");
					sb.Append("\r\n");
					sb.Append(@"\pard \ql \li0\ri0\widctlpar\intbl\aspalpha\aspnum\adjustright\rin0\lin0 {");
					sb.Append(@"\insrsid2981395 \trowd \irow0\irowband0\lastrow \ts15\trgaph108\trleft-108\trbrdrt\brdrs\brdrw10 \trbrdrl\brdrs\brdrw10 \trbrdrb\brdrs\brdrw10 \trbrdrr\brdrs\brdrw10 \trbrdrh\brdrs\brdrw10 \trbrdrv\brdrs\brdrw10 ");
		...
		private void GetPageInfo()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastPageInfo(16, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 16, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			// *** Criteria Trigger (before) Web Rule starts here *** //
			DataTable dt = (DataTable)Session[KEY_dtAdmAuthColGrid];
			if (cAdmAuthColGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
			if (IsPostBack && cCriteria.Visible) ReBindCriteria(GetScrCriteria());
			UpdCriteria(true);
			cAdmAuthCol16List.ClearSearch(); Session.Remove(KEY_dtAdmAuthCol16List);
			PopAdmAuthCol16List(sender, e, false, null);
			}
		}
			...
			try
			{
				foreach (DataRowView drv in dvCri)
				{
					...
					if (drv["DisplayName"].ToString() == "ComboBox")
					{
						...
						(new AdminSystem()).MkGetScreenIn("16", drv["ScreenCriId"].ToString(), "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), LcAppDb, LcDesDb, drv["MultiDesignDb"].ToString(), LcSysConnString, LcAppPw);
						DataView dv = new DataView((new AdminSystem()).GetScreenIn("16", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), (new AdminSystem()).CountScrCri(drv["ScreenCriId"].ToString(), drv["MultiDesignDb"].ToString(), drv["MultiDesignDb"].ToString() == "N" ? LcSysConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)), drv["RequiredValid"].ToString(), 0, string.Empty, drv["DisplayMode"].ToString() != "AutoListBox", val, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)));
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("16", LcSysConnString, LcAppPw);
				}
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmAuthCol16" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmAuthCol16" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			foreach (DataRowView drv in dvCri)
			{
			    if (drv["DisplayName"].ToString() == "ListBox")
			    {
					...
					if (cListBox != null)
					{
						...
						int TotalChoiceCnt = new DataView((new AdminSystem()).GetScreenIn("16", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), CriCnt, drv["RequiredValid"].ToString(), 0, string.Empty, true, string.Empty, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId))).Count;
						string selectedValues = string.Join(",", cListBox.Items.Cast<ListItem>().Where(x => x.Selected).Select(x => "'" + x.Value + "'").ToArray());
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("16", "AdmAuthCol16", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		private void SetPermKeyId241(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtPermKeyId241];
			...
			if (ddl != null)
			{
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(16,"GetDdlPermKeyId3S182",true,bAll,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					if (dv.Table.Columns.Contains("ScreenObjId"))
					{
						...
						if (string.IsNullOrEmpty(cAdmAuthCol16List.SelectedValue)) {ss = ss + ")";} else {ss = ss + " OR ScreenObjId = " + cAdmAuthCol16List.SelectedValue + ")";}
					}
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "PermKeyId241 is not NULL"; }
					dv.RowFilter = ss;
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(16,"GetDdlPermKeyId3S182",true,true,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtPermKeyId241] = dv.Table;
				}
			}
		}

		protected void cbPermKeyRowId241(object sender, System.EventArgs e)
		{
			SetPermKeyRowId241((RoboCoder.WebControls.ComboBox)sender,string.Empty,((DropDownList)cAdmAuthColGrid.EditItem.FindControl("cPermKeyId241")).SelectedValue,cAdmAuthColGrid.EditItem);
		}

		private void SetPermKeyRowId241(RoboCoder.WebControls.ComboBox ddl, string keyId, string filtr, ListViewItem li)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			context["refColCID"] = li != null ? li.FindControl("cPermKeyId241").ClientID : string.Empty;
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(16,"GetDdlPermKeyRowId3S1887",true,false,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
					if (dv.Table.Columns.Contains("ScreenObjId"))
					{
						context["pMKeyColID"] = cAdmAuthCol16List.ClientID;
						context["pMKeyCol"] = "ScreenObjId";
						string ss = "(ScreenObjId is null";
						if (string.IsNullOrEmpty(cAdmAuthCol16List.SelectedValue)) {ss = ss + ")";} else {ss = ss + " OR ScreenObjId = " + cAdmAuthCol16List.SelectedValue + ")";}
						dv.RowFilter = ss;
					}
					ddl.DataSource = dv; Session[KEY_dtPermKeyRowId241] = dv.Table;
		...
		private DataView GetAdmAuthCol16List()
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmAuthCol16List];
			if (dt == null)
			{
				CheckAuthentication(false);
				int filterId = 0;
				...
				try
				{
					try {dt = (new AdminSystem()).GetLis(16,"GetLisAdmAuthCol16",true,"Y",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
					catch {dt = (new AdminSystem()).GetLis(16,"GetLisAdmAuthCol16",true,"N",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				}
		...
		private void PopAdmAuthCol16List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			DataView dv = GetAdmAuthCol16List();
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			context["isSys"] = "N";
			context["conn"] = KEY_sysConnectionString;
			cAdmAuthCol16List.AutoCompleteUrl = "AutoComplete.aspx/LisSuggests";
			cAdmAuthCol16List.DataContext = context;
			if (dv.Table == null) return;
			cAdmAuthCol16List.DataSource = dv;
			cAdmAuthCol16List.Visible = true;
			if (cAdmAuthCol16List.Items.Count <= 0) {cAdmAuthCol16List.Visible = false; cAdmAuthCol16List_SelectedIndexChanged(sender,e);}
			else
			{
				if (bPageLoad && !string.IsNullOrEmpty(Request.QueryString["key"]) && bUseCri.Value == string.Empty)
				{
					try {cAdmAuthCol16List.SelectByValue(Request.QueryString["key"],string.Empty,true);} catch {cAdmAuthCol16List.Items[0].Selected = true; cAdmAuthCol16List_SelectedIndexChanged(sender, e);}
				    cScreenSearch.Visible = false; cSystem.Visible = false; cEditButton.Visible = true; cSaveCloseButton.Visible = cSaveButton.Visible;
				}
				else
				{
					if (ID != null) {if (!cAdmAuthCol16List.SelectByValue(ID,string.Empty,true)) {ID = null;}}
					if (ID == null)
					{
						cAdmAuthCol16List_SelectedIndexChanged(sender, e);
					}
				}
			}
		}
		...
		protected void cAdmAuthCol16List_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmAuthCol16List_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmAuthCol16List_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			InitMaster(sender, e);      // Do this first to enable defaults for non-database columns.
			DataTable dt = null;
			try
			{
				dt = (new AdminSystem()).GetMstById("GetAdmAuthCol16ById",cAdmAuthCol16List.SelectedValue,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId));
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				if (dt.Rows.Count > 0)
				{
					cAdmAuthCol16List.SetDdVisible();
					DataRow dr = dt.Rows[0];
			...
			DataTable dtAdmAuthColGrid = null;
			int filterId = 0; if (Utils.IsInt(cFilterId.SelectedValue)) { filterId = int.Parse(cFilterId.SelectedValue); }
			try
			{
				dtAdmAuthColGrid = (new AdminSystem()).GetDtlById(16,"GetAdmAuthCol16DtlById",cAdmAuthCol16List.SelectedValue,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,base.LImpr,base.LCurr);
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (!dtAdmAuthColGrid.Columns.Contains("_NewRow")) { dtAdmAuthColGrid.Columns.Add("_NewRow"); }
			Session[KEY_dtAdmAuthColGrid] = dtAdmAuthColGrid;
			cAdmAuthColGrid.EditIndex = -1;
			cAdmAuthColGridDataPager.PageSize = Int16.Parse(cPgSize.Text); GotoPage(0);
			if (Session[KEY_lastSortCol] != null)
			{
				cAdmAuthColGrid_OnSorting(sender, new ListViewSortEventArgs((string)Session[KEY_lastSortExp], SortDirection.Ascending));
			}
			else if (dtAdmAuthColGrid.Rows.Count <= 0 || (!((GetAuthRow().Rows[0]["AllowUpd"].ToString() == "N" || GetAuthRow().Rows[0]["ViewOnly"].ToString() == "G") && GetAuthRow().Rows[0]["AllowAdd"].ToString() == "N") && !(Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N") && dtAdmAuthColGrid.Rows.Count == 1))
			{
				cAdmAuthColGrid_DataBind(dtAdmAuthColGrid.DefaultView);
			}
			cFindButton_Click(sender, e);
			cNaviPanel.Visible = true; cImportPwdPanel.Visible = false; Session.Remove(KEY_scrImport);
			ScriptManager.GetCurrent(Parent.Page).SetFocus(cAdmAuthCol16List.FocusID);
			ShowDirty(false); PanelTop.Update();
			// *** List Selection (End of) Web Rule starts here *** //
		}

		protected void cPermKeyId241_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			...
			TextBox pwd = null;
			if (cAdmAuthColGrid.EditIndex > -1 && cAdmAuthColGrid.Items.Count > cAdmAuthColGrid.EditIndex)
			{
				if (Session[KEY_dtPermKeyId241] != null)
				{
					DataView dv1 = ((DataTable)Session[KEY_dtPermKeyId241]).DefaultView; dv1.RowFilter = string.Empty;
				}
				DataView dvAdmAuthColGrid = ((DataTable)Session[KEY_dtAdmAuthColGrid]).DefaultView;
				if (Session[KEY_dtPermKeyId241] != null)
				{
					DataView dv3 = ((DataTable)Session[KEY_dtPermKeyId241]).DefaultView;
				}
				Session[KEY_dtAdmAuthColGrid] = dvAdmAuthColGrid.Table;
				SetPermKeyRowId241((RoboCoder.WebControls.ComboBox)cAdmAuthColGrid.EditItem.FindControl("cPermKeyRowId241"),dvAdmAuthColGrid[cAdmAuthColGrid.EditItem.DataItemIndex]["PermKeyRowId241"].ToString(),((DropDownList)sender).SelectedValue, cAdmAuthColGrid.EditItem);
			}
		...
		public void cFindButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dv != null)
			{
				...
				if ((cc != null && cc.ID.Equals("cAdmAuthCol16List")) || cAdmAuthColGrid.EditIndex < 0 || UpdateGridRow(cAdmAuthColGrid, new CommandEventArgs("Save", "")))
				{
					...
					GotoPage(0); cAdmAuthColGrid_DataBind(dv);
					if (GetCurrPageIndex() != (int)Session[KEY_currPageIndex] && (int)Session[KEY_currPageIndex] < GetTotalPages()) { GotoPage((int)Session[KEY_currPageIndex]); cAdmAuthColGrid_DataBind(dv);}
		...
		public void cInsRowButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dt != null && (ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")))
			{
				...
				Session[KEY_lastAddedRow] = 0; Session[KEY_dtAdmAuthColGrid] = dt; Session[KEY_currPageIndex] = 0; GotoPage(0);
				cAdmAuthColGrid_DataBind(dt.DefaultView);
				cAdmAuthColGrid_OnItemEditing(cAdmAuthColGrid, new ListViewEditEventArgs(0));
		...
		public void cPgSizeButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cAdmAuthColGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				try {if (int.Parse(cPgSize.Text) < 1 || int.Parse(cPgSize.Text) > 200) {cPgSize.Text = "6";}} catch {cPgSize.Text = "6";}
				cAdmAuthColGridDataPager.PageSize = int.Parse(cPgSize.Text); GotoPage(0);
				if (dt.Rows.Count <= 0 || (!((GetAuthRow().Rows[0]["AllowUpd"].ToString() == "N" || GetAuthRow().Rows[0]["ViewOnly"].ToString() == "G") && GetAuthRow().Rows[0]["AllowAdd"].ToString() == "N") && !(Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N") && dt.Rows.Count == 1)) {cAdmAuthColGrid_DataBind(dt.DefaultView);} else {cFindButton_Click(sender, e);}
				try
				{
					(new AdminSystem()).UpdLastPageInfo(16, base.LUser.UsrId, cPgSize.Text, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		}
		...
		public void cShowImpButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cSchemaImage.Visible)
			{
				try
				{
					Session["ImportSchema"] = (new AdminSystem()).GetSchemaScrImp(16,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (cSchemaImage.Attributes["OnClick"] == null || cSchemaImage.Attributes["OnClick"].IndexOf("ImportSchema.aspx") < 0) {cSchemaImage.Attributes["OnClick"] += "SearchLink('ImportSchema.aspx?scm=S&key=16&csy=3','','',''); return false;";}
			}
			// *** System Button Click (after) Web Rule starts here *** //
		}
		...
		private void ScrImportDdl(DataView dvd, DataRowView drv, bool bComboBox, string PKey, string CKey, string CNam, int MaxLen, string MatchCd, bool bAllowNulls)
		{
			if (dvd != null)
			{
				...
				{
					if (string.IsNullOrEmpty(cAdmAuthCol16List.SelectedValue))
					{
						dvd.RowFilter = "(" + PKey + " is null)";
					}
					else
					{
						dvd.RowFilter = "(" + PKey + " is null OR " + PKey + " = " + cAdmAuthCol16List.SelectedValue + ")";
					}
				}
		...
		protected void ScrImport(bool bClear)
		{
			...
			DataTable dtAdmAuthColGrid = (DataTable)Session[KEY_dtAdmAuthColGrid];
			if (dti != null && dtAdmAuthColGrid != null)
			{
				if (bClear)
				{
					dtAdmAuthColGrid.Rows.Clear();
				}
				cFind.Text = string.Empty;
				int iExist = dtAdmAuthColGrid.Rows.Count;
				...
				foreach (DataRowView drv in dti.DefaultView)
				{
					...
					{
						DataTable dtd;
						dtd = (DataTable)Session[KEY_dtPermKeyId241];
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(16,"GetDdlPermKeyId3S182",true,true,0,string.Empty,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
							Session[KEY_dtPermKeyId241] = dtd;
						}
						ScrImportDdl(dtd.DefaultView, drv, false, "ScreenObjId", "PermKeyId241", "PermKeyId241Text", 50, "2", true);
						dtd = jj==iExist ? null : (DataTable)Session[KEY_dtPermKeyRowId241];
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(16,"GetDdlPermKeyRowId3S1887",true,true,0,string.Empty,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
							Session[KEY_dtPermKeyRowId241] = dtd;
						}
						ScrImportDdl(dtd.DefaultView, drv, true, "ScreenObjId", "PermKeyRowId241", "PermKeyRowId241Text", 105, "2", true);
					}
					jj = jj + 1;
				}
				iRow = dti.DefaultView.Count - iExist;
				if (!dti.Columns.Contains("_NewRow")) { dti.Columns.Add("_NewRow"); }
				Session[KEY_dtAdmAuthColGrid] = dti;
				cAdmAuthColGrid_DataBind(dti.DefaultView);
		...
		public void cImportButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cFNameO.Text != string.Empty && cWorkSheet.Items.Count > 0 && cWorkSheet.SelectedItem.Text != string.Empty && cStartRow.Text != string.Empty)
			{
				...
				try
				{
					if (cAdmAuthColGrid.EditIndex < 0 || UpdateGridRow(cAdmAuthColGrid, new CommandEventArgs("Save", "")))
					{
						Session[KEY_scrImport] = ImportFile(dtAdmAuthColGrid.Copy(),cFNameO.Text,cWorkSheet.SelectedItem.Text,cStartRow.Text,Config.PathTmpImport + cFName.Text);
					}
		...
		private void GotoPage(int pageNo)
		{
			if (pageNo >= 0 && pageNo < GetTotalPages())
			{
				if (cAdmAuthColGrid.EditIndex < 0 || UpdateGridRow(cAdmAuthColGrid, new CommandEventArgs("Save", "")))
				{
					try { cAdmAuthColGridDataPager.SetPageProperties(cAdmAuthColGridDataPager.PageSize * pageNo, cAdmAuthColGridDataPager.MaximumRows, false); cAdmAuthColGrid_DataBind(null); }
					catch
					{
						try { cAdmAuthColGridDataPager.SetPageProperties(0, cAdmAuthColGridDataPager.MaximumRows, false); cAdmAuthColGrid_DataBind(null); } catch {}
					}
				}
			}
		}

		protected void cAdmAuthColGrid_OnItemCommand(object sender, ListViewCommandEventArgs e)
		{
		}

		private void cAdmAuthColGrid_DataBind(DataView dvAdmAuthColGrid)
		{
			if (dvAdmAuthColGrid == null)
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmAuthColGrid];
				dvAdmAuthColGrid = dt != null ? dt.DefaultView : null;
			}
			if (dvAdmAuthColGrid != null)
			{
				LcAuth = GetAuthCol().AsEnumerable().ToDictionary<DataRow,string>(dr=>dr["ColName"].ToString());
				cAdmAuthColGrid.DataSource = dvAdmAuthColGrid;
				cAdmAuthColGrid.DataBind();
		...
		protected void cAdmAuthColGrid_OnSorting(object sender, ListViewSortEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmAuthColGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null)
			{
				if (cAdmAuthColGrid.EditIndex < 0 || UpdateGridRow(cAdmAuthColGrid, new CommandEventArgs("Save", "")))
				{
					...
					Session[KEY_dtAdmAuthColGrid] = dt;
					cAdmAuthColGrid_DataBind(dv);
				}
			}
		}

		protected void cAdmAuthColGrid_OnPagePropertiesChanging(object sender, PagePropertiesChangingEventArgs e)
		{
		    cAdmAuthColGridDataPager.SetPageProperties(e.StartRowIndex, e.MaximumRows, false);
		    DataTable dt = (DataTable)Session[KEY_dtAdmAuthColGrid];
		    DataView dv = dt != null ? dt.DefaultView : null;
			cAdmAuthColGrid_DataBind(dv); cAdmAuthColGrid.EditIndex = -1;
		}

		private void GridChkPgDirty(ListViewItem lvi)
		{
				WebControl cc = null;
				cc = ((WebControl)lvi.FindControl("cColVisible241"));
				...
				cc = ((WebControl)lvi.FindControl("cColReadOnly241"));
				if ((cc.Attributes["OnClick"] == null || cc.Attributes["OnClick"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnClick"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty(); this.focus();";}
				cc = ((WebControl)lvi.FindControl("cColExport241"));
				if ((cc.Attributes["OnClick"] == null || cc.Attributes["OnClick"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnClick"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty(); this.focus();";}
				cc = ((WebControl)lvi.FindControl("cPermKeyId241"));
				...
				cc = ((WebControl)lvi.FindControl("cPermKeyRowId241"));
				...
				cc = ((WebControl)lvi.FindControl("cPriority241"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cToolTip241"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cColumnHeader241"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cErrMessage241"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
		}

		protected void cAdmAuthColGrid_OnItemDataBound(object sender, ListViewItemEventArgs e)
		{
			// *** GridItemDataBound (before) Web Rule End *** //
			DataTable dt = (DataTable)Session[KEY_dtAdmAuthColGrid];
			bool isEditItem = false;
			DataView dvAdmAuthColGrid = dt != null ? dt.DefaultView : null;
			if (cAdmAuthColGrid.EditIndex > -1 && GetDataItemIndex(cAdmAuthColGrid.EditIndex) == e.Item.DataItemIndex)
			{
				isEditItem = true;
				base.SetGridEnabled(e.Item, GetAuthCol(), GetLabel(), 1);
				CheckBox cb = null;
				cb = (CheckBox)e.Item.FindControl("cColVisible241");
				if (cb != null)
				{
					cb.Checked = base.GetBool(dvAdmAuthColGrid[e.Item.DataItemIndex]["ColVisible241"].ToString());
				}
				cb = (CheckBox)e.Item.FindControl("cColReadOnly241");
				if (cb != null)
				{
					cb.Checked = base.GetBool(dvAdmAuthColGrid[e.Item.DataItemIndex]["ColReadOnly241"].ToString());
				}
				cb = (CheckBox)e.Item.FindControl("cColExport241");
				if (cb != null)
				{
					cb.Checked = base.GetBool(dvAdmAuthColGrid[e.Item.DataItemIndex]["ColExport241"].ToString());
				}
				SetPermKeyId241((DropDownList)e.Item.FindControl("cPermKeyId241"),dvAdmAuthColGrid[e.Item.DataItemIndex]["PermKeyId241"].ToString());
				SetPermKeyRowId241((RoboCoder.WebControls.ComboBox)e.Item.FindControl("cPermKeyRowId241"),dvAdmAuthColGrid[e.Item.DataItemIndex]["PermKeyRowId241"].ToString(),dvAdmAuthColGrid[e.Item.DataItemIndex]["PermKeyId241"].ToString(), e.Item);
			...
			if (dtAuthRow != null)
			{
				...
				{
					cc = e.Item.FindControl("cAdmAuthColGridDelete"); if (cc != null) { cc.Visible = false; }
					cc = e.Item.FindControl("cAdmAuthColGridEdit"); if (cc != null) { cc.Visible = false; }
				}
				else
				{
			        HtmlTableRow tr = e.Item.FindControl("cAdmAuthColGridRow") as HtmlTableRow;
			        LinkButton lb = e.Item.FindControl("cAdmAuthColGridEdit") as LinkButton;
		...
		protected void cColOvrdId241hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "0") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "0"; Session[KEY_lastSortExp] = "ColOvrdId241";Session[KEY_lastSortImg] = "cColOvrdId241hi";
			cAdmAuthColGrid_OnSorting(sender, new ListViewSortEventArgs("ColOvrdId241", SortDirection.Ascending));
		}

		protected void cColVisible241hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "1") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "1"; Session[KEY_lastSortExp] = "ColVisible241";Session[KEY_lastSortImg] = "cColVisible241hi";
			cAdmAuthColGrid_OnSorting(sender, new ListViewSortEventArgs("ColVisible241", SortDirection.Ascending));
		}

		protected void cColReadOnly241hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "2") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "2"; Session[KEY_lastSortExp] = "ColReadOnly241";Session[KEY_lastSortImg] = "cColReadOnly241hi";
			cAdmAuthColGrid_OnSorting(sender, new ListViewSortEventArgs("ColReadOnly241", SortDirection.Ascending));
		}

		protected void cColExport241hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "3") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "3"; Session[KEY_lastSortExp] = "ColExport241";Session[KEY_lastSortImg] = "cColExport241hi";
			cAdmAuthColGrid_OnSorting(sender, new ListViewSortEventArgs("ColExport241", SortDirection.Ascending));
		}

		protected void cPermKeyId241hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "4") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "4"; Session[KEY_lastSortExp] = "PermKeyId241Text";Session[KEY_lastSortImg] = "cPermKeyId241hi";
			cAdmAuthColGrid_OnSorting(sender, new ListViewSortEventArgs("PermKeyId241Text", SortDirection.Ascending));
		}

		protected void cPermKeyRowId241hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "5") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "5"; Session[KEY_lastSortExp] = "PermKeyRowId241Text";Session[KEY_lastSortImg] = "cPermKeyRowId241hi";
			cAdmAuthColGrid_OnSorting(sender, new ListViewSortEventArgs("PermKeyRowId241Text", SortDirection.Ascending));
		}

		protected void cPriority241hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "6") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "6"; Session[KEY_lastSortExp] = "Priority241";Session[KEY_lastSortImg] = "cPriority241hi";
			cAdmAuthColGrid_OnSorting(sender, new ListViewSortEventArgs("Priority241", SortDirection.Ascending));
		}

		protected void cToolTip241hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "7") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "7"; Session[KEY_lastSortExp] = "ToolTip241";Session[KEY_lastSortImg] = "cToolTip241hi";
			cAdmAuthColGrid_OnSorting(sender, new ListViewSortEventArgs("ToolTip241", SortDirection.Ascending));
		}

		protected void cColumnHeader241hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "8") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "8"; Session[KEY_lastSortExp] = "ColumnHeader241";Session[KEY_lastSortImg] = "cColumnHeader241hi";
			cAdmAuthColGrid_OnSorting(sender, new ListViewSortEventArgs("ColumnHeader241", SortDirection.Ascending));
		}

		protected void cErrMessage241hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "9") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "9"; Session[KEY_lastSortExp] = "ErrMessage241";Session[KEY_lastSortImg] = "cErrMessage241hi";
			cAdmAuthColGrid_OnSorting(sender, new ListViewSortEventArgs("ErrMessage241", SortDirection.Ascending));
		}

		protected void cAdmAuthColGrid_OnItemEditing(object sender, ListViewEditEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmAuthColGrid];
			if ((ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")) && dt != null && dt.DefaultView.Count > GetDataItemIndex(e.NewEditIndex))
			{
				cAdmAuthColGrid.EditIndex = e.NewEditIndex;
				cAdmAuthColGrid_DataBind(null);
				cPermKeyId241_SelectedIndexChanged((object)cAdmAuthColGrid.EditItem.FindControl("cPermKeyId241"), new EventArgs());
				Control wc = null;
				string idx = Page.Request["__EVENTARGUMENT"];
				if (!string.IsNullOrEmpty(idx))
				{
				    Control ctrlToFocus = cAdmAuthColGrid.EditItem.FindControl("c" + idx);
				    if (((WebControl)ctrlToFocus).Enabled) { wc = ctrlToFocus; }
				}
				if (wc == null) { wc = FindEditableControl(cAdmAuthColGrid.EditItem); }
				if (wc != null)
             {
             ...
             MkMobileLabel(cAdmAuthColGrid.EditItem);
		    }
		    else { e.Cancel = true; }
		}
		...
		{
		    Label ml = null;
		    ml = lvi.FindControl("cColOvrdId241ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(1); }
		    ml = lvi.FindControl("cColVisible241ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(2); }
		    ml = lvi.FindControl("cColReadOnly241ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(3); }
		    ml = lvi.FindControl("cColExport241ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(4); }
		    ml = lvi.FindControl("cPermKeyId241ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(5); }
		    ml = lvi.FindControl("cPermKeyRowId241ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(6); }
		    ml = lvi.FindControl("cPriority241ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(7); }
		    ml = lvi.FindControl("cToolTip241ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(8); }
		    ml = lvi.FindControl("cColumnHeader241ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(9); }
		    ml = lvi.FindControl("cErrMessage241ml") as Label;
		...
		{
			CheckBox cb = null;
			cb = (CheckBox)lvi.FindControl("cColVisible241");
			if (cb != null)
			{
				dr["ColVisible241"] = base.SetBool(cb.Checked);
			}
			cb = (CheckBox)lvi.FindControl("cColReadOnly241");
			if (cb != null)
			{
				dr["ColReadOnly241"] = base.SetBool(cb.Checked);
			}
			cb = (CheckBox)lvi.FindControl("cColExport241");
			if (cb != null)
			{
				dr["ColExport241"] = base.SetBool(cb.Checked);
			}
			DropDownList ddl = null;
			ddl = (DropDownList)lvi.FindControl("cPermKeyId241");
			if (ddl != null)
			{
				if (ddl.SelectedIndex >= 0 && ddl.SelectedValue.ToString() != string.Empty)
				{
					dr["PermKeyId241"] = ddl.SelectedValue;
					dr["PermKeyId241Text"] = ddl.SelectedItem.Text;
				}
				else
				{
					dr["PermKeyId241"] = System.DBNull.Value;
					dr["PermKeyId241Text"] = System.DBNull.Value;
				}
			}
			RoboCoder.WebControls.ComboBox cbb = null;
			cbb = (RoboCoder.WebControls.ComboBox)lvi.FindControl("cPermKeyRowId241");
			if (cbb != null)
			{
				if (cbb.SelectedValue != string.Empty)
				{
					dr["PermKeyRowId241"] = cbb.SelectedValue;
					dr["PermKeyRowId241Text"] = cbb.SelectedText;
				}
				else
				{
					dr["PermKeyRowId241"] = System.DBNull.Value;
					dr["PermKeyRowId241Text"] = System.DBNull.Value;
				}
			}
			TextBox tb = null;
			tb = (TextBox)lvi.FindControl("cPriority241");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["Priority241"] = tb.Text;} else {dr["Priority241"] = System.DBNull.Value;}
			}
			tb = (TextBox)lvi.FindControl("cToolTip241");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["ToolTip241"] = tb.Text;} else {dr["ToolTip241"] = System.DBNull.Value;}
			}
			tb = (TextBox)lvi.FindControl("cColumnHeader241");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["ColumnHeader241"] = tb.Text;} else {dr["ColumnHeader241"] = System.DBNull.Value;}
			}
			tb = (TextBox)lvi.FindControl("cErrMessage241");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["ErrMessage241"] = tb.Text;} else {dr["ErrMessage241"] = System.DBNull.Value;}
			}
		...
		protected void cAdmAuthColGrid_OnItemUpdating(object sender, ListViewUpdateEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmAuthColGrid];
			if (dt != null && dt.Rows.Count > 0)
			{
				DataRow dr = dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row;
			    GridFill(cAdmAuthColGrid.EditItem, dt, dr, false);
			    dr["_NewRow"] = "N";
			    if (dt.Columns.Contains("_SortOrder")) dr["_SortOrder"] = null;
			}
		}

		protected void cAdmAuthColGrid_OnItemCanceling(object sender, ListViewCancelEventArgs e)
		{
			PanelTop.Update();
			DataTable dt = (DataTable)Session[KEY_dtAdmAuthColGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			...
			{
				cAdmAuthColGrid_OnItemDeleting(sender, new ListViewDeleteEventArgs(e.ItemIndex));
			}
			if (dt.Columns.Contains("_SortOrder")) dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row["_SortOrder"] = null;
			cAdmAuthColGrid.EditIndex = -1; cAdmAuthColGrid_DataBind(null);
		}
		...
		protected void cDeleteAllButton_Click(object sender, System.EventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmAuthColGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null && (cAdmAuthColGrid.EditIndex < 0 || UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				GotoPage(0);
				while (dv.Count > 0) {dv.Delete(0);}
				Session[KEY_dtAdmAuthColGrid] = dt;
				cAdmAuthColGrid.EditIndex = -1; cAdmAuthColGrid_DataBind(dv);
			}
		}

		protected void cAdmAuthColGrid_OnItemDeleting(object sender, ListViewDeleteEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmAuthColGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			DataRow dr = dv != null ? dv[GetDataItemIndex(e.ItemIndex)].Row : null;
			if (dv != null && (cAdmAuthColGrid.EditIndex == e.ItemIndex || UpdateGridRow(sender, new CommandEventArgs("Save",""))))
			{
		...
		public void cUndoAllButton_Click(object sender, System.EventArgs e)
		{
			...
			else { PopAdmAuthCol16List(sender, e, false, cScreenObjId14.Text); }
			// *** System Button Click (After) Web Rule starts here *** //
		}
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmAuthCol16List); PopAdmAuthCol16List(sender, e, false, null);
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			if (ValidPage() && (cAdmAuthColGrid.EditIndex < 0 || UpdateGridRow(cAdmAuthColGrid, new CommandEventArgs("Save", ""))))
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmAuthColGrid];
				DataView dv = dt != null ? dt.DefaultView : null;
				string ftr = string.Empty;
				if (dv.RowFilter != string.Empty) { ftr = dv.RowFilter; dv.RowFilter = string.Empty; }
				AdmAuthCol16 ds = PrepAdmAuthColData(dv,cScreenObjId14.Text == string.Empty);
				if (ftr != string.Empty) {dv.RowFilter = ftr;}
				if (!string.IsNullOrEmpty(cAdmAuthCol16List.SelectedValue) && cScreenObjId14.Text != cAdmAuthCol16List.SelectedValue)
				{
					PreMsgPopup("Primary key cannot be changed");
					return rtn;
				}
				if (string.IsNullOrEmpty(cAdmAuthCol16List.SelectedValue))	// Add
				{
					if (ds != null)
					{
						pid = (new AdminSystem()).AddData(16,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc,noTrans,commandTimeOut);
					}
					if (!string.IsNullOrEmpty(pid))
					{
						cAdmAuthCol16List.ClearSearch(); Session.Remove(KEY_dtAdmAuthCol16List);
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmAuthCol16List(sender, e, false, pid);
						rtn = GetScreenHlp().Rows[0]["AddMsg"].ToString();
					}
				}
				else {
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(16,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc,noTrans,commandTimeOut)) {bValid7 = true;}
					if (bValid7)
					{
						cAdmAuthCol16List.ClearSearch(); Session.Remove(KEY_dtAdmAuthCol16List);
						Session[KEY_currPageIndex] = GetCurrPageIndex();
						ShowDirty(false); PopAdmAuthCol16List(sender, e, false, ds.Tables["AdmAuthCol"].Rows[0]["ScreenObjId14"]);
		...
		public void cDeleteButton_Click(object sender, System.EventArgs e)
		{
			...
			{
				AdmAuthCol16 ds = PrepAdmAuthColData(null,false);
				try
				{
					if (ds != null && (new AdminSystem()).DelData(16,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc))
					{
						cAdmAuthCol16List.ClearSearch(); Session.Remove(KEY_dtAdmAuthCol16List);
						ShowDirty(false); PopAdmAuthCol16List(sender, e, false, null);
		...
		private AdmAuthCol16 PrepAdmAuthColData(DataView dv, bool bAdd)
		{
			...
			if (dv != null)
			{
				...
				if (bAdd)
				{
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmAuthColAdd"].Rows.Add(MakeColRow(ds.Tables["AdmAuthColAdd"].NewRow(), drv, true));
					}
				}
				else
				{
					...
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmAuthColUpd"].Rows.Add(MakeColRow(ds.Tables["AdmAuthColUpd"].NewRow(), drv, false));
					}
					...
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmAuthColDel"].Rows.Add(MakeColRow(ds.Tables["AdmAuthColDel"].NewRow(), drv, false));
					}
		...
		public bool CanAct(char typ) { return CanAct(typ, GetAuthRow(), cAdmAuthCol16List.SelectedValue.ToString()); }
		...
		{
			dr["ScreenObjId14"] = cScreenObjId14.Text;
			DataTable dtAuth = GetAuthCol();
			if (dtAuth != null)
			{
				dr["ColOvrdId241"] = drv["ColOvrdId241"].ToString().Trim();
				...
				if (bAdd && dtAuth.Rows[5]["ColReadOnly"].ToString() == "Y" && dr["PermKeyId241"].ToString() == string.Empty) {dr["PermKeyId241"] = System.DBNull.Value;}
				dr["PermKeyRowId241"] = drv["PermKeyRowId241"];
				if (bAdd && dtAuth.Rows[6]["ColReadOnly"].ToString() == "Y" && dr["PermKeyRowId241"].ToString() == string.Empty) {dr["PermKeyRowId241"] = System.DBNull.Value;}
				dr["Priority241"] = drv["Priority241"].ToString().Trim();
				if (bAdd && dtAuth.Rows[7]["ColReadOnly"].ToString() == "Y" && dr["Priority241"].ToString() == string.Empty) {dr["Priority241"] = System.DBNull.Value;}
				dr["ToolTip241"] = drv["ToolTip241"].ToString().Trim();
				if (bAdd && dtAuth.Rows[8]["ColReadOnly"].ToString() == "Y" && dr["ToolTip241"].ToString() == string.Empty) {dr["ToolTip241"] = System.DBNull.Value;}
				dr["ColumnHeader241"] = drv["ColumnHeader241"].ToString().Trim();
				if (bAdd && dtAuth.Rows[9]["ColReadOnly"].ToString() == "Y" && dr["ColumnHeader241"].ToString() == string.Empty) {dr["ColumnHeader241"] = System.DBNull.Value;}
				dr["ErrMessage241"] = drv["ErrMessage241"].ToString().Trim();
				if (bAdd && dtAuth.Rows[10]["ColReadOnly"].ToString() == "Y" && dr["ErrMessage241"].ToString() == string.Empty) {dr["ErrMessage241"] = System.DBNull.Value;}
			}
			return dr;
		}
		...
		{
			if (cAdmAuthColGrid.EditIndex > -1)
			{
				cAdmAuthColGrid_OnItemUpdating(sender, new ListViewUpdateEventArgs(cAdmAuthColGrid.EditIndex));
			}
			return true;
		}

		protected void cAdmAuthColGrid_OnPreRender(object sender, System.EventArgs e)
		{
			System.Web.UI.WebControls.Image hi = null;
			hi = (System.Web.UI.WebControls.Image)cAdmAuthColGrid.FindControl("cColOvrdId241hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmAuthColGrid.FindControl("cColVisible241hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmAuthColGrid.FindControl("cColReadOnly241hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmAuthColGrid.FindControl("cColExport241hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmAuthColGrid.FindControl("cPermKeyId241hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmAuthColGrid.FindControl("cPermKeyRowId241hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmAuthColGrid.FindControl("cPriority241hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmAuthColGrid.FindControl("cToolTip241hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmAuthColGrid.FindControl("cColumnHeader241hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmAuthColGrid.FindControl("cErrMessage241hi"); if (hi != null) { hi.Visible = false; }
			if (Session[KEY_lastSortImg] != null)
			{
				hi = (System.Web.UI.WebControls.Image)cAdmAuthColGrid.FindControl((string)Session[KEY_lastSortImg]);
				if (hi != null) { hi.ImageUrl = Utils.AddTilde((string)Session[KEY_lastSortUrl]); hi.Visible = true; }
			}
			IgnoreHeaderConfirm((LinkButton)cAdmAuthColGrid.FindControl("cColOvrdId241hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmAuthColGrid.FindControl("cColVisible241hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmAuthColGrid.FindControl("cColReadOnly241hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmAuthColGrid.FindControl("cColExport241hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmAuthColGrid.FindControl("cPermKeyId241hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmAuthColGrid.FindControl("cPermKeyRowId241hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmAuthColGrid.FindControl("cPriority241hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmAuthColGrid.FindControl("cToolTip241hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmAuthColGrid.FindControl("cColumnHeader241hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmAuthColGrid.FindControl("cErrMessage241hl"));
		}

		private string GetButtonId(ListViewItem lvi)
		{
			string ButtonID = String.Empty;
			Control c = lvi.FindControl("cAdmAuthColGridEdit");
		...
		protected void cAdmAuthColGrid_OnLayoutCreated(object sender, EventArgs e)
		{
			...
			if (dtAuthRow != null)
			{
				...
				{
		            lb = cAdmAuthColGrid.FindControl("cDeleteAllButton") as LinkButton; if (lb != null) { lb.Visible = false; }
				}
			}
		    // footer:
		    Label gc = null;
		    gc = cAdmAuthColGrid.FindControl("cColOvrdId241fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(1); }
		    gc = cAdmAuthColGrid.FindControl("cColVisible241fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(2); }
		    gc = cAdmAuthColGrid.FindControl("cColReadOnly241fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(3); }
		    gc = cAdmAuthColGrid.FindControl("cColExport241fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(4); }
		    gc = cAdmAuthColGrid.FindControl("cPermKeyId241fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(5); }
		    gc = cAdmAuthColGrid.FindControl("cPermKeyRowId241fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(6); }
		    gc = cAdmAuthColGrid.FindControl("cPriority241fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(7); }
		    gc = cAdmAuthColGrid.FindControl("cToolTip241fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(8); }
		    gc = cAdmAuthColGrid.FindControl("cColumnHeader241fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(9); }
		    gc = cAdmAuthColGrid.FindControl("cErrMessage241fl") as Label;
		...
		private void PreMsgPopup(string msg) { PreMsgPopup(msg,cAdmAuthCol16List,null); }

...
namespace RO.Web
{
	public partial class AdmScrAuditModule : RO.Web.ModuleBase
	{

		private const string KEY_lastAddedRow = "Cache:lastAddedRow3_1019";
		private const string KEY_lastSortOrd = "Cache:lastSortOrd3_1019";
		private const string KEY_lastSortImg = "Cache:lastSortImg3_1019";
		private const string KEY_lastSortCol = "Cache:lastSortCol3_1019";
		private const string KEY_lastSortExp = "Cache:lastSortExp3_1019";
		private const string KEY_lastSortUrl = "Cache:lastSortUrl3_1019";
		private const string KEY_lastSortTog = "Cache:lastSortTog3_1019";
		private const string KEY_lastImpPwdOvride = "Cache:lastImpPwdOvride3_1019";
		private const string KEY_cntImpPwdOvride = "Cache:cntImpPwdOvride3_1019";
		private const string KEY_currPageIndex = "Cache:currPageIndex3_1019";
		private const string KEY_bHiImpVisible = "Cache:bHiImpVisible3_1019";
		private const string KEY_bShImpVisible = "Cache:bShImpVisible3_1019";
		private const string KEY_dtAdmScrAuditGrid = "Cache:dtAdmScrAuditGrid";
		private const string KEY_scrImport = "Cache:scrImport3_1019";
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_1019";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_1019";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_1019";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_1019";
		private const string KEY_dtLabel = "Cache:dtLabel3_1019";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_1019";
		private const string KEY_dtScrCri = "Cache:dtScrCri3_1019";
		private const string KEY_dsScrCriVal = "Cache:dsScrCriVal3_1019";

		private const string KEY_dtCudAction1300 = "Cache:dtCudAction1300";
		private const string KEY_dtScreenId1300 = "Cache:dtScreenId1300";
		private const string KEY_dtTableId1300 = "Cache:dtTableId1300";
		private const string KEY_dtChangedBy1300 = "Cache:dtChangedBy1300";

		private const string KEY_dtSystems = "Cache:dtSystems3_1019";
		private const string KEY_sysConnectionString = "Cache:sysConnectionString3_1019";
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_1019";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_1019";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_1019";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				DataTable dtMenuAccess = (new MenuSystem()).GetMenu(base.LUser.CultureId, 3, base.LImpr, LcSysConnString, LcAppPw,1019, null, null);
				...
				Session.Remove(KEY_dtAdmScrAuditGrid);
				...
				Session.Remove(KEY_dtCudAction1300);
				Session.Remove(KEY_dtScreenId1300);
				Session.Remove(KEY_dtTableId1300);
				Session.Remove(KEY_dtChangedBy1300);
				SetButtonHlp();
				GetSystems();
				SetColumnAuthority();
				GetGlobalFilter();
				...
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 1019, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmScrAudit1019List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=1019&typ=N&sys=3','','',''); return false;"; }
				//WebRule: Hide systems module and initialization
			...
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmScrAuditGrid];
				if (dt != null)
				{
		...
		private void InitializeComponent()
		{
			...
			if (LcSysConnString == null)
			{
				...
				{
					DataTable dt;
					dt = (new AdminSystem()).GetAuthRow(1019,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
					...
					{
						cAdmScrAuditGrid.PreRender += new EventHandler(cAdmScrAuditGrid_OnPreRender);
					}
				}
		...
		private void SetSystem(byte SystemId)
		{
			...
			try
			{
				...
				if ((Config.DeployType == "DEV" || row["dbAppDatabase"].ToString() == base.CPrj.EntityCode + "View") && !(base.CPrj.EntityCode != "RO" && row["SysProgram"].ToString() == "Y") && (new AdminSystem()).IsRegenNeeded(string.Empty,1019,0,0,LcSysConnString,LcAppPw))
				{
					(new GenScreensSystem()).CreateProgram(1019, "Screen Audit Header", row["dbAppDatabase"].ToString(), base.CPrj, base.CSrc, base.CTar, LcAppConnString, LcAppPw);
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(1019,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(1019,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void SetClientRule(ListViewDataItem ee, bool isEditItem)
		{
			...
			if (dvRul.Count > 0)
			{
				...
				{
					if (ee == null || drv["ScriptEvent"].ToString().Substring(0,2).ToLower() != "on" || (cAdmScrAuditGrid.EditIndex > -1 && GetDataItemIndex(cAdmScrAuditGrid.EditIndex) == ee.DataItemIndex))
					{
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(1019,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(1019,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,1019,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(1019,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(1019,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(1019,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				CheckAuthentication(false);
				DataView dv = null;
				try {dv = ((DataTable)Session[KEY_dtAdmScrAuditGrid]).DefaultView;}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(1019,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					{
						if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["ScrAuditId1300"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["CudAction1300"].ToString().Replace("\"","\"\"") + "\"" + (char)9 + "\"" + drv["CudAction1300Text"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(drv["ScreenId1300"].ToString() + (char)9 + drv["ScreenId1300Text"].ToString() + (char)9);}
						if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["MasterTable1300"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append(drv["TableId1300"].ToString() + (char)9 + drv["TableId1300Text"].ToString() + (char)9);}
						if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["RowId1300"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["RowDesc1300"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[7]["ColExport"].ToString() == "Y") {sb.Append(drv["ChangedBy1300"].ToString() + (char)9 + drv["ChangedBy1300Text"].ToString() + (char)9);}
						if (dtAu.Rows[8]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmShortDateTimeUTC(drv["ChangedOn1300"].ToString(),base.LUser.Culture,CurrTimeZoneInfo()) + (char)9);}
						sb.Append(Environment.NewLine);
					}
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmScrAudit.csv"; Session["ExportStr"] = (Config.ExportExcelCSV ? "sep=\t\n": "") + sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmScrAudit.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private string GetRtfGrid(DataView dv)
		{
			...
			try
			{
				...
				foreach (DataRowView drv in dv)
				{
					...
					sb.Append("\r\n");
					sb.Append(@"{");
					if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["ScrAuditId1300"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["CudAction1300Text"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(drv["ScreenId1300Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(drv["MasterTable1300"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append(drv["TableId1300Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["RowId1300"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append(drv["RowDesc1300"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[7]["ColExport"].ToString() == "Y") {sb.Append(drv["ChangedBy1300Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[8]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmShortDateTimeUTC(drv["ChangedOn1300"].ToString(),base.LUser.Culture,CurrTimeZoneInfo()) + @"\cell ");}
					sb.Append(@"}");
					sb.Append("\r\n");
					sb.Append(@"\pard \ql \li0\ri0\widctlpar\intbl\aspalpha\aspnum\adjustright\rin0\lin0 {");
					sb.Append(@"\insrsid2981395 \trowd \irow0\irowband0\lastrow \ts15\trgaph108\trleft-108\trbrdrt\brdrs\brdrw10 \trbrdrl\brdrs\brdrw10 \trbrdrb\brdrs\brdrw10 \trbrdrr\brdrs\brdrw10 \trbrdrh\brdrs\brdrw10 \trbrdrv\brdrs\brdrw10 ");
		...
		{
			//WebRule: Purge audit trail
            (new WebRule()).PurgeScrAudit(6, (string)Session[KEY_sysConnectionString], LcAppPw);
			// *** WebRule End *** //
		}
		...
		private void GetPageInfo()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastPageInfo(1019, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 1019, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			// *** Criteria Trigger (before) Web Rule starts here *** //
			DataTable dt = (DataTable)Session[KEY_dtAdmScrAuditGrid];
			if (cAdmScrAuditGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
			if (IsPostBack && cCriteria.Visible) ReBindCriteria(GetScrCriteria());
			UpdCriteria(true);
			PopAdmScrAudit1019List(sender, e, false, null);
			}
		}
			...
			try
			{
				foreach (DataRowView drv in dvCri)
				{
					...
					if (drv["DisplayName"].ToString() == "ComboBox")
					{
						...
						(new AdminSystem()).MkGetScreenIn("1019", drv["ScreenCriId"].ToString(), "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), LcAppDb, LcDesDb, drv["MultiDesignDb"].ToString(), LcSysConnString, LcAppPw);
						DataView dv = new DataView((new AdminSystem()).GetScreenIn("1019", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), (new AdminSystem()).CountScrCri(drv["ScreenCriId"].ToString(), drv["MultiDesignDb"].ToString(), drv["MultiDesignDb"].ToString() == "N" ? LcSysConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)), drv["RequiredValid"].ToString(), 0, string.Empty, drv["DisplayMode"].ToString() != "AutoListBox", val, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)));
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("1019", LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmScrAudit1019" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmScrAudit1019" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			foreach (DataRowView drv in dvCri)
			{
			    if (drv["DisplayName"].ToString() == "ListBox")
			    {
					...
					if (cListBox != null)
					{
						...
						int TotalChoiceCnt = new DataView((new AdminSystem()).GetScreenIn("1019", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), CriCnt, drv["RequiredValid"].ToString(), 0, string.Empty, true, string.Empty, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId))).Count;
						string selectedValues = string.Join(",", cListBox.Items.Cast<ListItem>().Where(x => x.Selected).Select(x => "'" + x.Value + "'").ToArray());
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("1019", "AdmScrAudit1019", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		private void SetCudAction1300(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtCudAction1300];
			...
			if (ddl != null)
			{
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(1019,"GetDdlCudAction3S4106",true,bAll,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "CudAction1300 is not NULL"; }
					dv.RowFilter = ss;
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(1019,"GetDdlCudAction3S4106",true,true,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtCudAction1300] = dv.Table;
				}
			}
		}

		protected void cbScreenId1300(object sender, System.EventArgs e)
		{
			SetScreenId1300((RoboCoder.WebControls.ComboBox)sender,string.Empty,cAdmScrAuditGrid.EditItem);
		}

		private void SetScreenId1300(RoboCoder.WebControls.ComboBox ddl, string keyId, ListViewItem li)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(1019,"GetDdlScreenId3S4107",true,false,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
					ddl.DataSource = dv; Session[KEY_dtScreenId1300] = dv.Table;
				    try { ddl.SelectByValue(keyId,string.Empty,true); } catch { try { ddl.SelectedIndex = 0; } catch { } }
				}
			}
		}

		protected void cbTableId1300(object sender, System.EventArgs e)
		{
			SetTableId1300((RoboCoder.WebControls.ComboBox)sender,string.Empty,cAdmScrAuditGrid.EditItem);
		}

		private void SetTableId1300(RoboCoder.WebControls.ComboBox ddl, string keyId, ListViewItem li)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(1019,"GetDdlTableId3S4109",true,false,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
					ddl.DataSource = dv; Session[KEY_dtTableId1300] = dv.Table;
				    try { ddl.SelectByValue(keyId,string.Empty,true); } catch { try { ddl.SelectedIndex = 0; } catch { } }
				}
			}
		}

		protected void cbChangedBy1300(object sender, System.EventArgs e)
		{
			SetChangedBy1300((RoboCoder.WebControls.ComboBox)sender,string.Empty,cAdmScrAuditGrid.EditItem);
		}

		private void SetChangedBy1300(RoboCoder.WebControls.ComboBox ddl, string keyId, ListViewItem li)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(1019,"GetDdlChangedBy3S4112",true,false,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
					ddl.DataSource = dv; Session[KEY_dtChangedBy1300] = dv.Table;
				    try { ddl.SelectByValue(keyId,string.Empty,true); } catch { try { ddl.SelectedIndex = 0; } catch { } }
				}
			}
		}

		private void PopAdmScrAudit1019List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			DataTable dt = null;
			CheckAuthentication(false);
			...
			try
			{
				try {dt = (new AdminSystem()).GetLis(1019,"GetLisAdmScrAudit1019",false,"Y",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				catch {dt = (new AdminSystem()).GetLis(1019,"GetLisAdmScrAudit1019",false,"N",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				Session[KEY_dtAdmScrAuditGrid] = dt;
				cAdmScrAuditGridDataPager.PageSize = Int16.Parse(cPgSize.Text); GotoPage(0);
			    if (Session[KEY_lastSortCol] != null)
			    {
					cAdmScrAuditGrid_OnSorting(sender, new ListViewSortEventArgs((string)Session[KEY_lastSortExp], SortDirection.Ascending));
			    }
				else
			    {
					cAdmScrAuditGrid_DataBind(dt.DefaultView);
			    }
		...
		public void cFindButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dv != null)
			{
				if (cAdmScrAuditGrid.EditIndex < 0 || UpdateGridRow(cAdmScrAuditGrid, new CommandEventArgs("Save", "")))
				{
					...
					GotoPage(0); cAdmScrAuditGrid_DataBind(dv);
					if (GetCurrPageIndex() != (int)Session[KEY_currPageIndex] && (int)Session[KEY_currPageIndex] < GetTotalPages()) { GotoPage((int)Session[KEY_currPageIndex]); cAdmScrAuditGrid_DataBind(dv);}
		...
		public void cInsRowButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dt != null && (ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")))
			{
				...
				Session[KEY_lastAddedRow] = 0; Session[KEY_dtAdmScrAuditGrid] = dt; Session[KEY_currPageIndex] = 0; GotoPage(0);
				cAdmScrAuditGrid_DataBind(dt.DefaultView);
				cAdmScrAuditGrid_OnItemEditing(cAdmScrAuditGrid, new ListViewEditEventArgs(0));
		...
		public void cPgSizeButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cAdmScrAuditGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				...
				cAdmScrAuditGridDataPager.PageSize = int.Parse(cPgSize.Text); GotoPage(0);
				if (dt.Rows.Count <= 0 || (GetAuthRow().Rows[0]["AllowAdd"].ToString() == "Y" && dt.Rows.Count == 1)) {cAdmScrAuditGrid_DataBind(dt.DefaultView);} else {cFindButton_Click(sender, e);}
				try
				{
					(new AdminSystem()).UpdLastPageInfo(1019, base.LUser.UsrId, cPgSize.Text, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		}
		...
		public void cShowImpButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cSchemaImage.Visible)
			{
				try
				{
					Session["ImportSchema"] = (new AdminSystem()).GetSchemaScrImp(1019,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (cSchemaImage.Attributes["OnClick"] == null || cSchemaImage.Attributes["OnClick"].IndexOf("ImportSchema.aspx") < 0) {cSchemaImage.Attributes["OnClick"] += "SearchLink('ImportSchema.aspx?scm=S&key=1019&csy=3','','',''); return false;";}
			}
			// *** System Button Click (after) Web Rule starts here *** //
		}
		...
		protected void ScrImport(bool bClear)
		{
			...
			DataTable dtAdmScrAuditGrid = (DataTable)Session[KEY_dtAdmScrAuditGrid];
			if (dti != null && dtAdmScrAuditGrid != null)
			{
				if (bClear)
				{
					dtAdmScrAuditGrid.Rows.Clear();
				}
				cFind.Text = string.Empty;
				int iExist = dtAdmScrAuditGrid.Rows.Count;
				...
				foreach (DataRowView drv in dti.DefaultView)
				{
					...
					{
						DataTable dtd;
						dtd = (DataTable)Session[KEY_dtCudAction1300];
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(1019,"GetDdlCudAction3S4106",true,true,0,string.Empty,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
							Session[KEY_dtCudAction1300] = dtd;
						}
						ScrImportDdl(dtd.DefaultView, drv, false, "ScrAuditId", "CudAction1300", "CudAction1300Text", 20, "", false);
						dtd = jj==iExist ? null : (DataTable)Session[KEY_dtScreenId1300];
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(1019,"GetDdlScreenId3S4107",true,true,0,string.Empty,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
							Session[KEY_dtScreenId1300] = dtd;
						}
						ScrImportDdl(dtd.DefaultView, drv, true, "ScrAuditId", "ScreenId1300", "ScreenId1300Text", 50, "", false);
						dtd = jj==iExist ? null : (DataTable)Session[KEY_dtTableId1300];
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(1019,"GetDdlTableId3S4109",true,true,0,string.Empty,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
							Session[KEY_dtTableId1300] = dtd;
						}
						ScrImportDdl(dtd.DefaultView, drv, true, "ScrAuditId", "TableId1300", "TableId1300Text", 100, "", false);
						dtd = jj==iExist ? null : (DataTable)Session[KEY_dtChangedBy1300];
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(1019,"GetDdlChangedBy3S4112",true,true,0,string.Empty,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
							Session[KEY_dtChangedBy1300] = dtd;
						}
						ScrImportDdl(dtd.DefaultView, drv, true, "ScrAuditId", "ChangedBy1300", "ChangedBy1300Text", 50, "", false);
					}
					jj = jj + 1;
				}
				iRow = dti.DefaultView.Count - iExist;
				if (!dti.Columns.Contains("_NewRow")) { dti.Columns.Add("_NewRow"); }
				Session[KEY_dtAdmScrAuditGrid] = dti;
				cAdmScrAuditGrid_DataBind(dti.DefaultView);
		...
		public void cImportButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cFNameO.Text != string.Empty && cWorkSheet.Items.Count > 0 && cWorkSheet.SelectedItem.Text != string.Empty && cStartRow.Text != string.Empty)
			{
				...
				try
				{
					if (cAdmScrAuditGrid.EditIndex < 0 || UpdateGridRow(cAdmScrAuditGrid, new CommandEventArgs("Save", "")))
					{
						Session[KEY_scrImport] = ImportFile(dtAdmScrAuditGrid.Copy(),cFNameO.Text,cWorkSheet.SelectedItem.Text,cStartRow.Text,Config.PathTmpImport + cFName.Text);
					}
		...
		private DataTable ImportFile(DataTable dt, string fileName, string workSheet, string startRow, string fileFullName)
		{
			try
			{
				...
				{
					if (rows[iRow][1].ToString() == string.Empty && rows[iRow][2].ToString() == string.Empty && rows[iRow][3].ToString() == string.Empty && rows[iRow][4].ToString() == string.Empty && rows[iRow][5].ToString() == string.Empty && rows[iRow][6].ToString() == string.Empty && rows[iRow][7].ToString() == string.Empty && rows[iRow][8].ToString() == string.Empty && rows[iRow][9].ToString() == string.Empty && rows[iRow][10].ToString() == string.Empty && rows[iRow][11].ToString() == string.Empty && rows[iRow][12].ToString() == string.Empty) {idel = idel + 1;}
					else
					{
		...
		private void GotoPage(int pageNo)
		{
			if (pageNo >= 0 && pageNo < GetTotalPages())
			{
				if (cAdmScrAuditGrid.EditIndex < 0 || UpdateGridRow(cAdmScrAuditGrid, new CommandEventArgs("Save", "")))
				{
					try { cAdmScrAuditGridDataPager.SetPageProperties(cAdmScrAuditGridDataPager.PageSize * pageNo, cAdmScrAuditGridDataPager.MaximumRows, false); cAdmScrAuditGrid_DataBind(null); }
					catch
					{
						try { cAdmScrAuditGridDataPager.SetPageProperties(0, cAdmScrAuditGridDataPager.MaximumRows, false); cAdmScrAuditGrid_DataBind(null); } catch {}
					}
				}
			}
		}

		protected void cAdmScrAuditGrid_OnItemCommand(object sender, ListViewCommandEventArgs e)
		{
		}

		private void cAdmScrAuditGrid_DataBind(DataView dvAdmScrAuditGrid)
		{
			if (dvAdmScrAuditGrid == null)
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmScrAuditGrid];
				dvAdmScrAuditGrid = dt != null ? dt.DefaultView : null;
			}
			if (dvAdmScrAuditGrid != null)
			{
				LcAuth = GetAuthCol().AsEnumerable().ToDictionary<DataRow,string>(dr=>dr["ColName"].ToString());
				cAdmScrAuditGrid.DataSource = dvAdmScrAuditGrid;
				cAdmScrAuditGrid.DataBind();
		...
		protected void cAdmScrAuditGrid_OnSorting(object sender, ListViewSortEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmScrAuditGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null)
			{
				if (cAdmScrAuditGrid.EditIndex < 0 || UpdateGridRow(cAdmScrAuditGrid, new CommandEventArgs("Save", "")))
				{
					...
					Session[KEY_dtAdmScrAuditGrid] = dt;
					cAdmScrAuditGrid_DataBind(dv);
				}
			}
		}

		protected void cAdmScrAuditGrid_OnPagePropertiesChanging(object sender, PagePropertiesChangingEventArgs e)
		{
		    cAdmScrAuditGridDataPager.SetPageProperties(e.StartRowIndex, e.MaximumRows, false);
		    DataTable dt = (DataTable)Session[KEY_dtAdmScrAuditGrid];
		    DataView dv = dt != null ? dt.DefaultView : null;
			cAdmScrAuditGrid_DataBind(dv); cAdmScrAuditGrid.EditIndex = -1;
		}

		private void GridChkPgDirty(ListViewItem lvi)
		{
				WebControl cc = null;
				cc = ((WebControl)lvi.FindControl("cCudAction1300"));
				...
				cc = ((WebControl)lvi.FindControl("cScreenId1300"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cMasterTable1300"));
				...
				cc = ((WebControl)lvi.FindControl("cTableId1300"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cRowId1300"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cRowDesc1300"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cChangedBy1300"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cChangedOn1300"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
		}

		protected void cAdmScrAuditGrid_OnItemDataBound(object sender, ListViewItemEventArgs e)
		{
			//WebRule: Pop up detail changes
            HtmlTableRow trow = e.Item.FindControl("cAdmScrAuditGridRow") as HtmlTableRow;
			...
			DataView dvAdmScrAuditGrid = dt != null ? dt.DefaultView : null;
			if (cAdmScrAuditGrid.EditIndex > -1 && GetDataItemIndex(cAdmScrAuditGrid.EditIndex) == e.Item.DataItemIndex)
			{
				isEditItem = true;
				base.SetGridEnabled(e.Item, GetAuthCol(), GetLabel(), 0);
				SetCudAction1300((DropDownList)e.Item.FindControl("cCudAction1300"),dvAdmScrAuditGrid[e.Item.DataItemIndex]["CudAction1300"].ToString());
				SetScreenId1300((RoboCoder.WebControls.ComboBox)e.Item.FindControl("cScreenId1300"),dvAdmScrAuditGrid[e.Item.DataItemIndex]["ScreenId1300"].ToString(), e.Item);
				CheckBox cb = null;
				cb = (CheckBox)e.Item.FindControl("cMasterTable1300");
				if (cb != null)
				{
					cb.Checked = base.GetBool(dvAdmScrAuditGrid[e.Item.DataItemIndex]["MasterTable1300"].ToString());
				}
				SetTableId1300((RoboCoder.WebControls.ComboBox)e.Item.FindControl("cTableId1300"),dvAdmScrAuditGrid[e.Item.DataItemIndex]["TableId1300"].ToString(), e.Item);
				SetChangedBy1300((RoboCoder.WebControls.ComboBox)e.Item.FindControl("cChangedBy1300"),dvAdmScrAuditGrid[e.Item.DataItemIndex]["ChangedBy1300"].ToString(), e.Item);
			...
			if (dtAuthRow != null)
			{
				...
				{
					cc = e.Item.FindControl("cAdmScrAuditGridDelete"); if (cc != null) { cc.Visible = false; }
				}
				if (!AllowRowEdit(GetAuthRow(),dvAdmScrAuditGrid[e.Item.DataItemIndex]["ScrAuditId1300"].ToString()) || (Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N"))
				{
					cc = e.Item.FindControl("cAdmScrAuditGridEdit"); if (cc != null) { cc.Visible = false; }
				}
				else
				{
			        HtmlTableRow tr = e.Item.FindControl("cAdmScrAuditGridRow") as HtmlTableRow;
			        LinkButton lb = e.Item.FindControl("cAdmScrAuditGridEdit") as LinkButton;
		...
		protected void cScrAuditId1300hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "0") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "0"; Session[KEY_lastSortExp] = "ScrAuditId1300";Session[KEY_lastSortImg] = "cScrAuditId1300hi";
			cAdmScrAuditGrid_OnSorting(sender, new ListViewSortEventArgs("ScrAuditId1300", SortDirection.Ascending));
		}

		protected void cCudAction1300hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "1") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "1"; Session[KEY_lastSortExp] = "CudAction1300Text";Session[KEY_lastSortImg] = "cCudAction1300hi";
			cAdmScrAuditGrid_OnSorting(sender, new ListViewSortEventArgs("CudAction1300Text", SortDirection.Ascending));
		}

		protected void cScreenId1300hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "2") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "2"; Session[KEY_lastSortExp] = "ScreenId1300Text";Session[KEY_lastSortImg] = "cScreenId1300hi";
			cAdmScrAuditGrid_OnSorting(sender, new ListViewSortEventArgs("ScreenId1300Text", SortDirection.Ascending));
		}

		protected void cMasterTable1300hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "3") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "3"; Session[KEY_lastSortExp] = "MasterTable1300";Session[KEY_lastSortImg] = "cMasterTable1300hi";
			cAdmScrAuditGrid_OnSorting(sender, new ListViewSortEventArgs("MasterTable1300", SortDirection.Ascending));
		}

		protected void cTableId1300hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "4") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "4"; Session[KEY_lastSortExp] = "TableId1300Text";Session[KEY_lastSortImg] = "cTableId1300hi";
			cAdmScrAuditGrid_OnSorting(sender, new ListViewSortEventArgs("TableId1300Text", SortDirection.Ascending));
		}

		protected void cRowId1300hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "5") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "5"; Session[KEY_lastSortExp] = "RowId1300";Session[KEY_lastSortImg] = "cRowId1300hi";
			cAdmScrAuditGrid_OnSorting(sender, new ListViewSortEventArgs("RowId1300", SortDirection.Ascending));
		}

		protected void cRowDesc1300hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "6") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "6"; Session[KEY_lastSortExp] = "RowDesc1300";Session[KEY_lastSortImg] = "cRowDesc1300hi";
			cAdmScrAuditGrid_OnSorting(sender, new ListViewSortEventArgs("RowDesc1300", SortDirection.Ascending));
		}

		protected void cChangedBy1300hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "7") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "7"; Session[KEY_lastSortExp] = "ChangedBy1300Text";Session[KEY_lastSortImg] = "cChangedBy1300hi";
			cAdmScrAuditGrid_OnSorting(sender, new ListViewSortEventArgs("ChangedBy1300Text", SortDirection.Ascending));
		}

		protected void cChangedOn1300hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "8") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "8"; Session[KEY_lastSortExp] = "ChangedOn1300";Session[KEY_lastSortImg] = "cChangedOn1300hi";
			cAdmScrAuditGrid_OnSorting(sender, new ListViewSortEventArgs("ChangedOn1300", SortDirection.Ascending));
		}

		protected void cAdmScrAuditGrid_OnItemEditing(object sender, ListViewEditEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmScrAuditGrid];
			if ((ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")) && dt != null && dt.DefaultView.Count > GetDataItemIndex(e.NewEditIndex))
			{
				cAdmScrAuditGrid.EditIndex = e.NewEditIndex;
				cAdmScrAuditGrid_DataBind(null);
				Control wc = null;
				string idx = Page.Request["__EVENTARGUMENT"];
				if (!string.IsNullOrEmpty(idx))
				{
				    Control ctrlToFocus = cAdmScrAuditGrid.EditItem.FindControl("c" + idx);
				    if (((WebControl)ctrlToFocus).Enabled) { wc = ctrlToFocus; }
				}
				if (wc == null) { wc = FindEditableControl(cAdmScrAuditGrid.EditItem); }
				if (wc != null)
             {
             ...
             MkMobileLabel(cAdmScrAuditGrid.EditItem);
		    }
		    else { e.Cancel = true; }
		}
		...
		{
		    Label ml = null;
		    ml = lvi.FindControl("cScrAuditId1300ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(0); }
		    ml = lvi.FindControl("cCudAction1300ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(1); }
		    ml = lvi.FindControl("cScreenId1300ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(2); }
		    ml = lvi.FindControl("cMasterTable1300ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(3); }
		    ml = lvi.FindControl("cTableId1300ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(4); }
		    ml = lvi.FindControl("cRowId1300ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(5); }
		    ml = lvi.FindControl("cRowDesc1300ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(6); }
		    ml = lvi.FindControl("cChangedBy1300ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(7); }
		    ml = lvi.FindControl("cChangedOn1300ml") as Label;
		...
		protected void GridFill(ListViewItem lvi, DataTable dt, DataRow dr, bool bInsert)
		{
			...
			ddl = (DropDownList)lvi.FindControl("cCudAction1300");
			if (ddl != null)
			{
				if (ddl.SelectedIndex >= 0 && ddl.SelectedValue.ToString() != string.Empty)
				{
					dr["CudAction1300"] = ddl.SelectedValue;
					dr["CudAction1300Text"] = ddl.SelectedItem.Text;
				}
				else
				{
					dr["CudAction1300"] = System.DBNull.Value;
					dr["CudAction1300Text"] = System.DBNull.Value;
				}
			}
			RoboCoder.WebControls.ComboBox cbb = null;
			cbb = (RoboCoder.WebControls.ComboBox)lvi.FindControl("cScreenId1300");
			if (cbb != null)
			{
				if (cbb.SelectedValue != string.Empty)
				{
					dr["ScreenId1300"] = cbb.SelectedValue;
					dr["ScreenId1300Text"] = cbb.SelectedText;
				}
				else
				{
					dr["ScreenId1300"] = System.DBNull.Value;
					dr["ScreenId1300Text"] = System.DBNull.Value;
				}
			}
			CheckBox cb = null;
			cb = (CheckBox)lvi.FindControl("cMasterTable1300");
			if (cb != null)
			{
				dr["MasterTable1300"] = base.SetBool(cb.Checked);
			}
			cbb = (RoboCoder.WebControls.ComboBox)lvi.FindControl("cTableId1300");
			if (cbb != null)
			{
				if (cbb.SelectedValue != string.Empty)
				{
					dr["TableId1300"] = cbb.SelectedValue;
					dr["TableId1300Text"] = cbb.SelectedText;
				}
				else
				{
					dr["TableId1300"] = System.DBNull.Value;
					dr["TableId1300Text"] = System.DBNull.Value;
				}
			}
			TextBox tb = null;
			tb = (TextBox)lvi.FindControl("cRowId1300");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["RowId1300"] = tb.Text;} else {dr["RowId1300"] = System.DBNull.Value;}
			}
			tb = (TextBox)lvi.FindControl("cRowDesc1300");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["RowDesc1300"] = tb.Text;} else {dr["RowDesc1300"] = System.DBNull.Value;}
			}
			cbb = (RoboCoder.WebControls.ComboBox)lvi.FindControl("cChangedBy1300");
			if (cbb != null)
			{
				if (cbb.SelectedValue != string.Empty)
				{
					dr["ChangedBy1300"] = cbb.SelectedValue;
					dr["ChangedBy1300Text"] = cbb.SelectedText;
				}
				else
				{
					dr["ChangedBy1300"] = System.DBNull.Value;
					dr["ChangedBy1300Text"] = System.DBNull.Value;
				}
			}
			tb = (TextBox)lvi.FindControl("cChangedOn1300");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["ChangedOn1300"] = base.ToIntDateTime(tb.Text,true,false);} else {dr["ChangedOn1300"] = System.DBNull.Value;}
			}
		    if (bInsert) { dt.Rows.InsertAt(dr, 0); }
			Session[KEY_dtAdmScrAuditGrid] = dt;
			cAdmScrAuditGrid.EditIndex = -1;
			cAdmScrAuditGrid_DataBind(dt.DefaultView);
		}

		protected void cAdmScrAuditGrid_OnItemUpdating(object sender, ListViewUpdateEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmScrAuditGrid];
			if (dt != null && dt.Rows.Count > 0)
			{
				DataRow dr = dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row;
			    GridFill(cAdmScrAuditGrid.EditItem, dt, dr, false);
			    dr["_NewRow"] = "N";
			    if (dt.Columns.Contains("_SortOrder")) dr["_SortOrder"] = null;
			}
		}

		protected void cAdmScrAuditGrid_OnItemCanceling(object sender, ListViewCancelEventArgs e)
		{
			PanelTop.Update();
			DataTable dt = (DataTable)Session[KEY_dtAdmScrAuditGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			...
			{
				cAdmScrAuditGrid_OnItemDeleting(sender, new ListViewDeleteEventArgs(e.ItemIndex));
			}
			if (dt.Columns.Contains("_SortOrder")) dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row["_SortOrder"] = null;
			cAdmScrAuditGrid.EditIndex = -1; cAdmScrAuditGrid_DataBind(null);
		}
		...
		protected void cDeleteAllButton_Click(object sender, System.EventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmScrAuditGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null && (cAdmScrAuditGrid.EditIndex < 0 || UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				GotoPage(0);
				while (dv.Count > 0) {dv.Delete(0);}
				Session[KEY_dtAdmScrAuditGrid] = dt;
				cAdmScrAuditGrid.EditIndex = -1; cAdmScrAuditGrid_DataBind(dv);
			}
		}

		protected void cAdmScrAuditGrid_OnItemDeleting(object sender, ListViewDeleteEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmScrAuditGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			DataRow dr = dv != null ? dv[GetDataItemIndex(e.ItemIndex)].Row : null;
			if (dv != null && (cAdmScrAuditGrid.EditIndex == e.ItemIndex || UpdateGridRow(sender, new CommandEventArgs("Save",""))))
			{
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmScrAuditGrid); PopAdmScrAudit1019List(sender, e, false, null);
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			// *** System Button Click (Before) Web Rule starts here *** //
			if (ValidPage() && UpdateGridRow(cAdmScrAuditGrid, new CommandEventArgs("Save", "")))
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmScrAuditGrid];
				DataView dv = dt != null ? dt.DefaultView : null;
				string ftr = string.Empty;
				if (dv.RowFilter != string.Empty) { ftr = dv.RowFilter; dv.RowFilter = string.Empty; }
				AdmScrAudit1019 ds = PrepAdmScrAuditData(dv,false);
				if (ftr != string.Empty) {dv.RowFilter = ftr;}
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(1019,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc,noTrans,commandTimeOut)) {bValid7 = true;}
					if (bValid7)
					{
						Session[KEY_currPageIndex] = GetCurrPageIndex();
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmScrAudit1019List(sender, e, false, null);
		...
		private AdmScrAudit1019 PrepAdmScrAuditData(DataView dv, bool bAdd)
		{
			...
			if (dv != null)
			{
				...
				ds.Tables["AdmScrAuditDef"].Rows.Add(MakeDisRow(ds.Tables["AdmScrAuditDef"].NewRow()));
					...
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmScrAuditUpd"].Rows.Add(MakeColRow(ds.Tables["AdmScrAuditUpd"].NewRow(), drv, false));
					}
					dv.RowStateFilter = DataViewRowState.Added;
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmScrAuditAdd"].Rows.Add(MakeColRow(ds.Tables["AdmScrAuditAdd"].NewRow(), drv, true));
					}
					dv.RowStateFilter = DataViewRowState.Deleted;
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmScrAuditDel"].Rows.Add(MakeColRow(ds.Tables["AdmScrAuditDel"].NewRow(), drv, false));
					}
		...
		private DataRow MakeColRow(DataRow dr, DataRowView drv, bool bAdd)
		{
			...
			if (dtAuth != null)
			{
				dr["ScrAuditId1300"] = drv["ScrAuditId1300"].ToString().Trim();
				...
				dr["RowId1300"] = drv["RowId1300"].ToString().Trim();
				dr["RowDesc1300"] = drv["RowDesc1300"].ToString().Trim();
		...
		{
			if (cAdmScrAuditGrid.EditIndex > -1)
			{
				cAdmScrAuditGrid_OnItemUpdating(sender, new ListViewUpdateEventArgs(cAdmScrAuditGrid.EditIndex));
			}
			return true;
		}

		protected void cAdmScrAuditGrid_OnPreRender(object sender, System.EventArgs e)
		{
			System.Web.UI.WebControls.Image hi = null;
			hi = (System.Web.UI.WebControls.Image)cAdmScrAuditGrid.FindControl("cScrAuditId1300hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmScrAuditGrid.FindControl("cCudAction1300hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmScrAuditGrid.FindControl("cScreenId1300hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmScrAuditGrid.FindControl("cMasterTable1300hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmScrAuditGrid.FindControl("cTableId1300hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmScrAuditGrid.FindControl("cRowId1300hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmScrAuditGrid.FindControl("cRowDesc1300hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmScrAuditGrid.FindControl("cChangedBy1300hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmScrAuditGrid.FindControl("cChangedOn1300hi"); if (hi != null) { hi.Visible = false; }
			if (Session[KEY_lastSortImg] != null)
			{
				hi = (System.Web.UI.WebControls.Image)cAdmScrAuditGrid.FindControl((string)Session[KEY_lastSortImg]);
				if (hi != null) { hi.ImageUrl = Utils.AddTilde((string)Session[KEY_lastSortUrl]); hi.Visible = true; }
			}
			IgnoreHeaderConfirm((LinkButton)cAdmScrAuditGrid.FindControl("cScrAuditId1300hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmScrAuditGrid.FindControl("cCudAction1300hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmScrAuditGrid.FindControl("cScreenId1300hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmScrAuditGrid.FindControl("cMasterTable1300hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmScrAuditGrid.FindControl("cTableId1300hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmScrAuditGrid.FindControl("cRowId1300hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmScrAuditGrid.FindControl("cRowDesc1300hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmScrAuditGrid.FindControl("cChangedBy1300hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmScrAuditGrid.FindControl("cChangedOn1300hl"));
		}

		private string GetButtonId(ListViewItem lvi)
		{
			string ButtonID = String.Empty;
			Control c = lvi.FindControl("cAdmScrAuditGridEdit");
		...
		{
		    return cAdmScrAuditGridDataPager.StartRowIndex / cAdmScrAuditGridDataPager.PageSize;
		}

		private int GetTotalPages()
		{
		    return (int)Math.Ceiling((double)cAdmScrAuditGridDataPager.TotalRowCount / cAdmScrAuditGridDataPager.PageSize);
		}

		private int GetDataItemIndex(int editIndex)
		{
		    return cAdmScrAuditGridDataPager.StartRowIndex + editIndex;
		}

		protected void cAdmScrAuditGrid_OnLayoutCreated(object sender, EventArgs e)
		{
		    // Header:
		    LinkButton lb = null;
		    lb = cAdmScrAuditGrid.FindControl("cScrAuditId1300hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(0); lb.ToolTip = ColumnToolTip(0); lb.Parent.Visible = GridColumnVisible(0); }
		    lb = cAdmScrAuditGrid.FindControl("cCudAction1300hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(1); lb.ToolTip = ColumnToolTip(1); lb.Parent.Visible = GridColumnVisible(1); }
		    lb = cAdmScrAuditGrid.FindControl("cScreenId1300hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(2); lb.ToolTip = ColumnToolTip(2); lb.Parent.Visible = GridColumnVisible(2); }
		    lb = cAdmScrAuditGrid.FindControl("cMasterTable1300hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(3); lb.ToolTip = ColumnToolTip(3); lb.Parent.Visible = GridColumnVisible(3); }
		    lb = cAdmScrAuditGrid.FindControl("cTableId1300hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(4); lb.ToolTip = ColumnToolTip(4); lb.Parent.Visible = GridColumnVisible(4); }
		    lb = cAdmScrAuditGrid.FindControl("cRowId1300hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(5); lb.ToolTip = ColumnToolTip(5); lb.Parent.Visible = GridColumnVisible(5); }
		    lb = cAdmScrAuditGrid.FindControl("cRowDesc1300hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(6); lb.ToolTip = ColumnToolTip(6); lb.Parent.Visible = GridColumnVisible(6); }
		    lb = cAdmScrAuditGrid.FindControl("cChangedBy1300hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(7); lb.ToolTip = ColumnToolTip(7); lb.Parent.Visible = GridColumnVisible(7); }
		    lb = cAdmScrAuditGrid.FindControl("cChangedOn1300hl") as LinkButton;
			...
			if (dtAuthRow != null)
			{
				...
				if (dr["AllowDel"].ToString() == "N" || (Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N"))
				{
		            lb = cAdmScrAuditGrid.FindControl("cDeleteAllButton") as LinkButton; if (lb != null) { lb.Visible = false; }
				}
			}
		    // footer:
		    Label gc = null;
		    gc = cAdmScrAuditGrid.FindControl("cScrAuditId1300fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(0); }
		    gc = cAdmScrAuditGrid.FindControl("cCudAction1300fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(1); }
		    gc = cAdmScrAuditGrid.FindControl("cScreenId1300fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(2); }
		    gc = cAdmScrAuditGrid.FindControl("cMasterTable1300fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(3); }
		    gc = cAdmScrAuditGrid.FindControl("cTableId1300fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(4); }
		    gc = cAdmScrAuditGrid.FindControl("cRowId1300fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(5); }
		    gc = cAdmScrAuditGrid.FindControl("cRowDesc1300fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(6); }
		    gc = cAdmScrAuditGrid.FindControl("cChangedBy1300fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(7); }
		    gc = cAdmScrAuditGrid.FindControl("cChangedOn1300fl") as Label;

...
{
	public class AdmStaticCs115 : DataSet
	{
		public AdmStaticCs115()
		{
			this.Tables.Add(MakeColumns(new DataTable("AdmStaticCs")));
			this.DataSetName = "AdmStaticCs115";
			this.Namespace = "http://Rintagi.com/DataSet/AdmStaticCs115";
		}

		private DataTable MakeColumns(DataTable dt)
		{
			DataColumnCollection columns = dt.Columns;
			columns.Add("StaticCsId260", typeof(string));
			columns.Add("StaticCsNm260", typeof(string));
			columns.Add("StyleDef260", typeof(string));
			return dt;
		}
	}
}
...
namespace RO.Web
{
	public partial class AdmStaticCsModule : RO.Web.ModuleBase
	{
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_115";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_115";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_115";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_115";
		private const string KEY_dtLabel = "Cache:dtLabel3_115";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_115";
		private const string KEY_dtScrCri = "Cache:dtScrCri3_115";
		private const string KEY_dsScrCriVal = "Cache:dsScrCriVal3_115";


		private const string KEY_dtSystems = "Cache:dtSystems3_115";
		private const string KEY_sysConnectionString = "Cache:sysConnectionString3_115";
		private const string KEY_dtAdmStaticCs115List = "Cache:dtAdmStaticCs115List";
		private const string KEY_bNewVisible = "Cache:bNewVisible3_115";
		private const string KEY_bNewSaveVisible = "Cache:bNewSaveVisible3_115";
		private const string KEY_bCopyVisible = "Cache:bCopyVisible3_115";
		private const string KEY_bCopySaveVisible = "Cache:bCopySaveVisible3_115";
		private const string KEY_bDeleteVisible = "Cache:bDeleteVisible3_115";
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_115";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_115";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_115";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				DataTable dtMenuAccess = (new MenuSystem()).GetMenu(base.LUser.CultureId, 3, base.LImpr, LcSysConnString, LcAppPw,115, null, null);
				...
				Session.Remove(KEY_dtAdmStaticCs115List);
				...
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 115, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmStaticCs115List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=115&typ=N&sys=3','','',''); return false;"; }
		...
		private void SetSystem(byte SystemId)
		{
			...
			try
			{
				...
				if ((Config.DeployType == "DEV" || row["dbAppDatabase"].ToString() == base.CPrj.EntityCode + "View") && !(base.CPrj.EntityCode != "RO" && row["SysProgram"].ToString() == "Y") && (new AdminSystem()).IsRegenNeeded(string.Empty,115,0,0,LcSysConnString,LcAppPw))
				{
					(new GenScreensSystem()).CreateProgram(115, "Static CSS", row["dbAppDatabase"].ToString(), base.CPrj, base.CSrc, base.CTar, LcAppConnString, LcAppPw);
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(115,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(115,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(115,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(115,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,115,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(115,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(115,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(115,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		protected DataTable GetAuthRow()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthRow(115,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				try
				{
					try {dv = new DataView((new AdminSystem()).GetExp(115,"GetExpAdmStaticCs115","Y",(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
					catch {dv = new DataView((new AdminSystem()).GetExp(115,"GetExpAdmStaticCs115","N",(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(115,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					{
						if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["StaticCsId260"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["StaticCsNm260"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["StyleDef260"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						sb.Append(Environment.NewLine);
					}
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmStaticCs.csv"; Session["ExportStr"] = (Config.ExportExcelCSV ? "sep=\t\n": "") + sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmStaticCs.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private string GetRtfGrid(DataView dv)
		{
			...
			try
			{
				...
				foreach (DataRowView drv in dv)
				{
					...
					sb.Append("\r\n");
					sb.Append(@"{");
					if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["StaticCsId260"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["StaticCsNm260"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(drv["StyleDef260"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					sb.Append(@"}");
					sb.Append("\r\n");
					sb.Append(@"\pard \ql \li0\ri0\widctlpar\intbl\aspalpha\aspnum\adjustright\rin0\lin0 {");
					sb.Append(@"\insrsid2981395 \trowd \irow0\irowband0\lastrow \ts15\trgaph108\trleft-108\trbrdrt\brdrs\brdrw10 \trbrdrl\brdrs\brdrw10 \trbrdrb\brdrs\brdrw10 \trbrdrr\brdrs\brdrw10 \trbrdrh\brdrs\brdrw10 \trbrdrv\brdrs\brdrw10 ");
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 115, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			...
			UpdCriteria(true);
			cAdmStaticCs115List.ClearSearch(); Session.Remove(KEY_dtAdmStaticCs115List);
			PopAdmStaticCs115List(sender, e, false, null);
		}
			...
			try
			{
				foreach (DataRowView drv in dvCri)
				{
					...
					if (drv["DisplayName"].ToString() == "ComboBox")
					{
						...
						(new AdminSystem()).MkGetScreenIn("115", drv["ScreenCriId"].ToString(), "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), LcAppDb, LcDesDb, drv["MultiDesignDb"].ToString(), LcSysConnString, LcAppPw);
						DataView dv = new DataView((new AdminSystem()).GetScreenIn("115", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), (new AdminSystem()).CountScrCri(drv["ScreenCriId"].ToString(), drv["MultiDesignDb"].ToString(), drv["MultiDesignDb"].ToString() == "N" ? LcSysConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)), drv["RequiredValid"].ToString(), 0, string.Empty, drv["DisplayMode"].ToString() != "AutoListBox", val, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)));
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("115", LcSysConnString, LcAppPw);
				}
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmStaticCs115" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmStaticCs115" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			foreach (DataRowView drv in dvCri)
			{
			    if (drv["DisplayName"].ToString() == "ListBox")
			    {
					...
					if (cListBox != null)
					{
						...
						int TotalChoiceCnt = new DataView((new AdminSystem()).GetScreenIn("115", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), CriCnt, drv["RequiredValid"].ToString(), 0, string.Empty, true, string.Empty, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId))).Count;
						string selectedValues = string.Join(",", cListBox.Items.Cast<ListItem>().Where(x => x.Selected).Select(x => "'" + x.Value + "'").ToArray());
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("115", "AdmStaticCs115", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		private DataView GetAdmStaticCs115List()
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmStaticCs115List];
			if (dt == null)
			{
				CheckAuthentication(false);
				int filterId = 0;
				...
				try
				{
					try {dt = (new AdminSystem()).GetLis(115,"GetLisAdmStaticCs115",true,"Y",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
					catch {dt = (new AdminSystem()).GetLis(115,"GetLisAdmStaticCs115",true,"N",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				}
		...
		private void PopAdmStaticCs115List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			DataView dv = GetAdmStaticCs115List();
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			context["method"] = "GetLisAdmStaticCs115";
			context["mKey"] = "StaticCsId260";
			context["mVal"] = "StaticCsId260Text";
			context["mTip"] = "StaticCsId260Text";
			context["mImg"] = "StaticCsId260Text";
			context["ssd"] = Request.QueryString["ssd"];
			context["scr"] = "115";
			context["csy"] = "3";
			...
			cAdmStaticCs115List.AutoCompleteUrl = "AutoComplete.aspx/LisSuggests";
			cAdmStaticCs115List.DataContext = context;
			if (dv.Table == null) return;
			cAdmStaticCs115List.DataSource = dv;
			cAdmStaticCs115List.Visible = true;
			if (cAdmStaticCs115List.Items.Count <= 0) {cAdmStaticCs115List.Visible = false; cAdmStaticCs115List_SelectedIndexChanged(sender,e);}
			else
			{
				if (bPageLoad && !string.IsNullOrEmpty(Request.QueryString["key"]) && bUseCri.Value == string.Empty)
				{
					try {cAdmStaticCs115List.SelectByValue(Request.QueryString["key"],string.Empty,true);} catch {cAdmStaticCs115List.Items[0].Selected = true; cAdmStaticCs115List_SelectedIndexChanged(sender, e);}
				    cScreenSearch.Visible = false; cSystem.Visible = false; cEditButton.Visible = true; cSaveCloseButton.Visible = cSaveButton.Visible;
				}
				else
				{
					if (ID != null) {if (!cAdmStaticCs115List.SelectByValue(ID,string.Empty,true)) {ID = null;}}
					if (ID == null)
					{
						cAdmStaticCs115List_SelectedIndexChanged(sender, e);
					}
				}
			}
		}
		...
		private void SetColumnAuthority()
		{
			...
			{
				base.SetFoldBehavior(cStaticCsId260, dtAuth.Rows[0], cStaticCsId260P1, cStaticCsId260Label, cStaticCsId260P2, null, dtLabel.Rows[0], null, null, null);
				base.SetFoldBehavior(cStaticCsNm260, dtAuth.Rows[1], cStaticCsNm260P1, cStaticCsNm260Label, cStaticCsNm260P2, null, dtLabel.Rows[1], cRFVStaticCsNm260, null, null);
				base.SetFoldBehavior(cStyleDef260, dtAuth.Rows[2], cStyleDef260P1, cStyleDef260Label, cStyleDef260P2, cStyleDef260E, null, dtLabel.Rows[2], null, null, null);
				cStyleDef260E.Attributes["label_id"] = cStyleDef260Label.ClientID; cStyleDef260E.Attributes["target_id"] = cStyleDef260.ClientID;
			}
			if ((cStaticCsNm260.Attributes["OnChange"] == null || cStaticCsNm260.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cStaticCsNm260.Visible && !cStaticCsNm260.ReadOnly) {cStaticCsNm260.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cStyleDef260.Attributes["OnChange"] == null || cStyleDef260.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cStyleDef260.Visible && !cStyleDef260.ReadOnly) {cStyleDef260.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
		}
		...
		private void ClearMaster(object sender, System.EventArgs e)
		{
			...
			if (dt.Rows[1]["ColVisible"].ToString() == "Y" && dt.Rows[1]["ColReadOnly"].ToString() != "Y") {cStaticCsNm260.Text = string.Empty;}
			if (dt.Rows[2]["ColVisible"].ToString() == "Y" && dt.Rows[2]["ColReadOnly"].ToString() != "Y") {cStyleDef260.Text = string.Empty;}
			// *** Default Value (Folder) Web Rule starts here *** //
		}

		private void InitMaster(object sender, System.EventArgs e)
		{
			cStaticCsId260.Text = string.Empty;
			cStaticCsNm260.Text = string.Empty;
			cStyleDef260.Text = string.Empty;
			// *** Default Value (Folder) Web Rule starts here *** //
		}
		protected void cAdmStaticCs115List_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmStaticCs115List_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmStaticCs115List_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			InitMaster(sender, e);      // Do this first to enable defaults for non-database columns.
			DataTable dt = null;
			try
			{
				dt = (new AdminSystem()).GetMstById("GetAdmStaticCs115ById",cAdmStaticCs115List.SelectedValue,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId));
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				if (dt.Rows.Count > 0)
				{
					cAdmStaticCs115List.SetDdVisible();
					DataRow dr = dt.Rows[0];
					try {cStaticCsId260.Text = RO.Common3.Utils.fmNumeric("0",dr["StaticCsId260"].ToString(),base.LUser.Culture);} catch {cStaticCsId260.Text = string.Empty;}
					try {cStaticCsNm260.Text = dr["StaticCsNm260"].ToString();} catch {cStaticCsNm260.Text = string.Empty;}
					try {cStyleDef260.Text = dr["StyleDef260"].ToString();} catch {cStyleDef260.Text = string.Empty;}
				}
			}
			cButPanel.DataBind();
			ScriptManager.GetCurrent(Parent.Page).SetFocus(cAdmStaticCs115List.FocusID);
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cStaticCsId260.Text == string.Empty) { cClearButton_Click(sender, new EventArgs()); ShowDirty(false); PanelTop.Update();}
			else { PopAdmStaticCs115List(sender, e, false, cStaticCsId260.Text); }
			// *** System Button Click (After) Web Rule starts here *** //
		}
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmStaticCs115List); PopAdmStaticCs115List(sender, e, false, null);
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			if (ValidPage())
			{
				AdmStaticCs115 ds = PrepAdmStaticCsData(null,cStaticCsId260.Text == string.Empty);
				if (!string.IsNullOrEmpty(cAdmStaticCs115List.SelectedValue) && cStaticCsId260.Text != cAdmStaticCs115List.SelectedValue)
				{
					PreMsgPopup("Primary key cannot be changed");
					return rtn;
				}
				if (string.IsNullOrEmpty(cAdmStaticCs115List.SelectedValue))	// Add
				{
					if (ds != null)
					{
						pid = (new AdminSystem()).AddData(115,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc,noTrans,commandTimeOut);
					}
					if (!string.IsNullOrEmpty(pid))
					{
						cAdmStaticCs115List.ClearSearch(); Session.Remove(KEY_dtAdmStaticCs115List);
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmStaticCs115List(sender, e, false, pid);
						rtn = GetScreenHlp().Rows[0]["AddMsg"].ToString();
					}
				}
				else {
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(115,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc,noTrans,commandTimeOut)) {bValid7 = true;}
					if (bValid7)
					{
						cAdmStaticCs115List.ClearSearch(); Session.Remove(KEY_dtAdmStaticCs115List);
						ShowDirty(false); PopAdmStaticCs115List(sender, e, false, ds.Tables["AdmStaticCs"].Rows[0]["StaticCsId260"]);
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cStaticCsId260.Text != string.Empty)
			{
				AdmStaticCs115 ds = PrepAdmStaticCsData(null,false);
				try
				{
					if (ds != null && (new AdminSystem()).DelData(115,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc))
					{
						cAdmStaticCs115List.ClearSearch(); Session.Remove(KEY_dtAdmStaticCs115List);
						ShowDirty(false); PopAdmStaticCs115List(sender, e, false, null);
		...
		private AdmStaticCs115 PrepAdmStaticCsData(DataView dv, bool bAdd)
		{
			AdmStaticCs115 ds = new AdmStaticCs115();
			DataRow dr = ds.Tables["AdmStaticCs"].NewRow();
			DataRow drType = ds.Tables["AdmStaticCs"].NewRow();
			DataRow drDisp = ds.Tables["AdmStaticCs"].NewRow();
			if (bAdd) { dr["StaticCsId260"] = string.Empty; } else { dr["StaticCsId260"] = cStaticCsId260.Text; }
			drType["StaticCsId260"] = "Numeric"; drDisp["StaticCsId260"] = "TextBox";
			try {dr["StaticCsNm260"] = cStaticCsNm260.Text.Trim();} catch {}
			drType["StaticCsNm260"] = "VarWChar"; drDisp["StaticCsNm260"] = "TextBox";
			try {dr["StyleDef260"] = cStyleDef260.Text;} catch {}
			drType["StyleDef260"] = "VarWChar"; drDisp["StyleDef260"] = "MultiLine";
			if (bAdd)
			{
			}
			ds.Tables["AdmStaticCs"].Rows.Add(dr); ds.Tables["AdmStaticCs"].Rows.Add(drType); ds.Tables["AdmStaticCs"].Rows.Add(drDisp);
			return ds;
		}

		public bool CanAct(char typ) { return CanAct(typ, GetAuthRow(), cAdmStaticCs115List.SelectedValue.ToString()); }
		...
		private void PreMsgPopup(string msg) { PreMsgPopup(msg,cAdmStaticCs115List,null); }

...
namespace RO.Web
{
	public partial class AdmServerRuleOvrdModule : RO.Web.ModuleBase
	{

		private const string KEY_lastAddedRow = "Cache:lastAddedRow3_1026";
		private const string KEY_lastSortOrd = "Cache:lastSortOrd3_1026";
		private const string KEY_lastSortImg = "Cache:lastSortImg3_1026";
		private const string KEY_lastSortCol = "Cache:lastSortCol3_1026";
		private const string KEY_lastSortExp = "Cache:lastSortExp3_1026";
		private const string KEY_lastSortUrl = "Cache:lastSortUrl3_1026";
		private const string KEY_lastSortTog = "Cache:lastSortTog3_1026";
		private const string KEY_lastImpPwdOvride = "Cache:lastImpPwdOvride3_1026";
		private const string KEY_cntImpPwdOvride = "Cache:cntImpPwdOvride3_1026";
		private const string KEY_currPageIndex = "Cache:currPageIndex3_1026";
		private const string KEY_bHiImpVisible = "Cache:bHiImpVisible3_1026";
		private const string KEY_bShImpVisible = "Cache:bShImpVisible3_1026";
		private const string KEY_dtAdmServerRuleOvrdGrid = "Cache:dtAdmServerRuleOvrdGrid";
		private const string KEY_scrImport = "Cache:scrImport3_1026";
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_1026";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_1026";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_1026";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_1026";
		private const string KEY_dtLabel = "Cache:dtLabel3_1026";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_1026";
		private const string KEY_dtScrCri = "Cache:dtScrCri3_1026";
		private const string KEY_dsScrCriVal = "Cache:dsScrCriVal3_1026";

		private const string KEY_dtServerRuleId1322 = "Cache:dtServerRuleId1322";
		private const string KEY_dtScreenId1322 = "Cache:dtScreenId1322";
		private const string KEY_dtRunMode1322 = "Cache:dtRunMode1322";
		private const string KEY_dtPermKeyId1321 = "Cache:dtPermKeyId1321";
		private const string KEY_dtPermKeyRowId1321 = "Cache:dtPermKeyRowId1321";

		private const string KEY_dtSystems = "Cache:dtSystems3_1026";
		private const string KEY_sysConnectionString = "Cache:sysConnectionString3_1026";
		private const string KEY_dtAdmServerRuleOvrd1026List = "Cache:dtAdmServerRuleOvrd1026List";
		private const string KEY_bNewVisible = "Cache:bNewVisible3_1026";
		private const string KEY_bNewSaveVisible = "Cache:bNewSaveVisible3_1026";
		private const string KEY_bCopyVisible = "Cache:bCopyVisible3_1026";
		private const string KEY_bCopySaveVisible = "Cache:bCopySaveVisible3_1026";
		private const string KEY_bDeleteVisible = "Cache:bDeleteVisible3_1026";
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_1026";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_1026";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_1026";
		...
		public AdmServerRuleOvrdModule()
		{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				DataTable dtMenuAccess = (new MenuSystem()).GetMenu(base.LUser.CultureId, 3, base.LImpr, LcSysConnString, LcAppPw,1026, null, null);
				...
				Session.Remove(KEY_dtServerRuleId1322);
				Session.Remove(KEY_dtScreenId1322);
				Session.Remove(KEY_dtRunMode1322);
				Session.Remove(KEY_dtPermKeyId1321);
				Session.Remove(KEY_dtPermKeyRowId1321);
				SetButtonHlp();
				GetSystems();
				SetColumnAuthority();
				GetGlobalFilter();
				...
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 1026, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmServerRuleOvrd1026List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=1026&typ=N&sys=3','','',''); return false;"; }
			...
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmServerRuleOvrdGrid];
				if (dt != null)
				{
		...
		private void InitializeComponent()
		{
			...
			if (LcSysConnString == null)
			{
				...
				{
					DataTable dt;
					dt = (new AdminSystem()).GetAuthRow(1026,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
					...
					{
						cAdmServerRuleOvrdGrid.PreRender += new EventHandler(cAdmServerRuleOvrdGrid_OnPreRender);
					}
				}
		...
		private void SetSystem(byte SystemId)
		{
			...
			try
			{
				...
				if ((Config.DeployType == "DEV" || row["dbAppDatabase"].ToString() == base.CPrj.EntityCode + "View") && !(base.CPrj.EntityCode != "RO" && row["SysProgram"].ToString() == "Y") && (new AdminSystem()).IsRegenNeeded(string.Empty,1026,0,0,LcSysConnString,LcAppPw))
				{
					(new GenScreensSystem()).CreateProgram(1026, "Server Rule Override", row["dbAppDatabase"].ToString(), base.CPrj, base.CSrc, base.CTar, LcAppConnString, LcAppPw);
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(1026,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(1026,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void SetClientRule(ListViewDataItem ee, bool isEditItem)
		{
			...
			if (dvRul.Count > 0)
			{
				...
				{
					if (ee == null || drv["ScriptEvent"].ToString().Substring(0,2).ToLower() != "on" || (cAdmServerRuleOvrdGrid.EditIndex > -1 && GetDataItemIndex(cAdmServerRuleOvrdGrid.EditIndex) == ee.DataItemIndex))
					{
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(1026,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(1026,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,1026,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(1026,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(1026,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(1026,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				try
				{
					try {dv = new DataView((new AdminSystem()).GetExp(1026,"GetExpAdmServerRuleOvrd1026","Y",(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
					catch {dv = new DataView((new AdminSystem()).GetExp(1026,"GetExpAdmServerRuleOvrd1026","N",(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(1026,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					{
						if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["AtServerRuleOvrdId1322"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["ServerRuleOvrdDesc1322"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["ServerRuleOvrdName1322"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(drv["ServerRuleId1322"].ToString() + (char)9 + drv["ServerRuleId1322Text"].ToString() + (char)9);}
						if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append(drv["ScreenId1322"].ToString() + (char)9 + drv["ScreenId1322Text"].ToString() + (char)9);}
						if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["Priority1322"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["Disable1322"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[7]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["RunMode1322"].ToString().Replace("\"","\"\"") + "\"" + (char)9 + "\"" + drv["RunMode1322Text"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[8]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["ServerRuledOvrdPrmId1321"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[9]["ColExport"].ToString() == "Y") {sb.Append(drv["PermKeyId1321"].ToString() + (char)9 + drv["PermKeyId1321Text"].ToString() + (char)9);}
						if (dtAu.Rows[10]["ColExport"].ToString() == "Y") {sb.Append(drv["PermKeyRowId1321"].ToString() + (char)9 + drv["PermKeyRowId1321Text"].ToString() + (char)9);}
						if (dtAu.Rows[11]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["AndCondition1321"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[12]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["Match1321"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						sb.Append(Environment.NewLine);
					}
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmServerRuleOvrd.csv"; Session["ExportStr"] = (Config.ExportExcelCSV ? "sep=\t\n": "") + sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmServerRuleOvrd.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private string GetRtfGrid(DataView dv)
		{
			...
			try
			{
				...
				foreach (DataRowView drv in dv)
				{
					...
					sb.Append("\r\n");
					sb.Append(@"{");
					if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["AtServerRuleOvrdId1322"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["ServerRuleOvrdDesc1322"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(drv["ServerRuleOvrdName1322"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(drv["ServerRuleId1322Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append(drv["ScreenId1322Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["Priority1322"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append(drv["Disable1322"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[7]["ColExport"].ToString() == "Y") {sb.Append(drv["RunMode1322Text"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[8]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["ServerRuledOvrdPrmId1321"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[9]["ColExport"].ToString() == "Y") {sb.Append(drv["PermKeyId1321Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[10]["ColExport"].ToString() == "Y") {sb.Append(drv["PermKeyRowId1321Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[11]["ColExport"].ToString() == "Y") {sb.Append(drv["AndCondition1321"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[12]["ColExport"].ToString() == "Y") {sb.Append(drv["Match1321"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					sb.Append(@"}");
					sb.Append("\r\n");
					sb.Append(@"\pard \ql \li0\ri0\widctlpar\intbl\aspalpha\aspnum\adjustright\rin0\lin0 {");
					sb.Append(@"\insrsid2981395 \trowd \irow0\irowband0\lastrow \ts15\trgaph108\trleft-108\trbrdrt\brdrs\brdrw10 \trbrdrl\brdrs\brdrw10 \trbrdrb\brdrs\brdrw10 \trbrdrr\brdrs\brdrw10 \trbrdrh\brdrs\brdrw10 \trbrdrv\brdrs\brdrw10 ");
		...
		private void GetPageInfo()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastPageInfo(1026, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 1026, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			// *** Criteria Trigger (before) Web Rule starts here *** //
			DataTable dt = (DataTable)Session[KEY_dtAdmServerRuleOvrdGrid];
			if (cAdmServerRuleOvrdGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
			if (IsPostBack && cCriteria.Visible) ReBindCriteria(GetScrCriteria());
			UpdCriteria(true);
			cAdmServerRuleOvrd1026List.ClearSearch(); Session.Remove(KEY_dtAdmServerRuleOvrd1026List);
			PopAdmServerRuleOvrd1026List(sender, e, false, null);
			}
		}
			...
			try
			{
				foreach (DataRowView drv in dvCri)
				{
					...
					if (drv["DisplayName"].ToString() == "ComboBox")
					{
						...
						(new AdminSystem()).MkGetScreenIn("1026", drv["ScreenCriId"].ToString(), "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), LcAppDb, LcDesDb, drv["MultiDesignDb"].ToString(), LcSysConnString, LcAppPw);
						DataView dv = new DataView((new AdminSystem()).GetScreenIn("1026", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), (new AdminSystem()).CountScrCri(drv["ScreenCriId"].ToString(), drv["MultiDesignDb"].ToString(), drv["MultiDesignDb"].ToString() == "N" ? LcSysConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)), drv["RequiredValid"].ToString(), 0, string.Empty, drv["DisplayMode"].ToString() != "AutoListBox", val, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)));
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("1026", LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmServerRuleOvrd1026" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmServerRuleOvrd1026" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			foreach (DataRowView drv in dvCri)
			{
			    if (drv["DisplayName"].ToString() == "ListBox")
			    {
					...
					if (cListBox != null)
					{
						...
						int TotalChoiceCnt = new DataView((new AdminSystem()).GetScreenIn("1026", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), CriCnt, drv["RequiredValid"].ToString(), 0, string.Empty, true, string.Empty, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId))).Count;
						string selectedValues = string.Join(",", cListBox.Items.Cast<ListItem>().Where(x => x.Selected).Select(x => "'" + x.Value + "'").ToArray());
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("1026", "AdmServerRuleOvrd1026", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		protected void cbServerRuleId1322(object sender, System.EventArgs e)
		{
			SetServerRuleId1322((RoboCoder.WebControls.ComboBox)sender,string.Empty);
		}

		private void SetServerRuleId1322(RoboCoder.WebControls.ComboBox ddl, string keyId)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(1026,"GetDdlServerRuleId3S4359",true,false,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
					if (dv.Table.Columns.Contains("AtServerRuleOvrdId"))
					{
						context["pMKeyColID"] = cAdmServerRuleOvrd1026List.ClientID;
						context["pMKeyCol"] = "AtServerRuleOvrdId";
						string ss = "(AtServerRuleOvrdId is null";
						if (string.IsNullOrEmpty(cAdmServerRuleOvrd1026List.SelectedValue)) {ss = ss + ")";} else {ss = ss + " OR AtServerRuleOvrdId = " + cAdmServerRuleOvrd1026List.SelectedValue + ")";}
						dv.RowFilter = ss;
					}
					ddl.DataSource = dv; Session[KEY_dtServerRuleId1322] = dv.Table;
				    try { ddl.SelectByValue(keyId,string.Empty,true); } catch { try { ddl.SelectedIndex = 0; } catch { } }
				}
			}
		}

		protected void cbScreenId1322(object sender, System.EventArgs e)
		{
			SetScreenId1322((RoboCoder.WebControls.ComboBox)sender,string.Empty);
		}

		private void SetScreenId1322(RoboCoder.WebControls.ComboBox ddl, string keyId)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(1026,"GetDdlScreenId3S4367",true,false,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
					...
					ddl.DataSource = dv; Session[KEY_dtScreenId1322] = dv.Table;
				    try { ddl.SelectByValue(keyId,string.Empty,true); } catch { try { ddl.SelectedIndex = 0; } catch { } }
				}
			}
		}

		private void SetRunMode1322(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtRunMode1322];
			...
			if (ddl != null)
			{
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(1026,"GetDdlRunMode3S4373",true,bAll,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					if (dv.Table.Columns.Contains("AtServerRuleOvrdId"))
					{
						ss = "(AtServerRuleOvrdId is null";
						if (string.IsNullOrEmpty(cAdmServerRuleOvrd1026List.SelectedValue)) {ss = ss + ")";} else {ss = ss + " OR AtServerRuleOvrdId = " + cAdmServerRuleOvrd1026List.SelectedValue + ")";}
					}
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "RunMode1322 is not NULL"; }
					dv.RowFilter = ss;
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(1026,"GetDdlRunMode3S4373",true,true,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtRunMode1322] = dv.Table;
				}
			}
		}

		private void SetPermKeyId1321(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtPermKeyId1321];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (ddl != null)
			{
				string ss = string.Empty;
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(1026,"GetDdlPermKeyId3S4363",true,bAll,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					...
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "PermKeyId1321 is not NULL"; }
					dv.RowFilter = ss;
					ddl.DataSource = dv; ddl.DataBind();
					li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					else if (!bFirst)
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(1026,"GetDdlPermKeyId3S4363",true,true,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtPermKeyId1321] = dv.Table;
				}
			}
		}

		protected void cbPermKeyRowId1321(object sender, System.EventArgs e)
		{
			SetPermKeyRowId1321((RoboCoder.WebControls.ComboBox)sender,string.Empty,((DropDownList)cAdmServerRuleOvrdGrid.EditItem.FindControl("cPermKeyId1321")).SelectedValue,cAdmServerRuleOvrdGrid.EditItem);
		}

		private void SetPermKeyRowId1321(RoboCoder.WebControls.ComboBox ddl, string keyId, string filtr, ListViewItem li)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			context["isSys"] = "N";
			context["conn"] = KEY_sysConnectionString;
			context["refColCID"] = li != null ? li.FindControl("cPermKeyId1321").ClientID : string.Empty;
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(1026,"GetDdlPermKeyRowId3S4369",true,false,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
					...
					ddl.DataSource = dv; Session[KEY_dtPermKeyRowId1321] = dv.Table;
				    try { ddl.SelectByValue(keyId,filtr != string.Empty ? "(PermKeyId is null OR PermKeyId = " + filtr + ")" : string.Empty,true); } catch { try { ddl.SelectedIndex = 0; } catch { } }
				}
			}
		}

		private DataView GetAdmServerRuleOvrd1026List()
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmServerRuleOvrd1026List];
			if (dt == null)
			{
				CheckAuthentication(false);
				int filterId = 0;
				...
				try
				{
					try {dt = (new AdminSystem()).GetLis(1026,"GetLisAdmServerRuleOvrd1026",true,"Y",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
					catch {dt = (new AdminSystem()).GetLis(1026,"GetLisAdmServerRuleOvrd1026",true,"N",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				}
		...
		private void PopAdmServerRuleOvrd1026List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			DataView dv = GetAdmServerRuleOvrd1026List();
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			context["isSys"] = "N";
			context["conn"] = KEY_sysConnectionString;
			cAdmServerRuleOvrd1026List.AutoCompleteUrl = "AutoComplete.aspx/LisSuggests";
			cAdmServerRuleOvrd1026List.DataContext = context;
			if (dv.Table == null) return;
			cAdmServerRuleOvrd1026List.DataSource = dv;
			cAdmServerRuleOvrd1026List.Visible = true;
			if (cAdmServerRuleOvrd1026List.Items.Count <= 0) {cAdmServerRuleOvrd1026List.Visible = false; cAdmServerRuleOvrd1026List_SelectedIndexChanged(sender,e);}
			else
			{
				if (bPageLoad && !string.IsNullOrEmpty(Request.QueryString["key"]) && bUseCri.Value == string.Empty)
				{
					try {cAdmServerRuleOvrd1026List.SelectByValue(Request.QueryString["key"],string.Empty,true);} catch {cAdmServerRuleOvrd1026List.Items[0].Selected = true; cAdmServerRuleOvrd1026List_SelectedIndexChanged(sender, e);}
				    cScreenSearch.Visible = false; cSystem.Visible = false; cEditButton.Visible = true; cSaveCloseButton.Visible = cSaveButton.Visible;
				}
				else
				{
					if (ID != null) {if (!cAdmServerRuleOvrd1026List.SelectByValue(ID,string.Empty,true)) {ID = null;}}
					if (ID == null)
					{
						cAdmServerRuleOvrd1026List_SelectedIndexChanged(sender, e);
					}
				}
			}
		}
		...
		private void SetColumnAuthority()
		{
			...
			{
				base.SetFoldBehavior(cAtServerRuleOvrdId1322, dtAuth.Rows[0], cAtServerRuleOvrdId1322P1, cAtServerRuleOvrdId1322Label, cAtServerRuleOvrdId1322P2, null, dtLabel.Rows[0], null, null, null);
				base.SetFoldBehavior(cServerRuleOvrdDesc1322, dtAuth.Rows[1], cServerRuleOvrdDesc1322P1, cServerRuleOvrdDesc1322Label, cServerRuleOvrdDesc1322P2, null, dtLabel.Rows[1], null, null, null);
				base.SetFoldBehavior(cServerRuleOvrdName1322, dtAuth.Rows[2], cServerRuleOvrdName1322P1, cServerRuleOvrdName1322Label, cServerRuleOvrdName1322P2, null, dtLabel.Rows[2], cRFVServerRuleOvrdName1322, null, null);
				base.SetFoldBehavior(cServerRuleId1322, dtAuth.Rows[3], cServerRuleId1322P1, cServerRuleId1322Label, cServerRuleId1322P2, null, dtLabel.Rows[3], cRFVServerRuleId1322, null, null);
				SetServerRuleId1322(cServerRuleId1322,string.Empty);
				base.SetFoldBehavior(cScreenId1322, dtAuth.Rows[4], cScreenId1322P1, cScreenId1322Label, cScreenId1322P2, null, dtLabel.Rows[4], null, null, null);
				SetScreenId1322(cScreenId1322,string.Empty);
				base.SetFoldBehavior(cPriority1322, dtAuth.Rows[5], cPriority1322P1, cPriority1322Label, cPriority1322P2, null, dtLabel.Rows[5], null, null, null);
				base.SetFoldBehavior(cDisable1322, dtAuth.Rows[6], cDisable1322P1, cDisable1322Label, cDisable1322P2, null, dtLabel.Rows[6], null, null, null);
				base.SetFoldBehavior(cRunMode1322, dtAuth.Rows[7], cRunMode1322P1, cRunMode1322Label, cRunMode1322P2, null, dtLabel.Rows[7], null, null, null);
			}
			if ((cServerRuleOvrdDesc1322.Attributes["OnChange"] == null || cServerRuleOvrdDesc1322.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cServerRuleOvrdDesc1322.Visible && !cServerRuleOvrdDesc1322.ReadOnly) {cServerRuleOvrdDesc1322.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cServerRuleOvrdName1322.Attributes["OnChange"] == null || cServerRuleOvrdName1322.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cServerRuleOvrdName1322.Visible && !cServerRuleOvrdName1322.ReadOnly) {cServerRuleOvrdName1322.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cServerRuleId1322.Attributes["OnChange"] == null || cServerRuleId1322.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cServerRuleId1322.Visible && cServerRuleId1322.Enabled) {cServerRuleId1322.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cScreenId1322.Attributes["OnChange"] == null || cScreenId1322.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cScreenId1322.Visible && cScreenId1322.Enabled) {cScreenId1322.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cPriority1322.Attributes["OnChange"] == null || cPriority1322.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cPriority1322.Visible && !cPriority1322.ReadOnly) {cPriority1322.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cDisable1322.Attributes["OnClick"] == null || cDisable1322.Attributes["OnClick"].IndexOf("ChkPgDirty") < 0) && cDisable1322.Visible && cDisable1322.Enabled) {cDisable1322.Attributes["OnClick"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty(); this.focus();";}
			if ((cRunMode1322.Attributes["OnChange"] == null || cRunMode1322.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cRunMode1322.Visible && cRunMode1322.Enabled) {cRunMode1322.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
		}
		...
		private void ClearMaster(object sender, System.EventArgs e)
		{
			...
			if (dt.Rows[1]["ColVisible"].ToString() == "Y" && dt.Rows[1]["ColReadOnly"].ToString() != "Y") {cServerRuleOvrdDesc1322.Text = string.Empty;}
			if (dt.Rows[2]["ColVisible"].ToString() == "Y" && dt.Rows[2]["ColReadOnly"].ToString() != "Y") {cServerRuleOvrdName1322.Text = string.Empty;}
			if (dt.Rows[3]["ColVisible"].ToString() == "Y" && dt.Rows[3]["ColReadOnly"].ToString() != "Y") {cServerRuleId1322.ClearSearch();}
			if (dt.Rows[4]["ColVisible"].ToString() == "Y" && dt.Rows[4]["ColReadOnly"].ToString() != "Y") {cScreenId1322.ClearSearch();}
			if (dt.Rows[5]["ColVisible"].ToString() == "Y" && dt.Rows[5]["ColReadOnly"].ToString() != "Y") {cPriority1322.Text = string.Empty;}
			if (dt.Rows[6]["ColVisible"].ToString() == "Y" && dt.Rows[6]["ColReadOnly"].ToString() != "Y") {cDisable1322.Checked = base.GetBool("Y");}
			if (dt.Rows[7]["ColVisible"].ToString() == "Y" && dt.Rows[7]["ColReadOnly"].ToString() != "Y") {SetRunMode1322(cRunMode1322,string.Empty);}
			// *** Default Value (Folder) Web Rule starts here *** //
		}

		private void InitMaster(object sender, System.EventArgs e)
		{
			cAtServerRuleOvrdId1322.Text = string.Empty;
			cServerRuleOvrdDesc1322.Text = string.Empty;
			cServerRuleOvrdName1322.Text = string.Empty;
			cServerRuleId1322.ClearSearch();
			cScreenId1322.ClearSearch();
			cPriority1322.Text = string.Empty;
			cDisable1322.Checked = base.GetBool("Y");
			SetRunMode1322(cRunMode1322,string.Empty);
			// *** Default Value (Folder) Web Rule starts here *** //
		}
		protected void cAdmServerRuleOvrd1026List_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmServerRuleOvrd1026List_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmServerRuleOvrd1026List_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			InitMaster(sender, e);      // Do this first to enable defaults for non-database columns.
			DataTable dt = null;
			try
			{
				dt = (new AdminSystem()).GetMstById("GetAdmServerRuleOvrd1026ById",cAdmServerRuleOvrd1026List.SelectedValue,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId));
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				if (dt.Rows.Count > 0)
				{
					cAdmServerRuleOvrd1026List.SetDdVisible();
					DataRow dr = dt.Rows[0];
					try {cAtServerRuleOvrdId1322.Text = RO.Common3.Utils.fmNumeric("0",dr["AtServerRuleOvrdId1322"].ToString(),base.LUser.Culture);} catch {cAtServerRuleOvrdId1322.Text = string.Empty;}
					try {cServerRuleOvrdDesc1322.Text = dr["ServerRuleOvrdDesc1322"].ToString();} catch {cServerRuleOvrdDesc1322.Text = string.Empty;}
					try {cServerRuleOvrdName1322.Text = dr["ServerRuleOvrdName1322"].ToString();} catch {cServerRuleOvrdName1322.Text = string.Empty;}
					SetServerRuleId1322(cServerRuleId1322,dr["ServerRuleId1322"].ToString());
					SetScreenId1322(cScreenId1322,dr["ScreenId1322"].ToString());
					try {cPriority1322.Text = RO.Common3.Utils.fmNumeric("0",dr["Priority1322"].ToString(),base.LUser.Culture);} catch {cPriority1322.Text = string.Empty;}
					try {cDisable1322.Checked = base.GetBool(dr["Disable1322"].ToString());} catch {cDisable1322.Checked = false;}
					SetRunMode1322(cRunMode1322,dr["RunMode1322"].ToString());
				}
			}
			cButPanel.DataBind(); if (!cSaveButton.Visible) { cInsRowButton.Visible = false; }
			DataTable dtAdmServerRuleOvrdGrid = null;
			int filterId = 0; if (Utils.IsInt(cFilterId.SelectedValue)) { filterId = int.Parse(cFilterId.SelectedValue); }
			try
			{
				dtAdmServerRuleOvrdGrid = (new AdminSystem()).GetDtlById(1026,"GetAdmServerRuleOvrd1026DtlById",cAdmServerRuleOvrd1026List.SelectedValue,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,base.LImpr,base.LCurr);
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (!dtAdmServerRuleOvrdGrid.Columns.Contains("_NewRow")) { dtAdmServerRuleOvrdGrid.Columns.Add("_NewRow"); }
			Session[KEY_dtAdmServerRuleOvrdGrid] = dtAdmServerRuleOvrdGrid;
			cAdmServerRuleOvrdGrid.EditIndex = -1;
			cAdmServerRuleOvrdGridDataPager.PageSize = Int16.Parse(cPgSize.Text); GotoPage(0);
			if (Session[KEY_lastSortCol] != null)
			{
				cAdmServerRuleOvrdGrid_OnSorting(sender, new ListViewSortEventArgs((string)Session[KEY_lastSortExp], SortDirection.Ascending));
			}
			else if (dtAdmServerRuleOvrdGrid.Rows.Count <= 0 || (!((GetAuthRow().Rows[0]["AllowUpd"].ToString() == "N" || GetAuthRow().Rows[0]["ViewOnly"].ToString() == "G") && GetAuthRow().Rows[0]["AllowAdd"].ToString() == "N") && !(Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N") && dtAdmServerRuleOvrdGrid.Rows.Count == 1))
			{
				cAdmServerRuleOvrdGrid_DataBind(dtAdmServerRuleOvrdGrid.DefaultView);
			}
			cFindButton_Click(sender, e);
			cNaviPanel.Visible = true; cImportPwdPanel.Visible = false; Session.Remove(KEY_scrImport);
			ScriptManager.GetCurrent(Parent.Page).SetFocus(cAdmServerRuleOvrd1026List.FocusID);
			ShowDirty(false); PanelTop.Update();
			// *** List Selection (End of) Web Rule starts here *** //
		}

		protected void cPermKeyId1321_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			...
			TextBox pwd = null;
			if (cAdmServerRuleOvrdGrid.EditIndex > -1 && cAdmServerRuleOvrdGrid.Items.Count > cAdmServerRuleOvrdGrid.EditIndex)
			{
				if (Session[KEY_dtPermKeyId1321] != null)
				{
					DataView dv1 = ((DataTable)Session[KEY_dtPermKeyId1321]).DefaultView; dv1.RowFilter = string.Empty;
				}
				DataView dvAdmServerRuleOvrdGrid = ((DataTable)Session[KEY_dtAdmServerRuleOvrdGrid]).DefaultView;
				if (Session[KEY_dtPermKeyId1321] != null)
				{
					DataView dv3 = ((DataTable)Session[KEY_dtPermKeyId1321]).DefaultView;
				}
				Session[KEY_dtAdmServerRuleOvrdGrid] = dvAdmServerRuleOvrdGrid.Table;
				SetPermKeyRowId1321((RoboCoder.WebControls.ComboBox)cAdmServerRuleOvrdGrid.EditItem.FindControl("cPermKeyRowId1321"),dvAdmServerRuleOvrdGrid[cAdmServerRuleOvrdGrid.EditItem.DataItemIndex]["PermKeyRowId1321"].ToString(),((DropDownList)sender).SelectedValue, cAdmServerRuleOvrdGrid.EditItem);
			}
		...
		public void cFindButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dv != null)
			{
				...
				if ((cc != null && cc.ID.Equals("cAdmServerRuleOvrd1026List")) || cAdmServerRuleOvrdGrid.EditIndex < 0 || UpdateGridRow(cAdmServerRuleOvrdGrid, new CommandEventArgs("Save", "")))
				{
					...
					GotoPage(0); cAdmServerRuleOvrdGrid_DataBind(dv);
					if (GetCurrPageIndex() != (int)Session[KEY_currPageIndex] && (int)Session[KEY_currPageIndex] < GetTotalPages()) { GotoPage((int)Session[KEY_currPageIndex]); cAdmServerRuleOvrdGrid_DataBind(dv);}
		...
		public void cInsRowButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dt != null && (ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")))
			{
				...
				Session[KEY_lastAddedRow] = 0; Session[KEY_dtAdmServerRuleOvrdGrid] = dt; Session[KEY_currPageIndex] = 0; GotoPage(0);
				cAdmServerRuleOvrdGrid_DataBind(dt.DefaultView);
				cAdmServerRuleOvrdGrid_OnItemEditing(cAdmServerRuleOvrdGrid, new ListViewEditEventArgs(0));
		...
		public void cPgSizeButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cAdmServerRuleOvrdGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				...
				cAdmServerRuleOvrdGridDataPager.PageSize = int.Parse(cPgSize.Text); GotoPage(0);
				if (dt.Rows.Count <= 0 || (!((GetAuthRow().Rows[0]["AllowUpd"].ToString() == "N" || GetAuthRow().Rows[0]["ViewOnly"].ToString() == "G") && GetAuthRow().Rows[0]["AllowAdd"].ToString() == "N") && !(Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N") && dt.Rows.Count == 1)) {cAdmServerRuleOvrdGrid_DataBind(dt.DefaultView);} else {cFindButton_Click(sender, e);}
				try
				{
					(new AdminSystem()).UpdLastPageInfo(1026, base.LUser.UsrId, cPgSize.Text, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		}
		...
		public void cShowImpButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cSchemaImage.Visible)
			{
				try
				{
					Session["ImportSchema"] = (new AdminSystem()).GetSchemaScrImp(1026,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (cSchemaImage.Attributes["OnClick"] == null || cSchemaImage.Attributes["OnClick"].IndexOf("ImportSchema.aspx") < 0) {cSchemaImage.Attributes["OnClick"] += "SearchLink('ImportSchema.aspx?scm=S&key=1026&csy=3','','',''); return false;";}
			}
			// *** System Button Click (after) Web Rule starts here *** //
		}
		...
		private void ScrImportDdl(DataView dvd, DataRowView drv, bool bComboBox, string PKey, string CKey, string CNam, int MaxLen, string MatchCd, bool bAllowNulls)
		{
			if (dvd != null)
			{
				...
				{
					if (string.IsNullOrEmpty(cAdmServerRuleOvrd1026List.SelectedValue))
					{
						dvd.RowFilter = "(" + PKey + " is null)";
					}
					else
					{
						dvd.RowFilter = "(" + PKey + " is null OR " + PKey + " = " + cAdmServerRuleOvrd1026List.SelectedValue + ")";
					}
				}
		...
		protected void ScrImport(bool bClear)
		{
			...
			DataTable dtAdmServerRuleOvrdGrid = (DataTable)Session[KEY_dtAdmServerRuleOvrdGrid];
			if (dti != null && dtAdmServerRuleOvrdGrid != null)
			{
				if (bClear)
				{
					dtAdmServerRuleOvrdGrid.Rows.Clear();
				}
				cFind.Text = string.Empty;
				int iExist = dtAdmServerRuleOvrdGrid.Rows.Count;
				...
				foreach (DataRowView drv in dti.DefaultView)
				{
					...
					{
						DataTable dtd;
						dtd = (DataTable)Session[KEY_dtPermKeyId1321];
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(1026,"GetDdlPermKeyId3S4363",true,true,0,string.Empty,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
							Session[KEY_dtPermKeyId1321] = dtd;
						}
						ScrImportDdl(dtd.DefaultView, drv, false, "AtServerRuleOvrdId", "PermKeyId1321", "PermKeyId1321Text", 50, "", false);
						dtd = jj==iExist ? null : (DataTable)Session[KEY_dtPermKeyRowId1321];
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(1026,"GetDdlPermKeyRowId3S4369",true,true,0,string.Empty,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
							Session[KEY_dtPermKeyRowId1321] = dtd;
						}
						ScrImportDdl(dtd.DefaultView, drv, true, "AtServerRuleOvrdId", "PermKeyRowId1321", "PermKeyRowId1321Text", 105, "", true);
					}
					jj = jj + 1;
				}
				iRow = dti.DefaultView.Count - iExist;
				if (!dti.Columns.Contains("_NewRow")) { dti.Columns.Add("_NewRow"); }
				Session[KEY_dtAdmServerRuleOvrdGrid] = dti;
				cAdmServerRuleOvrdGrid_DataBind(dti.DefaultView);
		...
		public void cImportButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cFNameO.Text != string.Empty && cWorkSheet.Items.Count > 0 && cWorkSheet.SelectedItem.Text != string.Empty && cStartRow.Text != string.Empty)
			{
				...
				try
				{
					if (cAdmServerRuleOvrdGrid.EditIndex < 0 || UpdateGridRow(cAdmServerRuleOvrdGrid, new CommandEventArgs("Save", "")))
					{
						Session[KEY_scrImport] = ImportFile(dtAdmServerRuleOvrdGrid.Copy(),cFNameO.Text,cWorkSheet.SelectedItem.Text,cStartRow.Text,Config.PathTmpImport + cFName.Text);
					}
		...
		private DataTable ImportFile(DataTable dt, string fileName, string workSheet, string startRow, string fileFullName)
		{
			try
			{
				...
				{
					if (rows[iRow][1].ToString() == string.Empty && rows[iRow][2].ToString() == string.Empty && rows[iRow][5].ToString() == string.Empty && rows[iRow][6].ToString() == string.Empty) {idel = idel + 1;}
					else
					{
						dt.Rows.Add(dt.NewRow());
						if (rows[iRow][5].ToString() == string.Empty) { rows[iRow][5] = "Y";}
		...
		private void GotoPage(int pageNo)
		{
			if (pageNo >= 0 && pageNo < GetTotalPages())
			{
				if (cAdmServerRuleOvrdGrid.EditIndex < 0 || UpdateGridRow(cAdmServerRuleOvrdGrid, new CommandEventArgs("Save", "")))
				{
					try { cAdmServerRuleOvrdGridDataPager.SetPageProperties(cAdmServerRuleOvrdGridDataPager.PageSize * pageNo, cAdmServerRuleOvrdGridDataPager.MaximumRows, false); cAdmServerRuleOvrdGrid_DataBind(null); }
					catch
					{
						try { cAdmServerRuleOvrdGridDataPager.SetPageProperties(0, cAdmServerRuleOvrdGridDataPager.MaximumRows, false); cAdmServerRuleOvrdGrid_DataBind(null); } catch {}
					}
				}
			}
		}

		protected void cAdmServerRuleOvrdGrid_OnItemCommand(object sender, ListViewCommandEventArgs e)
		{
		}

		private void cAdmServerRuleOvrdGrid_DataBind(DataView dvAdmServerRuleOvrdGrid)
		{
			if (dvAdmServerRuleOvrdGrid == null)
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmServerRuleOvrdGrid];
				dvAdmServerRuleOvrdGrid = dt != null ? dt.DefaultView : null;
			}
			if (dvAdmServerRuleOvrdGrid != null)
			{
				LcAuth = GetAuthCol().AsEnumerable().ToDictionary<DataRow,string>(dr=>dr["ColName"].ToString());
				cAdmServerRuleOvrdGrid.DataSource = dvAdmServerRuleOvrdGrid;
				cAdmServerRuleOvrdGrid.DataBind();
		...
		protected void cAdmServerRuleOvrdGrid_OnSorting(object sender, ListViewSortEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmServerRuleOvrdGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null)
			{
				if (cAdmServerRuleOvrdGrid.EditIndex < 0 || UpdateGridRow(cAdmServerRuleOvrdGrid, new CommandEventArgs("Save", "")))
				{
					...
					Session[KEY_dtAdmServerRuleOvrdGrid] = dt;
					cAdmServerRuleOvrdGrid_DataBind(dv);
				}
			}
		}

		protected void cAdmServerRuleOvrdGrid_OnPagePropertiesChanging(object sender, PagePropertiesChangingEventArgs e)
		{
		    cAdmServerRuleOvrdGridDataPager.SetPageProperties(e.StartRowIndex, e.MaximumRows, false);
		    DataTable dt = (DataTable)Session[KEY_dtAdmServerRuleOvrdGrid];
		    DataView dv = dt != null ? dt.DefaultView : null;
			cAdmServerRuleOvrdGrid_DataBind(dv); cAdmServerRuleOvrdGrid.EditIndex = -1;
		}

		private void GridChkPgDirty(ListViewItem lvi)
		{
				WebControl cc = null;
				cc = ((WebControl)lvi.FindControl("cPermKeyId1321"));
				...
				cc = ((WebControl)lvi.FindControl("cPermKeyRowId1321"));
				...
				cc = ((WebControl)lvi.FindControl("cAndCondition1321"));
				...
				cc = ((WebControl)lvi.FindControl("cMatch1321"));
				if ((cc.Attributes["OnClick"] == null || cc.Attributes["OnClick"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnClick"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty(); this.focus();";}
		}

		protected void cAdmServerRuleOvrdGrid_OnItemDataBound(object sender, ListViewItemEventArgs e)
		{
			// *** GridItemDataBound (before) Web Rule End *** //
			DataTable dt = (DataTable)Session[KEY_dtAdmServerRuleOvrdGrid];
			bool isEditItem = false;
			DataView dvAdmServerRuleOvrdGrid = dt != null ? dt.DefaultView : null;
			if (cAdmServerRuleOvrdGrid.EditIndex > -1 && GetDataItemIndex(cAdmServerRuleOvrdGrid.EditIndex) == e.Item.DataItemIndex)
			{
				isEditItem = true;
				base.SetGridEnabled(e.Item, GetAuthCol(), GetLabel(), 8);
				SetPermKeyId1321((DropDownList)e.Item.FindControl("cPermKeyId1321"),dvAdmServerRuleOvrdGrid[e.Item.DataItemIndex]["PermKeyId1321"].ToString());
				SetPermKeyRowId1321((RoboCoder.WebControls.ComboBox)e.Item.FindControl("cPermKeyRowId1321"),dvAdmServerRuleOvrdGrid[e.Item.DataItemIndex]["PermKeyRowId1321"].ToString(),dvAdmServerRuleOvrdGrid[e.Item.DataItemIndex]["PermKeyId1321"].ToString(), e.Item);
				CheckBox cb = null;
				cb = (CheckBox)e.Item.FindControl("cAndCondition1321");
				if (cb != null)
				{
					cb.Checked = base.GetBool(dvAdmServerRuleOvrdGrid[e.Item.DataItemIndex]["AndCondition1321"].ToString());
				}
				cb = (CheckBox)e.Item.FindControl("cMatch1321");
				if (cb != null)
				{
					cb.Checked = base.GetBool(dvAdmServerRuleOvrdGrid[e.Item.DataItemIndex]["Match1321"].ToString());
				}
			...
			if (dtAuthRow != null)
			{
				...
				{
					cc = e.Item.FindControl("cAdmServerRuleOvrdGridDelete"); if (cc != null) { cc.Visible = false; }
					cc = e.Item.FindControl("cAdmServerRuleOvrdGridEdit"); if (cc != null) { cc.Visible = false; }
				}
				else
				{
			        HtmlTableRow tr = e.Item.FindControl("cAdmServerRuleOvrdGridRow") as HtmlTableRow;
			        LinkButton lb = e.Item.FindControl("cAdmServerRuleOvrdGridEdit") as LinkButton;
		...
		protected void cServerRuledOvrdPrmId1321hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "0") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "0"; Session[KEY_lastSortExp] = "ServerRuledOvrdPrmId1321";Session[KEY_lastSortImg] = "cServerRuledOvrdPrmId1321hi";
			cAdmServerRuleOvrdGrid_OnSorting(sender, new ListViewSortEventArgs("ServerRuledOvrdPrmId1321", SortDirection.Ascending));
		}

		protected void cPermKeyId1321hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "1") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "1"; Session[KEY_lastSortExp] = "PermKeyId1321Text";Session[KEY_lastSortImg] = "cPermKeyId1321hi";
			cAdmServerRuleOvrdGrid_OnSorting(sender, new ListViewSortEventArgs("PermKeyId1321Text", SortDirection.Ascending));
		}

		protected void cPermKeyRowId1321hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "2") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "2"; Session[KEY_lastSortExp] = "PermKeyRowId1321Text";Session[KEY_lastSortImg] = "cPermKeyRowId1321hi";
			cAdmServerRuleOvrdGrid_OnSorting(sender, new ListViewSortEventArgs("PermKeyRowId1321Text", SortDirection.Ascending));
		}

		protected void cAndCondition1321hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "3") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "3"; Session[KEY_lastSortExp] = "AndCondition1321";Session[KEY_lastSortImg] = "cAndCondition1321hi";
			cAdmServerRuleOvrdGrid_OnSorting(sender, new ListViewSortEventArgs("AndCondition1321", SortDirection.Ascending));
		}

		protected void cMatch1321hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "4") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "4"; Session[KEY_lastSortExp] = "Match1321";Session[KEY_lastSortImg] = "cMatch1321hi";
			cAdmServerRuleOvrdGrid_OnSorting(sender, new ListViewSortEventArgs("Match1321", SortDirection.Ascending));
		}

		protected void cAdmServerRuleOvrdGrid_OnItemEditing(object sender, ListViewEditEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmServerRuleOvrdGrid];
			if ((ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")) && dt != null && dt.DefaultView.Count > GetDataItemIndex(e.NewEditIndex))
			{
				cAdmServerRuleOvrdGrid.EditIndex = e.NewEditIndex;
				cAdmServerRuleOvrdGrid_DataBind(null);
				cPermKeyId1321_SelectedIndexChanged((object)cAdmServerRuleOvrdGrid.EditItem.FindControl("cPermKeyId1321"), new EventArgs());
				Control wc = null;
				string idx = Page.Request["__EVENTARGUMENT"];
				if (!string.IsNullOrEmpty(idx))
				{
				    Control ctrlToFocus = cAdmServerRuleOvrdGrid.EditItem.FindControl("c" + idx);
				    if (((WebControl)ctrlToFocus).Enabled) { wc = ctrlToFocus; }
				}
				if (wc == null) { wc = FindEditableControl(cAdmServerRuleOvrdGrid.EditItem); }
				if (wc != null)
             {
             ...
             MkMobileLabel(cAdmServerRuleOvrdGrid.EditItem);
		    }
		    else { e.Cancel = true; }
		}
		...
		{
		    Label ml = null;
		    ml = lvi.FindControl("cServerRuledOvrdPrmId1321ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(8); }
		    ml = lvi.FindControl("cPermKeyId1321ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(9); }
		    ml = lvi.FindControl("cPermKeyRowId1321ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(10); }
		    ml = lvi.FindControl("cAndCondition1321ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(11); }
		    ml = lvi.FindControl("cMatch1321ml") as Label;
		...
		protected void GridFill(ListViewItem lvi, DataTable dt, DataRow dr, bool bInsert)
		{
			...
			ddl = (DropDownList)lvi.FindControl("cPermKeyId1321");
			if (ddl != null)
			{
				if (ddl.SelectedIndex >= 0 && ddl.SelectedValue.ToString() != string.Empty)
				{
					dr["PermKeyId1321"] = ddl.SelectedValue;
					dr["PermKeyId1321Text"] = ddl.SelectedItem.Text;
				}
				else
				{
					dr["PermKeyId1321"] = System.DBNull.Value;
					dr["PermKeyId1321Text"] = System.DBNull.Value;
				}
			}
			RoboCoder.WebControls.ComboBox cbb = null;
			cbb = (RoboCoder.WebControls.ComboBox)lvi.FindControl("cPermKeyRowId1321");
			if (cbb != null)
			{
				if (cbb.SelectedValue != string.Empty)
				{
					dr["PermKeyRowId1321"] = cbb.SelectedValue;
					dr["PermKeyRowId1321Text"] = cbb.SelectedText;
				}
				else
				{
					dr["PermKeyRowId1321"] = System.DBNull.Value;
					dr["PermKeyRowId1321Text"] = System.DBNull.Value;
				}
			}
			CheckBox cb = null;
			cb = (CheckBox)lvi.FindControl("cAndCondition1321");
			if (cb != null)
			{
				dr["AndCondition1321"] = base.SetBool(cb.Checked);
			}
			cb = (CheckBox)lvi.FindControl("cMatch1321");
			if (cb != null)
			{
				dr["Match1321"] = base.SetBool(cb.Checked);
			}
		...
		protected void cAdmServerRuleOvrdGrid_OnItemUpdating(object sender, ListViewUpdateEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmServerRuleOvrdGrid];
			if (dt != null && dt.Rows.Count > 0)
			{
				DataRow dr = dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row;
			    GridFill(cAdmServerRuleOvrdGrid.EditItem, dt, dr, false);
			    dr["_NewRow"] = "N";
			    if (dt.Columns.Contains("_SortOrder")) dr["_SortOrder"] = null;
			}
		}

		protected void cAdmServerRuleOvrdGrid_OnItemCanceling(object sender, ListViewCancelEventArgs e)
		{
			PanelTop.Update();
			DataTable dt = (DataTable)Session[KEY_dtAdmServerRuleOvrdGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			...
			{
				cAdmServerRuleOvrdGrid_OnItemDeleting(sender, new ListViewDeleteEventArgs(e.ItemIndex));
			}
			if (dt.Columns.Contains("_SortOrder")) dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row["_SortOrder"] = null;
			cAdmServerRuleOvrdGrid.EditIndex = -1; cAdmServerRuleOvrdGrid_DataBind(null);
		}
		...
		protected void cDeleteAllButton_Click(object sender, System.EventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmServerRuleOvrdGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null && (cAdmServerRuleOvrdGrid.EditIndex < 0 || UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				GotoPage(0);
				while (dv.Count > 0) {dv.Delete(0);}
				Session[KEY_dtAdmServerRuleOvrdGrid] = dt;
				cAdmServerRuleOvrdGrid.EditIndex = -1; cAdmServerRuleOvrdGrid_DataBind(dv);
			}
		}

		protected void cAdmServerRuleOvrdGrid_OnItemDeleting(object sender, ListViewDeleteEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmServerRuleOvrdGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			DataRow dr = dv != null ? dv[GetDataItemIndex(e.ItemIndex)].Row : null;
			if (dv != null && (cAdmServerRuleOvrdGrid.EditIndex == e.ItemIndex || UpdateGridRow(sender, new CommandEventArgs("Save",""))))
			{
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cAtServerRuleOvrdId1322.Text == string.Empty) { cClearButton_Click(sender, new EventArgs()); ShowDirty(false); PanelTop.Update();}
			else { PopAdmServerRuleOvrd1026List(sender, e, false, cAtServerRuleOvrdId1322.Text); }
			// *** System Button Click (After) Web Rule starts here *** //
		}
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmServerRuleOvrd1026List); PopAdmServerRuleOvrd1026List(sender, e, false, null);
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			if (ValidPage() && (cAdmServerRuleOvrdGrid.EditIndex < 0 || UpdateGridRow(cAdmServerRuleOvrdGrid, new CommandEventArgs("Save", ""))))
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmServerRuleOvrdGrid];
				DataView dv = dt != null ? dt.DefaultView : null;
				string ftr = string.Empty;
				if (dv.RowFilter != string.Empty) { ftr = dv.RowFilter; dv.RowFilter = string.Empty; }
				AdmServerRuleOvrd1026 ds = PrepAdmServerRuleOvrdData(dv,cAtServerRuleOvrdId1322.Text == string.Empty);
				if (ftr != string.Empty) {dv.RowFilter = ftr;}
				if (!string.IsNullOrEmpty(cAdmServerRuleOvrd1026List.SelectedValue) && cAtServerRuleOvrdId1322.Text != cAdmServerRuleOvrd1026List.SelectedValue)
				{
					PreMsgPopup("Primary key cannot be changed");
					return rtn;
				}
				if (string.IsNullOrEmpty(cAdmServerRuleOvrd1026List.SelectedValue))	// Add
				{
					if (ds != null)
					{
						pid = (new AdminSystem()).AddData(1026,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc,noTrans,commandTimeOut);
					}
					if (!string.IsNullOrEmpty(pid))
					{
						cAdmServerRuleOvrd1026List.ClearSearch(); Session.Remove(KEY_dtAdmServerRuleOvrd1026List);
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmServerRuleOvrd1026List(sender, e, false, pid);
						rtn = GetScreenHlp().Rows[0]["AddMsg"].ToString();
					}
				}
				else {
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(1026,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc,noTrans,commandTimeOut)) {bValid7 = true;}
					if (bValid7)
					{
						cAdmServerRuleOvrd1026List.ClearSearch(); Session.Remove(KEY_dtAdmServerRuleOvrd1026List);
						Session[KEY_currPageIndex] = GetCurrPageIndex();
						ShowDirty(false); PopAdmServerRuleOvrd1026List(sender, e, false, ds.Tables["AdmServerRuleOvrd"].Rows[0]["AtServerRuleOvrdId1322"]);
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cAtServerRuleOvrdId1322.Text != string.Empty)
			{
				AdmServerRuleOvrd1026 ds = PrepAdmServerRuleOvrdData(null,false);
				try
				{
					if (ds != null && (new AdminSystem()).DelData(1026,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc))
					{
						cAdmServerRuleOvrd1026List.ClearSearch(); Session.Remove(KEY_dtAdmServerRuleOvrd1026List);
						ShowDirty(false); PopAdmServerRuleOvrd1026List(sender, e, false, null);
		...
		private AdmServerRuleOvrd1026 PrepAdmServerRuleOvrdData(DataView dv, bool bAdd)
		{
			...
			if (bAdd) { dr["AtServerRuleOvrdId1322"] = string.Empty; } else { dr["AtServerRuleOvrdId1322"] = cAtServerRuleOvrdId1322.Text; }
			drType["AtServerRuleOvrdId1322"] = "Numeric"; drDisp["AtServerRuleOvrdId1322"] = "TextBox";
			try {dr["ServerRuleOvrdDesc1322"] = cServerRuleOvrdDesc1322.Text.Trim();} catch {}
			drType["ServerRuleOvrdDesc1322"] = "VarChar"; drDisp["ServerRuleOvrdDesc1322"] = "TextBox";
			try {dr["ServerRuleOvrdName1322"] = cServerRuleOvrdName1322.Text.Trim();} catch {}
			drType["ServerRuleOvrdName1322"] = "VarChar"; drDisp["ServerRuleOvrdName1322"] = "TextBox";
			try {dr["ServerRuleId1322"] = cServerRuleId1322.SelectedValue;} catch {}
			drType["ServerRuleId1322"] = "Numeric"; drDisp["ServerRuleId1322"] = "AutoComplete";
			try {dr["ScreenId1322"] = cScreenId1322.SelectedValue;} catch {}
			drType["ScreenId1322"] = "Numeric"; drDisp["ScreenId1322"] = "AutoComplete";
			try {dr["Priority1322"] = cPriority1322.Text.Trim();} catch {}
			drType["Priority1322"] = "Numeric"; drDisp["Priority1322"] = "TextBox";
			try {dr["Disable1322"] = base.SetBool(cDisable1322.Checked);} catch {}
			drType["Disable1322"] = "Char"; drDisp["Disable1322"] = "CheckBox";
			try {dr["RunMode1322"] = cRunMode1322.SelectedValue;} catch {}
			...
			if (dv != null)
			{
				...
				if (bAdd)
				{
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmServerRuleOvrdAdd"].Rows.Add(MakeColRow(ds.Tables["AdmServerRuleOvrdAdd"].NewRow(), drv, true));
					}
				}
				else
				{
					...
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmServerRuleOvrdUpd"].Rows.Add(MakeColRow(ds.Tables["AdmServerRuleOvrdUpd"].NewRow(), drv, false));
					}
					...
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmServerRuleOvrdDel"].Rows.Add(MakeColRow(ds.Tables["AdmServerRuleOvrdDel"].NewRow(), drv, false));
					}
		...
		public bool CanAct(char typ) { return CanAct(typ, GetAuthRow(), cAdmServerRuleOvrd1026List.SelectedValue.ToString()); }
		...
		private bool ValidPage()
		{
			...
			DataTable dt = null;
			dt = (DataTable)Session[KEY_dtServerRuleId1322];
			if (dt != null && dt.Rows.Count <= 0)
			{
				bErrNow.Value = "Y"; PreMsgPopup("Value is expected for 'ServerRuleId', please investigate."); return false;
			}
			dt = (DataTable)Session[KEY_dtPermKeyId1321];
			if (dt != null && dt.Rows.Count <= 0)
			{
		...
		{
			dr["AtServerRuleOvrdId1322"] = cAtServerRuleOvrdId1322.Text;
			DataTable dtAuth = GetAuthCol();
			if (dtAuth != null)
			{
				dr["ServerRuledOvrdPrmId1321"] = drv["ServerRuledOvrdPrmId1321"].ToString().Trim();
				dr["PermKeyId1321"] = drv["PermKeyId1321"];
				dr["PermKeyRowId1321"] = drv["PermKeyRowId1321"];
				if (bAdd && dtAuth.Rows[10]["ColReadOnly"].ToString() == "Y" && dr["PermKeyRowId1321"].ToString() == string.Empty) {dr["PermKeyRowId1321"] = System.DBNull.Value;}
		...
		{
			if (cAdmServerRuleOvrdGrid.EditIndex > -1)
			{
				cAdmServerRuleOvrdGrid_OnItemUpdating(sender, new ListViewUpdateEventArgs(cAdmServerRuleOvrdGrid.EditIndex));
			}
			return true;
		}

		protected void cAdmServerRuleOvrdGrid_OnPreRender(object sender, System.EventArgs e)
		{
			System.Web.UI.WebControls.Image hi = null;
			hi = (System.Web.UI.WebControls.Image)cAdmServerRuleOvrdGrid.FindControl("cServerRuledOvrdPrmId1321hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmServerRuleOvrdGrid.FindControl("cPermKeyId1321hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmServerRuleOvrdGrid.FindControl("cPermKeyRowId1321hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmServerRuleOvrdGrid.FindControl("cAndCondition1321hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmServerRuleOvrdGrid.FindControl("cMatch1321hi"); if (hi != null) { hi.Visible = false; }
			if (Session[KEY_lastSortImg] != null)
			{
				hi = (System.Web.UI.WebControls.Image)cAdmServerRuleOvrdGrid.FindControl((string)Session[KEY_lastSortImg]);
				if (hi != null) { hi.ImageUrl = Utils.AddTilde((string)Session[KEY_lastSortUrl]); hi.Visible = true; }
			}
			IgnoreHeaderConfirm((LinkButton)cAdmServerRuleOvrdGrid.FindControl("cServerRuledOvrdPrmId1321hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmServerRuleOvrdGrid.FindControl("cPermKeyId1321hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmServerRuleOvrdGrid.FindControl("cPermKeyRowId1321hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmServerRuleOvrdGrid.FindControl("cAndCondition1321hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmServerRuleOvrdGrid.FindControl("cMatch1321hl"));
		}

		private string GetButtonId(ListViewItem lvi)
		{
			string ButtonID = String.Empty;
			Control c = lvi.FindControl("cAdmServerRuleOvrdGridEdit");
		...
		{
		    return cAdmServerRuleOvrdGridDataPager.StartRowIndex / cAdmServerRuleOvrdGridDataPager.PageSize;
		}

		private int GetTotalPages()
		{
		    return (int)Math.Ceiling((double)cAdmServerRuleOvrdGridDataPager.TotalRowCount / cAdmServerRuleOvrdGridDataPager.PageSize);
		}

		private int GetDataItemIndex(int editIndex)
		{
		    return cAdmServerRuleOvrdGridDataPager.StartRowIndex + editIndex;
		}

		protected void cAdmServerRuleOvrdGrid_OnLayoutCreated(object sender, EventArgs e)
		{
		    // Header:
		    LinkButton lb = null;
		    lb = cAdmServerRuleOvrdGrid.FindControl("cServerRuledOvrdPrmId1321hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(8); lb.ToolTip = ColumnToolTip(8); lb.Parent.Visible = GridColumnVisible(8); }
		    lb = cAdmServerRuleOvrdGrid.FindControl("cPermKeyId1321hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(9); lb.ToolTip = ColumnToolTip(9); lb.Parent.Visible = GridColumnVisible(9); }
		    lb = cAdmServerRuleOvrdGrid.FindControl("cPermKeyRowId1321hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(10); lb.ToolTip = ColumnToolTip(10); lb.Parent.Visible = GridColumnVisible(10); }
		    lb = cAdmServerRuleOvrdGrid.FindControl("cAndCondition1321hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(11); lb.ToolTip = ColumnToolTip(11); lb.Parent.Visible = GridColumnVisible(11); }
		    lb = cAdmServerRuleOvrdGrid.FindControl("cMatch1321hl") as LinkButton;
			...
			if (dtAuthRow != null)
			{
				...
				{
		            lb = cAdmServerRuleOvrdGrid.FindControl("cDeleteAllButton") as LinkButton; if (lb != null) { lb.Visible = false; }
				}
			}
		    // footer:
		    Label gc = null;
		    gc = cAdmServerRuleOvrdGrid.FindControl("cServerRuledOvrdPrmId1321fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(8); }
		    gc = cAdmServerRuleOvrdGrid.FindControl("cPermKeyId1321fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(9); }
		    gc = cAdmServerRuleOvrdGrid.FindControl("cPermKeyRowId1321fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(10); }
		    gc = cAdmServerRuleOvrdGrid.FindControl("cAndCondition1321fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(11); }
		    gc = cAdmServerRuleOvrdGrid.FindControl("cMatch1321fl") as Label;
		...
		private void PreMsgPopup(string msg) { PreMsgPopup(msg,cAdmServerRuleOvrd1026List,null); }

...
namespace RO.Web
{
	public partial class AdmSignupModule : RO.Web.ModuleBase
	{
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_1018";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_1018";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_1018";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_1018";
		private const string KEY_dtLabel = "Cache:dtLabel3_1018";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_1018";
		private const string KEY_dtScrCri = "Cache:dtScrCri3_1018";
		private const string KEY_dsScrCriVal = "Cache:dsScrCriVal3_1018";


		private const string KEY_dtSystems = "Cache:dtSystems3_1018";
		private const string KEY_sysConnectionString = "Cache:sysConnectionString3_1018";
		private const string KEY_dtAdmSignup1018List = "Cache:dtAdmSignup1018List";
		private const string KEY_bNewVisible = "Cache:bNewVisible3_1018";
		private const string KEY_bNewSaveVisible = "Cache:bNewSaveVisible3_1018";
		private const string KEY_bCopyVisible = "Cache:bCopyVisible3_1018";
		private const string KEY_bCopySaveVisible = "Cache:bCopySaveVisible3_1018";
		private const string KEY_bDeleteVisible = "Cache:bDeleteVisible3_1018";
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_1018";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_1018";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_1018";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				Session.Remove(KEY_dtAdmSignup1018List);
				...
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 1018, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmSignup1018List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=1018&typ=N&sys=3','','',''); return false;"; }
		...
		protected void Page_Init(object sender, EventArgs e)
		{
            ...
            cTokenMsgP2.Visible = false; cConfirmationTokenP2.Visible = false; cResnedTokenP2.Visible = false; cSubmitP2.Visible = false; cUsrPassword270.Visible = false;
		...
		private void SetSystem(byte SystemId)
		{
			...
			try
			{
				...
				if ((Config.DeployType == "DEV" || row["dbAppDatabase"].ToString() == base.CPrj.EntityCode + "View") && !(base.CPrj.EntityCode != "RO" && row["SysProgram"].ToString() == "Y") && (new AdminSystem()).IsRegenNeeded(string.Empty,1018,0,0,LcSysConnString,LcAppPw))
				{
					(new GenScreensSystem()).CreateProgram(1018, "Sign Up", row["dbAppDatabase"].ToString(), base.CPrj, base.CSrc, base.CTar, LcAppConnString, LcAppPw);
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(1018,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(1018,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(1018,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(1018,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,1018,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(1018,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(1018,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(1018,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		protected DataTable GetAuthRow()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthRow(1018,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				try
				{
					try {dv = new DataView((new AdminSystem()).GetExp(1018,"GetExpAdmSignup1018","Y",null,null,filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
					catch {dv = new DataView((new AdminSystem()).GetExp(1018,"GetExpAdmSignup1018","N",null,null,filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(1018,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					{
						if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["UsrId270"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["UsrName270"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[7]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["LoginName270"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[8]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["UsrEmail270"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						sb.Append(Environment.NewLine);
					}
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmSignup.csv"; Session["ExportStr"] = (Config.ExportExcelCSV ? "sep=\t\n": "") + sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmSignup.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private string GetRtfGrid(DataView dv)
		{
			...
			try
			{
				...
				foreach (DataRowView drv in dv)
				{
					...
					sb.Append("\r\n");
					sb.Append(@"{");
					if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["UsrId270"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append(drv["UsrName270"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[7]["ColExport"].ToString() == "Y") {sb.Append(drv["LoginName270"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[8]["ColExport"].ToString() == "Y") {sb.Append(drv["UsrEmail270"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					sb.Append(@"}");
					sb.Append("\r\n");
					sb.Append(@"\pard \ql \li0\ri0\widctlpar\intbl\aspalpha\aspnum\adjustright\rin0\lin0 {");
					sb.Append(@"\insrsid2981395 \trowd \irow0\irowband0\lastrow \ts15\trgaph108\trleft-108\trbrdrt\brdrs\brdrw10 \trbrdrl\brdrs\brdrw10 \trbrdrb\brdrs\brdrw10 \trbrdrr\brdrs\brdrw10 \trbrdrh\brdrs\brdrw10 \trbrdrv\brdrs\brdrw10 ");
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 1018, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			...
			UpdCriteria(true);
			cAdmSignup1018List.ClearSearch(); Session.Remove(KEY_dtAdmSignup1018List);
			PopAdmSignup1018List(sender, e, false, null);
		}
			...
			try
			{
				foreach (DataRowView drv in dvCri)
				{
					...
					if (drv["DisplayName"].ToString() == "ComboBox")
					{
						...
						(new AdminSystem()).MkGetScreenIn("1018", drv["ScreenCriId"].ToString(), "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), LcAppDb, LcDesDb, drv["MultiDesignDb"].ToString(), LcSysConnString, LcAppPw);
						DataView dv = new DataView((new AdminSystem()).GetScreenIn("1018", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), (new AdminSystem()).CountScrCri(drv["ScreenCriId"].ToString(), drv["MultiDesignDb"].ToString(), drv["MultiDesignDb"].ToString() == "N" ? LcSysConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)), drv["RequiredValid"].ToString(), 0, string.Empty, drv["DisplayMode"].ToString() != "AutoListBox", val, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)));
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("1018", LcSysConnString, LcAppPw);
				}
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmSignup1018" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmSignup1018" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			foreach (DataRowView drv in dvCri)
			{
			    if (drv["DisplayName"].ToString() == "ListBox")
			    {
					...
					if (cListBox != null)
					{
						...
						int TotalChoiceCnt = new DataView((new AdminSystem()).GetScreenIn("1018", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), CriCnt, drv["RequiredValid"].ToString(), 0, string.Empty, true, string.Empty, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId))).Count;
						string selectedValues = string.Join(",", cListBox.Items.Cast<ListItem>().Where(x => x.Selected).Select(x => "'" + x.Value + "'").ToArray());
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("1018", "AdmSignup1018", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		private DataView GetAdmSignup1018List()
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmSignup1018List];
			if (dt == null)
			{
				CheckAuthentication(false);
				int filterId = 0;
				...
				try
				{
					try {dt = (new AdminSystem()).GetLis(1018,"GetLisAdmSignup1018",true,"Y",0,null,null,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
					catch {dt = (new AdminSystem()).GetLis(1018,"GetLisAdmSignup1018",true,"N",0,null,null,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				}
		...
		private void PopAdmSignup1018List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			DataView dv = GetAdmSignup1018List();
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			cAdmSignup1018List.AutoCompleteUrl = "AutoComplete.aspx/LisSuggests";
			cAdmSignup1018List.DataContext = context;
			if (dv.Table == null) return;
			cAdmSignup1018List.DataSource = dv;
			cAdmSignup1018List.Visible = true;
			if (cAdmSignup1018List.Items.Count <= 0) {cAdmSignup1018List.Visible = false; cAdmSignup1018List_SelectedIndexChanged(sender,e);}
			else
			{
				if (bPageLoad && !string.IsNullOrEmpty(Request.QueryString["key"]) && bUseCri.Value == string.Empty)
				{
					try {cAdmSignup1018List.SelectByValue(Request.QueryString["key"],string.Empty,true);} catch {cAdmSignup1018List.Items[0].Selected = true; cAdmSignup1018List_SelectedIndexChanged(sender, e);}
				    cScreenSearch.Visible = false; cSystem.Visible = false; cEditButton.Visible = true; cSaveCloseButton.Visible = cSaveButton.Visible;
				}
				else
				{
					if (ID != null) {if (!cAdmSignup1018List.SelectByValue(ID,string.Empty,true)) {ID = null;}}
					if (ID == null)
					{
						cAdmSignup1018List_SelectedIndexChanged(sender, e);
					}
				}
			}
		}
		...
		private void SetColumnAuthority()
		{
			...
			{
				base.SetFoldBehavior(cDummyWhiteSpace7, dtAuth.Rows[0], null, null, null, dtLabel.Rows[0], null, null, null);
				base.SetFoldBehavior(cSignUpTitle, dtAuth.Rows[1], null, null, null, dtLabel.Rows[1], null, null, null);
				base.SetFoldBehavior(cSignUpTopMsg, dtAuth.Rows[2], null, null, null, dtLabel.Rows[2], null, null, null);
				base.SetFoldBehavior(cDummyWhiteSpace8, dtAuth.Rows[3], null, null, null, dtLabel.Rows[3], null, null, null);
				base.SetFoldBehavior(cDummyWhiteSpace1, dtAuth.Rows[4], null, null, null, dtLabel.Rows[4], null, null, null);
				base.SetFoldBehavior(cUsrId270, dtAuth.Rows[5], cUsrId270P1, cUsrId270Label, cUsrId270P2, null, dtLabel.Rows[5], null, null, null);
				base.SetFoldBehavior(cUsrName270, dtAuth.Rows[6], cUsrName270P1, cUsrName270Label, cUsrName270P2, null, dtLabel.Rows[6], null, null, null);
				base.SetFoldBehavior(cLoginName270, dtAuth.Rows[7], cLoginName270P1, cLoginName270Label, cLoginName270P2, null, dtLabel.Rows[7], null, null, null);
				base.SetFoldBehavior(cUsrEmail270, dtAuth.Rows[8], cUsrEmail270P1, cUsrEmail270Label, cUsrEmail270P2, null, dtLabel.Rows[8], null, null, null);
				base.SetFoldBehavior(cUsrPassword270, dtAuth.Rows[9], cUsrPassword270P1, cUsrPassword270Label, cUsrPassword270P2, null, dtLabel.Rows[9], null, null, null);
				base.SetFoldBehavior(cDummyWhiteSpace2, dtAuth.Rows[10], null, null, null, dtLabel.Rows[10], null, null, null);
				base.SetFoldBehavior(cDummyWhiteSpace3, dtAuth.Rows[11], null, null, null, dtLabel.Rows[11], null, null, null);
				base.SetFoldBehavior(cTokenMsg, dtAuth.Rows[12], null, null, null, dtLabel.Rows[12], null, null, null);
				base.SetFoldBehavior(cConfirmationToken, dtAuth.Rows[13], cConfirmationTokenP1, cConfirmationTokenLabel, cConfirmationTokenP2, null, dtLabel.Rows[13], null, null, null);
				base.SetFoldBehavior(cToken, dtAuth.Rows[14], cTokenP1, cTokenLabel, cTokenP2, null, dtLabel.Rows[14], null, null, null);
				base.SetFoldBehavior(cDummyWhiteSpace9, dtAuth.Rows[15], null, null, null, dtLabel.Rows[15], null, null, null);
				base.SetFoldBehavior(cResnedToken, dtAuth.Rows[16], null, null, null, dtLabel.Rows[16], null, null, null);
				base.SetFoldBehavior(cDummyWhiteSpace4, dtAuth.Rows[17], null, null, null, dtLabel.Rows[17], null, null, null);
				base.SetFoldBehavior(cDummyWhiteSpace5, dtAuth.Rows[18], null, null, null, dtLabel.Rows[18], null, null, null);
				base.SetFoldBehavior(cSubmit, dtAuth.Rows[19], null, null, null, dtLabel.Rows[19], null, null, null);
				base.SetFoldBehavior(cSignUpBtn, dtAuth.Rows[20], null, null, null, dtLabel.Rows[20], null, null, null);
				base.SetFoldBehavior(cDummyWhiteSpace6, dtAuth.Rows[21], null, null, null, dtLabel.Rows[21], null, null, null);
				base.SetFoldBehavior(cDummyWhiteSpace10, dtAuth.Rows[22], null, null, null, dtLabel.Rows[22], null, null, null);
				base.SetFoldBehavior(cDummyWhiteSpace11, dtAuth.Rows[24], null, null, null, dtLabel.Rows[24], null, null, null);
			}
			if ((cUsrName270.Attributes["OnChange"] == null || cUsrName270.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cUsrName270.Visible && !cUsrName270.ReadOnly) {cUsrName270.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cLoginName270.Attributes["OnChange"] == null || cLoginName270.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cLoginName270.Visible && !cLoginName270.ReadOnly) {cLoginName270.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cUsrEmail270.Attributes["OnChange"] == null || cUsrEmail270.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cUsrEmail270.Visible && !cUsrEmail270.ReadOnly) {cUsrEmail270.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cUsrPassword270.Attributes["OnChange"] == null || cUsrPassword270.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cUsrPassword270.Visible && !cUsrPassword270.ReadOnly) {cUsrPassword270.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cConfirmationToken.Attributes["OnChange"] == null || cConfirmationToken.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cConfirmationToken.Visible && !cConfirmationToken.ReadOnly) {cConfirmationToken.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cToken.Attributes["OnChange"] == null || cToken.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cToken.Visible && !cToken.ReadOnly) {cToken.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
		}
		...
		private void ClearMaster(object sender, System.EventArgs e)
		{
			...
			if (dt.Rows[6]["ColVisible"].ToString() == "Y" && dt.Rows[6]["ColReadOnly"].ToString() != "Y") {cUsrName270.Text = string.Empty;}
			if (dt.Rows[7]["ColVisible"].ToString() == "Y" && dt.Rows[7]["ColReadOnly"].ToString() != "Y") {cLoginName270.Text = string.Empty;}
			if (dt.Rows[8]["ColVisible"].ToString() == "Y" && dt.Rows[8]["ColReadOnly"].ToString() != "Y") {cUsrEmail270.Text = string.Empty;}
			if (dt.Rows[9]["ColVisible"].ToString() == "Y" && dt.Rows[9]["ColReadOnly"].ToString() != "Y") {cUsrPassword270.Text = string.Empty;}
			if (dt.Rows[13]["ColVisible"].ToString() == "Y" && dt.Rows[13]["ColReadOnly"].ToString() != "Y") {cConfirmationToken.Text = string.Empty;}
			if (dt.Rows[14]["ColVisible"].ToString() == "Y" && dt.Rows[14]["ColReadOnly"].ToString() != "Y") {cToken.Text = string.Empty;}
			// *** Default Value (Folder) Web Rule starts here *** //
		}

		private void InitMaster(object sender, System.EventArgs e)
		{
			cUsrId270.Text = string.Empty;
			cUsrName270.Text = string.Empty;
			cLoginName270.Text = string.Empty;
			cUsrEmail270.Text = string.Empty;
			cUsrPassword270.Text = string.Empty;
			cConfirmationToken.Text = string.Empty;
			cToken.Text = string.Empty;

			// *** Default Value (Folder) Web Rule starts here *** //
		}
		protected void cAdmSignup1018List_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmSignup1018List_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmSignup1018List_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			InitMaster(sender, e);      // Do this first to enable defaults for non-database columns.
			DataTable dt = null;
			try
			{
				dt = (new AdminSystem()).GetMstById("GetAdmSignup1018ById",cAdmSignup1018List.SelectedValue,null,null);
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				if (dt.Rows.Count > 0)
				{
					cAdmSignup1018List.SetDdVisible();
					DataRow dr = dt.Rows[0];
					try {cUsrId270.Text = RO.Common3.Utils.fmNumeric("0",dr["UsrId270"].ToString(),base.LUser.Culture);} catch {cUsrId270.Text = string.Empty;}
					try {cUsrName270.Text = dr["UsrName270"].ToString();} catch {cUsrName270.Text = string.Empty;}
					try {cLoginName270.Text = dr["LoginName270"].ToString();} catch {cLoginName270.Text = string.Empty;}
					try {cUsrEmail270.Text = dr["UsrEmail270"].ToString();} catch {cUsrEmail270.Text = string.Empty;}
					try {cUsrPassword270.Text = dr["UsrPassword270"].ToString();} catch {cUsrPassword270.Text = string.Empty;}
				}
			}
			cButPanel.DataBind();
			ScriptManager.GetCurrent(Parent.Page).SetFocus(cAdmSignup1018List.FocusID);
			ShowDirty(false); PanelTop.Update();
			// *** List Selection (End of) Web Rule starts here *** //
		}

		protected void cResnedToken_Click(object sender, System.EventArgs e)
		{
			//WebRule: resend button click
            Guid g = Guid.NewGuid();
		...
		{
			//WebRule: submit button click
            if (ValidPage() && cConfirmationToken.Text == cToken.Text)
            {
		...
		protected void cSignUpBtn_Click(object sender, System.EventArgs e)
		{
			//WebRule: Join button click
            if (ValidPage())
            {
                Guid g = Guid.NewGuid();
                string GuidString = g.ToString();
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cUsrId270.Text == string.Empty) { cClearButton_Click(sender, new EventArgs()); ShowDirty(false); PanelTop.Update();}
			else { PopAdmSignup1018List(sender, e, false, cUsrId270.Text); }
			// *** System Button Click (After) Web Rule starts here *** //
		}
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmSignup1018List); PopAdmSignup1018List(sender, e, false, null);
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			if (ValidPage())
			{
				AdmSignup1018 ds = PrepAdmSignupData(null,cUsrId270.Text == string.Empty);
				if (!string.IsNullOrEmpty(cAdmSignup1018List.SelectedValue) && cUsrId270.Text != cAdmSignup1018List.SelectedValue)
				{
					PreMsgPopup("Primary key cannot be changed");
					return rtn;
				}
				if (string.IsNullOrEmpty(cAdmSignup1018List.SelectedValue))	// Add
				{
					if (ds != null)
					{
						pid = (new AdminSystem()).AddData(1018,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc,noTrans,commandTimeOut);
					}
					if (!string.IsNullOrEmpty(pid))
					{
						cAdmSignup1018List.ClearSearch(); Session.Remove(KEY_dtAdmSignup1018List);
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmSignup1018List(sender, e, false, pid);
						rtn = GetScreenHlp().Rows[0]["AddMsg"].ToString();
					}
				}
				else {
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(1018,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc,noTrans,commandTimeOut)) {bValid7 = true;}
					if (bValid7)
					{
						cAdmSignup1018List.ClearSearch(); Session.Remove(KEY_dtAdmSignup1018List);
						ShowDirty(false); PopAdmSignup1018List(sender, e, false, ds.Tables["AdmSignup"].Rows[0]["UsrId270"]);
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cUsrId270.Text != string.Empty)
			{
				AdmSignup1018 ds = PrepAdmSignupData(null,false);
				try
				{
					if (ds != null && (new AdminSystem()).DelData(1018,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc))
					{
						cAdmSignup1018List.ClearSearch(); Session.Remove(KEY_dtAdmSignup1018List);
						ShowDirty(false); PopAdmSignup1018List(sender, e, false, null);
		...
		private AdmSignup1018 PrepAdmSignupData(DataView dv, bool bAdd)
		{
		...
		public bool CanAct(char typ) { return CanAct(typ, GetAuthRow(), cAdmSignup1018List.SelectedValue.ToString()); }
		...
		private void PreMsgPopup(string msg) { PreMsgPopup(msg,cAdmSignup1018List,null); }

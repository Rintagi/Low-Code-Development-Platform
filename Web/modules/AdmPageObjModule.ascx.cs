		...
		private void getReport(string eExport)
		{
			try
			{
				...
				if (eExport == "TXT")
				{
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmPageObj.csv"; Session["ExportStr"] = (Config.ExportExcelCSV ? "sep=\t\n": "") + sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			if (ValidPage() && (cAdmPageObjGrid.EditIndex < 0 || UpdateGridRow(cAdmPageObjGrid, new CommandEventArgs("Save", ""))))
			{
				...
				if (ftr != string.Empty) {dv.RowFilter = ftr;}
				if (!string.IsNullOrEmpty(cAdmPageObj1001List.SelectedValue) && cPageObjId1277.Text != cAdmPageObj1001List.SelectedValue)
				{
				...
				if (string.IsNullOrEmpty(cAdmPageObj1001List.SelectedValue))	// Add
				{
					if (ds != null)
					{
						pid = (new AdminSystem()).AddData(1001,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc,noTrans,commandTimeOut);
					}
					...
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(1001,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc,noTrans,commandTimeOut)) {bValid7 = true;}
					if (bValid7)
					{
						cAdmPageObj1001List.ClearSearch(); Session.Remove(KEY_dtAdmPageObj1001List);
		...
		private bool UpdateGridRow(object sender, CommandEventArgs e)
		{
			...
			{
				cAdmPageObjGrid_OnItemUpdating(sender, new ListViewUpdateEventArgs(cAdmPageObjGrid.EditIndex));
			}
			return true;
		}

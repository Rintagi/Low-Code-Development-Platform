...
namespace RO.Web
{
	public partial class AdmSredTimeModule : RO.Web.ModuleBase
	{

		private const string KEY_lastAddedRow = "Cache:lastAddedRow3_123";
		private const string KEY_lastSortOrd = "Cache:lastSortOrd3_123";
		private const string KEY_lastSortImg = "Cache:lastSortImg3_123";
		private const string KEY_lastSortCol = "Cache:lastSortCol3_123";
		private const string KEY_lastSortExp = "Cache:lastSortExp3_123";
		private const string KEY_lastSortUrl = "Cache:lastSortUrl3_123";
		private const string KEY_lastSortTog = "Cache:lastSortTog3_123";
		private const string KEY_lastImpPwdOvride = "Cache:lastImpPwdOvride3_123";
		private const string KEY_cntImpPwdOvride = "Cache:cntImpPwdOvride3_123";
		private const string KEY_currPageIndex = "Cache:currPageIndex3_123";
		private const string KEY_bHiImpVisible = "Cache:bHiImpVisible3_123";
		private const string KEY_bShImpVisible = "Cache:bShImpVisible3_123";
		private const string KEY_dtAdmSredTimeGrid = "Cache:dtAdmSredTimeGrid";
		private const string KEY_scrImport = "Cache:scrImport3_123";
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_123";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_123";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_123";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_123";
		private const string KEY_dtLabel = "Cache:dtLabel3_123";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_123";
		private const string KEY_dtScrCri = "Cache:dtScrCri3_123";
		private const string KEY_dsScrCriVal = "Cache:dsScrCriVal3_123";

		private const string KEY_dtMemberId272 = "Cache:dtMemberId272";
		private const string KEY_dtEnteredBy272 = "Cache:dtEnteredBy272";
		private const string KEY_dtModifiedBy272 = "Cache:dtModifiedBy272";

		private const string KEY_dtSystems = "Cache:dtSystems3_123";
		private const string KEY_sysConnectionString = "Cache:sysConnectionString3_123";
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_123";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_123";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_123";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				DataTable dtMenuAccess = (new MenuSystem()).GetMenu(base.LUser.CultureId, 3, base.LImpr, LcSysConnString, LcAppPw,123, null, null);
				...
				Session.Remove(KEY_dtAdmSredTimeGrid);
				...
				Session.Remove(KEY_dtModifiedBy272);
				SetButtonHlp();
				GetSystems();
				SetColumnAuthority();
				GetGlobalFilter();
				...
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 123, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmSredTime123List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=123&typ=N&sys=3','','',''); return false;"; }
		...
		private void InitializeComponent()
		{
			...
			if (LcSysConnString == null)
			{
				...
				{
					DataTable dt;
					dt = (new AdminSystem()).GetAuthRow(123,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
					...
					{
						cAdmSredTimeGrid.PreRender += new EventHandler(cAdmSredTimeGrid_OnPreRender);
					}
				}
		...
		private void SetSystem(byte SystemId)
		{
			...
			try
			{
				...
				if ((Config.DeployType == "DEV" || row["dbAppDatabase"].ToString() == base.CPrj.EntityCode + "View") && !(base.CPrj.EntityCode != "RO" && row["SysProgram"].ToString() == "Y") && (new AdminSystem()).IsRegenNeeded(string.Empty,123,0,0,LcSysConnString,LcAppPw))
				{
					(new GenScreensSystem()).CreateProgram(123, "Time Tracking for SR&ED", row["dbAppDatabase"].ToString(), base.CPrj, base.CSrc, base.CTar, LcAppConnString, LcAppPw);
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(123,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(123,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void SetClientRule(ListViewDataItem ee, bool isEditItem)
		{
			...
			if (dvRul.Count > 0)
			{
				...
				{
					if (ee == null || drv["ScriptEvent"].ToString().Substring(0,2).ToLower() != "on" || (cAdmSredTimeGrid.EditIndex > -1 && GetDataItemIndex(cAdmSredTimeGrid.EditIndex) == ee.DataItemIndex))
					{
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(123,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(123,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,123,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(123,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(123,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(123,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				CheckAuthentication(false);
				DataView dv = null;
				try {dv = ((DataTable)Session[KEY_dtAdmSredTimeGrid]).DefaultView;}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(123,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					{
						if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["SredTimeId272"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["MemberId272"].ToString() + (char)9 + drv["MemberId272Text"].ToString() + (char)9);}
						if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmShortDate(drv["SredTimeDt272"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmMoney(drv["HourSpent272"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append(drv["EnteredBy272"].ToString() + (char)9 + drv["EnteredBy272Text"].ToString() + (char)9);}
						if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmShortDateTimeUTC(drv["EnteredOn272"].ToString(),base.LUser.Culture,CurrTimeZoneInfo()) + (char)9);}
						if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append(drv["ModifiedBy272"].ToString() + (char)9 + drv["ModifiedBy272Text"].ToString() + (char)9);}
						if (dtAu.Rows[7]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmShortDateTimeUTC(drv["ModifiedOn272"].ToString(),base.LUser.Culture,CurrTimeZoneInfo()) + (char)9);}
						if (dtAu.Rows[8]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["Accomplished272"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						sb.Append(Environment.NewLine);
					}
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmSredTime.csv"; Session["ExportStr"] = (Config.ExportExcelCSV ? "sep=\t\n": "") + sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmSredTime.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private string GetRtfGrid(DataView dv)
		{
			...
			try
			{
				...
				foreach (DataRowView drv in dv)
				{
					...
					sb.Append("\r\n");
					sb.Append(@"{");
					if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["SredTimeId272"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["MemberId272Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmShortDate(drv["SredTimeDt272"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmMoney(drv["HourSpent272"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append(drv["EnteredBy272Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmShortDateTimeUTC(drv["EnteredOn272"].ToString(),base.LUser.Culture,CurrTimeZoneInfo()) + @"\cell ");}
					if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append(drv["ModifiedBy272Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[7]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmShortDateTimeUTC(drv["ModifiedOn272"].ToString(),base.LUser.Culture,CurrTimeZoneInfo()) + @"\cell ");}
					if (dtAu.Rows[8]["ColExport"].ToString() == "Y") {sb.Append(drv["Accomplished272"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					sb.Append(@"}");
					sb.Append("\r\n");
					sb.Append(@"\pard \ql \li0\ri0\widctlpar\intbl\aspalpha\aspnum\adjustright\rin0\lin0 {");
					sb.Append(@"\insrsid2981395 \trowd \irow0\irowband0\lastrow \ts15\trgaph108\trleft-108\trbrdrt\brdrs\brdrw10 \trbrdrl\brdrs\brdrw10 \trbrdrb\brdrs\brdrw10 \trbrdrr\brdrs\brdrw10 \trbrdrh\brdrs\brdrw10 \trbrdrv\brdrs\brdrw10 ");
		...
		private void GetPageInfo()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastPageInfo(123, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 123, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			...
			UpdCriteria(true);
			PopAdmSredTime123List(sender, e, false, null);
			}
		}
			...
			try
			{
				foreach (DataRowView drv in dvCri)
				{
					...
					if (drv["DisplayName"].ToString() == "ComboBox")
					{
						...
						(new AdminSystem()).MkGetScreenIn("123", drv["ScreenCriId"].ToString(), "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), LcAppDb, LcDesDb, drv["MultiDesignDb"].ToString(), LcSysConnString, LcAppPw);
						DataView dv = new DataView((new AdminSystem()).GetScreenIn("123", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), (new AdminSystem()).CountScrCri(drv["ScreenCriId"].ToString(), drv["MultiDesignDb"].ToString(), drv["MultiDesignDb"].ToString() == "N" ? LcSysConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)), drv["RequiredValid"].ToString(), 0, string.Empty, drv["DisplayMode"].ToString() != "AutoListBox", val, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)));
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("123", LcSysConnString, LcAppPw);
				}
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmSredTime123" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmSredTime123" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			foreach (DataRowView drv in dvCri)
			{
			    if (drv["DisplayName"].ToString() == "ListBox")
			    {
					...
					if (cListBox != null)
					{
						...
						int TotalChoiceCnt = new DataView((new AdminSystem()).GetScreenIn("123", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), CriCnt, drv["RequiredValid"].ToString(), 0, string.Empty, true, string.Empty, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId))).Count;
						string selectedValues = string.Join(",", cListBox.Items.Cast<ListItem>().Where(x => x.Selected).Select(x => "'" + x.Value + "'").ToArray());
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("123", "AdmSredTime123", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		protected void cbMemberId272(object sender, System.EventArgs e)
		{
			SetMemberId272((RoboCoder.WebControls.ComboBox)sender,string.Empty,cAdmSredTimeGrid.EditItem);
		}

		private void SetMemberId272(RoboCoder.WebControls.ComboBox ddl, string keyId, ListViewItem li)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(123,"GetDdlMemberId3S3092",true,false,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
					ddl.DataSource = dv; Session[KEY_dtMemberId272] = dv.Table;
				    try { ddl.SelectByValue(keyId,string.Empty,true); } catch { try { ddl.SelectedIndex = 0; } catch { } }
				}
			}
		}

		private void SetEnteredBy272(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtEnteredBy272];
			...
			if (ddl != null)
			{
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(123,"GetDdlEnteredBy3S3096",true,bAll,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "EnteredBy272 is not NULL"; }
					dv.RowFilter = ss;
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(123,"GetDdlEnteredBy3S3096",true,true,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtEnteredBy272] = dv.Table;
				}
			}
		}

		private void SetModifiedBy272(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtModifiedBy272];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (ddl != null)
			{
				string ss = string.Empty;
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(123,"GetDdlModifiedBy3S3098",true,bAll,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "ModifiedBy272 is not NULL"; }
					dv.RowFilter = ss;
					ddl.DataSource = dv; ddl.DataBind();
					li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					else if (!bFirst)
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(123,"GetDdlModifiedBy3S3098",true,true,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtModifiedBy272] = dv.Table;
				}
			}
		}

		private void PopAdmSredTime123List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			DataTable dt = null;
			CheckAuthentication(false);
			...
			try
			{
				try {dt = (new AdminSystem()).GetLis(123,"GetLisAdmSredTime123",false,"Y",0,null,null,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				catch {dt = (new AdminSystem()).GetLis(123,"GetLisAdmSredTime123",false,"N",0,null,null,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				Session[KEY_dtAdmSredTimeGrid] = dt;
				cAdmSredTimeGridDataPager.PageSize = Int16.Parse(cPgSize.Text); GotoPage(0);
			    if (Session[KEY_lastSortCol] != null)
			    {
					cAdmSredTimeGrid_OnSorting(sender, new ListViewSortEventArgs((string)Session[KEY_lastSortExp], SortDirection.Ascending));
			    }
				else
			    {
					cAdmSredTimeGrid_DataBind(dt.DefaultView);
			    }
		...
		public void cFindButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dv != null)
			{
				if (cAdmSredTimeGrid.EditIndex < 0 || UpdateGridRow(cAdmSredTimeGrid, new CommandEventArgs("Save", "")))
				{
					...
					GotoPage(0); cAdmSredTimeGrid_DataBind(dv);
					if (GetCurrPageIndex() != (int)Session[KEY_currPageIndex] && (int)Session[KEY_currPageIndex] < GetTotalPages()) { GotoPage((int)Session[KEY_currPageIndex]); cAdmSredTimeGrid_DataBind(dv);}
		...
		public void cInsRowButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dt != null && (ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")))
			{
				...
				Session[KEY_lastAddedRow] = 0; Session[KEY_dtAdmSredTimeGrid] = dt; Session[KEY_currPageIndex] = 0; GotoPage(0);
				cAdmSredTimeGrid_DataBind(dt.DefaultView);
				cAdmSredTimeGrid_OnItemEditing(cAdmSredTimeGrid, new ListViewEditEventArgs(0));
		...
		public void cPgSizeButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cAdmSredTimeGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				...
				cAdmSredTimeGridDataPager.PageSize = int.Parse(cPgSize.Text); GotoPage(0);
				if (dt.Rows.Count <= 0 || (GetAuthRow().Rows[0]["AllowAdd"].ToString() == "Y" && dt.Rows.Count == 1)) {cAdmSredTimeGrid_DataBind(dt.DefaultView);} else {cFindButton_Click(sender, e);}
				try
				{
					(new AdminSystem()).UpdLastPageInfo(123, base.LUser.UsrId, cPgSize.Text, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		}
		...
		public void cShowImpButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cSchemaImage.Visible)
			{
				try
				{
					Session["ImportSchema"] = (new AdminSystem()).GetSchemaScrImp(123,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (cSchemaImage.Attributes["OnClick"] == null || cSchemaImage.Attributes["OnClick"].IndexOf("ImportSchema.aspx") < 0) {cSchemaImage.Attributes["OnClick"] += "SearchLink('ImportSchema.aspx?scm=S&key=123&csy=3','','',''); return false;";}
			}
			// *** System Button Click (after) Web Rule starts here *** //
		}
		...
		protected void ScrImport(bool bClear)
		{
			...
			DataTable dtAdmSredTimeGrid = (DataTable)Session[KEY_dtAdmSredTimeGrid];
			if (dti != null && dtAdmSredTimeGrid != null)
			{
				if (bClear)
				{
					dtAdmSredTimeGrid.Rows.Clear();
				}
				cFind.Text = string.Empty;
				int iExist = dtAdmSredTimeGrid.Rows.Count;
				...
				foreach (DataRowView drv in dti.DefaultView)
				{
					...
					{
						DataTable dtd;
						dtd = jj==iExist ? null : (DataTable)Session[KEY_dtMemberId272];
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(123,"GetDdlMemberId3S3092",true,true,0,string.Empty,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
							Session[KEY_dtMemberId272] = dtd;
						}
						ScrImportDdl(dtd.DefaultView, drv, true, "SredTimeId", "MemberId272", "MemberId272Text", 50, "", false);
						dtd = (DataTable)Session[KEY_dtEnteredBy272];
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(123,"GetDdlEnteredBy3S3096",true,true,0,string.Empty,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
							Session[KEY_dtEnteredBy272] = dtd;
						}
						ScrImportDdl(dtd.DefaultView, drv, false, "SredTimeId", "EnteredBy272", "EnteredBy272Text", 50, "", true);
						dtd = (DataTable)Session[KEY_dtModifiedBy272];
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(123,"GetDdlModifiedBy3S3098",true,true,0,string.Empty,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
							Session[KEY_dtModifiedBy272] = dtd;
						}
						ScrImportDdl(dtd.DefaultView, drv, false, "SredTimeId", "ModifiedBy272", "ModifiedBy272Text", 50, "", true);
					}
					jj = jj + 1;
				}
				iRow = dti.DefaultView.Count - iExist;
				if (!dti.Columns.Contains("_NewRow")) { dti.Columns.Add("_NewRow"); }
				Session[KEY_dtAdmSredTimeGrid] = dti;
				cAdmSredTimeGrid_DataBind(dti.DefaultView);
		...
		public void cImportButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cFNameO.Text != string.Empty && cWorkSheet.Items.Count > 0 && cWorkSheet.SelectedItem.Text != string.Empty && cStartRow.Text != string.Empty)
			{
				...
				try
				{
					if (cAdmSredTimeGrid.EditIndex < 0 || UpdateGridRow(cAdmSredTimeGrid, new CommandEventArgs("Save", "")))
					{
						Session[KEY_scrImport] = ImportFile(dtAdmSredTimeGrid.Copy(),cFNameO.Text,cWorkSheet.SelectedItem.Text,cStartRow.Text,Config.PathTmpImport + cFName.Text);
					}
		...
		private DataTable ImportFile(DataTable dt, string fileName, string workSheet, string startRow, string fileFullName)
		{
			try
			{
				...
				{
					if (rows[iRow][1].ToString() == string.Empty && rows[iRow][2].ToString() == string.Empty && rows[iRow][3].ToString() == string.Empty && rows[iRow][4].ToString() == string.Empty && rows[iRow][11].ToString() == string.Empty) {idel = idel + 1;}
					else
					{
		...
		private void GotoPage(int pageNo)
		{
			if (pageNo >= 0 && pageNo < GetTotalPages())
			{
				if (cAdmSredTimeGrid.EditIndex < 0 || UpdateGridRow(cAdmSredTimeGrid, new CommandEventArgs("Save", "")))
				{
					try { cAdmSredTimeGridDataPager.SetPageProperties(cAdmSredTimeGridDataPager.PageSize * pageNo, cAdmSredTimeGridDataPager.MaximumRows, false); cAdmSredTimeGrid_DataBind(null); }
					catch
					{
						try { cAdmSredTimeGridDataPager.SetPageProperties(0, cAdmSredTimeGridDataPager.MaximumRows, false); cAdmSredTimeGrid_DataBind(null); } catch {}
					}
				}
			}
		}

		protected void cAdmSredTimeGrid_OnItemCommand(object sender, ListViewCommandEventArgs e)
		{
		}

		private void cAdmSredTimeGrid_DataBind(DataView dvAdmSredTimeGrid)
		{
			if (dvAdmSredTimeGrid == null)
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmSredTimeGrid];
				dvAdmSredTimeGrid = dt != null ? dt.DefaultView : null;
			}
			if (dvAdmSredTimeGrid != null)
			{
				LcAuth = GetAuthCol().AsEnumerable().ToDictionary<DataRow,string>(dr=>dr["ColName"].ToString());
				cAdmSredTimeGrid.DataSource = dvAdmSredTimeGrid;
				cAdmSredTimeGrid.DataBind();
		...
		protected void cAdmSredTimeGrid_OnSorting(object sender, ListViewSortEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmSredTimeGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null)
			{
				if (cAdmSredTimeGrid.EditIndex < 0 || UpdateGridRow(cAdmSredTimeGrid, new CommandEventArgs("Save", "")))
				{
					...
					Session[KEY_dtAdmSredTimeGrid] = dt;
					cAdmSredTimeGrid_DataBind(dv);
				}
			}
		}

		protected void cAdmSredTimeGrid_OnPagePropertiesChanging(object sender, PagePropertiesChangingEventArgs e)
		{
		    cAdmSredTimeGridDataPager.SetPageProperties(e.StartRowIndex, e.MaximumRows, false);
		    DataTable dt = (DataTable)Session[KEY_dtAdmSredTimeGrid];
		    DataView dv = dt != null ? dt.DefaultView : null;
			cAdmSredTimeGrid_DataBind(dv); cAdmSredTimeGrid.EditIndex = -1;
		}

		private void GridChkPgDirty(ListViewItem lvi)
		{
				WebControl cc = null;
				cc = ((WebControl)lvi.FindControl("cMemberId272"));
				...
				cc = ((WebControl)lvi.FindControl("cSredTimeDt272"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cHourSpent272"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cEnteredBy272"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cEnteredOn272"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cModifiedBy272"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cModifiedOn272"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cAccomplished272"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
		}

		protected void cAdmSredTimeGrid_OnItemDataBound(object sender, ListViewItemEventArgs e)
		{
			// *** GridItemDataBound (before) Web Rule End *** //
			DataTable dt = (DataTable)Session[KEY_dtAdmSredTimeGrid];
			bool isEditItem = false;
			DataView dvAdmSredTimeGrid = dt != null ? dt.DefaultView : null;
			if (cAdmSredTimeGrid.EditIndex > -1 && GetDataItemIndex(cAdmSredTimeGrid.EditIndex) == e.Item.DataItemIndex)
			{
				isEditItem = true;
				base.SetGridEnabled(e.Item, GetAuthCol(), GetLabel(), 0);
				SetMemberId272((RoboCoder.WebControls.ComboBox)e.Item.FindControl("cMemberId272"),dvAdmSredTimeGrid[e.Item.DataItemIndex]["MemberId272"].ToString(), e.Item);
				SetEnteredBy272((DropDownList)e.Item.FindControl("cEnteredBy272"),dvAdmSredTimeGrid[e.Item.DataItemIndex]["EnteredBy272"].ToString());
				SetModifiedBy272((DropDownList)e.Item.FindControl("cModifiedBy272"),dvAdmSredTimeGrid[e.Item.DataItemIndex]["ModifiedBy272"].ToString());
			...
			if (dtAuthRow != null)
			{
				...
				{
					cc = e.Item.FindControl("cAdmSredTimeGridDelete"); if (cc != null) { cc.Visible = false; }
				}
				if (!AllowRowEdit(GetAuthRow(),dvAdmSredTimeGrid[e.Item.DataItemIndex]["SredTimeId272"].ToString()) || (Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N"))
				{
					cc = e.Item.FindControl("cAdmSredTimeGridEdit"); if (cc != null) { cc.Visible = false; }
				}
				else
				{
			        HtmlTableRow tr = e.Item.FindControl("cAdmSredTimeGridRow") as HtmlTableRow;
			        LinkButton lb = e.Item.FindControl("cAdmSredTimeGridEdit") as LinkButton;
			...
			//WebRule: Default member to chosen criteria
            if (cAdmSredTimeGrid.EditIndex > -1 && GetDataItemIndex(cAdmSredTimeGrid.EditIndex) == e.Item.DataItemIndex)
            {
		...
		protected void cSredTimeId272hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "0") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "0"; Session[KEY_lastSortExp] = "SredTimeId272";Session[KEY_lastSortImg] = "cSredTimeId272hi";
			cAdmSredTimeGrid_OnSorting(sender, new ListViewSortEventArgs("SredTimeId272", SortDirection.Ascending));
		}

		protected void cMemberId272hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "1") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "1"; Session[KEY_lastSortExp] = "MemberId272Text";Session[KEY_lastSortImg] = "cMemberId272hi";
			cAdmSredTimeGrid_OnSorting(sender, new ListViewSortEventArgs("MemberId272Text", SortDirection.Ascending));
		}

		protected void cSredTimeDt272hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "2") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "2"; Session[KEY_lastSortExp] = "SredTimeDt272";Session[KEY_lastSortImg] = "cSredTimeDt272hi";
			cAdmSredTimeGrid_OnSorting(sender, new ListViewSortEventArgs("SredTimeDt272", SortDirection.Ascending));
		}

		protected void cHourSpent272hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "3") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "3"; Session[KEY_lastSortExp] = "HourSpent272";Session[KEY_lastSortImg] = "cHourSpent272hi";
			cAdmSredTimeGrid_OnSorting(sender, new ListViewSortEventArgs("HourSpent272", SortDirection.Ascending));
		}

		protected void cEnteredBy272hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "4") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "4"; Session[KEY_lastSortExp] = "EnteredBy272Text";Session[KEY_lastSortImg] = "cEnteredBy272hi";
			cAdmSredTimeGrid_OnSorting(sender, new ListViewSortEventArgs("EnteredBy272Text", SortDirection.Ascending));
		}

		protected void cEnteredOn272hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "5") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "5"; Session[KEY_lastSortExp] = "EnteredOn272";Session[KEY_lastSortImg] = "cEnteredOn272hi";
			cAdmSredTimeGrid_OnSorting(sender, new ListViewSortEventArgs("EnteredOn272", SortDirection.Ascending));
		}

		protected void cModifiedBy272hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "6") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "6"; Session[KEY_lastSortExp] = "ModifiedBy272Text";Session[KEY_lastSortImg] = "cModifiedBy272hi";
			cAdmSredTimeGrid_OnSorting(sender, new ListViewSortEventArgs("ModifiedBy272Text", SortDirection.Ascending));
		}

		protected void cModifiedOn272hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "7") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "7"; Session[KEY_lastSortExp] = "ModifiedOn272";Session[KEY_lastSortImg] = "cModifiedOn272hi";
			cAdmSredTimeGrid_OnSorting(sender, new ListViewSortEventArgs("ModifiedOn272", SortDirection.Ascending));
		}

		protected void cAccomplished272hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "8") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "8"; Session[KEY_lastSortExp] = "Accomplished272";Session[KEY_lastSortImg] = "cAccomplished272hi";
			cAdmSredTimeGrid_OnSorting(sender, new ListViewSortEventArgs("Accomplished272", SortDirection.Ascending));
		}
		...
		protected void cAdmSredTimeGrid_OnItemEditing(object sender, ListViewEditEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmSredTimeGrid];
			if ((ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")) && dt != null && dt.DefaultView.Count > GetDataItemIndex(e.NewEditIndex))
			{
				cAdmSredTimeGrid.EditIndex = e.NewEditIndex;
				cAdmSredTimeGrid_DataBind(null);
				Control wc = null;
				string idx = Page.Request["__EVENTARGUMENT"];
				if (!string.IsNullOrEmpty(idx))
				{
				    Control ctrlToFocus = cAdmSredTimeGrid.EditItem.FindControl("c" + idx);
				    if (((WebControl)ctrlToFocus).Enabled) { wc = ctrlToFocus; }
				}
				if (wc == null) { wc = FindEditableControl(cAdmSredTimeGrid.EditItem); }
				if (wc != null)
             {
             ...
             MkMobileLabel(cAdmSredTimeGrid.EditItem);
		    }
		    else { e.Cancel = true; }
		}
		...
		{
		    Label ml = null;
		    ml = lvi.FindControl("cSredTimeId272ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(0); }
		    ml = lvi.FindControl("cMemberId272ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(1); }
		    ml = lvi.FindControl("cSredTimeDt272ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(2); }
		    ml = lvi.FindControl("cHourSpent272ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(3); }
		    ml = lvi.FindControl("cEnteredBy272ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(4); }
		    ml = lvi.FindControl("cEnteredOn272ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(5); }
		    ml = lvi.FindControl("cModifiedBy272ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(6); }
		    ml = lvi.FindControl("cModifiedOn272ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(7); }
		    ml = lvi.FindControl("cAccomplished272ml") as Label;
		...
		protected void GridFill(ListViewItem lvi, DataTable dt, DataRow dr, bool bInsert)
		{
			...
			cbb = (RoboCoder.WebControls.ComboBox)lvi.FindControl("cMemberId272");
			if (cbb != null)
			{
				if (cbb.SelectedValue != string.Empty)
				{
					dr["MemberId272"] = cbb.SelectedValue;
					dr["MemberId272Text"] = cbb.SelectedText;
				}
				else
				{
					dr["MemberId272"] = System.DBNull.Value;
					dr["MemberId272Text"] = System.DBNull.Value;
				}
			}
			TextBox tb = null;
			tb = (TextBox)lvi.FindControl("cSredTimeDt272");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["SredTimeDt272"] = base.ToIntDateTime(tb.Text,false,false);} else {dr["SredTimeDt272"] = System.DBNull.Value;}
			}
			tb = (TextBox)lvi.FindControl("cHourSpent272");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["HourSpent272"] = tb.Text;} else {dr["HourSpent272"] = System.DBNull.Value;}
			}
			DropDownList ddl = null;
			ddl = (DropDownList)lvi.FindControl("cEnteredBy272");
			if (ddl != null)
			{
				if (ddl.SelectedIndex >= 0 && ddl.SelectedValue.ToString() != string.Empty)
				{
					dr["EnteredBy272"] = ddl.SelectedValue;
					dr["EnteredBy272Text"] = ddl.SelectedItem.Text;
				}
				else
				{
					dr["EnteredBy272"] = System.DBNull.Value;
					dr["EnteredBy272Text"] = System.DBNull.Value;
				}
			}
			tb = (TextBox)lvi.FindControl("cEnteredOn272");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["EnteredOn272"] = base.ToIntDateTime(tb.Text,true,false);} else {dr["EnteredOn272"] = System.DBNull.Value;}
			}
			ddl = (DropDownList)lvi.FindControl("cModifiedBy272");
			if (ddl != null)
			{
				if (ddl.SelectedIndex >= 0 && ddl.SelectedValue.ToString() != string.Empty)
				{
					dr["ModifiedBy272"] = ddl.SelectedValue;
					dr["ModifiedBy272Text"] = ddl.SelectedItem.Text;
				}
				else
				{
					dr["ModifiedBy272"] = System.DBNull.Value;
					dr["ModifiedBy272Text"] = System.DBNull.Value;
				}
			}
			tb = (TextBox)lvi.FindControl("cAccomplished272");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["Accomplished272"] = tb.Text;} else {dr["Accomplished272"] = System.DBNull.Value;}
			}
		    if (bInsert) { dt.Rows.InsertAt(dr, 0); }
			Session[KEY_dtAdmSredTimeGrid] = dt;
			cAdmSredTimeGrid.EditIndex = -1;
			cAdmSredTimeGrid_DataBind(dt.DefaultView);
		}

		protected void cAdmSredTimeGrid_OnItemUpdating(object sender, ListViewUpdateEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmSredTimeGrid];
			if (dt != null && dt.Rows.Count > 0)
			{
				DataRow dr = dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row;
			    GridFill(cAdmSredTimeGrid.EditItem, dt, dr, false);
			    dr["_NewRow"] = "N";
			    if (dt.Columns.Contains("_SortOrder")) dr["_SortOrder"] = null;
			}
		}

		protected void cAdmSredTimeGrid_OnItemCanceling(object sender, ListViewCancelEventArgs e)
		{
			PanelTop.Update();
			DataTable dt = (DataTable)Session[KEY_dtAdmSredTimeGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			...
			{
				cAdmSredTimeGrid_OnItemDeleting(sender, new ListViewDeleteEventArgs(e.ItemIndex));
			}
			if (dt.Columns.Contains("_SortOrder")) dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row["_SortOrder"] = null;
			cAdmSredTimeGrid.EditIndex = -1; cAdmSredTimeGrid_DataBind(null);
		}
		...
		protected void cDeleteAllButton_Click(object sender, System.EventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmSredTimeGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null && (cAdmSredTimeGrid.EditIndex < 0 || UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				GotoPage(0);
				while (dv.Count > 0) {dv.Delete(0);}
				Session[KEY_dtAdmSredTimeGrid] = dt;
				cAdmSredTimeGrid.EditIndex = -1; cAdmSredTimeGrid_DataBind(dv);
			}
		}

		protected void cAdmSredTimeGrid_OnItemDeleting(object sender, ListViewDeleteEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmSredTimeGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			DataRow dr = dv != null ? dv[GetDataItemIndex(e.ItemIndex)].Row : null;
			if (dv != null && (cAdmSredTimeGrid.EditIndex == e.ItemIndex || UpdateGridRow(sender, new CommandEventArgs("Save",""))))
			{
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmSredTimeGrid); PopAdmSredTime123List(sender, e, false, null);
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			// *** System Button Click (Before) Web Rule starts here *** //
			if (ValidPage() && UpdateGridRow(cAdmSredTimeGrid, new CommandEventArgs("Save", "")))
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmSredTimeGrid];
				DataView dv = dt != null ? dt.DefaultView : null;
				string ftr = string.Empty;
				if (dv.RowFilter != string.Empty) { ftr = dv.RowFilter; dv.RowFilter = string.Empty; }
				AdmSredTime123 ds = PrepAdmSredTimeData(dv,false);
				if (ftr != string.Empty) {dv.RowFilter = ftr;}
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(123,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc,noTrans,commandTimeOut)) {bValid7 = true;}
					if (bValid7)
					{
						Session[KEY_currPageIndex] = GetCurrPageIndex();
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmSredTime123List(sender, e, false, null);
		...
		private AdmSredTime123 PrepAdmSredTimeData(DataView dv, bool bAdd)
		{
			...
			if (dv != null)
			{
				...
				ds.Tables["AdmSredTimeDef"].Rows.Add(MakeDisRow(ds.Tables["AdmSredTimeDef"].NewRow()));
					...
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmSredTimeUpd"].Rows.Add(MakeColRow(ds.Tables["AdmSredTimeUpd"].NewRow(), drv, false));
					}
					dv.RowStateFilter = DataViewRowState.Added;
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmSredTimeAdd"].Rows.Add(MakeColRow(ds.Tables["AdmSredTimeAdd"].NewRow(), drv, true));
					}
					dv.RowStateFilter = DataViewRowState.Deleted;
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmSredTimeDel"].Rows.Add(MakeColRow(ds.Tables["AdmSredTimeDel"].NewRow(), drv, false));
					}
		...
		private bool ValidPage()
		{
			...
			DataTable dt = null;
			dt = (DataTable)Session[KEY_dtMemberId272];
			if (dt != null && dt.Rows.Count <= 0)
			{
				bErrNow.Value = "Y"; PreMsgPopup("Value is expected for 'MemberId', please investigate."); return false;
			}
			return true;
		}
		...
		private DataRow MakeColRow(DataRow dr, DataRowView drv, bool bAdd)
		{
			...
			if (dtAuth != null)
			{
				dr["SredTimeId272"] = drv["SredTimeId272"].ToString().Trim();
				...
				dr["EnteredBy272"] = (drv["EnteredBy272"].ToString() == string.Empty && bAdd)  ? (object) base.LUser.UsrId.ToString()	: (object) drv["EnteredBy272"];
		...
		{
			if (cAdmSredTimeGrid.EditIndex > -1)
			{
				cAdmSredTimeGrid_OnItemUpdating(sender, new ListViewUpdateEventArgs(cAdmSredTimeGrid.EditIndex));
			}
			return true;
		}

		protected void cAdmSredTimeGrid_OnPreRender(object sender, System.EventArgs e)
		{
			System.Web.UI.WebControls.Image hi = null;
			hi = (System.Web.UI.WebControls.Image)cAdmSredTimeGrid.FindControl("cSredTimeId272hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmSredTimeGrid.FindControl("cMemberId272hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmSredTimeGrid.FindControl("cSredTimeDt272hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmSredTimeGrid.FindControl("cHourSpent272hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmSredTimeGrid.FindControl("cEnteredBy272hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmSredTimeGrid.FindControl("cEnteredOn272hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmSredTimeGrid.FindControl("cModifiedBy272hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmSredTimeGrid.FindControl("cModifiedOn272hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmSredTimeGrid.FindControl("cAccomplished272hi"); if (hi != null) { hi.Visible = false; }
			if (Session[KEY_lastSortImg] != null)
			{
				hi = (System.Web.UI.WebControls.Image)cAdmSredTimeGrid.FindControl((string)Session[KEY_lastSortImg]);
				if (hi != null) { hi.ImageUrl = Utils.AddTilde((string)Session[KEY_lastSortUrl]); hi.Visible = true; }
			}
			IgnoreHeaderConfirm((LinkButton)cAdmSredTimeGrid.FindControl("cSredTimeId272hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmSredTimeGrid.FindControl("cMemberId272hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmSredTimeGrid.FindControl("cSredTimeDt272hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmSredTimeGrid.FindControl("cHourSpent272hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmSredTimeGrid.FindControl("cEnteredBy272hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmSredTimeGrid.FindControl("cEnteredOn272hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmSredTimeGrid.FindControl("cModifiedBy272hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmSredTimeGrid.FindControl("cModifiedOn272hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmSredTimeGrid.FindControl("cAccomplished272hl"));

			// Take care of aggregates:
			Label aggr = null;
			aggr = cAdmSredTimeGrid.FindControl("cHourSpent272fl") as Label;
			if (aggr != null) { aggr.Text = RO.Common3.Utils.fmMoney(GetAggr("HourSpent272","S"),base.LUser.Culture); }
		}

		private string GetButtonId(ListViewItem lvi)
		{
			string ButtonID = String.Empty;
			Control c = lvi.FindControl("cAdmSredTimeGridEdit");
		...
		{
		    return cAdmSredTimeGridDataPager.StartRowIndex / cAdmSredTimeGridDataPager.PageSize;
		}

		private int GetTotalPages()
		{
		    return (int)Math.Ceiling((double)cAdmSredTimeGridDataPager.TotalRowCount / cAdmSredTimeGridDataPager.PageSize);
		}

		private int GetDataItemIndex(int editIndex)
		{
		    return cAdmSredTimeGridDataPager.StartRowIndex + editIndex;
		}

		protected void cAdmSredTimeGrid_OnLayoutCreated(object sender, EventArgs e)
		{
		    // Header:
		    LinkButton lb = null;
		    lb = cAdmSredTimeGrid.FindControl("cSredTimeId272hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(0); lb.ToolTip = ColumnToolTip(0); lb.Parent.Visible = GridColumnVisible(0); }
		    lb = cAdmSredTimeGrid.FindControl("cMemberId272hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(1); lb.ToolTip = ColumnToolTip(1); lb.Parent.Visible = GridColumnVisible(1); }
		    lb = cAdmSredTimeGrid.FindControl("cSredTimeDt272hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(2); lb.ToolTip = ColumnToolTip(2); lb.Parent.Visible = GridColumnVisible(2); }
		    lb = cAdmSredTimeGrid.FindControl("cHourSpent272hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(3); lb.ToolTip = ColumnToolTip(3); lb.Parent.Visible = GridColumnVisible(3); }
		    lb = cAdmSredTimeGrid.FindControl("cEnteredBy272hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(4); lb.ToolTip = ColumnToolTip(4); lb.Parent.Visible = GridColumnVisible(4); }
		    lb = cAdmSredTimeGrid.FindControl("cEnteredOn272hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(5); lb.ToolTip = ColumnToolTip(5); lb.Parent.Visible = GridColumnVisible(5); }
		    lb = cAdmSredTimeGrid.FindControl("cModifiedBy272hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(6); lb.ToolTip = ColumnToolTip(6); lb.Parent.Visible = GridColumnVisible(6); }
		    lb = cAdmSredTimeGrid.FindControl("cModifiedOn272hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(7); lb.ToolTip = ColumnToolTip(7); lb.Parent.Visible = GridColumnVisible(7); }
		    lb = cAdmSredTimeGrid.FindControl("cAccomplished272hl") as LinkButton;
			...
			if (dtAuthRow != null)
			{
				...
				if (dr["AllowDel"].ToString() == "N" || (Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N"))
				{
		            lb = cAdmSredTimeGrid.FindControl("cDeleteAllButton") as LinkButton; if (lb != null) { lb.Visible = false; }
				}
			}
		    // footer:
		    Label gc = null;
		    gc = cAdmSredTimeGrid.FindControl("cSredTimeId272fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(0); }
		    gc = cAdmSredTimeGrid.FindControl("cMemberId272fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(1); }
		    gc = cAdmSredTimeGrid.FindControl("cSredTimeDt272fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(2); }
		    gc = cAdmSredTimeGrid.FindControl("cHourSpent272fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(3); }
		    gc = cAdmSredTimeGrid.FindControl("cEnteredBy272fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(4); }
		    gc = cAdmSredTimeGrid.FindControl("cEnteredOn272fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(5); }
		    gc = cAdmSredTimeGrid.FindControl("cModifiedBy272fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(6); }
		    gc = cAdmSredTimeGrid.FindControl("cModifiedOn272fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(7); }
		    gc = cAdmSredTimeGrid.FindControl("cAccomplished272fl") as Label;

...
namespace RO.Web
{
	public partial class AdmMenuModule : RO.Web.ModuleBase
	{
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_35";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_35";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_35";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_35";
		private const string KEY_dtLabel = "Cache:dtLabel3_35";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_35";
		private const string KEY_dtScrCri = "Cache:dtScrCri3_35";
		private const string KEY_dsScrCriVal = "Cache:dsScrCriVal3_35";

		private const string KEY_dtScreenId39 = "Cache:dtScreenId39";
		private const string KEY_dtReportId39 = "Cache:dtReportId39";
		private const string KEY_dtWizardId39 = "Cache:dtWizardId39";
		private const string KEY_dtStaticPgId39 = "Cache:dtStaticPgId39";

		private const string KEY_dtSystems = "Cache:dtSystems3_35";
		private const string KEY_sysConnectionString = "Cache:sysConnectionString3_35";
		private const string KEY_dtAdmMenu35List = "Cache:dtAdmMenu35List";
		private const string KEY_bNewVisible = "Cache:bNewVisible3_35";
		private const string KEY_bNewSaveVisible = "Cache:bNewSaveVisible3_35";
		private const string KEY_bCopyVisible = "Cache:bCopyVisible3_35";
		private const string KEY_bCopySaveVisible = "Cache:bCopySaveVisible3_35";
		private const string KEY_bDeleteVisible = "Cache:bDeleteVisible3_35";
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_35";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_35";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_35";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				DataTable dtMenuAccess = (new MenuSystem()).GetMenu(base.LUser.CultureId, 3, base.LImpr, LcSysConnString, LcAppPw,35, null, null);
				...
				Session.Remove(KEY_dtAdmMenu35List);
				...
				Session.Remove(KEY_dtScreenId39);
				Session.Remove(KEY_dtReportId39);
				Session.Remove(KEY_dtWizardId39);
				Session.Remove(KEY_dtStaticPgId39);
				SetButtonHlp();
				GetSystems();
				SetColumnAuthority();
				GetGlobalFilter();
				...
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 35, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmMenu35List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=35&typ=N&sys=3','','',''); return false;"; }
				cScreenId39Search_Script();
				cReportId39Search_Script();
				cWizardId39Search_Script();
				cStaticPgId39Search_Script();
		...
		private void SetSystem(byte SystemId)
		{
			...
			try
			{
				...
				if ((Config.DeployType == "DEV" || row["dbAppDatabase"].ToString() == base.CPrj.EntityCode + "View") && !(base.CPrj.EntityCode != "RO" && row["SysProgram"].ToString() == "Y") && (new AdminSystem()).IsRegenNeeded(string.Empty,35,0,0,LcSysConnString,LcAppPw))
				{
					(new GenScreensSystem()).CreateProgram(35, "Menu Item", row["dbAppDatabase"].ToString(), base.CPrj, base.CSrc, base.CTar, LcAppConnString, LcAppPw);
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(35,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(35,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(35,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(35,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,35,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(35,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(35,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(35,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		protected DataTable GetAuthRow()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthRow(35,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				try
				{
					try {dv = new DataView((new AdminSystem()).GetExp(35,"GetExpAdmMenu35","Y",(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
					catch {dv = new DataView((new AdminSystem()).GetExp(35,"GetExpAdmMenu35","N",(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(35,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					foreach (DataRowView drv in dv)
					{
						...
						if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["Popup39"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["ParentId39"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["MenuIndex39"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append(drv["ScreenId39"].ToString() + (char)9 + drv["ScreenId39Text"].ToString() + (char)9);}
						if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append(drv["ReportId39"].ToString() + (char)9 + drv["ReportId39Text"].ToString() + (char)9);}
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmMenu.csv"; Session["ExportStr"] = (Config.ExportExcelCSV ? "sep=\t\n": "") + sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmMenu.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private string GetRtfGrid(DataView dv)
		{
			...
			try
			{
				...
				foreach (DataRowView drv in dv)
				{
					...
					if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["Popup39"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["ParentId39"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["MenuIndex39"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append(drv["ScreenId39Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append(drv["ReportId39Text"].ToString() + @"\cell ");}
		...
		public void cScreenId39Search_Click(object sender, System.Web.UI.ImageClickEventArgs e)
		{
			RoboCoder.WebControls.ComboBox cc = cScreenId39; cc.SetTbVisible(); Session["CtrlAdmScreen"] = cc.FocusID;
		}

		public void cScreenId39Search_Script()
		{
				ImageButton ib = cScreenId39Search;
				if (ib != null)
				{
			    	TextBox pp = cMenuId39;
					RoboCoder.WebControls.ComboBox cc = cScreenId39;
		...
		public void cReportId39Search_Click(object sender, System.Web.UI.ImageClickEventArgs e)
		{
			RoboCoder.WebControls.ComboBox cc = cReportId39; cc.SetTbVisible(); Session["CtrlAdmReport"] = cc.FocusID;
		}

		public void cReportId39Search_Script()
		{
				ImageButton ib = cReportId39Search;
				if (ib != null)
				{
			    	TextBox pp = cMenuId39;
					RoboCoder.WebControls.ComboBox cc = cReportId39;
					string ss = "&dsp=ComboBox"; if (cSystem.Visible) {ss = ss + "&sys=" + cSystemId.SelectedValue;} else {ss = ss + "&sys=3";}
					ib.Attributes["onclick"] = "SearchLink('AdmReport.aspx?col=ReportId" + ss + "&typ=N" + "','" + cc.FocusID + "','',''); return false;";
				}
		}

		public void cWizardId39Search_Click(object sender, System.Web.UI.ImageClickEventArgs e)
		{
			RoboCoder.WebControls.ComboBox cc = cWizardId39; cc.SetTbVisible(); Session["CtrlAdmWizardObj"] = cc.FocusID;
		}

		public void cWizardId39Search_Script()
		{
				ImageButton ib = cWizardId39Search;
				if (ib != null)
				{
			    	TextBox pp = cMenuId39;
					RoboCoder.WebControls.ComboBox cc = cWizardId39;
					string ss = "&dsp=ComboBox"; if (cSystem.Visible) {ss = ss + "&sys=" + cSystemId.SelectedValue;} else {ss = ss + "&sys=3";}
					ib.Attributes["onclick"] = "SearchLink('AdmWizardObj.aspx?col=WizardId" + ss + "&typ=N" + "','" + cc.FocusID + "','',''); return false;";
				}
		}

		public void cStaticPgId39Search_Click(object sender, System.Web.UI.ImageClickEventArgs e)
		{
			RoboCoder.WebControls.ComboBox cc = cStaticPgId39; cc.SetTbVisible(); Session["CtrlAdmStaticPg"] = cc.FocusID;
		}

		public void cStaticPgId39Search_Script()
		{
				ImageButton ib = cStaticPgId39Search;
				if (ib != null)
				{
			    	TextBox pp = cMenuId39;
					RoboCoder.WebControls.ComboBox cc = cStaticPgId39;
					string ss = "&dsp=ComboBox"; if (cSystem.Visible) {ss = ss + "&sys=" + cSystemId.SelectedValue;} else {ss = ss + "&sys=3";}
					ib.Attributes["onclick"] = "SearchLink('AdmStaticPg.aspx?col=StaticPgId" + ss + "&typ=N" + "','" + cc.FocusID + "','',''); return false;";
				}
		}

		public void cIconUrl39Tgo_Click(object sender, System.Web.UI.ImageClickEventArgs e)
		{
			if (cIconUrl39.Visible)
			{
				cIconUrl39Div.Visible = true; cIconUrl39.Visible = false;
			}
			else
			{
				cIconUrl39Div.Visible = false; cIconUrl39.Visible = true;
			}
		}
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 35, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			...
			UpdCriteria(true);
			cAdmMenu35List.ClearSearch(); Session.Remove(KEY_dtAdmMenu35List);
			PopAdmMenu35List(sender, e, false, null);
		}
			...
			try
			{
				foreach (DataRowView drv in dvCri)
				{
					...
					if (drv["DisplayName"].ToString() == "ComboBox")
					{
						...
						(new AdminSystem()).MkGetScreenIn("35", drv["ScreenCriId"].ToString(), "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), LcAppDb, LcDesDb, drv["MultiDesignDb"].ToString(), LcSysConnString, LcAppPw);
						DataView dv = new DataView((new AdminSystem()).GetScreenIn("35", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), (new AdminSystem()).CountScrCri(drv["ScreenCriId"].ToString(), drv["MultiDesignDb"].ToString(), drv["MultiDesignDb"].ToString() == "N" ? LcSysConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)), drv["RequiredValid"].ToString(), 0, string.Empty, drv["DisplayMode"].ToString() != "AutoListBox", val, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)));
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("35", LcSysConnString, LcAppPw);
				}
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmMenu35" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmMenu35" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			foreach (DataRowView drv in dvCri)
			{
			    if (drv["DisplayName"].ToString() == "ListBox")
			    {
					...
					if (cListBox != null)
					{
						...
						int TotalChoiceCnt = new DataView((new AdminSystem()).GetScreenIn("35", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), CriCnt, drv["RequiredValid"].ToString(), 0, string.Empty, true, string.Empty, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId))).Count;
						string selectedValues = string.Join(",", cListBox.Items.Cast<ListItem>().Where(x => x.Selected).Select(x => "'" + x.Value + "'").ToArray());
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("35", "AdmMenu35", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		protected void cbScreenId39(object sender, System.EventArgs e)
		{
			SetScreenId39((RoboCoder.WebControls.ComboBox)sender,string.Empty);
		}

		private void SetScreenId39(RoboCoder.WebControls.ComboBox ddl, string keyId)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			context["method"] = "GetDdlScreenId3S435";
			context["addnew"] = "Y";
			context["mKey"] = "ScreenId39";
			context["mVal"] = "ScreenId39Text";
			context["mTip"] = "ScreenId39Text";
			context["mImg"] = "ScreenId39Text";
			context["ssd"] = Request.QueryString["ssd"];
			context["scr"] = "35";
			context["csy"] = "3";
			context["filter"] = Utils.IsInt(cFilterId.SelectedValue)? cFilterId.SelectedValue : "0";
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(35,"GetDdlScreenId3S435",true,false,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
					...
					{
						context["pMKeyColID"] = cAdmMenu35List.ClientID;
						context["pMKeyCol"] = "MenuId";
						string ss = "(MenuId is null";
						if (string.IsNullOrEmpty(cAdmMenu35List.SelectedValue)) {ss = ss + ")";} else {ss = ss + " OR MenuId = " + cAdmMenu35List.SelectedValue + ")";}
						dv.RowFilter = ss;
					}
					ddl.DataSource = dv; Session[KEY_dtScreenId39] = dv.Table;
				    try { ddl.SelectByValue(keyId,string.Empty,true); } catch { try { ddl.SelectedIndex = 0; } catch { } }
				}
			}
		}

		protected void cbReportId39(object sender, System.EventArgs e)
		{
			SetReportId39((RoboCoder.WebControls.ComboBox)sender,string.Empty);
		}

		private void SetReportId39(RoboCoder.WebControls.ComboBox ddl, string keyId)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			context["method"] = "GetDdlReportId3S436";
			context["addnew"] = "Y";
			context["mKey"] = "ReportId39";
			context["mVal"] = "ReportId39Text";
			context["mTip"] = "ReportId39Text";
			context["mImg"] = "ReportId39Text";
			context["ssd"] = Request.QueryString["ssd"];
			context["scr"] = "35";
			context["csy"] = "3";
			context["filter"] = Utils.IsInt(cFilterId.SelectedValue)? cFilterId.SelectedValue : "0";
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(35,"GetDdlReportId3S436",true,false,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
					...
					ddl.DataSource = dv; Session[KEY_dtReportId39] = dv.Table;
				    try { ddl.SelectByValue(keyId,string.Empty,true); } catch { try { ddl.SelectedIndex = 0; } catch { } }
				}
			}
		}

		protected void cbWizardId39(object sender, System.EventArgs e)
		{
			SetWizardId39((RoboCoder.WebControls.ComboBox)sender,string.Empty);
		}

		private void SetWizardId39(RoboCoder.WebControls.ComboBox ddl, string keyId)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			context["method"] = "GetDdlWizardId3S839";
			context["addnew"] = "Y";
			context["mKey"] = "WizardId39";
			context["mVal"] = "WizardId39Text";
			context["mTip"] = "WizardId39Text";
			context["mImg"] = "WizardId39Text";
			context["ssd"] = Request.QueryString["ssd"];
			context["scr"] = "35";
			context["csy"] = "3";
			context["filter"] = Utils.IsInt(cFilterId.SelectedValue)? cFilterId.SelectedValue : "0";
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(35,"GetDdlWizardId3S839",true,false,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
					...
					ddl.DataSource = dv; Session[KEY_dtWizardId39] = dv.Table;
				    try { ddl.SelectByValue(keyId,string.Empty,true); } catch { try { ddl.SelectedIndex = 0; } catch { } }
				}
			}
		}

		protected void cbStaticPgId39(object sender, System.EventArgs e)
		{
			SetStaticPgId39((RoboCoder.WebControls.ComboBox)sender,string.Empty);
		}

		private void SetStaticPgId39(RoboCoder.WebControls.ComboBox ddl, string keyId)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			context["method"] = "GetDdlStaticPgId3S1998";
			context["addnew"] = "Y";
			context["mKey"] = "StaticPgId39";
			context["mVal"] = "StaticPgId39Text";
			context["mTip"] = "StaticPgId39Text";
			context["mImg"] = "StaticPgId39Text";
			context["ssd"] = Request.QueryString["ssd"];
			context["scr"] = "35";
			context["csy"] = "3";
			context["filter"] = Utils.IsInt(cFilterId.SelectedValue)? cFilterId.SelectedValue : "0";
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(35,"GetDdlStaticPgId3S1998",true,false,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
					...
					ddl.DataSource = dv; Session[KEY_dtStaticPgId39] = dv.Table;
				    try { ddl.SelectByValue(keyId,string.Empty,true); } catch { try { ddl.SelectedIndex = 0; } catch { } }
				}
			}
		}

		private DataView GetAdmMenu35List()
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmMenu35List];
			if (dt == null)
			{
				CheckAuthentication(false);
				int filterId = 0;
				...
				try
				{
					try {dt = (new AdminSystem()).GetLis(35,"GetLisAdmMenu35",true,"Y",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
					catch {dt = (new AdminSystem()).GetLis(35,"GetLisAdmMenu35",true,"N",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				}
		...
		private void PopAdmMenu35List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			DataView dv = GetAdmMenu35List();
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			context["isSys"] = "N";
			context["conn"] = KEY_sysConnectionString;
			cAdmMenu35List.AutoCompleteUrl = "AutoComplete.aspx/LisSuggests";
			cAdmMenu35List.DataContext = context;
			if (dv.Table == null) return;
			cAdmMenu35List.DataSource = dv;
			cAdmMenu35List.Visible = true;
			if (cAdmMenu35List.Items.Count <= 0) {cAdmMenu35List.Visible = false; cAdmMenu35List_SelectedIndexChanged(sender,e);}
			else
			{
				if (bPageLoad && !string.IsNullOrEmpty(Request.QueryString["key"]) && bUseCri.Value == string.Empty)
				{
					try {cAdmMenu35List.SelectByValue(Request.QueryString["key"],string.Empty,true);} catch {cAdmMenu35List.Items[0].Selected = true; cAdmMenu35List_SelectedIndexChanged(sender, e);}
				    cScreenSearch.Visible = false; cSystem.Visible = false; cEditButton.Visible = true; cSaveCloseButton.Visible = cSaveButton.Visible;
				}
				else
				{
					if (ID != null) {if (!cAdmMenu35List.SelectByValue(ID,string.Empty,true)) {ID = null;}}
					if (ID == null)
					{
						cAdmMenu35List_SelectedIndexChanged(sender, e);
					}
				}
			}
		}
		...
		private void SetColumnAuthority()
		{
			...
			if (dtAuth != null && dtLabel != null)
			{
				...
				base.SetFoldBehavior(cPopup39, dtAuth.Rows[1], cPopup39P1, cPopup39Label, cPopup39P2, null, dtLabel.Rows[1], null, null, null);
				base.SetFoldBehavior(cParentId39, dtAuth.Rows[2], cParentId39P1, cParentId39Label, cParentId39P2, null, dtLabel.Rows[2], null, null, null);
				base.SetFoldBehavior(cMenuIndex39, dtAuth.Rows[3], cMenuIndex39P1, cMenuIndex39Label, cMenuIndex39P2, null, dtLabel.Rows[3], null, null, cRVMenuIndex39);
				base.SetFoldBehavior(cScreenId39, dtAuth.Rows[4], cScreenId39P1, cScreenId39Label, cScreenId39P2, null, dtLabel.Rows[4], null, null, null);
				SetScreenId39(cScreenId39,string.Empty);
				base.SetFoldBehavior(cReportId39, dtAuth.Rows[5], cReportId39P1, cReportId39Label, cReportId39P2, null, dtLabel.Rows[5], null, null, null);
				SetReportId39(cReportId39,string.Empty);
				base.SetFoldBehavior(cWizardId39, dtAuth.Rows[6], cWizardId39P1, cWizardId39Label, cWizardId39P2, null, dtLabel.Rows[6], null, null, null);
				SetWizardId39(cWizardId39,string.Empty);
				base.SetFoldBehavior(cStaticPgId39, dtAuth.Rows[7], cStaticPgId39P1, cStaticPgId39Label, cStaticPgId39P2, null, dtLabel.Rows[7], null, null, null);
			...
			if ((cMenuIndex39.Attributes["OnChange"] == null || cMenuIndex39.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cMenuIndex39.Visible && !cMenuIndex39.ReadOnly) {cMenuIndex39.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cScreenId39.Attributes["OnChange"] == null || cScreenId39.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cScreenId39.Visible && cScreenId39.Enabled) {cScreenId39.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if (cScreenId39Search.Attributes["OnClick"] == null || cScreenId39Search.Attributes["OnClick"].IndexOf("_bConfirm") < 0) {cScreenId39Search.Attributes["OnClick"] += "document.getElementById('" + bConfirm.ClientID + "').value='N';";}
			if ((cReportId39.Attributes["OnChange"] == null || cReportId39.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cReportId39.Visible && cReportId39.Enabled) {cReportId39.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if (cReportId39Search.Attributes["OnClick"] == null || cReportId39Search.Attributes["OnClick"].IndexOf("_bConfirm") < 0) {cReportId39Search.Attributes["OnClick"] += "document.getElementById('" + bConfirm.ClientID + "').value='N';";}
			if ((cWizardId39.Attributes["OnChange"] == null || cWizardId39.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cWizardId39.Visible && cWizardId39.Enabled) {cWizardId39.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if (cWizardId39Search.Attributes["OnClick"] == null || cWizardId39Search.Attributes["OnClick"].IndexOf("_bConfirm") < 0) {cWizardId39Search.Attributes["OnClick"] += "document.getElementById('" + bConfirm.ClientID + "').value='N';";}
			if ((cStaticPgId39.Attributes["OnChange"] == null || cStaticPgId39.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cStaticPgId39.Visible && cStaticPgId39.Enabled) {cStaticPgId39.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
		...
		private void ClearMaster(object sender, System.EventArgs e)
		{
			...
			if (dt.Rows[1]["ColVisible"].ToString() == "Y" && dt.Rows[1]["ColReadOnly"].ToString() != "Y") {cPopup39.Checked = base.GetBool("N");}
			if (dt.Rows[2]["ColVisible"].ToString() == "Y" && dt.Rows[2]["ColReadOnly"].ToString() != "Y") {cParentId39.Text = string.Empty;}
			if (dt.Rows[3]["ColVisible"].ToString() == "Y" && dt.Rows[3]["ColReadOnly"].ToString() != "Y") {cMenuIndex39.Text = string.Empty;}
			if (dt.Rows[4]["ColVisible"].ToString() == "Y" && dt.Rows[4]["ColReadOnly"].ToString() != "Y") {cScreenId39.ClearSearch();}
			if (dt.Rows[5]["ColVisible"].ToString() == "Y" && dt.Rows[5]["ColReadOnly"].ToString() != "Y") {cReportId39.ClearSearch();}
			if (dt.Rows[6]["ColVisible"].ToString() == "Y" && dt.Rows[6]["ColReadOnly"].ToString() != "Y") {cWizardId39.ClearSearch();}
		...
		private void InitMaster(object sender, System.EventArgs e)
		{
			...
			cPopup39.Checked = base.GetBool("N");
			cParentId39.Text = string.Empty;
			cMenuIndex39.Text = string.Empty;
			cScreenId39.ClearSearch();
			cReportId39.ClearSearch();
			cWizardId39.ClearSearch();
		...
		protected void cAdmMenu35List_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmMenu35List_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			InitMaster(sender, e);      // Do this first to enable defaults for non-database columns.
			DataTable dt = null;
			try
			{
				dt = (new AdminSystem()).GetMstById("GetAdmMenu35ById",cAdmMenu35List.SelectedValue,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId));
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				if (dt.Rows.Count > 0)
				{
					...
					try {cPopup39.Checked = base.GetBool(dr["Popup39"].ToString());} catch {cPopup39.Checked = false;}
					try {cParentId39.Text = RO.Common3.Utils.fmNumeric("0",dr["ParentId39"].ToString(),base.LUser.Culture);} catch {cParentId39.Text = string.Empty;}
					try {cMenuIndex39.Text = RO.Common3.Utils.fmNumeric("0",dr["MenuIndex39"].ToString(),base.LUser.Culture);} catch {cMenuIndex39.Text = string.Empty;}
					SetScreenId39(cScreenId39,dr["ScreenId39"].ToString());
					cScreenId39Search_Script();
					SetReportId39(cReportId39,dr["ReportId39"].ToString());
					cReportId39Search_Script();
					SetWizardId39(cWizardId39,dr["WizardId39"].ToString());
					cWizardId39Search_Script();
					SetStaticPgId39(cStaticPgId39,dr["StaticPgId39"].ToString());
					cStaticPgId39Search_Script();
			...
			cButPanel.DataBind();
			ScriptManager.GetCurrent(Parent.Page).SetFocus(cAdmMenu35List.FocusID);
			ShowDirty(false); PanelTop.Update();
			// *** List Selection (End of) Web Rule starts here *** //
		}

		protected void cScreenId39_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cScreenId39_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cScreenId39_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			...
			TextBox pwd = null;
			if (cScreenId39.Items.Count > 0 && cScreenId39.DataSource != null)
			{
				DataView dv = (DataView) cScreenId39.DataSource; dv.RowFilter = string.Empty;
				DataRowView dr = cScreenId39.DataSetIndex >= 0 && cScreenId39.DataSetIndex < dv.Count ? dv[cScreenId39.DataSetIndex] : dv[0];
				cScreenId39Search_Script();
		...
		protected void cReportId39_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cReportId39_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cReportId39_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			// *** On Change/ On Click Web Rule starts here *** //
			EnableValidators(true); // Do not remove; Need to reenable after postback, especially in the grid.
			TextBox pwd = null;
			if (cReportId39.Items.Count > 0 && cReportId39.DataSource != null)
			{
				DataView dv = (DataView) cReportId39.DataSource; dv.RowFilter = string.Empty;
				DataRowView dr = cReportId39.DataSetIndex >= 0 && cReportId39.DataSetIndex < dv.Count ? dv[cReportId39.DataSetIndex] : dv[0];
				cReportId39Search_Script();
			if (pwd != null) {ScriptManager.GetCurrent(Parent.Page).SetFocus(pwd.ClientID);} else {ScriptManager.GetCurrent(Parent.Page).SetFocus(SenderFocusId(sender));}
			}
		}

		protected void cWizardId39_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cWizardId39_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cWizardId39_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			// *** On Change/ On Click Web Rule starts here *** //
			EnableValidators(true); // Do not remove; Need to reenable after postback, especially in the grid.
			TextBox pwd = null;
			if (cWizardId39.Items.Count > 0 && cWizardId39.DataSource != null)
			{
				DataView dv = (DataView) cWizardId39.DataSource; dv.RowFilter = string.Empty;
				DataRowView dr = cWizardId39.DataSetIndex >= 0 && cWizardId39.DataSetIndex < dv.Count ? dv[cWizardId39.DataSetIndex] : dv[0];
				cWizardId39Search_Script();
			if (pwd != null) {ScriptManager.GetCurrent(Parent.Page).SetFocus(pwd.ClientID);} else {ScriptManager.GetCurrent(Parent.Page).SetFocus(SenderFocusId(sender));}
			}
		}

		protected void cStaticPgId39_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cStaticPgId39_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cStaticPgId39_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			// *** On Change/ On Click Web Rule starts here *** //
			EnableValidators(true); // Do not remove; Need to reenable after postback, especially in the grid.
			TextBox pwd = null;
			if (cStaticPgId39.Items.Count > 0 && cStaticPgId39.DataSource != null)
			{
				DataView dv = (DataView) cStaticPgId39.DataSource; dv.RowFilter = string.Empty;
				DataRowView dr = cStaticPgId39.DataSetIndex >= 0 && cStaticPgId39.DataSetIndex < dv.Count ? dv[cStaticPgId39.DataSetIndex] : dv[0];
				cStaticPgId39Search_Script();
			if (pwd != null) {ScriptManager.GetCurrent(Parent.Page).SetFocus(pwd.ClientID);} else {ScriptManager.GetCurrent(Parent.Page).SetFocus(SenderFocusId(sender));}
			}
		}

		protected void cIconUrl39_Click(object sender, System.Web.UI.ImageClickEventArgs e)
		{
			// *** On Change/ On Click Web Rule starts here *** //
			EnableValidators(true); // Do not remove; Need to reenable after postback, especially in the grid.
		}
		...
		public void cUndoAllButton_Click(object sender, System.EventArgs e)
		{
			...
			else { PopAdmMenu35List(sender, e, false, cMenuId39.Text); }
			// *** System Button Click (After) Web Rule starts here *** //
		}
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmMenu35List); PopAdmMenu35List(sender, e, false, null);
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			if (ValidPage())
			{
				AdmMenu35 ds = PrepAdmMenuData(null,cMenuId39.Text == string.Empty);
				if (!string.IsNullOrEmpty(cAdmMenu35List.SelectedValue) && cMenuId39.Text != cAdmMenu35List.SelectedValue)
				{
					PreMsgPopup("Primary key cannot be changed");
					return rtn;
				}
				if (string.IsNullOrEmpty(cAdmMenu35List.SelectedValue))	// Add
				{
					if (ds != null)
					{
						pid = (new AdminSystem()).AddData(35,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc,noTrans,commandTimeOut);
					}
					if (!string.IsNullOrEmpty(pid))
					{
						cAdmMenu35List.ClearSearch(); Session.Remove(KEY_dtAdmMenu35List);
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmMenu35List(sender, e, false, pid);
						rtn = GetScreenHlp().Rows[0]["AddMsg"].ToString();
					}
				}
				else {
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(35,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc,noTrans,commandTimeOut)) {bValid7 = true;}
					if (bValid7)
					{
						cAdmMenu35List.ClearSearch(); Session.Remove(KEY_dtAdmMenu35List);
						ShowDirty(false); PopAdmMenu35List(sender, e, false, ds.Tables["AdmMenu"].Rows[0]["MenuId39"]);
						rtn = GetScreenHlp().Rows[0]["UpdMsg"].ToString();
					}
				}
			}
			//WebRule: Store the url for launch later
            string MenuUrl;
		...
		public void cDeleteButton_Click(object sender, System.EventArgs e)
		{
			...
			{
				AdmMenu35 ds = PrepAdmMenuData(null,false);
				try
				{
					if (ds != null && (new AdminSystem()).DelData(35,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc))
					{
						cAdmMenu35List.ClearSearch(); Session.Remove(KEY_dtAdmMenu35List);
						ShowDirty(false); PopAdmMenu35List(sender, e, false, null);
		...
		private AdmMenu35 PrepAdmMenuData(DataView dv, bool bAdd)
		{
			...
			try {dr["Popup39"] = base.SetBool(cPopup39.Checked);} catch {}
			drType["Popup39"] = "Char"; drDisp["Popup39"] = "CheckBox";
			try {dr["ParentId39"] = cParentId39.Text.Trim();} catch {}
			drType["ParentId39"] = "Numeric"; drDisp["ParentId39"] = "TextBox";
			try {dr["MenuIndex39"] = cMenuIndex39.Text.Trim();} catch {}
			drType["MenuIndex39"] = "Numeric"; drDisp["MenuIndex39"] = "TextBox";
			try {dr["ScreenId39"] = cScreenId39.SelectedValue;} catch {}
			drType["ScreenId39"] = "Numeric"; drDisp["ScreenId39"] = "AutoComplete";
			try {dr["ReportId39"] = cReportId39.SelectedValue;} catch {}
			drType["ReportId39"] = "Numeric"; drDisp["ReportId39"] = "AutoComplete";
			try {dr["WizardId39"] = cWizardId39.SelectedValue;} catch {}
			drType["WizardId39"] = "Numeric"; drDisp["WizardId39"] = "AutoComplete";
			try {dr["StaticPgId39"] = cStaticPgId39.SelectedValue;} catch {}
			drType["StaticPgId39"] = "Numeric"; drDisp["StaticPgId39"] = "AutoComplete";
			try {dr["Miscellaneous39"] = cMiscellaneous39.Text.Trim();} catch {}
		...
		public bool CanAct(char typ) { return CanAct(typ, GetAuthRow(), cAdmMenu35List.SelectedValue.ToString()); }
		...
		private void PreMsgPopup(string msg) { PreMsgPopup(msg,cAdmMenu35List,null); }

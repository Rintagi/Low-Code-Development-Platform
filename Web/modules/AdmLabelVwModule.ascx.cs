...
namespace RO.Web
{
	public partial class AdmLabelVwModule : RO.Web.ModuleBase
	{

		private const string KEY_lastAddedRow = "Cache:lastAddedRow3_119";
		private const string KEY_lastSortOrd = "Cache:lastSortOrd3_119";
		private const string KEY_lastSortImg = "Cache:lastSortImg3_119";
		private const string KEY_lastSortCol = "Cache:lastSortCol3_119";
		private const string KEY_lastSortExp = "Cache:lastSortExp3_119";
		private const string KEY_lastSortUrl = "Cache:lastSortUrl3_119";
		private const string KEY_lastSortTog = "Cache:lastSortTog3_119";
		private const string KEY_lastImpPwdOvride = "Cache:lastImpPwdOvride3_119";
		private const string KEY_cntImpPwdOvride = "Cache:cntImpPwdOvride3_119";
		private const string KEY_currPageIndex = "Cache:currPageIndex3_119";
		private const string KEY_bHiImpVisible = "Cache:bHiImpVisible3_119";
		private const string KEY_bShImpVisible = "Cache:bShImpVisible3_119";
		private const string KEY_dtAdmLabelVwGrid = "Cache:dtAdmLabelVwGrid";
		private const string KEY_scrImport = "Cache:scrImport3_119";
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_119";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_119";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_119";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_119";
		private const string KEY_dtLabel = "Cache:dtLabel3_119";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_119";
		private const string KEY_dtScrCri = "Cache:dtScrCri3_119";
		private const string KEY_dsScrCriVal = "Cache:dsScrCriVal3_119";


		private const string KEY_dtSystems = "Cache:dtSystems3_119";
		private const string KEY_sysConnectionString = "Cache:sysConnectionString3_119";
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_119";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_119";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_119";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				...
				Session.Remove(KEY_dtAdmLabelVwGrid);
				...
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 119, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmLabelVw119List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=119&typ=N&sys=3','','',''); return false;"; }
			...
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmLabelVwGrid];
				if (dt != null)
				{
		...
		private void InitializeComponent()
		{
			...
			if (LcSysConnString == null)
			{
				...
				{
					DataTable dt;
					dt = (new AdminSystem()).GetAuthRow(119,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
					...
					{
						cAdmLabelVwGrid.PreRender += new EventHandler(cAdmLabelVwGrid_OnPreRender);
					}
				}
		...
		private void SetSystem(byte SystemId)
		{
			...
			try
			{
				...
				if ((Config.DeployType == "DEV" || row["dbAppDatabase"].ToString() == base.CPrj.EntityCode + "View") && !(base.CPrj.EntityCode != "RO" && row["SysProgram"].ToString() == "Y") && (new AdminSystem()).IsRegenNeeded(string.Empty,119,0,0,LcSysConnString,LcAppPw))
				{
					(new GenScreensSystem()).CreateProgram(119, "Label View", row["dbAppDatabase"].ToString(), base.CPrj, base.CSrc, base.CTar, LcAppConnString, LcAppPw);
		...
		private void CheckAuthentication(bool pageLoad)
		{
          ...
          else CheckAuthentication(pageLoad, false);
		}
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(119,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(119,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void SetClientRule(ListViewDataItem ee, bool isEditItem)
		{
			...
			if (dvRul.Count > 0)
			{
				...
				{
					if (ee == null || drv["ScriptEvent"].ToString().Substring(0,2).ToLower() != "on" || (cAdmLabelVwGrid.EditIndex > -1 && GetDataItemIndex(cAdmLabelVwGrid.EditIndex) == ee.DataItemIndex))
					{
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(119,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(119,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,119,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(119,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(119,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(119,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				CheckAuthentication(false);
				DataView dv = null;
				try {dv = ((DataTable)Session[KEY_dtAdmLabelVwGrid]).DefaultView;}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(119,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					{
						if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["LabelId265"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["LabelLink265"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["CultureId265"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["LabelCat265"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["LabelKey265"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["LabelText265"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["SortOrder265"].ToString(),base.LUser.Culture) + (char)9);}
						sb.Append(Environment.NewLine);
					}
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmLabelVw.csv"; Session["ExportStr"] = (Config.ExportExcelCSV ? "sep=\t\n": "") + sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmLabelVw.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private string GetRtfGrid(DataView dv)
		{
			...
			try
			{
				...
				foreach (DataRowView drv in dv)
				{
					...
					sb.Append("\r\n");
					sb.Append(@"{");
					if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["LabelId265"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["LabelLink265"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["CultureId265"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(drv["LabelCat265"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append(drv["LabelKey265"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append(drv["LabelText265"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["SortOrder265"].ToString(),base.LUser.Culture) + @"\cell ");}
					sb.Append(@"}");
					sb.Append("\r\n");
					sb.Append(@"\pard \ql \li0\ri0\widctlpar\intbl\aspalpha\aspnum\adjustright\rin0\lin0 {");
					sb.Append(@"\insrsid2981395 \trowd \irow0\irowband0\lastrow \ts15\trgaph108\trleft-108\trbrdrt\brdrs\brdrw10 \trbrdrl\brdrs\brdrw10 \trbrdrb\brdrs\brdrw10 \trbrdrr\brdrs\brdrw10 \trbrdrh\brdrs\brdrw10 \trbrdrv\brdrs\brdrw10 ");
		...
		private void GetPageInfo()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastPageInfo(119, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 119, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			// *** Criteria Trigger (before) Web Rule starts here *** //
			DataTable dt = (DataTable)Session[KEY_dtAdmLabelVwGrid];
			if (cAdmLabelVwGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
			if (IsPostBack && cCriteria.Visible) ReBindCriteria(GetScrCriteria());
			UpdCriteria(true);
			PopAdmLabelVw119List(sender, e, false, null);
			}
		}
			...
			try
			{
				foreach (DataRowView drv in dvCri)
				{
					...
					if (drv["DisplayName"].ToString() == "ComboBox")
					{
						...
						(new AdminSystem()).MkGetScreenIn("119", drv["ScreenCriId"].ToString(), "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), LcAppDb, LcDesDb, drv["MultiDesignDb"].ToString(), LcSysConnString, LcAppPw);
						DataView dv = new DataView((new AdminSystem()).GetScreenIn("119", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), (new AdminSystem()).CountScrCri(drv["ScreenCriId"].ToString(), drv["MultiDesignDb"].ToString(), drv["MultiDesignDb"].ToString() == "N" ? LcSysConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)), drv["RequiredValid"].ToString(), 0, string.Empty, drv["DisplayMode"].ToString() != "AutoListBox", val, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)));
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("119", LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmLabelVw119" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmLabelVw119" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			foreach (DataRowView drv in dvCri)
			{
			    if (drv["DisplayName"].ToString() == "ListBox")
			    {
					...
					if (cListBox != null)
					{
						...
						int TotalChoiceCnt = new DataView((new AdminSystem()).GetScreenIn("119", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), CriCnt, drv["RequiredValid"].ToString(), 0, string.Empty, true, string.Empty, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId))).Count;
						string selectedValues = string.Join(",", cListBox.Items.Cast<ListItem>().Where(x => x.Selected).Select(x => "'" + x.Value + "'").ToArray());
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("119", "AdmLabelVw119", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		private void PopAdmLabelVw119List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			DataTable dt = null;
			CheckAuthentication(false);
			...
			try
			{
				try {dt = (new AdminSystem()).GetLis(119,"GetLisAdmLabelVw119",false,"Y",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				catch {dt = (new AdminSystem()).GetLis(119,"GetLisAdmLabelVw119",false,"N",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				Session[KEY_dtAdmLabelVwGrid] = dt;
				cAdmLabelVwGridDataPager.PageSize = Int16.Parse(cPgSize.Text); GotoPage(0);
			    if (Session[KEY_lastSortCol] != null)
			    {
					cAdmLabelVwGrid_OnSorting(sender, new ListViewSortEventArgs((string)Session[KEY_lastSortExp], SortDirection.Ascending));
			    }
				else
			    {
					cAdmLabelVwGrid_DataBind(dt.DefaultView);
			    }
		...
		public void cFindButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dv != null)
			{
				if (cAdmLabelVwGrid.EditIndex < 0 || UpdateGridRow(cAdmLabelVwGrid, new CommandEventArgs("Save", "")))
				{
					...
					GotoPage(0); cAdmLabelVwGrid_DataBind(dv);
					if (GetCurrPageIndex() != (int)Session[KEY_currPageIndex] && (int)Session[KEY_currPageIndex] < GetTotalPages()) { GotoPage((int)Session[KEY_currPageIndex]); cAdmLabelVwGrid_DataBind(dv);}
		...
		public void cInsRowButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dt != null && (ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")))
			{
				...
				Session[KEY_lastAddedRow] = 0; Session[KEY_dtAdmLabelVwGrid] = dt; Session[KEY_currPageIndex] = 0; GotoPage(0);
				cAdmLabelVwGrid_DataBind(dt.DefaultView);
				cAdmLabelVwGrid_OnItemEditing(cAdmLabelVwGrid, new ListViewEditEventArgs(0));
		...
		public void cPgSizeButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cAdmLabelVwGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				...
				cAdmLabelVwGridDataPager.PageSize = int.Parse(cPgSize.Text); GotoPage(0);
				if (dt.Rows.Count <= 0 || (GetAuthRow().Rows[0]["AllowAdd"].ToString() == "Y" && dt.Rows.Count == 1)) {cAdmLabelVwGrid_DataBind(dt.DefaultView);} else {cFindButton_Click(sender, e);}
				try
				{
					(new AdminSystem()).UpdLastPageInfo(119, base.LUser.UsrId, cPgSize.Text, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		}
		...
		public void cShowImpButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cSchemaImage.Visible)
			{
				try
				{
					Session["ImportSchema"] = (new AdminSystem()).GetSchemaScrImp(119,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (cSchemaImage.Attributes["OnClick"] == null || cSchemaImage.Attributes["OnClick"].IndexOf("ImportSchema.aspx") < 0) {cSchemaImage.Attributes["OnClick"] += "SearchLink('ImportSchema.aspx?scm=S&key=119&csy=3','','',''); return false;";}
			}
			// *** System Button Click (after) Web Rule starts here *** //
		}
		...
		protected void ScrImport(bool bClear)
		{
			...
			DataTable dtAdmLabelVwGrid = (DataTable)Session[KEY_dtAdmLabelVwGrid];
			if (dti != null && dtAdmLabelVwGrid != null)
			{
				if (bClear)
				{
					dtAdmLabelVwGrid.Rows.Clear();
				}
				cFind.Text = string.Empty;
				int iExist = dtAdmLabelVwGrid.Rows.Count;
				iRow = dti.DefaultView.Count - iExist;
				if (!dti.Columns.Contains("_NewRow")) { dti.Columns.Add("_NewRow"); }
				Session[KEY_dtAdmLabelVwGrid] = dti;
				cAdmLabelVwGrid_DataBind(dti.DefaultView);
		...
		public void cImportButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cFNameO.Text != string.Empty && cWorkSheet.Items.Count > 0 && cWorkSheet.SelectedItem.Text != string.Empty && cStartRow.Text != string.Empty)
			{
				...
				try
				{
					if (cAdmLabelVwGrid.EditIndex < 0 || UpdateGridRow(cAdmLabelVwGrid, new CommandEventArgs("Save", "")))
					{
						Session[KEY_scrImport] = ImportFile(dtAdmLabelVwGrid.Copy(),cFNameO.Text,cWorkSheet.SelectedItem.Text,cStartRow.Text,Config.PathTmpImport + cFName.Text);
					}
		...
		private DataTable ImportFile(DataTable dt, string fileName, string workSheet, string startRow, string fileFullName)
		{
			try
			{
				...
				{
					if (rows[iRow][2].ToString() == string.Empty && rows[iRow][3].ToString() == string.Empty && rows[iRow][4].ToString() == string.Empty && rows[iRow][5].ToString() == string.Empty) {idel = idel + 1;}
					else
					{
		...
		private void GotoPage(int pageNo)
		{
			if (pageNo >= 0 && pageNo < GetTotalPages())
			{
				if (cAdmLabelVwGrid.EditIndex < 0 || UpdateGridRow(cAdmLabelVwGrid, new CommandEventArgs("Save", "")))
				{
					try { cAdmLabelVwGridDataPager.SetPageProperties(cAdmLabelVwGridDataPager.PageSize * pageNo, cAdmLabelVwGridDataPager.MaximumRows, false); cAdmLabelVwGrid_DataBind(null); }
					catch
					{
						try { cAdmLabelVwGridDataPager.SetPageProperties(0, cAdmLabelVwGridDataPager.MaximumRows, false); cAdmLabelVwGrid_DataBind(null); } catch {}
					}
				}
			}
		}

		protected void cAdmLabelVwGrid_OnItemCommand(object sender, ListViewCommandEventArgs e)
		{
		}

		private void cAdmLabelVwGrid_DataBind(DataView dvAdmLabelVwGrid)
		{
			if (dvAdmLabelVwGrid == null)
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmLabelVwGrid];
				dvAdmLabelVwGrid = dt != null ? dt.DefaultView : null;
			}
			if (dvAdmLabelVwGrid != null)
			{
				LcAuth = GetAuthCol().AsEnumerable().ToDictionary<DataRow,string>(dr=>dr["ColName"].ToString());
				cAdmLabelVwGrid.DataSource = dvAdmLabelVwGrid;
				cAdmLabelVwGrid.DataBind();
		...
		protected void cAdmLabelVwGrid_OnSorting(object sender, ListViewSortEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmLabelVwGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null)
			{
				if (cAdmLabelVwGrid.EditIndex < 0 || UpdateGridRow(cAdmLabelVwGrid, new CommandEventArgs("Save", "")))
				{
					...
					Session[KEY_dtAdmLabelVwGrid] = dt;
					cAdmLabelVwGrid_DataBind(dv);
				}
			}
		}

		protected void cAdmLabelVwGrid_OnPagePropertiesChanging(object sender, PagePropertiesChangingEventArgs e)
		{
		    cAdmLabelVwGridDataPager.SetPageProperties(e.StartRowIndex, e.MaximumRows, false);
		    DataTable dt = (DataTable)Session[KEY_dtAdmLabelVwGrid];
		    DataView dv = dt != null ? dt.DefaultView : null;
			cAdmLabelVwGrid_DataBind(dv); cAdmLabelVwGrid.EditIndex = -1;
		}

		private void GridChkPgDirty(ListViewItem lvi)
		{
				WebControl cc = null; System.Web.UI.WebControls.Image ib = null;
				cc = ((WebControl)lvi.FindControl("cLabelLink265"));
				...
				cc = ((WebControl)lvi.FindControl("cCultureId265"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cLabelCat265"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cLabelKey265"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cLabelText265"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				ib = ((System.Web.UI.WebControls.Image)lvi.FindControl("cLabelText265E"));
				ib.Attributes["target_id"] = cc.ClientID;
				cc = ((WebControl)lvi.FindControl("cSortOrder265"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
		}

		protected void cAdmLabelVwGrid_OnItemDataBound(object sender, ListViewItemEventArgs e)
		{
			// *** GridItemDataBound (before) Web Rule End *** //
			DataTable dt = (DataTable)Session[KEY_dtAdmLabelVwGrid];
			bool isEditItem = false;
			DataView dvAdmLabelVwGrid = dt != null ? dt.DefaultView : null;
			if (cAdmLabelVwGrid.EditIndex > -1 && GetDataItemIndex(cAdmLabelVwGrid.EditIndex) == e.Item.DataItemIndex)
			{
				isEditItem = true;
			...
			if (dtAuthRow != null)
			{
				...
				{
					cc = e.Item.FindControl("cAdmLabelVwGridDelete"); if (cc != null) { cc.Visible = false; }
				}
				if (!AllowRowEdit(GetAuthRow(),dvAdmLabelVwGrid[e.Item.DataItemIndex]["LabelId265"].ToString()) || (Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N"))
				{
					cc = e.Item.FindControl("cAdmLabelVwGridEdit"); if (cc != null) { cc.Visible = false; }
				}
				else
				{
			        HtmlTableRow tr = e.Item.FindControl("cAdmLabelVwGridRow") as HtmlTableRow;
			        LinkButton lb = e.Item.FindControl("cAdmLabelVwGridEdit") as LinkButton;
		...
		protected void cLabelId265hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "0") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "0"; Session[KEY_lastSortExp] = "LabelId265";Session[KEY_lastSortImg] = "cLabelId265hi";
			cAdmLabelVwGrid_OnSorting(sender, new ListViewSortEventArgs("LabelId265", SortDirection.Ascending));
		}

		protected void cLabelLink265hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "1") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "1"; Session[KEY_lastSortExp] = "LabelLink265";Session[KEY_lastSortImg] = "cLabelLink265hi";
			cAdmLabelVwGrid_OnSorting(sender, new ListViewSortEventArgs("LabelLink265", SortDirection.Ascending));
		}

		protected void cCultureId265hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "2") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "2"; Session[KEY_lastSortExp] = "CultureId265";Session[KEY_lastSortImg] = "cCultureId265hi";
			cAdmLabelVwGrid_OnSorting(sender, new ListViewSortEventArgs("CultureId265", SortDirection.Ascending));
		}

		protected void cLabelCat265hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "3") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "3"; Session[KEY_lastSortExp] = "LabelCat265";Session[KEY_lastSortImg] = "cLabelCat265hi";
			cAdmLabelVwGrid_OnSorting(sender, new ListViewSortEventArgs("LabelCat265", SortDirection.Ascending));
		}

		protected void cLabelKey265hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "4") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "4"; Session[KEY_lastSortExp] = "LabelKey265";Session[KEY_lastSortImg] = "cLabelKey265hi";
			cAdmLabelVwGrid_OnSorting(sender, new ListViewSortEventArgs("LabelKey265", SortDirection.Ascending));
		}

		protected void cLabelText265hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "5") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "5"; Session[KEY_lastSortExp] = "LabelText265";Session[KEY_lastSortImg] = "cLabelText265hi";
			cAdmLabelVwGrid_OnSorting(sender, new ListViewSortEventArgs("LabelText265", SortDirection.Ascending));
		}

		protected void cSortOrder265hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "6") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "6"; Session[KEY_lastSortExp] = "SortOrder265";Session[KEY_lastSortImg] = "cSortOrder265hi";
			cAdmLabelVwGrid_OnSorting(sender, new ListViewSortEventArgs("SortOrder265", SortDirection.Ascending));
		}

		protected void cAdmLabelVwGrid_OnItemEditing(object sender, ListViewEditEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmLabelVwGrid];
			if ((ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")) && dt != null && dt.DefaultView.Count > GetDataItemIndex(e.NewEditIndex))
			{
				cAdmLabelVwGrid.EditIndex = e.NewEditIndex;
				cAdmLabelVwGrid_DataBind(null);
				Control wc = null;
				string idx = Page.Request["__EVENTARGUMENT"];
				if (!string.IsNullOrEmpty(idx))
				{
				    Control ctrlToFocus = cAdmLabelVwGrid.EditItem.FindControl("c" + idx);
				    if (((WebControl)ctrlToFocus).Enabled) { wc = ctrlToFocus; }
				}
				if (wc == null) { wc = FindEditableControl(cAdmLabelVwGrid.EditItem); }
				if (wc != null)
             {
             ...
             MkMobileLabel(cAdmLabelVwGrid.EditItem);
		    }
		    else { e.Cancel = true; }
		}
		...
		{
		    Label ml = null;
		    ml = lvi.FindControl("cLabelId265ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(0); }
		    ml = lvi.FindControl("cLabelLink265ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(1); }
		    ml = lvi.FindControl("cCultureId265ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(2); }
		    ml = lvi.FindControl("cLabelCat265ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(3); }
		    ml = lvi.FindControl("cLabelKey265ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(4); }
		    ml = lvi.FindControl("cLabelText265ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(5); }
		    ml = lvi.FindControl("cSortOrder265ml") as Label;
		...
		{
			TextBox tb = null;
			tb = (TextBox)lvi.FindControl("cCultureId265");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["CultureId265"] = tb.Text;} else {dr["CultureId265"] = System.DBNull.Value;}
			}
			tb = (TextBox)lvi.FindControl("cLabelCat265");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["LabelCat265"] = tb.Text;} else {dr["LabelCat265"] = System.DBNull.Value;}
			}
			tb = (TextBox)lvi.FindControl("cLabelKey265");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["LabelKey265"] = tb.Text;} else {dr["LabelKey265"] = System.DBNull.Value;}
			}
			tb = (TextBox)lvi.FindControl("cLabelText265");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["LabelText265"] = tb.Text;} else {dr["LabelText265"] = System.DBNull.Value;}
			}
			tb = (TextBox)lvi.FindControl("cSortOrder265");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["SortOrder265"] = tb.Text;} else {dr["SortOrder265"] = System.DBNull.Value;}
			}
		    if (bInsert) { dt.Rows.InsertAt(dr, 0); }
			Session[KEY_dtAdmLabelVwGrid] = dt;
			cAdmLabelVwGrid.EditIndex = -1;
			cAdmLabelVwGrid_DataBind(dt.DefaultView);
		}

		protected void cAdmLabelVwGrid_OnItemUpdating(object sender, ListViewUpdateEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmLabelVwGrid];
			if (dt != null && dt.Rows.Count > 0)
			{
				DataRow dr = dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row;
			    GridFill(cAdmLabelVwGrid.EditItem, dt, dr, false);
			    dr["_NewRow"] = "N";
			    if (dt.Columns.Contains("_SortOrder")) dr["_SortOrder"] = null;
			}
		}

		protected void cAdmLabelVwGrid_OnItemCanceling(object sender, ListViewCancelEventArgs e)
		{
			PanelTop.Update();
			DataTable dt = (DataTable)Session[KEY_dtAdmLabelVwGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			...
			{
				cAdmLabelVwGrid_OnItemDeleting(sender, new ListViewDeleteEventArgs(e.ItemIndex));
			}
			if (dt.Columns.Contains("_SortOrder")) dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row["_SortOrder"] = null;
			cAdmLabelVwGrid.EditIndex = -1; cAdmLabelVwGrid_DataBind(null);
		}
		...
		protected void cDeleteAllButton_Click(object sender, System.EventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmLabelVwGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null && (cAdmLabelVwGrid.EditIndex < 0 || UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				GotoPage(0);
				while (dv.Count > 0) {dv.Delete(0);}
				Session[KEY_dtAdmLabelVwGrid] = dt;
				cAdmLabelVwGrid.EditIndex = -1; cAdmLabelVwGrid_DataBind(dv);
			}
		}

		protected void cAdmLabelVwGrid_OnItemDeleting(object sender, ListViewDeleteEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmLabelVwGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			DataRow dr = dv != null ? dv[GetDataItemIndex(e.ItemIndex)].Row : null;
			if (dv != null && (cAdmLabelVwGrid.EditIndex == e.ItemIndex || UpdateGridRow(sender, new CommandEventArgs("Save",""))))
			{
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmLabelVwGrid); PopAdmLabelVw119List(sender, e, false, null);
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			// *** System Button Click (Before) Web Rule starts here *** //
			if (ValidPage() && UpdateGridRow(cAdmLabelVwGrid, new CommandEventArgs("Save", "")))
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmLabelVwGrid];
				DataView dv = dt != null ? dt.DefaultView : null;
				string ftr = string.Empty;
				if (dv.RowFilter != string.Empty) { ftr = dv.RowFilter; dv.RowFilter = string.Empty; }
				AdmLabelVw119 ds = PrepAdmLabelVwData(dv,false);
				if (ftr != string.Empty) {dv.RowFilter = ftr;}
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(119,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc,noTrans,commandTimeOut)) {bValid7 = true;}
					if (bValid7)
					{
						Session[KEY_currPageIndex] = GetCurrPageIndex();
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmLabelVw119List(sender, e, false, null);
		...
		private AdmLabelVw119 PrepAdmLabelVwData(DataView dv, bool bAdd)
		{
			...
			if (dv != null)
			{
				...
				ds.Tables["AdmLabelVwDef"].Rows.Add(MakeDisRow(ds.Tables["AdmLabelVwDef"].NewRow()));
					...
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmLabelVwUpd"].Rows.Add(MakeColRow(ds.Tables["AdmLabelVwUpd"].NewRow(), drv, false));
					}
					dv.RowStateFilter = DataViewRowState.Added;
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmLabelVwAdd"].Rows.Add(MakeColRow(ds.Tables["AdmLabelVwAdd"].NewRow(), drv, true));
					}
					dv.RowStateFilter = DataViewRowState.Deleted;
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmLabelVwDel"].Rows.Add(MakeColRow(ds.Tables["AdmLabelVwDel"].NewRow(), drv, false));
					}
		...
		private DataRow MakeColRow(DataRow dr, DataRowView drv, bool bAdd)
		{
			...
			if (dtAuth != null)
			{
				...
				dr["SortOrder265"] = drv["SortOrder265"].ToString().Trim();
				if (bAdd && dtAuth.Rows[6]["ColReadOnly"].ToString() == "Y" && dr["SortOrder265"].ToString() == string.Empty) {dr["SortOrder265"] = System.DBNull.Value;}
			}
			return dr;
		}
		...
		{
			if (cAdmLabelVwGrid.EditIndex > -1)
			{
				cAdmLabelVwGrid_OnItemUpdating(sender, new ListViewUpdateEventArgs(cAdmLabelVwGrid.EditIndex));
			}
			return true;
		}

		protected void cAdmLabelVwGrid_OnPreRender(object sender, System.EventArgs e)
		{
			System.Web.UI.WebControls.Image hi = null;
			hi = (System.Web.UI.WebControls.Image)cAdmLabelVwGrid.FindControl("cLabelId265hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmLabelVwGrid.FindControl("cLabelLink265hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmLabelVwGrid.FindControl("cCultureId265hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmLabelVwGrid.FindControl("cLabelCat265hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmLabelVwGrid.FindControl("cLabelKey265hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmLabelVwGrid.FindControl("cLabelText265hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmLabelVwGrid.FindControl("cSortOrder265hi"); if (hi != null) { hi.Visible = false; }
			if (Session[KEY_lastSortImg] != null)
			{
				hi = (System.Web.UI.WebControls.Image)cAdmLabelVwGrid.FindControl((string)Session[KEY_lastSortImg]);
				if (hi != null) { hi.ImageUrl = Utils.AddTilde((string)Session[KEY_lastSortUrl]); hi.Visible = true; }
			}
			IgnoreHeaderConfirm((LinkButton)cAdmLabelVwGrid.FindControl("cLabelId265hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmLabelVwGrid.FindControl("cLabelLink265hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmLabelVwGrid.FindControl("cCultureId265hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmLabelVwGrid.FindControl("cLabelCat265hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmLabelVwGrid.FindControl("cLabelKey265hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmLabelVwGrid.FindControl("cLabelText265hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmLabelVwGrid.FindControl("cSortOrder265hl"));
		}

		private string GetButtonId(ListViewItem lvi)
		{
			string ButtonID = String.Empty;
			Control c = lvi.FindControl("cAdmLabelVwGridEdit");
		...
		{
		    return cAdmLabelVwGridDataPager.StartRowIndex / cAdmLabelVwGridDataPager.PageSize;
		}

		private int GetTotalPages()
		{
		    return (int)Math.Ceiling((double)cAdmLabelVwGridDataPager.TotalRowCount / cAdmLabelVwGridDataPager.PageSize);
		}

		private int GetDataItemIndex(int editIndex)
		{
		    return cAdmLabelVwGridDataPager.StartRowIndex + editIndex;
		}

		protected void cAdmLabelVwGrid_OnLayoutCreated(object sender, EventArgs e)
		{
		    // Header:
		    LinkButton lb = null;
		    lb = cAdmLabelVwGrid.FindControl("cLabelId265hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(0); lb.ToolTip = ColumnToolTip(0); lb.Parent.Visible = GridColumnVisible(0); }
		    lb = cAdmLabelVwGrid.FindControl("cLabelLink265hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(1); lb.ToolTip = ColumnToolTip(1); lb.Parent.Visible = GridColumnVisible(1); }
		    lb = cAdmLabelVwGrid.FindControl("cCultureId265hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(2); lb.ToolTip = ColumnToolTip(2); lb.Parent.Visible = GridColumnVisible(2); }
		    lb = cAdmLabelVwGrid.FindControl("cLabelCat265hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(3); lb.ToolTip = ColumnToolTip(3); lb.Parent.Visible = GridColumnVisible(3); }
		    lb = cAdmLabelVwGrid.FindControl("cLabelKey265hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(4); lb.ToolTip = ColumnToolTip(4); lb.Parent.Visible = GridColumnVisible(4); }
		    lb = cAdmLabelVwGrid.FindControl("cLabelText265hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(5); lb.ToolTip = ColumnToolTip(5); lb.Parent.Visible = GridColumnVisible(5); }
		    lb = cAdmLabelVwGrid.FindControl("cSortOrder265hl") as LinkButton;
			...
			if (dtAuthRow != null)
			{
				...
				if (dr["AllowDel"].ToString() == "N" || (Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N"))
				{
		            lb = cAdmLabelVwGrid.FindControl("cDeleteAllButton") as LinkButton; if (lb != null) { lb.Visible = false; }
				}
			}
		    // footer:
		    Label gc = null;
		    gc = cAdmLabelVwGrid.FindControl("cLabelId265fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(0); }
		    gc = cAdmLabelVwGrid.FindControl("cLabelLink265fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(1); }
		    gc = cAdmLabelVwGrid.FindControl("cCultureId265fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(2); }
		    gc = cAdmLabelVwGrid.FindControl("cLabelCat265fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(3); }
		    gc = cAdmLabelVwGrid.FindControl("cLabelKey265fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(4); }
		    gc = cAdmLabelVwGrid.FindControl("cLabelText265fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(5); }
		    gc = cAdmLabelVwGrid.FindControl("cSortOrder265fl") as Label;

...
namespace RO.Web
{
	public partial class AdmScreenTabModule : RO.Web.ModuleBase
	{

		private const string KEY_lastAddedRow = "Cache:lastAddedRow3_54";
		private const string KEY_lastSortOrd = "Cache:lastSortOrd3_54";
		private const string KEY_lastSortImg = "Cache:lastSortImg3_54";
		private const string KEY_lastSortCol = "Cache:lastSortCol3_54";
		private const string KEY_lastSortExp = "Cache:lastSortExp3_54";
		private const string KEY_lastSortUrl = "Cache:lastSortUrl3_54";
		private const string KEY_lastSortTog = "Cache:lastSortTog3_54";
		private const string KEY_lastImpPwdOvride = "Cache:lastImpPwdOvride3_54";
		private const string KEY_cntImpPwdOvride = "Cache:cntImpPwdOvride3_54";
		private const string KEY_currPageIndex = "Cache:currPageIndex3_54";
		private const string KEY_bHiImpVisible = "Cache:bHiImpVisible3_54";
		private const string KEY_bShImpVisible = "Cache:bShImpVisible3_54";
		private const string KEY_dtAdmScreenTabGrid = "Cache:dtAdmScreenTabGrid";
		private const string KEY_scrImport = "Cache:scrImport3_54";
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_54";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_54";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_54";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_54";
		private const string KEY_dtLabel = "Cache:dtLabel3_54";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_54";
		private const string KEY_dtScrCri = "Cache:dtScrCri3_54";
		private const string KEY_dsScrCriVal = "Cache:dsScrCriVal3_54";

		private const string KEY_dtScreenId19 = "Cache:dtScreenId19";
		private const string KEY_dtCultureId80 = "Cache:dtCultureId80";

		private const string KEY_dtSystems = "Cache:dtSystems3_54";
		private const string KEY_sysConnectionString = "Cache:sysConnectionString3_54";
		private const string KEY_dtAdmScreenTab54List = "Cache:dtAdmScreenTab54List";
		private const string KEY_bNewVisible = "Cache:bNewVisible3_54";
		private const string KEY_bNewSaveVisible = "Cache:bNewSaveVisible3_54";
		private const string KEY_bCopyVisible = "Cache:bCopyVisible3_54";
		private const string KEY_bCopySaveVisible = "Cache:bCopySaveVisible3_54";
		private const string KEY_bDeleteVisible = "Cache:bDeleteVisible3_54";
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_54";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_54";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_54";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				DataTable dtMenuAccess = (new MenuSystem()).GetMenu(base.LUser.CultureId, 3, base.LImpr, LcSysConnString, LcAppPw,54, null, null);
				...
				Session.Remove(KEY_dtScreenId19);
				Session.Remove(KEY_dtCultureId80);
				SetButtonHlp();
				GetSystems();
				SetColumnAuthority();
				GetGlobalFilter();
				...
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 54, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmScreenTab54List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=54&typ=N&sys=3','','',''); return false;"; }
			...
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmScreenTabGrid];
				if (dt != null)
				{
		...
		private void InitializeComponent()
		{
			...
			if (LcSysConnString == null)
			{
				...
				{
					DataTable dt;
					dt = (new AdminSystem()).GetAuthRow(54,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
					...
					{
						cAdmScreenTabGrid.PreRender += new EventHandler(cAdmScreenTabGrid_OnPreRender);
					}
				}
		...
		private void SetSystem(byte SystemId)
		{
			...
			try
			{
				...
				if ((Config.DeployType == "DEV" || row["dbAppDatabase"].ToString() == base.CPrj.EntityCode + "View") && !(base.CPrj.EntityCode != "RO" && row["SysProgram"].ToString() == "Y") && (new AdminSystem()).IsRegenNeeded(string.Empty,54,0,0,LcSysConnString,LcAppPw))
				{
					(new GenScreensSystem()).CreateProgram(54, "Screen Tab Folders", row["dbAppDatabase"].ToString(), base.CPrj, base.CSrc, base.CTar, LcAppConnString, LcAppPw);
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(54,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(54,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void SetClientRule(ListViewDataItem ee, bool isEditItem)
		{
			...
			if (dvRul.Count > 0)
			{
				...
				{
					if (ee == null || drv["ScriptEvent"].ToString().Substring(0,2).ToLower() != "on" || (cAdmScreenTabGrid.EditIndex > -1 && GetDataItemIndex(cAdmScreenTabGrid.EditIndex) == ee.DataItemIndex))
					{
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(54,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(54,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,54,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(54,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(54,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(54,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				try
				{
					try {dv = new DataView((new AdminSystem()).GetExp(54,"GetExpAdmScreenTab54","Y",(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
					catch {dv = new DataView((new AdminSystem()).GetExp(54,"GetExpAdmScreenTab54","N",(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(54,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					{
						if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["ScreenTabId19"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["ScreenId19"].ToString() + (char)9 + drv["ScreenId19Text"].ToString() + (char)9);}
						if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["TabFolderName19"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["TabFolderOrder19"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["ScreenTabHlpId80"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append(drv["CultureId80"].ToString() + (char)9 + drv["CultureId80Text"].ToString() + (char)9);}
						if (dtAu.Rows[7]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["TabFolderName80"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						sb.Append(Environment.NewLine);
					}
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmScreenTab.csv"; Session["ExportStr"] = (Config.ExportExcelCSV ? "sep=\t\n": "") + sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmScreenTab.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private string GetRtfGrid(DataView dv)
		{
			...
			try
			{
				...
				foreach (DataRowView drv in dv)
				{
					...
					sb.Append("\r\n");
					sb.Append(@"{");
					if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["ScreenTabId19"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["ScreenId19Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(drv["TabFolderName19"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["TabFolderOrder19"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["ScreenTabHlpId80"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append(drv["CultureId80Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[7]["ColExport"].ToString() == "Y") {sb.Append(drv["TabFolderName80"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					sb.Append(@"}");
					sb.Append("\r\n");
					sb.Append(@"\pard \ql \li0\ri0\widctlpar\intbl\aspalpha\aspnum\adjustright\rin0\lin0 {");
					sb.Append(@"\insrsid2981395 \trowd \irow0\irowband0\lastrow \ts15\trgaph108\trleft-108\trbrdrt\brdrs\brdrw10 \trbrdrl\brdrs\brdrw10 \trbrdrb\brdrs\brdrw10 \trbrdrr\brdrs\brdrw10 \trbrdrh\brdrs\brdrw10 \trbrdrv\brdrs\brdrw10 ");
		...
		private void GetPageInfo()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastPageInfo(54, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 54, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			// *** Criteria Trigger (before) Web Rule starts here *** //
			DataTable dt = (DataTable)Session[KEY_dtAdmScreenTabGrid];
			if (cAdmScreenTabGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
			if (IsPostBack && cCriteria.Visible) ReBindCriteria(GetScrCriteria());
			UpdCriteria(true);
			cAdmScreenTab54List.ClearSearch(); Session.Remove(KEY_dtAdmScreenTab54List);
			PopAdmScreenTab54List(sender, e, false, null);
			}
		}
			...
			try
			{
				foreach (DataRowView drv in dvCri)
				{
					...
					if (drv["DisplayName"].ToString() == "ComboBox")
					{
						...
						(new AdminSystem()).MkGetScreenIn("54", drv["ScreenCriId"].ToString(), "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), LcAppDb, LcDesDb, drv["MultiDesignDb"].ToString(), LcSysConnString, LcAppPw);
						DataView dv = new DataView((new AdminSystem()).GetScreenIn("54", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), (new AdminSystem()).CountScrCri(drv["ScreenCriId"].ToString(), drv["MultiDesignDb"].ToString(), drv["MultiDesignDb"].ToString() == "N" ? LcSysConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)), drv["RequiredValid"].ToString(), 0, string.Empty, drv["DisplayMode"].ToString() != "AutoListBox", val, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)));
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("54", LcSysConnString, LcAppPw);
				}
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmScreenTab54" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmScreenTab54" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			foreach (DataRowView drv in dvCri)
			{
			    if (drv["DisplayName"].ToString() == "ListBox")
			    {
					...
					if (cListBox != null)
					{
						...
						int TotalChoiceCnt = new DataView((new AdminSystem()).GetScreenIn("54", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), CriCnt, drv["RequiredValid"].ToString(), 0, string.Empty, true, string.Empty, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId))).Count;
						string selectedValues = string.Join(",", cListBox.Items.Cast<ListItem>().Where(x => x.Selected).Select(x => "'" + x.Value + "'").ToArray());
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("54", "AdmScreenTab54", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		protected void cbScreenId19(object sender, System.EventArgs e)
		{
			SetScreenId19((RoboCoder.WebControls.ComboBox)sender,string.Empty);
		}

		private void SetScreenId19(RoboCoder.WebControls.ComboBox ddl, string keyId)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(54,"GetDdlScreenId3S3162",true,false,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
					if (dv.Table.Columns.Contains("ScreenTabId"))
					{
						context["pMKeyColID"] = cAdmScreenTab54List.ClientID;
						context["pMKeyCol"] = "ScreenTabId";
						string ss = "(ScreenTabId is null";
						if (string.IsNullOrEmpty(cAdmScreenTab54List.SelectedValue)) {ss = ss + ")";} else {ss = ss + " OR ScreenTabId = " + cAdmScreenTab54List.SelectedValue + ")";}
						dv.RowFilter = ss;
					}
					ddl.DataSource = dv; Session[KEY_dtScreenId19] = dv.Table;
				    try { ddl.SelectByValue(keyId,string.Empty,true); } catch { try { ddl.SelectedIndex = 0; } catch { } }
				}
			}
		}

		protected void cbCultureId80(object sender, System.EventArgs e)
		{
			SetCultureId80((RoboCoder.WebControls.ComboBox)sender,string.Empty,cAdmScreenTabGrid.EditItem);
		}

		private void SetCultureId80(RoboCoder.WebControls.ComboBox ddl, string keyId, ListViewItem li)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(54,"GetDdlCultureId3S721",true,false,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
					...
					ddl.DataSource = dv; Session[KEY_dtCultureId80] = dv.Table;
				    try { ddl.SelectByValue(keyId,string.Empty,true); } catch { try { ddl.SelectedIndex = 0; } catch { } }
				}
			}
		}

		private DataView GetAdmScreenTab54List()
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmScreenTab54List];
			if (dt == null)
			{
				CheckAuthentication(false);
				int filterId = 0;
				...
				try
				{
					try {dt = (new AdminSystem()).GetLis(54,"GetLisAdmScreenTab54",true,"Y",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
					catch {dt = (new AdminSystem()).GetLis(54,"GetLisAdmScreenTab54",true,"N",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				}
		...
		private void PopAdmScreenTab54List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			DataView dv = GetAdmScreenTab54List();
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			context["isSys"] = "N";
			context["conn"] = KEY_sysConnectionString;
			cAdmScreenTab54List.AutoCompleteUrl = "AutoComplete.aspx/LisSuggests";
			cAdmScreenTab54List.DataContext = context;
			if (dv.Table == null) return;
			cAdmScreenTab54List.DataSource = dv;
			cAdmScreenTab54List.Visible = true;
			if (cAdmScreenTab54List.Items.Count <= 0) {cAdmScreenTab54List.Visible = false; cAdmScreenTab54List_SelectedIndexChanged(sender,e);}
			else
			{
				if (bPageLoad && !string.IsNullOrEmpty(Request.QueryString["key"]) && bUseCri.Value == string.Empty)
				{
					try {cAdmScreenTab54List.SelectByValue(Request.QueryString["key"],string.Empty,true);} catch {cAdmScreenTab54List.Items[0].Selected = true; cAdmScreenTab54List_SelectedIndexChanged(sender, e);}
				    cScreenSearch.Visible = false; cSystem.Visible = false; cEditButton.Visible = true; cSaveCloseButton.Visible = cSaveButton.Visible;
				}
				else
				{
					if (ID != null) {if (!cAdmScreenTab54List.SelectByValue(ID,string.Empty,true)) {ID = null;}}
					if (ID == null)
					{
						cAdmScreenTab54List_SelectedIndexChanged(sender, e);
					}
				}
			}
		}
		...
		private void SetColumnAuthority()
		{
			...
			{
				base.SetFoldBehavior(cScreenTabId19, dtAuth.Rows[0], cScreenTabId19P1, cScreenTabId19Label, cScreenTabId19P2, null, dtLabel.Rows[0], null, null, null);
				base.SetFoldBehavior(cScreenId19, dtAuth.Rows[1], cScreenId19P1, cScreenId19Label, cScreenId19P2, null, dtLabel.Rows[1], cRFVScreenId19, null, null);
				SetScreenId19(cScreenId19,string.Empty);
				base.SetFoldBehavior(cTabFolderName19, dtAuth.Rows[2], cTabFolderName19P1, cTabFolderName19Label, cTabFolderName19P2, null, dtLabel.Rows[2], cRFVTabFolderName19, null, null);
				base.SetFoldBehavior(cTabFolderOrder19, dtAuth.Rows[3], cTabFolderOrder19P1, cTabFolderOrder19Label, cTabFolderOrder19P2, null, dtLabel.Rows[3], cRFVTabFolderOrder19, null, null);
				base.SetFoldBehavior(cTabFolderLabel, dtAuth.Rows[4], null, null, null, dtLabel.Rows[4], null, null, null);
			}
			if ((cScreenId19.Attributes["OnChange"] == null || cScreenId19.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cScreenId19.Visible && cScreenId19.Enabled) {cScreenId19.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cTabFolderName19.Attributes["OnChange"] == null || cTabFolderName19.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cTabFolderName19.Visible && !cTabFolderName19.ReadOnly) {cTabFolderName19.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cTabFolderOrder19.Attributes["OnChange"] == null || cTabFolderOrder19.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cTabFolderOrder19.Visible && !cTabFolderOrder19.ReadOnly) {cTabFolderOrder19.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
		}
		...
		private void ClearMaster(object sender, System.EventArgs e)
		{
			...
			if (dt.Rows[1]["ColVisible"].ToString() == "Y" && dt.Rows[1]["ColReadOnly"].ToString() != "Y") {cScreenId19.ClearSearch();}
			if (dt.Rows[2]["ColVisible"].ToString() == "Y" && dt.Rows[2]["ColReadOnly"].ToString() != "Y") {cTabFolderName19.Text = string.Empty;}
			if (dt.Rows[3]["ColVisible"].ToString() == "Y" && dt.Rows[3]["ColReadOnly"].ToString() != "Y") {cTabFolderOrder19.Text = string.Empty;}
			// *** Default Value (Folder) Web Rule starts here *** //
		}

		private void InitMaster(object sender, System.EventArgs e)
		{
			cScreenTabId19.Text = string.Empty;
			cScreenId19.ClearSearch();
			cTabFolderName19.Text = string.Empty;
			cTabFolderOrder19.Text = string.Empty;
			// *** Default Value (Folder) Web Rule starts here *** //
		}
		protected void cAdmScreenTab54List_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmScreenTab54List_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmScreenTab54List_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			InitMaster(sender, e);      // Do this first to enable defaults for non-database columns.
			DataTable dt = null;
			try
			{
				dt = (new AdminSystem()).GetMstById("GetAdmScreenTab54ById",cAdmScreenTab54List.SelectedValue,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId));
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				if (dt.Rows.Count > 0)
				{
					cAdmScreenTab54List.SetDdVisible();
					DataRow dr = dt.Rows[0];
					try {cScreenTabId19.Text = RO.Common3.Utils.fmNumeric("0",dr["ScreenTabId19"].ToString(),base.LUser.Culture);} catch {cScreenTabId19.Text = string.Empty;}
					SetScreenId19(cScreenId19,dr["ScreenId19"].ToString());
					try {cTabFolderName19.Text = dr["TabFolderName19"].ToString();} catch {cTabFolderName19.Text = string.Empty;}
					try {cTabFolderOrder19.Text = RO.Common3.Utils.fmNumeric("0",dr["TabFolderOrder19"].ToString(),base.LUser.Culture);} catch {cTabFolderOrder19.Text = string.Empty;}
				}
			}
			cButPanel.DataBind(); if (!cSaveButton.Visible) { cInsRowButton.Visible = false; }
			DataTable dtAdmScreenTabGrid = null;
			int filterId = 0; if (Utils.IsInt(cFilterId.SelectedValue)) { filterId = int.Parse(cFilterId.SelectedValue); }
			try
			{
				dtAdmScreenTabGrid = (new AdminSystem()).GetDtlById(54,"GetAdmScreenTab54DtlById",cAdmScreenTab54List.SelectedValue,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,base.LImpr,base.LCurr);
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (!dtAdmScreenTabGrid.Columns.Contains("_NewRow")) { dtAdmScreenTabGrid.Columns.Add("_NewRow"); }
			Session[KEY_dtAdmScreenTabGrid] = dtAdmScreenTabGrid;
			cAdmScreenTabGrid.EditIndex = -1;
			cAdmScreenTabGridDataPager.PageSize = Int16.Parse(cPgSize.Text); GotoPage(0);
			if (Session[KEY_lastSortCol] != null)
			{
				cAdmScreenTabGrid_OnSorting(sender, new ListViewSortEventArgs((string)Session[KEY_lastSortExp], SortDirection.Ascending));
			}
			else if (dtAdmScreenTabGrid.Rows.Count <= 0 || (!((GetAuthRow().Rows[0]["AllowUpd"].ToString() == "N" || GetAuthRow().Rows[0]["ViewOnly"].ToString() == "G") && GetAuthRow().Rows[0]["AllowAdd"].ToString() == "N") && !(Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N") && dtAdmScreenTabGrid.Rows.Count == 1))
			{
				cAdmScreenTabGrid_DataBind(dtAdmScreenTabGrid.DefaultView);
			}
			cFindButton_Click(sender, e);
			cNaviPanel.Visible = true; cImportPwdPanel.Visible = false; Session.Remove(KEY_scrImport);
			ScriptManager.GetCurrent(Parent.Page).SetFocus(cAdmScreenTab54List.FocusID);
		...
		public void cFindButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dv != null)
			{
				...
				if ((cc != null && cc.ID.Equals("cAdmScreenTab54List")) || cAdmScreenTabGrid.EditIndex < 0 || UpdateGridRow(cAdmScreenTabGrid, new CommandEventArgs("Save", "")))
				{
					string rf = string.Empty;
					if (cFind.Text != string.Empty) { rf = "(" + base.GetExpression(cFind.Text.Trim(), GetAuthCol(), 5, cFindFilter.SelectedValue) + ")"; }
					if (rf != string.Empty) { rf = "((" + rf + " or _NewRow = 'Y' ))"; }
					dv.RowFilter = rf;
					ViewState["_RowFilter"] = rf;
					GotoPage(0); cAdmScreenTabGrid_DataBind(dv);
					if (GetCurrPageIndex() != (int)Session[KEY_currPageIndex] && (int)Session[KEY_currPageIndex] < GetTotalPages()) { GotoPage((int)Session[KEY_currPageIndex]); cAdmScreenTabGrid_DataBind(dv);}
		...
		public void cInsRowButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dt != null && (ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")))
			{
				...
				Session[KEY_lastAddedRow] = 0; Session[KEY_dtAdmScreenTabGrid] = dt; Session[KEY_currPageIndex] = 0; GotoPage(0);
				cAdmScreenTabGrid_DataBind(dt.DefaultView);
				cAdmScreenTabGrid_OnItemEditing(cAdmScreenTabGrid, new ListViewEditEventArgs(0));
		...
		public void cPgSizeButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cAdmScreenTabGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				...
				cAdmScreenTabGridDataPager.PageSize = int.Parse(cPgSize.Text); GotoPage(0);
				if (dt.Rows.Count <= 0 || (!((GetAuthRow().Rows[0]["AllowUpd"].ToString() == "N" || GetAuthRow().Rows[0]["ViewOnly"].ToString() == "G") && GetAuthRow().Rows[0]["AllowAdd"].ToString() == "N") && !(Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N") && dt.Rows.Count == 1)) {cAdmScreenTabGrid_DataBind(dt.DefaultView);} else {cFindButton_Click(sender, e);}
				try
				{
					(new AdminSystem()).UpdLastPageInfo(54, base.LUser.UsrId, cPgSize.Text, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		}
		...
		public void cShowImpButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cSchemaImage.Visible)
			{
				try
				{
					Session["ImportSchema"] = (new AdminSystem()).GetSchemaScrImp(54,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (cSchemaImage.Attributes["OnClick"] == null || cSchemaImage.Attributes["OnClick"].IndexOf("ImportSchema.aspx") < 0) {cSchemaImage.Attributes["OnClick"] += "SearchLink('ImportSchema.aspx?scm=S&key=54&csy=3','','',''); return false;";}
			}
			// *** System Button Click (after) Web Rule starts here *** //
		}
		...
		private void ScrImportDdl(DataView dvd, DataRowView drv, bool bComboBox, string PKey, string CKey, string CNam, int MaxLen, string MatchCd, bool bAllowNulls)
		{
			if (dvd != null)
			{
				...
				{
					if (string.IsNullOrEmpty(cAdmScreenTab54List.SelectedValue))
					{
						dvd.RowFilter = "(" + PKey + " is null)";
					}
					else
					{
						dvd.RowFilter = "(" + PKey + " is null OR " + PKey + " = " + cAdmScreenTab54List.SelectedValue + ")";
					}
				}
		...
		protected void ScrImport(bool bClear)
		{
			...
			DataTable dtAdmScreenTabGrid = (DataTable)Session[KEY_dtAdmScreenTabGrid];
			if (dti != null && dtAdmScreenTabGrid != null)
			{
				if (bClear)
				{
					dtAdmScreenTabGrid.Rows.Clear();
				}
				cFind.Text = string.Empty;
				int iExist = dtAdmScreenTabGrid.Rows.Count;
				...
				foreach (DataRowView drv in dti.DefaultView)
				{
					...
					{
						DataTable dtd;
						dtd = jj==iExist ? null : (DataTable)Session[KEY_dtCultureId80];
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(54,"GetDdlCultureId3S721",true,true,0,string.Empty,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
							Session[KEY_dtCultureId80] = dtd;
						}
						ScrImportDdl(dtd.DefaultView, drv, true, "ScreenTabId", "CultureId80", "CultureId80Text", 50, "2", false);
					}
					jj = jj + 1;
				}
				iRow = dti.DefaultView.Count - iExist;
				if (!dti.Columns.Contains("_NewRow")) { dti.Columns.Add("_NewRow"); }
				Session[KEY_dtAdmScreenTabGrid] = dti;
				cAdmScreenTabGrid_DataBind(dti.DefaultView);
		...
		public void cImportButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cFNameO.Text != string.Empty && cWorkSheet.Items.Count > 0 && cWorkSheet.SelectedItem.Text != string.Empty && cStartRow.Text != string.Empty)
			{
				...
				try
				{
					if (cAdmScreenTabGrid.EditIndex < 0 || UpdateGridRow(cAdmScreenTabGrid, new CommandEventArgs("Save", "")))
					{
						Session[KEY_scrImport] = ImportFile(dtAdmScreenTabGrid.Copy(),cFNameO.Text,cWorkSheet.SelectedItem.Text,cStartRow.Text,Config.PathTmpImport + cFName.Text);
					}
		...
		private void GotoPage(int pageNo)
		{
			if (pageNo >= 0 && pageNo < GetTotalPages())
			{
				if (cAdmScreenTabGrid.EditIndex < 0 || UpdateGridRow(cAdmScreenTabGrid, new CommandEventArgs("Save", "")))
				{
					try { cAdmScreenTabGridDataPager.SetPageProperties(cAdmScreenTabGridDataPager.PageSize * pageNo, cAdmScreenTabGridDataPager.MaximumRows, false); cAdmScreenTabGrid_DataBind(null); }
					catch
					{
						try { cAdmScreenTabGridDataPager.SetPageProperties(0, cAdmScreenTabGridDataPager.MaximumRows, false); cAdmScreenTabGrid_DataBind(null); } catch {}
					}
				}
			}
		}

		protected void cAdmScreenTabGrid_OnItemCommand(object sender, ListViewCommandEventArgs e)
		{
		}

		private void cAdmScreenTabGrid_DataBind(DataView dvAdmScreenTabGrid)
		{
			if (dvAdmScreenTabGrid == null)
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmScreenTabGrid];
				dvAdmScreenTabGrid = dt != null ? dt.DefaultView : null;
			}
			if (dvAdmScreenTabGrid != null)
			{
				LcAuth = GetAuthCol().AsEnumerable().ToDictionary<DataRow,string>(dr=>dr["ColName"].ToString());
				cAdmScreenTabGrid.DataSource = dvAdmScreenTabGrid;
				cAdmScreenTabGrid.DataBind();
			...
			if (cFindFilter.Items.Count <= 0)
			{
				...
				{
					if (ii >= 5 && !string.IsNullOrEmpty(dr["ColumnHeader"].ToString()) && !string.IsNullOrEmpty(dr["TableId"].ToString()) && dtAuth.Rows[ii]["ColVisible"].ToString() == "Y")
					{
						li = new ListItem();
		...
		protected void cAdmScreenTabGrid_OnSorting(object sender, ListViewSortEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmScreenTabGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null)
			{
				if (cAdmScreenTabGrid.EditIndex < 0 || UpdateGridRow(cAdmScreenTabGrid, new CommandEventArgs("Save", "")))
				{
					...
					Session[KEY_dtAdmScreenTabGrid] = dt;
					cAdmScreenTabGrid_DataBind(dv);
				}
			}
		}

		protected void cAdmScreenTabGrid_OnPagePropertiesChanging(object sender, PagePropertiesChangingEventArgs e)
		{
		    cAdmScreenTabGridDataPager.SetPageProperties(e.StartRowIndex, e.MaximumRows, false);
		    DataTable dt = (DataTable)Session[KEY_dtAdmScreenTabGrid];
		    DataView dv = dt != null ? dt.DefaultView : null;
			cAdmScreenTabGrid_DataBind(dv); cAdmScreenTabGrid.EditIndex = -1;
		}

		private void GridChkPgDirty(ListViewItem lvi)
		{
				WebControl cc = null;
				cc = ((WebControl)lvi.FindControl("cCultureId80"));
				...
				cc = ((WebControl)lvi.FindControl("cTabFolderName80"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
		}

		protected void cAdmScreenTabGrid_OnItemDataBound(object sender, ListViewItemEventArgs e)
		{
			// *** GridItemDataBound (before) Web Rule End *** //
			DataTable dt = (DataTable)Session[KEY_dtAdmScreenTabGrid];
			bool isEditItem = false;
			DataView dvAdmScreenTabGrid = dt != null ? dt.DefaultView : null;
			if (cAdmScreenTabGrid.EditIndex > -1 && GetDataItemIndex(cAdmScreenTabGrid.EditIndex) == e.Item.DataItemIndex)
			{
				isEditItem = true;
				base.SetGridEnabled(e.Item, GetAuthCol(), GetLabel(), 5);
				if (dvAdmScreenTabGrid[e.Item.DataItemIndex]["CultureId80"].ToString() != string.Empty) {SetCultureId80((RoboCoder.WebControls.ComboBox)e.Item.FindControl("cCultureId80"),dvAdmScreenTabGrid[e.Item.DataItemIndex]["CultureId80"].ToString(), e.Item);} else {SetCultureId80((RoboCoder.WebControls.ComboBox)e.Item.FindControl("cCultureId80"),"1", e.Item);}
			...
			if (dtAuthRow != null)
			{
				...
				{
					cc = e.Item.FindControl("cAdmScreenTabGridDelete"); if (cc != null) { cc.Visible = false; }
					cc = e.Item.FindControl("cAdmScreenTabGridEdit"); if (cc != null) { cc.Visible = false; }
				}
				else
				{
			        HtmlTableRow tr = e.Item.FindControl("cAdmScreenTabGridRow") as HtmlTableRow;
			        LinkButton lb = e.Item.FindControl("cAdmScreenTabGridEdit") as LinkButton;
		...
		protected void cScreenTabHlpId80hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "0") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "0"; Session[KEY_lastSortExp] = "ScreenTabHlpId80";Session[KEY_lastSortImg] = "cScreenTabHlpId80hi";
			cAdmScreenTabGrid_OnSorting(sender, new ListViewSortEventArgs("ScreenTabHlpId80", SortDirection.Ascending));
		}

		protected void cCultureId80hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "1") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "1"; Session[KEY_lastSortExp] = "CultureId80Text";Session[KEY_lastSortImg] = "cCultureId80hi";
			cAdmScreenTabGrid_OnSorting(sender, new ListViewSortEventArgs("CultureId80Text", SortDirection.Ascending));
		}

		protected void cTabFolderName80hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "2") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "2"; Session[KEY_lastSortExp] = "TabFolderName80";Session[KEY_lastSortImg] = "cTabFolderName80hi";
			cAdmScreenTabGrid_OnSorting(sender, new ListViewSortEventArgs("TabFolderName80", SortDirection.Ascending));
		}

		protected void cAdmScreenTabGrid_OnItemEditing(object sender, ListViewEditEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmScreenTabGrid];
			if ((ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")) && dt != null && dt.DefaultView.Count > GetDataItemIndex(e.NewEditIndex))
			{
				cAdmScreenTabGrid.EditIndex = e.NewEditIndex;
				cAdmScreenTabGrid_DataBind(null);
				Control wc = null;
				string idx = Page.Request["__EVENTARGUMENT"];
				if (!string.IsNullOrEmpty(idx))
				{
				    Control ctrlToFocus = cAdmScreenTabGrid.EditItem.FindControl("c" + idx);
				    if (((WebControl)ctrlToFocus).Enabled) { wc = ctrlToFocus; }
				}
				if (wc == null) { wc = FindEditableControl(cAdmScreenTabGrid.EditItem); }
				if (wc != null)
             {
             ...
             MkMobileLabel(cAdmScreenTabGrid.EditItem);
		    }
		    else { e.Cancel = true; }
		}
		...
		{
		    Label ml = null;
		    ml = lvi.FindControl("cScreenTabHlpId80ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(5); }
		    ml = lvi.FindControl("cCultureId80ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(6); }
		    ml = lvi.FindControl("cTabFolderName80ml") as Label;
		...
		protected void GridFill(ListViewItem lvi, DataTable dt, DataRow dr, bool bInsert)
		{
			...
			cbb = (RoboCoder.WebControls.ComboBox)lvi.FindControl("cCultureId80");
			if (cbb != null)
			{
				if (cbb.SelectedValue != string.Empty)
				{
					dr["CultureId80"] = cbb.SelectedValue;
					dr["CultureId80Text"] = cbb.SelectedText;
				}
				else
				{
					dr["CultureId80"] = System.DBNull.Value;
					dr["CultureId80Text"] = System.DBNull.Value;
				}
			}
			TextBox tb = null;
			tb = (TextBox)lvi.FindControl("cTabFolderName80");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["TabFolderName80"] = tb.Text;} else {dr["TabFolderName80"] = System.DBNull.Value;}
			}
		...
		protected void cAdmScreenTabGrid_OnItemUpdating(object sender, ListViewUpdateEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmScreenTabGrid];
			if (dt != null && dt.Rows.Count > 0)
			{
				DataRow dr = dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row;
			    GridFill(cAdmScreenTabGrid.EditItem, dt, dr, false);
			    dr["_NewRow"] = "N";
			    if (dt.Columns.Contains("_SortOrder")) dr["_SortOrder"] = null;
			}
		}

		protected void cAdmScreenTabGrid_OnItemCanceling(object sender, ListViewCancelEventArgs e)
		{
			PanelTop.Update();
			DataTable dt = (DataTable)Session[KEY_dtAdmScreenTabGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			...
			{
				cAdmScreenTabGrid_OnItemDeleting(sender, new ListViewDeleteEventArgs(e.ItemIndex));
			}
			if (dt.Columns.Contains("_SortOrder")) dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row["_SortOrder"] = null;
			cAdmScreenTabGrid.EditIndex = -1; cAdmScreenTabGrid_DataBind(null);
		}
		...
		protected void cDeleteAllButton_Click(object sender, System.EventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmScreenTabGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null && (cAdmScreenTabGrid.EditIndex < 0 || UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				GotoPage(0);
				while (dv.Count > 0) {dv.Delete(0);}
				Session[KEY_dtAdmScreenTabGrid] = dt;
				cAdmScreenTabGrid.EditIndex = -1; cAdmScreenTabGrid_DataBind(dv);
			}
		}

		protected void cAdmScreenTabGrid_OnItemDeleting(object sender, ListViewDeleteEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmScreenTabGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			DataRow dr = dv != null ? dv[GetDataItemIndex(e.ItemIndex)].Row : null;
			if (dv != null && (cAdmScreenTabGrid.EditIndex == e.ItemIndex || UpdateGridRow(sender, new CommandEventArgs("Save",""))))
			{
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cScreenTabId19.Text == string.Empty) { cClearButton_Click(sender, new EventArgs()); ShowDirty(false); PanelTop.Update();}
			else { PopAdmScreenTab54List(sender, e, false, cScreenTabId19.Text); }
			// *** System Button Click (After) Web Rule starts here *** //
		}
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmScreenTab54List); PopAdmScreenTab54List(sender, e, false, null);
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			if (ValidPage() && (cAdmScreenTabGrid.EditIndex < 0 || UpdateGridRow(cAdmScreenTabGrid, new CommandEventArgs("Save", ""))))
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmScreenTabGrid];
				DataView dv = dt != null ? dt.DefaultView : null;
				string ftr = string.Empty;
				if (dv.RowFilter != string.Empty) { ftr = dv.RowFilter; dv.RowFilter = string.Empty; }
				AdmScreenTab54 ds = PrepAdmScreenTabData(dv,cScreenTabId19.Text == string.Empty);
				if (ftr != string.Empty) {dv.RowFilter = ftr;}
				if (!string.IsNullOrEmpty(cAdmScreenTab54List.SelectedValue) && cScreenTabId19.Text != cAdmScreenTab54List.SelectedValue)
				{
					PreMsgPopup("Primary key cannot be changed");
					return rtn;
				}
				if (string.IsNullOrEmpty(cAdmScreenTab54List.SelectedValue))	// Add
				{
					if (ds != null)
					{
						pid = (new AdminSystem()).AddData(54,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc,noTrans,commandTimeOut);
					}
					if (!string.IsNullOrEmpty(pid))
					{
						cAdmScreenTab54List.ClearSearch(); Session.Remove(KEY_dtAdmScreenTab54List);
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmScreenTab54List(sender, e, false, pid);
						rtn = GetScreenHlp().Rows[0]["AddMsg"].ToString();
					}
				}
				else {
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(54,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc,noTrans,commandTimeOut)) {bValid7 = true;}
					if (bValid7)
					{
						cAdmScreenTab54List.ClearSearch(); Session.Remove(KEY_dtAdmScreenTab54List);
						Session[KEY_currPageIndex] = GetCurrPageIndex();
						ShowDirty(false); PopAdmScreenTab54List(sender, e, false, ds.Tables["AdmScreenTab"].Rows[0]["ScreenTabId19"]);
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cScreenTabId19.Text != string.Empty)
			{
				AdmScreenTab54 ds = PrepAdmScreenTabData(null,false);
				try
				{
					if (ds != null && (new AdminSystem()).DelData(54,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc))
					{
						cAdmScreenTab54List.ClearSearch(); Session.Remove(KEY_dtAdmScreenTab54List);
						ShowDirty(false); PopAdmScreenTab54List(sender, e, false, null);
		...
		private AdmScreenTab54 PrepAdmScreenTabData(DataView dv, bool bAdd)
		{
			...
			if (bAdd) { dr["ScreenTabId19"] = string.Empty; } else { dr["ScreenTabId19"] = cScreenTabId19.Text; }
			drType["ScreenTabId19"] = "Numeric"; drDisp["ScreenTabId19"] = "TextBox";
			try {dr["ScreenId19"] = cScreenId19.SelectedValue;} catch {}
			drType["ScreenId19"] = "Numeric"; drDisp["ScreenId19"] = "AutoComplete";
			try {dr["TabFolderName19"] = cTabFolderName19.Text.Trim();} catch {}
			drType["TabFolderName19"] = "VarWChar"; drDisp["TabFolderName19"] = "TextBox";
			try {dr["TabFolderOrder19"] = cTabFolderOrder19.Text.Trim();} catch {}
			drType["TabFolderOrder19"] = "Numeric"; drDisp["TabFolderOrder19"] = "TextBox";
			try {dr["TabFolderLabel"] = cTabFolderLabel.Text;} catch {}
			...
			if (dv != null)
			{
				...
				if (bAdd)
				{
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmScreenTabAdd"].Rows.Add(MakeColRow(ds.Tables["AdmScreenTabAdd"].NewRow(), drv, true));
					}
				}
				else
				{
					...
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmScreenTabUpd"].Rows.Add(MakeColRow(ds.Tables["AdmScreenTabUpd"].NewRow(), drv, false));
					}
					...
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmScreenTabDel"].Rows.Add(MakeColRow(ds.Tables["AdmScreenTabDel"].NewRow(), drv, false));
					}
		...
		public bool CanAct(char typ) { return CanAct(typ, GetAuthRow(), cAdmScreenTab54List.SelectedValue.ToString()); }
		...
		{
			dr["ScreenTabId19"] = cScreenTabId19.Text;
			DataTable dtAuth = GetAuthCol();
			if (dtAuth != null)
			{
				dr["ScreenTabHlpId80"] = drv["ScreenTabHlpId80"].ToString().Trim();
				dr["CultureId80"] = drv["CultureId80"];
				dr["TabFolderName80"] = drv["TabFolderName80"].ToString().Trim();
			}
			return dr;
		}
		...
		{
			if (cAdmScreenTabGrid.EditIndex > -1)
			{
				cAdmScreenTabGrid_OnItemUpdating(sender, new ListViewUpdateEventArgs(cAdmScreenTabGrid.EditIndex));
			}
			return true;
		}

		protected void cAdmScreenTabGrid_OnPreRender(object sender, System.EventArgs e)
		{
			System.Web.UI.WebControls.Image hi = null;
			hi = (System.Web.UI.WebControls.Image)cAdmScreenTabGrid.FindControl("cScreenTabHlpId80hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmScreenTabGrid.FindControl("cCultureId80hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmScreenTabGrid.FindControl("cTabFolderName80hi"); if (hi != null) { hi.Visible = false; }
			if (Session[KEY_lastSortImg] != null)
			{
				hi = (System.Web.UI.WebControls.Image)cAdmScreenTabGrid.FindControl((string)Session[KEY_lastSortImg]);
				if (hi != null) { hi.ImageUrl = Utils.AddTilde((string)Session[KEY_lastSortUrl]); hi.Visible = true; }
			}
			IgnoreHeaderConfirm((LinkButton)cAdmScreenTabGrid.FindControl("cScreenTabHlpId80hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmScreenTabGrid.FindControl("cCultureId80hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmScreenTabGrid.FindControl("cTabFolderName80hl"));
		}

		private string GetButtonId(ListViewItem lvi)
		{
			string ButtonID = String.Empty;
			Control c = lvi.FindControl("cAdmScreenTabGridEdit");
		...
		{
		    return cAdmScreenTabGridDataPager.StartRowIndex / cAdmScreenTabGridDataPager.PageSize;
		}

		private int GetTotalPages()
		{
		    return (int)Math.Ceiling((double)cAdmScreenTabGridDataPager.TotalRowCount / cAdmScreenTabGridDataPager.PageSize);
		}

		private int GetDataItemIndex(int editIndex)
		{
		    return cAdmScreenTabGridDataPager.StartRowIndex + editIndex;
		}

		protected void cAdmScreenTabGrid_OnLayoutCreated(object sender, EventArgs e)
		{
		    // Header:
		    LinkButton lb = null;
		    lb = cAdmScreenTabGrid.FindControl("cScreenTabHlpId80hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(5); lb.ToolTip = ColumnToolTip(5); lb.Parent.Visible = GridColumnVisible(5); }
		    lb = cAdmScreenTabGrid.FindControl("cCultureId80hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(6); lb.ToolTip = ColumnToolTip(6); lb.Parent.Visible = GridColumnVisible(6); }
		    lb = cAdmScreenTabGrid.FindControl("cTabFolderName80hl") as LinkButton;
			...
			if (dtAuthRow != null)
			{
				...
				{
		            lb = cAdmScreenTabGrid.FindControl("cDeleteAllButton") as LinkButton; if (lb != null) { lb.Visible = false; }
				}
			}
		    // footer:
		    Label gc = null;
		    gc = cAdmScreenTabGrid.FindControl("cScreenTabHlpId80fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(5); }
		    gc = cAdmScreenTabGrid.FindControl("cCultureId80fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(6); }
		    gc = cAdmScreenTabGrid.FindControl("cTabFolderName80fl") as Label;
		...
		private void PreMsgPopup(string msg) { PreMsgPopup(msg,cAdmScreenTab54List,null); }

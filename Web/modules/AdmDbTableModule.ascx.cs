		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			if (ValidPage() && (cAdmDbTableGrid.EditIndex < 0 || UpdateGridRow(cAdmDbTableGrid, new CommandEventArgs("Save", ""))))
			{
				...
				if (string.IsNullOrEmpty(cAdmDbTable2List.SelectedValue))	// Add
				{
					if (ds != null)
					{
						pid = (new AdminSystem()).AddData(2,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc);
					}
					...
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(2,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc)) {bValid7 = true;}
					if (bValid7)
					{
						cAdmDbTable2List.ClearSearch(); Session.Remove(KEY_dtAdmDbTable2List);

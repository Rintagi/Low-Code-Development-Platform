		...
		private void getReport(string eExport)
		{
			try
			{
				...
				if (eExport == "TXT")
				{
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmMember.csv"; Session["ExportStr"] = (Config.ExportExcelCSV ? "sep=\t\n": "") + sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
		...
		private bool UpdateGridRow(object sender, CommandEventArgs e)
		{
			...
			{
				cAdmMemberGrid_OnItemUpdating(sender, new ListViewUpdateEventArgs(cAdmMemberGrid.EditIndex));
			}
			return true;
		}

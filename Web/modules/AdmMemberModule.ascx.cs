	...
	public partial class AdmMemberModule : RO.Web.ModuleBase
	{
		...
		private void SetSystem(byte SystemId)
		{
			...
			try
			{
				...
				{
					(new GenScreensSystem()).CreateProgram(122, "Member for SR&ED", row["dbAppDatabase"].ToString(), base.CPrj, base.CSrc, base.CTar, LcAppConnString, LcAppPw);
...
\par }}");
		...
		protected void cAdmMemberGrid_OnItemDataBound(object sender, ListViewItemEventArgs e)
		{
			...
			if (cAdmMemberGrid.EditIndex > -1 && GetDataItemIndex(cAdmMemberGrid.EditIndex) == e.Item.DataItemIndex)
			{
			    ImageButton ImageGridDisplay = e.Item.FindControl("cPicMed275") as ImageButton;
			    ((FileUpload)e.Item.FindControl("cPicMed275Fi")).Attributes["onchange"] = "if('" + dvAdmMemberGrid[e.Item.DataItemIndex]["SredMebrId274"].ToString() + "'==''){PopDialog('','Please save the record first before upload.','');}else{sendFile(this.files[0],'" + GetUrlWithQSHash("UpLoad.aspx?key=" + dvAdmMemberGrid[e.Item.DataItemIndex]["SredMebrId274"].ToString() + "&tbl=dbo.VwIntUsr&knm=SredMebrId&col=PicMed&hgt=25&sys=" + base.LCurr.SystemId.ToString()) + "',refreshUploadCallback(this,'" + ImageGridDisplay.ClientID + "')); return false;} ";
			    ((ImageButton)e.Item.FindControl("cPicMed275Del")).Attributes["onclick"] = "sendFile('','" + GetUrlWithQSHash("UpLoad.aspx?del=true&key=" + dvAdmMemberGrid[e.Item.DataItemIndex]["SredMebrId274"].ToString() + "&tbl=dbo.VwIntUsr&knm=SredMebrId&col=PicMed&hgt=25&sys=" + base.LCurr.SystemId.ToString()) + "',refreshUploadCallback(this,'" + ImageGridDisplay.ClientID + "'));return false;";
			}

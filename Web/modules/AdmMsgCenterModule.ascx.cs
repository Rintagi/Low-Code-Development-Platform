...
{
	public class AdmMsgCenter86 : DataSet
	{
		public AdmMsgCenter86()
		{
			this.Tables.Add(MakeColumns(new DataTable("AdmMsgCenter")));
			this.Tables.Add(MakeDtlColumns(new DataTable("AdmMsgCenterDef")));
			this.Tables.Add(MakeDtlColumns(new DataTable("AdmMsgCenterAdd")));
			this.Tables.Add(MakeDtlColumns(new DataTable("AdmMsgCenterUpd")));
			this.Tables.Add(MakeDtlColumns(new DataTable("AdmMsgCenterDel")));
			this.DataSetName = "AdmMsgCenter86";
			this.Namespace = "http://Rintagi.com/DataSet/AdmMsgCenter86";
		}

		private DataTable MakeColumns(DataTable dt)
		{
			DataColumnCollection columns = dt.Columns;
			columns.Add("MsgId146", typeof(string));
			columns.Add("MsgTypeCd146", typeof(string));
			columns.Add("MsgSource146", typeof(string));
			return dt;
		}
		...
		private DataTable MakeDtlColumns(DataTable dt)
		{
			DataColumnCollection columns = dt.Columns;
			columns.Add("MsgId146", typeof(string));
			columns.Add("MsgCenterId147", typeof(string));
			columns.Add("CultureId147", typeof(string));
			columns.Add("Msg147", typeof(string));
			return dt;
		}
	}
}
...
namespace RO.Web
{
	public partial class AdmMsgCenterModule : RO.Web.ModuleBase
	{

		private const string KEY_lastAddedRow = "Cache:lastAddedRow3_86";
		private const string KEY_lastSortOrd = "Cache:lastSortOrd3_86";
		private const string KEY_lastSortImg = "Cache:lastSortImg3_86";
		private const string KEY_lastSortCol = "Cache:lastSortCol3_86";
		private const string KEY_lastSortExp = "Cache:lastSortExp3_86";
		private const string KEY_lastSortUrl = "Cache:lastSortUrl3_86";
		private const string KEY_lastSortTog = "Cache:lastSortTog3_86";
		private const string KEY_lastImpPwdOvride = "Cache:lastImpPwdOvride3_86";
		private const string KEY_cntImpPwdOvride = "Cache:cntImpPwdOvride3_86";
		private const string KEY_currPageIndex = "Cache:currPageIndex3_86";
		private const string KEY_bHiImpVisible = "Cache:bHiImpVisible3_86";
		private const string KEY_bShImpVisible = "Cache:bShImpVisible3_86";
		private const string KEY_dtAdmMsgCenterGrid = "Cache:dtAdmMsgCenterGrid";
		private const string KEY_scrImport = "Cache:scrImport3_86";
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_86";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_86";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_86";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_86";
		private const string KEY_dtLabel = "Cache:dtLabel3_86";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_86";
		private const string KEY_dtScrCri = "Cache:dtScrCri3_86";
		private const string KEY_dsScrCriVal = "Cache:dsScrCriVal3_86";

		private const string KEY_dtMsgTypeCd146 = "Cache:dtMsgTypeCd146";
		private const string KEY_dtCultureId147 = "Cache:dtCultureId147";

		private const string KEY_dtSystems = "Cache:dtSystems3_86";
		private const string KEY_sysConnectionString = "Cache:sysConnectionString3_86";
		private const string KEY_dtAdmMsgCenter86List = "Cache:dtAdmMsgCenter86List";
		private const string KEY_bNewVisible = "Cache:bNewVisible3_86";
		private const string KEY_bNewSaveVisible = "Cache:bNewSaveVisible3_86";
		private const string KEY_bCopyVisible = "Cache:bCopyVisible3_86";
		private const string KEY_bCopySaveVisible = "Cache:bCopySaveVisible3_86";
		private const string KEY_bDeleteVisible = "Cache:bDeleteVisible3_86";
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_86";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_86";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_86";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				DataTable dtMenuAccess = (new MenuSystem()).GetMenu(base.LUser.CultureId, 3, base.LImpr, LcSysConnString, LcAppPw,86, null, null);
				...
				Session.Remove(KEY_dtMsgTypeCd146);
				Session.Remove(KEY_dtCultureId147);
				SetButtonHlp();
				GetSystems();
				SetColumnAuthority();
				GetGlobalFilter();
				...
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 86, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmMsgCenter86List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=86&typ=N&sys=3','','',''); return false;"; }
			...
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmMsgCenterGrid];
				if (dt != null)
				{
		...
		private void InitializeComponent()
		{
			...
			if (LcSysConnString == null)
			{
				...
				{
					DataTable dt;
					dt = (new AdminSystem()).GetAuthRow(86,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
					...
					{
						cAdmMsgCenterGrid.PreRender += new EventHandler(cAdmMsgCenterGrid_OnPreRender);
					}
				}
		...
		private void SetSystem(byte SystemId)
		{
			...
			try
			{
				...
				if ((Config.DeployType == "DEV" || row["dbAppDatabase"].ToString() == base.CPrj.EntityCode + "View") && !(base.CPrj.EntityCode != "RO" && row["SysProgram"].ToString() == "Y") && (new AdminSystem()).IsRegenNeeded(string.Empty,86,0,0,LcSysConnString,LcAppPw))
				{
					(new GenScreensSystem()).CreateProgram(86, "Message Center", row["dbAppDatabase"].ToString(), base.CPrj, base.CSrc, base.CTar, LcAppConnString, LcAppPw);
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(86,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(86,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void SetClientRule(ListViewDataItem ee, bool isEditItem)
		{
			...
			if (dvRul.Count > 0)
			{
				...
				{
					if (ee == null || drv["ScriptEvent"].ToString().Substring(0,2).ToLower() != "on" || (cAdmMsgCenterGrid.EditIndex > -1 && GetDataItemIndex(cAdmMsgCenterGrid.EditIndex) == ee.DataItemIndex))
					{
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(86,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(86,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,86,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(86,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(86,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(86,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				try
				{
					try {dv = new DataView((new AdminSystem()).GetExp(86,"GetExpAdmMsgCenter86","Y",(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
					catch {dv = new DataView((new AdminSystem()).GetExp(86,"GetExpAdmMsgCenter86","N",(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(86,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					{
						if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["MsgId146"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["MsgTypeCd146"].ToString().Replace("\"","\"\"") + "\"" + (char)9 + "\"" + drv["MsgTypeCd146Text"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["MsgSource146"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["MsgCenterId147"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append(drv["CultureId147"].ToString() + (char)9 + drv["CultureId147Text"].ToString() + (char)9);}
						if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["Msg147"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						sb.Append(Environment.NewLine);
					}
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmMsgCenter.csv"; Session["ExportStr"] = (Config.ExportExcelCSV ? "sep=\t\n": "") + sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmMsgCenter.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private string GetRtfGrid(DataView dv)
		{
			...
			try
			{
				...
				foreach (DataRowView drv in dv)
				{
					...
					sb.Append("\r\n");
					sb.Append(@"{");
					if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["MsgId146"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["MsgTypeCd146Text"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(drv["MsgSource146"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["MsgCenterId147"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append(drv["CultureId147Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append(drv["Msg147"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					sb.Append(@"}");
					sb.Append("\r\n");
					sb.Append(@"\pard \ql \li0\ri0\widctlpar\intbl\aspalpha\aspnum\adjustright\rin0\lin0 {");
					sb.Append(@"\insrsid2981395 \trowd \irow0\irowband0\lastrow \ts15\trgaph108\trleft-108\trbrdrt\brdrs\brdrw10 \trbrdrl\brdrs\brdrw10 \trbrdrb\brdrs\brdrw10 \trbrdrr\brdrs\brdrw10 \trbrdrh\brdrs\brdrw10 \trbrdrv\brdrs\brdrw10 ");
		...
		private void GetPageInfo()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastPageInfo(86, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 86, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			// *** Criteria Trigger (before) Web Rule starts here *** //
			DataTable dt = (DataTable)Session[KEY_dtAdmMsgCenterGrid];
			if (cAdmMsgCenterGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
			if (IsPostBack && cCriteria.Visible) ReBindCriteria(GetScrCriteria());
			UpdCriteria(true);
			cAdmMsgCenter86List.ClearSearch(); Session.Remove(KEY_dtAdmMsgCenter86List);
			PopAdmMsgCenter86List(sender, e, false, null);
			}
		}
			...
			try
			{
				foreach (DataRowView drv in dvCri)
				{
					...
					if (drv["DisplayName"].ToString() == "ComboBox")
					{
						...
						(new AdminSystem()).MkGetScreenIn("86", drv["ScreenCriId"].ToString(), "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), LcAppDb, LcDesDb, drv["MultiDesignDb"].ToString(), LcSysConnString, LcAppPw);
						DataView dv = new DataView((new AdminSystem()).GetScreenIn("86", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), (new AdminSystem()).CountScrCri(drv["ScreenCriId"].ToString(), drv["MultiDesignDb"].ToString(), drv["MultiDesignDb"].ToString() == "N" ? LcSysConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)), drv["RequiredValid"].ToString(), 0, string.Empty, drv["DisplayMode"].ToString() != "AutoListBox", val, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)));
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("86", LcSysConnString, LcAppPw);
				}
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmMsgCenter86" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmMsgCenter86" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			foreach (DataRowView drv in dvCri)
			{
			    if (drv["DisplayName"].ToString() == "ListBox")
			    {
					...
					if (cListBox != null)
					{
						...
						int TotalChoiceCnt = new DataView((new AdminSystem()).GetScreenIn("86", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), CriCnt, drv["RequiredValid"].ToString(), 0, string.Empty, true, string.Empty, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId))).Count;
						string selectedValues = string.Join(",", cListBox.Items.Cast<ListItem>().Where(x => x.Selected).Select(x => "'" + x.Value + "'").ToArray());
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("86", "AdmMsgCenter86", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		private void SetMsgTypeCd146(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtMsgTypeCd146];
			...
			if (ddl != null)
			{
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(86,"GetDdlMsgTypeCd3S1409",true,bAll,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					if (dv.Table.Columns.Contains("MsgId"))
					{
						ss = "(MsgId is null";
						if (string.IsNullOrEmpty(cAdmMsgCenter86List.SelectedValue)) {ss = ss + ")";} else {ss = ss + " OR MsgId = " + cAdmMsgCenter86List.SelectedValue + ")";}
					}
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "MsgTypeCd146 is not NULL"; }
					dv.RowFilter = ss;
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(86,"GetDdlMsgTypeCd3S1409",true,true,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtMsgTypeCd146] = dv.Table;
				}
			}
		}

		protected void cbCultureId147(object sender, System.EventArgs e)
		{
			SetCultureId147((RoboCoder.WebControls.ComboBox)sender,string.Empty,cAdmMsgCenterGrid.EditItem);
		}

		private void SetCultureId147(RoboCoder.WebControls.ComboBox ddl, string keyId, ListViewItem li)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			context["method"] = "GetDdlCultureId3S1414";
			context["addnew"] = "Y";
			context["mKey"] = "CultureId147";
			context["mVal"] = "CultureId147Text";
			context["mTip"] = "CultureId147Text";
			context["mImg"] = "CultureId147Text";
			context["ssd"] = Request.QueryString["ssd"];
			context["scr"] = "86";
			context["csy"] = "3";
			context["filter"] = Utils.IsInt(cFilterId.SelectedValue)? cFilterId.SelectedValue : "0";
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(86,"GetDdlCultureId3S1414",true,false,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
					if (dv.Table.Columns.Contains("MsgId"))
					{
						context["pMKeyColID"] = cAdmMsgCenter86List.ClientID;
						context["pMKeyCol"] = "MsgId";
						string ss = "(MsgId is null";
						if (string.IsNullOrEmpty(cAdmMsgCenter86List.SelectedValue)) {ss = ss + ")";} else {ss = ss + " OR MsgId = " + cAdmMsgCenter86List.SelectedValue + ")";}
						dv.RowFilter = ss;
					}
					ddl.DataSource = dv; Session[KEY_dtCultureId147] = dv.Table;
				    try { ddl.SelectByValue(keyId,string.Empty,true); } catch { try { ddl.SelectedIndex = 0; } catch { } }
				}
			}
		}

		private DataView GetAdmMsgCenter86List()
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmMsgCenter86List];
			if (dt == null)
			{
				CheckAuthentication(false);
				int filterId = 0;
				...
				try
				{
					try {dt = (new AdminSystem()).GetLis(86,"GetLisAdmMsgCenter86",true,"Y",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
					catch {dt = (new AdminSystem()).GetLis(86,"GetLisAdmMsgCenter86",true,"N",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				}
		...
		private void PopAdmMsgCenter86List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			DataView dv = GetAdmMsgCenter86List();
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			context["method"] = "GetLisAdmMsgCenter86";
			context["mKey"] = "MsgId146";
			context["mVal"] = "MsgId146Text";
			context["mTip"] = "MsgId146Text";
			context["mImg"] = "MsgId146Text";
			context["ssd"] = Request.QueryString["ssd"];
			context["scr"] = "86";
			context["csy"] = "3";
			...
			context["isSys"] = "N";
			context["conn"] = KEY_sysConnectionString;
			cAdmMsgCenter86List.AutoCompleteUrl = "AutoComplete.aspx/LisSuggests";
			cAdmMsgCenter86List.DataContext = context;
			if (dv.Table == null) return;
			cAdmMsgCenter86List.DataSource = dv;
			cAdmMsgCenter86List.Visible = true;
			if (cAdmMsgCenter86List.Items.Count <= 0) {cAdmMsgCenter86List.Visible = false; cAdmMsgCenter86List_SelectedIndexChanged(sender,e);}
			else
			{
				if (bPageLoad && !string.IsNullOrEmpty(Request.QueryString["key"]) && bUseCri.Value == string.Empty)
				{
					try {cAdmMsgCenter86List.SelectByValue(Request.QueryString["key"],string.Empty,true);} catch {cAdmMsgCenter86List.Items[0].Selected = true; cAdmMsgCenter86List_SelectedIndexChanged(sender, e);}
				    cScreenSearch.Visible = false; cSystem.Visible = false; cEditButton.Visible = true; cSaveCloseButton.Visible = cSaveButton.Visible;
				}
				else
				{
					if (ID != null) {if (!cAdmMsgCenter86List.SelectByValue(ID,string.Empty,true)) {ID = null;}}
					if (ID == null)
					{
						cAdmMsgCenter86List_SelectedIndexChanged(sender, e);
					}
				}
			}
		}
		...
		private void SetColumnAuthority()
		{
			...
			{
				base.SetFoldBehavior(cMsgId146, dtAuth.Rows[0], cMsgId146P1, cMsgId146Label, cMsgId146P2, null, dtLabel.Rows[0], null, null, null);
				base.SetFoldBehavior(cMsgTypeCd146, dtAuth.Rows[1], cMsgTypeCd146P1, cMsgTypeCd146Label, cMsgTypeCd146P2, null, dtLabel.Rows[1], cRFVMsgTypeCd146, null, null);
				base.SetFoldBehavior(cMsgSource146, dtAuth.Rows[2], cMsgSource146P1, cMsgSource146Label, cMsgSource146P2, null, dtLabel.Rows[2], cRFVMsgSource146, null, null);
			}
			if ((cMsgTypeCd146.Attributes["OnChange"] == null || cMsgTypeCd146.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cMsgTypeCd146.Visible && cMsgTypeCd146.Enabled) {cMsgTypeCd146.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cMsgSource146.Attributes["OnChange"] == null || cMsgSource146.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cMsgSource146.Visible && !cMsgSource146.ReadOnly) {cMsgSource146.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
		}
		...
		private void ClearMaster(object sender, System.EventArgs e)
		{
			...
			if (dt.Rows[1]["ColVisible"].ToString() == "Y" && dt.Rows[1]["ColReadOnly"].ToString() != "Y") {SetMsgTypeCd146(cMsgTypeCd146,string.Empty);}
			if (dt.Rows[2]["ColVisible"].ToString() == "Y" && dt.Rows[2]["ColReadOnly"].ToString() != "Y") {cMsgSource146.Text = string.Empty;}
			// *** Default Value (Folder) Web Rule starts here *** //
		}

		private void InitMaster(object sender, System.EventArgs e)
		{
			cMsgId146.Text = string.Empty;
			SetMsgTypeCd146(cMsgTypeCd146,string.Empty);
			cMsgSource146.Text = string.Empty;
			// *** Default Value (Folder) Web Rule starts here *** //
		}
		protected void cAdmMsgCenter86List_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmMsgCenter86List_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmMsgCenter86List_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			InitMaster(sender, e);      // Do this first to enable defaults for non-database columns.
			DataTable dt = null;
			try
			{
				dt = (new AdminSystem()).GetMstById("GetAdmMsgCenter86ById",cAdmMsgCenter86List.SelectedValue,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId));
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				if (dt.Rows.Count > 0)
				{
					cAdmMsgCenter86List.SetDdVisible();
					DataRow dr = dt.Rows[0];
					try {cMsgId146.Text = RO.Common3.Utils.fmNumeric("0",dr["MsgId146"].ToString(),base.LUser.Culture);} catch {cMsgId146.Text = string.Empty;}
					SetMsgTypeCd146(cMsgTypeCd146,dr["MsgTypeCd146"].ToString());
					try {cMsgSource146.Text = dr["MsgSource146"].ToString();} catch {cMsgSource146.Text = string.Empty;}
				}
			}
			cButPanel.DataBind(); if (!cSaveButton.Visible) { cInsRowButton.Visible = false; }
			DataTable dtAdmMsgCenterGrid = null;
			int filterId = 0; if (Utils.IsInt(cFilterId.SelectedValue)) { filterId = int.Parse(cFilterId.SelectedValue); }
			try
			{
				dtAdmMsgCenterGrid = (new AdminSystem()).GetDtlById(86,"GetAdmMsgCenter86DtlById",cAdmMsgCenter86List.SelectedValue,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,base.LImpr,base.LCurr);
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (!dtAdmMsgCenterGrid.Columns.Contains("_NewRow")) { dtAdmMsgCenterGrid.Columns.Add("_NewRow"); }
			Session[KEY_dtAdmMsgCenterGrid] = dtAdmMsgCenterGrid;
			cAdmMsgCenterGrid.EditIndex = -1;
			cAdmMsgCenterGridDataPager.PageSize = Int16.Parse(cPgSize.Text); GotoPage(0);
			if (Session[KEY_lastSortCol] != null)
			{
				cAdmMsgCenterGrid_OnSorting(sender, new ListViewSortEventArgs((string)Session[KEY_lastSortExp], SortDirection.Ascending));
			}
			else if (dtAdmMsgCenterGrid.Rows.Count <= 0 || (!((GetAuthRow().Rows[0]["AllowUpd"].ToString() == "N" || GetAuthRow().Rows[0]["ViewOnly"].ToString() == "G") && GetAuthRow().Rows[0]["AllowAdd"].ToString() == "N") && !(Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N") && dtAdmMsgCenterGrid.Rows.Count == 1))
			{
				cAdmMsgCenterGrid_DataBind(dtAdmMsgCenterGrid.DefaultView);
			}
			cFindButton_Click(sender, e);
			cNaviPanel.Visible = true; cImportPwdPanel.Visible = false; Session.Remove(KEY_scrImport);
			ScriptManager.GetCurrent(Parent.Page).SetFocus(cAdmMsgCenter86List.FocusID);
		...
		public void cFindButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dv != null)
			{
				...
				if ((cc != null && cc.ID.Equals("cAdmMsgCenter86List")) || cAdmMsgCenterGrid.EditIndex < 0 || UpdateGridRow(cAdmMsgCenterGrid, new CommandEventArgs("Save", "")))
				{
					string rf = string.Empty;
					if (cFind.Text != string.Empty) { rf = "(" + base.GetExpression(cFind.Text.Trim(), GetAuthCol(), 3, cFindFilter.SelectedValue) + ")"; }
					if (rf != string.Empty) { rf = "((" + rf + " or _NewRow = 'Y' ))"; }
					dv.RowFilter = rf;
					ViewState["_RowFilter"] = rf;
					GotoPage(0); cAdmMsgCenterGrid_DataBind(dv);
					if (GetCurrPageIndex() != (int)Session[KEY_currPageIndex] && (int)Session[KEY_currPageIndex] < GetTotalPages()) { GotoPage((int)Session[KEY_currPageIndex]); cAdmMsgCenterGrid_DataBind(dv);}
		...
		public void cInsRowButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dt != null && (ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")))
			{
				...
				Session[KEY_lastAddedRow] = 0; Session[KEY_dtAdmMsgCenterGrid] = dt; Session[KEY_currPageIndex] = 0; GotoPage(0);
				cAdmMsgCenterGrid_DataBind(dt.DefaultView);
				cAdmMsgCenterGrid_OnItemEditing(cAdmMsgCenterGrid, new ListViewEditEventArgs(0));
		...
		public void cPgSizeButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cAdmMsgCenterGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				...
				cAdmMsgCenterGridDataPager.PageSize = int.Parse(cPgSize.Text); GotoPage(0);
				if (dt.Rows.Count <= 0 || (!((GetAuthRow().Rows[0]["AllowUpd"].ToString() == "N" || GetAuthRow().Rows[0]["ViewOnly"].ToString() == "G") && GetAuthRow().Rows[0]["AllowAdd"].ToString() == "N") && !(Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N") && dt.Rows.Count == 1)) {cAdmMsgCenterGrid_DataBind(dt.DefaultView);} else {cFindButton_Click(sender, e);}
				try
				{
					(new AdminSystem()).UpdLastPageInfo(86, base.LUser.UsrId, cPgSize.Text, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		}
		...
		public void cShowImpButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cSchemaImage.Visible)
			{
				try
				{
					Session["ImportSchema"] = (new AdminSystem()).GetSchemaScrImp(86,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (cSchemaImage.Attributes["OnClick"] == null || cSchemaImage.Attributes["OnClick"].IndexOf("ImportSchema.aspx") < 0) {cSchemaImage.Attributes["OnClick"] += "SearchLink('ImportSchema.aspx?scm=S&key=86&csy=3','','',''); return false;";}
			}
			// *** System Button Click (after) Web Rule starts here *** //
		}
		...
		private void ScrImportDdl(DataView dvd, DataRowView drv, bool bComboBox, string PKey, string CKey, string CNam, int MaxLen, string MatchCd, bool bAllowNulls)
		{
			if (dvd != null)
			{
				...
				{
					if (string.IsNullOrEmpty(cAdmMsgCenter86List.SelectedValue))
					{
						dvd.RowFilter = "(" + PKey + " is null)";
					}
					else
					{
						dvd.RowFilter = "(" + PKey + " is null OR " + PKey + " = " + cAdmMsgCenter86List.SelectedValue + ")";
					}
				}
		...
		protected void ScrImport(bool bClear)
		{
			...
			DataTable dtAdmMsgCenterGrid = (DataTable)Session[KEY_dtAdmMsgCenterGrid];
			if (dti != null && dtAdmMsgCenterGrid != null)
			{
				if (bClear)
				{
					dtAdmMsgCenterGrid.Rows.Clear();
				}
				cFind.Text = string.Empty;
				int iExist = dtAdmMsgCenterGrid.Rows.Count;
				...
				foreach (DataRowView drv in dti.DefaultView)
				{
					...
					{
						DataTable dtd;
						dtd = jj==iExist ? null : (DataTable)Session[KEY_dtCultureId147];
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(86,"GetDdlCultureId3S1414",true,true,0,string.Empty,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
							Session[KEY_dtCultureId147] = dtd;
						}
						ScrImportDdl(dtd.DefaultView, drv, true, "MsgId", "CultureId147", "CultureId147Text", 50, "2", false);
					}
					jj = jj + 1;
				}
				iRow = dti.DefaultView.Count - iExist;
				if (!dti.Columns.Contains("_NewRow")) { dti.Columns.Add("_NewRow"); }
				Session[KEY_dtAdmMsgCenterGrid] = dti;
				cAdmMsgCenterGrid_DataBind(dti.DefaultView);
		...
		public void cImportButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cFNameO.Text != string.Empty && cWorkSheet.Items.Count > 0 && cWorkSheet.SelectedItem.Text != string.Empty && cStartRow.Text != string.Empty)
			{
				...
				try
				{
					if (cAdmMsgCenterGrid.EditIndex < 0 || UpdateGridRow(cAdmMsgCenterGrid, new CommandEventArgs("Save", "")))
					{
						Session[KEY_scrImport] = ImportFile(dtAdmMsgCenterGrid.Copy(),cFNameO.Text,cWorkSheet.SelectedItem.Text,cStartRow.Text,Config.PathTmpImport + cFName.Text);
					}
		...
		private void GotoPage(int pageNo)
		{
			if (pageNo >= 0 && pageNo < GetTotalPages())
			{
				if (cAdmMsgCenterGrid.EditIndex < 0 || UpdateGridRow(cAdmMsgCenterGrid, new CommandEventArgs("Save", "")))
				{
					try { cAdmMsgCenterGridDataPager.SetPageProperties(cAdmMsgCenterGridDataPager.PageSize * pageNo, cAdmMsgCenterGridDataPager.MaximumRows, false); cAdmMsgCenterGrid_DataBind(null); }
					catch
					{
						try { cAdmMsgCenterGridDataPager.SetPageProperties(0, cAdmMsgCenterGridDataPager.MaximumRows, false); cAdmMsgCenterGrid_DataBind(null); } catch {}
					}
				}
			}
		}

		protected void cAdmMsgCenterGrid_OnItemCommand(object sender, ListViewCommandEventArgs e)
		{
		}

		private void cAdmMsgCenterGrid_DataBind(DataView dvAdmMsgCenterGrid)
		{
			if (dvAdmMsgCenterGrid == null)
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmMsgCenterGrid];
				dvAdmMsgCenterGrid = dt != null ? dt.DefaultView : null;
			}
			if (dvAdmMsgCenterGrid != null)
			{
				LcAuth = GetAuthCol().AsEnumerable().ToDictionary<DataRow,string>(dr=>dr["ColName"].ToString());
				cAdmMsgCenterGrid.DataSource = dvAdmMsgCenterGrid;
				cAdmMsgCenterGrid.DataBind();
			...
			if (cFindFilter.Items.Count <= 0)
			{
				...
				{
					if (ii >= 3 && !string.IsNullOrEmpty(dr["ColumnHeader"].ToString()) && !string.IsNullOrEmpty(dr["TableId"].ToString()) && dtAuth.Rows[ii]["ColVisible"].ToString() == "Y")
					{
						li = new ListItem();
		...
		protected void cAdmMsgCenterGrid_OnSorting(object sender, ListViewSortEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmMsgCenterGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null)
			{
				if (cAdmMsgCenterGrid.EditIndex < 0 || UpdateGridRow(cAdmMsgCenterGrid, new CommandEventArgs("Save", "")))
				{
					...
					Session[KEY_dtAdmMsgCenterGrid] = dt;
					cAdmMsgCenterGrid_DataBind(dv);
				}
			}
		}

		protected void cAdmMsgCenterGrid_OnPagePropertiesChanging(object sender, PagePropertiesChangingEventArgs e)
		{
		    cAdmMsgCenterGridDataPager.SetPageProperties(e.StartRowIndex, e.MaximumRows, false);
		    DataTable dt = (DataTable)Session[KEY_dtAdmMsgCenterGrid];
		    DataView dv = dt != null ? dt.DefaultView : null;
			cAdmMsgCenterGrid_DataBind(dv); cAdmMsgCenterGrid.EditIndex = -1;
		}

		private void GridChkPgDirty(ListViewItem lvi)
		{
				WebControl cc = null;
				cc = ((WebControl)lvi.FindControl("cCultureId147"));
				...
				cc = ((WebControl)lvi.FindControl("cMsg147"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
		}

		protected void cAdmMsgCenterGrid_OnItemDataBound(object sender, ListViewItemEventArgs e)
		{
			// *** GridItemDataBound (before) Web Rule End *** //
			DataTable dt = (DataTable)Session[KEY_dtAdmMsgCenterGrid];
			bool isEditItem = false;
			DataView dvAdmMsgCenterGrid = dt != null ? dt.DefaultView : null;
			if (cAdmMsgCenterGrid.EditIndex > -1 && GetDataItemIndex(cAdmMsgCenterGrid.EditIndex) == e.Item.DataItemIndex)
			{
				isEditItem = true;
				base.SetGridEnabled(e.Item, GetAuthCol(), GetLabel(), 3);
				if (dvAdmMsgCenterGrid[e.Item.DataItemIndex]["CultureId147"].ToString() != string.Empty) {SetCultureId147((RoboCoder.WebControls.ComboBox)e.Item.FindControl("cCultureId147"),dvAdmMsgCenterGrid[e.Item.DataItemIndex]["CultureId147"].ToString(), e.Item);} else {SetCultureId147((RoboCoder.WebControls.ComboBox)e.Item.FindControl("cCultureId147"),"1", e.Item);}
			...
			if (dtAuthRow != null)
			{
				...
				{
					cc = e.Item.FindControl("cAdmMsgCenterGridDelete"); if (cc != null) { cc.Visible = false; }
					cc = e.Item.FindControl("cAdmMsgCenterGridEdit"); if (cc != null) { cc.Visible = false; }
				}
				else
				{
			        HtmlTableRow tr = e.Item.FindControl("cAdmMsgCenterGridRow") as HtmlTableRow;
			        LinkButton lb = e.Item.FindControl("cAdmMsgCenterGridEdit") as LinkButton;
		...
		protected void cMsgCenterId147hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "0") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "0"; Session[KEY_lastSortExp] = "MsgCenterId147";Session[KEY_lastSortImg] = "cMsgCenterId147hi";
			cAdmMsgCenterGrid_OnSorting(sender, new ListViewSortEventArgs("MsgCenterId147", SortDirection.Ascending));
		}

		protected void cCultureId147hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "1") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "1"; Session[KEY_lastSortExp] = "CultureId147Text";Session[KEY_lastSortImg] = "cCultureId147hi";
			cAdmMsgCenterGrid_OnSorting(sender, new ListViewSortEventArgs("CultureId147Text", SortDirection.Ascending));
		}

		protected void cMsg147hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "2") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "2"; Session[KEY_lastSortExp] = "Msg147";Session[KEY_lastSortImg] = "cMsg147hi";
			cAdmMsgCenterGrid_OnSorting(sender, new ListViewSortEventArgs("Msg147", SortDirection.Ascending));
		}

		protected void cAdmMsgCenterGrid_OnItemEditing(object sender, ListViewEditEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmMsgCenterGrid];
			if ((ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")) && dt != null && dt.DefaultView.Count > GetDataItemIndex(e.NewEditIndex))
			{
				cAdmMsgCenterGrid.EditIndex = e.NewEditIndex;
				cAdmMsgCenterGrid_DataBind(null);
				Control wc = null;
				string idx = Page.Request["__EVENTARGUMENT"];
				if (!string.IsNullOrEmpty(idx))
				{
				    Control ctrlToFocus = cAdmMsgCenterGrid.EditItem.FindControl("c" + idx);
				    if (((WebControl)ctrlToFocus).Enabled) { wc = ctrlToFocus; }
				}
				if (wc == null) { wc = FindEditableControl(cAdmMsgCenterGrid.EditItem); }
				if (wc != null)
             {
             ...
             MkMobileLabel(cAdmMsgCenterGrid.EditItem);
		    }
		    else { e.Cancel = true; }
		}
		...
		{
		    Label ml = null;
		    ml = lvi.FindControl("cMsgCenterId147ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(3); }
		    ml = lvi.FindControl("cCultureId147ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(4); }
		    ml = lvi.FindControl("cMsg147ml") as Label;
		...
		protected void GridFill(ListViewItem lvi, DataTable dt, DataRow dr, bool bInsert)
		{
			...
			cbb = (RoboCoder.WebControls.ComboBox)lvi.FindControl("cCultureId147");
			if (cbb != null)
			{
				if (cbb.SelectedValue != string.Empty)
				{
					dr["CultureId147"] = cbb.SelectedValue;
					dr["CultureId147Text"] = cbb.SelectedText;
				}
				else
				{
					dr["CultureId147"] = System.DBNull.Value;
					dr["CultureId147Text"] = System.DBNull.Value;
				}
			}
			TextBox tb = null;
			tb = (TextBox)lvi.FindControl("cMsg147");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["Msg147"] = tb.Text;} else {dr["Msg147"] = System.DBNull.Value;}
			}
		...
		protected void cAdmMsgCenterGrid_OnItemUpdating(object sender, ListViewUpdateEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmMsgCenterGrid];
			if (dt != null && dt.Rows.Count > 0)
			{
				DataRow dr = dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row;
			    GridFill(cAdmMsgCenterGrid.EditItem, dt, dr, false);
			    dr["_NewRow"] = "N";
			    if (dt.Columns.Contains("_SortOrder")) dr["_SortOrder"] = null;
			}
		}

		protected void cAdmMsgCenterGrid_OnItemCanceling(object sender, ListViewCancelEventArgs e)
		{
			PanelTop.Update();
			DataTable dt = (DataTable)Session[KEY_dtAdmMsgCenterGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			...
			{
				cAdmMsgCenterGrid_OnItemDeleting(sender, new ListViewDeleteEventArgs(e.ItemIndex));
			}
			if (dt.Columns.Contains("_SortOrder")) dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row["_SortOrder"] = null;
			cAdmMsgCenterGrid.EditIndex = -1; cAdmMsgCenterGrid_DataBind(null);
		}
		...
		protected void cDeleteAllButton_Click(object sender, System.EventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmMsgCenterGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null && (cAdmMsgCenterGrid.EditIndex < 0 || UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				GotoPage(0);
				while (dv.Count > 0) {dv.Delete(0);}
				Session[KEY_dtAdmMsgCenterGrid] = dt;
				cAdmMsgCenterGrid.EditIndex = -1; cAdmMsgCenterGrid_DataBind(dv);
			}
		}

		protected void cAdmMsgCenterGrid_OnItemDeleting(object sender, ListViewDeleteEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmMsgCenterGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			DataRow dr = dv != null ? dv[GetDataItemIndex(e.ItemIndex)].Row : null;
			if (dv != null && (cAdmMsgCenterGrid.EditIndex == e.ItemIndex || UpdateGridRow(sender, new CommandEventArgs("Save",""))))
			{
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cMsgId146.Text == string.Empty) { cClearButton_Click(sender, new EventArgs()); ShowDirty(false); PanelTop.Update();}
			else { PopAdmMsgCenter86List(sender, e, false, cMsgId146.Text); }
			// *** System Button Click (After) Web Rule starts here *** //
		}
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmMsgCenter86List); PopAdmMsgCenter86List(sender, e, false, null);
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			if (ValidPage() && (cAdmMsgCenterGrid.EditIndex < 0 || UpdateGridRow(cAdmMsgCenterGrid, new CommandEventArgs("Save", ""))))
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmMsgCenterGrid];
				DataView dv = dt != null ? dt.DefaultView : null;
				string ftr = string.Empty;
				if (dv.RowFilter != string.Empty) { ftr = dv.RowFilter; dv.RowFilter = string.Empty; }
				AdmMsgCenter86 ds = PrepAdmMsgCenterData(dv,cMsgId146.Text == string.Empty);
				if (ftr != string.Empty) {dv.RowFilter = ftr;}
				if (!string.IsNullOrEmpty(cAdmMsgCenter86List.SelectedValue) && cMsgId146.Text != cAdmMsgCenter86List.SelectedValue)
				{
					PreMsgPopup("Primary key cannot be changed");
					return rtn;
				}
				if (string.IsNullOrEmpty(cAdmMsgCenter86List.SelectedValue))	// Add
				{
					if (ds != null)
					{
						pid = (new AdminSystem()).AddData(86,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc,noTrans,commandTimeOut);
					}
					if (!string.IsNullOrEmpty(pid))
					{
						cAdmMsgCenter86List.ClearSearch(); Session.Remove(KEY_dtAdmMsgCenter86List);
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmMsgCenter86List(sender, e, false, pid);
						rtn = GetScreenHlp().Rows[0]["AddMsg"].ToString();
					}
				}
				else {
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(86,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc,noTrans,commandTimeOut)) {bValid7 = true;}
					if (bValid7)
					{
						cAdmMsgCenter86List.ClearSearch(); Session.Remove(KEY_dtAdmMsgCenter86List);
						Session[KEY_currPageIndex] = GetCurrPageIndex();
						ShowDirty(false); PopAdmMsgCenter86List(sender, e, false, ds.Tables["AdmMsgCenter"].Rows[0]["MsgId146"]);
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cMsgId146.Text != string.Empty)
			{
				AdmMsgCenter86 ds = PrepAdmMsgCenterData(null,false);
				try
				{
					if (ds != null && (new AdminSystem()).DelData(86,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc))
					{
						cAdmMsgCenter86List.ClearSearch(); Session.Remove(KEY_dtAdmMsgCenter86List);
						ShowDirty(false); PopAdmMsgCenter86List(sender, e, false, null);
		...
		private AdmMsgCenter86 PrepAdmMsgCenterData(DataView dv, bool bAdd)
		{
			AdmMsgCenter86 ds = new AdmMsgCenter86();
			DataRow dr = ds.Tables["AdmMsgCenter"].NewRow();
			DataRow drType = ds.Tables["AdmMsgCenter"].NewRow();
			DataRow drDisp = ds.Tables["AdmMsgCenter"].NewRow();
			if (bAdd) { dr["MsgId146"] = string.Empty; } else { dr["MsgId146"] = cMsgId146.Text; }
			drType["MsgId146"] = "Numeric"; drDisp["MsgId146"] = "TextBox";
			try {dr["MsgTypeCd146"] = cMsgTypeCd146.SelectedValue;} catch {}
			drType["MsgTypeCd146"] = "Char"; drDisp["MsgTypeCd146"] = "DropDownList";
			try {dr["MsgSource146"] = cMsgSource146.Text.Trim();} catch {}
			drType["MsgSource146"] = "VarChar"; drDisp["MsgSource146"] = "TextBox";
			if (dv != null)
			{
				ds.Tables["AdmMsgCenterDef"].Rows.Add(MakeTypRow(ds.Tables["AdmMsgCenterDef"].NewRow()));
				ds.Tables["AdmMsgCenterDef"].Rows.Add(MakeDisRow(ds.Tables["AdmMsgCenterDef"].NewRow()));
				if (bAdd)
				{
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmMsgCenterAdd"].Rows.Add(MakeColRow(ds.Tables["AdmMsgCenterAdd"].NewRow(), drv, true));
					}
				}
				else
				{
					...
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmMsgCenterUpd"].Rows.Add(MakeColRow(ds.Tables["AdmMsgCenterUpd"].NewRow(), drv, false));
					}
					...
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmMsgCenterDel"].Rows.Add(MakeColRow(ds.Tables["AdmMsgCenterDel"].NewRow(), drv, false));
					}
					dv.RowStateFilter = DataViewRowState.CurrentRows;
				}
			}
			ds.Tables["AdmMsgCenter"].Rows.Add(dr); ds.Tables["AdmMsgCenter"].Rows.Add(drType); ds.Tables["AdmMsgCenter"].Rows.Add(drDisp);
			return ds;
		}

		public bool CanAct(char typ) { return CanAct(typ, GetAuthRow(), cAdmMsgCenter86List.SelectedValue.ToString()); }
		...
		private bool ValidPage()
		{
			...
			DataTable dt = null;
			dt = (DataTable)Session[KEY_dtMsgTypeCd146];
			if (dt != null && dt.Rows.Count <= 0)
			{
				bErrNow.Value = "Y"; PreMsgPopup("Value is expected for 'MsgTypeCd', please investigate."); return false;
			}
			dt = (DataTable)Session[KEY_dtCultureId147];
			if (dt != null && dt.Rows.Count <= 0)
			{
		...
		{
			dr["MsgId146"] = System.Data.OleDb.OleDbType.Numeric.ToString();
			dr["MsgCenterId147"] = System.Data.OleDb.OleDbType.Numeric.ToString();
			dr["CultureId147"] = System.Data.OleDb.OleDbType.Numeric.ToString();
			dr["Msg147"] = System.Data.OleDb.OleDbType.VarWChar.ToString();
			return dr;
		}
		...
		{
			dr["MsgId146"] = "TextBox";
			dr["MsgCenterId147"] = "TextBox";
			dr["CultureId147"] = "AutoComplete";
			dr["Msg147"] = "TextBox";
			return dr;
		}
		...
		{
			dr["MsgId146"] = cMsgId146.Text;
			DataTable dtAuth = GetAuthCol();
			if (dtAuth != null)
			{
				dr["MsgCenterId147"] = drv["MsgCenterId147"].ToString().Trim();
				dr["CultureId147"] = drv["CultureId147"];
				dr["Msg147"] = drv["Msg147"].ToString().Trim();
			}
			return dr;
		}
		...
		{
			if (cAdmMsgCenterGrid.EditIndex > -1)
			{
				cAdmMsgCenterGrid_OnItemUpdating(sender, new ListViewUpdateEventArgs(cAdmMsgCenterGrid.EditIndex));
			}
			return true;
		}

		protected void cAdmMsgCenterGrid_OnPreRender(object sender, System.EventArgs e)
		{
			System.Web.UI.WebControls.Image hi = null;
			hi = (System.Web.UI.WebControls.Image)cAdmMsgCenterGrid.FindControl("cMsgCenterId147hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmMsgCenterGrid.FindControl("cCultureId147hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmMsgCenterGrid.FindControl("cMsg147hi"); if (hi != null) { hi.Visible = false; }
			if (Session[KEY_lastSortImg] != null)
			{
				hi = (System.Web.UI.WebControls.Image)cAdmMsgCenterGrid.FindControl((string)Session[KEY_lastSortImg]);
				if (hi != null) { hi.ImageUrl = Utils.AddTilde((string)Session[KEY_lastSortUrl]); hi.Visible = true; }
			}
			IgnoreHeaderConfirm((LinkButton)cAdmMsgCenterGrid.FindControl("cMsgCenterId147hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmMsgCenterGrid.FindControl("cCultureId147hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmMsgCenterGrid.FindControl("cMsg147hl"));
		}

		private string GetButtonId(ListViewItem lvi)
		{
			string ButtonID = String.Empty;
			Control c = lvi.FindControl("cAdmMsgCenterGridEdit");
		...
		{
		    return cAdmMsgCenterGridDataPager.StartRowIndex / cAdmMsgCenterGridDataPager.PageSize;
		}

		private int GetTotalPages()
		{
		    return (int)Math.Ceiling((double)cAdmMsgCenterGridDataPager.TotalRowCount / cAdmMsgCenterGridDataPager.PageSize);
		}

		private int GetDataItemIndex(int editIndex)
		{
		    return cAdmMsgCenterGridDataPager.StartRowIndex + editIndex;
		}

		protected void cAdmMsgCenterGrid_OnLayoutCreated(object sender, EventArgs e)
		{
		    // Header:
		    LinkButton lb = null;
		    lb = cAdmMsgCenterGrid.FindControl("cMsgCenterId147hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(3); lb.ToolTip = ColumnToolTip(3); lb.Parent.Visible = GridColumnVisible(3); }
		    lb = cAdmMsgCenterGrid.FindControl("cCultureId147hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(4); lb.ToolTip = ColumnToolTip(4); lb.Parent.Visible = GridColumnVisible(4); }
		    lb = cAdmMsgCenterGrid.FindControl("cMsg147hl") as LinkButton;
			...
			if (dtAuthRow != null)
			{
				...
				{
		            lb = cAdmMsgCenterGrid.FindControl("cDeleteAllButton") as LinkButton; if (lb != null) { lb.Visible = false; }
				}
			}
		    // footer:
		    Label gc = null;
		    gc = cAdmMsgCenterGrid.FindControl("cMsgCenterId147fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(3); }
		    gc = cAdmMsgCenterGrid.FindControl("cCultureId147fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(4); }
		    gc = cAdmMsgCenterGrid.FindControl("cMsg147fl") as Label;
		...
		private void PreMsgPopup(string msg) { PreMsgPopup(msg,cAdmMsgCenter86List,null); }

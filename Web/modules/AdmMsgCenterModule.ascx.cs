	...
	public partial class AdmMsgCenterModule : RO.Web.ModuleBase
	{
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				try
				{
					try {dv = new DataView((new AdminSystem()).GetExp(86,"GetExpAdmMsgCenter86","Y",(string)Session[KEY_sysConnectionString],LcAppPw,filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
					catch {dv = new DataView((new AdminSystem()).GetExp(86,"GetExpAdmMsgCenter86","N",(string)Session[KEY_sysConnectionString],LcAppPw,filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
...
\par }}");
		...
		private void SetMsgTypeCd146(DropDownList ddl, string keyId)
		{
			...
			if (ddl != null)
			{
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(86,"GetDdlMsgTypeCd3S1409",true,bAll,0,keyId,(string)Session[KEY_sysConnectionString],LcAppPw,string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(86,"GetDdlMsgTypeCd3S1409",true,true,0,keyId,(string)Session[KEY_sysConnectionString],LcAppPw,string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
		...
		private void SetCultureId147(RoboCoder.WebControls.ComboBox ddl, string keyId, ListViewItem li)
		{
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(86,"GetDdlCultureId3S1414",true,false,0,keyId,(string)Session[KEY_sysConnectionString],LcAppPw,string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
		...
		private DataView GetAdmMsgCenter86List()
		{
			...
			if (dt == null)
			{
				...
				try
				{
					try {dt = (new AdminSystem()).GetLis(86,"GetLisAdmMsgCenter86",true,"Y",0,(string)Session[KEY_sysConnectionString],LcAppPw,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
					catch {dt = (new AdminSystem()).GetLis(86,"GetLisAdmMsgCenter86",true,"N",0,(string)Session[KEY_sysConnectionString],LcAppPw,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				}
		...
		protected void cAdmMsgCenter86List_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			...
			try
			{
				dt = (new AdminSystem()).GetMstById("GetAdmMsgCenter86ById",cAdmMsgCenter86List.SelectedValue,(string)Session[KEY_sysConnectionString],LcAppPw);
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
			...
			try
			{
				dtAdmMsgCenterGrid = (new AdminSystem()).GetDtlById(86,"GetAdmMsgCenter86DtlById",cAdmMsgCenter86List.SelectedValue,(string)Session[KEY_sysConnectionString],LcAppPw,filterId,base.LImpr,base.LCurr);
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				...
				foreach (DataRowView drv in dti.DefaultView)
				{
					if ((drv.Row.RowState == System.Data.DataRowState.Added || drv.Row.RowState == System.Data.DataRowState.Detached) && jj >= iExist)
					{
						...
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(86,"GetDdlCultureId3S1414",true,true,0,string.Empty,(string)Session[KEY_sysConnectionString],LcAppPw,string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			if (ValidPage() && (cAdmMsgCenterGrid.EditIndex < 0 || UpdateGridRow(cAdmMsgCenterGrid, new CommandEventArgs("Save", ""))))
			{
				...
				if (string.IsNullOrEmpty(cAdmMsgCenter86List.SelectedValue))	// Add
				{
					if (ds != null)
					{
						pid = (new AdminSystem()).AddData(86,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],LcAppPw,base.CPrj,base.CSrc);
					}
					...
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(86,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],LcAppPw,base.CPrj,base.CSrc)) {bValid7 = true;}
					if (bValid7)
					{
						cAdmMsgCenter86List.ClearSearch(); Session.Remove(KEY_dtAdmMsgCenter86List);
		...
		public void cDeleteButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cMsgId146.Text != string.Empty)
			{
				...
				try
				{
					if (ds != null && (new AdminSystem()).DelData(86,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],LcAppPw,base.CPrj,base.CSrc))
					{
						cAdmMsgCenter86List.ClearSearch(); Session.Remove(KEY_dtAdmMsgCenter86List);
		...
		private bool UpdateGridRow(object sender, CommandEventArgs e)
		{
			...
			{
				TextBox pwd = null;				cAdmMsgCenterGrid_OnItemUpdating(sender, new ListViewUpdateEventArgs(cAdmMsgCenterGrid.EditIndex));
			}
			return true;
		}

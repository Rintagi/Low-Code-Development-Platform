...
{
	public class AdmScrAuditDtl1020 : DataSet
	{
		public AdmScrAuditDtl1020()
		{
			this.Tables.Add(MakeDtlColumns(new DataTable("AdmScrAuditDtlDef")));
			this.Tables.Add(MakeDtlColumns(new DataTable("AdmScrAuditDtlAdd")));
			this.Tables.Add(MakeDtlColumns(new DataTable("AdmScrAuditDtlUpd")));
			this.Tables.Add(MakeDtlColumns(new DataTable("AdmScrAuditDtlDel")));
			this.DataSetName = "AdmScrAuditDtl1020";
			this.Namespace = "http://Rintagi.com/DataSet/AdmScrAuditDtl1020";
		}

		private DataTable MakeDtlColumns(DataTable dt)
		{
			DataColumnCollection columns = dt.Columns;
			columns.Add("ScrAuditDtlId1301", typeof(string));
			columns.Add("ScrAuditId1301", typeof(string));
			columns.Add("ScreenObjId1301", typeof(string));
			columns.Add("ScreenObjDesc1301", typeof(string));
			columns.Add("ColumnId1301", typeof(string));
			columns.Add("ColumnDesc1301", typeof(string));
			columns.Add("ChangedFr1301", typeof(string));
			columns.Add("ChangedTo1301", typeof(string));
			return dt;
		}
	}
}
...
namespace RO.Web
{
	public partial class AdmScrAuditDtlModule : RO.Web.ModuleBase
	{

		private const string KEY_lastAddedRow = "Cache:lastAddedRow3_1020";
		private const string KEY_lastSortOrd = "Cache:lastSortOrd3_1020";
		private const string KEY_lastSortImg = "Cache:lastSortImg3_1020";
		private const string KEY_lastSortCol = "Cache:lastSortCol3_1020";
		private const string KEY_lastSortExp = "Cache:lastSortExp3_1020";
		private const string KEY_lastSortUrl = "Cache:lastSortUrl3_1020";
		private const string KEY_lastSortTog = "Cache:lastSortTog3_1020";
		private const string KEY_lastImpPwdOvride = "Cache:lastImpPwdOvride3_1020";
		private const string KEY_cntImpPwdOvride = "Cache:cntImpPwdOvride3_1020";
		private const string KEY_currPageIndex = "Cache:currPageIndex3_1020";
		private const string KEY_bHiImpVisible = "Cache:bHiImpVisible3_1020";
		private const string KEY_bShImpVisible = "Cache:bShImpVisible3_1020";
		private const string KEY_dtAdmScrAuditDtlGrid = "Cache:dtAdmScrAuditDtlGrid";
		private const string KEY_scrImport = "Cache:scrImport3_1020";
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_1020";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_1020";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_1020";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_1020";
		private const string KEY_dtLabel = "Cache:dtLabel3_1020";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_1020";
		private const string KEY_dtScrCri = "Cache:dtScrCri3_1020";
		private const string KEY_dsScrCriVal = "Cache:dsScrCriVal3_1020";


		private const string KEY_dtSystems = "Cache:dtSystems3_1020";
		private const string KEY_sysConnectionString = "Cache:sysConnectionString3_1020";
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_1020";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_1020";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_1020";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				DataTable dtMenuAccess = (new MenuSystem()).GetMenu(base.LUser.CultureId, 3, base.LImpr, LcSysConnString, LcAppPw,1020, null, null);
				...
				Session.Remove(KEY_dtAdmScrAuditDtlGrid);
				...
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 1020, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmScrAuditDtl1020List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=1020&typ=N&sys=3','','',''); return false;"; }
				//WebRule: Hide criteria panel and systems module
                cCriPanel.Visible = false;
			...
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmScrAuditDtlGrid];
				if (dt != null)
				{
		...
		private void InitializeComponent()
		{
			...
			if (LcSysConnString == null)
			{
				...
				{
					DataTable dt;
					dt = (new AdminSystem()).GetAuthRow(1020,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
					...
					{
						cAdmScrAuditDtlGrid.PreRender += new EventHandler(cAdmScrAuditDtlGrid_OnPreRender);
					}
				}
		...
		private void SetSystem(byte SystemId)
		{
			...
			try
			{
				...
				if ((Config.DeployType == "DEV" || row["dbAppDatabase"].ToString() == base.CPrj.EntityCode + "View") && !(base.CPrj.EntityCode != "RO" && row["SysProgram"].ToString() == "Y") && (new AdminSystem()).IsRegenNeeded(string.Empty,1020,0,0,LcSysConnString,LcAppPw))
				{
					(new GenScreensSystem()).CreateProgram(1020, "Screen Audit Detail", row["dbAppDatabase"].ToString(), base.CPrj, base.CSrc, base.CTar, LcAppConnString, LcAppPw);
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(1020,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(1020,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void SetClientRule(ListViewDataItem ee, bool isEditItem)
		{
			...
			if (dvRul.Count > 0)
			{
				...
				{
					if (ee == null || drv["ScriptEvent"].ToString().Substring(0,2).ToLower() != "on" || (cAdmScrAuditDtlGrid.EditIndex > -1 && GetDataItemIndex(cAdmScrAuditDtlGrid.EditIndex) == ee.DataItemIndex))
					{
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(1020,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(1020,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,1020,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(1020,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(1020,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(1020,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				CheckAuthentication(false);
				DataView dv = null;
				try {dv = ((DataTable)Session[KEY_dtAdmScrAuditDtlGrid]).DefaultView;}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(1020,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					{
						if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["ScrAuditDtlId1301"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["ScrAuditId1301"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["ScreenObjId1301"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["ScreenObjDesc1301"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["ColumnId1301"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["ColumnDesc1301"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["ChangedFr1301"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[7]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["ChangedTo1301"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						sb.Append(Environment.NewLine);
					}
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmScrAuditDtl.csv"; Session["ExportStr"] = (Config.ExportExcelCSV ? "sep=\t\n": "") + sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmScrAuditDtl.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private string GetRtfGrid(DataView dv)
		{
			...
			try
			{
				...
				foreach (DataRowView drv in dv)
				{
					...
					sb.Append("\r\n");
					sb.Append(@"{");
					if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["ScrAuditDtlId1301"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["ScrAuditId1301"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["ScreenObjId1301"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(drv["ScreenObjDesc1301"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["ColumnId1301"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append(drv["ColumnDesc1301"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append(drv["ChangedFr1301"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[7]["ColExport"].ToString() == "Y") {sb.Append(drv["ChangedTo1301"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					sb.Append(@"}");
					sb.Append("\r\n");
					sb.Append(@"\pard \ql \li0\ri0\widctlpar\intbl\aspalpha\aspnum\adjustright\rin0\lin0 {");
					sb.Append(@"\insrsid2981395 \trowd \irow0\irowband0\lastrow \ts15\trgaph108\trleft-108\trbrdrt\brdrs\brdrw10 \trbrdrl\brdrs\brdrw10 \trbrdrb\brdrs\brdrw10 \trbrdrr\brdrs\brdrw10 \trbrdrh\brdrs\brdrw10 \trbrdrv\brdrs\brdrw10 ");
		...
		private void GetPageInfo()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastPageInfo(1020, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 1020, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			// *** Criteria Trigger (before) Web Rule starts here *** //
			DataTable dt = (DataTable)Session[KEY_dtAdmScrAuditDtlGrid];
			if (cAdmScrAuditDtlGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
			if (IsPostBack && cCriteria.Visible) ReBindCriteria(GetScrCriteria());
			UpdCriteria(true);
			PopAdmScrAuditDtl1020List(sender, e, false, null);
			}
		}
			...
			try
			{
				foreach (DataRowView drv in dvCri)
				{
					...
					if (drv["DisplayName"].ToString() == "ComboBox")
					{
						...
						(new AdminSystem()).MkGetScreenIn("1020", drv["ScreenCriId"].ToString(), "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), LcAppDb, LcDesDb, drv["MultiDesignDb"].ToString(), LcSysConnString, LcAppPw);
						DataView dv = new DataView((new AdminSystem()).GetScreenIn("1020", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), (new AdminSystem()).CountScrCri(drv["ScreenCriId"].ToString(), drv["MultiDesignDb"].ToString(), drv["MultiDesignDb"].ToString() == "N" ? LcSysConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)), drv["RequiredValid"].ToString(), 0, string.Empty, drv["DisplayMode"].ToString() != "AutoListBox", val, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)));
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("1020", LcSysConnString, LcAppPw);
				}
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmScrAuditDtl1020" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmScrAuditDtl1020" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			foreach (DataRowView drv in dvCri)
			{
			    if (drv["DisplayName"].ToString() == "ListBox")
			    {
					...
					if (cListBox != null)
					{
						...
						int TotalChoiceCnt = new DataView((new AdminSystem()).GetScreenIn("1020", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), CriCnt, drv["RequiredValid"].ToString(), 0, string.Empty, true, string.Empty, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId))).Count;
						string selectedValues = string.Join(",", cListBox.Items.Cast<ListItem>().Where(x => x.Selected).Select(x => "'" + x.Value + "'").ToArray());
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("1020", "AdmScrAuditDtl1020", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		private void PopAdmScrAuditDtl1020List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			DataTable dt = null;
			CheckAuthentication(false);
			...
			try
			{
				try {dt = (new AdminSystem()).GetLis(1020,"GetLisAdmScrAuditDtl1020",false,"Y",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				catch {dt = (new AdminSystem()).GetLis(1020,"GetLisAdmScrAuditDtl1020",false,"N",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				Session[KEY_dtAdmScrAuditDtlGrid] = dt;
				cAdmScrAuditDtlGridDataPager.PageSize = Int16.Parse(cPgSize.Text); GotoPage(0);
			    if (Session[KEY_lastSortCol] != null)
			    {
					cAdmScrAuditDtlGrid_OnSorting(sender, new ListViewSortEventArgs((string)Session[KEY_lastSortExp], SortDirection.Ascending));
			    }
				else
			    {
					cAdmScrAuditDtlGrid_DataBind(dt.DefaultView);
			    }
		...
		public void cFindButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dv != null)
			{
				if (cAdmScrAuditDtlGrid.EditIndex < 0 || UpdateGridRow(cAdmScrAuditDtlGrid, new CommandEventArgs("Save", "")))
				{
					...
					GotoPage(0); cAdmScrAuditDtlGrid_DataBind(dv);
					if (GetCurrPageIndex() != (int)Session[KEY_currPageIndex] && (int)Session[KEY_currPageIndex] < GetTotalPages()) { GotoPage((int)Session[KEY_currPageIndex]); cAdmScrAuditDtlGrid_DataBind(dv);}
		...
		public void cInsRowButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dt != null && (ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")))
			{
				...
				Session[KEY_lastAddedRow] = 0; Session[KEY_dtAdmScrAuditDtlGrid] = dt; Session[KEY_currPageIndex] = 0; GotoPage(0);
				cAdmScrAuditDtlGrid_DataBind(dt.DefaultView);
				cAdmScrAuditDtlGrid_OnItemEditing(cAdmScrAuditDtlGrid, new ListViewEditEventArgs(0));
		...
		public void cPgSizeButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cAdmScrAuditDtlGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				...
				cAdmScrAuditDtlGridDataPager.PageSize = int.Parse(cPgSize.Text); GotoPage(0);
				if (dt.Rows.Count <= 0 || (GetAuthRow().Rows[0]["AllowAdd"].ToString() == "Y" && dt.Rows.Count == 1)) {cAdmScrAuditDtlGrid_DataBind(dt.DefaultView);} else {cFindButton_Click(sender, e);}
				try
				{
					(new AdminSystem()).UpdLastPageInfo(1020, base.LUser.UsrId, cPgSize.Text, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		}
		...
		public void cShowImpButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cSchemaImage.Visible)
			{
				try
				{
					Session["ImportSchema"] = (new AdminSystem()).GetSchemaScrImp(1020,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (cSchemaImage.Attributes["OnClick"] == null || cSchemaImage.Attributes["OnClick"].IndexOf("ImportSchema.aspx") < 0) {cSchemaImage.Attributes["OnClick"] += "SearchLink('ImportSchema.aspx?scm=S&key=1020&csy=3','','',''); return false;";}
			}
			// *** System Button Click (after) Web Rule starts here *** //
		}
		...
		protected void ScrImport(bool bClear)
		{
			...
			DataTable dtAdmScrAuditDtlGrid = (DataTable)Session[KEY_dtAdmScrAuditDtlGrid];
			if (dti != null && dtAdmScrAuditDtlGrid != null)
			{
				if (bClear)
				{
					dtAdmScrAuditDtlGrid.Rows.Clear();
				}
				cFind.Text = string.Empty;
				int iExist = dtAdmScrAuditDtlGrid.Rows.Count;
				iRow = dti.DefaultView.Count - iExist;
				if (!dti.Columns.Contains("_NewRow")) { dti.Columns.Add("_NewRow"); }
				Session[KEY_dtAdmScrAuditDtlGrid] = dti;
				cAdmScrAuditDtlGrid_DataBind(dti.DefaultView);
		...
		public void cImportButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cFNameO.Text != string.Empty && cWorkSheet.Items.Count > 0 && cWorkSheet.SelectedItem.Text != string.Empty && cStartRow.Text != string.Empty)
			{
				...
				try
				{
					if (cAdmScrAuditDtlGrid.EditIndex < 0 || UpdateGridRow(cAdmScrAuditDtlGrid, new CommandEventArgs("Save", "")))
					{
						Session[KEY_scrImport] = ImportFile(dtAdmScrAuditDtlGrid.Copy(),cFNameO.Text,cWorkSheet.SelectedItem.Text,cStartRow.Text,Config.PathTmpImport + cFName.Text);
					}
		...
		private DataTable ImportFile(DataTable dt, string fileName, string workSheet, string startRow, string fileFullName)
		{
			try
			{
				...
				{
					if (rows[iRow][1].ToString() == string.Empty && rows[iRow][2].ToString() == string.Empty && rows[iRow][3].ToString() == string.Empty && rows[iRow][4].ToString() == string.Empty && rows[iRow][5].ToString() == string.Empty) {idel = idel + 1;}
					else
					{
		...
		private void GotoPage(int pageNo)
		{
			if (pageNo >= 0 && pageNo < GetTotalPages())
			{
				if (cAdmScrAuditDtlGrid.EditIndex < 0 || UpdateGridRow(cAdmScrAuditDtlGrid, new CommandEventArgs("Save", "")))
				{
					try { cAdmScrAuditDtlGridDataPager.SetPageProperties(cAdmScrAuditDtlGridDataPager.PageSize * pageNo, cAdmScrAuditDtlGridDataPager.MaximumRows, false); cAdmScrAuditDtlGrid_DataBind(null); }
					catch
					{
						try { cAdmScrAuditDtlGridDataPager.SetPageProperties(0, cAdmScrAuditDtlGridDataPager.MaximumRows, false); cAdmScrAuditDtlGrid_DataBind(null); } catch {}
					}
				}
			}
		}

		protected void cAdmScrAuditDtlGrid_OnItemCommand(object sender, ListViewCommandEventArgs e)
		{
		}

		private void cAdmScrAuditDtlGrid_DataBind(DataView dvAdmScrAuditDtlGrid)
		{
			if (dvAdmScrAuditDtlGrid == null)
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmScrAuditDtlGrid];
				dvAdmScrAuditDtlGrid = dt != null ? dt.DefaultView : null;
			}
			if (dvAdmScrAuditDtlGrid != null)
			{
				LcAuth = GetAuthCol().AsEnumerable().ToDictionary<DataRow,string>(dr=>dr["ColName"].ToString());
				cAdmScrAuditDtlGrid.DataSource = dvAdmScrAuditDtlGrid;
				cAdmScrAuditDtlGrid.DataBind();
		...
		protected void cAdmScrAuditDtlGrid_OnSorting(object sender, ListViewSortEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmScrAuditDtlGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null)
			{
				if (cAdmScrAuditDtlGrid.EditIndex < 0 || UpdateGridRow(cAdmScrAuditDtlGrid, new CommandEventArgs("Save", "")))
				{
					...
					Session[KEY_dtAdmScrAuditDtlGrid] = dt;
					cAdmScrAuditDtlGrid_DataBind(dv);
				}
			}
		}

		protected void cAdmScrAuditDtlGrid_OnPagePropertiesChanging(object sender, PagePropertiesChangingEventArgs e)
		{
		    cAdmScrAuditDtlGridDataPager.SetPageProperties(e.StartRowIndex, e.MaximumRows, false);
		    DataTable dt = (DataTable)Session[KEY_dtAdmScrAuditDtlGrid];
		    DataView dv = dt != null ? dt.DefaultView : null;
			cAdmScrAuditDtlGrid_DataBind(dv); cAdmScrAuditDtlGrid.EditIndex = -1;
		}

		private void GridChkPgDirty(ListViewItem lvi)
		{
				WebControl cc = null;
				cc = ((WebControl)lvi.FindControl("cScrAuditId1301"));
				...
				cc = ((WebControl)lvi.FindControl("cScreenObjId1301"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cScreenObjDesc1301"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cColumnId1301"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cColumnDesc1301"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cChangedFr1301"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cChangedTo1301"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
		}

		protected void cAdmScrAuditDtlGrid_OnItemDataBound(object sender, ListViewItemEventArgs e)
		{
			// *** GridItemDataBound (before) Web Rule End *** //
			DataTable dt = (DataTable)Session[KEY_dtAdmScrAuditDtlGrid];
			bool isEditItem = false;
			DataView dvAdmScrAuditDtlGrid = dt != null ? dt.DefaultView : null;
			if (cAdmScrAuditDtlGrid.EditIndex > -1 && GetDataItemIndex(cAdmScrAuditDtlGrid.EditIndex) == e.Item.DataItemIndex)
			{
				isEditItem = true;
			...
			if (dtAuthRow != null)
			{
				...
				{
					cc = e.Item.FindControl("cAdmScrAuditDtlGridDelete"); if (cc != null) { cc.Visible = false; }
				}
				if (!AllowRowEdit(GetAuthRow(),dvAdmScrAuditDtlGrid[e.Item.DataItemIndex]["ScrAuditDtlId1301"].ToString()) || (Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N"))
				{
					cc = e.Item.FindControl("cAdmScrAuditDtlGridEdit"); if (cc != null) { cc.Visible = false; }
				}
				else
				{
			        HtmlTableRow tr = e.Item.FindControl("cAdmScrAuditDtlGridRow") as HtmlTableRow;
			        LinkButton lb = e.Item.FindControl("cAdmScrAuditDtlGridEdit") as LinkButton;
		...
		protected void cScrAuditDtlId1301hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "0") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "0"; Session[KEY_lastSortExp] = "ScrAuditDtlId1301";Session[KEY_lastSortImg] = "cScrAuditDtlId1301hi";
			cAdmScrAuditDtlGrid_OnSorting(sender, new ListViewSortEventArgs("ScrAuditDtlId1301", SortDirection.Ascending));
		}

		protected void cScrAuditId1301hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "1") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "1"; Session[KEY_lastSortExp] = "ScrAuditId1301";Session[KEY_lastSortImg] = "cScrAuditId1301hi";
			cAdmScrAuditDtlGrid_OnSorting(sender, new ListViewSortEventArgs("ScrAuditId1301", SortDirection.Ascending));
		}

		protected void cScreenObjId1301hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "2") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "2"; Session[KEY_lastSortExp] = "ScreenObjId1301";Session[KEY_lastSortImg] = "cScreenObjId1301hi";
			cAdmScrAuditDtlGrid_OnSorting(sender, new ListViewSortEventArgs("ScreenObjId1301", SortDirection.Ascending));
		}

		protected void cScreenObjDesc1301hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "3") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "3"; Session[KEY_lastSortExp] = "ScreenObjDesc1301";Session[KEY_lastSortImg] = "cScreenObjDesc1301hi";
			cAdmScrAuditDtlGrid_OnSorting(sender, new ListViewSortEventArgs("ScreenObjDesc1301", SortDirection.Ascending));
		}

		protected void cColumnId1301hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "4") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "4"; Session[KEY_lastSortExp] = "ColumnId1301";Session[KEY_lastSortImg] = "cColumnId1301hi";
			cAdmScrAuditDtlGrid_OnSorting(sender, new ListViewSortEventArgs("ColumnId1301", SortDirection.Ascending));
		}

		protected void cColumnDesc1301hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "5") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "5"; Session[KEY_lastSortExp] = "ColumnDesc1301";Session[KEY_lastSortImg] = "cColumnDesc1301hi";
			cAdmScrAuditDtlGrid_OnSorting(sender, new ListViewSortEventArgs("ColumnDesc1301", SortDirection.Ascending));
		}

		protected void cChangedFr1301hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "6") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "6"; Session[KEY_lastSortExp] = "ChangedFr1301";Session[KEY_lastSortImg] = "cChangedFr1301hi";
			cAdmScrAuditDtlGrid_OnSorting(sender, new ListViewSortEventArgs("ChangedFr1301", SortDirection.Ascending));
		}

		protected void cChangedTo1301hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "7") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "7"; Session[KEY_lastSortExp] = "ChangedTo1301";Session[KEY_lastSortImg] = "cChangedTo1301hi";
			cAdmScrAuditDtlGrid_OnSorting(sender, new ListViewSortEventArgs("ChangedTo1301", SortDirection.Ascending));
		}

		protected void cAdmScrAuditDtlGrid_OnItemEditing(object sender, ListViewEditEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmScrAuditDtlGrid];
			if ((ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")) && dt != null && dt.DefaultView.Count > GetDataItemIndex(e.NewEditIndex))
			{
				cAdmScrAuditDtlGrid.EditIndex = e.NewEditIndex;
				cAdmScrAuditDtlGrid_DataBind(null);
				Control wc = null;
				string idx = Page.Request["__EVENTARGUMENT"];
				if (!string.IsNullOrEmpty(idx))
				{
				    Control ctrlToFocus = cAdmScrAuditDtlGrid.EditItem.FindControl("c" + idx);
				    if (((WebControl)ctrlToFocus).Enabled) { wc = ctrlToFocus; }
				}
				if (wc == null) { wc = FindEditableControl(cAdmScrAuditDtlGrid.EditItem); }
				if (wc != null)
             {
             ...
             MkMobileLabel(cAdmScrAuditDtlGrid.EditItem);
		    }
		    else { e.Cancel = true; }
		}
		...
		{
		    Label ml = null;
		    ml = lvi.FindControl("cScrAuditDtlId1301ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(0); }
		    ml = lvi.FindControl("cScrAuditId1301ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(1); }
		    ml = lvi.FindControl("cScreenObjId1301ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(2); }
		    ml = lvi.FindControl("cScreenObjDesc1301ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(3); }
		    ml = lvi.FindControl("cColumnId1301ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(4); }
		    ml = lvi.FindControl("cColumnDesc1301ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(5); }
		    ml = lvi.FindControl("cChangedFr1301ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(6); }
		    ml = lvi.FindControl("cChangedTo1301ml") as Label;
		...
		{
			TextBox tb = null;
			tb = (TextBox)lvi.FindControl("cScrAuditId1301");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["ScrAuditId1301"] = tb.Text;} else {dr["ScrAuditId1301"] = System.DBNull.Value;}
			}
			tb = (TextBox)lvi.FindControl("cScreenObjId1301");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["ScreenObjId1301"] = tb.Text;} else {dr["ScreenObjId1301"] = System.DBNull.Value;}
			}
			tb = (TextBox)lvi.FindControl("cScreenObjDesc1301");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["ScreenObjDesc1301"] = tb.Text;} else {dr["ScreenObjDesc1301"] = System.DBNull.Value;}
			}
			tb = (TextBox)lvi.FindControl("cColumnId1301");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["ColumnId1301"] = tb.Text;} else {dr["ColumnId1301"] = System.DBNull.Value;}
			}
			tb = (TextBox)lvi.FindControl("cColumnDesc1301");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["ColumnDesc1301"] = tb.Text;} else {dr["ColumnDesc1301"] = System.DBNull.Value;}
			}
			tb = (TextBox)lvi.FindControl("cChangedFr1301");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["ChangedFr1301"] = tb.Text;} else {dr["ChangedFr1301"] = System.DBNull.Value;}
			}
			tb = (TextBox)lvi.FindControl("cChangedTo1301");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["ChangedTo1301"] = tb.Text;} else {dr["ChangedTo1301"] = System.DBNull.Value;}
			}
		    if (bInsert) { dt.Rows.InsertAt(dr, 0); }
			Session[KEY_dtAdmScrAuditDtlGrid] = dt;
			cAdmScrAuditDtlGrid.EditIndex = -1;
			cAdmScrAuditDtlGrid_DataBind(dt.DefaultView);
		}

		protected void cAdmScrAuditDtlGrid_OnItemUpdating(object sender, ListViewUpdateEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmScrAuditDtlGrid];
			if (dt != null && dt.Rows.Count > 0)
			{
				DataRow dr = dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row;
			    GridFill(cAdmScrAuditDtlGrid.EditItem, dt, dr, false);
			    dr["_NewRow"] = "N";
			    if (dt.Columns.Contains("_SortOrder")) dr["_SortOrder"] = null;
			}
		}

		protected void cAdmScrAuditDtlGrid_OnItemCanceling(object sender, ListViewCancelEventArgs e)
		{
			PanelTop.Update();
			DataTable dt = (DataTable)Session[KEY_dtAdmScrAuditDtlGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			...
			{
				cAdmScrAuditDtlGrid_OnItemDeleting(sender, new ListViewDeleteEventArgs(e.ItemIndex));
			}
			if (dt.Columns.Contains("_SortOrder")) dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row["_SortOrder"] = null;
			cAdmScrAuditDtlGrid.EditIndex = -1; cAdmScrAuditDtlGrid_DataBind(null);
		}
		...
		protected void cDeleteAllButton_Click(object sender, System.EventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmScrAuditDtlGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null && (cAdmScrAuditDtlGrid.EditIndex < 0 || UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				GotoPage(0);
				while (dv.Count > 0) {dv.Delete(0);}
				Session[KEY_dtAdmScrAuditDtlGrid] = dt;
				cAdmScrAuditDtlGrid.EditIndex = -1; cAdmScrAuditDtlGrid_DataBind(dv);
			}
		}

		protected void cAdmScrAuditDtlGrid_OnItemDeleting(object sender, ListViewDeleteEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmScrAuditDtlGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			DataRow dr = dv != null ? dv[GetDataItemIndex(e.ItemIndex)].Row : null;
			if (dv != null && (cAdmScrAuditDtlGrid.EditIndex == e.ItemIndex || UpdateGridRow(sender, new CommandEventArgs("Save",""))))
			{
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmScrAuditDtlGrid); PopAdmScrAuditDtl1020List(sender, e, false, null);
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			// *** System Button Click (Before) Web Rule starts here *** //
			if (ValidPage() && UpdateGridRow(cAdmScrAuditDtlGrid, new CommandEventArgs("Save", "")))
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmScrAuditDtlGrid];
				DataView dv = dt != null ? dt.DefaultView : null;
				string ftr = string.Empty;
				if (dv.RowFilter != string.Empty) { ftr = dv.RowFilter; dv.RowFilter = string.Empty; }
				AdmScrAuditDtl1020 ds = PrepAdmScrAuditDtlData(dv,false);
				if (ftr != string.Empty) {dv.RowFilter = ftr;}
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(1020,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc,noTrans,commandTimeOut)) {bValid7 = true;}
					if (bValid7)
					{
						Session[KEY_currPageIndex] = GetCurrPageIndex();
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmScrAuditDtl1020List(sender, e, false, null);
		...
		private AdmScrAuditDtl1020 PrepAdmScrAuditDtlData(DataView dv, bool bAdd)
		{
			AdmScrAuditDtl1020 ds = new AdmScrAuditDtl1020();
			if (dv != null)
			{
				ds.Tables["AdmScrAuditDtlDef"].Rows.Add(MakeTypRow(ds.Tables["AdmScrAuditDtlDef"].NewRow()));
				ds.Tables["AdmScrAuditDtlDef"].Rows.Add(MakeDisRow(ds.Tables["AdmScrAuditDtlDef"].NewRow()));
					dv.RowStateFilter = DataViewRowState.ModifiedCurrent;
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmScrAuditDtlUpd"].Rows.Add(MakeColRow(ds.Tables["AdmScrAuditDtlUpd"].NewRow(), drv, false));
					}
					dv.RowStateFilter = DataViewRowState.Added;
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmScrAuditDtlAdd"].Rows.Add(MakeColRow(ds.Tables["AdmScrAuditDtlAdd"].NewRow(), drv, true));
					}
					dv.RowStateFilter = DataViewRowState.Deleted;
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmScrAuditDtlDel"].Rows.Add(MakeColRow(ds.Tables["AdmScrAuditDtlDel"].NewRow(), drv, false));
					}
		...
		{
			dr["ScrAuditDtlId1301"] = System.Data.OleDb.OleDbType.Numeric.ToString();
			dr["ScrAuditId1301"] = System.Data.OleDb.OleDbType.Numeric.ToString();
			dr["ScreenObjId1301"] = System.Data.OleDb.OleDbType.Numeric.ToString();
			dr["ScreenObjDesc1301"] = System.Data.OleDb.OleDbType.VarWChar.ToString();
			dr["ColumnId1301"] = System.Data.OleDb.OleDbType.Numeric.ToString();
			dr["ColumnDesc1301"] = System.Data.OleDb.OleDbType.VarWChar.ToString();
			dr["ChangedFr1301"] = System.Data.OleDb.OleDbType.VarWChar.ToString();
			dr["ChangedTo1301"] = System.Data.OleDb.OleDbType.VarWChar.ToString();
			return dr;
		}

		private DataRow MakeDisRow(DataRow dr)
		{
			dr["ScrAuditDtlId1301"] = "TextBox";
			dr["ScrAuditId1301"] = "TextBox";
			dr["ScreenObjId1301"] = "TextBox";
			dr["ScreenObjDesc1301"] = "TextBox";
			dr["ColumnId1301"] = "TextBox";
			dr["ColumnDesc1301"] = "TextBox";
			dr["ChangedFr1301"] = "TextBox";
			dr["ChangedTo1301"] = "TextBox";
			return dr;
		}
		...
		private DataRow MakeColRow(DataRow dr, DataRowView drv, bool bAdd)
		{
			...
			if (dtAuth != null)
			{
				dr["ScrAuditDtlId1301"] = drv["ScrAuditDtlId1301"].ToString().Trim();
				dr["ScrAuditId1301"] = drv["ScrAuditId1301"].ToString().Trim();
				dr["ScreenObjId1301"] = drv["ScreenObjId1301"].ToString().Trim();
				dr["ScreenObjDesc1301"] = drv["ScreenObjDesc1301"].ToString().Trim();
				dr["ColumnId1301"] = drv["ColumnId1301"].ToString().Trim();
				dr["ColumnDesc1301"] = drv["ColumnDesc1301"].ToString().Trim();
				dr["ChangedFr1301"] = drv["ChangedFr1301"].ToString().Trim();
				if (bAdd && dtAuth.Rows[6]["ColReadOnly"].ToString() == "Y" && dr["ChangedFr1301"].ToString() == string.Empty) {dr["ChangedFr1301"] = System.DBNull.Value;}
				dr["ChangedTo1301"] = drv["ChangedTo1301"].ToString().Trim();
				if (bAdd && dtAuth.Rows[7]["ColReadOnly"].ToString() == "Y" && dr["ChangedTo1301"].ToString() == string.Empty) {dr["ChangedTo1301"] = System.DBNull.Value;}
			}
			return dr;
		}
		...
		{
			if (cAdmScrAuditDtlGrid.EditIndex > -1)
			{
				cAdmScrAuditDtlGrid_OnItemUpdating(sender, new ListViewUpdateEventArgs(cAdmScrAuditDtlGrid.EditIndex));
			}
			return true;
		}

		protected void cAdmScrAuditDtlGrid_OnPreRender(object sender, System.EventArgs e)
		{
			System.Web.UI.WebControls.Image hi = null;
			hi = (System.Web.UI.WebControls.Image)cAdmScrAuditDtlGrid.FindControl("cScrAuditDtlId1301hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmScrAuditDtlGrid.FindControl("cScrAuditId1301hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmScrAuditDtlGrid.FindControl("cScreenObjId1301hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmScrAuditDtlGrid.FindControl("cScreenObjDesc1301hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmScrAuditDtlGrid.FindControl("cColumnId1301hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmScrAuditDtlGrid.FindControl("cColumnDesc1301hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmScrAuditDtlGrid.FindControl("cChangedFr1301hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmScrAuditDtlGrid.FindControl("cChangedTo1301hi"); if (hi != null) { hi.Visible = false; }
			if (Session[KEY_lastSortImg] != null)
			{
				hi = (System.Web.UI.WebControls.Image)cAdmScrAuditDtlGrid.FindControl((string)Session[KEY_lastSortImg]);
				if (hi != null) { hi.ImageUrl = Utils.AddTilde((string)Session[KEY_lastSortUrl]); hi.Visible = true; }
			}
			IgnoreHeaderConfirm((LinkButton)cAdmScrAuditDtlGrid.FindControl("cScrAuditDtlId1301hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmScrAuditDtlGrid.FindControl("cScrAuditId1301hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmScrAuditDtlGrid.FindControl("cScreenObjId1301hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmScrAuditDtlGrid.FindControl("cScreenObjDesc1301hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmScrAuditDtlGrid.FindControl("cColumnId1301hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmScrAuditDtlGrid.FindControl("cColumnDesc1301hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmScrAuditDtlGrid.FindControl("cChangedFr1301hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmScrAuditDtlGrid.FindControl("cChangedTo1301hl"));
		}

		private string GetButtonId(ListViewItem lvi)
		{
			string ButtonID = String.Empty;
			Control c = lvi.FindControl("cAdmScrAuditDtlGridEdit");
		...
		{
		    return cAdmScrAuditDtlGridDataPager.StartRowIndex / cAdmScrAuditDtlGridDataPager.PageSize;
		}

		private int GetTotalPages()
		{
		    return (int)Math.Ceiling((double)cAdmScrAuditDtlGridDataPager.TotalRowCount / cAdmScrAuditDtlGridDataPager.PageSize);
		}

		private int GetDataItemIndex(int editIndex)
		{
		    return cAdmScrAuditDtlGridDataPager.StartRowIndex + editIndex;
		}

		protected void cAdmScrAuditDtlGrid_OnLayoutCreated(object sender, EventArgs e)
		{
		    // Header:
		    LinkButton lb = null;
		    lb = cAdmScrAuditDtlGrid.FindControl("cScrAuditDtlId1301hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(0); lb.ToolTip = ColumnToolTip(0); lb.Parent.Visible = GridColumnVisible(0); }
		    lb = cAdmScrAuditDtlGrid.FindControl("cScrAuditId1301hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(1); lb.ToolTip = ColumnToolTip(1); lb.Parent.Visible = GridColumnVisible(1); }
		    lb = cAdmScrAuditDtlGrid.FindControl("cScreenObjId1301hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(2); lb.ToolTip = ColumnToolTip(2); lb.Parent.Visible = GridColumnVisible(2); }
		    lb = cAdmScrAuditDtlGrid.FindControl("cScreenObjDesc1301hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(3); lb.ToolTip = ColumnToolTip(3); lb.Parent.Visible = GridColumnVisible(3); }
		    lb = cAdmScrAuditDtlGrid.FindControl("cColumnId1301hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(4); lb.ToolTip = ColumnToolTip(4); lb.Parent.Visible = GridColumnVisible(4); }
		    lb = cAdmScrAuditDtlGrid.FindControl("cColumnDesc1301hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(5); lb.ToolTip = ColumnToolTip(5); lb.Parent.Visible = GridColumnVisible(5); }
		    lb = cAdmScrAuditDtlGrid.FindControl("cChangedFr1301hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(6); lb.ToolTip = ColumnToolTip(6); lb.Parent.Visible = GridColumnVisible(6); }
		    lb = cAdmScrAuditDtlGrid.FindControl("cChangedTo1301hl") as LinkButton;
			...
			if (dtAuthRow != null)
			{
				...
				if (dr["AllowDel"].ToString() == "N" || (Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N"))
				{
		            lb = cAdmScrAuditDtlGrid.FindControl("cDeleteAllButton") as LinkButton; if (lb != null) { lb.Visible = false; }
				}
			}
		    // footer:
		    Label gc = null;
		    gc = cAdmScrAuditDtlGrid.FindControl("cScrAuditDtlId1301fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(0); }
		    gc = cAdmScrAuditDtlGrid.FindControl("cScrAuditId1301fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(1); }
		    gc = cAdmScrAuditDtlGrid.FindControl("cScreenObjId1301fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(2); }
		    gc = cAdmScrAuditDtlGrid.FindControl("cScreenObjDesc1301fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(3); }
		    gc = cAdmScrAuditDtlGrid.FindControl("cColumnId1301fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(4); }
		    gc = cAdmScrAuditDtlGrid.FindControl("cColumnDesc1301fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(5); }
		    gc = cAdmScrAuditDtlGrid.FindControl("cChangedFr1301fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(6); }
		    gc = cAdmScrAuditDtlGrid.FindControl("cChangedTo1301fl") as Label;

...
namespace RO.Web
{
	public partial class AdmRptTblModule : RO.Web.ModuleBase
	{

		private const string KEY_lastAddedRow = "Cache:lastAddedRow3_92";
		private const string KEY_lastSortOrd = "Cache:lastSortOrd3_92";
		private const string KEY_lastSortImg = "Cache:lastSortImg3_92";
		private const string KEY_lastSortCol = "Cache:lastSortCol3_92";
		private const string KEY_lastSortExp = "Cache:lastSortExp3_92";
		private const string KEY_lastSortUrl = "Cache:lastSortUrl3_92";
		private const string KEY_lastSortTog = "Cache:lastSortTog3_92";
		private const string KEY_lastImpPwdOvride = "Cache:lastImpPwdOvride3_92";
		private const string KEY_cntImpPwdOvride = "Cache:cntImpPwdOvride3_92";
		private const string KEY_currPageIndex = "Cache:currPageIndex3_92";
		private const string KEY_bHiImpVisible = "Cache:bHiImpVisible3_92";
		private const string KEY_bShImpVisible = "Cache:bShImpVisible3_92";
		private const string KEY_dtAdmRptTblGrid = "Cache:dtAdmRptTblGrid";
		private const string KEY_scrImport = "Cache:scrImport3_92";
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_92";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_92";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_92";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_92";
		private const string KEY_dtLabel = "Cache:dtLabel3_92";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_92";
		private const string KEY_dtScrCri = "Cache:dtScrCri3_92";
		private const string KEY_dsScrCriVal = "Cache:dsScrCriVal3_92";

		private const string KEY_dtRptCtrId162 = "Cache:dtRptCtrId162";
		private const string KEY_dtParentId162 = "Cache:dtParentId162";
		private const string KEY_dtReportId162 = "Cache:dtReportId162";
		private const string KEY_dtTblToggle162 = "Cache:dtTblToggle162";
		private const string KEY_dtTblGrouping162 = "Cache:dtTblGrouping162";
		private const string KEY_dtRptTblTypeCd162 = "Cache:dtRptTblTypeCd162";
		private const string KEY_dtTblVisibility162 = "Cache:dtTblVisibility162";
		private const string KEY_dtCelNum164 = "Cache:dtCelNum164";

		private const string KEY_dtSystems = "Cache:dtSystems3_92";
		private const string KEY_sysConnectionString = "Cache:sysConnectionString3_92";
		private const string KEY_dtAdmRptTbl92List = "Cache:dtAdmRptTbl92List";
		private const string KEY_bNewVisible = "Cache:bNewVisible3_92";
		private const string KEY_bNewSaveVisible = "Cache:bNewSaveVisible3_92";
		private const string KEY_bCopyVisible = "Cache:bCopyVisible3_92";
		private const string KEY_bCopySaveVisible = "Cache:bCopySaveVisible3_92";
		private const string KEY_bDeleteVisible = "Cache:bDeleteVisible3_92";
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_92";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_92";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_92";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				DataTable dtMenuAccess = (new MenuSystem()).GetMenu(base.LUser.CultureId, 3, base.LImpr, LcSysConnString, LcAppPw,92, null, null);
				...
				Session.Remove(KEY_dtAdmRptTblGrid);
				...
				Session.Remove(KEY_dtTblToggle162);
				Session.Remove(KEY_dtTblGrouping162);
				Session.Remove(KEY_dtRptTblTypeCd162);
				Session.Remove(KEY_dtTblVisibility162);
				Session.Remove(KEY_dtCelNum164);
				SetButtonHlp();
				GetSystems();
				SetColumnAuthority();
				GetGlobalFilter();
				...
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 92, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmRptTbl92List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=92&typ=N&sys=3','','',''); return false;"; }
				cRptCtrId162Search_Script();
		...
		private void InitializeComponent()
		{
			...
			if (LcSysConnString == null)
			{
				...
				{
					DataTable dt;
					dt = (new AdminSystem()).GetAuthRow(92,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
					...
					{
						cAdmRptTblGrid.PreRender += new EventHandler(cAdmRptTblGrid_OnPreRender);
					}
				}
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(92,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(92,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void SetClientRule(ListViewDataItem ee, bool isEditItem)
		{
			...
			if (dvRul.Count > 0)
			{
				...
				{
					if (ee == null || drv["ScriptEvent"].ToString().Substring(0,2).ToLower() != "on" || (cAdmRptTblGrid.EditIndex > -1 && GetDataItemIndex(cAdmRptTblGrid.EditIndex) == ee.DataItemIndex))
					{
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(92,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(92,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,92,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(92,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(92,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(92,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(92,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					{
						if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["RptTblId162"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["RptCtrId162"].ToString() + (char)9 + drv["RptCtrId162Text"].ToString() + (char)9);}
						if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(drv["ParentId162"].ToString() + (char)9 + drv["ParentId162Text"].ToString() + (char)9);}
						if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(drv["ReportId162"].ToString() + (char)9 + drv["ReportId162Text"].ToString() + (char)9);}
						if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append(drv["TblToggle162"].ToString() + (char)9 + drv["TblToggle162Text"].ToString() + (char)9);}
						if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append(drv["TblGrouping162"].ToString() + (char)9 + drv["TblGrouping162Text"].ToString() + (char)9);}
						if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["RptTblTypeCd162"].ToString().Replace("\"","\"\"") + "\"" + (char)9 + "\"" + drv["RptTblTypeCd162Text"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[7]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["TblVisibility162"].ToString().Replace("\"","\"\"") + "\"" + (char)9 + "\"" + drv["TblVisibility162Text"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[8]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["TblRepeatNew162"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[9]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["TblOrder162"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[10]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("2",drv["ColWidth162"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[11]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["RptCelId164"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[12]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["RowNum164"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[13]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("2",drv["RowHeight164"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[14]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["RowVisibility164"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[15]["ColExport"].ToString() == "Y") {sb.Append(drv["CelNum164"].ToString() + (char)9 + drv["CelNum164Text"].ToString() + (char)9);}
						if (dtAu.Rows[16]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["CelColSpan164"].ToString(),base.LUser.Culture) + (char)9);}
						sb.Append(Environment.NewLine);
					}
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmRptTbl.csv"; Session["ExportStr"] = (Config.ExportExcelCSV ? "sep=\t\n": "") + sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmRptTbl.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private string GetRtfGrid(DataView dv)
		{
			...
			try
			{
				...
				foreach (DataRowView drv in dv)
				{
					...
					sb.Append("\r\n");
					sb.Append(@"{");
					if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["RptTblId162"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["RptCtrId162Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(drv["ParentId162Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(drv["ReportId162Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append(drv["TblToggle162Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append(drv["TblGrouping162Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append(drv["RptTblTypeCd162Text"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[7]["ColExport"].ToString() == "Y") {sb.Append(drv["TblVisibility162Text"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[8]["ColExport"].ToString() == "Y") {sb.Append(drv["TblRepeatNew162"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[9]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["TblOrder162"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[10]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("2",drv["ColWidth162"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[11]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["RptCelId164"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[12]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["RowNum164"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[13]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("2",drv["RowHeight164"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[14]["ColExport"].ToString() == "Y") {sb.Append(drv["RowVisibility164"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[15]["ColExport"].ToString() == "Y") {sb.Append(drv["CelNum164Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[16]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["CelColSpan164"].ToString(),base.LUser.Culture) + @"\cell ");}
					sb.Append(@"}");
					sb.Append("\r\n");
					sb.Append(@"\pard \ql \li0\ri0\widctlpar\intbl\aspalpha\aspnum\adjustright\rin0\lin0 {");
					sb.Append(@"\insrsid2981395 \trowd \irow0\irowband0\lastrow \ts15\trgaph108\trleft-108\trbrdrt\brdrs\brdrw10 \trbrdrl\brdrs\brdrw10 \trbrdrb\brdrs\brdrw10 \trbrdrr\brdrs\brdrw10 \trbrdrh\brdrs\brdrw10 \trbrdrv\brdrs\brdrw10 ");
		...
		public void cRptCtrId162Search_Click(object sender, System.Web.UI.ImageClickEventArgs e)
		{
			RoboCoder.WebControls.ComboBox cc = cRptCtrId162; cc.SetTbVisible(); Session["CtrlAdmRptCtr"] = cc.FocusID;
		}

		public void cRptCtrId162Search_Script()
		{
				ImageButton ib = cRptCtrId162Search;
				if (ib != null)
				{
			    	TextBox pp = cRptTblId162;
					RoboCoder.WebControls.ComboBox cc = cRptCtrId162;
					string ss = "&dsp=ComboBox"; if (cSystem.Visible) {ss = ss + "&sys=" + cSystemId.SelectedValue;} else {ss = ss + "&sys=3";}
					ib.Attributes["onclick"] = "SearchLink('AdmRptCtr.aspx?col=RptCtrId" + ss + "&typ=N" + "','" + cc.FocusID + "','',''); return false;";
				}
		}
		...
		private void GetPageInfo()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastPageInfo(92, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 92, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			// *** Criteria Trigger (before) Web Rule starts here *** //
			DataTable dt = (DataTable)Session[KEY_dtAdmRptTblGrid];
			if (cAdmRptTblGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
			...
			PopAdmRptTbl92List(sender, e, false, null);
			}
		}
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("92", LcSysConnString, LcAppPw);
				}
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmRptTbl92" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmRptTbl92" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("92", "AdmRptTbl92", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		protected void cbRptCtrId162(object sender, System.EventArgs e)
		{
			SetRptCtrId162((RoboCoder.WebControls.ComboBox)sender,string.Empty);
		}

		private void SetRptCtrId162(RoboCoder.WebControls.ComboBox ddl, string keyId)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			if (ddl != null)
			{
				...
				if (dv != null)
				{
					if (dv.Table.Columns.Contains("RptTblId"))
					{
						context["pMKeyColID"] = cAdmRptTbl92List.ClientID;
						context["pMKeyCol"] = "RptTblId";
						string ss = "(RptTblId is null";
						if (string.IsNullOrEmpty(cAdmRptTbl92List.SelectedValue)) {ss = ss + ")";} else {ss = ss + " OR RptTblId = " + cAdmRptTbl92List.SelectedValue + ")";}
						dv.RowFilter = ss;
					}
					ddl.DataSource = dv; Session[KEY_dtRptCtrId162] = dv.Table;
				    try { ddl.SelectByValue(keyId,string.Empty,true); } catch { try { ddl.SelectedIndex = 0; } catch { } }
				}
			}
		}

		protected void cbParentId162(object sender, System.EventArgs e)
		{
			SetParentId162((RoboCoder.WebControls.ComboBox)sender,string.Empty);
		}

		private void SetParentId162(RoboCoder.WebControls.ComboBox ddl, string keyId)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			if (ddl != null)
			{
				...
				if (dv != null)
				{
					...
					ddl.DataSource = dv; Session[KEY_dtParentId162] = dv.Table;
				    try { ddl.SelectByValue(keyId,string.Empty,true); } catch { try { ddl.SelectedIndex = 0; } catch { } }
				}
			}
		}

		protected void cbReportId162(object sender, System.EventArgs e)
		{
			SetReportId162((RoboCoder.WebControls.ComboBox)sender,string.Empty);
		}

		private void SetReportId162(RoboCoder.WebControls.ComboBox ddl, string keyId)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			if (ddl != null)
			{
				...
				if (dv != null)
				{
					...
					ddl.DataSource = dv; Session[KEY_dtReportId162] = dv.Table;
				    try { ddl.SelectByValue(keyId,string.Empty,true); } catch { try { ddl.SelectedIndex = 0; } catch { } }
				}
			}
		}

		protected void cbTblToggle162(object sender, System.EventArgs e)
		{
			SetTblToggle162((RoboCoder.WebControls.ComboBox)sender,string.Empty,cReportId162.SelectedValue);
		}

		private void SetTblToggle162(RoboCoder.WebControls.ComboBox ddl, string keyId, string filtr)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			context["isSys"] = "N";
			context["conn"] = KEY_sysConnectionString;
			context["refColCID"] = cReportId162.ClientID;
			...
			if (ddl != null)
			{
				...
				if (dv != null)
				{
					...
					ddl.DataSource = dv; Session[KEY_dtTblToggle162] = dv.Table;
		...
		protected void cbTblGrouping162(object sender, System.EventArgs e)
		{
			SetTblGrouping162((RoboCoder.WebControls.ComboBox)sender,string.Empty,cReportId162.SelectedValue);
		}

		private void SetTblGrouping162(RoboCoder.WebControls.ComboBox ddl, string keyId, string filtr)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			if (ddl != null)
			{
				...
				if (dv != null)
				{
					...
					ddl.DataSource = dv; Session[KEY_dtTblGrouping162] = dv.Table;
				    try { ddl.SelectByValue(keyId,filtr != string.Empty ? "(ReportId is null OR ReportId = " + filtr + ")" : string.Empty,true); } catch { try { ddl.SelectedIndex = 0; } catch { } }
				}
			}
		}

		private void SetRptTblTypeCd162(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtRptTblTypeCd162];
			...
			if (ddl != null)
			{
				...
				if (dv != null)
				{
					...
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "RptTblTypeCd162 is not NULL"; }
					dv.RowFilter = ss;
					...
					Session[KEY_dtRptTblTypeCd162] = dv.Table;
				}
			}
		}

		private void SetTblVisibility162(RadioButtonList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtTblVisibility162];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (ddl != null)
			{
				string ss = string.Empty;
				...
				if (dv != null)
				{
					...
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "TblVisibility162 is not NULL"; }
					dv.RowFilter = ss;
					ddl.DataSource = dv; ddl.DataBind();
					li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					else if (!bFirst)
					...
					Session[KEY_dtTblVisibility162] = dv.Table;
				}
			}
		}

		protected void cbCelNum164(object sender, System.EventArgs e)
		{
			SetCelNum164((RoboCoder.WebControls.ComboBox)sender,string.Empty,cAdmRptTblGrid.EditItem);
		}

		private void SetCelNum164(RoboCoder.WebControls.ComboBox ddl, string keyId, ListViewItem li)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			if (ddl != null)
			{
				...
				if (dv != null)
				{
					...
					ddl.DataSource = dv; Session[KEY_dtCelNum164] = dv.Table;
				    try { ddl.SelectByValue(keyId,string.Empty,true); } catch { try { ddl.SelectedIndex = 0; } catch { } }
				}
			}
		}

		private DataView GetAdmRptTbl92List()
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmRptTbl92List];
			if (dt == null)
			{
				CheckAuthentication(false);
				int filterId = 0;
		...
		private void PopAdmRptTbl92List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			DataView dv = GetAdmRptTbl92List();
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			context["isSys"] = "N";
			context["conn"] = KEY_sysConnectionString;
			cAdmRptTbl92List.AutoCompleteUrl = "AutoComplete.aspx/LisSuggests";
			cAdmRptTbl92List.DataContext = context;
			if (dv.Table == null) return;
			cAdmRptTbl92List.DataSource = dv;
			cAdmRptTbl92List.Visible = true;
			if (cAdmRptTbl92List.Items.Count <= 0) {cAdmRptTbl92List.Visible = false; cAdmRptTbl92List_SelectedIndexChanged(sender,e);}
			else
			{
				if (bPageLoad && !string.IsNullOrEmpty(Request.QueryString["key"]) && bUseCri.Value == string.Empty)
				{
					try {cAdmRptTbl92List.SelectByValue(Request.QueryString["key"],string.Empty,true);} catch {cAdmRptTbl92List.Items[0].Selected = true; cAdmRptTbl92List_SelectedIndexChanged(sender, e);}
				    cScreenSearch.Visible = false; cSystem.Visible = false; cEditButton.Visible = true; cSaveCloseButton.Visible = cSaveButton.Visible;
				}
				else
				{
					if (ID != null) {if (!cAdmRptTbl92List.SelectByValue(ID,string.Empty,true)) {ID = null;}}
					if (ID == null)
					{
						cAdmRptTbl92List_SelectedIndexChanged(sender, e);
					}
				}
			}
		}
		...
		private void SetColumnAuthority()
		{
			...
			{
				base.SetFoldBehavior(cRptTblId162, dtAuth.Rows[0], cRptTblId162P1, cRptTblId162Label, cRptTblId162P2, null, dtLabel.Rows[0], null, null, null);
				base.SetFoldBehavior(cRptCtrId162, dtAuth.Rows[1], cRptCtrId162P1, cRptCtrId162Label, cRptCtrId162P2, null, dtLabel.Rows[1], cRFVRptCtrId162, null, null);
				SetRptCtrId162(cRptCtrId162,string.Empty);
				base.SetFoldBehavior(cParentId162, dtAuth.Rows[2], cParentId162P1, cParentId162Label, cParentId162P2, null, dtLabel.Rows[2], null, null, null);
				SetParentId162(cParentId162,string.Empty);
				base.SetFoldBehavior(cReportId162, dtAuth.Rows[3], cReportId162P1, cReportId162Label, cReportId162P2, null, dtLabel.Rows[3], cRFVReportId162, null, null);
				SetReportId162(cReportId162,string.Empty);
				base.SetFoldBehavior(cTblToggle162, dtAuth.Rows[4], cTblToggle162P1, cTblToggle162Label, cTblToggle162P2, null, dtLabel.Rows[4], null, null, null);
				SetTblToggle162(cTblToggle162,string.Empty,string.Empty);
				base.SetFoldBehavior(cTblGrouping162, dtAuth.Rows[5], cTblGrouping162P1, cTblGrouping162Label, cTblGrouping162P2, null, dtLabel.Rows[5], null, null, null);
				SetTblGrouping162(cTblGrouping162,string.Empty,string.Empty);
				base.SetFoldBehavior(cRptTblTypeCd162, dtAuth.Rows[6], cRptTblTypeCd162P1, cRptTblTypeCd162Label, cRptTblTypeCd162P2, null, dtLabel.Rows[6], cRFVRptTblTypeCd162, null, null);
				base.SetFoldBehavior(cTblVisibility162, dtAuth.Rows[7], cTblVisibility162P1, cTblVisibility162Label, cTblVisibility162P2, null, dtLabel.Rows[7], null, null, null);
				base.SetFoldBehavior(cTblRepeatNew162, dtAuth.Rows[8], cTblRepeatNew162P1, cTblRepeatNew162Label, cTblRepeatNew162P2, null, dtLabel.Rows[8], null, null, null);
				base.SetFoldBehavior(cTblOrder162, dtAuth.Rows[9], cTblOrder162P1, cTblOrder162Label, cTblOrder162P2, null, dtLabel.Rows[9], null, null, null);
				base.SetFoldBehavior(cColWidth162, dtAuth.Rows[10], cColWidth162P1, cColWidth162Label, cColWidth162P2, null, dtLabel.Rows[10], null, null, null);
			}
			if ((cRptCtrId162.Attributes["OnChange"] == null || cRptCtrId162.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cRptCtrId162.Visible && cRptCtrId162.Enabled) {cRptCtrId162.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if (cRptCtrId162Search.Attributes["OnClick"] == null || cRptCtrId162Search.Attributes["OnClick"].IndexOf("_bConfirm") < 0) {cRptCtrId162Search.Attributes["OnClick"] += "document.getElementById('" + bConfirm.ClientID + "').value='N';";}
			if ((cParentId162.Attributes["OnChange"] == null || cParentId162.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cParentId162.Visible && cParentId162.Enabled) {cParentId162.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cReportId162.Attributes["OnChange"] == null || cReportId162.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cReportId162.Visible && cReportId162.Enabled) {cReportId162.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();document.getElementById('" + bConfirm.ClientID + "').value='N';";}
			if ((cTblToggle162.Attributes["OnChange"] == null || cTblToggle162.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cTblToggle162.Visible && cTblToggle162.Enabled) {cTblToggle162.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cTblGrouping162.Attributes["OnChange"] == null || cTblGrouping162.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cTblGrouping162.Visible && cTblGrouping162.Enabled) {cTblGrouping162.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cRptTblTypeCd162.Attributes["OnChange"] == null || cRptTblTypeCd162.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cRptTblTypeCd162.Visible && cRptTblTypeCd162.Enabled) {cRptTblTypeCd162.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cTblVisibility162.Attributes["OnClick"] == null || cTblVisibility162.Attributes["OnClick"].IndexOf("ChkPgDirty") < 0) && cTblVisibility162.Visible && cTblVisibility162.Enabled) {cTblVisibility162.Attributes["OnClick"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty(); this.focus();";}
			if ((cTblRepeatNew162.Attributes["OnClick"] == null || cTblRepeatNew162.Attributes["OnClick"].IndexOf("ChkPgDirty") < 0) && cTblRepeatNew162.Visible && cTblRepeatNew162.Enabled) {cTblRepeatNew162.Attributes["OnClick"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty(); this.focus();";}
			if ((cTblOrder162.Attributes["OnChange"] == null || cTblOrder162.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cTblOrder162.Visible && !cTblOrder162.ReadOnly) {cTblOrder162.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cColWidth162.Attributes["OnChange"] == null || cColWidth162.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cColWidth162.Visible && !cColWidth162.ReadOnly) {cColWidth162.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
		}
		...
		private void ClearMaster(object sender, System.EventArgs e)
		{
			...
			if (dt.Rows[1]["ColVisible"].ToString() == "Y" && dt.Rows[1]["ColReadOnly"].ToString() != "Y") {cRptCtrId162.ClearSearch();}
			if (dt.Rows[2]["ColVisible"].ToString() == "Y" && dt.Rows[2]["ColReadOnly"].ToString() != "Y") {cParentId162.ClearSearch();}
			if (dt.Rows[3]["ColVisible"].ToString() == "Y" && dt.Rows[3]["ColReadOnly"].ToString() != "Y") {cReportId162.ClearSearch();}
			if (dt.Rows[4]["ColVisible"].ToString() == "Y" && dt.Rows[4]["ColReadOnly"].ToString() != "Y") {cTblToggle162.ClearSearch();}
			if (dt.Rows[5]["ColVisible"].ToString() == "Y" && dt.Rows[5]["ColReadOnly"].ToString() != "Y") {cTblGrouping162.ClearSearch();}
			if (dt.Rows[6]["ColVisible"].ToString() == "Y" && dt.Rows[6]["ColReadOnly"].ToString() != "Y") {SetRptTblTypeCd162(cRptTblTypeCd162,string.Empty);}
			if (dt.Rows[7]["ColVisible"].ToString() == "Y" && dt.Rows[7]["ColReadOnly"].ToString() != "Y") {SetTblVisibility162(cTblVisibility162,string.Empty);}
			if (dt.Rows[8]["ColVisible"].ToString() == "Y" && dt.Rows[8]["ColReadOnly"].ToString() != "Y") {cTblRepeatNew162.Checked = base.GetBool("N");}
			if (dt.Rows[9]["ColVisible"].ToString() == "Y" && dt.Rows[9]["ColReadOnly"].ToString() != "Y") {cTblOrder162.Text = string.Empty;}
			if (dt.Rows[10]["ColVisible"].ToString() == "Y" && dt.Rows[10]["ColReadOnly"].ToString() != "Y") {cColWidth162.Text = string.Empty;}
			// *** Default Value (Folder) Web Rule starts here *** //
		}

		private void InitMaster(object sender, System.EventArgs e)
		{
			cRptTblId162.Text = string.Empty;
			cRptCtrId162.ClearSearch();
			cParentId162.ClearSearch();
			cReportId162.ClearSearch();
			cTblToggle162.ClearSearch();
			cTblGrouping162.ClearSearch();
			SetRptTblTypeCd162(cRptTblTypeCd162,string.Empty);
			SetTblVisibility162(cTblVisibility162,string.Empty);
			cTblRepeatNew162.Checked = base.GetBool("N");
			cTblOrder162.Text = string.Empty;
			cColWidth162.Text = string.Empty;
			// *** Default Value (Folder) Web Rule starts here *** //
		}
		protected void cAdmRptTbl92List_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmRptTbl92List_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmRptTbl92List_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			...
			if (dt != null)
			{
				...
				if (dt.Rows.Count > 0)
				{
					cAdmRptTbl92List.SetDdVisible();
					DataRow dr = dt.Rows[0];
					try {cRptTblId162.Text = RO.Common3.Utils.fmNumeric("0",dr["RptTblId162"].ToString(),base.LUser.Culture);} catch {cRptTblId162.Text = string.Empty;}
					SetRptCtrId162(cRptCtrId162,dr["RptCtrId162"].ToString());
					cRptCtrId162Search_Script();
					SetParentId162(cParentId162,dr["ParentId162"].ToString());
					SetReportId162(cReportId162,dr["ReportId162"].ToString()); cReportId162_SelectedIndexChanged(cReportId162, new EventArgs());
					SetTblToggle162(cTblToggle162,dr["TblToggle162"].ToString(),cReportId162.SelectedValue);
					SetTblGrouping162(cTblGrouping162,dr["TblGrouping162"].ToString(),cReportId162.SelectedValue);
					SetRptTblTypeCd162(cRptTblTypeCd162,dr["RptTblTypeCd162"].ToString());
					SetTblVisibility162(cTblVisibility162,dr["TblVisibility162"].ToString());
					try {cTblRepeatNew162.Checked = base.GetBool(dr["TblRepeatNew162"].ToString());} catch {cTblRepeatNew162.Checked = false;}
					try {cTblOrder162.Text = RO.Common3.Utils.fmNumeric("0",dr["TblOrder162"].ToString(),base.LUser.Culture);} catch {cTblOrder162.Text = string.Empty;}
					try {cColWidth162.Text = RO.Common3.Utils.fmNumeric("2",dr["ColWidth162"].ToString(),base.LUser.Culture);} catch {cColWidth162.Text = string.Empty;}
				}
			}
			cButPanel.DataBind(); if (!cSaveButton.Visible) { cInsRowButton.Visible = false; }
			DataTable dtAdmRptTblGrid = null;
			int filterId = 0; if (Utils.IsInt(cFilterId.SelectedValue)) { filterId = int.Parse(cFilterId.SelectedValue); }
			try
			{
			...
			Session[KEY_dtAdmRptTblGrid] = dtAdmRptTblGrid;
			cAdmRptTblGrid.EditIndex = -1;
			cAdmRptTblGridDataPager.PageSize = Int16.Parse(cPgSize.Text); GotoPage(0);
			if (Session[KEY_lastSortCol] != null)
			{
				cAdmRptTblGrid_OnSorting(sender, new ListViewSortEventArgs((string)Session[KEY_lastSortExp], SortDirection.Ascending));
			}
			else if (dtAdmRptTblGrid.Rows.Count <= 0 || (!((GetAuthRow().Rows[0]["AllowUpd"].ToString() == "N" || GetAuthRow().Rows[0]["ViewOnly"].ToString() == "G") && GetAuthRow().Rows[0]["AllowAdd"].ToString() == "N") && !(Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N") && dtAdmRptTblGrid.Rows.Count == 1))
			{
				cAdmRptTblGrid_DataBind(dtAdmRptTblGrid.DefaultView);
			}
			cFindButton_Click(sender, e);
			cNaviPanel.Visible = true; cImportPwdPanel.Visible = false; Session.Remove(KEY_scrImport);
			ScriptManager.GetCurrent(Parent.Page).SetFocus(cAdmRptTbl92List.FocusID);
			ShowDirty(false); PanelTop.Update();
			// *** List Selection (End of) Web Rule starts here *** //
		}

		protected void cRptCtrId162_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cRptCtrId162_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cRptCtrId162_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			...
			TextBox pwd = null;
			if (cRptCtrId162.Items.Count > 0 && cRptCtrId162.DataSource != null)
			{
				DataView dv = (DataView) cRptCtrId162.DataSource; dv.RowFilter = string.Empty;
				DataRowView dr = cRptCtrId162.DataSetIndex >= 0 && cRptCtrId162.DataSetIndex < dv.Count ? dv[cRptCtrId162.DataSetIndex] : dv[0];
				cRptCtrId162Search_Script();
		...
		protected void cReportId162_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cReportId162_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cReportId162_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			// *** On Change/ On Click Web Rule starts here *** //
			EnableValidators(true); // Do not remove; Need to reenable after postback, especially in the grid.
			TextBox pwd = null;
			if (cReportId162.Items.Count > 0 && cReportId162.DataSource != null)
			{
				DataView dv = (DataView) cReportId162.DataSource; dv.RowFilter = string.Empty;
				DataRowView dr = cReportId162.DataSetIndex >= 0 && cReportId162.DataSetIndex < dv.Count ? dv[cReportId162.DataSetIndex] : dv[0];
				if (cTblToggle162.Mode!="A") cTblToggle162.ClearSearch();				else SetTblToggle162(cTblToggle162,cTblToggle162.SelectedValue,cReportId162.SelectedValue);
				if (cTblGrouping162.Mode!="A") cTblGrouping162.ClearSearch();				else SetTblGrouping162(cTblGrouping162,cTblGrouping162.SelectedValue,cReportId162.SelectedValue);
			if (pwd != null) {ScriptManager.GetCurrent(Parent.Page).SetFocus(pwd.ClientID);} else {ScriptManager.GetCurrent(Parent.Page).SetFocus(SenderFocusId(sender));}
			}
		}
		...
		public void cFindButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dv != null)
			{
				...
				if ((cc != null && cc.ID.Equals("cAdmRptTbl92List")) || cAdmRptTblGrid.EditIndex < 0 || UpdateGridRow(cAdmRptTblGrid, new CommandEventArgs("Save", "")))
				{
					...
					GotoPage(0); cAdmRptTblGrid_DataBind(dv);
					if (GetCurrPageIndex() != (int)Session[KEY_currPageIndex] && (int)Session[KEY_currPageIndex] < GetTotalPages()) { GotoPage((int)Session[KEY_currPageIndex]); cAdmRptTblGrid_DataBind(dv);}
		...
		public void cInsRowButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dt != null && (ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")))
			{
				...
				Session[KEY_lastAddedRow] = 0; Session[KEY_dtAdmRptTblGrid] = dt; Session[KEY_currPageIndex] = 0; GotoPage(0);
				cAdmRptTblGrid_DataBind(dt.DefaultView);
				cAdmRptTblGrid_OnItemEditing(cAdmRptTblGrid, new ListViewEditEventArgs(0));
		...
		public void cPgSizeButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cAdmRptTblGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				...
				cAdmRptTblGridDataPager.PageSize = int.Parse(cPgSize.Text); GotoPage(0);
				if (dt.Rows.Count <= 0 || (!((GetAuthRow().Rows[0]["AllowUpd"].ToString() == "N" || GetAuthRow().Rows[0]["ViewOnly"].ToString() == "G") && GetAuthRow().Rows[0]["AllowAdd"].ToString() == "N") && !(Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N") && dt.Rows.Count == 1)) {cAdmRptTblGrid_DataBind(dt.DefaultView);} else {cFindButton_Click(sender, e);}
				try
				{
					(new AdminSystem()).UpdLastPageInfo(92, base.LUser.UsrId, cPgSize.Text, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		}
		...
		public void cShowImpButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cSchemaImage.Visible)
			{
				try
				{
					Session["ImportSchema"] = (new AdminSystem()).GetSchemaScrImp(92,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (cSchemaImage.Attributes["OnClick"] == null || cSchemaImage.Attributes["OnClick"].IndexOf("ImportSchema.aspx") < 0) {cSchemaImage.Attributes["OnClick"] += "SearchLink('ImportSchema.aspx?scm=S&key=92&csy=3','','',''); return false;";}
			}
			// *** System Button Click (after) Web Rule starts here *** //
		}
		...
		private void ScrImportDdl(DataView dvd, DataRowView drv, bool bComboBox, string PKey, string CKey, string CNam, int MaxLen, string MatchCd, bool bAllowNulls)
		{
			if (dvd != null)
			{
				...
				{
					if (string.IsNullOrEmpty(cAdmRptTbl92List.SelectedValue))
					{
						dvd.RowFilter = "(" + PKey + " is null)";
					}
					else
					{
						dvd.RowFilter = "(" + PKey + " is null OR " + PKey + " = " + cAdmRptTbl92List.SelectedValue + ")";
					}
				}
		...
		protected void ScrImport(bool bClear)
		{
			...
			DataTable dtAdmRptTblGrid = (DataTable)Session[KEY_dtAdmRptTblGrid];
			if (dti != null && dtAdmRptTblGrid != null)
			{
				if (bClear)
				{
					dtAdmRptTblGrid.Rows.Clear();
				}
				cFind.Text = string.Empty;
				int iExist = dtAdmRptTblGrid.Rows.Count;
				...
				foreach (DataRowView drv in dti.DefaultView)
				{
					...
					{
						DataTable dtd;
						dtd = jj==iExist ? null : (DataTable)Session[KEY_dtCelNum164];
						if (dtd == null)
						{
							try
							{
						...
						ScrImportDdl(dtd.DefaultView, drv, true, "RptTblId", "CelNum164", "CelNum164Text", 200, "2", false);
					}
					jj = jj + 1;
				}
				iRow = dti.DefaultView.Count - iExist;
				if (!dti.Columns.Contains("_NewRow")) { dti.Columns.Add("_NewRow"); }
				Session[KEY_dtAdmRptTblGrid] = dti;
				cAdmRptTblGrid_DataBind(dti.DefaultView);
		...
		public void cImportButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cFNameO.Text != string.Empty && cWorkSheet.Items.Count > 0 && cWorkSheet.SelectedItem.Text != string.Empty && cStartRow.Text != string.Empty)
			{
				...
				try
				{
					if (cAdmRptTblGrid.EditIndex < 0 || UpdateGridRow(cAdmRptTblGrid, new CommandEventArgs("Save", "")))
					{
						Session[KEY_scrImport] = ImportFile(dtAdmRptTblGrid.Copy(),cFNameO.Text,cWorkSheet.SelectedItem.Text,cStartRow.Text,Config.PathTmpImport + cFName.Text);
					}
		...
		private DataTable ImportFile(DataTable dt, string fileName, string workSheet, string startRow, string fileFullName)
		{
			try
			{
				...
				{
					if (rows[iRow][1].ToString() == string.Empty && rows[iRow][2].ToString() == string.Empty && rows[iRow][4].ToString() == string.Empty && rows[iRow][5].ToString() == string.Empty) {idel = idel + 1;}
					else
					{
		...
		private void GotoPage(int pageNo)
		{
			if (pageNo >= 0 && pageNo < GetTotalPages())
			{
				if (cAdmRptTblGrid.EditIndex < 0 || UpdateGridRow(cAdmRptTblGrid, new CommandEventArgs("Save", "")))
				{
					try { cAdmRptTblGridDataPager.SetPageProperties(cAdmRptTblGridDataPager.PageSize * pageNo, cAdmRptTblGridDataPager.MaximumRows, false); cAdmRptTblGrid_DataBind(null); }
					catch
					{
						try { cAdmRptTblGridDataPager.SetPageProperties(0, cAdmRptTblGridDataPager.MaximumRows, false); cAdmRptTblGrid_DataBind(null); } catch {}
					}
				}
			}
		}

		protected void cAdmRptTblGrid_OnItemCommand(object sender, ListViewCommandEventArgs e)
		{
		}

		private void cAdmRptTblGrid_DataBind(DataView dvAdmRptTblGrid)
		{
			if (dvAdmRptTblGrid == null)
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmRptTblGrid];
				dvAdmRptTblGrid = dt != null ? dt.DefaultView : null;
			}
			if (dvAdmRptTblGrid != null)
			{
				LcAuth = GetAuthCol().AsEnumerable().ToDictionary<DataRow,string>(dr=>dr["ColName"].ToString());
				cAdmRptTblGrid.DataSource = dvAdmRptTblGrid;
				cAdmRptTblGrid.DataBind();
		...
		protected void cAdmRptTblGrid_OnSorting(object sender, ListViewSortEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmRptTblGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null)
			{
				if (cAdmRptTblGrid.EditIndex < 0 || UpdateGridRow(cAdmRptTblGrid, new CommandEventArgs("Save", "")))
				{
					...
					Session[KEY_dtAdmRptTblGrid] = dt;
					cAdmRptTblGrid_DataBind(dv);
				}
			}
		}

		protected void cAdmRptTblGrid_OnPagePropertiesChanging(object sender, PagePropertiesChangingEventArgs e)
		{
		    cAdmRptTblGridDataPager.SetPageProperties(e.StartRowIndex, e.MaximumRows, false);
		    DataTable dt = (DataTable)Session[KEY_dtAdmRptTblGrid];
		    DataView dv = dt != null ? dt.DefaultView : null;
			cAdmRptTblGrid_DataBind(dv); cAdmRptTblGrid.EditIndex = -1;
		}

		private void GridChkPgDirty(ListViewItem lvi)
		{
				WebControl cc = null;
				cc = ((WebControl)lvi.FindControl("cRowNum164"));
				...
				cc = ((WebControl)lvi.FindControl("cRowHeight164"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cRowVisibility164"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cCelNum164"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cCelColSpan164"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
		}

		protected void cAdmRptTblGrid_OnItemDataBound(object sender, ListViewItemEventArgs e)
		{
			...
			if (cAdmRptTblGrid.EditIndex > -1 && GetDataItemIndex(cAdmRptTblGrid.EditIndex) == e.Item.DataItemIndex)
			{
				...
				SetCelNum164((RoboCoder.WebControls.ComboBox)e.Item.FindControl("cCelNum164"),dvAdmRptTblGrid[e.Item.DataItemIndex]["CelNum164"].ToString(), e.Item);
			...
			if (dtAuthRow != null)
			{
				...
				{
					cc = e.Item.FindControl("cAdmRptTblGridDelete"); if (cc != null) { cc.Visible = false; }
					cc = e.Item.FindControl("cAdmRptTblGridEdit"); if (cc != null) { cc.Visible = false; }
				}
				else
				{
			        HtmlTableRow tr = e.Item.FindControl("cAdmRptTblGridRow") as HtmlTableRow;
			        LinkButton lb = e.Item.FindControl("cAdmRptTblGridEdit") as LinkButton;
		...
		protected void cRptCelId164hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "0") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "0"; Session[KEY_lastSortExp] = "RptCelId164";Session[KEY_lastSortImg] = "cRptCelId164hi";
			cAdmRptTblGrid_OnSorting(sender, new ListViewSortEventArgs("RptCelId164", SortDirection.Ascending));
		}

		protected void cRowNum164hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "1") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "1"; Session[KEY_lastSortExp] = "RowNum164";Session[KEY_lastSortImg] = "cRowNum164hi";
			cAdmRptTblGrid_OnSorting(sender, new ListViewSortEventArgs("RowNum164", SortDirection.Ascending));
		}

		protected void cRowHeight164hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "2") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "2"; Session[KEY_lastSortExp] = "RowHeight164";Session[KEY_lastSortImg] = "cRowHeight164hi";
			cAdmRptTblGrid_OnSorting(sender, new ListViewSortEventArgs("RowHeight164", SortDirection.Ascending));
		}

		protected void cRowVisibility164hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "3") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "3"; Session[KEY_lastSortExp] = "RowVisibility164";Session[KEY_lastSortImg] = "cRowVisibility164hi";
			cAdmRptTblGrid_OnSorting(sender, new ListViewSortEventArgs("RowVisibility164", SortDirection.Ascending));
		}

		protected void cCelNum164hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "4") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "4"; Session[KEY_lastSortExp] = "CelNum164Text";Session[KEY_lastSortImg] = "cCelNum164hi";
			cAdmRptTblGrid_OnSorting(sender, new ListViewSortEventArgs("CelNum164Text", SortDirection.Ascending));
		}

		protected void cCelColSpan164hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "5") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "5"; Session[KEY_lastSortExp] = "CelColSpan164";Session[KEY_lastSortImg] = "cCelColSpan164hi";
			cAdmRptTblGrid_OnSorting(sender, new ListViewSortEventArgs("CelColSpan164", SortDirection.Ascending));
		}

		protected void cAdmRptTblGrid_OnItemEditing(object sender, ListViewEditEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmRptTblGrid];
			if ((ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")) && dt != null && dt.DefaultView.Count > GetDataItemIndex(e.NewEditIndex))
			{
				cAdmRptTblGrid.EditIndex = e.NewEditIndex;
				cAdmRptTblGrid_DataBind(null);
				Control wc = null;
				string idx = Page.Request["__EVENTARGUMENT"];
				if (!string.IsNullOrEmpty(idx))
				{
				    Control ctrlToFocus = cAdmRptTblGrid.EditItem.FindControl("c" + idx);
				    if (((WebControl)ctrlToFocus).Enabled) { wc = ctrlToFocus; }
				}
				if (wc == null) { wc = FindEditableControl(cAdmRptTblGrid.EditItem); }
				if (wc != null)
             {
             ...
             MkMobileLabel(cAdmRptTblGrid.EditItem);
		    }
		    else { e.Cancel = true; }
		}
		...
		{
		    Label ml = null;
		    ml = lvi.FindControl("cRptCelId164ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(11); }
		    ml = lvi.FindControl("cRowNum164ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(12); }
		    ml = lvi.FindControl("cRowHeight164ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(13); }
		    ml = lvi.FindControl("cRowVisibility164ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(14); }
		    ml = lvi.FindControl("cCelNum164ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(15); }
		    ml = lvi.FindControl("cCelColSpan164ml") as Label;
		...
		{
			TextBox tb = null;
			tb = (TextBox)lvi.FindControl("cRowNum164");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["RowNum164"] = tb.Text;} else {dr["RowNum164"] = System.DBNull.Value;}
			}
			tb = (TextBox)lvi.FindControl("cRowHeight164");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["RowHeight164"] = tb.Text;} else {dr["RowHeight164"] = System.DBNull.Value;}
			}
			tb = (TextBox)lvi.FindControl("cRowVisibility164");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["RowVisibility164"] = tb.Text;} else {dr["RowVisibility164"] = System.DBNull.Value;}
			}
			RoboCoder.WebControls.ComboBox cbb = null;
			cbb = (RoboCoder.WebControls.ComboBox)lvi.FindControl("cCelNum164");
			if (cbb != null)
			{
				if (cbb.SelectedValue != string.Empty)
				{
					dr["CelNum164"] = cbb.SelectedValue;
					dr["CelNum164Text"] = cbb.SelectedText;
				}
				else
				{
					dr["CelNum164"] = System.DBNull.Value;
					dr["CelNum164Text"] = System.DBNull.Value;
				}
			}
			tb = (TextBox)lvi.FindControl("cCelColSpan164");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["CelColSpan164"] = tb.Text;} else {dr["CelColSpan164"] = System.DBNull.Value;}
			}
		...
		protected void cAdmRptTblGrid_OnItemUpdating(object sender, ListViewUpdateEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmRptTblGrid];
			if (dt != null && dt.Rows.Count > 0)
			{
				DataRow dr = dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row;
			    GridFill(cAdmRptTblGrid.EditItem, dt, dr, false);
			    dr["_NewRow"] = "N";
			    if (dt.Columns.Contains("_SortOrder")) dr["_SortOrder"] = null;
			}
		}

		protected void cAdmRptTblGrid_OnItemCanceling(object sender, ListViewCancelEventArgs e)
		{
			PanelTop.Update();
			DataTable dt = (DataTable)Session[KEY_dtAdmRptTblGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			...
			{
				cAdmRptTblGrid_OnItemDeleting(sender, new ListViewDeleteEventArgs(e.ItemIndex));
			}
			if (dt.Columns.Contains("_SortOrder")) dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row["_SortOrder"] = null;
			cAdmRptTblGrid.EditIndex = -1; cAdmRptTblGrid_DataBind(null);
		}
		...
		protected void cDeleteAllButton_Click(object sender, System.EventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmRptTblGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null && (cAdmRptTblGrid.EditIndex < 0 || UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				GotoPage(0);
				while (dv.Count > 0) {dv.Delete(0);}
				Session[KEY_dtAdmRptTblGrid] = dt;
				cAdmRptTblGrid.EditIndex = -1; cAdmRptTblGrid_DataBind(dv);
			}
		}

		protected void cAdmRptTblGrid_OnItemDeleting(object sender, ListViewDeleteEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmRptTblGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			DataRow dr = dv != null ? dv[GetDataItemIndex(e.ItemIndex)].Row : null;
			if (dv != null && (cAdmRptTblGrid.EditIndex == e.ItemIndex || UpdateGridRow(sender, new CommandEventArgs("Save",""))))
			{
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cRptTblId162.Text == string.Empty) { cClearButton_Click(sender, new EventArgs()); ShowDirty(false); PanelTop.Update();}
			else { PopAdmRptTbl92List(sender, e, false, cRptTblId162.Text); }
			// *** System Button Click (After) Web Rule starts here *** //
		}
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmRptTbl92List); PopAdmRptTbl92List(sender, e, false, null);
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			if (ValidPage() && (cAdmRptTblGrid.EditIndex < 0 || UpdateGridRow(cAdmRptTblGrid, new CommandEventArgs("Save", ""))))
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmRptTblGrid];
				DataView dv = dt != null ? dt.DefaultView : null;
				string ftr = string.Empty;
				...
				if (string.IsNullOrEmpty(cAdmRptTbl92List.SelectedValue))	// Add
				{
					if (ds != null)
					{
						pid = (new AdminSystem()).AddData(92,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc,noTrans,commandTimeOut);
					}
					if (!string.IsNullOrEmpty(pid))
					{
						cAdmRptTbl92List.ClearSearch(); Session.Remove(KEY_dtAdmRptTbl92List);
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmRptTbl92List(sender, e, false, pid);
						rtn = GetScreenHlp().Rows[0]["AddMsg"].ToString();
					}
				}
				else {
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(92,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc,noTrans,commandTimeOut)) {bValid7 = true;}
					if (bValid7)
					{
						cAdmRptTbl92List.ClearSearch(); Session.Remove(KEY_dtAdmRptTbl92List);
						Session[KEY_currPageIndex] = GetCurrPageIndex();
						ShowDirty(false); PopAdmRptTbl92List(sender, e, false, ds.Tables["AdmRptTbl"].Rows[0]["RptTblId162"]);
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cRptTblId162.Text != string.Empty)
			{
		...
		private AdmRptTbl92 PrepAdmRptTblData(DataView dv, bool bAdd)
		{
			...
			if (bAdd) { dr["RptTblId162"] = string.Empty; } else { dr["RptTblId162"] = cRptTblId162.Text; }
			drType["RptTblId162"] = "Numeric"; drDisp["RptTblId162"] = "TextBox";
			try {dr["RptCtrId162"] = cRptCtrId162.SelectedValue;} catch {}
			drType["RptCtrId162"] = "Numeric"; drDisp["RptCtrId162"] = "AutoComplete";
			try {dr["ParentId162"] = cParentId162.SelectedValue;} catch {}
			drType["ParentId162"] = "Numeric"; drDisp["ParentId162"] = "AutoComplete";
			try {dr["ReportId162"] = cReportId162.SelectedValue;} catch {}
			drType["ReportId162"] = "Numeric"; drDisp["ReportId162"] = "AutoComplete";
			try {dr["TblToggle162"] = cTblToggle162.SelectedValue;} catch {}
			drType["TblToggle162"] = "Numeric"; drDisp["TblToggle162"] = "AutoComplete";
			try {dr["TblGrouping162"] = cTblGrouping162.SelectedValue;} catch {}
			drType["TblGrouping162"] = "Numeric"; drDisp["TblGrouping162"] = "AutoComplete";
			try {dr["RptTblTypeCd162"] = cRptTblTypeCd162.SelectedValue;} catch {}
			drType["RptTblTypeCd162"] = "Char"; drDisp["RptTblTypeCd162"] = "DropDownList";
			try {dr["TblVisibility162"] = cTblVisibility162.SelectedValue;} catch {}
			drType["TblVisibility162"] = "Char"; drDisp["TblVisibility162"] = "RadioButtonList";
			try {dr["TblRepeatNew162"] = base.SetBool(cTblRepeatNew162.Checked);} catch {}
			drType["TblRepeatNew162"] = "Char"; drDisp["TblRepeatNew162"] = "CheckBox";
			try {dr["TblOrder162"] = cTblOrder162.Text.Trim();} catch {}
			drType["TblOrder162"] = "Numeric"; drDisp["TblOrder162"] = "TextBox";
			try {dr["ColWidth162"] = cColWidth162.Text.Trim();} catch {}
			...
			if (dv != null)
			{
				...
				if (bAdd)
				{
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmRptTblAdd"].Rows.Add(MakeColRow(ds.Tables["AdmRptTblAdd"].NewRow(), drv, true));
					}
				}
				else
				{
					...
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmRptTblUpd"].Rows.Add(MakeColRow(ds.Tables["AdmRptTblUpd"].NewRow(), drv, false));
					}
					...
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmRptTblDel"].Rows.Add(MakeColRow(ds.Tables["AdmRptTblDel"].NewRow(), drv, false));
					}
		...
		public bool CanAct(char typ) { return CanAct(typ, GetAuthRow(), cAdmRptTbl92List.SelectedValue.ToString()); }
		...
		private bool ValidPage()
		{
			...
			DataTable dt = null;
			dt = (DataTable)Session[KEY_dtRptCtrId162];
			if (dt != null && dt.Rows.Count <= 0)
			{
				bErrNow.Value = "Y"; PreMsgPopup("Value is expected for 'RptCtrId', please investigate."); return false;
			}
			dt = (DataTable)Session[KEY_dtReportId162];
			if (dt != null && dt.Rows.Count <= 0)
			{
				bErrNow.Value = "Y"; PreMsgPopup("Value is expected for 'ReportId', please investigate."); return false;
			}
			dt = (DataTable)Session[KEY_dtRptTblTypeCd162];
			if (dt != null && dt.Rows.Count <= 0)
			{
				bErrNow.Value = "Y"; PreMsgPopup("Value is expected for 'RptTblTypeCd', please investigate."); return false;
			}
			dt = (DataTable)Session[KEY_dtCelNum164];
			if (dt != null && dt.Rows.Count <= 0)
			{
				bErrNow.Value = "Y"; PreMsgPopup("Value is expected for 'CelNum', please investigate."); return false;
			}
			return true;
		}
		...
		{
			dr["RptTblId162"] = cRptTblId162.Text;
			DataTable dtAuth = GetAuthCol();
			if (dtAuth != null)
			{
				dr["RptCelId164"] = drv["RptCelId164"].ToString().Trim();
				dr["RowNum164"] = drv["RowNum164"].ToString().Trim();
				dr["RowHeight164"] = drv["RowHeight164"].ToString().Trim();
				dr["RowVisibility164"] = drv["RowVisibility164"].ToString().Trim();
				if (bAdd && dtAuth.Rows[14]["ColReadOnly"].ToString() == "Y" && dr["RowVisibility164"].ToString() == string.Empty) {dr["RowVisibility164"] = System.DBNull.Value;}
				dr["CelNum164"] = drv["CelNum164"];
				dr["CelColSpan164"] = drv["CelColSpan164"].ToString().Trim();
				if (bAdd && dtAuth.Rows[16]["ColReadOnly"].ToString() == "Y" && dr["CelColSpan164"].ToString() == string.Empty) {dr["CelColSpan164"] = System.DBNull.Value;}
			}
			return dr;
		}
		...
		protected void cAdmRptTblGrid_OnPreRender(object sender, System.EventArgs e)
		{
			System.Web.UI.WebControls.Image hi = null;
			hi = (System.Web.UI.WebControls.Image)cAdmRptTblGrid.FindControl("cRptCelId164hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmRptTblGrid.FindControl("cRowNum164hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmRptTblGrid.FindControl("cRowHeight164hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmRptTblGrid.FindControl("cRowVisibility164hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmRptTblGrid.FindControl("cCelNum164hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmRptTblGrid.FindControl("cCelColSpan164hi"); if (hi != null) { hi.Visible = false; }
			if (Session[KEY_lastSortImg] != null)
			{
				hi = (System.Web.UI.WebControls.Image)cAdmRptTblGrid.FindControl((string)Session[KEY_lastSortImg]);
				if (hi != null) { hi.ImageUrl = Utils.AddTilde((string)Session[KEY_lastSortUrl]); hi.Visible = true; }
			}
			IgnoreHeaderConfirm((LinkButton)cAdmRptTblGrid.FindControl("cRptCelId164hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmRptTblGrid.FindControl("cRowNum164hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmRptTblGrid.FindControl("cRowHeight164hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmRptTblGrid.FindControl("cRowVisibility164hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmRptTblGrid.FindControl("cCelNum164hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmRptTblGrid.FindControl("cCelColSpan164hl"));
		}

		private string GetButtonId(ListViewItem lvi)
		{
			string ButtonID = String.Empty;
			Control c = lvi.FindControl("cAdmRptTblGridEdit");
		...
		{
		    return cAdmRptTblGridDataPager.StartRowIndex / cAdmRptTblGridDataPager.PageSize;
		}

		private int GetTotalPages()
		{
		    return (int)Math.Ceiling((double)cAdmRptTblGridDataPager.TotalRowCount / cAdmRptTblGridDataPager.PageSize);
		}

		private int GetDataItemIndex(int editIndex)
		{
		    return cAdmRptTblGridDataPager.StartRowIndex + editIndex;
		}

		protected void cAdmRptTblGrid_OnLayoutCreated(object sender, EventArgs e)
		{
		    // Header:
		    LinkButton lb = null;
		    lb = cAdmRptTblGrid.FindControl("cRptCelId164hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(11); lb.ToolTip = ColumnToolTip(11); lb.Parent.Visible = GridColumnVisible(11); }
		    lb = cAdmRptTblGrid.FindControl("cRowNum164hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(12); lb.ToolTip = ColumnToolTip(12); lb.Parent.Visible = GridColumnVisible(12); }
		    lb = cAdmRptTblGrid.FindControl("cRowHeight164hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(13); lb.ToolTip = ColumnToolTip(13); lb.Parent.Visible = GridColumnVisible(13); }
		    lb = cAdmRptTblGrid.FindControl("cRowVisibility164hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(14); lb.ToolTip = ColumnToolTip(14); lb.Parent.Visible = GridColumnVisible(14); }
		    lb = cAdmRptTblGrid.FindControl("cCelNum164hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(15); lb.ToolTip = ColumnToolTip(15); lb.Parent.Visible = GridColumnVisible(15); }
		    lb = cAdmRptTblGrid.FindControl("cCelColSpan164hl") as LinkButton;
			...
			if (dtAuthRow != null)
			{
				...
				{
		            lb = cAdmRptTblGrid.FindControl("cDeleteAllButton") as LinkButton; if (lb != null) { lb.Visible = false; }
				}
			}
		    // footer:
		    Label gc = null;
		    gc = cAdmRptTblGrid.FindControl("cRptCelId164fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(11); }
		    gc = cAdmRptTblGrid.FindControl("cRowNum164fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(12); }
		    gc = cAdmRptTblGrid.FindControl("cRowHeight164fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(13); }
		    gc = cAdmRptTblGrid.FindControl("cRowVisibility164fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(14); }
		    gc = cAdmRptTblGrid.FindControl("cCelNum164fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(15); }
		    gc = cAdmRptTblGrid.FindControl("cCelColSpan164fl") as Label;
		...
		private void PreMsgPopup(string msg) { PreMsgPopup(msg,cAdmRptTbl92List,null); }

...
namespace RO.Web
{
	public partial class AdmUsrGroupModule : RO.Web.ModuleBase
	{

		private const string KEY_lastAddedRow = "Cache:lastAddedRow3_5";
		private const string KEY_lastSortOrd = "Cache:lastSortOrd3_5";
		private const string KEY_lastSortImg = "Cache:lastSortImg3_5";
		private const string KEY_lastSortCol = "Cache:lastSortCol3_5";
		private const string KEY_lastSortExp = "Cache:lastSortExp3_5";
		private const string KEY_lastSortUrl = "Cache:lastSortUrl3_5";
		private const string KEY_lastSortTog = "Cache:lastSortTog3_5";
		private const string KEY_lastImpPwdOvride = "Cache:lastImpPwdOvride3_5";
		private const string KEY_cntImpPwdOvride = "Cache:cntImpPwdOvride3_5";
		private const string KEY_currPageIndex = "Cache:currPageIndex3_5";
		private const string KEY_bHiImpVisible = "Cache:bHiImpVisible3_5";
		private const string KEY_bShImpVisible = "Cache:bShImpVisible3_5";
		private const string KEY_dtAdmUsrGroupGrid = "Cache:dtAdmUsrGroupGrid";
		private const string KEY_scrImport = "Cache:scrImport3_5";
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_5";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_5";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_5";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_5";
		private const string KEY_dtLabel = "Cache:dtLabel3_5";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_5";
		private const string KEY_dtScrCri = "Cache:dtScrCri3_5";
		private const string KEY_dsScrCriVal = "Cache:dsScrCriVal3_5";

		private const string KEY_dtRowAuthorityId7 = "Cache:dtRowAuthorityId7";
		private const string KEY_dtCompanyId7 = "Cache:dtCompanyId7";
		private const string KEY_dtCompanyId58 = "Cache:dtCompanyId58";
		private const string KEY_dtProjectId58 = "Cache:dtProjectId58";
		private const string KEY_dtSystemId58 = "Cache:dtSystemId58";
		private const string KEY_dtSysRowAuthorityId58 = "Cache:dtSysRowAuthorityId58";

		private const string KEY_dtSystems = "Cache:dtSystems3_5";
		private const string KEY_sysConnectionString = "Cache:sysConnectionString3_5";
		private const string KEY_dtAdmUsrGroup5List = "Cache:dtAdmUsrGroup5List";
		private const string KEY_bNewVisible = "Cache:bNewVisible3_5";
		private const string KEY_bNewSaveVisible = "Cache:bNewSaveVisible3_5";
		private const string KEY_bCopyVisible = "Cache:bCopyVisible3_5";
		private const string KEY_bCopySaveVisible = "Cache:bCopySaveVisible3_5";
		private const string KEY_bDeleteVisible = "Cache:bDeleteVisible3_5";
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_5";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_5";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_5";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				DataTable dtMenuAccess = (new MenuSystem()).GetMenu(base.LUser.CultureId, 3, base.LImpr, LcSysConnString, LcAppPw,5, null, null);
				...
				Session.Remove(KEY_dtRowAuthorityId7);
				Session.Remove(KEY_dtCompanyId7);
				Session.Remove(KEY_dtCompanyId58);
				Session.Remove(KEY_dtProjectId58);
				Session.Remove(KEY_dtSystemId58);
				Session.Remove(KEY_dtSysRowAuthorityId58);
				SetButtonHlp();
				GetSystems();
				SetColumnAuthority();
				GetGlobalFilter();
				...
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 5, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmUsrGroup5List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=5&typ=N&sys=3','','',''); return false;"; }
			...
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmUsrGroupGrid];
				if (dt != null)
				{
		...
		private void InitializeComponent()
		{
			...
			if (LcSysConnString == null)
			{
				...
				{
					DataTable dt;
					dt = (new AdminSystem()).GetAuthRow(5,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
					...
					{
						cAdmUsrGroupGrid.PreRender += new EventHandler(cAdmUsrGroupGrid_OnPreRender);
					}
				}
		...
		private void SetSystem(byte SystemId)
		{
			...
			try
			{
				...
				if ((Config.DeployType == "DEV" || row["dbAppDatabase"].ToString() == base.CPrj.EntityCode + "View") && !(base.CPrj.EntityCode != "RO" && row["SysProgram"].ToString() == "Y") && (new AdminSystem()).IsRegenNeeded(string.Empty,5,0,0,LcSysConnString,LcAppPw))
				{
					(new GenScreensSystem()).CreateProgram(5, "User Group", row["dbAppDatabase"].ToString(), base.CPrj, base.CSrc, base.CTar, LcAppConnString, LcAppPw);
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(5,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(5,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void SetClientRule(ListViewDataItem ee, bool isEditItem)
		{
			...
			if (dvRul.Count > 0)
			{
				...
				{
					if (ee == null || drv["ScriptEvent"].ToString().Substring(0,2).ToLower() != "on" || (cAdmUsrGroupGrid.EditIndex > -1 && GetDataItemIndex(cAdmUsrGroupGrid.EditIndex) == ee.DataItemIndex))
					{
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(5,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(5,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,5,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(5,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(5,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(5,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				try
				{
					try {dv = new DataView((new AdminSystem()).GetExp(5,"GetExpAdmUsrGroup5","Y",null,null,filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
					catch {dv = new DataView((new AdminSystem()).GetExp(5,"GetExpAdmUsrGroup5","N",null,null,filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(5,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					{
						if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["UsrGroupId7"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["UsrGroupName7"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(drv["RowAuthorityId7"].ToString() + (char)9 + drv["RowAuthorityId7Text"].ToString() + (char)9);}
						if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(drv["CompanyId7"].ToString() + (char)9 + drv["CompanyId7Text"].ToString() + (char)9);}
						if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["UsrGroupAuthId58"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append(drv["CompanyId58"].ToString() + (char)9 + drv["CompanyId58Text"].ToString() + (char)9);}
						if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append(drv["ProjectId58"].ToString() + (char)9 + drv["ProjectId58Text"].ToString() + (char)9);}
						if (dtAu.Rows[10]["ColExport"].ToString() == "Y") {sb.Append(drv["SystemId58"].ToString() + (char)9 + drv["SystemId58Text"].ToString() + (char)9);}
						if (dtAu.Rows[11]["ColExport"].ToString() == "Y") {sb.Append(drv["SysRowAuthorityId58"].ToString() + (char)9 + drv["SysRowAuthorityId58Text"].ToString() + (char)9);}
						sb.Append(Environment.NewLine);
					}
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmUsrGroup.csv"; Session["ExportStr"] = (Config.ExportExcelCSV ? "sep=\t\n": "") + sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmUsrGroup.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private string GetRtfGrid(DataView dv)
		{
			...
			try
			{
				...
				foreach (DataRowView drv in dv)
				{
					...
					sb.Append("\r\n");
					sb.Append(@"{");
					if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["UsrGroupId7"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["UsrGroupName7"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(drv["RowAuthorityId7Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(drv["CompanyId7Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["UsrGroupAuthId58"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append(drv["CompanyId58Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append(drv["ProjectId58Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[10]["ColExport"].ToString() == "Y") {sb.Append(drv["SystemId58Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[11]["ColExport"].ToString() == "Y") {sb.Append(drv["SysRowAuthorityId58Text"].ToString() + @"\cell ");}
					sb.Append(@"}");
					sb.Append("\r\n");
					sb.Append(@"\pard \ql \li0\ri0\widctlpar\intbl\aspalpha\aspnum\adjustright\rin0\lin0 {");
					sb.Append(@"\insrsid2981395 \trowd \irow0\irowband0\lastrow \ts15\trgaph108\trleft-108\trbrdrt\brdrs\brdrw10 \trbrdrl\brdrs\brdrw10 \trbrdrb\brdrs\brdrw10 \trbrdrr\brdrs\brdrw10 \trbrdrh\brdrs\brdrw10 \trbrdrv\brdrs\brdrw10 ");
		...
		private void GetPageInfo()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastPageInfo(5, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 5, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			// *** Criteria Trigger (before) Web Rule starts here *** //
			DataTable dt = (DataTable)Session[KEY_dtAdmUsrGroupGrid];
			if (cAdmUsrGroupGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
			if (IsPostBack && cCriteria.Visible) ReBindCriteria(GetScrCriteria());
			UpdCriteria(true);
			cAdmUsrGroup5List.ClearSearch(); Session.Remove(KEY_dtAdmUsrGroup5List);
			PopAdmUsrGroup5List(sender, e, false, null);
			}
		}
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("5", LcSysConnString, LcAppPw);
				}
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmUsrGroup5" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmUsrGroup5" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("5", "AdmUsrGroup5", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		private void SetRowAuthorityId7(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtRowAuthorityId7];
			...
			if (ddl != null)
			{
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(5,"GetDdlRowAuthorityId3S26",true,bAll,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					if (dv.Table.Columns.Contains("UsrGroupId"))
					{
						ss = "(UsrGroupId is null";
						if (string.IsNullOrEmpty(cAdmUsrGroup5List.SelectedValue)) {ss = ss + ")";} else {ss = ss + " OR UsrGroupId = " + cAdmUsrGroup5List.SelectedValue + ")";}
					}
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "RowAuthorityId7 is not NULL"; }
					dv.RowFilter = ss;
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(5,"GetDdlRowAuthorityId3S26",true,true,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtRowAuthorityId7] = dv.Table;
				}
			}
		}

		private void SetCompanyId7(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtCompanyId7];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (ddl != null)
			{
				string ss = string.Empty;
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(5,"GetDdlCompanyId3S1752",true,bAll,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					...
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "CompanyId7 is not NULL"; }
					dv.RowFilter = ss;
					ddl.DataSource = dv; ddl.DataBind();
					li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					else if (!bFirst)
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(5,"GetDdlCompanyId3S1752",true,true,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtCompanyId7] = dv.Table;
				}
			}
		}

		private void SetCompanyId58(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtCompanyId58];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (ddl != null)
			{
				string ss = string.Empty;
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(5,"GetDdlCompanyId3S1744",true,bAll,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					...
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "CompanyId58 is not NULL"; }
					dv.RowFilter = ss;
					ddl.DataSource = dv; ddl.DataBind();
					li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					else if (!bFirst)
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(5,"GetDdlCompanyId3S1744",true,true,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtCompanyId58] = dv.Table;
				}
			}
		}

		private void SetProjectId58(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtProjectId58];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (ddl != null)
			{
				string ss = string.Empty;
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(5,"GetDdlProjectId3S1745",true,bAll,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					...
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "ProjectId58 is not NULL"; }
					dv.RowFilter = ss;
					ddl.DataSource = dv; ddl.DataBind();
					li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					else if (!bFirst)
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(5,"GetDdlProjectId3S1745",true,true,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtProjectId58] = dv.Table;
				}
			}
		}

		private void SetSystemId58(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtSystemId58];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (ddl != null)
			{
				string ss = string.Empty;
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(5,"GetDdlSystemId3S552",true,bAll,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					...
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "SystemId58 is not NULL"; }
					dv.RowFilter = ss;
					ddl.DataSource = dv; ddl.DataBind();
					li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					else if (!bFirst)
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(5,"GetDdlSystemId3S552",true,true,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtSystemId58] = dv.Table;
				}
			}
		}

		private void SetSysRowAuthorityId58(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtSysRowAuthorityId58];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (ddl != null)
			{
				string ss = string.Empty;
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(5,"GetDdlSysRowAuthorityId3S553",true,bAll,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					...
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "SysRowAuthorityId58 is not NULL"; }
					dv.RowFilter = ss;
					ddl.DataSource = dv; ddl.DataBind();
					li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					else if (!bFirst)
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(5,"GetDdlSysRowAuthorityId3S553",true,true,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtSysRowAuthorityId58] = dv.Table;
				}
			}
		}

		private DataView GetAdmUsrGroup5List()
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmUsrGroup5List];
			if (dt == null)
			{
				CheckAuthentication(false);
				int filterId = 0;
				...
				try
				{
					try {dt = (new AdminSystem()).GetLis(5,"GetLisAdmUsrGroup5",true,"Y",0,null,null,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
					catch {dt = (new AdminSystem()).GetLis(5,"GetLisAdmUsrGroup5",true,"N",0,null,null,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				}
		...
		private void PopAdmUsrGroup5List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			DataView dv = GetAdmUsrGroup5List();
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			cAdmUsrGroup5List.AutoCompleteUrl = "AutoComplete.aspx/LisSuggests";
			cAdmUsrGroup5List.DataContext = context;
			if (dv.Table == null) return;
			cAdmUsrGroup5List.DataSource = dv;
			cAdmUsrGroup5List.Visible = true;
			if (cAdmUsrGroup5List.Items.Count <= 0) {cAdmUsrGroup5List.Visible = false; cAdmUsrGroup5List_SelectedIndexChanged(sender,e);}
			else
			{
				if (bPageLoad && !string.IsNullOrEmpty(Request.QueryString["key"]) && bUseCri.Value == string.Empty)
				{
					try {cAdmUsrGroup5List.SelectByValue(Request.QueryString["key"],string.Empty,true);} catch {cAdmUsrGroup5List.Items[0].Selected = true; cAdmUsrGroup5List_SelectedIndexChanged(sender, e);}
				    cScreenSearch.Visible = false; cSystem.Visible = false; cEditButton.Visible = true; cSaveCloseButton.Visible = cSaveButton.Visible;
				}
				else
				{
					if (ID != null) {if (!cAdmUsrGroup5List.SelectByValue(ID,string.Empty,true)) {ID = null;}}
					if (ID == null)
					{
						cAdmUsrGroup5List_SelectedIndexChanged(sender, e);
					}
				}
			}
		}
		...
		private void SetColumnAuthority()
		{
			...
			{
				base.SetFoldBehavior(cUsrGroupId7, dtAuth.Rows[0], cUsrGroupId7P1, cUsrGroupId7Label, cUsrGroupId7P2, null, dtLabel.Rows[0], null, null, null);
				base.SetFoldBehavior(cUsrGroupName7, dtAuth.Rows[1], cUsrGroupName7P1, cUsrGroupName7Label, cUsrGroupName7P2, null, dtLabel.Rows[1], cRFVUsrGroupName7, null, null);
				base.SetFoldBehavior(cRowAuthorityId7, dtAuth.Rows[2], cRowAuthorityId7P1, cRowAuthorityId7Label, cRowAuthorityId7P2, null, dtLabel.Rows[2], cRFVRowAuthorityId7, null, null);
				base.SetFoldBehavior(cCompanyId7, dtAuth.Rows[3], cCompanyId7P1, cCompanyId7Label, cCompanyId7P2, null, dtLabel.Rows[3], null, null, null);
			}
			if ((cUsrGroupName7.Attributes["OnChange"] == null || cUsrGroupName7.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cUsrGroupName7.Visible && !cUsrGroupName7.ReadOnly) {cUsrGroupName7.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cRowAuthorityId7.Attributes["OnChange"] == null || cRowAuthorityId7.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cRowAuthorityId7.Visible && cRowAuthorityId7.Enabled) {cRowAuthorityId7.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();document.getElementById('" + bConfirm.ClientID + "').value='N';";}
			if (cRowAuthorityId7Pwd.Attributes["OnChange"] == null || cRowAuthorityId7Pwd.Attributes["OnChange"].IndexOf("_bConfirm") < 0) {cRowAuthorityId7Pwd.Attributes["OnChange"] += "document.getElementById('" + bConfirm.ClientID + "').value='N';";}
			if ((cCompanyId7.Attributes["OnChange"] == null || cCompanyId7.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cCompanyId7.Visible && cCompanyId7.Enabled) {cCompanyId7.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
		}
		...
		private void ClearMaster(object sender, System.EventArgs e)
		{
			...
			if (dt.Rows[1]["ColVisible"].ToString() == "Y" && dt.Rows[1]["ColReadOnly"].ToString() != "Y") {cUsrGroupName7.Text = string.Empty;}
			if (dt.Rows[2]["ColVisible"].ToString() == "Y" && dt.Rows[2]["ColReadOnly"].ToString() != "Y") {SetRowAuthorityId7(cRowAuthorityId7,"7");}
			if (dt.Rows[3]["ColVisible"].ToString() == "Y" && dt.Rows[3]["ColReadOnly"].ToString() != "Y") {SetCompanyId7(cCompanyId7,string.Empty);}
			// *** Default Value (Folder) Web Rule starts here *** //
		}

		private void InitMaster(object sender, System.EventArgs e)
		{
			cUsrGroupId7.Text = string.Empty;
			cUsrGroupName7.Text = string.Empty;
			SetRowAuthorityId7(cRowAuthorityId7,"7");
			SetCompanyId7(cCompanyId7,string.Empty);
			// *** Default Value (Folder) Web Rule starts here *** //
		}
		protected void cAdmUsrGroup5List_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmUsrGroup5List_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmUsrGroup5List_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			InitMaster(sender, e);      // Do this first to enable defaults for non-database columns.
			DataTable dt = null;
			try
			{
				dt = (new AdminSystem()).GetMstById("GetAdmUsrGroup5ById",cAdmUsrGroup5List.SelectedValue,null,null);
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				if (dt.Rows.Count > 0)
				{
					cAdmUsrGroup5List.SetDdVisible();
					DataRow dr = dt.Rows[0];
					try {cUsrGroupId7.Text = RO.Common3.Utils.fmNumeric("0",dr["UsrGroupId7"].ToString(),base.LUser.Culture);} catch {cUsrGroupId7.Text = string.Empty;}
					try {cUsrGroupName7.Text = dr["UsrGroupName7"].ToString();} catch {cUsrGroupName7.Text = string.Empty;}
					SetRowAuthorityId7(cRowAuthorityId7,dr["RowAuthorityId7"].ToString()); if (cRowAuthorityId7.SelectedIndex <= 0 && !(cRowAuthorityId7.Enabled && cRowAuthorityId7.Visible)) { SetRowAuthorityId7(cRowAuthorityId7,"7"); }
					SetCompanyId7(cCompanyId7,dr["CompanyId7"].ToString());
				}
			}
			cButPanel.DataBind(); if (!cSaveButton.Visible) { cInsRowButton.Visible = false; }
			DataTable dtAdmUsrGroupGrid = null;
			int filterId = 0; if (Utils.IsInt(cFilterId.SelectedValue)) { filterId = int.Parse(cFilterId.SelectedValue); }
			try
			{
				dtAdmUsrGroupGrid = (new AdminSystem()).GetDtlById(5,"GetAdmUsrGroup5DtlById",cAdmUsrGroup5List.SelectedValue,null,null,filterId,base.LImpr,base.LCurr);
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (!dtAdmUsrGroupGrid.Columns.Contains("_NewRow")) { dtAdmUsrGroupGrid.Columns.Add("_NewRow"); }
			Session[KEY_dtAdmUsrGroupGrid] = dtAdmUsrGroupGrid;
			cAdmUsrGroupGrid.EditIndex = -1;
			cAdmUsrGroupGridDataPager.PageSize = Int16.Parse(cPgSize.Text); GotoPage(0);
			if (Session[KEY_lastSortCol] != null)
			{
				cAdmUsrGroupGrid_OnSorting(sender, new ListViewSortEventArgs((string)Session[KEY_lastSortExp], SortDirection.Ascending));
			}
			else if (dtAdmUsrGroupGrid.Rows.Count <= 0 || (!((GetAuthRow().Rows[0]["AllowUpd"].ToString() == "N" || GetAuthRow().Rows[0]["ViewOnly"].ToString() == "G") && GetAuthRow().Rows[0]["AllowAdd"].ToString() == "N") && !(Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N") && dtAdmUsrGroupGrid.Rows.Count == 1))
			{
				cAdmUsrGroupGrid_DataBind(dtAdmUsrGroupGrid.DefaultView);
			}
			cFindButton_Click(sender, e);
			cNaviPanel.Visible = true; cImportPwdPanel.Visible = false; Session.Remove(KEY_scrImport);
			ScriptManager.GetCurrent(Parent.Page).SetFocus(cAdmUsrGroup5List.FocusID);
			ShowDirty(false); PanelTop.Update();
			// *** List Selection (End of) Web Rule starts here *** //
		}

		protected void cRowAuthorityId7_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			...
			TextBox pwd = null;
			if (cRowAuthorityId7.Items.Count > 0 && Session[KEY_dtRowAuthorityId7] != null)
			{
				DataView dv = ((DataTable)Session[KEY_dtRowAuthorityId7]).DefaultView; dv.RowFilter = string.Empty;
				DataRowView dr = cRowAuthorityId7.SelectedIndex >= 0 && cRowAuthorityId7.SelectedIndex < dv.Count ? dv[cRowAuthorityId7.SelectedIndex] : dv[0];
				try
				{
					...
					{
						cRowAuthorityId7.Visible = false; cRowAuthorityId7Pwd.Visible = true; pwd = cRowAuthorityId7Pwd;
					}
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
		...
		protected void cRowAuthorityId7Pwd_TextChanged(object sender, System.EventArgs e)
		{
			bool bOk = true;
			DataView dv = ((DataTable)Session[KEY_dtRowAuthorityId7]).DefaultView;
			DataRowView dr = dv[cRowAuthorityId7.SelectedIndex];
			bool bValid2 = false;
			try
			{
			...
			if (bValid2)
			{
				if (dr["PromptModal"].ToString() == "N") {SetRowAuthorityId7(cRowAuthorityId7,string.Empty);} else {bOk = false;}
			}
			if (bOk)
			{
				cRowAuthorityId7.Visible = true; cRowAuthorityId7Pwd.Visible = false; cRowAuthorityId7Pwl.Visible = false;
				ScriptManager.GetCurrent(Parent.Page).SetFocus(cRowAuthorityId7.ClientID);
			}
			else {ScriptManager.GetCurrent(Parent.Page).SetFocus(cRowAuthorityId7Pwd.ClientID); cRowAuthorityId7Pwl.Visible = true;}
		}

		protected void cFillerBtn_Click(object sender, System.EventArgs e)
		{
			// *** On Change/ On Click Web Rule starts here *** //
			EnableValidators(true);  // Do not remove; Need to reenable after postback, especially in the grid.
		}

		protected void cSysRowAuthorityId58_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			// *** On Change/ On Click Web Rule starts here *** //
			EnableValidators(true);  // Do not remove; Need to reenable after postback, especially in the grid.
			TextBox pwd = null;
			if (cAdmUsrGroupGrid.EditIndex > -1 && cAdmUsrGroupGrid.Items.Count > cAdmUsrGroupGrid.EditIndex)
			{
				if (Session[KEY_dtSysRowAuthorityId58] != null)
				{
					DataView dv1 = ((DataTable)Session[KEY_dtSysRowAuthorityId58]).DefaultView; dv1.RowFilter = string.Empty;
				}
				if (Session[KEY_dtSysRowAuthorityId58] != null)
				{
					DataView dv2 = ((DataTable)Session[KEY_dtSysRowAuthorityId58]).DefaultView;
					if (dv2.Count > 0)	// In case of Invalid.
					{
						DataRowView dr = dv2[((DropDownList)sender).SelectedIndex];
						bool bValid3 = false;
						try
						{
							if ((dv2.Table.Columns.Contains("OvrideId") && dr["OvrideId"].ToString() != string.Empty) && (dr["PromptAlways"].ToString() == "Y" || !(new AdminSystem()).IsValidOvride(new Credential(dr["OvrideId"].ToString(),string.Empty),base.LUser.UsrId,LcSysConnString,LcAppPw))) {bValid3 = true;}
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						if (bValid3)
						{
						DropDownList ddl = (DropDownList)cAdmUsrGroupGrid.EditItem.FindControl("cSysRowAuthorityId58");
							pwd = (TextBox)cAdmUsrGroupGrid.EditItem.FindControl("cSysRowAuthorityId58Pwd");
							ddl.Visible = false; pwd.Visible = true;
							ScriptManager.GetCurrent(Parent.Page).SetFocus(pwd.ClientID);
						}
					}
				}
				DataView dvAdmUsrGroupGrid = ((DataTable)Session[KEY_dtAdmUsrGroupGrid]).DefaultView;
				if (Session[KEY_dtSysRowAuthorityId58] != null)
				{
					DataView dv3 = ((DataTable)Session[KEY_dtSysRowAuthorityId58]).DefaultView;
				}
				Session[KEY_dtAdmUsrGroupGrid] = dvAdmUsrGroupGrid.Table;
			}
			if (pwd != null) {ScriptManager.GetCurrent(Parent.Page).SetFocus(pwd.ClientID);} else {ScriptManager.GetCurrent(Parent.Page).SetFocus(SenderFocusId(sender));}
		}

		protected void cSysRowAuthorityId58Pwd_TextChanged(object sender, System.EventArgs e)
		{
			if (cAdmUsrGroupGrid.EditIndex > -1 && cAdmUsrGroupGrid.Items.Count > cAdmUsrGroupGrid.EditIndex && Session[KEY_dtSysRowAuthorityId58] != null)
			{
				bool bOk = true;
				DataTable dt = (DataTable)Session[KEY_dtSysRowAuthorityId58];
				DataView dv = dt != null ? dt.DefaultView : null;
				DropDownList ddl = (DropDownList)cAdmUsrGroupGrid.EditItem.FindControl("cSysRowAuthorityId58");
				DataRowView dr = dv[ddl.SelectedIndex];
				bool bValid4 = false;
				try
				{
					if (!(new AdminSystem()).IsValidOvride(new Credential(dr["OvrideId"].ToString(),((TextBox)sender).Text),0,LcSysConnString,LcAppPw)) {bValid4 = true;}
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (bValid4)
				{
					if (dr["PromptModal"].ToString() == "N") {SetSysRowAuthorityId58(ddl,string.Empty);} else {bOk = false;}
				}
				Label pwl = (Label)cAdmUsrGroupGrid.EditItem.FindControl("cSysRowAuthorityId58Pwl");
				if (bOk)
				{
					ddl.Visible = true; ((TextBox)sender).Visible = false; pwl.Visible = false;
					ScriptManager.GetCurrent(Parent.Page).SetFocus(ddl.ClientID);
				}
				else {ScriptManager.GetCurrent(Parent.Page).SetFocus(((TextBox)sender).ClientID); pwl.Visible = true;}
			}
		}
		...
		public void cFindButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dv != null)
			{
				...
				if ((cc != null && cc.ID.Equals("cAdmUsrGroup5List")) || cAdmUsrGroupGrid.EditIndex < 0 || UpdateGridRow(cAdmUsrGroupGrid, new CommandEventArgs("Save", "")))
				{
					string rf = string.Empty;
					if (cFind.Text != string.Empty) { rf = "(" + base.GetExpression(cFind.Text.Trim(), GetAuthCol(), 4, cFindFilter.SelectedValue) + ")"; }
					if (rf != string.Empty) { rf = "((" + rf + " or _NewRow = 'Y' ))"; }
					dv.RowFilter = rf;
					ViewState["_RowFilter"] = rf;
					GotoPage(0); cAdmUsrGroupGrid_DataBind(dv);
					if (GetCurrPageIndex() != (int)Session[KEY_currPageIndex] && (int)Session[KEY_currPageIndex] < GetTotalPages()) { GotoPage((int)Session[KEY_currPageIndex]); cAdmUsrGroupGrid_DataBind(dv);}
		...
		public void cInsRowButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dt != null && (ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")))
			{
				...
				Session[KEY_lastAddedRow] = 0; Session[KEY_dtAdmUsrGroupGrid] = dt; Session[KEY_currPageIndex] = 0; GotoPage(0);
				cAdmUsrGroupGrid_DataBind(dt.DefaultView);
				cAdmUsrGroupGrid_OnItemEditing(cAdmUsrGroupGrid, new ListViewEditEventArgs(0));
		...
		public void cPgSizeButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cAdmUsrGroupGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				...
				cAdmUsrGroupGridDataPager.PageSize = int.Parse(cPgSize.Text); GotoPage(0);
				if (dt.Rows.Count <= 0 || (!((GetAuthRow().Rows[0]["AllowUpd"].ToString() == "N" || GetAuthRow().Rows[0]["ViewOnly"].ToString() == "G") && GetAuthRow().Rows[0]["AllowAdd"].ToString() == "N") && !(Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N") && dt.Rows.Count == 1)) {cAdmUsrGroupGrid_DataBind(dt.DefaultView);} else {cFindButton_Click(sender, e);}
				try
				{
					(new AdminSystem()).UpdLastPageInfo(5, base.LUser.UsrId, cPgSize.Text, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		}
		...
		public void cShowImpButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cSchemaImage.Visible)
			{
				try
				{
					Session["ImportSchema"] = (new AdminSystem()).GetSchemaScrImp(5,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (cSchemaImage.Attributes["OnClick"] == null || cSchemaImage.Attributes["OnClick"].IndexOf("ImportSchema.aspx") < 0) {cSchemaImage.Attributes["OnClick"] += "SearchLink('ImportSchema.aspx?scm=S&key=5&csy=3','','',''); return false;";}
			}
			// *** System Button Click (after) Web Rule starts here *** //
		}
		...
		private void ScrImportDdl(DataView dvd, DataRowView drv, bool bComboBox, string PKey, string CKey, string CNam, int MaxLen, string MatchCd, bool bAllowNulls)
		{
			if (dvd != null)
			{
				...
				{
					if (string.IsNullOrEmpty(cAdmUsrGroup5List.SelectedValue))
					{
						dvd.RowFilter = "(" + PKey + " is null)";
					}
					else
					{
						dvd.RowFilter = "(" + PKey + " is null OR " + PKey + " = " + cAdmUsrGroup5List.SelectedValue + ")";
					}
				}
		...
		protected void ScrImport(bool bClear)
		{
			...
			DataTable dtAdmUsrGroupGrid = (DataTable)Session[KEY_dtAdmUsrGroupGrid];
			if (dti != null && dtAdmUsrGroupGrid != null)
			{
				if (bClear)
				{
					dtAdmUsrGroupGrid.Rows.Clear();
				}
				cFind.Text = string.Empty;
				int iExist = dtAdmUsrGroupGrid.Rows.Count;
				DataTable dt = null;
				int iLast = (int)Session[KEY_lastImpPwdOvride];
				int ii = 0;
				try
				{
					foreach (DataRowView drv in dti.DefaultView)
					{
						if ((drv.Row.RowState == System.Data.DataRowState.Added || drv.Row.RowState == System.Data.DataRowState.Detached) && ii >= iLast && ii >= iExist)
						{
							dt = (new AdminSystem()).GetDdl(5,"GetDdlSysRowAuthorityId3S553",true,false,0,drv["SysRowAuthorityId58"].ToString(),LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr);
							if (dt != null && dt.Rows.Count == 1)
							{
								if (dt.Columns.Contains("OvrideId") && dt.Rows[0]["OvrideId"].ToString() != string.Empty)
								{
									bool bValid5 = false;
									if ((dt.Rows[0]["PromptAlways"].ToString() == "Y" || !(new AdminSystem()).IsValidOvride(new Credential(dt.Rows[0]["OvrideId"].ToString(),string.Empty),base.LUser.UsrId,LcSysConnString,LcAppPw)) && !(new AdminSystem()).IsValidOvride(new Credential(dt.Rows[0]["OvrideId"].ToString(),cImportPwd.Text),0,LcSysConnString,LcAppPw)) {bValid5 = true;}
									if (bValid5)
									{
										cNaviPanel.Visible = false; cImportPwdPanel.Visible = true; Session[KEY_lastImpPwdOvride] = ii; ScriptManager.GetCurrent(Parent.Page).SetFocus(cImportPwd.ClientID);
										if ((cContinueButton.Attributes["OnClick"] == null || cContinueButton.Attributes["OnClick"].IndexOf("_bConfirm") < 0) && cContinueButton.Visible) {cContinueButton.Attributes["OnClick"] += "document.getElementById('" + bConfirm.ClientID + "').value='N';";}
										ii = ii - iExist + 1;
										bInfoNow.Value = "Y"; PreMsgPopup("Please enter a password override for '" + drv["SysRowAuthorityId58Text"].ToString() + "' on import row " + ii.ToString() + " and select Continue.");
										return;
									}
									Session[KEY_cntImpPwdOvride] = (int)Session[KEY_cntImpPwdOvride] + 1;
									if (Session["CtrlAcctList"] == null)
									{
										System.Collections.Hashtable ht = new System.Collections.Hashtable();
										ht.Add(drv["SysRowAuthorityId58Text"].ToString(), 1);
										Session["CtrlAcctList"] = ht;
									}
									else
									{
										System.Collections.Hashtable ht = (System.Collections.Hashtable) Session["CtrlAcctList"];
										if (ht.Contains(drv["SysRowAuthorityId58Text"].ToString())) {ht[drv["SysRowAuthorityId58Text"].ToString()] = Int32.Parse(ht[drv["SysRowAuthorityId58Text"].ToString()].ToString()) + 1;} else {ht.Add(drv["SysRowAuthorityId58Text"].ToString(), 1);}
										Session["CtrlAcctList"] = ht;
									}
								}
							}
				...
				foreach (DataRowView drv in dti.DefaultView)
				{
					...
					{
						DataTable dtd;
						dtd = (DataTable)Session[KEY_dtCompanyId58];
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(5,"GetDdlCompanyId3S1744",true,true,0,string.Empty,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
							Session[KEY_dtCompanyId58] = dtd;
						}
						ScrImportDdl(dtd.DefaultView, drv, false, "UsrGroupId", "CompanyId58", "CompanyId58Text", 100, "2", true);
						dtd = (DataTable)Session[KEY_dtProjectId58];
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(5,"GetDdlProjectId3S1745",true,true,0,string.Empty,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
							Session[KEY_dtProjectId58] = dtd;
						}
						ScrImportDdl(dtd.DefaultView, drv, false, "UsrGroupId", "ProjectId58", "ProjectId58Text", 100, "2", true);
						dtd = (DataTable)Session[KEY_dtSystemId58];
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(5,"GetDdlSystemId3S552",true,true,0,string.Empty,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
							Session[KEY_dtSystemId58] = dtd;
						}
						ScrImportDdl(dtd.DefaultView, drv, false, "UsrGroupId", "SystemId58", "SystemId58Text", 50, "2", true);
						dtd = (DataTable)Session[KEY_dtSysRowAuthorityId58];
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(5,"GetDdlSysRowAuthorityId3S553",true,true,0,string.Empty,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
							Session[KEY_dtSysRowAuthorityId58] = dtd;
						}
						ScrImportDdl(dtd.DefaultView, drv, false, "UsrGroupId", "SysRowAuthorityId58", "SysRowAuthorityId58Text", 50, "2", false);
					}
					jj = jj + 1;
				}
				iRow = dti.DefaultView.Count - iExist;
				if (!dti.Columns.Contains("_NewRow")) { dti.Columns.Add("_NewRow"); }
				Session[KEY_dtAdmUsrGroupGrid] = dti;
				cAdmUsrGroupGrid_DataBind(dti.DefaultView);
		...
		public void cImportButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cFNameO.Text != string.Empty && cWorkSheet.Items.Count > 0 && cWorkSheet.SelectedItem.Text != string.Empty && cStartRow.Text != string.Empty)
			{
				...
				try
				{
					if (cAdmUsrGroupGrid.EditIndex < 0 || UpdateGridRow(cAdmUsrGroupGrid, new CommandEventArgs("Save", "")))
					{
						Session[KEY_scrImport] = ImportFile(dtAdmUsrGroupGrid.Copy(),cFNameO.Text,cWorkSheet.SelectedItem.Text,cStartRow.Text,Config.PathTmpImport + cFName.Text);
					}
		...
		private DataTable ImportFile(DataTable dt, string fileName, string workSheet, string startRow, string fileFullName)
		{
			try
			{
				...
				{
					if (rows[iRow][10].ToString() == string.Empty && rows[iRow][11].ToString() == string.Empty) {idel = idel + 1;}
					else
					{
		...
		private void GotoPage(int pageNo)
		{
			if (pageNo >= 0 && pageNo < GetTotalPages())
			{
				if (cAdmUsrGroupGrid.EditIndex < 0 || UpdateGridRow(cAdmUsrGroupGrid, new CommandEventArgs("Save", "")))
				{
					try { cAdmUsrGroupGridDataPager.SetPageProperties(cAdmUsrGroupGridDataPager.PageSize * pageNo, cAdmUsrGroupGridDataPager.MaximumRows, false); cAdmUsrGroupGrid_DataBind(null); }
					catch
					{
						try { cAdmUsrGroupGridDataPager.SetPageProperties(0, cAdmUsrGroupGridDataPager.MaximumRows, false); cAdmUsrGroupGrid_DataBind(null); } catch {}
					}
				}
			}
		}

		protected void cAdmUsrGroupGrid_OnItemCommand(object sender, ListViewCommandEventArgs e)
		{
		}

		private void cAdmUsrGroupGrid_DataBind(DataView dvAdmUsrGroupGrid)
		{
			if (dvAdmUsrGroupGrid == null)
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmUsrGroupGrid];
				dvAdmUsrGroupGrid = dt != null ? dt.DefaultView : null;
			}
			if (dvAdmUsrGroupGrid != null)
			{
				LcAuth = GetAuthCol().AsEnumerable().ToDictionary<DataRow,string>(dr=>dr["ColName"].ToString());
				cAdmUsrGroupGrid.DataSource = dvAdmUsrGroupGrid;
				cAdmUsrGroupGrid.DataBind();
			...
			if (cFindFilter.Items.Count <= 0)
			{
				...
				{
					if (ii >= 4 && !string.IsNullOrEmpty(dr["ColumnHeader"].ToString()) && !string.IsNullOrEmpty(dr["TableId"].ToString()) && dtAuth.Rows[ii]["ColVisible"].ToString() == "Y")
					{
						li = new ListItem();
		...
		protected void cAdmUsrGroupGrid_OnSorting(object sender, ListViewSortEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmUsrGroupGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null)
			{
				if (cAdmUsrGroupGrid.EditIndex < 0 || UpdateGridRow(cAdmUsrGroupGrid, new CommandEventArgs("Save", "")))
				{
					...
					Session[KEY_dtAdmUsrGroupGrid] = dt;
					cAdmUsrGroupGrid_DataBind(dv);
				}
			}
		}

		protected void cAdmUsrGroupGrid_OnPagePropertiesChanging(object sender, PagePropertiesChangingEventArgs e)
		{
		    cAdmUsrGroupGridDataPager.SetPageProperties(e.StartRowIndex, e.MaximumRows, false);
		    DataTable dt = (DataTable)Session[KEY_dtAdmUsrGroupGrid];
		    DataView dv = dt != null ? dt.DefaultView : null;
			cAdmUsrGroupGrid_DataBind(dv); cAdmUsrGroupGrid.EditIndex = -1;
		}

		private void GridChkPgDirty(ListViewItem lvi)
		{
				WebControl cc = null;
				cc = ((WebControl)lvi.FindControl("cCompanyId58"));
				...
				cc = ((WebControl)lvi.FindControl("cProjectId58"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cFiller"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cMoreInfo"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cSystemId58"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cSysRowAuthorityId58"));
				...
				cc = ((WebControl)lvi.FindControl("cSysRowAuthorityId58Pwd"));
				if (cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("_bConfirm") < 0) {cc.Attributes["OnChange"] += "document.getElementById('" + bConfirm.ClientID + "').value='N';";}
		}

		protected void cAdmUsrGroupGrid_OnItemDataBound(object sender, ListViewItemEventArgs e)
		{
			// *** GridItemDataBound (before) Web Rule End *** //
			DataTable dt = (DataTable)Session[KEY_dtAdmUsrGroupGrid];
			bool isEditItem = false;
			DataView dvAdmUsrGroupGrid = dt != null ? dt.DefaultView : null;
			if (cAdmUsrGroupGrid.EditIndex > -1 && GetDataItemIndex(cAdmUsrGroupGrid.EditIndex) == e.Item.DataItemIndex)
			{
				isEditItem = true;
				base.SetGridEnabled(e.Item, GetAuthCol(), GetLabel(), 4);
				SetCompanyId58((DropDownList)e.Item.FindControl("cCompanyId58"),dvAdmUsrGroupGrid[e.Item.DataItemIndex]["CompanyId58"].ToString());
				SetProjectId58((DropDownList)e.Item.FindControl("cProjectId58"),dvAdmUsrGroupGrid[e.Item.DataItemIndex]["ProjectId58"].ToString());
				SetSystemId58((DropDownList)e.Item.FindControl("cSystemId58"),dvAdmUsrGroupGrid[e.Item.DataItemIndex]["SystemId58"].ToString());
				SetSysRowAuthorityId58((DropDownList)e.Item.FindControl("cSysRowAuthorityId58"),dvAdmUsrGroupGrid[e.Item.DataItemIndex]["SysRowAuthorityId58"].ToString());
			...
			if (dtAuthRow != null)
			{
				...
				{
					cc = e.Item.FindControl("cAdmUsrGroupGridDelete"); if (cc != null) { cc.Visible = false; }
					cc = e.Item.FindControl("cAdmUsrGroupGridEdit"); if (cc != null) { cc.Visible = false; }
				}
				else
				{
			        HtmlTableRow tr = e.Item.FindControl("cAdmUsrGroupGridRow") as HtmlTableRow;
			        LinkButton lb = e.Item.FindControl("cAdmUsrGroupGridEdit") as LinkButton;
			        if (tr != null && lb != null) { SetDefaultCtrl(tr, lb, string.Empty); }
				}
			}
			SetGridButtonToolTip(e, "FillerBtn",ColumnToolTip(8));
			if (cAdmUsrGroupGrid.EditIndex > -1 && GetDataItemIndex(cAdmUsrGroupGrid.EditIndex) == e.Item.DataItemIndex)
			{
			}
			else
		...
		protected void cUsrGroupAuthId58hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "0") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "0"; Session[KEY_lastSortExp] = "UsrGroupAuthId58";Session[KEY_lastSortImg] = "cUsrGroupAuthId58hi";
			cAdmUsrGroupGrid_OnSorting(sender, new ListViewSortEventArgs("UsrGroupAuthId58", SortDirection.Ascending));
		}

		protected void cCompanyId58hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "1") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "1"; Session[KEY_lastSortExp] = "CompanyId58Text";Session[KEY_lastSortImg] = "cCompanyId58hi";
			cAdmUsrGroupGrid_OnSorting(sender, new ListViewSortEventArgs("CompanyId58Text", SortDirection.Ascending));
		}

		protected void cProjectId58hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "2") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "2"; Session[KEY_lastSortExp] = "ProjectId58Text";Session[KEY_lastSortImg] = "cProjectId58hi";
			cAdmUsrGroupGrid_OnSorting(sender, new ListViewSortEventArgs("ProjectId58Text", SortDirection.Ascending));
		}

		protected void cFillerhl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "3") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "3"; Session[KEY_lastSortExp] = "Filler";Session[KEY_lastSortImg] = "cFillerhi";
			cAdmUsrGroupGrid_OnSorting(sender, new ListViewSortEventArgs("Filler", SortDirection.Ascending));
		}

		protected void cMoreInfohl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "5") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "5"; Session[KEY_lastSortExp] = "MoreInfo";Session[KEY_lastSortImg] = "cMoreInfohi";
			cAdmUsrGroupGrid_OnSorting(sender, new ListViewSortEventArgs("MoreInfo", SortDirection.Ascending));
		}

		protected void cSystemId58hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "6") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "6"; Session[KEY_lastSortExp] = "SystemId58Text";Session[KEY_lastSortImg] = "cSystemId58hi";
			cAdmUsrGroupGrid_OnSorting(sender, new ListViewSortEventArgs("SystemId58Text", SortDirection.Ascending));
		}

		protected void cSysRowAuthorityId58hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "7") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "7"; Session[KEY_lastSortExp] = "SysRowAuthorityId58Text";Session[KEY_lastSortImg] = "cSysRowAuthorityId58hi";
			cAdmUsrGroupGrid_OnSorting(sender, new ListViewSortEventArgs("SysRowAuthorityId58Text", SortDirection.Ascending));
		}

		protected void cAdmUsrGroupGrid_OnItemEditing(object sender, ListViewEditEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmUsrGroupGrid];
			if ((ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")) && dt != null && dt.DefaultView.Count > GetDataItemIndex(e.NewEditIndex))
			{
				cAdmUsrGroupGrid.EditIndex = e.NewEditIndex;
				cAdmUsrGroupGrid_DataBind(null);
				Control wc = null;
				string idx = Page.Request["__EVENTARGUMENT"];
				if (!string.IsNullOrEmpty(idx))
				{
				    Control ctrlToFocus = cAdmUsrGroupGrid.EditItem.FindControl("c" + idx);
				    if (((WebControl)ctrlToFocus).Enabled) { wc = ctrlToFocus; }
				}
				if (wc == null) { wc = FindEditableControl(cAdmUsrGroupGrid.EditItem); }
				if (wc != null)
             {
             ...
             MkMobileLabel(cAdmUsrGroupGrid.EditItem);
		    }
		    else { e.Cancel = true; }
		}
		...
		{
		    Label ml = null;
		    ml = lvi.FindControl("cUsrGroupAuthId58ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(4); }
		    ml = lvi.FindControl("cCompanyId58ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(5); }
		    ml = lvi.FindControl("cProjectId58ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(6); }
		    ml = lvi.FindControl("cFillerml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(7); }
		    ml = lvi.FindControl("cFillerBtnml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(8); }
		    ml = lvi.FindControl("cMoreInfoml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(9); }
		    ml = lvi.FindControl("cSystemId58ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(10); }
		    ml = lvi.FindControl("cSysRowAuthorityId58ml") as Label;
		...
		protected void GridFill(ListViewItem lvi, DataTable dt, DataRow dr, bool bInsert)
		{
			...
			ddl = (DropDownList)lvi.FindControl("cCompanyId58");
			if (ddl != null)
			{
				if (ddl.SelectedIndex >= 0 && ddl.SelectedValue.ToString() != string.Empty)
				{
					dr["CompanyId58"] = ddl.SelectedValue;
					dr["CompanyId58Text"] = ddl.SelectedItem.Text;
				}
				else
				{
					dr["CompanyId58"] = System.DBNull.Value;
					dr["CompanyId58Text"] = System.DBNull.Value;
				}
			}
			ddl = (DropDownList)lvi.FindControl("cProjectId58");
			if (ddl != null)
			{
				if (ddl.SelectedIndex >= 0 && ddl.SelectedValue.ToString() != string.Empty)
				{
					dr["ProjectId58"] = ddl.SelectedValue;
					dr["ProjectId58Text"] = ddl.SelectedItem.Text;
				}
				else
				{
					dr["ProjectId58"] = System.DBNull.Value;
					dr["ProjectId58Text"] = System.DBNull.Value;
				}
			}
			TextBox tb = null;
			tb = (TextBox)lvi.FindControl("cFiller");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["Filler"] = tb.Text;} else {dr["Filler"] = System.DBNull.Value;}
			}
			ddl = (DropDownList)lvi.FindControl("cSystemId58");
			if (ddl != null)
			{
				if (ddl.SelectedIndex >= 0 && ddl.SelectedValue.ToString() != string.Empty)
				{
					dr["SystemId58"] = ddl.SelectedValue;
					dr["SystemId58Text"] = ddl.SelectedItem.Text;
				}
				else
				{
					dr["SystemId58"] = System.DBNull.Value;
					dr["SystemId58Text"] = System.DBNull.Value;
				}
			}
			ddl = (DropDownList)lvi.FindControl("cSysRowAuthorityId58");
			if (ddl != null)
			{
				if (ddl.SelectedIndex >= 0 && ddl.SelectedValue.ToString() != string.Empty)
				{
					dr["SysRowAuthorityId58"] = ddl.SelectedValue;
					dr["SysRowAuthorityId58Text"] = ddl.SelectedItem.Text;
				}
				else
				{
					dr["SysRowAuthorityId58"] = System.DBNull.Value;
					dr["SysRowAuthorityId58Text"] = System.DBNull.Value;
				}
			}
		...
		protected void cAdmUsrGroupGrid_OnItemUpdating(object sender, ListViewUpdateEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmUsrGroupGrid];
			if (dt != null && dt.Rows.Count > 0)
			{
				DataRow dr = dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row;
			    GridFill(cAdmUsrGroupGrid.EditItem, dt, dr, false);
			    dr["_NewRow"] = "N";
			    if (dt.Columns.Contains("_SortOrder")) dr["_SortOrder"] = null;
			}
		}

		protected void cAdmUsrGroupGrid_OnItemCanceling(object sender, ListViewCancelEventArgs e)
		{
			PanelTop.Update();
			DataTable dt = (DataTable)Session[KEY_dtAdmUsrGroupGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			...
			{
				cAdmUsrGroupGrid_OnItemDeleting(sender, new ListViewDeleteEventArgs(e.ItemIndex));
			}
			if (dt.Columns.Contains("_SortOrder")) dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row["_SortOrder"] = null;
			cAdmUsrGroupGrid.EditIndex = -1; cAdmUsrGroupGrid_DataBind(null);
		}
		...
		protected void cDeleteAllButton_Click(object sender, System.EventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmUsrGroupGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null && (cAdmUsrGroupGrid.EditIndex < 0 || UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				GotoPage(0);
				while (dv.Count > 0) {dv.Delete(0);}
				Session[KEY_dtAdmUsrGroupGrid] = dt;
				cAdmUsrGroupGrid.EditIndex = -1; cAdmUsrGroupGrid_DataBind(dv);
			}
		}

		protected void cAdmUsrGroupGrid_OnItemDeleting(object sender, ListViewDeleteEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmUsrGroupGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			DataRow dr = dv != null ? dv[GetDataItemIndex(e.ItemIndex)].Row : null;
			if (dv != null && (cAdmUsrGroupGrid.EditIndex == e.ItemIndex || UpdateGridRow(sender, new CommandEventArgs("Save",""))))
			{
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cUsrGroupId7.Text == string.Empty) { cClearButton_Click(sender, new EventArgs()); ShowDirty(false); PanelTop.Update();}
			else { PopAdmUsrGroup5List(sender, e, false, cUsrGroupId7.Text); }
			// *** System Button Click (After) Web Rule starts here *** //
		}
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmUsrGroup5List); PopAdmUsrGroup5List(sender, e, false, null);
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			if (cRowAuthorityId7Pwd.Visible) {cRowAuthorityId7Pwd_TextChanged(cRowAuthorityId7Pwd,new EventArgs());}
			if (ValidPage() && !cRowAuthorityId7Pwd.Visible && (cAdmUsrGroupGrid.EditIndex < 0 || UpdateGridRow(cAdmUsrGroupGrid, new CommandEventArgs("Save", ""))))
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmUsrGroupGrid];
				DataView dv = dt != null ? dt.DefaultView : null;
				string ftr = string.Empty;
				if (dv.RowFilter != string.Empty) { ftr = dv.RowFilter; dv.RowFilter = string.Empty; }
				AdmUsrGroup5 ds = PrepAdmUsrGroupData(dv,cUsrGroupId7.Text == string.Empty);
				if (ftr != string.Empty) {dv.RowFilter = ftr;}
				if (!string.IsNullOrEmpty(cAdmUsrGroup5List.SelectedValue) && cUsrGroupId7.Text != cAdmUsrGroup5List.SelectedValue)
				{
					PreMsgPopup("Primary key cannot be changed");
					return rtn;
				}
				if (string.IsNullOrEmpty(cAdmUsrGroup5List.SelectedValue))	// Add
				{
					if (ds != null)
					{
						pid = (new AdminSystem()).AddData(5,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc,noTrans,commandTimeOut);
					}
					if (!string.IsNullOrEmpty(pid))
					{
						cAdmUsrGroup5List.ClearSearch(); Session.Remove(KEY_dtAdmUsrGroup5List);
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmUsrGroup5List(sender, e, false, pid);
						rtn = GetScreenHlp().Rows[0]["AddMsg"].ToString();
					}
				}
				else {
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(5,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc,noTrans,commandTimeOut)) {bValid7 = true;}
					if (bValid7)
					{
						cAdmUsrGroup5List.ClearSearch(); Session.Remove(KEY_dtAdmUsrGroup5List);
						Session[KEY_currPageIndex] = GetCurrPageIndex();
						ShowDirty(false); PopAdmUsrGroup5List(sender, e, false, ds.Tables["AdmUsrGroup"].Rows[0]["UsrGroupId7"]);
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cUsrGroupId7.Text != string.Empty)
			{
				AdmUsrGroup5 ds = PrepAdmUsrGroupData(null,false);
				try
				{
					if (ds != null && (new AdminSystem()).DelData(5,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc))
					{
						cAdmUsrGroup5List.ClearSearch(); Session.Remove(KEY_dtAdmUsrGroup5List);
						ShowDirty(false); PopAdmUsrGroup5List(sender, e, false, null);
		...
		private AdmUsrGroup5 PrepAdmUsrGroupData(DataView dv, bool bAdd)
		{
			...
			if (bAdd) { dr["UsrGroupId7"] = string.Empty; } else { dr["UsrGroupId7"] = cUsrGroupId7.Text; }
			drType["UsrGroupId7"] = "Numeric"; drDisp["UsrGroupId7"] = "TextBox";
			try {dr["UsrGroupName7"] = cUsrGroupName7.Text.Trim();} catch {}
			drType["UsrGroupName7"] = "VarWChar"; drDisp["UsrGroupName7"] = "TextBox";
			try {dr["RowAuthorityId7"] = cRowAuthorityId7.SelectedValue;} catch {}
			drType["RowAuthorityId7"] = "Numeric"; drDisp["RowAuthorityId7"] = "DropDownList";
			try {dr["CompanyId7"] = cCompanyId7.SelectedValue;} catch {}
			...
			if (dv != null)
			{
				...
				if (bAdd)
				{
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmUsrGroupAdd"].Rows.Add(MakeColRow(ds.Tables["AdmUsrGroupAdd"].NewRow(), drv, true));
					}
				}
				else
				{
					...
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmUsrGroupUpd"].Rows.Add(MakeColRow(ds.Tables["AdmUsrGroupUpd"].NewRow(), drv, false));
					}
					...
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmUsrGroupDel"].Rows.Add(MakeColRow(ds.Tables["AdmUsrGroupDel"].NewRow(), drv, false));
					}
		...
		public bool CanAct(char typ) { return CanAct(typ, GetAuthRow(), cAdmUsrGroup5List.SelectedValue.ToString()); }
		...
		private bool ValidPage()
		{
			...
			DataTable dt = null;
			dt = (DataTable)Session[KEY_dtRowAuthorityId7];
			if (dt != null && dt.Rows.Count <= 0)
			{
				bErrNow.Value = "Y"; PreMsgPopup("Value is expected for 'RowAuthorityId', please investigate."); return false;
			}
			dt = (DataTable)Session[KEY_dtSysRowAuthorityId58];
			if (dt != null && dt.Rows.Count <= 0)
			{
				bErrNow.Value = "Y"; PreMsgPopup("Value is expected for 'SysRowAuthorityId', please investigate."); return false;
			}
			return true;
		}
		...
		{
			dr["UsrGroupId7"] = System.Data.OleDb.OleDbType.Numeric.ToString();
			dr["UsrGroupAuthId58"] = System.Data.OleDb.OleDbType.Numeric.ToString();
		...
		{
			dr["UsrGroupId7"] = cUsrGroupId7.Text;
			DataTable dtAuth = GetAuthCol();
			if (dtAuth != null)
			{
				dr["UsrGroupAuthId58"] = drv["UsrGroupAuthId58"].ToString().Trim();
		...
		{
			if (cAdmUsrGroupGrid.EditIndex > -1)
			{
				TextBox pwd = null;
				pwd = (TextBox)cAdmUsrGroupGrid.EditItem.FindControl("cSysRowAuthorityId58Pwd");
				if (pwd.Visible) {cSysRowAuthorityId58Pwd_TextChanged(cAdmUsrGroupGrid.EditItem.FindControl("cSysRowAuthorityId58Pwd"),new EventArgs());}
				if (pwd.Visible) {return false;}
				cAdmUsrGroupGrid_OnItemUpdating(sender, new ListViewUpdateEventArgs(cAdmUsrGroupGrid.EditIndex));
			}
			return true;
		}

		protected void cAdmUsrGroupGrid_OnPreRender(object sender, System.EventArgs e)
		{
			System.Web.UI.WebControls.Image hi = null;
			hi = (System.Web.UI.WebControls.Image)cAdmUsrGroupGrid.FindControl("cUsrGroupAuthId58hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmUsrGroupGrid.FindControl("cCompanyId58hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmUsrGroupGrid.FindControl("cProjectId58hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmUsrGroupGrid.FindControl("cFillerhi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmUsrGroupGrid.FindControl("cMoreInfohi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmUsrGroupGrid.FindControl("cSystemId58hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmUsrGroupGrid.FindControl("cSysRowAuthorityId58hi"); if (hi != null) { hi.Visible = false; }
			if (Session[KEY_lastSortImg] != null)
			{
				hi = (System.Web.UI.WebControls.Image)cAdmUsrGroupGrid.FindControl((string)Session[KEY_lastSortImg]);
				if (hi != null) { hi.ImageUrl = Utils.AddTilde((string)Session[KEY_lastSortUrl]); hi.Visible = true; }
			}
			IgnoreHeaderConfirm((LinkButton)cAdmUsrGroupGrid.FindControl("cUsrGroupAuthId58hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmUsrGroupGrid.FindControl("cCompanyId58hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmUsrGroupGrid.FindControl("cProjectId58hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmUsrGroupGrid.FindControl("cFillerhl"));
			IgnoreHeaderConfirm((LinkButton)cAdmUsrGroupGrid.FindControl("cMoreInfohl"));
			IgnoreHeaderConfirm((LinkButton)cAdmUsrGroupGrid.FindControl("cSystemId58hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmUsrGroupGrid.FindControl("cSysRowAuthorityId58hl"));
		}

		private string GetButtonId(ListViewItem lvi)
		{
			string ButtonID = String.Empty;
			Control c = lvi.FindControl("cAdmUsrGroupGridEdit");
		...
		{
		    return cAdmUsrGroupGridDataPager.StartRowIndex / cAdmUsrGroupGridDataPager.PageSize;
		}

		private int GetTotalPages()
		{
		    return (int)Math.Ceiling((double)cAdmUsrGroupGridDataPager.TotalRowCount / cAdmUsrGroupGridDataPager.PageSize);
		}

		private int GetDataItemIndex(int editIndex)
		{
		    return cAdmUsrGroupGridDataPager.StartRowIndex + editIndex;
		}

		protected void cAdmUsrGroupGrid_OnLayoutCreated(object sender, EventArgs e)
		{
		    // Header:
		    LinkButton lb = null;
		    lb = cAdmUsrGroupGrid.FindControl("cUsrGroupAuthId58hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(4); lb.ToolTip = ColumnToolTip(4); lb.Parent.Visible = GridColumnVisible(4); }
		    lb = cAdmUsrGroupGrid.FindControl("cCompanyId58hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(5); lb.ToolTip = ColumnToolTip(5); lb.Parent.Visible = GridColumnVisible(5); }
		    lb = cAdmUsrGroupGrid.FindControl("cProjectId58hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(6); lb.ToolTip = ColumnToolTip(6); lb.Parent.Visible = GridColumnVisible(6); }
		    lb = cAdmUsrGroupGrid.FindControl("cFillerhl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(7); lb.ToolTip = ColumnToolTip(7); lb.Parent.Visible = GridColumnVisible(7); }
		    lb = cAdmUsrGroupGrid.FindControl("cFillerBtnhl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(8); lb.ToolTip = ColumnToolTip(8); lb.Parent.Visible = GridColumnVisible(8); }
		    lb = cAdmUsrGroupGrid.FindControl("cMoreInfohl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(9); lb.ToolTip = ColumnToolTip(9); lb.Parent.Visible = GridColumnVisible(9); }
		    lb = cAdmUsrGroupGrid.FindControl("cSystemId58hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(10); lb.ToolTip = ColumnToolTip(10); lb.Parent.Visible = GridColumnVisible(10); }
		    lb = cAdmUsrGroupGrid.FindControl("cSysRowAuthorityId58hl") as LinkButton;
			...
			if (dtAuthRow != null)
			{
				...
				{
		            lb = cAdmUsrGroupGrid.FindControl("cDeleteAllButton") as LinkButton; if (lb != null) { lb.Visible = false; }
				}
			}
		    // footer:
		    Label gc = null;
		    gc = cAdmUsrGroupGrid.FindControl("cUsrGroupAuthId58fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(4); }
		    gc = cAdmUsrGroupGrid.FindControl("cCompanyId58fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(5); }
		    gc = cAdmUsrGroupGrid.FindControl("cProjectId58fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(6); }
		    gc = cAdmUsrGroupGrid.FindControl("cFillerfl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(7); }
		    gc = cAdmUsrGroupGrid.FindControl("cFillerBtnfl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(8); }
		    gc = cAdmUsrGroupGrid.FindControl("cMoreInfofl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(9); }
		    gc = cAdmUsrGroupGrid.FindControl("cSystemId58fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(10); }
		    gc = cAdmUsrGroupGrid.FindControl("cSysRowAuthorityId58fl") as Label;
		...
		private void PreMsgPopup(string msg) { PreMsgPopup(msg,cAdmUsrGroup5List,null); }

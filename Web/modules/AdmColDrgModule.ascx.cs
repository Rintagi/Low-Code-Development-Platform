...
{
	public class AdmColDrg1007 : DataSet
	{
		public AdmColDrg1007()
		{
			this.Tables.Add(MakeDtlColumns(new DataTable("AdmColDrgDef")));
			this.Tables.Add(MakeDtlColumns(new DataTable("AdmColDrgAdd")));
			this.Tables.Add(MakeDtlColumns(new DataTable("AdmColDrgUpd")));
			this.Tables.Add(MakeDtlColumns(new DataTable("AdmColDrgDel")));
			this.DataSetName = "AdmColDrg1007";
			this.Namespace = "http://Rintagi.com/DataSet/AdmColDrg1007";
		}
		...
		private DataTable MakeDtlColumns(DataTable dt)
		{
			...
			columns.Add("TabFolderId14", typeof(string));
			columns.Add("TabIndex14", typeof(string));
...
namespace RO.Web
{
	public partial class AdmColDrgModule : RO.Web.ModuleBase
	{

		private const string KEY_lastAddedRow = "Cache:lastAddedRow3_1007";
		private const string KEY_lastSortOrd = "Cache:lastSortOrd3_1007";
		private const string KEY_lastSortImg = "Cache:lastSortImg3_1007";
		private const string KEY_lastSortCol = "Cache:lastSortCol3_1007";
		private const string KEY_lastSortExp = "Cache:lastSortExp3_1007";
		private const string KEY_lastSortUrl = "Cache:lastSortUrl3_1007";
		private const string KEY_lastSortTog = "Cache:lastSortTog3_1007";
		private const string KEY_lastImpPwdOvride = "Cache:lastImpPwdOvride3_1007";
		private const string KEY_cntImpPwdOvride = "Cache:cntImpPwdOvride3_1007";
		private const string KEY_currPageIndex = "Cache:currPageIndex3_1007";
		private const string KEY_bHiImpVisible = "Cache:bHiImpVisible3_1007";
		private const string KEY_bShImpVisible = "Cache:bShImpVisible3_1007";
		private const string KEY_dtAdmColDrgGrid = "Cache:dtAdmColDrgGrid";
		private const string KEY_scrImport = "Cache:scrImport3_1007";
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_1007";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_1007";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_1007";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_1007";
		private const string KEY_dtLabel = "Cache:dtLabel3_1007";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_1007";
		private const string KEY_dtScrCri = "Cache:dtScrCri3_1007";
		private const string KEY_dsScrCriVal = "Cache:dsScrCriVal3_1007";
		...
		private const string KEY_dtTabFolderId14 = "Cache:dtTabFolderId14";

		private const string KEY_dtSystems = "Cache:dtSystems3_1007";
		private const string KEY_sysConnectionString = "Cache:sysConnectionString3_1007";
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_1007";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_1007";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_1007";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				DataTable dtMenuAccess = (new MenuSystem()).GetMenu(base.LUser.CultureId, 3, base.LImpr, LcSysConnString, LcAppPw,1007, null, null);
				...
				Session.Remove(KEY_dtAdmColDrgGrid);
				...
				Session.Remove(KEY_dtTabFolderId14);
				SetButtonHlp();
				GetSystems();
				SetColumnAuthority();
				GetGlobalFilter();
				...
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 1007, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmColDrg1007List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=1007&typ=N&sys=3','','',''); return false;"; }
			...
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmColDrgGrid];
				if (dt != null)
				{
		...
		private void InitializeComponent()
		{
			...
			if (LcSysConnString == null)
			{
				...
				{
					DataTable dt;
					dt = (new AdminSystem()).GetAuthRow(1007,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
					...
					{
						cAdmColDrgGrid.PreRender += new EventHandler(cAdmColDrgGrid_OnPreRender);
					}
				}
		...
		private void SetSystem(byte SystemId)
		{
			...
			try
			{
				...
				if ((Config.DeployType == "DEV" || row["dbAppDatabase"].ToString() == base.CPrj.EntityCode + "View") && !(base.CPrj.EntityCode != "RO" && row["SysProgram"].ToString() == "Y") && (new AdminSystem()).IsRegenNeeded(string.Empty,1007,0,0,LcSysConnString,LcAppPw))
				{
					(new GenScreensSystem()).CreateProgram(1007, "Object IDE", row["dbAppDatabase"].ToString(), base.CPrj, base.CSrc, base.CTar, LcAppConnString, LcAppPw);
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(1007,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(1007,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void SetClientRule(ListViewDataItem ee, bool isEditItem)
		{
			...
			if (dvRul.Count > 0)
			{
				...
				{
					if (ee == null || drv["ScriptEvent"].ToString().Substring(0,2).ToLower() != "on" || (cAdmColDrgGrid.EditIndex > -1 && GetDataItemIndex(cAdmColDrgGrid.EditIndex) == ee.DataItemIndex))
					{
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(1007,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(1007,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,1007,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(1007,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(1007,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(1007,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				CheckAuthentication(false);
				DataView dv = null;
				try {dv = ((DataTable)Session[KEY_dtAdmColDrgGrid]).DefaultView;}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(1007,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					foreach (DataRowView drv in dv)
					{
						...
						if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["ScreenId14"].ToString() + (char)9 + drv["ScreenId14Text"].ToString() + (char)9);}
						if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(drv["GroupRowId14"].ToString() + (char)9 + drv["GroupRowId14Text"].ToString() + (char)9);}
						if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(drv["GroupColId14"].ToString() + (char)9 + drv["GroupColId14Text"].ToString() + (char)9);}
						if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["ColumnName14"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append(drv["TabFolderId14"].ToString() + (char)9 + drv["TabFolderId14Text"].ToString() + (char)9);}
						if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["TabIndex14"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[7]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["MasterTable14"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						sb.Append(Environment.NewLine);
					}
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmColDrg.csv"; Session["ExportStr"] = (Config.ExportExcelCSV ? "sep=\t\n": "") + sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmColDrg.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private string GetRtfGrid(DataView dv)
		{
			...
			try
			{
				...
				foreach (DataRowView drv in dv)
				{
					...
					if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["ScreenId14Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(drv["GroupRowId14Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(drv["GroupColId14Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append(drv["ColumnName14"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append(drv["TabFolderId14Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["TabIndex14"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[7]["ColExport"].ToString() == "Y") {sb.Append(drv["MasterTable14"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					sb.Append(@"}");
					sb.Append("\r\n");
					sb.Append(@"\pard \ql \li0\ri0\widctlpar\intbl\aspalpha\aspnum\adjustright\rin0\lin0 {");
					sb.Append(@"\insrsid2981395 \trowd \irow0\irowband0\lastrow \ts15\trgaph108\trleft-108\trbrdrt\brdrs\brdrw10 \trbrdrl\brdrs\brdrw10 \trbrdrb\brdrs\brdrw10 \trbrdrr\brdrs\brdrw10 \trbrdrh\brdrs\brdrw10 \trbrdrv\brdrs\brdrw10 ");
		...
		private void GetPageInfo()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastPageInfo(1007, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 1007, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			// *** Criteria Trigger (before) Web Rule starts here *** //
			DataTable dt = (DataTable)Session[KEY_dtAdmColDrgGrid];
			if (cAdmColDrgGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
			if (IsPostBack && cCriteria.Visible) ReBindCriteria(GetScrCriteria());
			UpdCriteria(true);
			PopAdmColDrg1007List(sender, e, false, null);
			}
		}
			...
			try
			{
				foreach (DataRowView drv in dvCri)
				{
					...
					if (drv["DisplayName"].ToString() == "ComboBox")
					{
						...
						(new AdminSystem()).MkGetScreenIn("1007", drv["ScreenCriId"].ToString(), "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), LcAppDb, LcDesDb, drv["MultiDesignDb"].ToString(), LcSysConnString, LcAppPw);
						DataView dv = new DataView((new AdminSystem()).GetScreenIn("1007", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), (new AdminSystem()).CountScrCri(drv["ScreenCriId"].ToString(), drv["MultiDesignDb"].ToString(), drv["MultiDesignDb"].ToString() == "N" ? LcSysConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)), drv["RequiredValid"].ToString(), 0, string.Empty, drv["DisplayMode"].ToString() != "AutoListBox", val, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)));
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("1007", LcSysConnString, LcAppPw);
				}
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmColDrg1007" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmColDrg1007" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			foreach (DataRowView drv in dvCri)
			{
			    if (drv["DisplayName"].ToString() == "ListBox")
			    {
					...
					if (cListBox != null)
					{
						...
						int TotalChoiceCnt = new DataView((new AdminSystem()).GetScreenIn("1007", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), CriCnt, drv["RequiredValid"].ToString(), 0, string.Empty, true, string.Empty, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId))).Count;
						string selectedValues = string.Join(",", cListBox.Items.Cast<ListItem>().Where(x => x.Selected).Select(x => "'" + x.Value + "'").ToArray());
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("1007", "AdmColDrg1007", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		{
			SetScreenId14((RoboCoder.WebControls.ComboBox)sender,string.Empty,cAdmColDrgGrid.EditItem);
		}

		private void SetScreenId14(RoboCoder.WebControls.ComboBox ddl, string keyId, ListViewItem li)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			context["method"] = "GetDdlScreenId3S3261";
			context["addnew"] = "Y";
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(1007,"GetDdlScreenId3S3261",true,false,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
		...
		private void SetGroupRowId14(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtGroupRowId14];
			...
			if (ddl != null)
			{
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(1007,"GetDdlGroupRowId3S3263",true,bAll,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "GroupRowId14 is not NULL"; }
					dv.RowFilter = ss;
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(1007,"GetDdlGroupRowId3S3263",true,true,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtGroupRowId14] = dv.Table;
				}
			}
		}

		private void SetGroupColId14(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtGroupColId14];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (ddl != null)
			{
				string ss = string.Empty;
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(1007,"GetDdlGroupColId3S3264",true,bAll,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "GroupColId14 is not NULL"; }
					dv.RowFilter = ss;
					ddl.DataSource = dv; ddl.DataBind();
					li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					else if (!bFirst)
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(1007,"GetDdlGroupColId3S3264",true,true,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtGroupColId14] = dv.Table;
				}
			}
		}

		private void SetTabFolderId14(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtTabFolderId14];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (ddl != null)
			{
				string ss = string.Empty;
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(1007,"GetDdlTabFolderId3S3291",true,bAll,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "TabFolderId14 is not NULL"; }
					dv.RowFilter = ss;
					ddl.DataSource = dv; ddl.DataBind();
					li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					else if (!bFirst)
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(1007,"GetDdlTabFolderId3S3291",true,true,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtTabFolderId14] = dv.Table;
				}
			}
		}

		private void PopAdmColDrg1007List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			DataTable dt = null;
			CheckAuthentication(false);
			...
			try
			{
				try {dt = (new AdminSystem()).GetLis(1007,"GetLisAdmColDrg1007",false,"Y",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				catch {dt = (new AdminSystem()).GetLis(1007,"GetLisAdmColDrg1007",false,"N",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				Session[KEY_dtAdmColDrgGrid] = dt;
				cAdmColDrgGridDataPager.PageSize = Int16.Parse(cPgSize.Text); GotoPage(0);
			    if (Session[KEY_lastSortCol] != null)
			    {
					cAdmColDrgGrid_OnSorting(sender, new ListViewSortEventArgs((string)Session[KEY_lastSortExp], SortDirection.Ascending));
			    }
				else
			    {
					cAdmColDrgGrid_DataBind(dt.DefaultView);
			    }
			...
			//WebRule: Replace the screen with the entire object tree.
            /* This code was after Add New Tab Button
		...
		public void cFindButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dv != null)
			{
				if (cAdmColDrgGrid.EditIndex < 0 || UpdateGridRow(cAdmColDrgGrid, new CommandEventArgs("Save", "")))
				{
					...
					GotoPage(0); cAdmColDrgGrid_DataBind(dv);
					if (GetCurrPageIndex() != (int)Session[KEY_currPageIndex] && (int)Session[KEY_currPageIndex] < GetTotalPages()) { GotoPage((int)Session[KEY_currPageIndex]); cAdmColDrgGrid_DataBind(dv);}
		...
		public void cInsRowButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dt != null && (ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")))
			{
				...
				Session[KEY_lastAddedRow] = 0; Session[KEY_dtAdmColDrgGrid] = dt; Session[KEY_currPageIndex] = 0; GotoPage(0);
				cAdmColDrgGrid_DataBind(dt.DefaultView);
				cAdmColDrgGrid_OnItemEditing(cAdmColDrgGrid, new ListViewEditEventArgs(0));
		...
		public void cPgSizeButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cAdmColDrgGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				...
				cAdmColDrgGridDataPager.PageSize = int.Parse(cPgSize.Text); GotoPage(0);
				if (dt.Rows.Count <= 0 || (GetAuthRow().Rows[0]["AllowAdd"].ToString() == "Y" && dt.Rows.Count == 1)) {cAdmColDrgGrid_DataBind(dt.DefaultView);} else {cFindButton_Click(sender, e);}
				try
				{
					(new AdminSystem()).UpdLastPageInfo(1007, base.LUser.UsrId, cPgSize.Text, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		}
		...
		public void cShowImpButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cSchemaImage.Visible)
			{
				try
				{
					Session["ImportSchema"] = (new AdminSystem()).GetSchemaScrImp(1007,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (cSchemaImage.Attributes["OnClick"] == null || cSchemaImage.Attributes["OnClick"].IndexOf("ImportSchema.aspx") < 0) {cSchemaImage.Attributes["OnClick"] += "SearchLink('ImportSchema.aspx?scm=S&key=1007&csy=3','','',''); return false;";}
			}
			// *** System Button Click (after) Web Rule starts here *** //
		}
		...
		protected void ScrImport(bool bClear)
		{
			...
			DataTable dtAdmColDrgGrid = (DataTable)Session[KEY_dtAdmColDrgGrid];
			if (dti != null && dtAdmColDrgGrid != null)
			{
				if (bClear)
				{
					dtAdmColDrgGrid.Rows.Clear();
				}
				cFind.Text = string.Empty;
				int iExist = dtAdmColDrgGrid.Rows.Count;
				...
				foreach (DataRowView drv in dti.DefaultView)
				{
					...
					{
						DataTable dtd;
						dtd = jj==iExist ? null : (DataTable)Session[KEY_dtScreenId14];
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(1007,"GetDdlScreenId3S3261",true,true,0,string.Empty,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
							Session[KEY_dtScreenId14] = dtd;
						}
						ScrImportDdl(dtd.DefaultView, drv, true, "ScreenObjId", "ScreenId14", "ScreenId14Text", 20, "", false);
						dtd = (DataTable)Session[KEY_dtGroupRowId14];
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(1007,"GetDdlGroupRowId3S3263",true,true,0,string.Empty,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
							Session[KEY_dtGroupRowId14] = dtd;
						}
						ScrImportDdl(dtd.DefaultView, drv, false, "ScreenObjId", "GroupRowId14", "GroupRowId14Text", 50, "", false);
						dtd = (DataTable)Session[KEY_dtGroupColId14];
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(1007,"GetDdlGroupColId3S3264",true,true,0,string.Empty,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
							Session[KEY_dtGroupColId14] = dtd;
						}
						ScrImportDdl(dtd.DefaultView, drv, false, "ScreenObjId", "GroupColId14", "GroupColId14Text", 50, "", false);
						dtd = (DataTable)Session[KEY_dtTabFolderId14];
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(1007,"GetDdlTabFolderId3S3291",true,true,0,string.Empty,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
							Session[KEY_dtTabFolderId14] = dtd;
						}
						ScrImportDdl(dtd.DefaultView, drv, false, "ScreenObjId", "TabFolderId14", "TabFolderId14Text", 30, "", false);
					}
					jj = jj + 1;
				}
				iRow = dti.DefaultView.Count - iExist;
				if (!dti.Columns.Contains("_NewRow")) { dti.Columns.Add("_NewRow"); }
				Session[KEY_dtAdmColDrgGrid] = dti;
				cAdmColDrgGrid_DataBind(dti.DefaultView);
		...
		public void cImportButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cFNameO.Text != string.Empty && cWorkSheet.Items.Count > 0 && cWorkSheet.SelectedItem.Text != string.Empty && cStartRow.Text != string.Empty)
			{
				...
				try
				{
					if (cAdmColDrgGrid.EditIndex < 0 || UpdateGridRow(cAdmColDrgGrid, new CommandEventArgs("Save", "")))
					{
						Session[KEY_scrImport] = ImportFile(dtAdmColDrgGrid.Copy(),cFNameO.Text,cWorkSheet.SelectedItem.Text,cStartRow.Text,Config.PathTmpImport + cFName.Text);
					}
		...
		private DataTable ImportFile(DataTable dt, string fileName, string workSheet, string startRow, string fileFullName)
		{
			try
			{
				...
				{
					if (rows[iRow][1].ToString() == string.Empty && rows[iRow][2].ToString() == string.Empty && rows[iRow][3].ToString() == string.Empty && rows[iRow][4].ToString() == string.Empty && rows[iRow][5].ToString() == string.Empty && rows[iRow][6].ToString() == string.Empty && rows[iRow][7].ToString() == string.Empty && rows[iRow][8].ToString() == string.Empty && rows[iRow][9].ToString() == string.Empty && rows[iRow][10].ToString() == string.Empty && rows[iRow][11].ToString() == string.Empty) {idel = idel + 1;}
					else
					{
		...
		private void GotoPage(int pageNo)
		{
			if (pageNo >= 0 && pageNo < GetTotalPages())
			{
				if (cAdmColDrgGrid.EditIndex < 0 || UpdateGridRow(cAdmColDrgGrid, new CommandEventArgs("Save", "")))
				{
					try { cAdmColDrgGridDataPager.SetPageProperties(cAdmColDrgGridDataPager.PageSize * pageNo, cAdmColDrgGridDataPager.MaximumRows, false); cAdmColDrgGrid_DataBind(null); }
					catch
					{
						try { cAdmColDrgGridDataPager.SetPageProperties(0, cAdmColDrgGridDataPager.MaximumRows, false); cAdmColDrgGrid_DataBind(null); } catch {}
					}
				}
			}
		}

		protected void cAdmColDrgGrid_OnItemCommand(object sender, ListViewCommandEventArgs e)
		{
		}

		private void cAdmColDrgGrid_DataBind(DataView dvAdmColDrgGrid)
		{
			if (dvAdmColDrgGrid == null)
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmColDrgGrid];
				dvAdmColDrgGrid = dt != null ? dt.DefaultView : null;
			}
			if (dvAdmColDrgGrid != null)
			{
				LcAuth = GetAuthCol().AsEnumerable().ToDictionary<DataRow,string>(dr=>dr["ColName"].ToString());
				cAdmColDrgGrid.DataSource = dvAdmColDrgGrid;
				cAdmColDrgGrid.DataBind();
		...
		protected void cAdmColDrgGrid_OnSorting(object sender, ListViewSortEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmColDrgGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null)
			{
				if (cAdmColDrgGrid.EditIndex < 0 || UpdateGridRow(cAdmColDrgGrid, new CommandEventArgs("Save", "")))
				{
					...
					Session[KEY_dtAdmColDrgGrid] = dt;
					cAdmColDrgGrid_DataBind(dv);
				}
			}
		}

		protected void cAdmColDrgGrid_OnPagePropertiesChanging(object sender, PagePropertiesChangingEventArgs e)
		{
		    cAdmColDrgGridDataPager.SetPageProperties(e.StartRowIndex, e.MaximumRows, false);
		    DataTable dt = (DataTable)Session[KEY_dtAdmColDrgGrid];
		    DataView dv = dt != null ? dt.DefaultView : null;
			cAdmColDrgGrid_DataBind(dv); cAdmColDrgGrid.EditIndex = -1;
		}

		private void GridChkPgDirty(ListViewItem lvi)
		{
				WebControl cc = null;
				cc = ((WebControl)lvi.FindControl("cScreenId14"));
				...
				cc = ((WebControl)lvi.FindControl("cGroupRowId14"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cGroupColId14"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cColumnName14"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cTabFolderId14"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cTabIndex14"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cMasterTable14"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
		}

		protected void cAdmColDrgGrid_OnItemDataBound(object sender, ListViewItemEventArgs e)
		{
			// *** GridItemDataBound (before) Web Rule End *** //
			DataTable dt = (DataTable)Session[KEY_dtAdmColDrgGrid];
			bool isEditItem = false;
			DataView dvAdmColDrgGrid = dt != null ? dt.DefaultView : null;
			if (cAdmColDrgGrid.EditIndex > -1 && GetDataItemIndex(cAdmColDrgGrid.EditIndex) == e.Item.DataItemIndex)
			{
				isEditItem = true;
				base.SetGridEnabled(e.Item, GetAuthCol(), GetLabel(), 0);
				SetScreenId14((RoboCoder.WebControls.ComboBox)e.Item.FindControl("cScreenId14"),dvAdmColDrgGrid[e.Item.DataItemIndex]["ScreenId14"].ToString(), e.Item);
				SetGroupRowId14((DropDownList)e.Item.FindControl("cGroupRowId14"),dvAdmColDrgGrid[e.Item.DataItemIndex]["GroupRowId14"].ToString());
				SetGroupColId14((DropDownList)e.Item.FindControl("cGroupColId14"),dvAdmColDrgGrid[e.Item.DataItemIndex]["GroupColId14"].ToString());
				SetTabFolderId14((DropDownList)e.Item.FindControl("cTabFolderId14"),dvAdmColDrgGrid[e.Item.DataItemIndex]["TabFolderId14"].ToString());
			...
			if (dtAuthRow != null)
			{
				...
				{
					cc = e.Item.FindControl("cAdmColDrgGridDelete"); if (cc != null) { cc.Visible = false; }
				}
				if (!AllowRowEdit(GetAuthRow(),dvAdmColDrgGrid[e.Item.DataItemIndex]["ScreenObjId14"].ToString()) || (Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N"))
				{
					cc = e.Item.FindControl("cAdmColDrgGridEdit"); if (cc != null) { cc.Visible = false; }
				}
				else
				{
			        HtmlTableRow tr = e.Item.FindControl("cAdmColDrgGridRow") as HtmlTableRow;
			        LinkButton lb = e.Item.FindControl("cAdmColDrgGridEdit") as LinkButton;
		...
		protected void cScreenObjId14hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "0") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "0"; Session[KEY_lastSortExp] = "ScreenObjId14";Session[KEY_lastSortImg] = "cScreenObjId14hi";
			cAdmColDrgGrid_OnSorting(sender, new ListViewSortEventArgs("ScreenObjId14", SortDirection.Ascending));
		}

		protected void cScreenId14hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "1") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "1"; Session[KEY_lastSortExp] = "ScreenId14Text";Session[KEY_lastSortImg] = "cScreenId14hi";
			cAdmColDrgGrid_OnSorting(sender, new ListViewSortEventArgs("ScreenId14Text", SortDirection.Ascending));
		}

		protected void cGroupRowId14hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "2") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "2"; Session[KEY_lastSortExp] = "GroupRowId14Text";Session[KEY_lastSortImg] = "cGroupRowId14hi";
			cAdmColDrgGrid_OnSorting(sender, new ListViewSortEventArgs("GroupRowId14Text", SortDirection.Ascending));
		}

		protected void cGroupColId14hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "3") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "3"; Session[KEY_lastSortExp] = "GroupColId14Text";Session[KEY_lastSortImg] = "cGroupColId14hi";
			cAdmColDrgGrid_OnSorting(sender, new ListViewSortEventArgs("GroupColId14Text", SortDirection.Ascending));
		}

		protected void cColumnName14hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "4") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "4"; Session[KEY_lastSortExp] = "ColumnName14";Session[KEY_lastSortImg] = "cColumnName14hi";
			cAdmColDrgGrid_OnSorting(sender, new ListViewSortEventArgs("ColumnName14", SortDirection.Ascending));
		}

		protected void cTabFolderId14hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "5") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "5"; Session[KEY_lastSortExp] = "TabFolderId14Text";Session[KEY_lastSortImg] = "cTabFolderId14hi";
			cAdmColDrgGrid_OnSorting(sender, new ListViewSortEventArgs("TabFolderId14Text", SortDirection.Ascending));
		}

		protected void cTabIndex14hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "6") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "6"; Session[KEY_lastSortExp] = "TabIndex14";Session[KEY_lastSortImg] = "cTabIndex14hi";
			cAdmColDrgGrid_OnSorting(sender, new ListViewSortEventArgs("TabIndex14", SortDirection.Ascending));
		}

		protected void cMasterTable14hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "7") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "7"; Session[KEY_lastSortExp] = "MasterTable14";Session[KEY_lastSortImg] = "cMasterTable14hi";
			cAdmColDrgGrid_OnSorting(sender, new ListViewSortEventArgs("MasterTable14", SortDirection.Ascending));
		}

		protected void cAdmColDrgGrid_OnItemEditing(object sender, ListViewEditEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmColDrgGrid];
			if ((ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")) && dt != null && dt.DefaultView.Count > GetDataItemIndex(e.NewEditIndex))
			{
				cAdmColDrgGrid.EditIndex = e.NewEditIndex;
				cAdmColDrgGrid_DataBind(null);
				Control wc = null;
				string idx = Page.Request["__EVENTARGUMENT"];
				if (!string.IsNullOrEmpty(idx))
				{
				    Control ctrlToFocus = cAdmColDrgGrid.EditItem.FindControl("c" + idx);
				    if (((WebControl)ctrlToFocus).Enabled) { wc = ctrlToFocus; }
				}
				if (wc == null) { wc = FindEditableControl(cAdmColDrgGrid.EditItem); }
				if (wc != null)
             {
             ...
             MkMobileLabel(cAdmColDrgGrid.EditItem);
		    }
		    else { e.Cancel = true; }
		}
		...
		{
		    Label ml = null;
		    ml = lvi.FindControl("cScreenObjId14ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(0); }
		    ml = lvi.FindControl("cScreenId14ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(1); }
		    ml = lvi.FindControl("cGroupRowId14ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(2); }
		    ml = lvi.FindControl("cGroupColId14ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(3); }
		    ml = lvi.FindControl("cColumnName14ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(4); }
		    ml = lvi.FindControl("cTabFolderId14ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(5); }
		    ml = lvi.FindControl("cTabIndex14ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(6); }
		    ml = lvi.FindControl("cMasterTable14ml") as Label;
		...
		protected void GridFill(ListViewItem lvi, DataTable dt, DataRow dr, bool bInsert)
		{
			...
			cbb = (RoboCoder.WebControls.ComboBox)lvi.FindControl("cScreenId14");
			if (cbb != null)
			{
				if (cbb.SelectedValue != string.Empty)
				{
					dr["ScreenId14"] = cbb.SelectedValue;
					dr["ScreenId14Text"] = cbb.SelectedText;
				}
				else
				{
					dr["ScreenId14"] = System.DBNull.Value;
					dr["ScreenId14Text"] = System.DBNull.Value;
				}
			}
			DropDownList ddl = null;
			ddl = (DropDownList)lvi.FindControl("cGroupRowId14");
			if (ddl != null)
			{
				if (ddl.SelectedIndex >= 0 && ddl.SelectedValue.ToString() != string.Empty)
				{
					dr["GroupRowId14"] = ddl.SelectedValue;
					dr["GroupRowId14Text"] = ddl.SelectedItem.Text;
				}
				else
				{
					dr["GroupRowId14"] = System.DBNull.Value;
					dr["GroupRowId14Text"] = System.DBNull.Value;
				}
			}
			ddl = (DropDownList)lvi.FindControl("cGroupColId14");
			if (ddl != null)
			{
				if (ddl.SelectedIndex >= 0 && ddl.SelectedValue.ToString() != string.Empty)
				{
					dr["GroupColId14"] = ddl.SelectedValue;
					dr["GroupColId14Text"] = ddl.SelectedItem.Text;
				}
				else
				{
					dr["GroupColId14"] = System.DBNull.Value;
					dr["GroupColId14Text"] = System.DBNull.Value;
				}
			}
			TextBox tb = null;
			tb = (TextBox)lvi.FindControl("cColumnName14");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["ColumnName14"] = tb.Text;} else {dr["ColumnName14"] = System.DBNull.Value;}
			}
			ddl = (DropDownList)lvi.FindControl("cTabFolderId14");
			if (ddl != null)
			{
				if (ddl.SelectedIndex >= 0 && ddl.SelectedValue.ToString() != string.Empty)
				{
					dr["TabFolderId14"] = ddl.SelectedValue;
					dr["TabFolderId14Text"] = ddl.SelectedItem.Text;
				}
				else
				{
					dr["TabFolderId14"] = System.DBNull.Value;
					dr["TabFolderId14Text"] = System.DBNull.Value;
				}
			}
			tb = (TextBox)lvi.FindControl("cTabIndex14");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["TabIndex14"] = tb.Text;} else {dr["TabIndex14"] = System.DBNull.Value;}
			}
			tb = (TextBox)lvi.FindControl("cMasterTable14");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["MasterTable14"] = tb.Text;} else {dr["MasterTable14"] = System.DBNull.Value;}
			}
		    if (bInsert) { dt.Rows.InsertAt(dr, 0); }
			Session[KEY_dtAdmColDrgGrid] = dt;
			cAdmColDrgGrid.EditIndex = -1;
			cAdmColDrgGrid_DataBind(dt.DefaultView);
		}

		protected void cAdmColDrgGrid_OnItemUpdating(object sender, ListViewUpdateEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmColDrgGrid];
			if (dt != null && dt.Rows.Count > 0)
			{
				DataRow dr = dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row;
			    GridFill(cAdmColDrgGrid.EditItem, dt, dr, false);
			    dr["_NewRow"] = "N";
			    if (dt.Columns.Contains("_SortOrder")) dr["_SortOrder"] = null;
			}
		}

		protected void cAdmColDrgGrid_OnItemCanceling(object sender, ListViewCancelEventArgs e)
		{
			PanelTop.Update();
			DataTable dt = (DataTable)Session[KEY_dtAdmColDrgGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			...
			{
				cAdmColDrgGrid_OnItemDeleting(sender, new ListViewDeleteEventArgs(e.ItemIndex));
			}
			if (dt.Columns.Contains("_SortOrder")) dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row["_SortOrder"] = null;
			cAdmColDrgGrid.EditIndex = -1; cAdmColDrgGrid_DataBind(null);
		}
		...
		protected void cDeleteAllButton_Click(object sender, System.EventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmColDrgGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null && (cAdmColDrgGrid.EditIndex < 0 || UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				GotoPage(0);
				while (dv.Count > 0) {dv.Delete(0);}
				Session[KEY_dtAdmColDrgGrid] = dt;
				cAdmColDrgGrid.EditIndex = -1; cAdmColDrgGrid_DataBind(dv);
			}
		}

		protected void cAdmColDrgGrid_OnItemDeleting(object sender, ListViewDeleteEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmColDrgGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			DataRow dr = dv != null ? dv[GetDataItemIndex(e.ItemIndex)].Row : null;
			if (dv != null && (cAdmColDrgGrid.EditIndex == e.ItemIndex || UpdateGridRow(sender, new CommandEventArgs("Save",""))))
			{
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmColDrgGrid); PopAdmColDrg1007List(sender, e, false, null);
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			// *** System Button Click (Before) Web Rule starts here *** //
			if (ValidPage() && UpdateGridRow(cAdmColDrgGrid, new CommandEventArgs("Save", "")))
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmColDrgGrid];
				DataView dv = dt != null ? dt.DefaultView : null;
				string ftr = string.Empty;
				if (dv.RowFilter != string.Empty) { ftr = dv.RowFilter; dv.RowFilter = string.Empty; }
				AdmColDrg1007 ds = PrepAdmColDrgData(dv,false);
				if (ftr != string.Empty) {dv.RowFilter = ftr;}
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(1007,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc,noTrans,commandTimeOut)) {bValid7 = true;}
					if (bValid7)
					{
						Session[KEY_currPageIndex] = GetCurrPageIndex();
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmColDrg1007List(sender, e, false, null);
		...
		private AdmColDrg1007 PrepAdmColDrgData(DataView dv, bool bAdd)
		{
			AdmColDrg1007 ds = new AdmColDrg1007();
			if (dv != null)
			{
				ds.Tables["AdmColDrgDef"].Rows.Add(MakeTypRow(ds.Tables["AdmColDrgDef"].NewRow()));
				ds.Tables["AdmColDrgDef"].Rows.Add(MakeDisRow(ds.Tables["AdmColDrgDef"].NewRow()));
					dv.RowStateFilter = DataViewRowState.ModifiedCurrent;
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmColDrgUpd"].Rows.Add(MakeColRow(ds.Tables["AdmColDrgUpd"].NewRow(), drv, false));
					}
					dv.RowStateFilter = DataViewRowState.Added;
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmColDrgAdd"].Rows.Add(MakeColRow(ds.Tables["AdmColDrgAdd"].NewRow(), drv, true));
					}
					dv.RowStateFilter = DataViewRowState.Deleted;
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmColDrgDel"].Rows.Add(MakeColRow(ds.Tables["AdmColDrgDel"].NewRow(), drv, false));
					}
		...
		private DataRow MakeTypRow(DataRow dr)
		{
			...
			dr["ScreenId14"] = System.Data.OleDb.OleDbType.Numeric.ToString();
			dr["GroupRowId14"] = System.Data.OleDb.OleDbType.Numeric.ToString();
			dr["GroupColId14"] = System.Data.OleDb.OleDbType.Numeric.ToString();
			dr["ColumnName14"] = System.Data.OleDb.OleDbType.VarChar.ToString();
			dr["TabFolderId14"] = System.Data.OleDb.OleDbType.Numeric.ToString();
			dr["TabIndex14"] = System.Data.OleDb.OleDbType.Numeric.ToString();
			dr["MasterTable14"] = System.Data.OleDb.OleDbType.Char.ToString();
			return dr;
		}
		...
		private DataRow MakeDisRow(DataRow dr)
		{
			...
			dr["ScreenId14"] = "AutoComplete";
			dr["GroupRowId14"] = "DropDownList";
			dr["GroupColId14"] = "DropDownList";
			dr["ColumnName14"] = "TextBox";
			dr["TabFolderId14"] = "DropDownList";
			dr["TabIndex14"] = "TextBox";
			dr["MasterTable14"] = "TextBox";
			return dr;
		}
		...
		private DataRow MakeColRow(DataRow dr, DataRowView drv, bool bAdd)
		{
			...
			if (dtAuth != null)
			{
				dr["ScreenObjId14"] = drv["ScreenObjId14"].ToString().Trim();
				dr["ScreenId14"] = drv["ScreenId14"];
				dr["GroupRowId14"] = drv["GroupRowId14"];
				dr["GroupColId14"] = drv["GroupColId14"];
				dr["ColumnName14"] = drv["ColumnName14"].ToString().Trim();
				dr["TabFolderId14"] = drv["TabFolderId14"];
				dr["TabIndex14"] = drv["TabIndex14"].ToString().Trim();
				dr["MasterTable14"] = drv["MasterTable14"].ToString().Trim();
			}
			return dr;
		}
		...
		{
			if (cAdmColDrgGrid.EditIndex > -1)
			{
				cAdmColDrgGrid_OnItemUpdating(sender, new ListViewUpdateEventArgs(cAdmColDrgGrid.EditIndex));
			}
			return true;
		}

		protected void cAdmColDrgGrid_OnPreRender(object sender, System.EventArgs e)
		{
			System.Web.UI.WebControls.Image hi = null;
			hi = (System.Web.UI.WebControls.Image)cAdmColDrgGrid.FindControl("cScreenObjId14hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmColDrgGrid.FindControl("cScreenId14hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmColDrgGrid.FindControl("cGroupRowId14hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmColDrgGrid.FindControl("cGroupColId14hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmColDrgGrid.FindControl("cColumnName14hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmColDrgGrid.FindControl("cTabFolderId14hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmColDrgGrid.FindControl("cTabIndex14hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmColDrgGrid.FindControl("cMasterTable14hi"); if (hi != null) { hi.Visible = false; }
			if (Session[KEY_lastSortImg] != null)
			{
				hi = (System.Web.UI.WebControls.Image)cAdmColDrgGrid.FindControl((string)Session[KEY_lastSortImg]);
				if (hi != null) { hi.ImageUrl = Utils.AddTilde((string)Session[KEY_lastSortUrl]); hi.Visible = true; }
			}
			IgnoreHeaderConfirm((LinkButton)cAdmColDrgGrid.FindControl("cScreenObjId14hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmColDrgGrid.FindControl("cScreenId14hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmColDrgGrid.FindControl("cGroupRowId14hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmColDrgGrid.FindControl("cGroupColId14hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmColDrgGrid.FindControl("cColumnName14hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmColDrgGrid.FindControl("cTabFolderId14hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmColDrgGrid.FindControl("cTabIndex14hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmColDrgGrid.FindControl("cMasterTable14hl"));
		}

		private string GetButtonId(ListViewItem lvi)
		{
			string ButtonID = String.Empty;
			Control c = lvi.FindControl("cAdmColDrgGridEdit");
		...
		{
		    return cAdmColDrgGridDataPager.StartRowIndex / cAdmColDrgGridDataPager.PageSize;
		}

		private int GetTotalPages()
		{
		    return (int)Math.Ceiling((double)cAdmColDrgGridDataPager.TotalRowCount / cAdmColDrgGridDataPager.PageSize);
		}

		private int GetDataItemIndex(int editIndex)
		{
		    return cAdmColDrgGridDataPager.StartRowIndex + editIndex;
		}

		protected void cAdmColDrgGrid_OnLayoutCreated(object sender, EventArgs e)
		{
		    // Header:
		    LinkButton lb = null;
		    lb = cAdmColDrgGrid.FindControl("cScreenObjId14hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(0); lb.ToolTip = ColumnToolTip(0); lb.Parent.Visible = GridColumnVisible(0); }
		    lb = cAdmColDrgGrid.FindControl("cScreenId14hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(1); lb.ToolTip = ColumnToolTip(1); lb.Parent.Visible = GridColumnVisible(1); }
		    lb = cAdmColDrgGrid.FindControl("cGroupRowId14hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(2); lb.ToolTip = ColumnToolTip(2); lb.Parent.Visible = GridColumnVisible(2); }
		    lb = cAdmColDrgGrid.FindControl("cGroupColId14hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(3); lb.ToolTip = ColumnToolTip(3); lb.Parent.Visible = GridColumnVisible(3); }
		    lb = cAdmColDrgGrid.FindControl("cColumnName14hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(4); lb.ToolTip = ColumnToolTip(4); lb.Parent.Visible = GridColumnVisible(4); }
		    lb = cAdmColDrgGrid.FindControl("cTabFolderId14hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(5); lb.ToolTip = ColumnToolTip(5); lb.Parent.Visible = GridColumnVisible(5); }
		    lb = cAdmColDrgGrid.FindControl("cTabIndex14hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(6); lb.ToolTip = ColumnToolTip(6); lb.Parent.Visible = GridColumnVisible(6); }
		    lb = cAdmColDrgGrid.FindControl("cMasterTable14hl") as LinkButton;
			...
			if (dtAuthRow != null)
			{
				...
				if (dr["AllowDel"].ToString() == "N" || (Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N"))
				{
		            lb = cAdmColDrgGrid.FindControl("cDeleteAllButton") as LinkButton; if (lb != null) { lb.Visible = false; }
				}
			}
		    // footer:
		    Label gc = null;
		    gc = cAdmColDrgGrid.FindControl("cScreenObjId14fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(0); }
		    gc = cAdmColDrgGrid.FindControl("cScreenId14fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(1); }
		    gc = cAdmColDrgGrid.FindControl("cGroupRowId14fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(2); }
		    gc = cAdmColDrgGrid.FindControl("cGroupColId14fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(3); }
		    gc = cAdmColDrgGrid.FindControl("cColumnName14fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(4); }
		    gc = cAdmColDrgGrid.FindControl("cTabFolderId14fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(5); }
		    gc = cAdmColDrgGrid.FindControl("cTabIndex14fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(6); }
		    gc = cAdmColDrgGrid.FindControl("cMasterTable14fl") as Label;

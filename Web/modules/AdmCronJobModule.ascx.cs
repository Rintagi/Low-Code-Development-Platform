	...
	public partial class AdmCronJobModule : RO.Web.ModuleBase
	{
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				try
				{
					try {dv = new DataView((new AdminSystem()).GetExp(118,"GetExpAdmCronJob118","Y",(string)Session[KEY_sysConnectionString],LcAppPw,filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
					catch {dv = new DataView((new AdminSystem()).GetExp(118,"GetExpAdmCronJob118","N",(string)Session[KEY_sysConnectionString],LcAppPw,filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
...
\par }}");
		...
		private DataView GetAdmCronJob118List()
		{
			...
			if (dt == null)
			{
				...
				try
				{
					try {dt = (new AdminSystem()).GetLis(118,"GetLisAdmCronJob118",true,"Y",0,(string)Session[KEY_sysConnectionString],LcAppPw,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
					catch {dt = (new AdminSystem()).GetLis(118,"GetLisAdmCronJob118",true,"N",0,(string)Session[KEY_sysConnectionString],LcAppPw,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				}
		...
		protected void cAdmCronJob118List_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			...
			try
			{
				dt = (new AdminSystem()).GetMstById("GetAdmCronJob118ById",cAdmCronJob118List.SelectedValue,(string)Session[KEY_sysConnectionString],LcAppPw);
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			if (ValidPage())
			{
				...
				if (string.IsNullOrEmpty(cAdmCronJob118List.SelectedValue))	// Add
				{
					if (ds != null)
					{
						pid = (new AdminSystem()).AddData(118,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],LcAppPw,base.CPrj,base.CSrc);
					}
					...
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(118,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],LcAppPw,base.CPrj,base.CSrc)) {bValid7 = true;}
					if (bValid7)
					{
						cAdmCronJob118List.ClearSearch(); Session.Remove(KEY_dtAdmCronJob118List);
		...
		public void cDeleteButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cCronJobId264.Text != string.Empty)
			{
				...
				try
				{
					if (ds != null && (new AdminSystem()).DelData(118,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],LcAppPw,base.CPrj,base.CSrc))
					{
						cAdmCronJob118List.ClearSearch(); Session.Remove(KEY_dtAdmCronJob118List);

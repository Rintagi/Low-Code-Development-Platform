		...
		private void getReport(string eExport)
		{
			try
			{
				...
				if (eExport == "TXT")
				{
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmRelease.csv"; Session["ExportStr"] = (Config.ExportExcelCSV ? "sep=\t\n": "") + sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			if (ValidPage() && (cAdmReleaseGrid.EditIndex < 0 || UpdateGridRow(cAdmReleaseGrid, new CommandEventArgs("Save", ""))))
			{
				...
				if (ftr != string.Empty) {dv.RowFilter = ftr;}
				if (!string.IsNullOrEmpty(cAdmRelease98List.SelectedValue) && cReleaseId191.Text != cAdmRelease98List.SelectedValue)
				{
				...
				if (string.IsNullOrEmpty(cAdmRelease98List.SelectedValue))	// Add
				{
					if (ds != null)
					{
						pid = (new AdminSystem()).AddData(98,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc,noTrans,commandTimeOut);
					}
					...
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(98,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc,noTrans,commandTimeOut)) {bValid7 = true;}
					if (bValid7)
					{
						cAdmRelease98List.ClearSearch(); Session.Remove(KEY_dtAdmRelease98List);
		...
		private bool UpdateGridRow(object sender, CommandEventArgs e)
		{
			...
			{
				cAdmReleaseGrid_OnItemUpdating(sender, new ListViewUpdateEventArgs(cAdmReleaseGrid.EditIndex));
			}
			return true;
		}

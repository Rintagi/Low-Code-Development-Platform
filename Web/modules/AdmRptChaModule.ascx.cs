...
namespace RO.Web
{
	public partial class AdmRptChaModule : RO.Web.ModuleBase
	{
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_100";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_100";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_100";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_100";
		private const string KEY_dtLabel = "Cache:dtLabel3_100";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_100";
		private const string KEY_dtScrCri = "Cache:dtScrCri3_100";
		private const string KEY_dsScrCriVal = "Cache:dsScrCriVal3_100";

		private const string KEY_dtRptCtrId206 = "Cache:dtRptCtrId206";
		private const string KEY_dtReportId206 = "Cache:dtReportId206";
		private const string KEY_dtRptChaTypeCd206 = "Cache:dtRptChaTypeCd206";
		private const string KEY_dtCategoryGrp206 = "Cache:dtCategoryGrp206";
		private const string KEY_dtSeriesGrp206 = "Cache:dtSeriesGrp206";

		private const string KEY_dtSystems = "Cache:dtSystems3_100";
		private const string KEY_sysConnectionString = "Cache:sysConnectionString3_100";
		private const string KEY_dtAdmRptCha100List = "Cache:dtAdmRptCha100List";
		private const string KEY_bNewVisible = "Cache:bNewVisible3_100";
		private const string KEY_bNewSaveVisible = "Cache:bNewSaveVisible3_100";
		private const string KEY_bCopyVisible = "Cache:bCopyVisible3_100";
		private const string KEY_bCopySaveVisible = "Cache:bCopySaveVisible3_100";
		private const string KEY_bDeleteVisible = "Cache:bDeleteVisible3_100";
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_100";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_100";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_100";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				DataTable dtMenuAccess = (new MenuSystem()).GetMenu(base.LUser.CultureId, 3, base.LImpr, LcSysConnString, LcAppPw,100, null, null);
				...
				Session.Remove(KEY_dtAdmRptCha100List);
				...
				Session.Remove(KEY_dtRptCtrId206);
				Session.Remove(KEY_dtReportId206);
				Session.Remove(KEY_dtRptChaTypeCd206);
				Session.Remove(KEY_dtCategoryGrp206);
				Session.Remove(KEY_dtSeriesGrp206);
				SetButtonHlp();
				GetSystems();
				SetColumnAuthority();
				GetGlobalFilter();
				...
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 100, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmRptCha100List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=100&typ=N&sys=3','','',''); return false;"; }
		...
		private void SetSystem(byte SystemId)
		{
			...
			try
			{
				...
				if ((Config.DeployType == "DEV" || row["dbAppDatabase"].ToString() == base.CPrj.EntityCode + "View") && !(base.CPrj.EntityCode != "RO" && row["SysProgram"].ToString() == "Y") && (new AdminSystem()).IsRegenNeeded(string.Empty,100,0,0,LcSysConnString,LcAppPw))
				{
					(new GenScreensSystem()).CreateProgram(100, "Report Chart", row["dbAppDatabase"].ToString(), base.CPrj, base.CSrc, base.CTar, LcAppConnString, LcAppPw);
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(100,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(100,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(100,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(100,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,100,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(100,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(100,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(100,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		protected DataTable GetAuthRow()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthRow(100,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				try
				{
					try {dv = new DataView((new AdminSystem()).GetExp(100,"GetExpAdmRptCha100","Y",(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
					catch {dv = new DataView((new AdminSystem()).GetExp(100,"GetExpAdmRptCha100","N",(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(100,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					{
						if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["RptChaId206"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["RptCtrId206"].ToString() + (char)9 + drv["RptCtrId206Text"].ToString() + (char)9);}
						if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(drv["ReportId206"].ToString() + (char)9 + drv["ReportId206Text"].ToString() + (char)9);}
						if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["RptChaTypeCd206"].ToString().Replace("\"","\"\"") + "\"" + (char)9 + "\"" + drv["RptChaTypeCd206Text"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["ThreeD206"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append(drv["CategoryGrp206"].ToString() + (char)9 + drv["CategoryGrp206Text"].ToString() + (char)9);}
						if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["ChartData206"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[7]["ColExport"].ToString() == "Y") {sb.Append(drv["SeriesGrp206"].ToString() + (char)9 + drv["SeriesGrp206Text"].ToString() + (char)9);}
						sb.Append(Environment.NewLine);
					}
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmRptCha.csv"; Session["ExportStr"] = (Config.ExportExcelCSV ? "sep=\t\n": "") + sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmRptCha.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private string GetRtfGrid(DataView dv)
		{
			...
			try
			{
				...
				foreach (DataRowView drv in dv)
				{
					...
					sb.Append("\r\n");
					sb.Append(@"{");
					if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["RptChaId206"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["RptCtrId206Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(drv["ReportId206Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(drv["RptChaTypeCd206Text"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append(drv["ThreeD206"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append(drv["CategoryGrp206Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append(drv["ChartData206"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[7]["ColExport"].ToString() == "Y") {sb.Append(drv["SeriesGrp206Text"].ToString() + @"\cell ");}
					sb.Append(@"}");
					sb.Append("\r\n");
					sb.Append(@"\pard \ql \li0\ri0\widctlpar\intbl\aspalpha\aspnum\adjustright\rin0\lin0 {");
					sb.Append(@"\insrsid2981395 \trowd \irow0\irowband0\lastrow \ts15\trgaph108\trleft-108\trbrdrt\brdrs\brdrw10 \trbrdrl\brdrs\brdrw10 \trbrdrb\brdrs\brdrw10 \trbrdrr\brdrs\brdrw10 \trbrdrh\brdrs\brdrw10 \trbrdrv\brdrs\brdrw10 ");
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 100, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			...
			UpdCriteria(true);
			cAdmRptCha100List.ClearSearch(); Session.Remove(KEY_dtAdmRptCha100List);
			PopAdmRptCha100List(sender, e, false, null);
		}
			...
			try
			{
				foreach (DataRowView drv in dvCri)
				{
					...
					if (drv["DisplayName"].ToString() == "ComboBox")
					{
						...
						(new AdminSystem()).MkGetScreenIn("100", drv["ScreenCriId"].ToString(), "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), LcAppDb, LcDesDb, drv["MultiDesignDb"].ToString(), LcSysConnString, LcAppPw);
						DataView dv = new DataView((new AdminSystem()).GetScreenIn("100", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), (new AdminSystem()).CountScrCri(drv["ScreenCriId"].ToString(), drv["MultiDesignDb"].ToString(), drv["MultiDesignDb"].ToString() == "N" ? LcSysConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)), drv["RequiredValid"].ToString(), 0, string.Empty, drv["DisplayMode"].ToString() != "AutoListBox", val, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)));
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("100", LcSysConnString, LcAppPw);
				}
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmRptCha100" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmRptCha100" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			foreach (DataRowView drv in dvCri)
			{
			    if (drv["DisplayName"].ToString() == "ListBox")
			    {
					...
					if (cListBox != null)
					{
						...
						int TotalChoiceCnt = new DataView((new AdminSystem()).GetScreenIn("100", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), CriCnt, drv["RequiredValid"].ToString(), 0, string.Empty, true, string.Empty, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId))).Count;
						string selectedValues = string.Join(",", cListBox.Items.Cast<ListItem>().Where(x => x.Selected).Select(x => "'" + x.Value + "'").ToArray());
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("100", "AdmRptCha100", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		protected void cbRptCtrId206(object sender, System.EventArgs e)
		{
			SetRptCtrId206((RoboCoder.WebControls.ComboBox)sender,string.Empty);
		}

		private void SetRptCtrId206(RoboCoder.WebControls.ComboBox ddl, string keyId)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			context["method"] = "GetDdlRptCtrId3S1732";
			context["addnew"] = "Y";
			context["mKey"] = "RptCtrId206";
			context["mVal"] = "RptCtrId206Text";
			context["mTip"] = "RptCtrId206Text";
			context["mImg"] = "RptCtrId206Text";
			context["ssd"] = Request.QueryString["ssd"];
			context["scr"] = "100";
			context["csy"] = "3";
			context["filter"] = Utils.IsInt(cFilterId.SelectedValue)? cFilterId.SelectedValue : "0";
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(100,"GetDdlRptCtrId3S1732",true,false,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
					if (dv.Table.Columns.Contains("RptChaId"))
					{
						context["pMKeyColID"] = cAdmRptCha100List.ClientID;
						context["pMKeyCol"] = "RptChaId";
						string ss = "(RptChaId is null";
						if (string.IsNullOrEmpty(cAdmRptCha100List.SelectedValue)) {ss = ss + ")";} else {ss = ss + " OR RptChaId = " + cAdmRptCha100List.SelectedValue + ")";}
						dv.RowFilter = ss;
					}
					ddl.DataSource = dv; Session[KEY_dtRptCtrId206] = dv.Table;
				    try { ddl.SelectByValue(keyId,string.Empty,true); } catch { try { ddl.SelectedIndex = 0; } catch { } }
				}
			}
		}

		protected void cbReportId206(object sender, System.EventArgs e)
		{
			SetReportId206((RoboCoder.WebControls.ComboBox)sender,string.Empty);
		}

		private void SetReportId206(RoboCoder.WebControls.ComboBox ddl, string keyId)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			context["method"] = "GetDdlReportId3S1733";
			context["addnew"] = "Y";
			context["mKey"] = "ReportId206";
			context["mVal"] = "ReportId206Text";
			context["mTip"] = "ReportId206Text";
			context["mImg"] = "ReportId206Text";
			context["ssd"] = Request.QueryString["ssd"];
			context["scr"] = "100";
			context["csy"] = "3";
			context["filter"] = Utils.IsInt(cFilterId.SelectedValue)? cFilterId.SelectedValue : "0";
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(100,"GetDdlReportId3S1733",true,false,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
					...
					ddl.DataSource = dv; Session[KEY_dtReportId206] = dv.Table;
				    try { ddl.SelectByValue(keyId,string.Empty,true); } catch { try { ddl.SelectedIndex = 0; } catch { } }
				}
			}
		}

		private void SetRptChaTypeCd206(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtRptChaTypeCd206];
			...
			if (ddl != null)
			{
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(100,"GetDdlRptChaTypeCd3S1734",true,bAll,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					...
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "RptChaTypeCd206 is not NULL"; }
					dv.RowFilter = ss;
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(100,"GetDdlRptChaTypeCd3S1734",true,true,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtRptChaTypeCd206] = dv.Table;
				}
			}
		}

		private void SetCategoryGrp206(DropDownList ddl, string keyId, string filtr)
		{
			DataTable dt = (DataTable)Session[KEY_dtCategoryGrp206];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (ddl != null)
			{
				string ss = string.Empty;
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(100,"GetDdlCategoryGrp3S1736",true,bAll,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					if (dv.Table.Columns.Contains("RptChaId"))
					{
						ss = ss + " AND (RptChaId is null";
						if (string.IsNullOrEmpty(cAdmRptCha100List.SelectedValue)) {ss = ss + ")";} else {ss = ss + " OR RptChaId = " + cAdmRptCha100List.SelectedValue + ")";}
					}
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "CategoryGrp206 is not NULL"; }
					dv.RowFilter = ss;
					ddl.DataSource = dv; ddl.DataBind();
					li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					else if (!bFirst)
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(100,"GetDdlCategoryGrp3S1736",true,true,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtCategoryGrp206] = dv.Table;
				}
			}
		}

		private void SetSeriesGrp206(DropDownList ddl, string keyId, string filtr)
		{
			DataTable dt = (DataTable)Session[KEY_dtSeriesGrp206];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (ddl != null)
			{
				string ss = string.Empty;
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(100,"GetDdlSeriesGrp3S1738",true,bAll,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					...
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "SeriesGrp206 is not NULL"; }
					dv.RowFilter = ss;
					ddl.DataSource = dv; ddl.DataBind();
					li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					else if (!bFirst)
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(100,"GetDdlSeriesGrp3S1738",true,true,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtSeriesGrp206] = dv.Table;
				}
			}
		}

		private DataView GetAdmRptCha100List()
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmRptCha100List];
			if (dt == null)
			{
				CheckAuthentication(false);
				int filterId = 0;
				...
				try
				{
					try {dt = (new AdminSystem()).GetLis(100,"GetLisAdmRptCha100",true,"Y",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
					catch {dt = (new AdminSystem()).GetLis(100,"GetLisAdmRptCha100",true,"N",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				}
		...
		private void PopAdmRptCha100List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			DataView dv = GetAdmRptCha100List();
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			context["isSys"] = "N";
			context["conn"] = KEY_sysConnectionString;
			cAdmRptCha100List.AutoCompleteUrl = "AutoComplete.aspx/LisSuggests";
			cAdmRptCha100List.DataContext = context;
			if (dv.Table == null) return;
			cAdmRptCha100List.DataSource = dv;
			cAdmRptCha100List.Visible = true;
			if (cAdmRptCha100List.Items.Count <= 0) {cAdmRptCha100List.Visible = false; cAdmRptCha100List_SelectedIndexChanged(sender,e);}
			else
			{
				if (bPageLoad && !string.IsNullOrEmpty(Request.QueryString["key"]) && bUseCri.Value == string.Empty)
				{
					try {cAdmRptCha100List.SelectByValue(Request.QueryString["key"],string.Empty,true);} catch {cAdmRptCha100List.Items[0].Selected = true; cAdmRptCha100List_SelectedIndexChanged(sender, e);}
				    cScreenSearch.Visible = false; cSystem.Visible = false; cEditButton.Visible = true; cSaveCloseButton.Visible = cSaveButton.Visible;
				}
				else
				{
					if (ID != null) {if (!cAdmRptCha100List.SelectByValue(ID,string.Empty,true)) {ID = null;}}
					if (ID == null)
					{
						cAdmRptCha100List_SelectedIndexChanged(sender, e);
					}
				}
			}
		}
		...
		private void SetColumnAuthority()
		{
			...
			{
				base.SetFoldBehavior(cRptChaId206, dtAuth.Rows[0], cRptChaId206P1, cRptChaId206Label, cRptChaId206P2, null, dtLabel.Rows[0], null, null, null);
				base.SetFoldBehavior(cRptCtrId206, dtAuth.Rows[1], cRptCtrId206P1, cRptCtrId206Label, cRptCtrId206P2, null, dtLabel.Rows[1], cRFVRptCtrId206, null, null);
				SetRptCtrId206(cRptCtrId206,string.Empty);
				base.SetFoldBehavior(cReportId206, dtAuth.Rows[2], cReportId206P1, cReportId206Label, cReportId206P2, null, dtLabel.Rows[2], cRFVReportId206, null, null);
				SetReportId206(cReportId206,string.Empty);
				base.SetFoldBehavior(cRptChaTypeCd206, dtAuth.Rows[3], cRptChaTypeCd206P1, cRptChaTypeCd206Label, cRptChaTypeCd206P2, null, dtLabel.Rows[3], cRFVRptChaTypeCd206, null, null);
				base.SetFoldBehavior(cThreeD206, dtAuth.Rows[4], cThreeD206P1, cThreeD206Label, cThreeD206P2, null, dtLabel.Rows[4], null, null, null);
				base.SetFoldBehavior(cCategoryGrp206, dtAuth.Rows[5], cCategoryGrp206P1, cCategoryGrp206Label, cCategoryGrp206P2, null, dtLabel.Rows[5], cRFVCategoryGrp206, null, null);
				base.SetFoldBehavior(cChartData206, dtAuth.Rows[6], cChartData206P1, cChartData206Label, cChartData206P2, null, dtLabel.Rows[6], cRFVChartData206, null, null);
				base.SetFoldBehavior(cSeriesGrp206, dtAuth.Rows[7], cSeriesGrp206P1, cSeriesGrp206Label, cSeriesGrp206P2, null, dtLabel.Rows[7], null, null, null);
			}
			if ((cRptCtrId206.Attributes["OnChange"] == null || cRptCtrId206.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cRptCtrId206.Visible && cRptCtrId206.Enabled) {cRptCtrId206.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cReportId206.Attributes["OnChange"] == null || cReportId206.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cReportId206.Visible && cReportId206.Enabled) {cReportId206.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();document.getElementById('" + bConfirm.ClientID + "').value='N';";}
			if ((cRptChaTypeCd206.Attributes["OnChange"] == null || cRptChaTypeCd206.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cRptChaTypeCd206.Visible && cRptChaTypeCd206.Enabled) {cRptChaTypeCd206.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cThreeD206.Attributes["OnClick"] == null || cThreeD206.Attributes["OnClick"].IndexOf("ChkPgDirty") < 0) && cThreeD206.Visible && cThreeD206.Enabled) {cThreeD206.Attributes["OnClick"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty(); this.focus();";}
			if ((cCategoryGrp206.Attributes["OnChange"] == null || cCategoryGrp206.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cCategoryGrp206.Visible && cCategoryGrp206.Enabled) {cCategoryGrp206.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cChartData206.Attributes["OnChange"] == null || cChartData206.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cChartData206.Visible && !cChartData206.ReadOnly) {cChartData206.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cSeriesGrp206.Attributes["OnChange"] == null || cSeriesGrp206.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cSeriesGrp206.Visible && cSeriesGrp206.Enabled) {cSeriesGrp206.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
		}
		...
		private void ClearMaster(object sender, System.EventArgs e)
		{
			...
			if (dt.Rows[1]["ColVisible"].ToString() == "Y" && dt.Rows[1]["ColReadOnly"].ToString() != "Y") {cRptCtrId206.ClearSearch();}
			if (dt.Rows[2]["ColVisible"].ToString() == "Y" && dt.Rows[2]["ColReadOnly"].ToString() != "Y") {cReportId206.ClearSearch();}
			if (dt.Rows[3]["ColVisible"].ToString() == "Y" && dt.Rows[3]["ColReadOnly"].ToString() != "Y") {SetRptChaTypeCd206(cRptChaTypeCd206,string.Empty);}
			if (dt.Rows[4]["ColVisible"].ToString() == "Y" && dt.Rows[4]["ColReadOnly"].ToString() != "Y") {cThreeD206.Checked = base.GetBool("N");}
			if (dt.Rows[5]["ColVisible"].ToString() == "Y" && dt.Rows[5]["ColReadOnly"].ToString() != "Y") {SetCategoryGrp206(cCategoryGrp206,string.Empty,cReportId206.SelectedValue);}
			if (dt.Rows[6]["ColVisible"].ToString() == "Y" && dt.Rows[6]["ColReadOnly"].ToString() != "Y") {cChartData206.Text = string.Empty;}
			if (dt.Rows[7]["ColVisible"].ToString() == "Y" && dt.Rows[7]["ColReadOnly"].ToString() != "Y") {SetSeriesGrp206(cSeriesGrp206,string.Empty,cReportId206.SelectedValue);}
			// *** Default Value (Folder) Web Rule starts here *** //
		}

		private void InitMaster(object sender, System.EventArgs e)
		{
			cRptChaId206.Text = string.Empty;
			cRptCtrId206.ClearSearch();
			cReportId206.ClearSearch();
			SetRptChaTypeCd206(cRptChaTypeCd206,string.Empty);
			cThreeD206.Checked = base.GetBool("N");
			SetCategoryGrp206(cCategoryGrp206,string.Empty,cReportId206.SelectedValue);
			cChartData206.Text = string.Empty;
			SetSeriesGrp206(cSeriesGrp206,string.Empty,cReportId206.SelectedValue);
			// *** Default Value (Folder) Web Rule starts here *** //
		}
		protected void cAdmRptCha100List_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmRptCha100List_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmRptCha100List_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			InitMaster(sender, e);      // Do this first to enable defaults for non-database columns.
			DataTable dt = null;
			try
			{
				dt = (new AdminSystem()).GetMstById("GetAdmRptCha100ById",cAdmRptCha100List.SelectedValue,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId));
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				if (dt.Rows.Count > 0)
				{
					cAdmRptCha100List.SetDdVisible();
					DataRow dr = dt.Rows[0];
					try {cRptChaId206.Text = RO.Common3.Utils.fmNumeric("0",dr["RptChaId206"].ToString(),base.LUser.Culture);} catch {cRptChaId206.Text = string.Empty;}
					SetRptCtrId206(cRptCtrId206,dr["RptCtrId206"].ToString());
					SetReportId206(cReportId206,dr["ReportId206"].ToString()); cReportId206_SelectedIndexChanged(cReportId206, new EventArgs());
					SetRptChaTypeCd206(cRptChaTypeCd206,dr["RptChaTypeCd206"].ToString());
					try {cThreeD206.Checked = base.GetBool(dr["ThreeD206"].ToString());} catch {cThreeD206.Checked = false;}
					SetCategoryGrp206(cCategoryGrp206,dr["CategoryGrp206"].ToString(),cReportId206.SelectedValue);
					try {cChartData206.Text = dr["ChartData206"].ToString();} catch {cChartData206.Text = string.Empty;}
					SetSeriesGrp206(cSeriesGrp206,dr["SeriesGrp206"].ToString(),cReportId206.SelectedValue);
				}
			}
			cButPanel.DataBind();
			ScriptManager.GetCurrent(Parent.Page).SetFocus(cAdmRptCha100List.FocusID);
			ShowDirty(false); PanelTop.Update();
			// *** List Selection (End of) Web Rule starts here *** //
		}

		protected void cReportId206_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cReportId206_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cReportId206_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			...
			TextBox pwd = null;
			if (cReportId206.Items.Count > 0 && cReportId206.DataSource != null)
			{
				DataView dv = (DataView) cReportId206.DataSource; dv.RowFilter = string.Empty;
				DataRowView dr = cReportId206.DataSetIndex >= 0 && cReportId206.DataSetIndex < dv.Count ? dv[cReportId206.DataSetIndex] : dv[0];
				SetCategoryGrp206(cCategoryGrp206,cCategoryGrp206.SelectedValue,((RoboCoder.WebControls.ComboBox)sender).SelectedValue);
				SetSeriesGrp206(cSeriesGrp206,cSeriesGrp206.SelectedValue,((RoboCoder.WebControls.ComboBox)sender).SelectedValue);
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cRptChaId206.Text == string.Empty) { cClearButton_Click(sender, new EventArgs()); ShowDirty(false); PanelTop.Update();}
			else { PopAdmRptCha100List(sender, e, false, cRptChaId206.Text); }
			// *** System Button Click (After) Web Rule starts here *** //
		}
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmRptCha100List); PopAdmRptCha100List(sender, e, false, null);
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			if (ValidPage())
			{
				AdmRptCha100 ds = PrepAdmRptChaData(null,cRptChaId206.Text == string.Empty);
				if (!string.IsNullOrEmpty(cAdmRptCha100List.SelectedValue) && cRptChaId206.Text != cAdmRptCha100List.SelectedValue)
				{
					PreMsgPopup("Primary key cannot be changed");
					return rtn;
				}
				if (string.IsNullOrEmpty(cAdmRptCha100List.SelectedValue))	// Add
				{
					if (ds != null)
					{
						pid = (new AdminSystem()).AddData(100,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc,noTrans,commandTimeOut);
					}
					if (!string.IsNullOrEmpty(pid))
					{
						cAdmRptCha100List.ClearSearch(); Session.Remove(KEY_dtAdmRptCha100List);
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmRptCha100List(sender, e, false, pid);
						rtn = GetScreenHlp().Rows[0]["AddMsg"].ToString();
					}
				}
				else {
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(100,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc,noTrans,commandTimeOut)) {bValid7 = true;}
					if (bValid7)
					{
						cAdmRptCha100List.ClearSearch(); Session.Remove(KEY_dtAdmRptCha100List);
						ShowDirty(false); PopAdmRptCha100List(sender, e, false, ds.Tables["AdmRptCha"].Rows[0]["RptChaId206"]);
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cRptChaId206.Text != string.Empty)
			{
				AdmRptCha100 ds = PrepAdmRptChaData(null,false);
				try
				{
					if (ds != null && (new AdminSystem()).DelData(100,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc))
					{
						cAdmRptCha100List.ClearSearch(); Session.Remove(KEY_dtAdmRptCha100List);
						ShowDirty(false); PopAdmRptCha100List(sender, e, false, null);
		...
		private AdmRptCha100 PrepAdmRptChaData(DataView dv, bool bAdd)
		{
			...
			if (bAdd) { dr["RptChaId206"] = string.Empty; } else { dr["RptChaId206"] = cRptChaId206.Text; }
			drType["RptChaId206"] = "Numeric"; drDisp["RptChaId206"] = "TextBox";
			try {dr["RptCtrId206"] = cRptCtrId206.SelectedValue;} catch {}
			drType["RptCtrId206"] = "Numeric"; drDisp["RptCtrId206"] = "AutoComplete";
			try {dr["ReportId206"] = cReportId206.SelectedValue;} catch {}
			drType["ReportId206"] = "Numeric"; drDisp["ReportId206"] = "AutoComplete";
			try {dr["RptChaTypeCd206"] = cRptChaTypeCd206.SelectedValue;} catch {}
			drType["RptChaTypeCd206"] = "Char"; drDisp["RptChaTypeCd206"] = "DropDownList";
			try {dr["ThreeD206"] = base.SetBool(cThreeD206.Checked);} catch {}
			drType["ThreeD206"] = "Char"; drDisp["ThreeD206"] = "CheckBox";
			try {dr["CategoryGrp206"] = cCategoryGrp206.SelectedValue;} catch {}
			drType["CategoryGrp206"] = "Numeric"; drDisp["CategoryGrp206"] = "DropDownList";
			try {dr["ChartData206"] = cChartData206.Text.Trim();} catch {}
			drType["ChartData206"] = "VarWChar"; drDisp["ChartData206"] = "TextBox";
			try {dr["SeriesGrp206"] = cSeriesGrp206.SelectedValue;} catch {}
		...
		public bool CanAct(char typ) { return CanAct(typ, GetAuthRow(), cAdmRptCha100List.SelectedValue.ToString()); }
		...
		private bool ValidPage()
		{
			...
			DataTable dt = null;
			dt = (DataTable)Session[KEY_dtRptCtrId206];
			if (dt != null && dt.Rows.Count <= 0)
			{
				bErrNow.Value = "Y"; PreMsgPopup("Value is expected for 'RptCtrId', please investigate."); return false;
			}
			dt = (DataTable)Session[KEY_dtReportId206];
			if (dt != null && dt.Rows.Count <= 0)
			{
				bErrNow.Value = "Y"; PreMsgPopup("Value is expected for 'ReportId', please investigate."); return false;
			}
			dt = (DataTable)Session[KEY_dtRptChaTypeCd206];
			if (dt != null && dt.Rows.Count <= 0)
			{
				bErrNow.Value = "Y"; PreMsgPopup("Value is expected for 'RptChaTypeCd', please investigate."); return false;
			}
			dt = (DataTable)Session[KEY_dtCategoryGrp206];
			if (dt != null && dt.Rows.Count <= 0)
			{
				bErrNow.Value = "Y"; PreMsgPopup("Value is expected for 'CategoryGrp', please investigate."); return false;
			}
			return true;
		}
		...
		private void PreMsgPopup(string msg) { PreMsgPopup(msg,cAdmRptCha100List,null); }

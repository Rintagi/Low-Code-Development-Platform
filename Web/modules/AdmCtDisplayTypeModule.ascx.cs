...
{
	public class AdmCtDisplayType32 : DataSet
	{
		public AdmCtDisplayType32()
		{
			this.Tables.Add(MakeDtlColumns(new DataTable("AdmCtDisplayTypeDef")));
			this.Tables.Add(MakeDtlColumns(new DataTable("AdmCtDisplayTypeAdd")));
			this.Tables.Add(MakeDtlColumns(new DataTable("AdmCtDisplayTypeUpd")));
			this.Tables.Add(MakeDtlColumns(new DataTable("AdmCtDisplayTypeDel")));
			this.DataSetName = "AdmCtDisplayType32";
			this.Namespace = "http://Rintagi.com/DataSet/AdmCtDisplayType32";
		}

		private DataTable MakeDtlColumns(DataTable dt)
		{
			DataColumnCollection columns = dt.Columns;
			columns.Add("TypeId18", typeof(string));
			columns.Add("TypeName18", typeof(string));
			columns.Add("TypeDesc18", typeof(string));
			columns.Add("DisplayDefault18", typeof(string));
			return dt;
		}
	}
}
...
namespace RO.Web
{
	public partial class AdmCtDisplayTypeModule : RO.Web.ModuleBase
	{

		private const string KEY_lastAddedRow = "Cache:lastAddedRow3_32";
		private const string KEY_lastSortOrd = "Cache:lastSortOrd3_32";
		private const string KEY_lastSortImg = "Cache:lastSortImg3_32";
		private const string KEY_lastSortCol = "Cache:lastSortCol3_32";
		private const string KEY_lastSortExp = "Cache:lastSortExp3_32";
		private const string KEY_lastSortUrl = "Cache:lastSortUrl3_32";
		private const string KEY_lastSortTog = "Cache:lastSortTog3_32";
		private const string KEY_lastImpPwdOvride = "Cache:lastImpPwdOvride3_32";
		private const string KEY_cntImpPwdOvride = "Cache:cntImpPwdOvride3_32";
		private const string KEY_currPageIndex = "Cache:currPageIndex3_32";
		private const string KEY_bHiImpVisible = "Cache:bHiImpVisible3_32";
		private const string KEY_bShImpVisible = "Cache:bShImpVisible3_32";
		private const string KEY_dtAdmCtDisplayTypeGrid = "Cache:dtAdmCtDisplayTypeGrid";
		private const string KEY_scrImport = "Cache:scrImport3_32";
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_32";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_32";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_32";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_32";
		private const string KEY_dtLabel = "Cache:dtLabel3_32";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_32";
		private const string KEY_dtScrCri = "Cache:dtScrCri3_32";
		private const string KEY_dsScrCriVal = "Cache:dsScrCriVal3_32";


		private const string KEY_dtSystems = "Cache:dtSystems3_32";
		private const string KEY_sysConnectionString = "Cache:sysConnectionString3_32";
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_32";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_32";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_32";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				DataTable dtMenuAccess = (new MenuSystem()).GetMenu(base.LUser.CultureId, 3, base.LImpr, LcSysConnString, LcAppPw,32, null, null);
				...
				Session.Remove(KEY_dtAdmCtDisplayTypeGrid);
				...
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 32, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmCtDisplayType32List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=32&typ=N&sys=3','','',''); return false;"; }
			...
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmCtDisplayTypeGrid];
				if (dt != null)
				{
		...
		private void InitializeComponent()
		{
			...
			if (LcSysConnString == null)
			{
				...
				{
					DataTable dt;
					dt = (new AdminSystem()).GetAuthRow(32,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
					...
					{
						cAdmCtDisplayTypeGrid.PreRender += new EventHandler(cAdmCtDisplayTypeGrid_OnPreRender);
					}
				}
		...
		private void SetSystem(byte SystemId)
		{
			...
			try
			{
				...
				if ((Config.DeployType == "DEV" || row["dbAppDatabase"].ToString() == base.CPrj.EntityCode + "View") && !(base.CPrj.EntityCode != "RO" && row["SysProgram"].ToString() == "Y") && (new AdminSystem()).IsRegenNeeded(string.Empty,32,0,0,LcSysConnString,LcAppPw))
				{
					(new GenScreensSystem()).CreateProgram(32, "Display Type Code ( -- no react gen -- )", row["dbAppDatabase"].ToString(), base.CPrj, base.CSrc, base.CTar, LcAppConnString, LcAppPw);
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(32,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(32,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void SetClientRule(ListViewDataItem ee, bool isEditItem)
		{
			...
			if (dvRul.Count > 0)
			{
				...
				{
					if (ee == null || drv["ScriptEvent"].ToString().Substring(0,2).ToLower() != "on" || (cAdmCtDisplayTypeGrid.EditIndex > -1 && GetDataItemIndex(cAdmCtDisplayTypeGrid.EditIndex) == ee.DataItemIndex))
					{
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(32,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(32,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,32,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(32,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(32,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(32,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				CheckAuthentication(false);
				DataView dv = null;
				try {dv = ((DataTable)Session[KEY_dtAdmCtDisplayTypeGrid]).DefaultView;}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(32,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					{
						if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["TypeId18"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["TypeName18"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["TypeDesc18"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["DisplayDefault18"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						sb.Append(Environment.NewLine);
					}
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmCtDisplayType.xls"; Session["ExportStr"] = sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmCtDisplayType.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private string GetRtfGrid(DataView dv)
		{
			...
			try
			{
				...
				foreach (DataRowView drv in dv)
				{
					...
					sb.Append("\r\n");
					sb.Append(@"{");
					if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["TypeId18"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["TypeName18"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(drv["TypeDesc18"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(drv["DisplayDefault18"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					sb.Append(@"}");
					sb.Append("\r\n");
					sb.Append(@"\pard \ql \li0\ri0\widctlpar\intbl\aspalpha\aspnum\adjustright\rin0\lin0 {");
					sb.Append(@"\insrsid2981395 \trowd \irow0\irowband0\lastrow \ts15\trgaph108\trleft-108\trbrdrt\brdrs\brdrw10 \trbrdrl\brdrs\brdrw10 \trbrdrb\brdrs\brdrw10 \trbrdrr\brdrs\brdrw10 \trbrdrh\brdrs\brdrw10 \trbrdrv\brdrs\brdrw10 ");
		...
		private void GetPageInfo()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastPageInfo(32, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 32, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			// *** Criteria Trigger (before) Web Rule starts here *** //
			DataTable dt = (DataTable)Session[KEY_dtAdmCtDisplayTypeGrid];
			if (cAdmCtDisplayTypeGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
			if (IsPostBack && cCriteria.Visible) ReBindCriteria(GetScrCriteria());
			UpdCriteria(true);
			PopAdmCtDisplayType32List(sender, e, false, null);
			}
		}
			...
			try
			{
				foreach (DataRowView drv in dvCri)
				{
					...
					if (drv["DisplayName"].ToString() == "ComboBox")
					{
						...
						(new AdminSystem()).MkGetScreenIn("32", drv["ScreenCriId"].ToString(), "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), LcAppDb, LcDesDb, drv["MultiDesignDb"].ToString(), LcSysConnString, LcAppPw);
						DataView dv = new DataView((new AdminSystem()).GetScreenIn("32", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), (new AdminSystem()).CountScrCri(drv["ScreenCriId"].ToString(), drv["MultiDesignDb"].ToString(), drv["MultiDesignDb"].ToString() == "N" ? LcSysConnString : (string)Session[KEY_sysConnectionString], LcAppPw), drv["RequiredValid"].ToString(), 0, string.Empty, drv["DisplayMode"].ToString() != "AutoListBox", val, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], LcAppPw));
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("32", LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmCtDisplayType32" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmCtDisplayType32" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			foreach (DataRowView drv in dvCri)
			{
			    if (drv["DisplayName"].ToString() == "ListBox")
			    {
					...
					if (cListBox != null)
					{
						...
						int TotalChoiceCnt = new DataView((new AdminSystem()).GetScreenIn("32", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), CriCnt, drv["RequiredValid"].ToString(), 0, string.Empty, true, string.Empty, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], LcAppPw)).Count;
						string selectedValues = string.Join(",", cListBox.Items.Cast<ListItem>().Where(x => x.Selected).Select(x => "'" + x.Value + "'").ToArray());
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("32", "AdmCtDisplayType32", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		private void PopAdmCtDisplayType32List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			DataTable dt = null;
			CheckAuthentication(false);
			...
			try
			{
				try {dt = (new AdminSystem()).GetLis(32,"GetLisAdmCtDisplayType32",false,"Y",0,null,null,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				catch {dt = (new AdminSystem()).GetLis(32,"GetLisAdmCtDisplayType32",false,"N",0,null,null,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				Session[KEY_dtAdmCtDisplayTypeGrid] = dt;
				cAdmCtDisplayTypeGridDataPager.PageSize = Int16.Parse(cPgSize.Text); GotoPage(0);
			    if (Session[KEY_lastSortCol] != null)
			    {
					cAdmCtDisplayTypeGrid_OnSorting(sender, new ListViewSortEventArgs((string)Session[KEY_lastSortExp], SortDirection.Ascending));
			    }
				else
			    {
					cAdmCtDisplayTypeGrid_DataBind(dt.DefaultView);
			    }
		...
		public void cFindButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dv != null)
			{
				if (cAdmCtDisplayTypeGrid.EditIndex < 0 || UpdateGridRow(cAdmCtDisplayTypeGrid, new CommandEventArgs("Save", "")))
				{
					...
					GotoPage(0); cAdmCtDisplayTypeGrid_DataBind(dv);
					if (GetCurrPageIndex() != (int)Session[KEY_currPageIndex] && (int)Session[KEY_currPageIndex] < GetTotalPages()) { GotoPage((int)Session[KEY_currPageIndex]); cAdmCtDisplayTypeGrid_DataBind(dv);}
		...
		public void cInsRowButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dt != null && (ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")))
			{
				...
				Session[KEY_lastAddedRow] = 0; Session[KEY_dtAdmCtDisplayTypeGrid] = dt; Session[KEY_currPageIndex] = 0; GotoPage(0);
				cAdmCtDisplayTypeGrid_DataBind(dt.DefaultView);
				cAdmCtDisplayTypeGrid_OnItemEditing(cAdmCtDisplayTypeGrid, new ListViewEditEventArgs(0));
		...
		public void cPgSizeButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cAdmCtDisplayTypeGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				...
				cAdmCtDisplayTypeGridDataPager.PageSize = int.Parse(cPgSize.Text); GotoPage(0);
				if (dt.Rows.Count <= 0 || (GetAuthRow().Rows[0]["AllowAdd"].ToString() == "Y" && dt.Rows.Count == 1)) {cAdmCtDisplayTypeGrid_DataBind(dt.DefaultView);} else {cFindButton_Click(sender, e);}
				try
				{
					(new AdminSystem()).UpdLastPageInfo(32, base.LUser.UsrId, cPgSize.Text, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		}
		...
		public void cShowImpButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cSchemaImage.Visible)
			{
				try
				{
					Session["ImportSchema"] = (new AdminSystem()).GetSchemaScrImp(32,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (cSchemaImage.Attributes["OnClick"] == null || cSchemaImage.Attributes["OnClick"].IndexOf("ImportSchema.aspx") < 0) {cSchemaImage.Attributes["OnClick"] += "SearchLink('ImportSchema.aspx?scm=S&key=32&csy=3','','',''); return false;";}
			}
			// *** System Button Click (after) Web Rule starts here *** //
		}
		...
		protected void ScrImport(bool bClear)
		{
			...
			DataTable dtAdmCtDisplayTypeGrid = (DataTable)Session[KEY_dtAdmCtDisplayTypeGrid];
			if (dti != null && dtAdmCtDisplayTypeGrid != null)
			{
				if (bClear)
				{
					dtAdmCtDisplayTypeGrid.Rows.Clear();
				}
				cFind.Text = string.Empty;
				int iExist = dtAdmCtDisplayTypeGrid.Rows.Count;
				iRow = dti.DefaultView.Count - iExist;
				if (!dti.Columns.Contains("_NewRow")) { dti.Columns.Add("_NewRow"); }
				Session[KEY_dtAdmCtDisplayTypeGrid] = dti;
				cAdmCtDisplayTypeGrid_DataBind(dti.DefaultView);
		...
		public void cImportButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cFNameO.Text != string.Empty && cWorkSheet.Items.Count > 0 && cWorkSheet.SelectedItem.Text != string.Empty && cStartRow.Text != string.Empty)
			{
				...
				try
				{
					if (cAdmCtDisplayTypeGrid.EditIndex < 0 || UpdateGridRow(cAdmCtDisplayTypeGrid, new CommandEventArgs("Save", "")))
					{
						Session[KEY_scrImport] = ImportFile(dtAdmCtDisplayTypeGrid.Copy(),cFNameO.Text,cWorkSheet.SelectedItem.Text,cStartRow.Text,Config.PathTmpImport + cFName.Text);
					}
		...
		private DataTable ImportFile(DataTable dt, string fileName, string workSheet, string startRow, string fileFullName)
		{
			try
			{
				...
				{
					if (rows[iRow][0].ToString() == string.Empty && rows[iRow][1].ToString() == string.Empty && rows[iRow][2].ToString() == string.Empty && rows[iRow][3].ToString() == string.Empty) {idel = idel + 1;}
					else
					{
						dt.Rows.Add(dt.NewRow());
						if (rows[iRow][3].ToString() == string.Empty) { rows[iRow][3] = "N";}
		...
		private void GotoPage(int pageNo)
		{
			if (pageNo >= 0 && pageNo < GetTotalPages())
			{
				if (cAdmCtDisplayTypeGrid.EditIndex < 0 || UpdateGridRow(cAdmCtDisplayTypeGrid, new CommandEventArgs("Save", "")))
				{
					try { cAdmCtDisplayTypeGridDataPager.SetPageProperties(cAdmCtDisplayTypeGridDataPager.PageSize * pageNo, cAdmCtDisplayTypeGridDataPager.MaximumRows, false); cAdmCtDisplayTypeGrid_DataBind(null); }
					catch
					{
						try { cAdmCtDisplayTypeGridDataPager.SetPageProperties(0, cAdmCtDisplayTypeGridDataPager.MaximumRows, false); cAdmCtDisplayTypeGrid_DataBind(null); } catch {}
					}
				}
			}
		}

		protected void cAdmCtDisplayTypeGrid_OnItemCommand(object sender, ListViewCommandEventArgs e)
		{
		}

		private void cAdmCtDisplayTypeGrid_DataBind(DataView dvAdmCtDisplayTypeGrid)
		{
			if (dvAdmCtDisplayTypeGrid == null)
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmCtDisplayTypeGrid];
				dvAdmCtDisplayTypeGrid = dt != null ? dt.DefaultView : null;
			}
			if (dvAdmCtDisplayTypeGrid != null)
			{
				LcAuth = GetAuthCol().AsEnumerable().ToDictionary<DataRow,string>(dr=>dr["ColName"].ToString());
				cAdmCtDisplayTypeGrid.DataSource = dvAdmCtDisplayTypeGrid;
				cAdmCtDisplayTypeGrid.DataBind();
		...
		protected void cAdmCtDisplayTypeGrid_OnSorting(object sender, ListViewSortEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmCtDisplayTypeGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null)
			{
				if (cAdmCtDisplayTypeGrid.EditIndex < 0 || UpdateGridRow(cAdmCtDisplayTypeGrid, new CommandEventArgs("Save", "")))
				{
					...
					Session[KEY_dtAdmCtDisplayTypeGrid] = dt;
					cAdmCtDisplayTypeGrid_DataBind(dv);
				}
			}
		}

		protected void cAdmCtDisplayTypeGrid_OnPagePropertiesChanging(object sender, PagePropertiesChangingEventArgs e)
		{
		    cAdmCtDisplayTypeGridDataPager.SetPageProperties(e.StartRowIndex, e.MaximumRows, false);
		    DataTable dt = (DataTable)Session[KEY_dtAdmCtDisplayTypeGrid];
		    DataView dv = dt != null ? dt.DefaultView : null;
			cAdmCtDisplayTypeGrid_DataBind(dv); cAdmCtDisplayTypeGrid.EditIndex = -1;
		}

		private void GridChkPgDirty(ListViewItem lvi)
		{
				WebControl cc = null;
				cc = ((WebControl)lvi.FindControl("cTypeId18"));
				...
				cc = ((WebControl)lvi.FindControl("cTypeName18"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cTypeDesc18"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cDisplayDefault18"));
		...
		protected void cAdmCtDisplayTypeGrid_OnItemDataBound(object sender, ListViewItemEventArgs e)
		{
			...
			DataView dvAdmCtDisplayTypeGrid = dt != null ? dt.DefaultView : null;
			if (cAdmCtDisplayTypeGrid.EditIndex > -1 && GetDataItemIndex(cAdmCtDisplayTypeGrid.EditIndex) == e.Item.DataItemIndex)
			{
				isEditItem = true;
				base.SetGridEnabled(e.Item, GetAuthCol(), GetLabel(), 0);
				CheckBox cb = null;
				cb = (CheckBox)e.Item.FindControl("cDisplayDefault18");
				if (cb != null)
				{
					cb.Checked = base.GetBool(dvAdmCtDisplayTypeGrid[e.Item.DataItemIndex]["DisplayDefault18"].ToString());
				}
			...
			if (dtAuthRow != null)
			{
				...
				{
					cc = e.Item.FindControl("cAdmCtDisplayTypeGridDelete"); if (cc != null) { cc.Visible = false; }
				}
				if (!AllowRowEdit(GetAuthRow(),dvAdmCtDisplayTypeGrid[e.Item.DataItemIndex]["TypeId18"].ToString()) || (Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N"))
				{
					cc = e.Item.FindControl("cAdmCtDisplayTypeGridEdit"); if (cc != null) { cc.Visible = false; }
				}
				else
				{
			        HtmlTableRow tr = e.Item.FindControl("cAdmCtDisplayTypeGridRow") as HtmlTableRow;
			        LinkButton lb = e.Item.FindControl("cAdmCtDisplayTypeGridEdit") as LinkButton;
		...
		protected void cTypeId18hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "0") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "0"; Session[KEY_lastSortExp] = "TypeId18";Session[KEY_lastSortImg] = "cTypeId18hi";
			cAdmCtDisplayTypeGrid_OnSorting(sender, new ListViewSortEventArgs("TypeId18", SortDirection.Ascending));
		}

		protected void cTypeName18hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "1") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "1"; Session[KEY_lastSortExp] = "TypeName18";Session[KEY_lastSortImg] = "cTypeName18hi";
			cAdmCtDisplayTypeGrid_OnSorting(sender, new ListViewSortEventArgs("TypeName18", SortDirection.Ascending));
		}

		protected void cTypeDesc18hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "2") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "2"; Session[KEY_lastSortExp] = "TypeDesc18";Session[KEY_lastSortImg] = "cTypeDesc18hi";
			cAdmCtDisplayTypeGrid_OnSorting(sender, new ListViewSortEventArgs("TypeDesc18", SortDirection.Ascending));
		}

		protected void cDisplayDefault18hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "3") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "3"; Session[KEY_lastSortExp] = "DisplayDefault18";Session[KEY_lastSortImg] = "cDisplayDefault18hi";
			cAdmCtDisplayTypeGrid_OnSorting(sender, new ListViewSortEventArgs("DisplayDefault18", SortDirection.Ascending));
		}

		protected void cAdmCtDisplayTypeGrid_OnItemEditing(object sender, ListViewEditEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmCtDisplayTypeGrid];
			if ((ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")) && dt != null && dt.DefaultView.Count > GetDataItemIndex(e.NewEditIndex))
			{
				cAdmCtDisplayTypeGrid.EditIndex = e.NewEditIndex;
				cAdmCtDisplayTypeGrid_DataBind(null);
				Control wc = null;
				string idx = Page.Request["__EVENTARGUMENT"];
				if (!string.IsNullOrEmpty(idx))
				{
				    Control ctrlToFocus = cAdmCtDisplayTypeGrid.EditItem.FindControl("c" + idx);
				    if (((WebControl)ctrlToFocus).Enabled) { wc = ctrlToFocus; }
				}
				if (wc == null) { wc = FindEditableControl(cAdmCtDisplayTypeGrid.EditItem); }
				if (wc != null)
             {
             ...
             MkMobileLabel(cAdmCtDisplayTypeGrid.EditItem);
		    }
		    else { e.Cancel = true; }
		}
		...
		{
		    Label ml = null;
		    ml = lvi.FindControl("cTypeId18ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(0); }
		    ml = lvi.FindControl("cTypeName18ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(1); }
		    ml = lvi.FindControl("cTypeDesc18ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(2); }
		    ml = lvi.FindControl("cDisplayDefault18ml") as Label;
		...
		{
			TextBox tb = null;
			tb = (TextBox)lvi.FindControl("cTypeId18");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["TypeId18"] = tb.Text;} else {dr["TypeId18"] = System.DBNull.Value;}
			}
			tb = (TextBox)lvi.FindControl("cTypeName18");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["TypeName18"] = tb.Text;} else {dr["TypeName18"] = System.DBNull.Value;}
			}
			tb = (TextBox)lvi.FindControl("cTypeDesc18");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["TypeDesc18"] = tb.Text;} else {dr["TypeDesc18"] = System.DBNull.Value;}
			}
			CheckBox cb = null;
			cb = (CheckBox)lvi.FindControl("cDisplayDefault18");
			if (cb != null)
			{
				dr["DisplayDefault18"] = base.SetBool(cb.Checked);
			}
		    if (bInsert) { dt.Rows.InsertAt(dr, 0); }
			Session[KEY_dtAdmCtDisplayTypeGrid] = dt;
			cAdmCtDisplayTypeGrid.EditIndex = -1;
			cAdmCtDisplayTypeGrid_DataBind(dt.DefaultView);
		}

		protected void cAdmCtDisplayTypeGrid_OnItemUpdating(object sender, ListViewUpdateEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmCtDisplayTypeGrid];
			if (dt != null && dt.Rows.Count > 0)
			{
				DataRow dr = dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row;
			    GridFill(cAdmCtDisplayTypeGrid.EditItem, dt, dr, false);
			    dr["_NewRow"] = "N";
			    if (dt.Columns.Contains("_SortOrder")) dr["_SortOrder"] = null;
			}
		}

		protected void cAdmCtDisplayTypeGrid_OnItemCanceling(object sender, ListViewCancelEventArgs e)
		{
			PanelTop.Update();
			DataTable dt = (DataTable)Session[KEY_dtAdmCtDisplayTypeGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			...
			{
				cAdmCtDisplayTypeGrid_OnItemDeleting(sender, new ListViewDeleteEventArgs(e.ItemIndex));
			}
			if (dt.Columns.Contains("_SortOrder")) dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row["_SortOrder"] = null;
			cAdmCtDisplayTypeGrid.EditIndex = -1; cAdmCtDisplayTypeGrid_DataBind(null);
		}
		...
		protected void cDeleteAllButton_Click(object sender, System.EventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmCtDisplayTypeGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null && (cAdmCtDisplayTypeGrid.EditIndex < 0 || UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				GotoPage(0);
				while (dv.Count > 0) {dv.Delete(0);}
				Session[KEY_dtAdmCtDisplayTypeGrid] = dt;
				cAdmCtDisplayTypeGrid.EditIndex = -1; cAdmCtDisplayTypeGrid_DataBind(dv);
			}
		}

		protected void cAdmCtDisplayTypeGrid_OnItemDeleting(object sender, ListViewDeleteEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmCtDisplayTypeGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			DataRow dr = dv != null ? dv[GetDataItemIndex(e.ItemIndex)].Row : null;
			if (dv != null && (cAdmCtDisplayTypeGrid.EditIndex == e.ItemIndex || UpdateGridRow(sender, new CommandEventArgs("Save",""))))
			{
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmCtDisplayTypeGrid); PopAdmCtDisplayType32List(sender, e, false, null);
			...
			if (ValidPage() && UpdateGridRow(cAdmCtDisplayTypeGrid, new CommandEventArgs("Save", "")))
			{
				...
				if (ftr != string.Empty) {dv.RowFilter = ftr;}
					...
					if (bValid7)
					{
						...
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmCtDisplayType32List(sender, e, false, null);
		...
		private AdmCtDisplayType32 PrepAdmCtDisplayTypeData(DataView dv, bool bAdd)
		{
			AdmCtDisplayType32 ds = new AdmCtDisplayType32();
			if (dv != null)
			{
				ds.Tables["AdmCtDisplayTypeDef"].Rows.Add(MakeTypRow(ds.Tables["AdmCtDisplayTypeDef"].NewRow()));
				ds.Tables["AdmCtDisplayTypeDef"].Rows.Add(MakeDisRow(ds.Tables["AdmCtDisplayTypeDef"].NewRow()));
					dv.RowStateFilter = DataViewRowState.ModifiedCurrent;
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmCtDisplayTypeUpd"].Rows.Add(MakeColRow(ds.Tables["AdmCtDisplayTypeUpd"].NewRow(), drv, false));
					}
					dv.RowStateFilter = DataViewRowState.Added;
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmCtDisplayTypeAdd"].Rows.Add(MakeColRow(ds.Tables["AdmCtDisplayTypeAdd"].NewRow(), drv, true));
					}
					dv.RowStateFilter = DataViewRowState.Deleted;
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmCtDisplayTypeDel"].Rows.Add(MakeColRow(ds.Tables["AdmCtDisplayTypeDel"].NewRow(), drv, false));
					}
		...
		{
			dr["TypeId18"] = System.Data.OleDb.OleDbType.Numeric.ToString();
			dr["TypeName18"] = System.Data.OleDb.OleDbType.VarChar.ToString();
			dr["TypeDesc18"] = System.Data.OleDb.OleDbType.VarChar.ToString();
			dr["DisplayDefault18"] = System.Data.OleDb.OleDbType.Char.ToString();
			return dr;
		}
		...
		{
			dr["TypeId18"] = "TextBox";
			dr["TypeName18"] = "TextBox";
			dr["TypeDesc18"] = "TextBox";
			dr["DisplayDefault18"] = "CheckBox";
			return dr;
		}
		...
		private DataRow MakeColRow(DataRow dr, DataRowView drv, bool bAdd)
		{
			...
			if (dtAuth != null)
			{
				dr["TypeId18"] = drv["TypeId18"].ToString().Trim();
				dr["TypeName18"] = drv["TypeName18"].ToString().Trim();
				dr["TypeDesc18"] = drv["TypeDesc18"].ToString().Trim();
				dr["DisplayDefault18"] = drv["DisplayDefault18"];
			}
			return dr;
		}
		...
		{
			if (cAdmCtDisplayTypeGrid.EditIndex > -1)
			{
				TextBox pwd = null;				cAdmCtDisplayTypeGrid_OnItemUpdating(sender, new ListViewUpdateEventArgs(cAdmCtDisplayTypeGrid.EditIndex));
			}
			return true;
		}

		protected void cAdmCtDisplayTypeGrid_OnPreRender(object sender, System.EventArgs e)
		{
			System.Web.UI.WebControls.Image hi = null;
			hi = (System.Web.UI.WebControls.Image)cAdmCtDisplayTypeGrid.FindControl("cTypeId18hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmCtDisplayTypeGrid.FindControl("cTypeName18hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmCtDisplayTypeGrid.FindControl("cTypeDesc18hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmCtDisplayTypeGrid.FindControl("cDisplayDefault18hi"); if (hi != null) { hi.Visible = false; }
			if (Session[KEY_lastSortImg] != null)
			{
				hi = (System.Web.UI.WebControls.Image)cAdmCtDisplayTypeGrid.FindControl((string)Session[KEY_lastSortImg]);
				if (hi != null) { hi.ImageUrl = Utils.AddTilde((string)Session[KEY_lastSortUrl]); hi.Visible = true; }
			}
			IgnoreHeaderConfirm((LinkButton)cAdmCtDisplayTypeGrid.FindControl("cTypeId18hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmCtDisplayTypeGrid.FindControl("cTypeName18hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmCtDisplayTypeGrid.FindControl("cTypeDesc18hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmCtDisplayTypeGrid.FindControl("cDisplayDefault18hl"));
		}

		private string GetButtonId(ListViewItem lvi)
		{
			string ButtonID = String.Empty;
			Control c = lvi.FindControl("cAdmCtDisplayTypeGridEdit");
		...
		{
		    return cAdmCtDisplayTypeGridDataPager.StartRowIndex / cAdmCtDisplayTypeGridDataPager.PageSize;
		}

		private int GetTotalPages()
		{
		    return (int)Math.Ceiling((double)cAdmCtDisplayTypeGridDataPager.TotalRowCount / cAdmCtDisplayTypeGridDataPager.PageSize);
		}

		private int GetDataItemIndex(int editIndex)
		{
		    return cAdmCtDisplayTypeGridDataPager.StartRowIndex + editIndex;
		}

		protected void cAdmCtDisplayTypeGrid_OnLayoutCreated(object sender, EventArgs e)
		{
		    // Header:
		    LinkButton lb = null;
		    lb = cAdmCtDisplayTypeGrid.FindControl("cTypeId18hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(0); lb.ToolTip = ColumnToolTip(0); lb.Parent.Visible = GridColumnVisible(0); }
		    lb = cAdmCtDisplayTypeGrid.FindControl("cTypeName18hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(1); lb.ToolTip = ColumnToolTip(1); lb.Parent.Visible = GridColumnVisible(1); }
		    lb = cAdmCtDisplayTypeGrid.FindControl("cTypeDesc18hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(2); lb.ToolTip = ColumnToolTip(2); lb.Parent.Visible = GridColumnVisible(2); }
		    lb = cAdmCtDisplayTypeGrid.FindControl("cDisplayDefault18hl") as LinkButton;
			...
			if (dtAuthRow != null)
			{
				...
				if (dr["AllowDel"].ToString() == "N" || (Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N"))
				{
		            lb = cAdmCtDisplayTypeGrid.FindControl("cDeleteAllButton") as LinkButton; if (lb != null) { lb.Visible = false; }
				}
			}
		    // footer:
		    Label gc = null;
		    gc = cAdmCtDisplayTypeGrid.FindControl("cTypeId18fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(0); }
		    gc = cAdmCtDisplayTypeGrid.FindControl("cTypeName18fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(1); }
		    gc = cAdmCtDisplayTypeGrid.FindControl("cTypeDesc18fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(2); }
		    gc = cAdmCtDisplayTypeGrid.FindControl("cDisplayDefault18fl") as Label;

...
namespace RO.Web
{
	public partial class AdmFlowchartModule : RO.Web.ModuleBase
	{
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_1027";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_1027";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_1027";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_1027";
		private const string KEY_dtLabel = "Cache:dtLabel3_1027";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_1027";
		...
		private const string KEY_dtAdmFlowchart1027List = "Cache:dtAdmFlowchart1027List";
		private const string KEY_bNewVisible = "Cache:bNewVisible3_1027";
		private const string KEY_bNewSaveVisible = "Cache:bNewSaveVisible3_1027";
		private const string KEY_bCopyVisible = "Cache:bCopyVisible3_1027";
		private const string KEY_bCopySaveVisible = "Cache:bCopySaveVisible3_1027";
		private const string KEY_bDeleteVisible = "Cache:bDeleteVisible3_1027";
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_1027";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_1027";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_1027";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				DataTable dtMenuAccess = (new MenuSystem()).GetMenu(base.LUser.CultureId, 3, base.LImpr, LcSysConnString, LcAppPw,1027, null, null);
				...
				Session.Remove(KEY_dtAdmFlowchart1027List);
				...
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 1027, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmFlowchart1027List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=1027&typ=N&sys=3','','',''); return false;"; }
				//WebRule: Button to launch flowchart editor
if (claunch.Attributes["OnClick"] == null) { claunch.Attributes["OnClick"] += "SearchLink('flowchart.html','myFlowChart','100%','90vh'); return false;"; }
				// *** WebRule End *** //
			}
		...
		private void SetSystem(byte SystemId)
		{
			...
			try
			{
				...
				if ((Config.DeployType == "DEV" || row["dbAppDatabase"].ToString() == base.CPrj.EntityCode + "View") && !(base.CPrj.EntityCode != "RO" && row["SysProgram"].ToString() == "Y") && (new AdminSystem()).IsRegenNeeded(string.Empty,1027,0,0,LcSysConnString,LcAppPw))
				{
					(new GenScreensSystem()).CreateProgram(1027, "Flowchart IDE", row["dbAppDatabase"].ToString(), base.CPrj, base.CSrc, base.CTar, LcAppConnString, LcAppPw);
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(1027,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(1027,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(1027,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(1027,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,1027,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(1027,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(1027,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(1027,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		protected DataTable GetAuthRow()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthRow(1027,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				try
				{
					try {dv = new DataView((new AdminSystem()).GetExp(1027,"GetExpAdmFlowchart1027","Y",null,null,filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
					catch {dv = new DataView((new AdminSystem()).GetExp(1027,"GetExpAdmFlowchart1027","N",null,null,filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(1027,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					foreach (DataRowView drv in dv)
					{
						...
						if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["CompanyDefault1325"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["ChartData1325"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						sb.Append(Environment.NewLine);
					}
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmFlowchart.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private string GetRtfGrid(DataView dv)
		{
			...
			try
			{
				...
				foreach (DataRowView drv in dv)
				{
					...
					if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append(drv["CompanyDefault1325"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append(drv["ChartData1325"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					sb.Append(@"}");
					sb.Append("\r\n");
					sb.Append(@"\pard \ql \li0\ri0\widctlpar\intbl\aspalpha\aspnum\adjustright\rin0\lin0 {");
					sb.Append(@"\insrsid2981395 \trowd \irow0\irowband0\lastrow \ts15\trgaph108\trleft-108\trbrdrt\brdrs\brdrw10 \trbrdrl\brdrs\brdrw10 \trbrdrb\brdrs\brdrw10 \trbrdrr\brdrs\brdrw10 \trbrdrh\brdrs\brdrw10 \trbrdrv\brdrs\brdrw10 ");
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 1027, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			...
			UpdCriteria(true);
			cAdmFlowchart1027List.ClearSearch(); Session.Remove(KEY_dtAdmFlowchart1027List);
			PopAdmFlowchart1027List(sender, e, false, null);
		}
			...
			try
			{
				foreach (DataRowView drv in dvCri)
				{
					...
					if (drv["DisplayName"].ToString() == "ComboBox")
					{
						...
						(new AdminSystem()).MkGetScreenIn("1027", drv["ScreenCriId"].ToString(), "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), LcAppDb, LcDesDb, drv["MultiDesignDb"].ToString(), LcSysConnString, LcAppPw);
						DataView dv = new DataView((new AdminSystem()).GetScreenIn("1027", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), (new AdminSystem()).CountScrCri(drv["ScreenCriId"].ToString(), drv["MultiDesignDb"].ToString(), drv["MultiDesignDb"].ToString() == "N" ? LcSysConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)), drv["RequiredValid"].ToString(), 0, string.Empty, drv["DisplayMode"].ToString() != "AutoListBox", val, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)));
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("1027", LcSysConnString, LcAppPw);
				}
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmFlowchart1027" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmFlowchart1027" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			foreach (DataRowView drv in dvCri)
			{
			    if (drv["DisplayName"].ToString() == "ListBox")
			    {
					...
					if (cListBox != null)
					{
						...
						int TotalChoiceCnt = new DataView((new AdminSystem()).GetScreenIn("1027", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), CriCnt, drv["RequiredValid"].ToString(), 0, string.Empty, true, string.Empty, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId))).Count;
						string selectedValues = string.Join(",", cListBox.Items.Cast<ListItem>().Where(x => x.Selected).Select(x => "'" + x.Value + "'").ToArray());
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("1027", "AdmFlowchart1027", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		private void SetCompanyId1325(RoboCoder.WebControls.ComboBox ddl, string keyId)
		{
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(1027,"GetDdlCompanyId3S4428",true,false,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
		...
		private DataView GetAdmFlowchart1027List()
		{
			...
			if (dt == null)
			{
				...
				try
				{
					try {dt = (new AdminSystem()).GetLis(1027,"GetLisAdmFlowchart1027",true,"Y",0,null,null,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
					catch {dt = (new AdminSystem()).GetLis(1027,"GetLisAdmFlowchart1027",true,"N",0,null,null,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				}
		...
		private void PopAdmFlowchart1027List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			DataView dv = GetAdmFlowchart1027List();
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			context["isSys"] = "Y";
			context["conn"] = string.Empty;
			cAdmFlowchart1027List.AutoCompleteUrl = "AutoComplete.aspx/LisSuggests";
			cAdmFlowchart1027List.DataContext = context;
			if (dv.Table == null) return;
			cAdmFlowchart1027List.DataSource = dv;
			cAdmFlowchart1027List.Visible = true;
			if (cAdmFlowchart1027List.Items.Count <= 0) {cAdmFlowchart1027List.Visible = false; cAdmFlowchart1027List_SelectedIndexChanged(sender,e);}
			else
			{
				if (bPageLoad && !string.IsNullOrEmpty(Request.QueryString["key"]) && bUseCri.Value == string.Empty)
				{
					try {cAdmFlowchart1027List.SelectByValue(Request.QueryString["key"],string.Empty,true);} catch {cAdmFlowchart1027List.Items[0].Selected = true; cAdmFlowchart1027List_SelectedIndexChanged(sender, e);}
				    cScreenSearch.Visible = false; cSystem.Visible = false; cEditButton.Visible = true; cSaveCloseButton.Visible = cSaveButton.Visible;
				}
				else
				{
					if (ID != null) {if (!cAdmFlowchart1027List.SelectByValue(ID,string.Empty,true)) {ID = null;}}
					if (ID == null)
					{
						cAdmFlowchart1027List_SelectedIndexChanged(sender, e);
					}
				}
			}
		}
		...
		private void SetColumnAuthority()
		{
			...
			{
				base.SetFoldBehavior(cChartId1325, dtAuth.Rows[0], cChartId1325P1, cChartId1325Label, cChartId1325P2, null, dtLabel.Rows[0], null, null, null);
				...
				base.SetFoldBehavior(cCompanyDefault1325, dtAuth.Rows[4], cCompanyDefault1325P1, cCompanyDefault1325Label, cCompanyDefault1325P2, null, dtLabel.Rows[4], null, null, null);
				base.SetFoldBehavior(cChartData1325, dtAuth.Rows[5], cChartData1325P1, cChartData1325Label, cChartData1325P2, cChartData1325E, null, dtLabel.Rows[5], null, null, null);
				...
				base.SetFoldBehavior(claunch, dtAuth.Rows[6], null, null, null, dtLabel.Rows[6], null, null, null);
				base.SetFoldBehavior(cView, dtAuth.Rows[7], null, null, null, dtLabel.Rows[7], null, null, null);
				base.SetFoldBehavior(cDisplay, dtAuth.Rows[8], null, null, null, dtLabel.Rows[8], null, null, null);
			}
			...
			if ((cCompanyDefault1325.Attributes["OnClick"] == null || cCompanyDefault1325.Attributes["OnClick"].IndexOf("ChkPgDirty") < 0) && cCompanyDefault1325.Visible && cCompanyDefault1325.Enabled) {cCompanyDefault1325.Attributes["OnClick"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty(); this.focus();";}
		...
		private void ClearMaster(object sender, System.EventArgs e)
		{
			...
			if (dt.Rows[4]["ColVisible"].ToString() == "Y" && dt.Rows[4]["ColReadOnly"].ToString() != "Y") {cCompanyDefault1325.Checked = base.GetBool("N");}
			if (dt.Rows[5]["ColVisible"].ToString() == "Y" && dt.Rows[5]["ColReadOnly"].ToString() != "Y") {cChartData1325.Text = string.Empty;}
		...
		private void InitMaster(object sender, System.EventArgs e)
		{
			...
			cCompanyDefault1325.Checked = base.GetBool("N");
			cChartData1325.Text = string.Empty;
			// *** Default Value (Folder) Web Rule starts here *** //
		}
		protected void cAdmFlowchart1027List_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmFlowchart1027List_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmFlowchart1027List_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			InitMaster(sender, e);      // Do this first to enable defaults for non-database columns.
			DataTable dt = null;
			try
			{
				dt = (new AdminSystem()).GetMstById("GetAdmFlowchart1027ById",cAdmFlowchart1027List.SelectedValue,null,null);
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				if (dt.Rows.Count > 0)
				{
					cAdmFlowchart1027List.SetDdVisible();
					...
					try {cCompanyDefault1325.Checked = base.GetBool(dr["CompanyDefault1325"].ToString());} catch {cCompanyDefault1325.Checked = false;}
...
if (!string.IsNullOrEmpty(cChartId1325.Text))
 ...
 if (string.IsNullOrEmpty(cAdmFlowchart1027List.SelectedValue))
		...
		protected void claunch_Click(object sender, System.EventArgs e)
		{
		...
		protected void cView_Click(object sender, System.EventArgs e)
		{
			// *** On Change/ On Click Web Rule starts here *** //
			EnableValidators(true); // Do not remove; Need to reenable after postback, especially in the grid.
		}
...
string projectRootPath = Config.RuleTierPath;
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cChartId1325.Text == string.Empty) { cClearButton_Click(sender, new EventArgs()); ShowDirty(false); PanelTop.Update();}
			else { PopAdmFlowchart1027List(sender, e, false, cChartId1325.Text); }
			// *** System Button Click (After) Web Rule starts here *** //
		}
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmFlowchart1027List); PopAdmFlowchart1027List(sender, e, false, null);
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			if (ValidPage())
			{
				AdmFlowchart1027 ds = PrepAdmFlowchartData(null,cChartId1325.Text == string.Empty);
				if (!string.IsNullOrEmpty(cAdmFlowchart1027List.SelectedValue) && cChartId1325.Text != cAdmFlowchart1027List.SelectedValue)
				{
					PreMsgPopup("Primary key cannot be changed");
					return rtn;
				}
				if (string.IsNullOrEmpty(cAdmFlowchart1027List.SelectedValue))	// Add
				{
					if (ds != null)
					{
						pid = (new AdminSystem()).AddData(1027,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc,noTrans,commandTimeOut);
					}
					if (!string.IsNullOrEmpty(pid))
					{
						cAdmFlowchart1027List.ClearSearch(); Session.Remove(KEY_dtAdmFlowchart1027List);
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmFlowchart1027List(sender, e, false, pid);
						rtn = GetScreenHlp().Rows[0]["AddMsg"].ToString();
					}
				}
				else {
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(1027,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc,noTrans,commandTimeOut)) {bValid7 = true;}
					if (bValid7)
					{
						cAdmFlowchart1027List.ClearSearch(); Session.Remove(KEY_dtAdmFlowchart1027List);
						ShowDirty(false); PopAdmFlowchart1027List(sender, e, false, ds.Tables["AdmFlowchart"].Rows[0]["ChartId1325"]);
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cChartId1325.Text != string.Empty)
			{
				AdmFlowchart1027 ds = PrepAdmFlowchartData(null,false);
				try
				{
					if (ds != null && (new AdminSystem()).DelData(1027,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc))
					{
						cAdmFlowchart1027List.ClearSearch(); Session.Remove(KEY_dtAdmFlowchart1027List);
						ShowDirty(false); PopAdmFlowchart1027List(sender, e, false, null);
		...
		private AdmFlowchart1027 PrepAdmFlowchartData(DataView dv, bool bAdd)
		{
			...
			if (bAdd) { dr["ChartId1325"] = string.Empty; } else { dr["ChartId1325"] = cChartId1325.Text; }
			drType["ChartId1325"] = "Numeric"; drDisp["ChartId1325"] = "TextBox";
			try {dr["ChartName1325"] = cChartName1325.Text.Trim();} catch {}
			...
			try {dr["CompanyDefault1325"] = base.SetBool(cCompanyDefault1325.Checked);} catch {}
		...
		public bool CanAct(char typ) { return CanAct(typ, GetAuthRow(), cAdmFlowchart1027List.SelectedValue.ToString()); }
		...
		private void PreMsgPopup(string msg) { PreMsgPopup(msg,cAdmFlowchart1027List,null); }

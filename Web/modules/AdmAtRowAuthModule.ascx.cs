...
namespace RO.Web
{
	public partial class AdmAtRowAuthModule : RO.Web.ModuleBase
	{

		private const string KEY_lastAddedRow = "Cache:lastAddedRow3_22";
		private const string KEY_lastSortOrd = "Cache:lastSortOrd3_22";
		private const string KEY_lastSortImg = "Cache:lastSortImg3_22";
		private const string KEY_lastSortCol = "Cache:lastSortCol3_22";
		private const string KEY_lastSortExp = "Cache:lastSortExp3_22";
		private const string KEY_lastSortUrl = "Cache:lastSortUrl3_22";
		private const string KEY_lastSortTog = "Cache:lastSortTog3_22";
		private const string KEY_lastImpPwdOvride = "Cache:lastImpPwdOvride3_22";
		private const string KEY_cntImpPwdOvride = "Cache:cntImpPwdOvride3_22";
		private const string KEY_currPageIndex = "Cache:currPageIndex3_22";
		private const string KEY_bHiImpVisible = "Cache:bHiImpVisible3_22";
		private const string KEY_bShImpVisible = "Cache:bShImpVisible3_22";
		private const string KEY_dtAdmAtRowAuthGrid = "Cache:dtAdmAtRowAuthGrid";
		private const string KEY_scrImport = "Cache:scrImport3_22";
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_22";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_22";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_22";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_22";
		private const string KEY_dtLabel = "Cache:dtLabel3_22";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_22";
		private const string KEY_dtScrCri = "Cache:dtScrCri3_22";
		private const string KEY_dsScrCriVal = "Cache:dsScrCriVal3_22";

		private const string KEY_dtOvrideId236 = "Cache:dtOvrideId236";
		private const string KEY_dtAllowSel236 = "Cache:dtAllowSel236";
		private const string KEY_dtPermKeyId237 = "Cache:dtPermKeyId237";
		private const string KEY_dtSelLevel237 = "Cache:dtSelLevel237";

		private const string KEY_dtSystems = "Cache:dtSystems3_22";
		private const string KEY_sysConnectionString = "Cache:sysConnectionString3_22";
		private const string KEY_dtAdmAtRowAuth22List = "Cache:dtAdmAtRowAuth22List";
		private const string KEY_bNewVisible = "Cache:bNewVisible3_22";
		private const string KEY_bNewSaveVisible = "Cache:bNewSaveVisible3_22";
		private const string KEY_bCopyVisible = "Cache:bCopyVisible3_22";
		private const string KEY_bCopySaveVisible = "Cache:bCopySaveVisible3_22";
		private const string KEY_bDeleteVisible = "Cache:bDeleteVisible3_22";
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_22";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_22";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_22";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				DataTable dtMenuAccess = (new MenuSystem()).GetMenu(base.LUser.CultureId, 3, base.LImpr, LcSysConnString, LcAppPw,22, null, null);
				...
				Session.Remove(KEY_dtOvrideId236);
				Session.Remove(KEY_dtAllowSel236);
				Session.Remove(KEY_dtPermKeyId237);
				Session.Remove(KEY_dtSelLevel237);
				SetButtonHlp();
				GetSystems();
				SetColumnAuthority();
				GetGlobalFilter();
				...
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 22, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmAtRowAuth22List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=22&typ=N&sys=3','','',''); return false;"; }
			...
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmAtRowAuthGrid];
				if (dt != null)
				{
		...
		private void InitializeComponent()
		{
			...
			if (LcSysConnString == null)
			{
				...
				{
					DataTable dt;
					dt = (new AdminSystem()).GetAuthRow(22,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
					...
					{
						cAdmAtRowAuthGrid.PreRender += new EventHandler(cAdmAtRowAuthGrid_OnPreRender);
					}
				}
		...
		private void SetSystem(byte SystemId)
		{
			...
			try
			{
				...
				if ((Config.DeployType == "DEV" || row["dbAppDatabase"].ToString() == base.CPrj.EntityCode + "View") && !(base.CPrj.EntityCode != "RO" && row["SysProgram"].ToString() == "Y") && (new AdminSystem()).IsRegenNeeded(string.Empty,22,0,0,LcSysConnString,LcAppPw))
				{
					(new GenScreensSystem()).CreateProgram(22, "Authority Default (ROW)", row["dbAppDatabase"].ToString(), base.CPrj, base.CSrc, base.CTar, LcAppConnString, LcAppPw);
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(22,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(22,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void SetClientRule(ListViewDataItem ee, bool isEditItem)
		{
			...
			if (dvRul.Count > 0)
			{
				...
				{
					if (ee == null || drv["ScriptEvent"].ToString().Substring(0,2).ToLower() != "on" || (cAdmAtRowAuthGrid.EditIndex > -1 && GetDataItemIndex(cAdmAtRowAuthGrid.EditIndex) == ee.DataItemIndex))
					{
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(22,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(22,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,22,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(22,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(22,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(22,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				try
				{
					try {dv = new DataView((new AdminSystem()).GetExp(22,"GetExpAdmAtRowAuth22","Y",null,null,filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
					catch {dv = new DataView((new AdminSystem()).GetExp(22,"GetExpAdmAtRowAuth22","N",null,null,filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(22,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					{
						if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["RowAuthId236"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["RowAuthName236"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(drv["OvrideId236"].ToString() + (char)9 + drv["OvrideId236Text"].ToString() + (char)9);}
						if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["AllowSel236"].ToString().Replace("\"","\"\"") + "\"" + (char)9 + "\"" + drv["AllowSel236Text"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["AllowAdd236"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["AllowUpd236"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["AllowDel236"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[7]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["SysAdmin236"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[8]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["RowAuthPrmId237"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[9]["ColExport"].ToString() == "Y") {sb.Append(drv["PermKeyId237"].ToString() + (char)9 + drv["PermKeyId237Text"].ToString() + (char)9);}
						if (dtAu.Rows[10]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["SelLevel237"].ToString().Replace("\"","\"\"") + "\"" + (char)9 + "\"" + drv["SelLevel237Text"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						sb.Append(Environment.NewLine);
					}
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmAtRowAuth.csv"; Session["ExportStr"] = (Config.ExportExcelCSV ? "sep=\t\n": "") + sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmAtRowAuth.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private string GetRtfGrid(DataView dv)
		{
			...
			try
			{
				...
				foreach (DataRowView drv in dv)
				{
					...
					sb.Append("\r\n");
					sb.Append(@"{");
					if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["RowAuthId236"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["RowAuthName236"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(drv["OvrideId236Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(drv["AllowSel236Text"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append(drv["AllowAdd236"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append(drv["AllowUpd236"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append(drv["AllowDel236"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[7]["ColExport"].ToString() == "Y") {sb.Append(drv["SysAdmin236"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[8]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["RowAuthPrmId237"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[9]["ColExport"].ToString() == "Y") {sb.Append(drv["PermKeyId237Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[10]["ColExport"].ToString() == "Y") {sb.Append(drv["SelLevel237Text"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					sb.Append(@"}");
					sb.Append("\r\n");
					sb.Append(@"\pard \ql \li0\ri0\widctlpar\intbl\aspalpha\aspnum\adjustright\rin0\lin0 {");
					sb.Append(@"\insrsid2981395 \trowd \irow0\irowband0\lastrow \ts15\trgaph108\trleft-108\trbrdrt\brdrs\brdrw10 \trbrdrl\brdrs\brdrw10 \trbrdrb\brdrs\brdrw10 \trbrdrr\brdrs\brdrw10 \trbrdrh\brdrs\brdrw10 \trbrdrv\brdrs\brdrw10 ");
		...
		private void GetPageInfo()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastPageInfo(22, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 22, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			// *** Criteria Trigger (before) Web Rule starts here *** //
			DataTable dt = (DataTable)Session[KEY_dtAdmAtRowAuthGrid];
			if (cAdmAtRowAuthGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
			if (IsPostBack && cCriteria.Visible) ReBindCriteria(GetScrCriteria());
			UpdCriteria(true);
			cAdmAtRowAuth22List.ClearSearch(); Session.Remove(KEY_dtAdmAtRowAuth22List);
			PopAdmAtRowAuth22List(sender, e, false, null);
			}
		}
			...
			try
			{
				foreach (DataRowView drv in dvCri)
				{
					...
					if (drv["DisplayName"].ToString() == "ComboBox")
					{
						...
						(new AdminSystem()).MkGetScreenIn("22", drv["ScreenCriId"].ToString(), "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), LcAppDb, LcDesDb, drv["MultiDesignDb"].ToString(), LcSysConnString, LcAppPw);
						DataView dv = new DataView((new AdminSystem()).GetScreenIn("22", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), (new AdminSystem()).CountScrCri(drv["ScreenCriId"].ToString(), drv["MultiDesignDb"].ToString(), drv["MultiDesignDb"].ToString() == "N" ? LcSysConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)), drv["RequiredValid"].ToString(), 0, string.Empty, drv["DisplayMode"].ToString() != "AutoListBox", val, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)));
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("22", LcSysConnString, LcAppPw);
				}
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmAtRowAuth22" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmAtRowAuth22" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			foreach (DataRowView drv in dvCri)
			{
			    if (drv["DisplayName"].ToString() == "ListBox")
			    {
					...
					if (cListBox != null)
					{
						...
						int TotalChoiceCnt = new DataView((new AdminSystem()).GetScreenIn("22", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), CriCnt, drv["RequiredValid"].ToString(), 0, string.Empty, true, string.Empty, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId))).Count;
						string selectedValues = string.Join(",", cListBox.Items.Cast<ListItem>().Where(x => x.Selected).Select(x => "'" + x.Value + "'").ToArray());
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("22", "AdmAtRowAuth22", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		private void SetOvrideId236(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtOvrideId236];
			...
			if (ddl != null)
			{
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(22,"GetDdlOvrideId3S1239",true,bAll,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					if (dv.Table.Columns.Contains("RowAuthId"))
					{
						ss = "(RowAuthId is null";
						if (string.IsNullOrEmpty(cAdmAtRowAuth22List.SelectedValue)) {ss = ss + ")";} else {ss = ss + " OR RowAuthId = " + cAdmAtRowAuth22List.SelectedValue + ")";}
					}
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "OvrideId236 is not NULL"; }
					dv.RowFilter = ss;
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(22,"GetDdlOvrideId3S1239",true,true,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtOvrideId236] = dv.Table;
				}
			}
		}

		private void SetAllowSel236(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtAllowSel236];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (ddl != null)
			{
				string ss = string.Empty;
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(22,"GetDdlAllowSel3S309",true,bAll,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					...
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "AllowSel236 is not NULL"; }
					dv.RowFilter = ss;
					ddl.DataSource = dv; ddl.DataBind();
					li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					else if (!bFirst)
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(22,"GetDdlAllowSel3S309",true,true,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtAllowSel236] = dv.Table;
				}
			}
		}

		private void SetPermKeyId237(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtPermKeyId237];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (ddl != null)
			{
				string ss = string.Empty;
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(22,"GetDdlPermKeyId3S1877",true,bAll,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					...
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "PermKeyId237 is not NULL"; }
					dv.RowFilter = ss;
					ddl.DataSource = dv; ddl.DataBind();
					li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					else if (!bFirst)
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(22,"GetDdlPermKeyId3S1877",true,true,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtPermKeyId237] = dv.Table;
				}
			}
		}

		private void SetSelLevel237(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtSelLevel237];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (ddl != null)
			{
				string ss = string.Empty;
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(22,"GetDdlSelLevel3S1879",true,bAll,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					...
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "SelLevel237 is not NULL"; }
					dv.RowFilter = ss;
					ddl.DataSource = dv; ddl.DataBind();
					li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					else if (!bFirst)
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(22,"GetDdlSelLevel3S1879",true,true,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtSelLevel237] = dv.Table;
				}
			}
		}

		private DataView GetAdmAtRowAuth22List()
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmAtRowAuth22List];
			if (dt == null)
			{
				CheckAuthentication(false);
				int filterId = 0;
				...
				try
				{
					try {dt = (new AdminSystem()).GetLis(22,"GetLisAdmAtRowAuth22",true,"Y",0,null,null,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
					catch {dt = (new AdminSystem()).GetLis(22,"GetLisAdmAtRowAuth22",true,"N",0,null,null,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				}
		...
		private void PopAdmAtRowAuth22List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			DataView dv = GetAdmAtRowAuth22List();
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			cAdmAtRowAuth22List.AutoCompleteUrl = "AutoComplete.aspx/LisSuggests";
			cAdmAtRowAuth22List.DataContext = context;
			if (dv.Table == null) return;
			cAdmAtRowAuth22List.DataSource = dv;
			cAdmAtRowAuth22List.Visible = true;
			if (cAdmAtRowAuth22List.Items.Count <= 0) {cAdmAtRowAuth22List.Visible = false; cAdmAtRowAuth22List_SelectedIndexChanged(sender,e);}
			else
			{
				if (bPageLoad && !string.IsNullOrEmpty(Request.QueryString["key"]) && bUseCri.Value == string.Empty)
				{
					try {cAdmAtRowAuth22List.SelectByValue(Request.QueryString["key"],string.Empty,true);} catch {cAdmAtRowAuth22List.Items[0].Selected = true; cAdmAtRowAuth22List_SelectedIndexChanged(sender, e);}
				    cScreenSearch.Visible = false; cSystem.Visible = false; cEditButton.Visible = true; cSaveCloseButton.Visible = cSaveButton.Visible;
				}
				else
				{
					if (ID != null) {if (!cAdmAtRowAuth22List.SelectByValue(ID,string.Empty,true)) {ID = null;}}
					if (ID == null)
					{
						cAdmAtRowAuth22List_SelectedIndexChanged(sender, e);
					}
				}
			}
		}
		...
		private void SetColumnAuthority()
		{
			...
			{
				base.SetFoldBehavior(cRowAuthId236, dtAuth.Rows[0], cRowAuthId236P1, cRowAuthId236Label, cRowAuthId236P2, null, dtLabel.Rows[0], null, null, null);
				base.SetFoldBehavior(cRowAuthName236, dtAuth.Rows[1], cRowAuthName236P1, cRowAuthName236Label, cRowAuthName236P2, null, dtLabel.Rows[1], cRFVRowAuthName236, null, null);
				base.SetFoldBehavior(cOvrideId236, dtAuth.Rows[2], cOvrideId236P1, cOvrideId236Label, cOvrideId236P2, null, dtLabel.Rows[2], null, null, null);
				base.SetFoldBehavior(cAllowSel236, dtAuth.Rows[3], cAllowSel236P1, cAllowSel236Label, cAllowSel236P2, null, dtLabel.Rows[3], cRFVAllowSel236, null, null);
				base.SetFoldBehavior(cAllowAdd236, dtAuth.Rows[4], cAllowAdd236P1, cAllowAdd236Label, cAllowAdd236P2, null, dtLabel.Rows[4], null, null, null);
				base.SetFoldBehavior(cAllowUpd236, dtAuth.Rows[5], cAllowUpd236P1, cAllowUpd236Label, cAllowUpd236P2, null, dtLabel.Rows[5], null, null, null);
				base.SetFoldBehavior(cAllowDel236, dtAuth.Rows[6], cAllowDel236P1, cAllowDel236Label, cAllowDel236P2, null, dtLabel.Rows[6], null, null, null);
				base.SetFoldBehavior(cSysAdmin236, dtAuth.Rows[7], cSysAdmin236P1, cSysAdmin236Label, cSysAdmin236P2, null, dtLabel.Rows[7], null, null, null);
			}
			if ((cRowAuthName236.Attributes["OnChange"] == null || cRowAuthName236.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cRowAuthName236.Visible && !cRowAuthName236.ReadOnly) {cRowAuthName236.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cOvrideId236.Attributes["OnChange"] == null || cOvrideId236.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cOvrideId236.Visible && cOvrideId236.Enabled) {cOvrideId236.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();document.getElementById('" + bConfirm.ClientID + "').value='N';";}
			if (cOvrideId236Pwd.Attributes["OnChange"] == null || cOvrideId236Pwd.Attributes["OnChange"].IndexOf("_bConfirm") < 0) {cOvrideId236Pwd.Attributes["OnChange"] += "document.getElementById('" + bConfirm.ClientID + "').value='N';";}
			if ((cAllowSel236.Attributes["OnChange"] == null || cAllowSel236.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cAllowSel236.Visible && cAllowSel236.Enabled) {cAllowSel236.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cAllowAdd236.Attributes["OnClick"] == null || cAllowAdd236.Attributes["OnClick"].IndexOf("ChkPgDirty") < 0) && cAllowAdd236.Visible && cAllowAdd236.Enabled) {cAllowAdd236.Attributes["OnClick"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty(); this.focus();";}
			if ((cAllowUpd236.Attributes["OnClick"] == null || cAllowUpd236.Attributes["OnClick"].IndexOf("ChkPgDirty") < 0) && cAllowUpd236.Visible && cAllowUpd236.Enabled) {cAllowUpd236.Attributes["OnClick"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty(); this.focus();";}
			if ((cAllowDel236.Attributes["OnClick"] == null || cAllowDel236.Attributes["OnClick"].IndexOf("ChkPgDirty") < 0) && cAllowDel236.Visible && cAllowDel236.Enabled) {cAllowDel236.Attributes["OnClick"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty(); this.focus();";}
			if ((cSysAdmin236.Attributes["OnClick"] == null || cSysAdmin236.Attributes["OnClick"].IndexOf("ChkPgDirty") < 0) && cSysAdmin236.Visible && cSysAdmin236.Enabled) {cSysAdmin236.Attributes["OnClick"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty(); this.focus();";}
		}
		...
		private void ClearMaster(object sender, System.EventArgs e)
		{
			...
			if (dt.Rows[1]["ColVisible"].ToString() == "Y" && dt.Rows[1]["ColReadOnly"].ToString() != "Y") {cRowAuthName236.Text = string.Empty;}
			if (dt.Rows[2]["ColVisible"].ToString() == "Y" && dt.Rows[2]["ColReadOnly"].ToString() != "Y") {SetOvrideId236(cOvrideId236,string.Empty);}
			if (dt.Rows[3]["ColVisible"].ToString() == "Y" && dt.Rows[3]["ColReadOnly"].ToString() != "Y") {SetAllowSel236(cAllowSel236,"S");}
			if (dt.Rows[4]["ColVisible"].ToString() == "Y" && dt.Rows[4]["ColReadOnly"].ToString() != "Y") {cAllowAdd236.Checked = base.GetBool("N");}
			if (dt.Rows[5]["ColVisible"].ToString() == "Y" && dt.Rows[5]["ColReadOnly"].ToString() != "Y") {cAllowUpd236.Checked = base.GetBool("N");}
			if (dt.Rows[6]["ColVisible"].ToString() == "Y" && dt.Rows[6]["ColReadOnly"].ToString() != "Y") {cAllowDel236.Checked = base.GetBool("N");}
			if (dt.Rows[7]["ColVisible"].ToString() == "Y" && dt.Rows[7]["ColReadOnly"].ToString() != "Y") {cSysAdmin236.Checked = base.GetBool("N");}
			// *** Default Value (Folder) Web Rule starts here *** //
		}

		private void InitMaster(object sender, System.EventArgs e)
		{
			cRowAuthId236.Text = string.Empty;
			cRowAuthName236.Text = string.Empty;
			SetOvrideId236(cOvrideId236,string.Empty);
			SetAllowSel236(cAllowSel236,"S");
			cAllowAdd236.Checked = base.GetBool("N");
			cAllowUpd236.Checked = base.GetBool("N");
			cAllowDel236.Checked = base.GetBool("N");
			cSysAdmin236.Checked = base.GetBool("N");
			// *** Default Value (Folder) Web Rule starts here *** //
		}
		protected void cAdmAtRowAuth22List_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmAtRowAuth22List_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmAtRowAuth22List_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			InitMaster(sender, e);      // Do this first to enable defaults for non-database columns.
			DataTable dt = null;
			try
			{
				dt = (new AdminSystem()).GetMstById("GetAdmAtRowAuth22ById",cAdmAtRowAuth22List.SelectedValue,null,null);
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				if (dt.Rows.Count > 0)
				{
					cAdmAtRowAuth22List.SetDdVisible();
					DataRow dr = dt.Rows[0];
					try {cRowAuthId236.Text = RO.Common3.Utils.fmNumeric("0",dr["RowAuthId236"].ToString(),base.LUser.Culture);} catch {cRowAuthId236.Text = string.Empty;}
					try {cRowAuthName236.Text = dr["RowAuthName236"].ToString();} catch {cRowAuthName236.Text = string.Empty;}
					SetOvrideId236(cOvrideId236,dr["OvrideId236"].ToString());
					SetAllowSel236(cAllowSel236,dr["AllowSel236"].ToString()); if (cAllowSel236.SelectedIndex <= 0 && !(cAllowSel236.Enabled && cAllowSel236.Visible)) { SetAllowSel236(cAllowSel236,"S"); }
					try {cAllowAdd236.Checked = base.GetBool(dr["AllowAdd236"].ToString());} catch {cAllowAdd236.Checked = false;}
					try {cAllowUpd236.Checked = base.GetBool(dr["AllowUpd236"].ToString());} catch {cAllowUpd236.Checked = false;}
					try {cAllowDel236.Checked = base.GetBool(dr["AllowDel236"].ToString());} catch {cAllowDel236.Checked = false;}
					try {cSysAdmin236.Checked = base.GetBool(dr["SysAdmin236"].ToString());} catch {cSysAdmin236.Checked = false;}
				}
			}
			cButPanel.DataBind(); if (!cSaveButton.Visible) { cInsRowButton.Visible = false; }
			DataTable dtAdmAtRowAuthGrid = null;
			int filterId = 0; if (Utils.IsInt(cFilterId.SelectedValue)) { filterId = int.Parse(cFilterId.SelectedValue); }
			try
			{
				dtAdmAtRowAuthGrid = (new AdminSystem()).GetDtlById(22,"GetAdmAtRowAuth22DtlById",cAdmAtRowAuth22List.SelectedValue,null,null,filterId,base.LImpr,base.LCurr);
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (!dtAdmAtRowAuthGrid.Columns.Contains("_NewRow")) { dtAdmAtRowAuthGrid.Columns.Add("_NewRow"); }
			Session[KEY_dtAdmAtRowAuthGrid] = dtAdmAtRowAuthGrid;
			cAdmAtRowAuthGrid.EditIndex = -1;
			cAdmAtRowAuthGridDataPager.PageSize = Int16.Parse(cPgSize.Text); GotoPage(0);
			if (Session[KEY_lastSortCol] != null)
			{
				cAdmAtRowAuthGrid_OnSorting(sender, new ListViewSortEventArgs((string)Session[KEY_lastSortExp], SortDirection.Ascending));
			}
			else if (dtAdmAtRowAuthGrid.Rows.Count <= 0 || (!((GetAuthRow().Rows[0]["AllowUpd"].ToString() == "N" || GetAuthRow().Rows[0]["ViewOnly"].ToString() == "G") && GetAuthRow().Rows[0]["AllowAdd"].ToString() == "N") && !(Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N") && dtAdmAtRowAuthGrid.Rows.Count == 1))
			{
				cAdmAtRowAuthGrid_DataBind(dtAdmAtRowAuthGrid.DefaultView);
			}
			cFindButton_Click(sender, e);
			cNaviPanel.Visible = true; cImportPwdPanel.Visible = false; Session.Remove(KEY_scrImport);
			ScriptManager.GetCurrent(Parent.Page).SetFocus(cAdmAtRowAuth22List.FocusID);
			ShowDirty(false); PanelTop.Update();
			// *** List Selection (End of) Web Rule starts here *** //
		}

		protected void cOvrideId236_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			...
			TextBox pwd = null;
			if (cOvrideId236.Items.Count > 0 && Session[KEY_dtOvrideId236] != null)
			{
				DataView dv = ((DataTable)Session[KEY_dtOvrideId236]).DefaultView; dv.RowFilter = string.Empty;
				DataRowView dr = cOvrideId236.SelectedIndex >= 0 && cOvrideId236.SelectedIndex < dv.Count ? dv[cOvrideId236.SelectedIndex] : dv[0];
				try
				{
					...
					{
						cOvrideId236.Visible = false; cOvrideId236Pwd.Visible = true; pwd = cOvrideId236Pwd;
					}
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
		...
		protected void cOvrideId236Pwd_TextChanged(object sender, System.EventArgs e)
		{
			bool bOk = true;
			DataView dv = ((DataTable)Session[KEY_dtOvrideId236]).DefaultView;
			DataRowView dr = dv[cOvrideId236.SelectedIndex];
			bool bValid2 = false;
			try
			{
			...
			if (bValid2)
			{
				if (dr["PromptModal"].ToString() == "N") {SetOvrideId236(cOvrideId236,string.Empty);} else {bOk = false;}
			}
			if (bOk)
			{
				cOvrideId236.Visible = true; cOvrideId236Pwd.Visible = false; cOvrideId236Pwl.Visible = false;
				ScriptManager.GetCurrent(Parent.Page).SetFocus(cOvrideId236.ClientID);
			}
			else {ScriptManager.GetCurrent(Parent.Page).SetFocus(cOvrideId236Pwd.ClientID); cOvrideId236Pwl.Visible = true;}
		}
		...
		public void cFindButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dv != null)
			{
				...
				if ((cc != null && cc.ID.Equals("cAdmAtRowAuth22List")) || cAdmAtRowAuthGrid.EditIndex < 0 || UpdateGridRow(cAdmAtRowAuthGrid, new CommandEventArgs("Save", "")))
				{
					...
					GotoPage(0); cAdmAtRowAuthGrid_DataBind(dv);
					if (GetCurrPageIndex() != (int)Session[KEY_currPageIndex] && (int)Session[KEY_currPageIndex] < GetTotalPages()) { GotoPage((int)Session[KEY_currPageIndex]); cAdmAtRowAuthGrid_DataBind(dv);}
		...
		public void cInsRowButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dt != null && (ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")))
			{
				...
				Session[KEY_lastAddedRow] = 0; Session[KEY_dtAdmAtRowAuthGrid] = dt; Session[KEY_currPageIndex] = 0; GotoPage(0);
				cAdmAtRowAuthGrid_DataBind(dt.DefaultView);
				cAdmAtRowAuthGrid_OnItemEditing(cAdmAtRowAuthGrid, new ListViewEditEventArgs(0));
		...
		public void cPgSizeButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cAdmAtRowAuthGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				...
				cAdmAtRowAuthGridDataPager.PageSize = int.Parse(cPgSize.Text); GotoPage(0);
				if (dt.Rows.Count <= 0 || (!((GetAuthRow().Rows[0]["AllowUpd"].ToString() == "N" || GetAuthRow().Rows[0]["ViewOnly"].ToString() == "G") && GetAuthRow().Rows[0]["AllowAdd"].ToString() == "N") && !(Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N") && dt.Rows.Count == 1)) {cAdmAtRowAuthGrid_DataBind(dt.DefaultView);} else {cFindButton_Click(sender, e);}
				try
				{
					(new AdminSystem()).UpdLastPageInfo(22, base.LUser.UsrId, cPgSize.Text, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		}
		...
		public void cShowImpButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cSchemaImage.Visible)
			{
				try
				{
					Session["ImportSchema"] = (new AdminSystem()).GetSchemaScrImp(22,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (cSchemaImage.Attributes["OnClick"] == null || cSchemaImage.Attributes["OnClick"].IndexOf("ImportSchema.aspx") < 0) {cSchemaImage.Attributes["OnClick"] += "SearchLink('ImportSchema.aspx?scm=S&key=22&csy=3','','',''); return false;";}
			}
			// *** System Button Click (after) Web Rule starts here *** //
		}
		...
		private void ScrImportDdl(DataView dvd, DataRowView drv, bool bComboBox, string PKey, string CKey, string CNam, int MaxLen, string MatchCd, bool bAllowNulls)
		{
			if (dvd != null)
			{
				...
				{
					if (string.IsNullOrEmpty(cAdmAtRowAuth22List.SelectedValue))
					{
						dvd.RowFilter = "(" + PKey + " is null)";
					}
					else
					{
						dvd.RowFilter = "(" + PKey + " is null OR " + PKey + " = " + cAdmAtRowAuth22List.SelectedValue + ")";
					}
				}
		...
		protected void ScrImport(bool bClear)
		{
			...
			DataTable dtAdmAtRowAuthGrid = (DataTable)Session[KEY_dtAdmAtRowAuthGrid];
			if (dti != null && dtAdmAtRowAuthGrid != null)
			{
				if (bClear)
				{
					dtAdmAtRowAuthGrid.Rows.Clear();
				}
				cFind.Text = string.Empty;
				int iExist = dtAdmAtRowAuthGrid.Rows.Count;
				...
				foreach (DataRowView drv in dti.DefaultView)
				{
					...
					{
						DataTable dtd;
						dtd = (DataTable)Session[KEY_dtPermKeyId237];
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(22,"GetDdlPermKeyId3S1877",true,true,0,string.Empty,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
							Session[KEY_dtPermKeyId237] = dtd;
						}
						ScrImportDdl(dtd.DefaultView, drv, false, "RowAuthId", "PermKeyId237", "PermKeyId237Text", 50, "2", false);
						dtd = (DataTable)Session[KEY_dtSelLevel237];
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(22,"GetDdlSelLevel3S1879",true,true,0,string.Empty,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
							Session[KEY_dtSelLevel237] = dtd;
						}
						ScrImportDdl(dtd.DefaultView, drv, false, "RowAuthId", "SelLevel237", "SelLevel237Text", 10, "2", false);
					}
					jj = jj + 1;
				}
				iRow = dti.DefaultView.Count - iExist;
				if (!dti.Columns.Contains("_NewRow")) { dti.Columns.Add("_NewRow"); }
				Session[KEY_dtAdmAtRowAuthGrid] = dti;
				cAdmAtRowAuthGrid_DataBind(dti.DefaultView);
		...
		public void cImportButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cFNameO.Text != string.Empty && cWorkSheet.Items.Count > 0 && cWorkSheet.SelectedItem.Text != string.Empty && cStartRow.Text != string.Empty)
			{
				...
				try
				{
					if (cAdmAtRowAuthGrid.EditIndex < 0 || UpdateGridRow(cAdmAtRowAuthGrid, new CommandEventArgs("Save", "")))
					{
						Session[KEY_scrImport] = ImportFile(dtAdmAtRowAuthGrid.Copy(),cFNameO.Text,cWorkSheet.SelectedItem.Text,cStartRow.Text,Config.PathTmpImport + cFName.Text);
					}
			...
			try
			{
				...
				for ( int iRow = iStart; iRow < rows.Count; iRow++ )
				{
					...
					else
					{
						...
						if (rows[iRow][3].ToString() == string.Empty && rows[iRow][4].ToString() == string.Empty) { rows[iRow][3] = "S";}
		...
		private void GotoPage(int pageNo)
		{
			if (pageNo >= 0 && pageNo < GetTotalPages())
			{
				if (cAdmAtRowAuthGrid.EditIndex < 0 || UpdateGridRow(cAdmAtRowAuthGrid, new CommandEventArgs("Save", "")))
				{
					try { cAdmAtRowAuthGridDataPager.SetPageProperties(cAdmAtRowAuthGridDataPager.PageSize * pageNo, cAdmAtRowAuthGridDataPager.MaximumRows, false); cAdmAtRowAuthGrid_DataBind(null); }
					catch
					{
						try { cAdmAtRowAuthGridDataPager.SetPageProperties(0, cAdmAtRowAuthGridDataPager.MaximumRows, false); cAdmAtRowAuthGrid_DataBind(null); } catch {}
					}
				}
			}
		}

		protected void cAdmAtRowAuthGrid_OnItemCommand(object sender, ListViewCommandEventArgs e)
		{
		}

		private void cAdmAtRowAuthGrid_DataBind(DataView dvAdmAtRowAuthGrid)
		{
			if (dvAdmAtRowAuthGrid == null)
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmAtRowAuthGrid];
				dvAdmAtRowAuthGrid = dt != null ? dt.DefaultView : null;
			}
			if (dvAdmAtRowAuthGrid != null)
			{
				LcAuth = GetAuthCol().AsEnumerable().ToDictionary<DataRow,string>(dr=>dr["ColName"].ToString());
				cAdmAtRowAuthGrid.DataSource = dvAdmAtRowAuthGrid;
				cAdmAtRowAuthGrid.DataBind();
		...
		protected void cAdmAtRowAuthGrid_OnSorting(object sender, ListViewSortEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmAtRowAuthGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null)
			{
				if (cAdmAtRowAuthGrid.EditIndex < 0 || UpdateGridRow(cAdmAtRowAuthGrid, new CommandEventArgs("Save", "")))
				{
					...
					Session[KEY_dtAdmAtRowAuthGrid] = dt;
					cAdmAtRowAuthGrid_DataBind(dv);
				}
			}
		}

		protected void cAdmAtRowAuthGrid_OnPagePropertiesChanging(object sender, PagePropertiesChangingEventArgs e)
		{
		    cAdmAtRowAuthGridDataPager.SetPageProperties(e.StartRowIndex, e.MaximumRows, false);
		    DataTable dt = (DataTable)Session[KEY_dtAdmAtRowAuthGrid];
		    DataView dv = dt != null ? dt.DefaultView : null;
			cAdmAtRowAuthGrid_DataBind(dv); cAdmAtRowAuthGrid.EditIndex = -1;
		}

		private void GridChkPgDirty(ListViewItem lvi)
		{
				WebControl cc = null;
				cc = ((WebControl)lvi.FindControl("cPermKeyId237"));
				...
				cc = ((WebControl)lvi.FindControl("cSelLevel237"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
		}

		protected void cAdmAtRowAuthGrid_OnItemDataBound(object sender, ListViewItemEventArgs e)
		{
			// *** GridItemDataBound (before) Web Rule End *** //
			DataTable dt = (DataTable)Session[KEY_dtAdmAtRowAuthGrid];
			bool isEditItem = false;
			DataView dvAdmAtRowAuthGrid = dt != null ? dt.DefaultView : null;
			if (cAdmAtRowAuthGrid.EditIndex > -1 && GetDataItemIndex(cAdmAtRowAuthGrid.EditIndex) == e.Item.DataItemIndex)
			{
				isEditItem = true;
				base.SetGridEnabled(e.Item, GetAuthCol(), GetLabel(), 8);
				SetPermKeyId237((DropDownList)e.Item.FindControl("cPermKeyId237"),dvAdmAtRowAuthGrid[e.Item.DataItemIndex]["PermKeyId237"].ToString());
				if (dvAdmAtRowAuthGrid[e.Item.DataItemIndex]["SelLevel237"].ToString() != string.Empty) {SetSelLevel237((DropDownList)e.Item.FindControl("cSelLevel237"),dvAdmAtRowAuthGrid[e.Item.DataItemIndex]["SelLevel237"].ToString());} else {SetSelLevel237((DropDownList)e.Item.FindControl("cSelLevel237"),"S");}
			...
			if (dtAuthRow != null)
			{
				...
				{
					cc = e.Item.FindControl("cAdmAtRowAuthGridDelete"); if (cc != null) { cc.Visible = false; }
					cc = e.Item.FindControl("cAdmAtRowAuthGridEdit"); if (cc != null) { cc.Visible = false; }
				}
				else
				{
			        HtmlTableRow tr = e.Item.FindControl("cAdmAtRowAuthGridRow") as HtmlTableRow;
			        LinkButton lb = e.Item.FindControl("cAdmAtRowAuthGridEdit") as LinkButton;
		...
		protected void cRowAuthPrmId237hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "0") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "0"; Session[KEY_lastSortExp] = "RowAuthPrmId237";Session[KEY_lastSortImg] = "cRowAuthPrmId237hi";
			cAdmAtRowAuthGrid_OnSorting(sender, new ListViewSortEventArgs("RowAuthPrmId237", SortDirection.Ascending));
		}

		protected void cPermKeyId237hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "1") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "1"; Session[KEY_lastSortExp] = "PermKeyId237Text";Session[KEY_lastSortImg] = "cPermKeyId237hi";
			cAdmAtRowAuthGrid_OnSorting(sender, new ListViewSortEventArgs("PermKeyId237Text", SortDirection.Ascending));
		}

		protected void cSelLevel237hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "2") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "2"; Session[KEY_lastSortExp] = "SelLevel237Text";Session[KEY_lastSortImg] = "cSelLevel237hi";
			cAdmAtRowAuthGrid_OnSorting(sender, new ListViewSortEventArgs("SelLevel237Text", SortDirection.Ascending));
		}

		protected void cAdmAtRowAuthGrid_OnItemEditing(object sender, ListViewEditEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmAtRowAuthGrid];
			if ((ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")) && dt != null && dt.DefaultView.Count > GetDataItemIndex(e.NewEditIndex))
			{
				cAdmAtRowAuthGrid.EditIndex = e.NewEditIndex;
				cAdmAtRowAuthGrid_DataBind(null);
				Control wc = null;
				string idx = Page.Request["__EVENTARGUMENT"];
				if (!string.IsNullOrEmpty(idx))
				{
				    Control ctrlToFocus = cAdmAtRowAuthGrid.EditItem.FindControl("c" + idx);
				    if (((WebControl)ctrlToFocus).Enabled) { wc = ctrlToFocus; }
				}
				if (wc == null) { wc = FindEditableControl(cAdmAtRowAuthGrid.EditItem); }
				if (wc != null)
             {
             ...
             MkMobileLabel(cAdmAtRowAuthGrid.EditItem);
		    }
		    else { e.Cancel = true; }
		}
		...
		{
		    Label ml = null;
		    ml = lvi.FindControl("cRowAuthPrmId237ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(8); }
		    ml = lvi.FindControl("cPermKeyId237ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(9); }
		    ml = lvi.FindControl("cSelLevel237ml") as Label;
		...
		protected void GridFill(ListViewItem lvi, DataTable dt, DataRow dr, bool bInsert)
		{
			...
			ddl = (DropDownList)lvi.FindControl("cPermKeyId237");
			if (ddl != null)
			{
				if (ddl.SelectedIndex >= 0 && ddl.SelectedValue.ToString() != string.Empty)
				{
					dr["PermKeyId237"] = ddl.SelectedValue;
					dr["PermKeyId237Text"] = ddl.SelectedItem.Text;
				}
				else
				{
					dr["PermKeyId237"] = System.DBNull.Value;
					dr["PermKeyId237Text"] = System.DBNull.Value;
				}
			}
			ddl = (DropDownList)lvi.FindControl("cSelLevel237");
			if (ddl != null)
			{
				if (ddl.SelectedIndex >= 0 && ddl.SelectedValue.ToString() != string.Empty)
				{
					dr["SelLevel237"] = ddl.SelectedValue;
					dr["SelLevel237Text"] = ddl.SelectedItem.Text;
				}
				else
				{
					dr["SelLevel237"] = System.DBNull.Value;
					dr["SelLevel237Text"] = System.DBNull.Value;
				}
			}
		...
		protected void cAdmAtRowAuthGrid_OnItemUpdating(object sender, ListViewUpdateEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmAtRowAuthGrid];
			if (dt != null && dt.Rows.Count > 0)
			{
				DataRow dr = dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row;
			    GridFill(cAdmAtRowAuthGrid.EditItem, dt, dr, false);
			    dr["_NewRow"] = "N";
			    if (dt.Columns.Contains("_SortOrder")) dr["_SortOrder"] = null;
			}
		}

		protected void cAdmAtRowAuthGrid_OnItemCanceling(object sender, ListViewCancelEventArgs e)
		{
			PanelTop.Update();
			DataTable dt = (DataTable)Session[KEY_dtAdmAtRowAuthGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			...
			{
				cAdmAtRowAuthGrid_OnItemDeleting(sender, new ListViewDeleteEventArgs(e.ItemIndex));
			}
			if (dt.Columns.Contains("_SortOrder")) dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row["_SortOrder"] = null;
			cAdmAtRowAuthGrid.EditIndex = -1; cAdmAtRowAuthGrid_DataBind(null);
		}
		...
		protected void cDeleteAllButton_Click(object sender, System.EventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmAtRowAuthGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null && (cAdmAtRowAuthGrid.EditIndex < 0 || UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				GotoPage(0);
				while (dv.Count > 0) {dv.Delete(0);}
				Session[KEY_dtAdmAtRowAuthGrid] = dt;
				cAdmAtRowAuthGrid.EditIndex = -1; cAdmAtRowAuthGrid_DataBind(dv);
			}
		}

		protected void cAdmAtRowAuthGrid_OnItemDeleting(object sender, ListViewDeleteEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmAtRowAuthGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			DataRow dr = dv != null ? dv[GetDataItemIndex(e.ItemIndex)].Row : null;
			if (dv != null && (cAdmAtRowAuthGrid.EditIndex == e.ItemIndex || UpdateGridRow(sender, new CommandEventArgs("Save",""))))
			{
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cRowAuthId236.Text == string.Empty) { cClearButton_Click(sender, new EventArgs()); ShowDirty(false); PanelTop.Update();}
			else { PopAdmAtRowAuth22List(sender, e, false, cRowAuthId236.Text); }
			// *** System Button Click (After) Web Rule starts here *** //
		}
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmAtRowAuth22List); PopAdmAtRowAuth22List(sender, e, false, null);
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			if (cOvrideId236Pwd.Visible) {cOvrideId236Pwd_TextChanged(cOvrideId236Pwd,new EventArgs());}
			if (ValidPage() && !cOvrideId236Pwd.Visible && (cAdmAtRowAuthGrid.EditIndex < 0 || UpdateGridRow(cAdmAtRowAuthGrid, new CommandEventArgs("Save", ""))))
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmAtRowAuthGrid];
				DataView dv = dt != null ? dt.DefaultView : null;
				string ftr = string.Empty;
				if (dv.RowFilter != string.Empty) { ftr = dv.RowFilter; dv.RowFilter = string.Empty; }
				AdmAtRowAuth22 ds = PrepAdmAtRowAuthData(dv,cRowAuthId236.Text == string.Empty);
				if (ftr != string.Empty) {dv.RowFilter = ftr;}
				if (!string.IsNullOrEmpty(cAdmAtRowAuth22List.SelectedValue) && cRowAuthId236.Text != cAdmAtRowAuth22List.SelectedValue)
				{
					PreMsgPopup("Primary key cannot be changed");
					return rtn;
				}
				if (string.IsNullOrEmpty(cAdmAtRowAuth22List.SelectedValue))	// Add
				{
					if (ds != null)
					{
						pid = (new AdminSystem()).AddData(22,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc,noTrans,commandTimeOut);
					}
					if (!string.IsNullOrEmpty(pid))
					{
						cAdmAtRowAuth22List.ClearSearch(); Session.Remove(KEY_dtAdmAtRowAuth22List);
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmAtRowAuth22List(sender, e, false, pid);
						rtn = GetScreenHlp().Rows[0]["AddMsg"].ToString();
					}
				}
				else {
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(22,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc,noTrans,commandTimeOut)) {bValid7 = true;}
					if (bValid7)
					{
						cAdmAtRowAuth22List.ClearSearch(); Session.Remove(KEY_dtAdmAtRowAuth22List);
						Session[KEY_currPageIndex] = GetCurrPageIndex();
						ShowDirty(false); PopAdmAtRowAuth22List(sender, e, false, ds.Tables["AdmAtRowAuth"].Rows[0]["RowAuthId236"]);
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cRowAuthId236.Text != string.Empty)
			{
				AdmAtRowAuth22 ds = PrepAdmAtRowAuthData(null,false);
				try
				{
					if (ds != null && (new AdminSystem()).DelData(22,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc))
					{
						cAdmAtRowAuth22List.ClearSearch(); Session.Remove(KEY_dtAdmAtRowAuth22List);
						ShowDirty(false); PopAdmAtRowAuth22List(sender, e, false, null);
		...
		private AdmAtRowAuth22 PrepAdmAtRowAuthData(DataView dv, bool bAdd)
		{
			...
			if (bAdd) { dr["RowAuthId236"] = string.Empty; } else { dr["RowAuthId236"] = cRowAuthId236.Text; }
			drType["RowAuthId236"] = "Numeric"; drDisp["RowAuthId236"] = "TextBox";
			try {dr["RowAuthName236"] = cRowAuthName236.Text.Trim();} catch {}
			drType["RowAuthName236"] = "VarChar"; drDisp["RowAuthName236"] = "TextBox";
			try {dr["OvrideId236"] = cOvrideId236.SelectedValue;} catch {}
			drType["OvrideId236"] = "Numeric"; drDisp["OvrideId236"] = "DropDownList";
			try {dr["AllowSel236"] = cAllowSel236.SelectedValue;} catch {}
			drType["AllowSel236"] = "Char"; drDisp["AllowSel236"] = "DropDownList";
			try {dr["AllowAdd236"] = base.SetBool(cAllowAdd236.Checked);} catch {}
			drType["AllowAdd236"] = "Char"; drDisp["AllowAdd236"] = "CheckBox";
			try {dr["AllowUpd236"] = base.SetBool(cAllowUpd236.Checked);} catch {}
			drType["AllowUpd236"] = "Char"; drDisp["AllowUpd236"] = "CheckBox";
			try {dr["AllowDel236"] = base.SetBool(cAllowDel236.Checked);} catch {}
			drType["AllowDel236"] = "Char"; drDisp["AllowDel236"] = "CheckBox";
			try {dr["SysAdmin236"] = base.SetBool(cSysAdmin236.Checked);} catch {}
			...
			if (dv != null)
			{
				...
				if (bAdd)
				{
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmAtRowAuthAdd"].Rows.Add(MakeColRow(ds.Tables["AdmAtRowAuthAdd"].NewRow(), drv, true));
					}
				}
				else
				{
					...
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmAtRowAuthUpd"].Rows.Add(MakeColRow(ds.Tables["AdmAtRowAuthUpd"].NewRow(), drv, false));
					}
					...
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmAtRowAuthDel"].Rows.Add(MakeColRow(ds.Tables["AdmAtRowAuthDel"].NewRow(), drv, false));
					}
		...
		public bool CanAct(char typ) { return CanAct(typ, GetAuthRow(), cAdmAtRowAuth22List.SelectedValue.ToString()); }
		...
		{
			dr["RowAuthId236"] = cRowAuthId236.Text;
			DataTable dtAuth = GetAuthCol();
			if (dtAuth != null)
			{
				dr["RowAuthPrmId237"] = drv["RowAuthPrmId237"].ToString().Trim();
		...
		{
			if (cAdmAtRowAuthGrid.EditIndex > -1)
			{
				cAdmAtRowAuthGrid_OnItemUpdating(sender, new ListViewUpdateEventArgs(cAdmAtRowAuthGrid.EditIndex));
			}
			return true;
		}

		protected void cAdmAtRowAuthGrid_OnPreRender(object sender, System.EventArgs e)
		{
			System.Web.UI.WebControls.Image hi = null;
			hi = (System.Web.UI.WebControls.Image)cAdmAtRowAuthGrid.FindControl("cRowAuthPrmId237hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmAtRowAuthGrid.FindControl("cPermKeyId237hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmAtRowAuthGrid.FindControl("cSelLevel237hi"); if (hi != null) { hi.Visible = false; }
			if (Session[KEY_lastSortImg] != null)
			{
				hi = (System.Web.UI.WebControls.Image)cAdmAtRowAuthGrid.FindControl((string)Session[KEY_lastSortImg]);
				if (hi != null) { hi.ImageUrl = Utils.AddTilde((string)Session[KEY_lastSortUrl]); hi.Visible = true; }
			}
			IgnoreHeaderConfirm((LinkButton)cAdmAtRowAuthGrid.FindControl("cRowAuthPrmId237hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmAtRowAuthGrid.FindControl("cPermKeyId237hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmAtRowAuthGrid.FindControl("cSelLevel237hl"));
		}

		private string GetButtonId(ListViewItem lvi)
		{
			string ButtonID = String.Empty;
			Control c = lvi.FindControl("cAdmAtRowAuthGridEdit");
		...
		{
		    return cAdmAtRowAuthGridDataPager.StartRowIndex / cAdmAtRowAuthGridDataPager.PageSize;
		}

		private int GetTotalPages()
		{
		    return (int)Math.Ceiling((double)cAdmAtRowAuthGridDataPager.TotalRowCount / cAdmAtRowAuthGridDataPager.PageSize);
		}

		private int GetDataItemIndex(int editIndex)
		{
		    return cAdmAtRowAuthGridDataPager.StartRowIndex + editIndex;
		}

		protected void cAdmAtRowAuthGrid_OnLayoutCreated(object sender, EventArgs e)
		{
		    // Header:
		    LinkButton lb = null;
		    lb = cAdmAtRowAuthGrid.FindControl("cRowAuthPrmId237hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(8); lb.ToolTip = ColumnToolTip(8); lb.Parent.Visible = GridColumnVisible(8); }
		    lb = cAdmAtRowAuthGrid.FindControl("cPermKeyId237hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(9); lb.ToolTip = ColumnToolTip(9); lb.Parent.Visible = GridColumnVisible(9); }
		    lb = cAdmAtRowAuthGrid.FindControl("cSelLevel237hl") as LinkButton;
			...
			if (dtAuthRow != null)
			{
				...
				{
		            lb = cAdmAtRowAuthGrid.FindControl("cDeleteAllButton") as LinkButton; if (lb != null) { lb.Visible = false; }
				}
			}
		    // footer:
		    Label gc = null;
		    gc = cAdmAtRowAuthGrid.FindControl("cRowAuthPrmId237fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(8); }
		    gc = cAdmAtRowAuthGrid.FindControl("cPermKeyId237fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(9); }
		    gc = cAdmAtRowAuthGrid.FindControl("cSelLevel237fl") as Label;
		...
		private void PreMsgPopup(string msg) { PreMsgPopup(msg,cAdmAtRowAuth22List,null); }

...
{
	public class AdmButtonHlp76 : DataSet
	{
		public AdmButtonHlp76()
		{
			this.Tables.Add(MakeDtlColumns(new DataTable("AdmButtonHlpDef")));
			this.Tables.Add(MakeDtlColumns(new DataTable("AdmButtonHlpAdd")));
			this.Tables.Add(MakeDtlColumns(new DataTable("AdmButtonHlpUpd")));
			this.Tables.Add(MakeDtlColumns(new DataTable("AdmButtonHlpDel")));
			this.DataSetName = "AdmButtonHlp76";
			this.Namespace = "http://Rintagi.com/DataSet/AdmButtonHlp76";
		}
...
namespace RO.Web
{
	public partial class AdmButtonHlpModule : RO.Web.ModuleBase
	{

		private const string KEY_lastAddedRow = "Cache:lastAddedRow3_76";
		private const string KEY_lastSortOrd = "Cache:lastSortOrd3_76";
		private const string KEY_lastSortImg = "Cache:lastSortImg3_76";
		private const string KEY_lastSortCol = "Cache:lastSortCol3_76";
		private const string KEY_lastSortExp = "Cache:lastSortExp3_76";
		private const string KEY_lastSortUrl = "Cache:lastSortUrl3_76";
		private const string KEY_lastSortTog = "Cache:lastSortTog3_76";
		private const string KEY_lastImpPwdOvride = "Cache:lastImpPwdOvride3_76";
		private const string KEY_cntImpPwdOvride = "Cache:cntImpPwdOvride3_76";
		private const string KEY_currPageIndex = "Cache:currPageIndex3_76";
		private const string KEY_bHiImpVisible = "Cache:bHiImpVisible3_76";
		private const string KEY_bShImpVisible = "Cache:bShImpVisible3_76";
		private const string KEY_dtAdmButtonHlpGrid = "Cache:dtAdmButtonHlpGrid";
		private const string KEY_scrImport = "Cache:scrImport3_76";
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_76";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_76";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_76";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_76";
		private const string KEY_dtLabel = "Cache:dtLabel3_76";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_76";
		private const string KEY_dtScrCri = "Cache:dtScrCri3_76";
		private const string KEY_dsScrCriVal = "Cache:dsScrCriVal3_76";

		private const string KEY_dtCultureId116 = "Cache:dtCultureId116";
		private const string KEY_dtButtonTypeId116 = "Cache:dtButtonTypeId116";
		...
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_76";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_76";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_76";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				DataTable dtMenuAccess = (new MenuSystem()).GetMenu(base.LUser.CultureId, 3, base.LImpr, LcSysConnString, LcAppPw,76, null, null);
				...
				Session.Remove(KEY_dtAdmButtonHlpGrid);
				...
				Session.Remove(KEY_dtCultureId116);
				Session.Remove(KEY_dtButtonTypeId116);
				...
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 76, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmButtonHlp76List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=76&typ=N&sys=3','','',''); return false;"; }
		...
		private void InitializeComponent()
		{
			...
			if (LcSysConnString == null)
			{
				...
				{
					DataTable dt;
					dt = (new AdminSystem()).GetAuthRow(76,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
					...
					{
						cAdmButtonHlpGrid.PreRender += new EventHandler(cAdmButtonHlpGrid_OnPreRender);
					}
				}
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(76,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(76,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void SetClientRule(ListViewDataItem ee, bool isEditItem)
		{
			...
			if (dvRul.Count > 0)
			{
				...
				{
					if (ee == null || drv["ScriptEvent"].ToString().Substring(0,2).ToLower() != "on" || (cAdmButtonHlpGrid.EditIndex > -1 && GetDataItemIndex(cAdmButtonHlpGrid.EditIndex) == ee.DataItemIndex))
					{
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(76,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(76,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,76,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(76,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(76,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(76,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				CheckAuthentication(false);
				DataView dv = null;
				try {dv = ((DataTable)Session[KEY_dtAdmButtonHlpGrid]).DefaultView;}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(76,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmButtonHlp.csv"; Session["ExportStr"] = (Config.ExportExcelCSV ? "sep=\t\n": "") + sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmButtonHlp.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private void GetPageInfo()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastPageInfo(76, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 76, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			// *** Criteria Trigger (before) Web Rule starts here *** //
			DataTable dt = (DataTable)Session[KEY_dtAdmButtonHlpGrid];
			if (cAdmButtonHlpGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
			...
			try
			{
				foreach (DataRowView drv in dvCri)
				{
					...
					if (drv["DisplayName"].ToString() == "ComboBox")
					{
						...
						(new AdminSystem()).MkGetScreenIn("76", drv["ScreenCriId"].ToString(), "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), LcAppDb, LcDesDb, drv["MultiDesignDb"].ToString(), LcSysConnString, LcAppPw);
						DataView dv = new DataView((new AdminSystem()).GetScreenIn("76", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), (new AdminSystem()).CountScrCri(drv["ScreenCriId"].ToString(), drv["MultiDesignDb"].ToString(), drv["MultiDesignDb"].ToString() == "N" ? LcSysConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)), drv["RequiredValid"].ToString(), 0, string.Empty, drv["DisplayMode"].ToString() != "AutoListBox", val, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)));
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("76", LcSysConnString, LcAppPw);
				}
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmButtonHlp76" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmButtonHlp76" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			foreach (DataRowView drv in dvCri)
			{
			    if (drv["DisplayName"].ToString() == "ListBox")
			    {
					...
					if (cListBox != null)
					{
						...
						int TotalChoiceCnt = new DataView((new AdminSystem()).GetScreenIn("76", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), CriCnt, drv["RequiredValid"].ToString(), 0, string.Empty, true, string.Empty, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId))).Count;
						string selectedValues = string.Join(",", cListBox.Items.Cast<ListItem>().Where(x => x.Selected).Select(x => "'" + x.Value + "'").ToArray());
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("76", "AdmButtonHlp76", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		protected void cbCultureId116(object sender, System.EventArgs e)
		{
			SetCultureId116((RoboCoder.WebControls.ComboBox)sender,string.Empty,cAdmButtonHlpGrid.EditItem);
		}

		private void SetCultureId116(RoboCoder.WebControls.ComboBox ddl, string keyId, ListViewItem li)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			context["method"] = "GetDdlCultureId3S1209";
			context["addnew"] = "Y";
			context["mKey"] = "CultureId116";
			context["mVal"] = "CultureId116Text";
			context["mTip"] = "CultureId116Text";
			context["mImg"] = "CultureId116Text";
			context["ssd"] = Request.QueryString["ssd"];
			context["scr"] = "76";
			context["csy"] = "3";
			context["filter"] = Utils.IsInt(cFilterId.SelectedValue)? cFilterId.SelectedValue : "0";
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(76,"GetDdlCultureId3S1209",true,false,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
					ddl.DataSource = dv; Session[KEY_dtCultureId116] = dv.Table;
				    try { ddl.SelectByValue(keyId,string.Empty,true); } catch { try { ddl.SelectedIndex = 0; } catch { } }
				}
			}
		}

		protected void cbButtonTypeId116(object sender, System.EventArgs e)
		{
			SetButtonTypeId116((RoboCoder.WebControls.ComboBox)sender,string.Empty,cAdmButtonHlpGrid.EditItem);
		}

		private void SetButtonTypeId116(RoboCoder.WebControls.ComboBox ddl, string keyId, ListViewItem li)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			context["method"] = "GetDdlButtonTypeId3S1210";
			context["addnew"] = "Y";
			context["mKey"] = "ButtonTypeId116";
			context["mVal"] = "ButtonTypeId116Text";
			context["mTip"] = "ButtonTypeId116Text";
			context["mImg"] = "ButtonTypeId116Text";
			context["ssd"] = Request.QueryString["ssd"];
			context["scr"] = "76";
			context["csy"] = "3";
			context["filter"] = Utils.IsInt(cFilterId.SelectedValue)? cFilterId.SelectedValue : "0";
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(76,"GetDdlButtonTypeId3S1210",true,false,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
					ddl.DataSource = dv; Session[KEY_dtButtonTypeId116] = dv.Table;
				    try { ddl.SelectByValue(keyId,string.Empty,true); } catch { try { ddl.SelectedIndex = 0; } catch { } }
				}
			}
		}

		protected void cbScreenId116(object sender, System.EventArgs e)
		{
			SetScreenId116((RoboCoder.WebControls.ComboBox)sender,string.Empty,cAdmButtonHlpGrid.EditItem);
		}

		private void SetScreenId116(RoboCoder.WebControls.ComboBox ddl, string keyId, ListViewItem li)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			context["method"] = "GetDdlScreenId3S1208";
			context["addnew"] = "Y";
			context["mKey"] = "ScreenId116";
			context["mVal"] = "ScreenId116Text";
			context["mTip"] = "ScreenId116Text";
			context["mImg"] = "ScreenId116Text";
			context["ssd"] = Request.QueryString["ssd"];
			context["scr"] = "76";
			context["csy"] = "3";
			context["filter"] = Utils.IsInt(cFilterId.SelectedValue)? cFilterId.SelectedValue : "0";
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(76,"GetDdlScreenId3S1208",true,false,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
					ddl.DataSource = dv; Session[KEY_dtScreenId116] = dv.Table;
				    try { ddl.SelectByValue(keyId,string.Empty,true); } catch { try { ddl.SelectedIndex = 0; } catch { } }
				}
			}
		}

		protected void cbReportId116(object sender, System.EventArgs e)
		{
			SetReportId116((RoboCoder.WebControls.ComboBox)sender,string.Empty,cAdmButtonHlpGrid.EditItem);
		}

		private void SetReportId116(RoboCoder.WebControls.ComboBox ddl, string keyId, ListViewItem li)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			context["method"] = "GetDdlReportId3S1214";
			context["addnew"] = "Y";
			context["mKey"] = "ReportId116";
			context["mVal"] = "ReportId116Text";
			context["mTip"] = "ReportId116Text";
			context["mImg"] = "ReportId116Text";
			context["ssd"] = Request.QueryString["ssd"];
			context["scr"] = "76";
			context["csy"] = "3";
			context["filter"] = Utils.IsInt(cFilterId.SelectedValue)? cFilterId.SelectedValue : "0";
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(76,"GetDdlReportId3S1214",true,false,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
					ddl.DataSource = dv; Session[KEY_dtReportId116] = dv.Table;
				    try { ddl.SelectByValue(keyId,string.Empty,true); } catch { try { ddl.SelectedIndex = 0; } catch { } }
				}
			}
		}

		protected void cbWizardId116(object sender, System.EventArgs e)
		{
			SetWizardId116((RoboCoder.WebControls.ComboBox)sender,string.Empty,cAdmButtonHlpGrid.EditItem);
		}

		private void SetWizardId116(RoboCoder.WebControls.ComboBox ddl, string keyId, ListViewItem li)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			context["method"] = "GetDdlWizardId3S1215";
			context["addnew"] = "Y";
			context["mKey"] = "WizardId116";
			context["mVal"] = "WizardId116Text";
			context["mTip"] = "WizardId116Text";
			context["mImg"] = "WizardId116Text";
			context["ssd"] = Request.QueryString["ssd"];
			context["scr"] = "76";
			context["csy"] = "3";
			context["filter"] = Utils.IsInt(cFilterId.SelectedValue)? cFilterId.SelectedValue : "0";
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(76,"GetDdlWizardId3S1215",true,false,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
					ddl.DataSource = dv; Session[KEY_dtWizardId116] = dv.Table;
				    try { ddl.SelectByValue(keyId,string.Empty,true); } catch { try { ddl.SelectedIndex = 0; } catch { } }
				}
			}
		}
		...
		private void SetTopVisible116(DropDownList ddl, string keyId)
		{
			...
			if (ddl != null)
			{
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(76,"GetDdlTopVisible3S4161",true,bAll,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(76,"GetDdlTopVisible3S4161",true,true,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
		...
		private void SetRowVisible116(DropDownList ddl, string keyId)
		{
			...
			if (ddl != null)
			{
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(76,"GetDdlRowVisible3S4160",true,bAll,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(76,"GetDdlRowVisible3S4160",true,true,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
		...
		private void SetBotVisible116(DropDownList ddl, string keyId)
		{
			...
			if (ddl != null)
			{
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(76,"GetDdlBotVisible3S4163",true,bAll,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(76,"GetDdlBotVisible3S4163",true,true,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
		...
		private void PopAdmButtonHlp76List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			...
			try
			{
				try {dt = (new AdminSystem()).GetLis(76,"GetLisAdmButtonHlp76",false,"Y",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				catch {dt = (new AdminSystem()).GetLis(76,"GetLisAdmButtonHlp76",false,"N",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				cAdmButtonHlpGridDataPager.PageSize = Int16.Parse(cPgSize.Text); GotoPage(0);
			    if (Session[KEY_lastSortCol] != null)
			    {
					cAdmButtonHlpGrid_OnSorting(sender, new ListViewSortEventArgs((string)Session[KEY_lastSortExp], SortDirection.Ascending));
			    }
				else
			    {
					cAdmButtonHlpGrid_DataBind(dt.DefaultView);
			    }
		...
		public void cFindButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dv != null)
			{
				if (cAdmButtonHlpGrid.EditIndex < 0 || UpdateGridRow(cAdmButtonHlpGrid, new CommandEventArgs("Save", "")))
				{
					...
					if (GetCurrPageIndex() != (int)Session[KEY_currPageIndex] && (int)Session[KEY_currPageIndex] < GetTotalPages()) { GotoPage((int)Session[KEY_currPageIndex]); cAdmButtonHlpGrid_DataBind(dv);}
		...
		public void cInsRowButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dt != null && (ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")))
			{
				...
				Session[KEY_lastAddedRow] = 0; Session[KEY_dtAdmButtonHlpGrid] = dt; Session[KEY_currPageIndex] = 0; GotoPage(0);
				cAdmButtonHlpGrid_DataBind(dt.DefaultView);
				cAdmButtonHlpGrid_OnItemEditing(cAdmButtonHlpGrid, new ListViewEditEventArgs(0));
		...
		public void cPgSizeButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cAdmButtonHlpGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				...
				cAdmButtonHlpGridDataPager.PageSize = int.Parse(cPgSize.Text); GotoPage(0);
				if (dt.Rows.Count <= 0 || (GetAuthRow().Rows[0]["AllowAdd"].ToString() == "Y" && dt.Rows.Count == 1)) {cAdmButtonHlpGrid_DataBind(dt.DefaultView);} else {cFindButton_Click(sender, e);}
				try
				{
					(new AdminSystem()).UpdLastPageInfo(76, base.LUser.UsrId, cPgSize.Text, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		}
		...
		public void cShowImpButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cSchemaImage.Visible)
			{
				try
				{
					Session["ImportSchema"] = (new AdminSystem()).GetSchemaScrImp(76,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (cSchemaImage.Attributes["OnClick"] == null || cSchemaImage.Attributes["OnClick"].IndexOf("ImportSchema.aspx") < 0) {cSchemaImage.Attributes["OnClick"] += "SearchLink('ImportSchema.aspx?scm=S&key=76&csy=3','','',''); return false;";}
			}
			// *** System Button Click (after) Web Rule starts here *** //
		}
		...
		protected void ScrImport(bool bClear)
		{
			...
			DataTable dtAdmButtonHlpGrid = (DataTable)Session[KEY_dtAdmButtonHlpGrid];
			if (dti != null && dtAdmButtonHlpGrid != null)
			{
				if (bClear)
				{
					dtAdmButtonHlpGrid.Rows.Clear();
				}
				cFind.Text = string.Empty;
				int iExist = dtAdmButtonHlpGrid.Rows.Count;
				...
				foreach (DataRowView drv in dti.DefaultView)
				{
					...
					{
						DataTable dtd;
						dtd = jj==iExist ? null : (DataTable)Session[KEY_dtCultureId116];
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(76,"GetDdlCultureId3S1209",true,true,0,string.Empty,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
							Session[KEY_dtCultureId116] = dtd;
						}
						ScrImportDdl(dtd.DefaultView, drv, true, "ButtonHlpId", "CultureId116", "CultureId116Text", 50, "2", false);
						dtd = jj==iExist ? null : (DataTable)Session[KEY_dtButtonTypeId116];
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(76,"GetDdlButtonTypeId3S1210",true,true,0,string.Empty,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
							Session[KEY_dtButtonTypeId116] = dtd;
						}
						ScrImportDdl(dtd.DefaultView, drv, true, "ButtonHlpId", "ButtonTypeId116", "ButtonTypeId116Text", 50, "2", false);
						dtd = jj==iExist ? null : (DataTable)Session[KEY_dtScreenId116];
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(76,"GetDdlScreenId3S1208",true,true,0,string.Empty,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
							Session[KEY_dtScreenId116] = dtd;
						}
						ScrImportDdl(dtd.DefaultView, drv, true, "ButtonHlpId", "ScreenId116", "ScreenId116Text", 50, "2", true);
						dtd = jj==iExist ? null : (DataTable)Session[KEY_dtReportId116];
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(76,"GetDdlReportId3S1214",true,true,0,string.Empty,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
							Session[KEY_dtReportId116] = dtd;
						}
						ScrImportDdl(dtd.DefaultView, drv, true, "ButtonHlpId", "ReportId116", "ReportId116Text", 50, "2", true);
						dtd = jj==iExist ? null : (DataTable)Session[KEY_dtWizardId116];
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(76,"GetDdlWizardId3S1215",true,true,0,string.Empty,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						...
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(76,"GetDdlTopVisible3S4161",true,true,0,string.Empty,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						...
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(76,"GetDdlRowVisible3S4160",true,true,0,string.Empty,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						...
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(76,"GetDdlBotVisible3S4163",true,true,0,string.Empty,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				...
				Session[KEY_dtAdmButtonHlpGrid] = dti;
				cAdmButtonHlpGrid_DataBind(dti.DefaultView);
		...
		public void cImportButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cFNameO.Text != string.Empty && cWorkSheet.Items.Count > 0 && cWorkSheet.SelectedItem.Text != string.Empty && cStartRow.Text != string.Empty)
			{
				...
				try
				{
					if (cAdmButtonHlpGrid.EditIndex < 0 || UpdateGridRow(cAdmButtonHlpGrid, new CommandEventArgs("Save", "")))
					{
						Session[KEY_scrImport] = ImportFile(dtAdmButtonHlpGrid.Copy(),cFNameO.Text,cWorkSheet.SelectedItem.Text,cStartRow.Text,Config.PathTmpImport + cFName.Text);
					}
		...
		private void GotoPage(int pageNo)
		{
			if (pageNo >= 0 && pageNo < GetTotalPages())
			{
				if (cAdmButtonHlpGrid.EditIndex < 0 || UpdateGridRow(cAdmButtonHlpGrid, new CommandEventArgs("Save", "")))
				{
					try { cAdmButtonHlpGridDataPager.SetPageProperties(cAdmButtonHlpGridDataPager.PageSize * pageNo, cAdmButtonHlpGridDataPager.MaximumRows, false); cAdmButtonHlpGrid_DataBind(null); }
					catch
					{
						try { cAdmButtonHlpGridDataPager.SetPageProperties(0, cAdmButtonHlpGridDataPager.MaximumRows, false); cAdmButtonHlpGrid_DataBind(null); } catch {}
					}
				}
			}
		}

		protected void cAdmButtonHlpGrid_OnItemCommand(object sender, ListViewCommandEventArgs e)
		{
		}

		private void cAdmButtonHlpGrid_DataBind(DataView dvAdmButtonHlpGrid)
		{
			if (dvAdmButtonHlpGrid == null)
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmButtonHlpGrid];
				dvAdmButtonHlpGrid = dt != null ? dt.DefaultView : null;
			}
			if (dvAdmButtonHlpGrid != null)
			{
				LcAuth = GetAuthCol().AsEnumerable().ToDictionary<DataRow,string>(dr=>dr["ColName"].ToString());
				cAdmButtonHlpGrid.DataSource = dvAdmButtonHlpGrid;
				cAdmButtonHlpGrid.DataBind();
		...
		protected void cAdmButtonHlpGrid_OnSorting(object sender, ListViewSortEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmButtonHlpGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null)
			{
				if (cAdmButtonHlpGrid.EditIndex < 0 || UpdateGridRow(cAdmButtonHlpGrid, new CommandEventArgs("Save", "")))
				{
					...
					Session[KEY_dtAdmButtonHlpGrid] = dt;
					cAdmButtonHlpGrid_DataBind(dv);
				}
			}
		}

		protected void cAdmButtonHlpGrid_OnPagePropertiesChanging(object sender, PagePropertiesChangingEventArgs e)
		{
		    cAdmButtonHlpGridDataPager.SetPageProperties(e.StartRowIndex, e.MaximumRows, false);
		    DataTable dt = (DataTable)Session[KEY_dtAdmButtonHlpGrid];
		    DataView dv = dt != null ? dt.DefaultView : null;
			cAdmButtonHlpGrid_DataBind(dv); cAdmButtonHlpGrid.EditIndex = -1;
		}

		private void GridChkPgDirty(ListViewItem lvi)
		{
				WebControl cc = null;
				cc = ((WebControl)lvi.FindControl("cCultureId116"));
				...
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cWizardId116"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cButtonToolTip116"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
		...
		protected void cAdmButtonHlpGrid_OnItemDataBound(object sender, ListViewItemEventArgs e)
		{
			...
			if (cAdmButtonHlpGrid.EditIndex > -1 && GetDataItemIndex(cAdmButtonHlpGrid.EditIndex) == e.Item.DataItemIndex)
			{
				...
				if (dvAdmButtonHlpGrid[e.Item.DataItemIndex]["CultureId116"].ToString() != string.Empty) {SetCultureId116((RoboCoder.WebControls.ComboBox)e.Item.FindControl("cCultureId116"),dvAdmButtonHlpGrid[e.Item.DataItemIndex]["CultureId116"].ToString(), e.Item);} else {SetCultureId116((RoboCoder.WebControls.ComboBox)e.Item.FindControl("cCultureId116"),"1", e.Item);}
				SetButtonTypeId116((RoboCoder.WebControls.ComboBox)e.Item.FindControl("cButtonTypeId116"),dvAdmButtonHlpGrid[e.Item.DataItemIndex]["ButtonTypeId116"].ToString(), e.Item);
				SetScreenId116((RoboCoder.WebControls.ComboBox)e.Item.FindControl("cScreenId116"),dvAdmButtonHlpGrid[e.Item.DataItemIndex]["ScreenId116"].ToString(), e.Item);
				SetReportId116((RoboCoder.WebControls.ComboBox)e.Item.FindControl("cReportId116"),dvAdmButtonHlpGrid[e.Item.DataItemIndex]["ReportId116"].ToString(), e.Item);
				SetWizardId116((RoboCoder.WebControls.ComboBox)e.Item.FindControl("cWizardId116"),dvAdmButtonHlpGrid[e.Item.DataItemIndex]["WizardId116"].ToString(), e.Item);
				CheckBox cb = null;
				cb = (CheckBox)e.Item.FindControl("cButtonVisible116");
				if (cb != null)
				{
			...
			if (dtAuthRow != null)
			{
				...
				{
					cc = e.Item.FindControl("cAdmButtonHlpGridDelete"); if (cc != null) { cc.Visible = false; }
				}
				if (!AllowRowEdit(GetAuthRow(),dvAdmButtonHlpGrid[e.Item.DataItemIndex]["ButtonHlpId116"].ToString()) || (Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N"))
				{
					cc = e.Item.FindControl("cAdmButtonHlpGridEdit"); if (cc != null) { cc.Visible = false; }
				}
				else
				{
			        HtmlTableRow tr = e.Item.FindControl("cAdmButtonHlpGridRow") as HtmlTableRow;
			        LinkButton lb = e.Item.FindControl("cAdmButtonHlpGridEdit") as LinkButton;
		...
		protected void cButtonHlpId116hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "0") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "0"; Session[KEY_lastSortExp] = "ButtonHlpId116";Session[KEY_lastSortImg] = "cButtonHlpId116hi";
			cAdmButtonHlpGrid_OnSorting(sender, new ListViewSortEventArgs("ButtonHlpId116", SortDirection.Ascending));
		}

		protected void cCultureId116hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "1") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "1"; Session[KEY_lastSortExp] = "CultureId116Text";Session[KEY_lastSortImg] = "cCultureId116hi";
			cAdmButtonHlpGrid_OnSorting(sender, new ListViewSortEventArgs("CultureId116Text", SortDirection.Ascending));
		}

		protected void cButtonTypeId116hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "2") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "2"; Session[KEY_lastSortExp] = "ButtonTypeId116Text";Session[KEY_lastSortImg] = "cButtonTypeId116hi";
		...
		protected void cAdmButtonHlpGrid_OnItemEditing(object sender, ListViewEditEventArgs e)
		{
			...
			{
				cAdmButtonHlpGrid.EditIndex = e.NewEditIndex;
				cAdmButtonHlpGrid_DataBind(null);
				Control wc = null;
				string idx = Page.Request["__EVENTARGUMENT"];
				if (!string.IsNullOrEmpty(idx))
				{
				    Control ctrlToFocus = cAdmButtonHlpGrid.EditItem.FindControl("c" + idx);
				    if (((WebControl)ctrlToFocus).Enabled) { wc = ctrlToFocus; }
				}
				if (wc == null) { wc = FindEditableControl(cAdmButtonHlpGrid.EditItem); }
				if (wc != null)
             {
             ...
             MkMobileLabel(cAdmButtonHlpGrid.EditItem);
		    }
		    else { e.Cancel = true; }
		}
		...
		{
		    Label ml = null;
		    ml = lvi.FindControl("cButtonHlpId116ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(0); }
		    ml = lvi.FindControl("cCultureId116ml") as Label;
		...
		protected void GridFill(ListViewItem lvi, DataTable dt, DataRow dr, bool bInsert)
		{
			...
			cbb = (RoboCoder.WebControls.ComboBox)lvi.FindControl("cCultureId116");
			if (cbb != null)
			{
				if (cbb.SelectedValue != string.Empty)
				{
					dr["CultureId116"] = cbb.SelectedValue;
					dr["CultureId116Text"] = cbb.SelectedText;
				}
				else
				{
					dr["CultureId116"] = System.DBNull.Value;
					dr["CultureId116Text"] = System.DBNull.Value;
				}
			}
			cbb = (RoboCoder.WebControls.ComboBox)lvi.FindControl("cButtonTypeId116");
			if (cbb != null)
			{
				if (cbb.SelectedValue != string.Empty)
				{
					dr["ButtonTypeId116"] = cbb.SelectedValue;
					dr["ButtonTypeId116Text"] = cbb.SelectedText;
				}
				else
				{
					dr["ButtonTypeId116"] = System.DBNull.Value;
			...
			if (cbb != null)
			{
				if (cbb.SelectedValue != string.Empty)
				{
					dr["ScreenId116"] = cbb.SelectedValue;
					dr["ScreenId116Text"] = cbb.SelectedText;
				}
				else
				{
			...
			if (cbb != null)
			{
				if (cbb.SelectedValue != string.Empty)
				{
					dr["ReportId116"] = cbb.SelectedValue;
					dr["ReportId116Text"] = cbb.SelectedText;
				}
				else
				{
					dr["ReportId116"] = System.DBNull.Value;
					dr["ReportId116Text"] = System.DBNull.Value;
				}
			}
			cbb = (RoboCoder.WebControls.ComboBox)lvi.FindControl("cWizardId116");
			if (cbb != null)
			{
				if (cbb.SelectedValue != string.Empty)
				{
					dr["WizardId116"] = cbb.SelectedValue;
					dr["WizardId116Text"] = cbb.SelectedText;
				}
				else
				{
					dr["WizardId116"] = System.DBNull.Value;
					dr["WizardId116Text"] = System.DBNull.Value;
				}
			}
			tb = (TextBox)lvi.FindControl("cButtonToolTip116");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["ButtonToolTip116"] = tb.Text;} else {dr["ButtonToolTip116"] = System.DBNull.Value;}
			}
			CheckBox cb = null;
			cb = (CheckBox)lvi.FindControl("cButtonVisible116");
			if (cb != null)
			{
			...
			Session[KEY_dtAdmButtonHlpGrid] = dt;
			cAdmButtonHlpGrid.EditIndex = -1;
			cAdmButtonHlpGrid_DataBind(dt.DefaultView);
		}

		protected void cAdmButtonHlpGrid_OnItemUpdating(object sender, ListViewUpdateEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmButtonHlpGrid];
			if (dt != null && dt.Rows.Count > 0)
			{
				DataRow dr = dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row;
			    GridFill(cAdmButtonHlpGrid.EditItem, dt, dr, false);
			    dr["_NewRow"] = "N";
			    if (dt.Columns.Contains("_SortOrder")) dr["_SortOrder"] = null;
			}
		}

		protected void cAdmButtonHlpGrid_OnItemCanceling(object sender, ListViewCancelEventArgs e)
		{
			PanelTop.Update();
			DataTable dt = (DataTable)Session[KEY_dtAdmButtonHlpGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			...
			{
				cAdmButtonHlpGrid_OnItemDeleting(sender, new ListViewDeleteEventArgs(e.ItemIndex));
			}
			if (dt.Columns.Contains("_SortOrder")) dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row["_SortOrder"] = null;
			cAdmButtonHlpGrid.EditIndex = -1; cAdmButtonHlpGrid_DataBind(null);
		}
		...
		protected void cDeleteAllButton_Click(object sender, System.EventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmButtonHlpGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null && (cAdmButtonHlpGrid.EditIndex < 0 || UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				GotoPage(0);
				while (dv.Count > 0) {dv.Delete(0);}
				Session[KEY_dtAdmButtonHlpGrid] = dt;
				cAdmButtonHlpGrid.EditIndex = -1; cAdmButtonHlpGrid_DataBind(dv);
			}
		}

		protected void cAdmButtonHlpGrid_OnItemDeleting(object sender, ListViewDeleteEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmButtonHlpGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			DataRow dr = dv != null ? dv[GetDataItemIndex(e.ItemIndex)].Row : null;
			if (dv != null && (cAdmButtonHlpGrid.EditIndex == e.ItemIndex || UpdateGridRow(sender, new CommandEventArgs("Save",""))))
			{
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmButtonHlpGrid); PopAdmButtonHlp76List(sender, e, false, null);
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			// *** System Button Click (Before) Web Rule starts here *** //
			if (ValidPage() && UpdateGridRow(cAdmButtonHlpGrid, new CommandEventArgs("Save", "")))
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmButtonHlpGrid];
				DataView dv = dt != null ? dt.DefaultView : null;
				string ftr = string.Empty;
				if (dv.RowFilter != string.Empty) { ftr = dv.RowFilter; dv.RowFilter = string.Empty; }
				AdmButtonHlp76 ds = PrepAdmButtonHlpData(dv,false);
				if (ftr != string.Empty) {dv.RowFilter = ftr;}
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(76,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc,noTrans,commandTimeOut)) {bValid7 = true;}
					if (bValid7)
					{
						Session[KEY_currPageIndex] = GetCurrPageIndex();
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmButtonHlp76List(sender, e, false, null);
		...
		private AdmButtonHlp76 PrepAdmButtonHlpData(DataView dv, bool bAdd)
		{
			AdmButtonHlp76 ds = new AdmButtonHlp76();
			if (dv != null)
			{
				ds.Tables["AdmButtonHlpDef"].Rows.Add(MakeTypRow(ds.Tables["AdmButtonHlpDef"].NewRow()));
				ds.Tables["AdmButtonHlpDef"].Rows.Add(MakeDisRow(ds.Tables["AdmButtonHlpDef"].NewRow()));
					dv.RowStateFilter = DataViewRowState.ModifiedCurrent;
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmButtonHlpUpd"].Rows.Add(MakeColRow(ds.Tables["AdmButtonHlpUpd"].NewRow(), drv, false));
					}
					dv.RowStateFilter = DataViewRowState.Added;
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmButtonHlpAdd"].Rows.Add(MakeColRow(ds.Tables["AdmButtonHlpAdd"].NewRow(), drv, true));
					}
					dv.RowStateFilter = DataViewRowState.Deleted;
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmButtonHlpDel"].Rows.Add(MakeColRow(ds.Tables["AdmButtonHlpDel"].NewRow(), drv, false));
					}
		...
		{
			if (cAdmButtonHlpGrid.EditIndex > -1)
			{
				cAdmButtonHlpGrid_OnItemUpdating(sender, new ListViewUpdateEventArgs(cAdmButtonHlpGrid.EditIndex));
			}
			return true;
		}

		protected void cAdmButtonHlpGrid_OnPreRender(object sender, System.EventArgs e)
		{
		...
		private string GetButtonId(ListViewItem lvi)
		{
			...
			Control c = lvi.FindControl("cAdmButtonHlpGridEdit");
		...
		{
		    return cAdmButtonHlpGridDataPager.StartRowIndex / cAdmButtonHlpGridDataPager.PageSize;
		}

		private int GetTotalPages()
		{
		    return (int)Math.Ceiling((double)cAdmButtonHlpGridDataPager.TotalRowCount / cAdmButtonHlpGridDataPager.PageSize);
		}

		private int GetDataItemIndex(int editIndex)
		{
		    return cAdmButtonHlpGridDataPager.StartRowIndex + editIndex;
		}

		protected void cAdmButtonHlpGrid_OnLayoutCreated(object sender, EventArgs e)
		{
		    // Header:
		    LinkButton lb = null;
		    lb = cAdmButtonHlpGrid.FindControl("cButtonHlpId116hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(0); lb.ToolTip = ColumnToolTip(0); lb.Parent.Visible = GridColumnVisible(0); }
		    lb = cAdmButtonHlpGrid.FindControl("cCultureId116hl") as LinkButton;
			...
			if (dtAuthRow != null)
			{
				...
				if (dr["AllowDel"].ToString() == "N" || (Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N"))
				{
		            lb = cAdmButtonHlpGrid.FindControl("cDeleteAllButton") as LinkButton; if (lb != null) { lb.Visible = false; }
				}
			}
		    // footer:
		    Label gc = null;
		    gc = cAdmButtonHlpGrid.FindControl("cButtonHlpId116fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(0); }
		    gc = cAdmButtonHlpGrid.FindControl("cCultureId116fl") as Label;

...
namespace RO.Web
{
	public partial class AdmSctGrpColModule : RO.Web.ModuleBase
	{
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_1003";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_1003";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_1003";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_1003";
		private const string KEY_dtLabel = "Cache:dtLabel3_1003";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_1003";
		private const string KEY_dtScrCri = "Cache:dtScrCri3_1003";
		private const string KEY_dsScrCriVal = "Cache:dsScrCriVal3_1003";

		private const string KEY_dtSectionCd1284 = "Cache:dtSectionCd1284";
		private const string KEY_dtGroupColId1284 = "Cache:dtGroupColId1284";

		private const string KEY_dtSystems = "Cache:dtSystems3_1003";
		private const string KEY_sysConnectionString = "Cache:sysConnectionString3_1003";
		private const string KEY_dtAdmSctGrpCol1003List = "Cache:dtAdmSctGrpCol1003List";
		private const string KEY_bNewVisible = "Cache:bNewVisible3_1003";
		private const string KEY_bNewSaveVisible = "Cache:bNewSaveVisible3_1003";
		private const string KEY_bCopyVisible = "Cache:bCopyVisible3_1003";
		private const string KEY_bCopySaveVisible = "Cache:bCopySaveVisible3_1003";
		private const string KEY_bDeleteVisible = "Cache:bDeleteVisible3_1003";
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_1003";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_1003";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_1003";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				DataTable dtMenuAccess = (new MenuSystem()).GetMenu(base.LUser.CultureId, 3, base.LImpr, LcSysConnString, LcAppPw,1003, null, null);
				...
				Session.Remove(KEY_dtAdmSctGrpCol1003List);
				...
				Session.Remove(KEY_dtSectionCd1284);
				Session.Remove(KEY_dtGroupColId1284);
				SetButtonHlp();
				GetSystems();
				SetColumnAuthority();
				GetGlobalFilter();
				...
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 1003, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmSctGrpCol1003List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=1003&typ=N&sys=3','','',''); return false;"; }
		...
		private void SetSystem(byte SystemId)
		{
			...
			try
			{
				...
				if ((Config.DeployType == "DEV" || row["dbAppDatabase"].ToString() == base.CPrj.EntityCode + "View") && !(base.CPrj.EntityCode != "RO" && row["SysProgram"].ToString() == "Y") && (new AdminSystem()).IsRegenNeeded(string.Empty,1003,0,0,LcSysConnString,LcAppPw))
				{
					(new GenScreensSystem()).CreateProgram(1003, "Section Column Style", row["dbAppDatabase"].ToString(), base.CPrj, base.CSrc, base.CTar, LcAppConnString, LcAppPw);
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(1003,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(1003,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(1003,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(1003,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,1003,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(1003,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(1003,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(1003,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		protected DataTable GetAuthRow()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthRow(1003,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				try
				{
					try {dv = new DataView((new AdminSystem()).GetExp(1003,"GetExpAdmSctGrpCol1003","Y",null,null,filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
					catch {dv = new DataView((new AdminSystem()).GetExp(1003,"GetExpAdmSctGrpCol1003","N",null,null,filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(1003,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					{
						if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["SctGrpColId1284"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["SectionCd1284"].ToString().Replace("\"","\"\"") + "\"" + (char)9 + "\"" + drv["SectionCd1284Text"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(drv["GroupColId1284"].ToString() + (char)9 + drv["GroupColId1284Text"].ToString() + (char)9);}
						if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["SctGrpColCss1284"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["SctGrpColDiv1284"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						sb.Append(Environment.NewLine);
					}
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmSctGrpCol.csv"; Session["ExportStr"] = (Config.ExportExcelCSV ? "sep=\t\n": "") + sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmSctGrpCol.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private string GetRtfGrid(DataView dv)
		{
			...
			try
			{
				...
				foreach (DataRowView drv in dv)
				{
					...
					sb.Append("\r\n");
					sb.Append(@"{");
					if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["SctGrpColId1284"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["SectionCd1284Text"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(drv["GroupColId1284Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(drv["SctGrpColCss1284"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append(drv["SctGrpColDiv1284"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					sb.Append(@"}");
					sb.Append("\r\n");
					sb.Append(@"\pard \ql \li0\ri0\widctlpar\intbl\aspalpha\aspnum\adjustright\rin0\lin0 {");
					sb.Append(@"\insrsid2981395 \trowd \irow0\irowband0\lastrow \ts15\trgaph108\trleft-108\trbrdrt\brdrs\brdrw10 \trbrdrl\brdrs\brdrw10 \trbrdrb\brdrs\brdrw10 \trbrdrr\brdrs\brdrw10 \trbrdrh\brdrs\brdrw10 \trbrdrv\brdrs\brdrw10 ");
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 1003, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			...
			UpdCriteria(true);
			cAdmSctGrpCol1003List.ClearSearch(); Session.Remove(KEY_dtAdmSctGrpCol1003List);
			PopAdmSctGrpCol1003List(sender, e, false, null);
		}
			...
			try
			{
				foreach (DataRowView drv in dvCri)
				{
					...
					if (drv["DisplayName"].ToString() == "ComboBox")
					{
						...
						(new AdminSystem()).MkGetScreenIn("1003", drv["ScreenCriId"].ToString(), "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), LcAppDb, LcDesDb, drv["MultiDesignDb"].ToString(), LcSysConnString, LcAppPw);
						DataView dv = new DataView((new AdminSystem()).GetScreenIn("1003", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), (new AdminSystem()).CountScrCri(drv["ScreenCriId"].ToString(), drv["MultiDesignDb"].ToString(), drv["MultiDesignDb"].ToString() == "N" ? LcSysConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)), drv["RequiredValid"].ToString(), 0, string.Empty, drv["DisplayMode"].ToString() != "AutoListBox", val, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)));
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("1003", LcSysConnString, LcAppPw);
				}
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmSctGrpCol1003" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmSctGrpCol1003" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			foreach (DataRowView drv in dvCri)
			{
			    if (drv["DisplayName"].ToString() == "ListBox")
			    {
					...
					if (cListBox != null)
					{
						...
						int TotalChoiceCnt = new DataView((new AdminSystem()).GetScreenIn("1003", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), CriCnt, drv["RequiredValid"].ToString(), 0, string.Empty, true, string.Empty, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId))).Count;
						string selectedValues = string.Join(",", cListBox.Items.Cast<ListItem>().Where(x => x.Selected).Select(x => "'" + x.Value + "'").ToArray());
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("1003", "AdmSctGrpCol1003", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		private void SetSectionCd1284(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtSectionCd1284];
			...
			if (ddl != null)
			{
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(1003,"GetDdlSectionCd3S3139",true,bAll,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					if (dv.Table.Columns.Contains("SctGrpColId"))
					{
						ss = "(SctGrpColId is null";
						if (string.IsNullOrEmpty(cAdmSctGrpCol1003List.SelectedValue)) {ss = ss + ")";} else {ss = ss + " OR SctGrpColId = " + cAdmSctGrpCol1003List.SelectedValue + ")";}
					}
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "SectionCd1284 is not NULL"; }
					dv.RowFilter = ss;
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(1003,"GetDdlSectionCd3S3139",true,true,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtSectionCd1284] = dv.Table;
				}
			}
		}

		private void SetGroupColId1284(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtGroupColId1284];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (ddl != null)
			{
				string ss = string.Empty;
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(1003,"GetDdlGroupColId3S3140",true,bAll,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					...
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "GroupColId1284 is not NULL"; }
					dv.RowFilter = ss;
					ddl.DataSource = dv; ddl.DataBind();
					li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					else if (!bFirst)
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(1003,"GetDdlGroupColId3S3140",true,true,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtGroupColId1284] = dv.Table;
				}
			}
		}

		private DataView GetAdmSctGrpCol1003List()
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmSctGrpCol1003List];
			if (dt == null)
			{
				CheckAuthentication(false);
				int filterId = 0;
				...
				try
				{
					try {dt = (new AdminSystem()).GetLis(1003,"GetLisAdmSctGrpCol1003",true,"Y",0,null,null,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
					catch {dt = (new AdminSystem()).GetLis(1003,"GetLisAdmSctGrpCol1003",true,"N",0,null,null,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				}
		...
		private void PopAdmSctGrpCol1003List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			DataView dv = GetAdmSctGrpCol1003List();
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			cAdmSctGrpCol1003List.AutoCompleteUrl = "AutoComplete.aspx/LisSuggests";
			cAdmSctGrpCol1003List.DataContext = context;
			if (dv.Table == null) return;
			cAdmSctGrpCol1003List.DataSource = dv;
			cAdmSctGrpCol1003List.Visible = true;
			if (cAdmSctGrpCol1003List.Items.Count <= 0) {cAdmSctGrpCol1003List.Visible = false; cAdmSctGrpCol1003List_SelectedIndexChanged(sender,e);}
			else
			{
				if (bPageLoad && !string.IsNullOrEmpty(Request.QueryString["key"]) && bUseCri.Value == string.Empty)
				{
					try {cAdmSctGrpCol1003List.SelectByValue(Request.QueryString["key"],string.Empty,true);} catch {cAdmSctGrpCol1003List.Items[0].Selected = true; cAdmSctGrpCol1003List_SelectedIndexChanged(sender, e);}
				    cScreenSearch.Visible = false; cSystem.Visible = false; cEditButton.Visible = true; cSaveCloseButton.Visible = cSaveButton.Visible;
				}
				else
				{
					if (ID != null) {if (!cAdmSctGrpCol1003List.SelectByValue(ID,string.Empty,true)) {ID = null;}}
					if (ID == null)
					{
						cAdmSctGrpCol1003List_SelectedIndexChanged(sender, e);
					}
				}
			}
		}
		...
		private void SetColumnAuthority()
		{
			...
			{
				base.SetFoldBehavior(cSctGrpColId1284, dtAuth.Rows[0], cSctGrpColId1284P1, cSctGrpColId1284Label, cSctGrpColId1284P2, null, dtLabel.Rows[0], null, null, null);
				base.SetFoldBehavior(cSectionCd1284, dtAuth.Rows[1], cSectionCd1284P1, cSectionCd1284Label, cSectionCd1284P2, null, dtLabel.Rows[1], null, null, null);
				base.SetFoldBehavior(cGroupColId1284, dtAuth.Rows[2], cGroupColId1284P1, cGroupColId1284Label, cGroupColId1284P2, null, dtLabel.Rows[2], null, null, null);
				base.SetFoldBehavior(cSctGrpColCss1284, dtAuth.Rows[3], cSctGrpColCss1284P1, cSctGrpColCss1284Label, cSctGrpColCss1284P2, cSctGrpColCss1284E, null, dtLabel.Rows[3], null, null, null);
				cSctGrpColCss1284E.Attributes["label_id"] = cSctGrpColCss1284Label.ClientID; cSctGrpColCss1284E.Attributes["target_id"] = cSctGrpColCss1284.ClientID;
				base.SetFoldBehavior(cSctGrpColDiv1284, dtAuth.Rows[4], cSctGrpColDiv1284P1, cSctGrpColDiv1284Label, cSctGrpColDiv1284P2, cSctGrpColDiv1284E, null, dtLabel.Rows[4], null, null, null);
				cSctGrpColDiv1284E.Attributes["label_id"] = cSctGrpColDiv1284Label.ClientID; cSctGrpColDiv1284E.Attributes["target_id"] = cSctGrpColDiv1284.ClientID;
			}
			if ((cSectionCd1284.Attributes["OnChange"] == null || cSectionCd1284.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cSectionCd1284.Visible && cSectionCd1284.Enabled) {cSectionCd1284.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cGroupColId1284.Attributes["OnChange"] == null || cGroupColId1284.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cGroupColId1284.Visible && cGroupColId1284.Enabled) {cGroupColId1284.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cSctGrpColCss1284.Attributes["OnChange"] == null || cSctGrpColCss1284.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cSctGrpColCss1284.Visible && !cSctGrpColCss1284.ReadOnly) {cSctGrpColCss1284.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cSctGrpColDiv1284.Attributes["OnChange"] == null || cSctGrpColDiv1284.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cSctGrpColDiv1284.Visible && !cSctGrpColDiv1284.ReadOnly) {cSctGrpColDiv1284.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
		}
		...
		private void ClearMaster(object sender, System.EventArgs e)
		{
			...
			if (dt.Rows[1]["ColVisible"].ToString() == "Y" && dt.Rows[1]["ColReadOnly"].ToString() != "Y") {SetSectionCd1284(cSectionCd1284,string.Empty);}
			if (dt.Rows[2]["ColVisible"].ToString() == "Y" && dt.Rows[2]["ColReadOnly"].ToString() != "Y") {SetGroupColId1284(cGroupColId1284,string.Empty);}
			if (dt.Rows[3]["ColVisible"].ToString() == "Y" && dt.Rows[3]["ColReadOnly"].ToString() != "Y") {cSctGrpColCss1284.Text = string.Empty;}
			if (dt.Rows[4]["ColVisible"].ToString() == "Y" && dt.Rows[4]["ColReadOnly"].ToString() != "Y") {cSctGrpColDiv1284.Text = string.Empty;}
			// *** Default Value (Folder) Web Rule starts here *** //
		}

		private void InitMaster(object sender, System.EventArgs e)
		{
			cSctGrpColId1284.Text = string.Empty;
			SetSectionCd1284(cSectionCd1284,string.Empty);
			SetGroupColId1284(cGroupColId1284,string.Empty);
			cSctGrpColCss1284.Text = string.Empty;
			cSctGrpColDiv1284.Text = string.Empty;
			// *** Default Value (Folder) Web Rule starts here *** //
		}
		protected void cAdmSctGrpCol1003List_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmSctGrpCol1003List_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmSctGrpCol1003List_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			InitMaster(sender, e);      // Do this first to enable defaults for non-database columns.
			DataTable dt = null;
			try
			{
				dt = (new AdminSystem()).GetMstById("GetAdmSctGrpCol1003ById",cAdmSctGrpCol1003List.SelectedValue,null,null);
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				if (dt.Rows.Count > 0)
				{
					cAdmSctGrpCol1003List.SetDdVisible();
					DataRow dr = dt.Rows[0];
					try {cSctGrpColId1284.Text = RO.Common3.Utils.fmNumeric("0",dr["SctGrpColId1284"].ToString(),base.LUser.Culture);} catch {cSctGrpColId1284.Text = string.Empty;}
					SetSectionCd1284(cSectionCd1284,dr["SectionCd1284"].ToString());
					SetGroupColId1284(cGroupColId1284,dr["GroupColId1284"].ToString());
					try {cSctGrpColCss1284.Text = dr["SctGrpColCss1284"].ToString();} catch {cSctGrpColCss1284.Text = string.Empty;}
					try {cSctGrpColDiv1284.Text = dr["SctGrpColDiv1284"].ToString();} catch {cSctGrpColDiv1284.Text = string.Empty;}
				}
			}
			cButPanel.DataBind();
			ScriptManager.GetCurrent(Parent.Page).SetFocus(cAdmSctGrpCol1003List.FocusID);
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cSctGrpColId1284.Text == string.Empty) { cClearButton_Click(sender, new EventArgs()); ShowDirty(false); PanelTop.Update();}
			else { PopAdmSctGrpCol1003List(sender, e, false, cSctGrpColId1284.Text); }
			// *** System Button Click (After) Web Rule starts here *** //
		}
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmSctGrpCol1003List); PopAdmSctGrpCol1003List(sender, e, false, null);
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			if (ValidPage())
			{
				AdmSctGrpCol1003 ds = PrepAdmSctGrpColData(null,cSctGrpColId1284.Text == string.Empty);
				if (!string.IsNullOrEmpty(cAdmSctGrpCol1003List.SelectedValue) && cSctGrpColId1284.Text != cAdmSctGrpCol1003List.SelectedValue)
				{
					PreMsgPopup("Primary key cannot be changed");
					return rtn;
				}
				if (string.IsNullOrEmpty(cAdmSctGrpCol1003List.SelectedValue))	// Add
				{
					if (ds != null)
					{
						pid = (new AdminSystem()).AddData(1003,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc,noTrans,commandTimeOut);
					}
					if (!string.IsNullOrEmpty(pid))
					{
						cAdmSctGrpCol1003List.ClearSearch(); Session.Remove(KEY_dtAdmSctGrpCol1003List);
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmSctGrpCol1003List(sender, e, false, pid);
						rtn = GetScreenHlp().Rows[0]["AddMsg"].ToString();
					}
				}
				else {
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(1003,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc,noTrans,commandTimeOut)) {bValid7 = true;}
					if (bValid7)
					{
						cAdmSctGrpCol1003List.ClearSearch(); Session.Remove(KEY_dtAdmSctGrpCol1003List);
						ShowDirty(false); PopAdmSctGrpCol1003List(sender, e, false, ds.Tables["AdmSctGrpCol"].Rows[0]["SctGrpColId1284"]);
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cSctGrpColId1284.Text != string.Empty)
			{
				AdmSctGrpCol1003 ds = PrepAdmSctGrpColData(null,false);
				try
				{
					if (ds != null && (new AdminSystem()).DelData(1003,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc))
					{
						cAdmSctGrpCol1003List.ClearSearch(); Session.Remove(KEY_dtAdmSctGrpCol1003List);
						ShowDirty(false); PopAdmSctGrpCol1003List(sender, e, false, null);
		...
		private AdmSctGrpCol1003 PrepAdmSctGrpColData(DataView dv, bool bAdd)
		{
			...
			if (bAdd) { dr["SctGrpColId1284"] = string.Empty; } else { dr["SctGrpColId1284"] = cSctGrpColId1284.Text; }
			drType["SctGrpColId1284"] = "Numeric"; drDisp["SctGrpColId1284"] = "TextBox";
			try {dr["SectionCd1284"] = cSectionCd1284.SelectedValue;} catch {}
			drType["SectionCd1284"] = "Char"; drDisp["SectionCd1284"] = "DropDownList";
			try {dr["GroupColId1284"] = cGroupColId1284.SelectedValue;} catch {}
			drType["GroupColId1284"] = "Numeric"; drDisp["GroupColId1284"] = "DropDownList";
			try {dr["SctGrpColCss1284"] = cSctGrpColCss1284.Text;} catch {}
			drType["SctGrpColCss1284"] = "VarChar"; drDisp["SctGrpColCss1284"] = "MultiLine";
			try {dr["SctGrpColDiv1284"] = cSctGrpColDiv1284.Text;} catch {}
		...
		public bool CanAct(char typ) { return CanAct(typ, GetAuthRow(), cAdmSctGrpCol1003List.SelectedValue.ToString()); }
		...
		private void PreMsgPopup(string msg) { PreMsgPopup(msg,cAdmSctGrpCol1003List,null); }

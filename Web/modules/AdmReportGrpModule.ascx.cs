...
{
	public class AdmReportGrp65 : DataSet
	{
		public AdmReportGrp65()
		{
			this.Tables.Add(MakeDtlColumns(new DataTable("AdmReportGrpDef")));
			this.Tables.Add(MakeDtlColumns(new DataTable("AdmReportGrpAdd")));
			this.Tables.Add(MakeDtlColumns(new DataTable("AdmReportGrpUpd")));
			this.Tables.Add(MakeDtlColumns(new DataTable("AdmReportGrpDel")));
			this.DataSetName = "AdmReportGrp65";
			this.Namespace = "http://Rintagi.com/DataSet/AdmReportGrp65";
		}

		private DataTable MakeDtlColumns(DataTable dt)
		{
			DataColumnCollection columns = dt.Columns;
			columns.Add("ReportGrpId94", typeof(string));
			columns.Add("ReportId94", typeof(string));
			columns.Add("ReportGrpIndex94", typeof(string));
			columns.Add("ParentGrpId94", typeof(string));
			columns.Add("ReportGrpName94", typeof(string));
			columns.Add("ContentVertical94", typeof(string));
			columns.Add("LabelVertical94", typeof(string));
			columns.Add("BorderWidth94", typeof(string));
			columns.Add("GrpStyle94", typeof(string));
			return dt;
		}
	}
}
...
namespace RO.Web
{
	public partial class AdmReportGrpModule : RO.Web.ModuleBase
	{

		private const string KEY_lastAddedRow = "Cache:lastAddedRow3_65";
		private const string KEY_lastSortOrd = "Cache:lastSortOrd3_65";
		private const string KEY_lastSortImg = "Cache:lastSortImg3_65";
		private const string KEY_lastSortCol = "Cache:lastSortCol3_65";
		private const string KEY_lastSortExp = "Cache:lastSortExp3_65";
		private const string KEY_lastSortUrl = "Cache:lastSortUrl3_65";
		private const string KEY_lastSortTog = "Cache:lastSortTog3_65";
		private const string KEY_lastImpPwdOvride = "Cache:lastImpPwdOvride3_65";
		private const string KEY_cntImpPwdOvride = "Cache:cntImpPwdOvride3_65";
		private const string KEY_currPageIndex = "Cache:currPageIndex3_65";
		private const string KEY_bHiImpVisible = "Cache:bHiImpVisible3_65";
		private const string KEY_bShImpVisible = "Cache:bShImpVisible3_65";
		private const string KEY_dtAdmReportGrpGrid = "Cache:dtAdmReportGrpGrid";
		private const string KEY_scrImport = "Cache:scrImport3_65";
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_65";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_65";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_65";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_65";
		private const string KEY_dtLabel = "Cache:dtLabel3_65";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_65";
		private const string KEY_dtScrCri = "Cache:dtScrCri3_65";
		private const string KEY_dsScrCriVal = "Cache:dsScrCriVal3_65";

		private const string KEY_dtReportId94 = "Cache:dtReportId94";
		private const string KEY_dtParentGrpId94 = "Cache:dtParentGrpId94";

		private const string KEY_dtSystems = "Cache:dtSystems3_65";
		private const string KEY_sysConnectionString = "Cache:sysConnectionString3_65";
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_65";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_65";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_65";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				DataTable dtMenuAccess = (new MenuSystem()).GetMenu(base.LUser.CultureId, 3, base.LImpr, LcSysConnString, LcAppPw,65, null, null);
				...
				Session.Remove(KEY_dtAdmReportGrpGrid);
				...
				Session.Remove(KEY_dtReportId94);
				Session.Remove(KEY_dtParentGrpId94);
				SetButtonHlp();
				GetSystems();
				SetColumnAuthority();
				GetGlobalFilter();
				...
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 65, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmReportGrp65List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=65&typ=N&sys=3','','',''); return false;"; }
			...
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmReportGrpGrid];
				if (dt != null)
				{
		...
		private void InitializeComponent()
		{
			...
			if (LcSysConnString == null)
			{
				...
				{
					DataTable dt;
					dt = (new AdminSystem()).GetAuthRow(65,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
					...
					{
						cAdmReportGrpGrid.PreRender += new EventHandler(cAdmReportGrpGrid_OnPreRender);
					}
				}
		...
		private void SetSystem(byte SystemId)
		{
			...
			try
			{
				...
				if ((Config.DeployType == "DEV" || row["dbAppDatabase"].ToString() == base.CPrj.EntityCode + "View") && !(base.CPrj.EntityCode != "RO" && row["SysProgram"].ToString() == "Y") && (new AdminSystem()).IsRegenNeeded(string.Empty,65,0,0,LcSysConnString,LcAppPw))
				{
					(new GenScreensSystem()).CreateProgram(65, "Report Criteria Group", row["dbAppDatabase"].ToString(), base.CPrj, base.CSrc, base.CTar, LcAppConnString, LcAppPw);
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(65,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(65,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void SetClientRule(ListViewDataItem ee, bool isEditItem)
		{
			...
			if (dvRul.Count > 0)
			{
				...
				{
					if (ee == null || drv["ScriptEvent"].ToString().Substring(0,2).ToLower() != "on" || (cAdmReportGrpGrid.EditIndex > -1 && GetDataItemIndex(cAdmReportGrpGrid.EditIndex) == ee.DataItemIndex))
					{
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(65,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(65,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,65,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(65,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(65,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(65,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				CheckAuthentication(false);
				DataView dv = null;
				try {dv = ((DataTable)Session[KEY_dtAdmReportGrpGrid]).DefaultView;}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(65,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					{
						if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["ReportGrpId94"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["ReportId94"].ToString() + (char)9 + drv["ReportId94Text"].ToString() + (char)9);}
						if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["ReportGrpIndex94"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(drv["ParentGrpId94"].ToString() + (char)9 + drv["ParentGrpId94Text"].ToString() + (char)9);}
						if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["ReportGrpName94"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["ContentVertical94"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["LabelVertical94"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[7]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["BorderWidth94"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[8]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["GrpStyle94"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						sb.Append(Environment.NewLine);
					}
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmReportGrp.csv"; Session["ExportStr"] = (Config.ExportExcelCSV ? "sep=\t\n": "") + sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmReportGrp.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private string GetRtfGrid(DataView dv)
		{
			...
			try
			{
				...
				foreach (DataRowView drv in dv)
				{
					...
					sb.Append("\r\n");
					sb.Append(@"{");
					if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["ReportGrpId94"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["ReportId94Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(drv["ReportGrpIndex94"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(drv["ParentGrpId94Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append(drv["ReportGrpName94"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append(drv["ContentVertical94"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append(drv["LabelVertical94"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[7]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["BorderWidth94"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[8]["ColExport"].ToString() == "Y") {sb.Append(drv["GrpStyle94"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					sb.Append(@"}");
					sb.Append("\r\n");
					sb.Append(@"\pard \ql \li0\ri0\widctlpar\intbl\aspalpha\aspnum\adjustright\rin0\lin0 {");
					sb.Append(@"\insrsid2981395 \trowd \irow0\irowband0\lastrow \ts15\trgaph108\trleft-108\trbrdrt\brdrs\brdrw10 \trbrdrl\brdrs\brdrw10 \trbrdrb\brdrs\brdrw10 \trbrdrr\brdrs\brdrw10 \trbrdrh\brdrs\brdrw10 \trbrdrv\brdrs\brdrw10 ");
		...
		private void GetPageInfo()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastPageInfo(65, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 65, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			// *** Criteria Trigger (before) Web Rule starts here *** //
			DataTable dt = (DataTable)Session[KEY_dtAdmReportGrpGrid];
			if (cAdmReportGrpGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
			if (IsPostBack && cCriteria.Visible) ReBindCriteria(GetScrCriteria());
			UpdCriteria(true);
			PopAdmReportGrp65List(sender, e, false, null);
			}
		}
			...
			try
			{
				foreach (DataRowView drv in dvCri)
				{
					...
					if (drv["DisplayName"].ToString() == "ComboBox")
					{
						...
						(new AdminSystem()).MkGetScreenIn("65", drv["ScreenCriId"].ToString(), "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), LcAppDb, LcDesDb, drv["MultiDesignDb"].ToString(), LcSysConnString, LcAppPw);
						DataView dv = new DataView((new AdminSystem()).GetScreenIn("65", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), (new AdminSystem()).CountScrCri(drv["ScreenCriId"].ToString(), drv["MultiDesignDb"].ToString(), drv["MultiDesignDb"].ToString() == "N" ? LcSysConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)), drv["RequiredValid"].ToString(), 0, string.Empty, drv["DisplayMode"].ToString() != "AutoListBox", val, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)));
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("65", LcSysConnString, LcAppPw);
				}
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmReportGrp65" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmReportGrp65" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			foreach (DataRowView drv in dvCri)
			{
			    if (drv["DisplayName"].ToString() == "ListBox")
			    {
					...
					if (cListBox != null)
					{
						...
						int TotalChoiceCnt = new DataView((new AdminSystem()).GetScreenIn("65", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), CriCnt, drv["RequiredValid"].ToString(), 0, string.Empty, true, string.Empty, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId))).Count;
						string selectedValues = string.Join(",", cListBox.Items.Cast<ListItem>().Where(x => x.Selected).Select(x => "'" + x.Value + "'").ToArray());
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("65", "AdmReportGrp65", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		protected void cbReportId94(object sender, System.EventArgs e)
		{
			SetReportId94((RoboCoder.WebControls.ComboBox)sender,string.Empty,cAdmReportGrpGrid.EditItem);
		}

		private void SetReportId94(RoboCoder.WebControls.ComboBox ddl, string keyId, ListViewItem li)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			context["method"] = "GetDdlReportId3S967";
			context["addnew"] = "Y";
			context["mKey"] = "ReportId94";
			context["mVal"] = "ReportId94Text";
			context["mTip"] = "ReportId94Text";
			context["mImg"] = "ReportId94Text";
			context["ssd"] = Request.QueryString["ssd"];
			context["scr"] = "65";
			context["csy"] = "3";
			context["filter"] = Utils.IsInt(cFilterId.SelectedValue)? cFilterId.SelectedValue : "0";
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(65,"GetDdlReportId3S967",true,false,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
					ddl.DataSource = dv; Session[KEY_dtReportId94] = dv.Table;
				    try { ddl.SelectByValue(keyId,string.Empty,true); } catch { try { ddl.SelectedIndex = 0; } catch { } }
				}
			}
		}

		private void SetParentGrpId94(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtParentGrpId94];
			...
			if (ddl != null)
			{
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(65,"GetDdlParentGrpId3S982",true,bAll,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "ParentGrpId94 is not NULL"; }
					dv.RowFilter = ss;
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(65,"GetDdlParentGrpId3S982",true,true,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtParentGrpId94] = dv.Table;
				}
			}
		}

		private void PopAdmReportGrp65List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			DataTable dt = null;
			CheckAuthentication(false);
			...
			try
			{
				try {dt = (new AdminSystem()).GetLis(65,"GetLisAdmReportGrp65",false,"Y",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				catch {dt = (new AdminSystem()).GetLis(65,"GetLisAdmReportGrp65",false,"N",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				Session[KEY_dtAdmReportGrpGrid] = dt;
				cAdmReportGrpGridDataPager.PageSize = Int16.Parse(cPgSize.Text); GotoPage(0);
			    if (Session[KEY_lastSortCol] != null)
			    {
					cAdmReportGrpGrid_OnSorting(sender, new ListViewSortEventArgs((string)Session[KEY_lastSortExp], SortDirection.Ascending));
			    }
				else
			    {
					cAdmReportGrpGrid_DataBind(dt.DefaultView);
			    }
		...
		public void cFindButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dv != null)
			{
				if (cAdmReportGrpGrid.EditIndex < 0 || UpdateGridRow(cAdmReportGrpGrid, new CommandEventArgs("Save", "")))
				{
					...
					GotoPage(0); cAdmReportGrpGrid_DataBind(dv);
					if (GetCurrPageIndex() != (int)Session[KEY_currPageIndex] && (int)Session[KEY_currPageIndex] < GetTotalPages()) { GotoPage((int)Session[KEY_currPageIndex]); cAdmReportGrpGrid_DataBind(dv);}
		...
		public void cInsRowButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dt != null && (ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")))
			{
				...
				Session[KEY_lastAddedRow] = 0; Session[KEY_dtAdmReportGrpGrid] = dt; Session[KEY_currPageIndex] = 0; GotoPage(0);
				cAdmReportGrpGrid_DataBind(dt.DefaultView);
				cAdmReportGrpGrid_OnItemEditing(cAdmReportGrpGrid, new ListViewEditEventArgs(0));
		...
		public void cPgSizeButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cAdmReportGrpGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				try {if (int.Parse(cPgSize.Text) < 1 || int.Parse(cPgSize.Text) > 200) {cPgSize.Text = "18";}} catch {cPgSize.Text = "18";}
				cAdmReportGrpGridDataPager.PageSize = int.Parse(cPgSize.Text); GotoPage(0);
				if (dt.Rows.Count <= 0 || (GetAuthRow().Rows[0]["AllowAdd"].ToString() == "Y" && dt.Rows.Count == 1)) {cAdmReportGrpGrid_DataBind(dt.DefaultView);} else {cFindButton_Click(sender, e);}
				try
				{
					(new AdminSystem()).UpdLastPageInfo(65, base.LUser.UsrId, cPgSize.Text, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		}
		...
		public void cShowImpButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cSchemaImage.Visible)
			{
				try
				{
					Session["ImportSchema"] = (new AdminSystem()).GetSchemaScrImp(65,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (cSchemaImage.Attributes["OnClick"] == null || cSchemaImage.Attributes["OnClick"].IndexOf("ImportSchema.aspx") < 0) {cSchemaImage.Attributes["OnClick"] += "SearchLink('ImportSchema.aspx?scm=S&key=65&csy=3','','',''); return false;";}
			}
			// *** System Button Click (after) Web Rule starts here *** //
		}
		...
		protected void ScrImport(bool bClear)
		{
			...
			DataTable dtAdmReportGrpGrid = (DataTable)Session[KEY_dtAdmReportGrpGrid];
			if (dti != null && dtAdmReportGrpGrid != null)
			{
				if (bClear)
				{
					dtAdmReportGrpGrid.Rows.Clear();
				}
				cFind.Text = string.Empty;
				int iExist = dtAdmReportGrpGrid.Rows.Count;
				...
				foreach (DataRowView drv in dti.DefaultView)
				{
					...
					{
						DataTable dtd;
						dtd = jj==iExist ? null : (DataTable)Session[KEY_dtReportId94];
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(65,"GetDdlReportId3S967",true,true,0,string.Empty,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
							Session[KEY_dtReportId94] = dtd;
						}
						ScrImportDdl(dtd.DefaultView, drv, true, "ReportGrpId", "ReportId94", "ReportId94Text", 50, "2", true);
						dtd = (DataTable)Session[KEY_dtParentGrpId94];
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(65,"GetDdlParentGrpId3S982",true,true,0,string.Empty,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
							Session[KEY_dtParentGrpId94] = dtd;
						}
						ScrImportDdl(dtd.DefaultView, drv, false, "ReportGrpId", "ParentGrpId94", "ParentGrpId94Text", 50, "2", true);
					}
					jj = jj + 1;
				}
				iRow = dti.DefaultView.Count - iExist;
				if (!dti.Columns.Contains("_NewRow")) { dti.Columns.Add("_NewRow"); }
				Session[KEY_dtAdmReportGrpGrid] = dti;
				cAdmReportGrpGrid_DataBind(dti.DefaultView);
		...
		public void cImportButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cFNameO.Text != string.Empty && cWorkSheet.Items.Count > 0 && cWorkSheet.SelectedItem.Text != string.Empty && cStartRow.Text != string.Empty)
			{
				...
				try
				{
					if (cAdmReportGrpGrid.EditIndex < 0 || UpdateGridRow(cAdmReportGrpGrid, new CommandEventArgs("Save", "")))
					{
						Session[KEY_scrImport] = ImportFile(dtAdmReportGrpGrid.Copy(),cFNameO.Text,cWorkSheet.SelectedItem.Text,cStartRow.Text,Config.PathTmpImport + cFName.Text);
					}
		...
		private DataTable ImportFile(DataTable dt, string fileName, string workSheet, string startRow, string fileFullName)
		{
			try
			{
				...
				{
					if (rows[iRow][6].ToString() == string.Empty && rows[iRow][7].ToString() == string.Empty && rows[iRow][8].ToString() == string.Empty && rows[iRow][9].ToString() == string.Empty) {idel = idel + 1;}
					else
					{
		...
		private void GotoPage(int pageNo)
		{
			if (pageNo >= 0 && pageNo < GetTotalPages())
			{
				if (cAdmReportGrpGrid.EditIndex < 0 || UpdateGridRow(cAdmReportGrpGrid, new CommandEventArgs("Save", "")))
				{
					try { cAdmReportGrpGridDataPager.SetPageProperties(cAdmReportGrpGridDataPager.PageSize * pageNo, cAdmReportGrpGridDataPager.MaximumRows, false); cAdmReportGrpGrid_DataBind(null); }
					catch
					{
						try { cAdmReportGrpGridDataPager.SetPageProperties(0, cAdmReportGrpGridDataPager.MaximumRows, false); cAdmReportGrpGrid_DataBind(null); } catch {}
					}
				}
			}
		}

		protected void cAdmReportGrpGrid_OnItemCommand(object sender, ListViewCommandEventArgs e)
		{
		}

		private void cAdmReportGrpGrid_DataBind(DataView dvAdmReportGrpGrid)
		{
			if (dvAdmReportGrpGrid == null)
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmReportGrpGrid];
				dvAdmReportGrpGrid = dt != null ? dt.DefaultView : null;
			}
			if (dvAdmReportGrpGrid != null)
			{
				LcAuth = GetAuthCol().AsEnumerable().ToDictionary<DataRow,string>(dr=>dr["ColName"].ToString());
				cAdmReportGrpGrid.DataSource = dvAdmReportGrpGrid;
				cAdmReportGrpGrid.DataBind();
		...
		protected void cAdmReportGrpGrid_OnSorting(object sender, ListViewSortEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmReportGrpGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null)
			{
				if (cAdmReportGrpGrid.EditIndex < 0 || UpdateGridRow(cAdmReportGrpGrid, new CommandEventArgs("Save", "")))
				{
					...
					Session[KEY_dtAdmReportGrpGrid] = dt;
					cAdmReportGrpGrid_DataBind(dv);
				}
			}
		}

		protected void cAdmReportGrpGrid_OnPagePropertiesChanging(object sender, PagePropertiesChangingEventArgs e)
		{
		    cAdmReportGrpGridDataPager.SetPageProperties(e.StartRowIndex, e.MaximumRows, false);
		    DataTable dt = (DataTable)Session[KEY_dtAdmReportGrpGrid];
		    DataView dv = dt != null ? dt.DefaultView : null;
			cAdmReportGrpGrid_DataBind(dv); cAdmReportGrpGrid.EditIndex = -1;
		}

		private void GridChkPgDirty(ListViewItem lvi)
		{
				WebControl cc = null;
				cc = ((WebControl)lvi.FindControl("cReportId94"));
				...
				cc = ((WebControl)lvi.FindControl("cReportGrpIndex94"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cParentGrpId94"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cReportGrpName94"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cContentVertical94"));
				...
				cc = ((WebControl)lvi.FindControl("cLabelVertical94"));
				if ((cc.Attributes["OnClick"] == null || cc.Attributes["OnClick"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnClick"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty(); this.focus();";}
				cc = ((WebControl)lvi.FindControl("cBorderWidth94"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cGrpStyle94"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
		}

		protected void cAdmReportGrpGrid_OnItemDataBound(object sender, ListViewItemEventArgs e)
		{
			// *** GridItemDataBound (before) Web Rule End *** //
			DataTable dt = (DataTable)Session[KEY_dtAdmReportGrpGrid];
			bool isEditItem = false;
			DataView dvAdmReportGrpGrid = dt != null ? dt.DefaultView : null;
			if (cAdmReportGrpGrid.EditIndex > -1 && GetDataItemIndex(cAdmReportGrpGrid.EditIndex) == e.Item.DataItemIndex)
			{
				isEditItem = true;
				base.SetGridEnabled(e.Item, GetAuthCol(), GetLabel(), 0);
				SetReportId94((RoboCoder.WebControls.ComboBox)e.Item.FindControl("cReportId94"),dvAdmReportGrpGrid[e.Item.DataItemIndex]["ReportId94"].ToString(), e.Item);
				SetParentGrpId94((DropDownList)e.Item.FindControl("cParentGrpId94"),dvAdmReportGrpGrid[e.Item.DataItemIndex]["ParentGrpId94"].ToString());
				CheckBox cb = null;
				cb = (CheckBox)e.Item.FindControl("cContentVertical94");
				if (cb != null)
				{
					cb.Checked = base.GetBool(dvAdmReportGrpGrid[e.Item.DataItemIndex]["ContentVertical94"].ToString());
				}
				cb = (CheckBox)e.Item.FindControl("cLabelVertical94");
				if (cb != null)
				{
					cb.Checked = base.GetBool(dvAdmReportGrpGrid[e.Item.DataItemIndex]["LabelVertical94"].ToString());
				}
			...
			if (dtAuthRow != null)
			{
				...
				{
					cc = e.Item.FindControl("cAdmReportGrpGridDelete"); if (cc != null) { cc.Visible = false; }
				}
				if (!AllowRowEdit(GetAuthRow(),dvAdmReportGrpGrid[e.Item.DataItemIndex]["ReportGrpId94"].ToString()) || (Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N"))
				{
					cc = e.Item.FindControl("cAdmReportGrpGridEdit"); if (cc != null) { cc.Visible = false; }
				}
				else
				{
			        HtmlTableRow tr = e.Item.FindControl("cAdmReportGrpGridRow") as HtmlTableRow;
			        LinkButton lb = e.Item.FindControl("cAdmReportGrpGridEdit") as LinkButton;
		...
		protected void cReportGrpId94hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "0") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "0"; Session[KEY_lastSortExp] = "ReportGrpId94";Session[KEY_lastSortImg] = "cReportGrpId94hi";
			cAdmReportGrpGrid_OnSorting(sender, new ListViewSortEventArgs("ReportGrpId94", SortDirection.Ascending));
		}

		protected void cReportId94hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "1") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "1"; Session[KEY_lastSortExp] = "ReportId94Text";Session[KEY_lastSortImg] = "cReportId94hi";
			cAdmReportGrpGrid_OnSorting(sender, new ListViewSortEventArgs("ReportId94Text", SortDirection.Ascending));
		}

		protected void cReportGrpIndex94hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "2") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "2"; Session[KEY_lastSortExp] = "ReportGrpIndex94";Session[KEY_lastSortImg] = "cReportGrpIndex94hi";
			cAdmReportGrpGrid_OnSorting(sender, new ListViewSortEventArgs("ReportGrpIndex94", SortDirection.Ascending));
		}

		protected void cParentGrpId94hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "3") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "3"; Session[KEY_lastSortExp] = "ParentGrpId94Text";Session[KEY_lastSortImg] = "cParentGrpId94hi";
			cAdmReportGrpGrid_OnSorting(sender, new ListViewSortEventArgs("ParentGrpId94Text", SortDirection.Ascending));
		}

		protected void cReportGrpName94hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "4") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "4"; Session[KEY_lastSortExp] = "ReportGrpName94";Session[KEY_lastSortImg] = "cReportGrpName94hi";
			cAdmReportGrpGrid_OnSorting(sender, new ListViewSortEventArgs("ReportGrpName94", SortDirection.Ascending));
		}

		protected void cContentVertical94hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "5") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "5"; Session[KEY_lastSortExp] = "ContentVertical94";Session[KEY_lastSortImg] = "cContentVertical94hi";
			cAdmReportGrpGrid_OnSorting(sender, new ListViewSortEventArgs("ContentVertical94", SortDirection.Ascending));
		}

		protected void cLabelVertical94hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "6") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "6"; Session[KEY_lastSortExp] = "LabelVertical94";Session[KEY_lastSortImg] = "cLabelVertical94hi";
			cAdmReportGrpGrid_OnSorting(sender, new ListViewSortEventArgs("LabelVertical94", SortDirection.Ascending));
		}

		protected void cBorderWidth94hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "7") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "7"; Session[KEY_lastSortExp] = "BorderWidth94";Session[KEY_lastSortImg] = "cBorderWidth94hi";
			cAdmReportGrpGrid_OnSorting(sender, new ListViewSortEventArgs("BorderWidth94", SortDirection.Ascending));
		}

		protected void cGrpStyle94hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "8") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "8"; Session[KEY_lastSortExp] = "GrpStyle94";Session[KEY_lastSortImg] = "cGrpStyle94hi";
			cAdmReportGrpGrid_OnSorting(sender, new ListViewSortEventArgs("GrpStyle94", SortDirection.Ascending));
		}

		protected void cAdmReportGrpGrid_OnItemEditing(object sender, ListViewEditEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmReportGrpGrid];
			if ((ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")) && dt != null && dt.DefaultView.Count > GetDataItemIndex(e.NewEditIndex))
			{
				cAdmReportGrpGrid.EditIndex = e.NewEditIndex;
				cAdmReportGrpGrid_DataBind(null);
				Control wc = null;
				string idx = Page.Request["__EVENTARGUMENT"];
				if (!string.IsNullOrEmpty(idx))
				{
				    Control ctrlToFocus = cAdmReportGrpGrid.EditItem.FindControl("c" + idx);
				    if (((WebControl)ctrlToFocus).Enabled) { wc = ctrlToFocus; }
				}
				if (wc == null) { wc = FindEditableControl(cAdmReportGrpGrid.EditItem); }
				if (wc != null)
             {
             ...
             MkMobileLabel(cAdmReportGrpGrid.EditItem);
		    }
		    else { e.Cancel = true; }
		}
		...
		{
		    Label ml = null;
		    ml = lvi.FindControl("cReportGrpId94ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(0); }
		    ml = lvi.FindControl("cReportId94ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(1); }
		    ml = lvi.FindControl("cReportGrpIndex94ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(2); }
		    ml = lvi.FindControl("cParentGrpId94ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(3); }
		    ml = lvi.FindControl("cReportGrpName94ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(4); }
		    ml = lvi.FindControl("cContentVertical94ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(5); }
		    ml = lvi.FindControl("cLabelVertical94ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(6); }
		    ml = lvi.FindControl("cBorderWidth94ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(7); }
		    ml = lvi.FindControl("cGrpStyle94ml") as Label;
		...
		protected void GridFill(ListViewItem lvi, DataTable dt, DataRow dr, bool bInsert)
		{
			...
			cbb = (RoboCoder.WebControls.ComboBox)lvi.FindControl("cReportId94");
			if (cbb != null)
			{
				if (cbb.SelectedValue != string.Empty)
				{
					dr["ReportId94"] = cbb.SelectedValue;
					dr["ReportId94Text"] = cbb.SelectedText;
				}
				else
				{
					dr["ReportId94"] = System.DBNull.Value;
					dr["ReportId94Text"] = System.DBNull.Value;
				}
			}
			TextBox tb = null;
			tb = (TextBox)lvi.FindControl("cReportGrpIndex94");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["ReportGrpIndex94"] = tb.Text;} else {dr["ReportGrpIndex94"] = System.DBNull.Value;}
			}
			DropDownList ddl = null;
			ddl = (DropDownList)lvi.FindControl("cParentGrpId94");
			if (ddl != null)
			{
				if (ddl.SelectedIndex >= 0 && ddl.SelectedValue.ToString() != string.Empty)
				{
					dr["ParentGrpId94"] = ddl.SelectedValue;
					dr["ParentGrpId94Text"] = ddl.SelectedItem.Text;
				}
				else
				{
					dr["ParentGrpId94"] = System.DBNull.Value;
					dr["ParentGrpId94Text"] = System.DBNull.Value;
				}
			}
			tb = (TextBox)lvi.FindControl("cReportGrpName94");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["ReportGrpName94"] = tb.Text;} else {dr["ReportGrpName94"] = System.DBNull.Value;}
			}
			CheckBox cb = null;
			cb = (CheckBox)lvi.FindControl("cContentVertical94");
			if (cb != null)
			{
				dr["ContentVertical94"] = base.SetBool(cb.Checked);
			}
			cb = (CheckBox)lvi.FindControl("cLabelVertical94");
			if (cb != null)
			{
				dr["LabelVertical94"] = base.SetBool(cb.Checked);
			}
			tb = (TextBox)lvi.FindControl("cBorderWidth94");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["BorderWidth94"] = tb.Text;} else {dr["BorderWidth94"] = System.DBNull.Value;}
			}
			tb = (TextBox)lvi.FindControl("cGrpStyle94");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["GrpStyle94"] = tb.Text;} else {dr["GrpStyle94"] = System.DBNull.Value;}
			}
		    if (bInsert) { dt.Rows.InsertAt(dr, 0); }
			Session[KEY_dtAdmReportGrpGrid] = dt;
			cAdmReportGrpGrid.EditIndex = -1;
			cAdmReportGrpGrid_DataBind(dt.DefaultView);
		}

		protected void cAdmReportGrpGrid_OnItemUpdating(object sender, ListViewUpdateEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmReportGrpGrid];
			if (dt != null && dt.Rows.Count > 0)
			{
				DataRow dr = dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row;
			    GridFill(cAdmReportGrpGrid.EditItem, dt, dr, false);
			    dr["_NewRow"] = "N";
			    if (dt.Columns.Contains("_SortOrder")) dr["_SortOrder"] = null;
			}
		}

		protected void cAdmReportGrpGrid_OnItemCanceling(object sender, ListViewCancelEventArgs e)
		{
			PanelTop.Update();
			DataTable dt = (DataTable)Session[KEY_dtAdmReportGrpGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			...
			{
				cAdmReportGrpGrid_OnItemDeleting(sender, new ListViewDeleteEventArgs(e.ItemIndex));
			}
			if (dt.Columns.Contains("_SortOrder")) dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row["_SortOrder"] = null;
			cAdmReportGrpGrid.EditIndex = -1; cAdmReportGrpGrid_DataBind(null);
		}
		...
		protected void cDeleteAllButton_Click(object sender, System.EventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmReportGrpGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null && (cAdmReportGrpGrid.EditIndex < 0 || UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				GotoPage(0);
				while (dv.Count > 0) {dv.Delete(0);}
				Session[KEY_dtAdmReportGrpGrid] = dt;
				cAdmReportGrpGrid.EditIndex = -1; cAdmReportGrpGrid_DataBind(dv);
			}
		}

		protected void cAdmReportGrpGrid_OnItemDeleting(object sender, ListViewDeleteEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmReportGrpGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			DataRow dr = dv != null ? dv[GetDataItemIndex(e.ItemIndex)].Row : null;
			if (dv != null && (cAdmReportGrpGrid.EditIndex == e.ItemIndex || UpdateGridRow(sender, new CommandEventArgs("Save",""))))
			{
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmReportGrpGrid); PopAdmReportGrp65List(sender, e, false, null);
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			// *** System Button Click (Before) Web Rule starts here *** //
			if (ValidPage() && UpdateGridRow(cAdmReportGrpGrid, new CommandEventArgs("Save", "")))
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmReportGrpGrid];
				DataView dv = dt != null ? dt.DefaultView : null;
				string ftr = string.Empty;
				if (dv.RowFilter != string.Empty) { ftr = dv.RowFilter; dv.RowFilter = string.Empty; }
				AdmReportGrp65 ds = PrepAdmReportGrpData(dv,false);
				if (ftr != string.Empty) {dv.RowFilter = ftr;}
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(65,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc,noTrans,commandTimeOut)) {bValid7 = true;}
					if (bValid7)
					{
						Session[KEY_currPageIndex] = GetCurrPageIndex();
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmReportGrp65List(sender, e, false, null);
		...
		private AdmReportGrp65 PrepAdmReportGrpData(DataView dv, bool bAdd)
		{
			AdmReportGrp65 ds = new AdmReportGrp65();
			if (dv != null)
			{
				ds.Tables["AdmReportGrpDef"].Rows.Add(MakeTypRow(ds.Tables["AdmReportGrpDef"].NewRow()));
				ds.Tables["AdmReportGrpDef"].Rows.Add(MakeDisRow(ds.Tables["AdmReportGrpDef"].NewRow()));
					dv.RowStateFilter = DataViewRowState.ModifiedCurrent;
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmReportGrpUpd"].Rows.Add(MakeColRow(ds.Tables["AdmReportGrpUpd"].NewRow(), drv, false));
					}
					dv.RowStateFilter = DataViewRowState.Added;
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmReportGrpAdd"].Rows.Add(MakeColRow(ds.Tables["AdmReportGrpAdd"].NewRow(), drv, true));
					}
					dv.RowStateFilter = DataViewRowState.Deleted;
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmReportGrpDel"].Rows.Add(MakeColRow(ds.Tables["AdmReportGrpDel"].NewRow(), drv, false));
					}
		...
		private bool ValidPage()
		{
			...
			DataTable dt = null;
			dt = (DataTable)Session[KEY_dtReportId94];
			if (dt != null && dt.Rows.Count <= 0)
			{
				bErrNow.Value = "Y"; PreMsgPopup("Value is expected for 'ReportId', please investigate."); return false;
			}
			dt = (DataTable)Session[KEY_dtParentGrpId94];
			if (dt != null && dt.Rows.Count <= 0)
			{
				bErrNow.Value = "Y"; PreMsgPopup("Value is expected for 'ParentGrpId', please investigate."); return false;
			}
			return true;
		}
		...
		{
			dr["ReportGrpId94"] = System.Data.OleDb.OleDbType.Numeric.ToString();
			dr["ReportId94"] = System.Data.OleDb.OleDbType.Numeric.ToString();
			dr["ReportGrpIndex94"] = System.Data.OleDb.OleDbType.VarChar.ToString();
			dr["ParentGrpId94"] = System.Data.OleDb.OleDbType.Numeric.ToString();
			dr["ReportGrpName94"] = System.Data.OleDb.OleDbType.VarWChar.ToString();
			dr["ContentVertical94"] = System.Data.OleDb.OleDbType.Char.ToString();
			dr["LabelVertical94"] = System.Data.OleDb.OleDbType.Char.ToString();
			dr["BorderWidth94"] = System.Data.OleDb.OleDbType.Numeric.ToString();
			dr["GrpStyle94"] = System.Data.OleDb.OleDbType.VarChar.ToString();
			return dr;
		}

		private DataRow MakeDisRow(DataRow dr)
		{
			dr["ReportGrpId94"] = "TextBox";
			dr["ReportId94"] = "AutoComplete";
			dr["ReportGrpIndex94"] = "TextBox";
			dr["ParentGrpId94"] = "DropDownList";
			dr["ReportGrpName94"] = "TextBox";
			dr["ContentVertical94"] = "CheckBox";
			dr["LabelVertical94"] = "CheckBox";
			dr["BorderWidth94"] = "TextBox";
			dr["GrpStyle94"] = "TextBox";
			return dr;
		}
		...
		private DataRow MakeColRow(DataRow dr, DataRowView drv, bool bAdd)
		{
			...
			if (dtAuth != null)
			{
				dr["ReportGrpId94"] = drv["ReportGrpId94"].ToString().Trim();
				dr["ReportId94"] = drv["ReportId94"];
				if (bAdd && dtAuth.Rows[1]["ColReadOnly"].ToString() == "Y" && dr["ReportId94"].ToString() == string.Empty) {dr["ReportId94"] = System.DBNull.Value;}
				dr["ReportGrpIndex94"] = drv["ReportGrpIndex94"].ToString().Trim();
				if (bAdd && dtAuth.Rows[2]["ColReadOnly"].ToString() == "Y" && dr["ReportGrpIndex94"].ToString() == string.Empty) {dr["ReportGrpIndex94"] = System.DBNull.Value;}
				dr["ParentGrpId94"] = drv["ParentGrpId94"];
				if (bAdd && dtAuth.Rows[3]["ColReadOnly"].ToString() == "Y" && dr["ParentGrpId94"].ToString() == string.Empty) {dr["ParentGrpId94"] = System.DBNull.Value;}
				dr["ReportGrpName94"] = drv["ReportGrpName94"].ToString().Trim();
				dr["ContentVertical94"] = drv["ContentVertical94"];
				dr["LabelVertical94"] = drv["LabelVertical94"];
				dr["BorderWidth94"] = drv["BorderWidth94"].ToString().Trim();
				dr["GrpStyle94"] = drv["GrpStyle94"].ToString().Trim();
				if (bAdd && dtAuth.Rows[8]["ColReadOnly"].ToString() == "Y" && dr["GrpStyle94"].ToString() == string.Empty) {dr["GrpStyle94"] = System.DBNull.Value;}
			}
			return dr;
		}
		...
		{
			if (cAdmReportGrpGrid.EditIndex > -1)
			{
				cAdmReportGrpGrid_OnItemUpdating(sender, new ListViewUpdateEventArgs(cAdmReportGrpGrid.EditIndex));
			}
			return true;
		}

		protected void cAdmReportGrpGrid_OnPreRender(object sender, System.EventArgs e)
		{
			System.Web.UI.WebControls.Image hi = null;
			hi = (System.Web.UI.WebControls.Image)cAdmReportGrpGrid.FindControl("cReportGrpId94hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmReportGrpGrid.FindControl("cReportId94hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmReportGrpGrid.FindControl("cReportGrpIndex94hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmReportGrpGrid.FindControl("cParentGrpId94hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmReportGrpGrid.FindControl("cReportGrpName94hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmReportGrpGrid.FindControl("cContentVertical94hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmReportGrpGrid.FindControl("cLabelVertical94hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmReportGrpGrid.FindControl("cBorderWidth94hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmReportGrpGrid.FindControl("cGrpStyle94hi"); if (hi != null) { hi.Visible = false; }
			if (Session[KEY_lastSortImg] != null)
			{
				hi = (System.Web.UI.WebControls.Image)cAdmReportGrpGrid.FindControl((string)Session[KEY_lastSortImg]);
				if (hi != null) { hi.ImageUrl = Utils.AddTilde((string)Session[KEY_lastSortUrl]); hi.Visible = true; }
			}
			IgnoreHeaderConfirm((LinkButton)cAdmReportGrpGrid.FindControl("cReportGrpId94hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmReportGrpGrid.FindControl("cReportId94hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmReportGrpGrid.FindControl("cReportGrpIndex94hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmReportGrpGrid.FindControl("cParentGrpId94hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmReportGrpGrid.FindControl("cReportGrpName94hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmReportGrpGrid.FindControl("cContentVertical94hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmReportGrpGrid.FindControl("cLabelVertical94hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmReportGrpGrid.FindControl("cBorderWidth94hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmReportGrpGrid.FindControl("cGrpStyle94hl"));
		}

		private string GetButtonId(ListViewItem lvi)
		{
			string ButtonID = String.Empty;
			Control c = lvi.FindControl("cAdmReportGrpGridEdit");
		...
		{
		    return cAdmReportGrpGridDataPager.StartRowIndex / cAdmReportGrpGridDataPager.PageSize;
		}

		private int GetTotalPages()
		{
		    return (int)Math.Ceiling((double)cAdmReportGrpGridDataPager.TotalRowCount / cAdmReportGrpGridDataPager.PageSize);
		}

		private int GetDataItemIndex(int editIndex)
		{
		    return cAdmReportGrpGridDataPager.StartRowIndex + editIndex;
		}

		protected void cAdmReportGrpGrid_OnLayoutCreated(object sender, EventArgs e)
		{
		    // Header:
		    LinkButton lb = null;
		    lb = cAdmReportGrpGrid.FindControl("cReportGrpId94hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(0); lb.ToolTip = ColumnToolTip(0); lb.Parent.Visible = GridColumnVisible(0); }
		    lb = cAdmReportGrpGrid.FindControl("cReportId94hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(1); lb.ToolTip = ColumnToolTip(1); lb.Parent.Visible = GridColumnVisible(1); }
		    lb = cAdmReportGrpGrid.FindControl("cReportGrpIndex94hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(2); lb.ToolTip = ColumnToolTip(2); lb.Parent.Visible = GridColumnVisible(2); }
		    lb = cAdmReportGrpGrid.FindControl("cParentGrpId94hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(3); lb.ToolTip = ColumnToolTip(3); lb.Parent.Visible = GridColumnVisible(3); }
		    lb = cAdmReportGrpGrid.FindControl("cReportGrpName94hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(4); lb.ToolTip = ColumnToolTip(4); lb.Parent.Visible = GridColumnVisible(4); }
		    lb = cAdmReportGrpGrid.FindControl("cContentVertical94hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(5); lb.ToolTip = ColumnToolTip(5); lb.Parent.Visible = GridColumnVisible(5); }
		    lb = cAdmReportGrpGrid.FindControl("cLabelVertical94hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(6); lb.ToolTip = ColumnToolTip(6); lb.Parent.Visible = GridColumnVisible(6); }
		    lb = cAdmReportGrpGrid.FindControl("cBorderWidth94hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(7); lb.ToolTip = ColumnToolTip(7); lb.Parent.Visible = GridColumnVisible(7); }
		    lb = cAdmReportGrpGrid.FindControl("cGrpStyle94hl") as LinkButton;
			...
			if (dtAuthRow != null)
			{
				...
				if (dr["AllowDel"].ToString() == "N" || (Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N"))
				{
		            lb = cAdmReportGrpGrid.FindControl("cDeleteAllButton") as LinkButton; if (lb != null) { lb.Visible = false; }
				}
			}
		    // footer:
		    Label gc = null;
		    gc = cAdmReportGrpGrid.FindControl("cReportGrpId94fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(0); }
		    gc = cAdmReportGrpGrid.FindControl("cReportId94fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(1); }
		    gc = cAdmReportGrpGrid.FindControl("cReportGrpIndex94fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(2); }
		    gc = cAdmReportGrpGrid.FindControl("cParentGrpId94fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(3); }
		    gc = cAdmReportGrpGrid.FindControl("cReportGrpName94fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(4); }
		    gc = cAdmReportGrpGrid.FindControl("cContentVertical94fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(5); }
		    gc = cAdmReportGrpGrid.FindControl("cLabelVertical94fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(6); }
		    gc = cAdmReportGrpGrid.FindControl("cBorderWidth94fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(7); }
		    gc = cAdmReportGrpGrid.FindControl("cGrpStyle94fl") as Label;

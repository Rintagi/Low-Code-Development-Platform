...
{
	public class AdmStaticJs116 : DataSet
	{
		public AdmStaticJs116()
		{
			this.Tables.Add(MakeColumns(new DataTable("AdmStaticJs")));
			this.DataSetName = "AdmStaticJs116";
			this.Namespace = "http://Rintagi.com/DataSet/AdmStaticJs116";
		}

		private DataTable MakeColumns(DataTable dt)
		{
			DataColumnCollection columns = dt.Columns;
			columns.Add("StaticJsId261", typeof(string));
			columns.Add("StaticJsNm261", typeof(string));
			columns.Add("ScriptDef261", typeof(string));
			return dt;
		}
	}
}
...
namespace RO.Web
{
	public partial class AdmStaticJsModule : RO.Web.ModuleBase
	{
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_116";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_116";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_116";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_116";
		private const string KEY_dtLabel = "Cache:dtLabel3_116";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_116";
		private const string KEY_dtScrCri = "Cache:dtScrCri3_116";
		private const string KEY_dsScrCriVal = "Cache:dsScrCriVal3_116";


		private const string KEY_dtSystems = "Cache:dtSystems3_116";
		private const string KEY_sysConnectionString = "Cache:sysConnectionString3_116";
		private const string KEY_dtAdmStaticJs116List = "Cache:dtAdmStaticJs116List";
		private const string KEY_bNewVisible = "Cache:bNewVisible3_116";
		private const string KEY_bNewSaveVisible = "Cache:bNewSaveVisible3_116";
		private const string KEY_bCopyVisible = "Cache:bCopyVisible3_116";
		private const string KEY_bCopySaveVisible = "Cache:bCopySaveVisible3_116";
		private const string KEY_bDeleteVisible = "Cache:bDeleteVisible3_116";
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_116";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_116";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_116";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				DataTable dtMenuAccess = (new MenuSystem()).GetMenu(base.LUser.CultureId, 3, base.LImpr, LcSysConnString, LcAppPw,116, null, null);
				...
				Session.Remove(KEY_dtAdmStaticJs116List);
				...
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 116, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmStaticJs116List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=116&typ=N&sys=3','','',''); return false;"; }
		...
		private void SetSystem(byte SystemId)
		{
			...
			try
			{
				...
				if ((Config.DeployType == "DEV" || row["dbAppDatabase"].ToString() == base.CPrj.EntityCode + "View") && !(base.CPrj.EntityCode != "RO" && row["SysProgram"].ToString() == "Y") && (new AdminSystem()).IsRegenNeeded(string.Empty,116,0,0,LcSysConnString,LcAppPw))
				{
					(new GenScreensSystem()).CreateProgram(116, "Static Javascript", row["dbAppDatabase"].ToString(), base.CPrj, base.CSrc, base.CTar, LcAppConnString, LcAppPw);
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(116,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(116,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(116,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(116,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,116,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(116,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(116,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(116,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		protected DataTable GetAuthRow()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthRow(116,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				try
				{
					try {dv = new DataView((new AdminSystem()).GetExp(116,"GetExpAdmStaticJs116","Y",(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
					catch {dv = new DataView((new AdminSystem()).GetExp(116,"GetExpAdmStaticJs116","N",(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(116,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					{
						if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["StaticJsId261"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["StaticJsNm261"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["ScriptDef261"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						sb.Append(Environment.NewLine);
					}
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmStaticJs.csv"; Session["ExportStr"] = (Config.ExportExcelCSV ? "sep=\t\n": "") + sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmStaticJs.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private string GetRtfGrid(DataView dv)
		{
			...
			try
			{
				...
				foreach (DataRowView drv in dv)
				{
					...
					sb.Append("\r\n");
					sb.Append(@"{");
					if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["StaticJsId261"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["StaticJsNm261"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(drv["ScriptDef261"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					sb.Append(@"}");
					sb.Append("\r\n");
					sb.Append(@"\pard \ql \li0\ri0\widctlpar\intbl\aspalpha\aspnum\adjustright\rin0\lin0 {");
					sb.Append(@"\insrsid2981395 \trowd \irow0\irowband0\lastrow \ts15\trgaph108\trleft-108\trbrdrt\brdrs\brdrw10 \trbrdrl\brdrs\brdrw10 \trbrdrb\brdrs\brdrw10 \trbrdrr\brdrs\brdrw10 \trbrdrh\brdrs\brdrw10 \trbrdrv\brdrs\brdrw10 ");
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 116, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			...
			UpdCriteria(true);
			cAdmStaticJs116List.ClearSearch(); Session.Remove(KEY_dtAdmStaticJs116List);
			PopAdmStaticJs116List(sender, e, false, null);
		}
			...
			try
			{
				foreach (DataRowView drv in dvCri)
				{
					...
					if (drv["DisplayName"].ToString() == "ComboBox")
					{
						...
						(new AdminSystem()).MkGetScreenIn("116", drv["ScreenCriId"].ToString(), "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), LcAppDb, LcDesDb, drv["MultiDesignDb"].ToString(), LcSysConnString, LcAppPw);
						DataView dv = new DataView((new AdminSystem()).GetScreenIn("116", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), (new AdminSystem()).CountScrCri(drv["ScreenCriId"].ToString(), drv["MultiDesignDb"].ToString(), drv["MultiDesignDb"].ToString() == "N" ? LcSysConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)), drv["RequiredValid"].ToString(), 0, string.Empty, drv["DisplayMode"].ToString() != "AutoListBox", val, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)));
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("116", LcSysConnString, LcAppPw);
				}
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmStaticJs116" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmStaticJs116" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			foreach (DataRowView drv in dvCri)
			{
			    if (drv["DisplayName"].ToString() == "ListBox")
			    {
					...
					if (cListBox != null)
					{
						...
						int TotalChoiceCnt = new DataView((new AdminSystem()).GetScreenIn("116", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), CriCnt, drv["RequiredValid"].ToString(), 0, string.Empty, true, string.Empty, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId))).Count;
						string selectedValues = string.Join(",", cListBox.Items.Cast<ListItem>().Where(x => x.Selected).Select(x => "'" + x.Value + "'").ToArray());
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("116", "AdmStaticJs116", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		private DataView GetAdmStaticJs116List()
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmStaticJs116List];
			if (dt == null)
			{
				CheckAuthentication(false);
				int filterId = 0;
				...
				try
				{
					try {dt = (new AdminSystem()).GetLis(116,"GetLisAdmStaticJs116",true,"Y",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
					catch {dt = (new AdminSystem()).GetLis(116,"GetLisAdmStaticJs116",true,"N",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				}
		...
		private void PopAdmStaticJs116List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			DataView dv = GetAdmStaticJs116List();
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			context["method"] = "GetLisAdmStaticJs116";
			context["mKey"] = "StaticJsId261";
			context["mVal"] = "StaticJsId261Text";
			context["mTip"] = "StaticJsId261Text";
			context["mImg"] = "StaticJsId261Text";
			context["ssd"] = Request.QueryString["ssd"];
			context["scr"] = "116";
			context["csy"] = "3";
			...
			cAdmStaticJs116List.AutoCompleteUrl = "AutoComplete.aspx/LisSuggests";
			cAdmStaticJs116List.DataContext = context;
			if (dv.Table == null) return;
			cAdmStaticJs116List.DataSource = dv;
			cAdmStaticJs116List.Visible = true;
			if (cAdmStaticJs116List.Items.Count <= 0) {cAdmStaticJs116List.Visible = false; cAdmStaticJs116List_SelectedIndexChanged(sender,e);}
			else
			{
				if (bPageLoad && !string.IsNullOrEmpty(Request.QueryString["key"]) && bUseCri.Value == string.Empty)
				{
					try {cAdmStaticJs116List.SelectByValue(Request.QueryString["key"],string.Empty,true);} catch {cAdmStaticJs116List.Items[0].Selected = true; cAdmStaticJs116List_SelectedIndexChanged(sender, e);}
				    cScreenSearch.Visible = false; cSystem.Visible = false; cEditButton.Visible = true; cSaveCloseButton.Visible = cSaveButton.Visible;
				}
				else
				{
					if (ID != null) {if (!cAdmStaticJs116List.SelectByValue(ID,string.Empty,true)) {ID = null;}}
					if (ID == null)
					{
						cAdmStaticJs116List_SelectedIndexChanged(sender, e);
					}
				}
			}
		}
		...
		private void SetColumnAuthority()
		{
			...
			{
				base.SetFoldBehavior(cStaticJsId261, dtAuth.Rows[0], cStaticJsId261P1, cStaticJsId261Label, cStaticJsId261P2, null, dtLabel.Rows[0], null, null, null);
				base.SetFoldBehavior(cStaticJsNm261, dtAuth.Rows[1], cStaticJsNm261P1, cStaticJsNm261Label, cStaticJsNm261P2, null, dtLabel.Rows[1], cRFVStaticJsNm261, null, null);
				base.SetFoldBehavior(cScriptDef261, dtAuth.Rows[2], cScriptDef261P1, cScriptDef261Label, cScriptDef261P2, cScriptDef261E, null, dtLabel.Rows[2], null, null, null);
				cScriptDef261E.Attributes["label_id"] = cScriptDef261Label.ClientID; cScriptDef261E.Attributes["target_id"] = cScriptDef261.ClientID;
			}
			if ((cStaticJsNm261.Attributes["OnChange"] == null || cStaticJsNm261.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cStaticJsNm261.Visible && !cStaticJsNm261.ReadOnly) {cStaticJsNm261.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cScriptDef261.Attributes["OnChange"] == null || cScriptDef261.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cScriptDef261.Visible && !cScriptDef261.ReadOnly) {cScriptDef261.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
		}
		...
		private void ClearMaster(object sender, System.EventArgs e)
		{
			...
			if (dt.Rows[1]["ColVisible"].ToString() == "Y" && dt.Rows[1]["ColReadOnly"].ToString() != "Y") {cStaticJsNm261.Text = string.Empty;}
			if (dt.Rows[2]["ColVisible"].ToString() == "Y" && dt.Rows[2]["ColReadOnly"].ToString() != "Y") {cScriptDef261.Text = string.Empty;}
			// *** Default Value (Folder) Web Rule starts here *** //
		}

		private void InitMaster(object sender, System.EventArgs e)
		{
			cStaticJsId261.Text = string.Empty;
			cStaticJsNm261.Text = string.Empty;
			cScriptDef261.Text = string.Empty;
			// *** Default Value (Folder) Web Rule starts here *** //
		}
		protected void cAdmStaticJs116List_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmStaticJs116List_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmStaticJs116List_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			InitMaster(sender, e);      // Do this first to enable defaults for non-database columns.
			DataTable dt = null;
			try
			{
				dt = (new AdminSystem()).GetMstById("GetAdmStaticJs116ById",cAdmStaticJs116List.SelectedValue,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId));
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				if (dt.Rows.Count > 0)
				{
					cAdmStaticJs116List.SetDdVisible();
					DataRow dr = dt.Rows[0];
					try {cStaticJsId261.Text = RO.Common3.Utils.fmNumeric("0",dr["StaticJsId261"].ToString(),base.LUser.Culture);} catch {cStaticJsId261.Text = string.Empty;}
					try {cStaticJsNm261.Text = dr["StaticJsNm261"].ToString();} catch {cStaticJsNm261.Text = string.Empty;}
					try {cScriptDef261.Text = dr["ScriptDef261"].ToString();} catch {cScriptDef261.Text = string.Empty;}
				}
			}
			cButPanel.DataBind();
			ScriptManager.GetCurrent(Parent.Page).SetFocus(cAdmStaticJs116List.FocusID);
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cStaticJsId261.Text == string.Empty) { cClearButton_Click(sender, new EventArgs()); ShowDirty(false); PanelTop.Update();}
			else { PopAdmStaticJs116List(sender, e, false, cStaticJsId261.Text); }
			// *** System Button Click (After) Web Rule starts here *** //
		}
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmStaticJs116List); PopAdmStaticJs116List(sender, e, false, null);
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			if (ValidPage())
			{
				AdmStaticJs116 ds = PrepAdmStaticJsData(null,cStaticJsId261.Text == string.Empty);
				if (!string.IsNullOrEmpty(cAdmStaticJs116List.SelectedValue) && cStaticJsId261.Text != cAdmStaticJs116List.SelectedValue)
				{
					PreMsgPopup("Primary key cannot be changed");
					return rtn;
				}
				if (string.IsNullOrEmpty(cAdmStaticJs116List.SelectedValue))	// Add
				{
					if (ds != null)
					{
						pid = (new AdminSystem()).AddData(116,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc,noTrans,commandTimeOut);
					}
					if (!string.IsNullOrEmpty(pid))
					{
						cAdmStaticJs116List.ClearSearch(); Session.Remove(KEY_dtAdmStaticJs116List);
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmStaticJs116List(sender, e, false, pid);
						rtn = GetScreenHlp().Rows[0]["AddMsg"].ToString();
					}
				}
				else {
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(116,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc,noTrans,commandTimeOut)) {bValid7 = true;}
					if (bValid7)
					{
						cAdmStaticJs116List.ClearSearch(); Session.Remove(KEY_dtAdmStaticJs116List);
						ShowDirty(false); PopAdmStaticJs116List(sender, e, false, ds.Tables["AdmStaticJs"].Rows[0]["StaticJsId261"]);
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cStaticJsId261.Text != string.Empty)
			{
				AdmStaticJs116 ds = PrepAdmStaticJsData(null,false);
				try
				{
					if (ds != null && (new AdminSystem()).DelData(116,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc))
					{
						cAdmStaticJs116List.ClearSearch(); Session.Remove(KEY_dtAdmStaticJs116List);
						ShowDirty(false); PopAdmStaticJs116List(sender, e, false, null);
		...
		private AdmStaticJs116 PrepAdmStaticJsData(DataView dv, bool bAdd)
		{
			AdmStaticJs116 ds = new AdmStaticJs116();
			DataRow dr = ds.Tables["AdmStaticJs"].NewRow();
			DataRow drType = ds.Tables["AdmStaticJs"].NewRow();
			DataRow drDisp = ds.Tables["AdmStaticJs"].NewRow();
			if (bAdd) { dr["StaticJsId261"] = string.Empty; } else { dr["StaticJsId261"] = cStaticJsId261.Text; }
			drType["StaticJsId261"] = "Numeric"; drDisp["StaticJsId261"] = "TextBox";
			try {dr["StaticJsNm261"] = cStaticJsNm261.Text.Trim();} catch {}
			drType["StaticJsNm261"] = "VarWChar"; drDisp["StaticJsNm261"] = "TextBox";
			try {dr["ScriptDef261"] = cScriptDef261.Text;} catch {}
			drType["ScriptDef261"] = "VarWChar"; drDisp["ScriptDef261"] = "MultiLine";
			if (bAdd)
			{
			}
			ds.Tables["AdmStaticJs"].Rows.Add(dr); ds.Tables["AdmStaticJs"].Rows.Add(drType); ds.Tables["AdmStaticJs"].Rows.Add(drDisp);
			return ds;
		}

		public bool CanAct(char typ) { return CanAct(typ, GetAuthRow(), cAdmStaticJs116List.SelectedValue.ToString()); }
		...
		private void PreMsgPopup(string msg) { PreMsgPopup(msg,cAdmStaticJs116List,null); }

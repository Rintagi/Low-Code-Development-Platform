namespace RO.Web
{
    ...
    public partial class ComInstallModule : RO.Web.ModuleBase
    {
        ...
        private void Page_Load(object sender, System.EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                else
                {
                    ...
                    {
                        Tuple<string, bool, string> _license = (new RO.Rule3.AdminRules()).UpdateLicense(null, "");
                        if (_license.Item2)
                        {
        ...
        protected void cQuickGenerate_Click(object sender, System.EventArgs e)
        {
            string somethingRunning = Application["BuildRunning"] as string;
            string entity = CPrj.DeployPath.ToString();
            var installerName = Directory.GetParent(entity + "//").Name; // make sure it is 'directory'(end with slash) then get the name which is the 'installer' name
            string prepInstallerUrl = "AdmLicense.aspx?CreateInstaller=" + installerName;
            if (string.IsNullOrEmpty(somethingRunning) || true)
            {
                try
                {
                    var invokeResult = SelfInvoke(prepInstallerUrl, "text/plain");
                    var result = System.Text.UTF8Encoding.UTF8.GetString(invokeResult.content);
                    if (result == "submitted")
                    {
                        bInfoNow.Value = "Y";
                        PreMsgPopup("Request submitted via "
                            + prepInstallerUrl + ".\r\n" + " The installer package is being prepared and will be dropped to Git deploy directory. A confirmation email will be sent once it's ready."); return;
                    }
                    else
                    {
                        bErrNow.Value = "Y";
                        PreMsgPopup("Action ignored. It's currently " + somethingRunning + ". Please wait and try it again later."); return;
                    }
                }
                catch (Exception ex)
                {
                    bErrNow.Value = "Y";
                    PreMsgPopup("Error:" + ex.Message); return;
                }
            }
            else
            {

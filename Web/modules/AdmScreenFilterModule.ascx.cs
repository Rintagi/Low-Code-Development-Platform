...
namespace RO.Web
{
	public partial class AdmScreenFilterModule : RO.Web.ModuleBase
	{

		private const string KEY_lastAddedRow = "Cache:lastAddedRow3_59";
		private const string KEY_lastSortOrd = "Cache:lastSortOrd3_59";
		private const string KEY_lastSortImg = "Cache:lastSortImg3_59";
		private const string KEY_lastSortCol = "Cache:lastSortCol3_59";
		private const string KEY_lastSortExp = "Cache:lastSortExp3_59";
		private const string KEY_lastSortUrl = "Cache:lastSortUrl3_59";
		private const string KEY_lastSortTog = "Cache:lastSortTog3_59";
		private const string KEY_lastImpPwdOvride = "Cache:lastImpPwdOvride3_59";
		private const string KEY_cntImpPwdOvride = "Cache:cntImpPwdOvride3_59";
		private const string KEY_currPageIndex = "Cache:currPageIndex3_59";
		private const string KEY_bHiImpVisible = "Cache:bHiImpVisible3_59";
		private const string KEY_bShImpVisible = "Cache:bShImpVisible3_59";
		private const string KEY_dtAdmScreenFilterGrid = "Cache:dtAdmScreenFilterGrid";
		private const string KEY_scrImport = "Cache:scrImport3_59";
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_59";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_59";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_59";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_59";
		private const string KEY_dtLabel = "Cache:dtLabel3_59";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_59";
		private const string KEY_dtScrCri = "Cache:dtScrCri3_59";
		private const string KEY_dsScrCriVal = "Cache:dsScrCriVal3_59";

		private const string KEY_dtScreenId86 = "Cache:dtScreenId86";
		private const string KEY_dtCultureId87 = "Cache:dtCultureId87";

		private const string KEY_dtSystems = "Cache:dtSystems3_59";
		private const string KEY_sysConnectionString = "Cache:sysConnectionString3_59";
		private const string KEY_dtAdmScreenFilter59List = "Cache:dtAdmScreenFilter59List";
		private const string KEY_bNewVisible = "Cache:bNewVisible3_59";
		private const string KEY_bNewSaveVisible = "Cache:bNewSaveVisible3_59";
		private const string KEY_bCopyVisible = "Cache:bCopyVisible3_59";
		private const string KEY_bCopySaveVisible = "Cache:bCopySaveVisible3_59";
		private const string KEY_bDeleteVisible = "Cache:bDeleteVisible3_59";
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_59";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_59";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_59";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				DataTable dtMenuAccess = (new MenuSystem()).GetMenu(base.LUser.CultureId, 3, base.LImpr, LcSysConnString, LcAppPw,59, null, null);
				...
				Session.Remove(KEY_dtScreenId86);
				Session.Remove(KEY_dtCultureId87);
				SetButtonHlp();
				GetSystems();
				SetColumnAuthority();
				GetGlobalFilter();
				...
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 59, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmScreenFilter59List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=59&typ=N&sys=3','','',''); return false;"; }
			...
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmScreenFilterGrid];
				if (dt != null)
				{
		...
		private void InitializeComponent()
		{
			...
			if (LcSysConnString == null)
			{
				...
				{
					DataTable dt;
					dt = (new AdminSystem()).GetAuthRow(59,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
					...
					{
						cAdmScreenFilterGrid.PreRender += new EventHandler(cAdmScreenFilterGrid_OnPreRender);
					}
				}
		...
		private void SetSystem(byte SystemId)
		{
			...
			try
			{
				...
				if ((Config.DeployType == "DEV" || row["dbAppDatabase"].ToString() == base.CPrj.EntityCode + "View") && !(base.CPrj.EntityCode != "RO" && row["SysProgram"].ToString() == "Y") && (new AdminSystem()).IsRegenNeeded(string.Empty,59,0,0,LcSysConnString,LcAppPw))
				{
					(new GenScreensSystem()).CreateProgram(59, "Screen Filters", row["dbAppDatabase"].ToString(), base.CPrj, base.CSrc, base.CTar, LcAppConnString, LcAppPw);
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(59,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(59,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void SetClientRule(ListViewDataItem ee, bool isEditItem)
		{
			...
			if (dvRul.Count > 0)
			{
				...
				{
					if (ee == null || drv["ScriptEvent"].ToString().Substring(0,2).ToLower() != "on" || (cAdmScreenFilterGrid.EditIndex > -1 && GetDataItemIndex(cAdmScreenFilterGrid.EditIndex) == ee.DataItemIndex))
					{
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(59,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(59,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,59,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(59,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(59,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(59,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				try
				{
					try {dv = new DataView((new AdminSystem()).GetExp(59,"GetExpAdmScreenFilter59","Y",(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
					catch {dv = new DataView((new AdminSystem()).GetExp(59,"GetExpAdmScreenFilter59","N",(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(59,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					{
						if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["ScreenFilterId86"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["ScreenId86"].ToString() + (char)9 + drv["ScreenId86Text"].ToString() + (char)9);}
						if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["ScreenFilterName86"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["FilterClause86"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["FilterOrder86"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["ApplyToMst86"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["ScreenFilterHlpId87"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[7]["ColExport"].ToString() == "Y") {sb.Append(drv["CultureId87"].ToString() + (char)9 + drv["CultureId87Text"].ToString() + (char)9);}
						if (dtAu.Rows[8]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["FilterName87"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						sb.Append(Environment.NewLine);
					}
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmScreenFilter.csv"; Session["ExportStr"] = (Config.ExportExcelCSV ? "sep=\t\n": "") + sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmScreenFilter.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private string GetRtfGrid(DataView dv)
		{
			...
			try
			{
				...
				foreach (DataRowView drv in dv)
				{
					...
					sb.Append("\r\n");
					sb.Append(@"{");
					if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["ScreenFilterId86"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["ScreenId86Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(drv["ScreenFilterName86"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(drv["FilterClause86"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["FilterOrder86"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append(drv["ApplyToMst86"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["ScreenFilterHlpId87"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[7]["ColExport"].ToString() == "Y") {sb.Append(drv["CultureId87Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[8]["ColExport"].ToString() == "Y") {sb.Append(drv["FilterName87"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					sb.Append(@"}");
					sb.Append("\r\n");
					sb.Append(@"\pard \ql \li0\ri0\widctlpar\intbl\aspalpha\aspnum\adjustright\rin0\lin0 {");
					sb.Append(@"\insrsid2981395 \trowd \irow0\irowband0\lastrow \ts15\trgaph108\trleft-108\trbrdrt\brdrs\brdrw10 \trbrdrl\brdrs\brdrw10 \trbrdrb\brdrs\brdrw10 \trbrdrr\brdrs\brdrw10 \trbrdrh\brdrs\brdrw10 \trbrdrv\brdrs\brdrw10 ");
		...
		private void GetPageInfo()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastPageInfo(59, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 59, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			// *** Criteria Trigger (before) Web Rule starts here *** //
			DataTable dt = (DataTable)Session[KEY_dtAdmScreenFilterGrid];
			if (cAdmScreenFilterGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
			if (IsPostBack && cCriteria.Visible) ReBindCriteria(GetScrCriteria());
			UpdCriteria(true);
			cAdmScreenFilter59List.ClearSearch(); Session.Remove(KEY_dtAdmScreenFilter59List);
			PopAdmScreenFilter59List(sender, e, false, null);
			}
		}
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("59", LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmScreenFilter59" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmScreenFilter59" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("59", "AdmScreenFilter59", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		protected void cbScreenId86(object sender, System.EventArgs e)
		{
			SetScreenId86((RoboCoder.WebControls.ComboBox)sender,string.Empty);
		}

		private void SetScreenId86(RoboCoder.WebControls.ComboBox ddl, string keyId)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(59,"GetDdlScreenId3S841",true,false,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
					if (dv.Table.Columns.Contains("ScreenFilterId"))
					{
						context["pMKeyColID"] = cAdmScreenFilter59List.ClientID;
						context["pMKeyCol"] = "ScreenFilterId";
						string ss = "(ScreenFilterId is null";
						if (string.IsNullOrEmpty(cAdmScreenFilter59List.SelectedValue)) {ss = ss + ")";} else {ss = ss + " OR ScreenFilterId = " + cAdmScreenFilter59List.SelectedValue + ")";}
						dv.RowFilter = ss;
					}
					ddl.DataSource = dv; Session[KEY_dtScreenId86] = dv.Table;
				    try { ddl.SelectByValue(keyId,string.Empty,true); } catch { try { ddl.SelectedIndex = 0; } catch { } }
				}
			}
		}

		protected void cbCultureId87(object sender, System.EventArgs e)
		{
			SetCultureId87((RoboCoder.WebControls.ComboBox)sender,string.Empty,cAdmScreenFilterGrid.EditItem);
		}

		private void SetCultureId87(RoboCoder.WebControls.ComboBox ddl, string keyId, ListViewItem li)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(59,"GetDdlCultureId3S848",true,false,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
					...
					ddl.DataSource = dv; Session[KEY_dtCultureId87] = dv.Table;
				    try { ddl.SelectByValue(keyId,string.Empty,true); } catch { try { ddl.SelectedIndex = 0; } catch { } }
				}
			}
		}

		private DataView GetAdmScreenFilter59List()
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmScreenFilter59List];
			if (dt == null)
			{
				CheckAuthentication(false);
				int filterId = 0;
				...
				try
				{
					try {dt = (new AdminSystem()).GetLis(59,"GetLisAdmScreenFilter59",true,"Y",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
					catch {dt = (new AdminSystem()).GetLis(59,"GetLisAdmScreenFilter59",true,"N",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				}
		...
		private void PopAdmScreenFilter59List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			DataView dv = GetAdmScreenFilter59List();
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			context["isSys"] = "N";
			context["conn"] = KEY_sysConnectionString;
			cAdmScreenFilter59List.AutoCompleteUrl = "AutoComplete.aspx/LisSuggests";
			cAdmScreenFilter59List.DataContext = context;
			if (dv.Table == null) return;
			cAdmScreenFilter59List.DataSource = dv;
			cAdmScreenFilter59List.Visible = true;
			if (cAdmScreenFilter59List.Items.Count <= 0) {cAdmScreenFilter59List.Visible = false; cAdmScreenFilter59List_SelectedIndexChanged(sender,e);}
			else
			{
				if (bPageLoad && !string.IsNullOrEmpty(Request.QueryString["key"]) && bUseCri.Value == string.Empty)
				{
					try {cAdmScreenFilter59List.SelectByValue(Request.QueryString["key"],string.Empty,true);} catch {cAdmScreenFilter59List.Items[0].Selected = true; cAdmScreenFilter59List_SelectedIndexChanged(sender, e);}
				    cScreenSearch.Visible = false; cSystem.Visible = false; cEditButton.Visible = true; cSaveCloseButton.Visible = cSaveButton.Visible;
				}
				else
				{
					if (ID != null) {if (!cAdmScreenFilter59List.SelectByValue(ID,string.Empty,true)) {ID = null;}}
					if (ID == null)
					{
						cAdmScreenFilter59List_SelectedIndexChanged(sender, e);
					}
				}
			}
		}
		...
		private void SetColumnAuthority()
		{
			...
			{
				base.SetFoldBehavior(cScreenFilterId86, dtAuth.Rows[0], cScreenFilterId86P1, cScreenFilterId86Label, cScreenFilterId86P2, null, dtLabel.Rows[0], null, null, null);
				base.SetFoldBehavior(cScreenId86, dtAuth.Rows[1], cScreenId86P1, cScreenId86Label, cScreenId86P2, null, dtLabel.Rows[1], cRFVScreenId86, null, null);
				SetScreenId86(cScreenId86,string.Empty);
				base.SetFoldBehavior(cScreenFilterName86, dtAuth.Rows[2], cScreenFilterName86P1, cScreenFilterName86Label, cScreenFilterName86P2, null, dtLabel.Rows[2], cRFVScreenFilterName86, null, null);
				base.SetFoldBehavior(cFilterClause86, dtAuth.Rows[3], cFilterClause86P1, cFilterClause86Label, cFilterClause86P2, cFilterClause86E, null, dtLabel.Rows[3], cRFVFilterClause86, null, null);
				cFilterClause86E.Attributes["label_id"] = cFilterClause86Label.ClientID; cFilterClause86E.Attributes["target_id"] = cFilterClause86.ClientID;
				base.SetFoldBehavior(cFilterOrder86, dtAuth.Rows[4], cFilterOrder86P1, cFilterOrder86Label, cFilterOrder86P2, null, dtLabel.Rows[4], cRFVFilterOrder86, null, null);
				base.SetFoldBehavior(cApplyToMst86, dtAuth.Rows[5], cApplyToMst86P1, cApplyToMst86Label, cApplyToMst86P2, null, dtLabel.Rows[5], null, null, null);
			}
			if ((cScreenId86.Attributes["OnChange"] == null || cScreenId86.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cScreenId86.Visible && cScreenId86.Enabled) {cScreenId86.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cScreenFilterName86.Attributes["OnChange"] == null || cScreenFilterName86.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cScreenFilterName86.Visible && !cScreenFilterName86.ReadOnly) {cScreenFilterName86.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cFilterClause86.Attributes["OnChange"] == null || cFilterClause86.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cFilterClause86.Visible && !cFilterClause86.ReadOnly) {cFilterClause86.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cFilterOrder86.Attributes["OnChange"] == null || cFilterOrder86.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cFilterOrder86.Visible && !cFilterOrder86.ReadOnly) {cFilterOrder86.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cApplyToMst86.Attributes["OnClick"] == null || cApplyToMst86.Attributes["OnClick"].IndexOf("ChkPgDirty") < 0) && cApplyToMst86.Visible && cApplyToMst86.Enabled) {cApplyToMst86.Attributes["OnClick"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty(); this.focus();";}
		}
		...
		private void ClearMaster(object sender, System.EventArgs e)
		{
			...
			if (dt.Rows[1]["ColVisible"].ToString() == "Y" && dt.Rows[1]["ColReadOnly"].ToString() != "Y") {cScreenId86.ClearSearch();}
			if (dt.Rows[2]["ColVisible"].ToString() == "Y" && dt.Rows[2]["ColReadOnly"].ToString() != "Y") {cScreenFilterName86.Text = string.Empty;}
			if (dt.Rows[3]["ColVisible"].ToString() == "Y" && dt.Rows[3]["ColReadOnly"].ToString() != "Y") {cFilterClause86.Text = string.Empty;}
			if (dt.Rows[4]["ColVisible"].ToString() == "Y" && dt.Rows[4]["ColReadOnly"].ToString() != "Y") {cFilterOrder86.Text = string.Empty;}
			if (dt.Rows[5]["ColVisible"].ToString() == "Y" && dt.Rows[5]["ColReadOnly"].ToString() != "Y") {cApplyToMst86.Checked = base.GetBool("Y");}
			// *** Default Value (Folder) Web Rule starts here *** //
		}

		private void InitMaster(object sender, System.EventArgs e)
		{
			cScreenFilterId86.Text = string.Empty;
			cScreenId86.ClearSearch();
			cScreenFilterName86.Text = string.Empty;
			cFilterClause86.Text = string.Empty;
			cFilterOrder86.Text = string.Empty;
			cApplyToMst86.Checked = base.GetBool("Y");
			// *** Default Value (Folder) Web Rule starts here *** //
		}
		protected void cAdmScreenFilter59List_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmScreenFilter59List_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmScreenFilter59List_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			InitMaster(sender, e);      // Do this first to enable defaults for non-database columns.
			DataTable dt = null;
			try
			{
				dt = (new AdminSystem()).GetMstById("GetAdmScreenFilter59ById",cAdmScreenFilter59List.SelectedValue,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId));
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				if (dt.Rows.Count > 0)
				{
					cAdmScreenFilter59List.SetDdVisible();
					DataRow dr = dt.Rows[0];
					try {cScreenFilterId86.Text = RO.Common3.Utils.fmNumeric("0",dr["ScreenFilterId86"].ToString(),base.LUser.Culture);} catch {cScreenFilterId86.Text = string.Empty;}
					SetScreenId86(cScreenId86,dr["ScreenId86"].ToString());
					try {cScreenFilterName86.Text = dr["ScreenFilterName86"].ToString();} catch {cScreenFilterName86.Text = string.Empty;}
					try {cFilterClause86.Text = dr["FilterClause86"].ToString();} catch {cFilterClause86.Text = string.Empty;}
					try {cFilterOrder86.Text = RO.Common3.Utils.fmNumeric("0",dr["FilterOrder86"].ToString(),base.LUser.Culture);} catch {cFilterOrder86.Text = string.Empty;}
					try {cApplyToMst86.Checked = base.GetBool(dr["ApplyToMst86"].ToString());} catch {cApplyToMst86.Checked = false;}
				}
			}
			cButPanel.DataBind(); if (!cSaveButton.Visible) { cInsRowButton.Visible = false; }
			DataTable dtAdmScreenFilterGrid = null;
			int filterId = 0; if (Utils.IsInt(cFilterId.SelectedValue)) { filterId = int.Parse(cFilterId.SelectedValue); }
			try
			{
				dtAdmScreenFilterGrid = (new AdminSystem()).GetDtlById(59,"GetAdmScreenFilter59DtlById",cAdmScreenFilter59List.SelectedValue,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,base.LImpr,base.LCurr);
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (!dtAdmScreenFilterGrid.Columns.Contains("_NewRow")) { dtAdmScreenFilterGrid.Columns.Add("_NewRow"); }
			Session[KEY_dtAdmScreenFilterGrid] = dtAdmScreenFilterGrid;
			cAdmScreenFilterGrid.EditIndex = -1;
			cAdmScreenFilterGridDataPager.PageSize = Int16.Parse(cPgSize.Text); GotoPage(0);
			if (Session[KEY_lastSortCol] != null)
			{
				cAdmScreenFilterGrid_OnSorting(sender, new ListViewSortEventArgs((string)Session[KEY_lastSortExp], SortDirection.Ascending));
			}
			else if (dtAdmScreenFilterGrid.Rows.Count <= 0 || (!((GetAuthRow().Rows[0]["AllowUpd"].ToString() == "N" || GetAuthRow().Rows[0]["ViewOnly"].ToString() == "G") && GetAuthRow().Rows[0]["AllowAdd"].ToString() == "N") && !(Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N") && dtAdmScreenFilterGrid.Rows.Count == 1))
			{
				cAdmScreenFilterGrid_DataBind(dtAdmScreenFilterGrid.DefaultView);
			}
			cFindButton_Click(sender, e);
			cNaviPanel.Visible = true; cImportPwdPanel.Visible = false; Session.Remove(KEY_scrImport);
			ScriptManager.GetCurrent(Parent.Page).SetFocus(cAdmScreenFilter59List.FocusID);
		...
		public void cFindButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dv != null)
			{
				...
				if ((cc != null && cc.ID.Equals("cAdmScreenFilter59List")) || cAdmScreenFilterGrid.EditIndex < 0 || UpdateGridRow(cAdmScreenFilterGrid, new CommandEventArgs("Save", "")))
				{
					...
					GotoPage(0); cAdmScreenFilterGrid_DataBind(dv);
					if (GetCurrPageIndex() != (int)Session[KEY_currPageIndex] && (int)Session[KEY_currPageIndex] < GetTotalPages()) { GotoPage((int)Session[KEY_currPageIndex]); cAdmScreenFilterGrid_DataBind(dv);}
		...
		public void cInsRowButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dt != null && (ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")))
			{
				...
				Session[KEY_lastAddedRow] = 0; Session[KEY_dtAdmScreenFilterGrid] = dt; Session[KEY_currPageIndex] = 0; GotoPage(0);
				cAdmScreenFilterGrid_DataBind(dt.DefaultView);
				cAdmScreenFilterGrid_OnItemEditing(cAdmScreenFilterGrid, new ListViewEditEventArgs(0));
		...
		public void cPgSizeButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cAdmScreenFilterGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				...
				cAdmScreenFilterGridDataPager.PageSize = int.Parse(cPgSize.Text); GotoPage(0);
				if (dt.Rows.Count <= 0 || (!((GetAuthRow().Rows[0]["AllowUpd"].ToString() == "N" || GetAuthRow().Rows[0]["ViewOnly"].ToString() == "G") && GetAuthRow().Rows[0]["AllowAdd"].ToString() == "N") && !(Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N") && dt.Rows.Count == 1)) {cAdmScreenFilterGrid_DataBind(dt.DefaultView);} else {cFindButton_Click(sender, e);}
				try
				{
					(new AdminSystem()).UpdLastPageInfo(59, base.LUser.UsrId, cPgSize.Text, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		}
		...
		public void cShowImpButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cSchemaImage.Visible)
			{
				try
				{
					Session["ImportSchema"] = (new AdminSystem()).GetSchemaScrImp(59,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (cSchemaImage.Attributes["OnClick"] == null || cSchemaImage.Attributes["OnClick"].IndexOf("ImportSchema.aspx") < 0) {cSchemaImage.Attributes["OnClick"] += "SearchLink('ImportSchema.aspx?scm=S&key=59&csy=3','','',''); return false;";}
			}
			// *** System Button Click (after) Web Rule starts here *** //
		}
		...
		private void ScrImportDdl(DataView dvd, DataRowView drv, bool bComboBox, string PKey, string CKey, string CNam, int MaxLen, string MatchCd, bool bAllowNulls)
		{
			if (dvd != null)
			{
				...
				{
					if (string.IsNullOrEmpty(cAdmScreenFilter59List.SelectedValue))
					{
						dvd.RowFilter = "(" + PKey + " is null)";
					}
					else
					{
						dvd.RowFilter = "(" + PKey + " is null OR " + PKey + " = " + cAdmScreenFilter59List.SelectedValue + ")";
					}
				}
		...
		protected void ScrImport(bool bClear)
		{
			...
			DataTable dtAdmScreenFilterGrid = (DataTable)Session[KEY_dtAdmScreenFilterGrid];
			if (dti != null && dtAdmScreenFilterGrid != null)
			{
				if (bClear)
				{
					dtAdmScreenFilterGrid.Rows.Clear();
				}
				cFind.Text = string.Empty;
				int iExist = dtAdmScreenFilterGrid.Rows.Count;
				...
				foreach (DataRowView drv in dti.DefaultView)
				{
					...
					{
						DataTable dtd;
						dtd = jj==iExist ? null : (DataTable)Session[KEY_dtCultureId87];
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(59,"GetDdlCultureId3S848",true,true,0,string.Empty,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
							Session[KEY_dtCultureId87] = dtd;
						}
						ScrImportDdl(dtd.DefaultView, drv, true, "ScreenFilterId", "CultureId87", "CultureId87Text", 50, "2", false);
					}
					jj = jj + 1;
				}
				iRow = dti.DefaultView.Count - iExist;
				if (!dti.Columns.Contains("_NewRow")) { dti.Columns.Add("_NewRow"); }
				Session[KEY_dtAdmScreenFilterGrid] = dti;
				cAdmScreenFilterGrid_DataBind(dti.DefaultView);
		...
		public void cImportButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cFNameO.Text != string.Empty && cWorkSheet.Items.Count > 0 && cWorkSheet.SelectedItem.Text != string.Empty && cStartRow.Text != string.Empty)
			{
				...
				try
				{
					if (cAdmScreenFilterGrid.EditIndex < 0 || UpdateGridRow(cAdmScreenFilterGrid, new CommandEventArgs("Save", "")))
					{
						Session[KEY_scrImport] = ImportFile(dtAdmScreenFilterGrid.Copy(),cFNameO.Text,cWorkSheet.SelectedItem.Text,cStartRow.Text,Config.PathTmpImport + cFName.Text);
					}
		...
		private void GotoPage(int pageNo)
		{
			if (pageNo >= 0 && pageNo < GetTotalPages())
			{
				if (cAdmScreenFilterGrid.EditIndex < 0 || UpdateGridRow(cAdmScreenFilterGrid, new CommandEventArgs("Save", "")))
				{
					try { cAdmScreenFilterGridDataPager.SetPageProperties(cAdmScreenFilterGridDataPager.PageSize * pageNo, cAdmScreenFilterGridDataPager.MaximumRows, false); cAdmScreenFilterGrid_DataBind(null); }
					catch
					{
						try { cAdmScreenFilterGridDataPager.SetPageProperties(0, cAdmScreenFilterGridDataPager.MaximumRows, false); cAdmScreenFilterGrid_DataBind(null); } catch {}
					}
				}
			}
		}

		protected void cAdmScreenFilterGrid_OnItemCommand(object sender, ListViewCommandEventArgs e)
		{
		}

		private void cAdmScreenFilterGrid_DataBind(DataView dvAdmScreenFilterGrid)
		{
			if (dvAdmScreenFilterGrid == null)
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmScreenFilterGrid];
				dvAdmScreenFilterGrid = dt != null ? dt.DefaultView : null;
			}
			if (dvAdmScreenFilterGrid != null)
			{
				LcAuth = GetAuthCol().AsEnumerable().ToDictionary<DataRow,string>(dr=>dr["ColName"].ToString());
				cAdmScreenFilterGrid.DataSource = dvAdmScreenFilterGrid;
				cAdmScreenFilterGrid.DataBind();
		...
		protected void cAdmScreenFilterGrid_OnSorting(object sender, ListViewSortEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmScreenFilterGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null)
			{
				if (cAdmScreenFilterGrid.EditIndex < 0 || UpdateGridRow(cAdmScreenFilterGrid, new CommandEventArgs("Save", "")))
				{
					...
					Session[KEY_dtAdmScreenFilterGrid] = dt;
					cAdmScreenFilterGrid_DataBind(dv);
				}
			}
		}

		protected void cAdmScreenFilterGrid_OnPagePropertiesChanging(object sender, PagePropertiesChangingEventArgs e)
		{
		    cAdmScreenFilterGridDataPager.SetPageProperties(e.StartRowIndex, e.MaximumRows, false);
		    DataTable dt = (DataTable)Session[KEY_dtAdmScreenFilterGrid];
		    DataView dv = dt != null ? dt.DefaultView : null;
			cAdmScreenFilterGrid_DataBind(dv); cAdmScreenFilterGrid.EditIndex = -1;
		}

		private void GridChkPgDirty(ListViewItem lvi)
		{
				WebControl cc = null;
				cc = ((WebControl)lvi.FindControl("cCultureId87"));
				...
				cc = ((WebControl)lvi.FindControl("cFilterName87"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
		}

		protected void cAdmScreenFilterGrid_OnItemDataBound(object sender, ListViewItemEventArgs e)
		{
			// *** GridItemDataBound (before) Web Rule End *** //
			DataTable dt = (DataTable)Session[KEY_dtAdmScreenFilterGrid];
			bool isEditItem = false;
			DataView dvAdmScreenFilterGrid = dt != null ? dt.DefaultView : null;
			if (cAdmScreenFilterGrid.EditIndex > -1 && GetDataItemIndex(cAdmScreenFilterGrid.EditIndex) == e.Item.DataItemIndex)
			{
				isEditItem = true;
				base.SetGridEnabled(e.Item, GetAuthCol(), GetLabel(), 6);
				if (dvAdmScreenFilterGrid[e.Item.DataItemIndex]["CultureId87"].ToString() != string.Empty) {SetCultureId87((RoboCoder.WebControls.ComboBox)e.Item.FindControl("cCultureId87"),dvAdmScreenFilterGrid[e.Item.DataItemIndex]["CultureId87"].ToString(), e.Item);} else {SetCultureId87((RoboCoder.WebControls.ComboBox)e.Item.FindControl("cCultureId87"),"1", e.Item);}
			...
			if (dtAuthRow != null)
			{
				...
				{
					cc = e.Item.FindControl("cAdmScreenFilterGridDelete"); if (cc != null) { cc.Visible = false; }
					cc = e.Item.FindControl("cAdmScreenFilterGridEdit"); if (cc != null) { cc.Visible = false; }
				}
				else
				{
			        HtmlTableRow tr = e.Item.FindControl("cAdmScreenFilterGridRow") as HtmlTableRow;
			        LinkButton lb = e.Item.FindControl("cAdmScreenFilterGridEdit") as LinkButton;
		...
		protected void cScreenFilterHlpId87hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "0") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "0"; Session[KEY_lastSortExp] = "ScreenFilterHlpId87";Session[KEY_lastSortImg] = "cScreenFilterHlpId87hi";
			cAdmScreenFilterGrid_OnSorting(sender, new ListViewSortEventArgs("ScreenFilterHlpId87", SortDirection.Ascending));
		}

		protected void cCultureId87hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "1") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "1"; Session[KEY_lastSortExp] = "CultureId87Text";Session[KEY_lastSortImg] = "cCultureId87hi";
			cAdmScreenFilterGrid_OnSorting(sender, new ListViewSortEventArgs("CultureId87Text", SortDirection.Ascending));
		}

		protected void cFilterName87hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "2") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "2"; Session[KEY_lastSortExp] = "FilterName87";Session[KEY_lastSortImg] = "cFilterName87hi";
			cAdmScreenFilterGrid_OnSorting(sender, new ListViewSortEventArgs("FilterName87", SortDirection.Ascending));
		}

		protected void cAdmScreenFilterGrid_OnItemEditing(object sender, ListViewEditEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmScreenFilterGrid];
			if ((ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")) && dt != null && dt.DefaultView.Count > GetDataItemIndex(e.NewEditIndex))
			{
				cAdmScreenFilterGrid.EditIndex = e.NewEditIndex;
				cAdmScreenFilterGrid_DataBind(null);
				Control wc = null;
				string idx = Page.Request["__EVENTARGUMENT"];
				if (!string.IsNullOrEmpty(idx))
				{
				    Control ctrlToFocus = cAdmScreenFilterGrid.EditItem.FindControl("c" + idx);
				    if (((WebControl)ctrlToFocus).Enabled) { wc = ctrlToFocus; }
				}
				if (wc == null) { wc = FindEditableControl(cAdmScreenFilterGrid.EditItem); }
				if (wc != null)
             {
             ...
             MkMobileLabel(cAdmScreenFilterGrid.EditItem);
		    }
		    else { e.Cancel = true; }
		}
		...
		{
		    Label ml = null;
		    ml = lvi.FindControl("cScreenFilterHlpId87ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(6); }
		    ml = lvi.FindControl("cCultureId87ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(7); }
		    ml = lvi.FindControl("cFilterName87ml") as Label;
		...
		protected void GridFill(ListViewItem lvi, DataTable dt, DataRow dr, bool bInsert)
		{
			...
			cbb = (RoboCoder.WebControls.ComboBox)lvi.FindControl("cCultureId87");
			if (cbb != null)
			{
				if (cbb.SelectedValue != string.Empty)
				{
					dr["CultureId87"] = cbb.SelectedValue;
					dr["CultureId87Text"] = cbb.SelectedText;
				}
				else
				{
					dr["CultureId87"] = System.DBNull.Value;
					dr["CultureId87Text"] = System.DBNull.Value;
				}
			}
			TextBox tb = null;
			tb = (TextBox)lvi.FindControl("cFilterName87");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["FilterName87"] = tb.Text;} else {dr["FilterName87"] = System.DBNull.Value;}
			}
		...
		protected void cAdmScreenFilterGrid_OnItemUpdating(object sender, ListViewUpdateEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmScreenFilterGrid];
			if (dt != null && dt.Rows.Count > 0)
			{
				DataRow dr = dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row;
			    GridFill(cAdmScreenFilterGrid.EditItem, dt, dr, false);
			    dr["_NewRow"] = "N";
			    if (dt.Columns.Contains("_SortOrder")) dr["_SortOrder"] = null;
			}
		}

		protected void cAdmScreenFilterGrid_OnItemCanceling(object sender, ListViewCancelEventArgs e)
		{
			PanelTop.Update();
			DataTable dt = (DataTable)Session[KEY_dtAdmScreenFilterGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			...
			{
				cAdmScreenFilterGrid_OnItemDeleting(sender, new ListViewDeleteEventArgs(e.ItemIndex));
			}
			if (dt.Columns.Contains("_SortOrder")) dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row["_SortOrder"] = null;
			cAdmScreenFilterGrid.EditIndex = -1; cAdmScreenFilterGrid_DataBind(null);
		}
		...
		protected void cDeleteAllButton_Click(object sender, System.EventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmScreenFilterGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null && (cAdmScreenFilterGrid.EditIndex < 0 || UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				GotoPage(0);
				while (dv.Count > 0) {dv.Delete(0);}
				Session[KEY_dtAdmScreenFilterGrid] = dt;
				cAdmScreenFilterGrid.EditIndex = -1; cAdmScreenFilterGrid_DataBind(dv);
			}
		}

		protected void cAdmScreenFilterGrid_OnItemDeleting(object sender, ListViewDeleteEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmScreenFilterGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			DataRow dr = dv != null ? dv[GetDataItemIndex(e.ItemIndex)].Row : null;
			if (dv != null && (cAdmScreenFilterGrid.EditIndex == e.ItemIndex || UpdateGridRow(sender, new CommandEventArgs("Save",""))))
			{
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cScreenFilterId86.Text == string.Empty) { cClearButton_Click(sender, new EventArgs()); ShowDirty(false); PanelTop.Update();}
			else { PopAdmScreenFilter59List(sender, e, false, cScreenFilterId86.Text); }
			// *** System Button Click (After) Web Rule starts here *** //
		}
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmScreenFilter59List); PopAdmScreenFilter59List(sender, e, false, null);
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			if (ValidPage() && (cAdmScreenFilterGrid.EditIndex < 0 || UpdateGridRow(cAdmScreenFilterGrid, new CommandEventArgs("Save", ""))))
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmScreenFilterGrid];
				DataView dv = dt != null ? dt.DefaultView : null;
				string ftr = string.Empty;
				if (dv.RowFilter != string.Empty) { ftr = dv.RowFilter; dv.RowFilter = string.Empty; }
				AdmScreenFilter59 ds = PrepAdmScreenFilterData(dv,cScreenFilterId86.Text == string.Empty);
				if (ftr != string.Empty) {dv.RowFilter = ftr;}
				if (!string.IsNullOrEmpty(cAdmScreenFilter59List.SelectedValue) && cScreenFilterId86.Text != cAdmScreenFilter59List.SelectedValue)
				{
					PreMsgPopup("Primary key cannot be changed");
					return rtn;
				}
				if (string.IsNullOrEmpty(cAdmScreenFilter59List.SelectedValue))	// Add
				{
					if (ds != null)
					{
						pid = (new AdminSystem()).AddData(59,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc,noTrans,commandTimeOut);
					}
					if (!string.IsNullOrEmpty(pid))
					{
						cAdmScreenFilter59List.ClearSearch(); Session.Remove(KEY_dtAdmScreenFilter59List);
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmScreenFilter59List(sender, e, false, pid);
						rtn = GetScreenHlp().Rows[0]["AddMsg"].ToString();
					}
				}
				else {
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(59,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc,noTrans,commandTimeOut)) {bValid7 = true;}
					if (bValid7)
					{
						cAdmScreenFilter59List.ClearSearch(); Session.Remove(KEY_dtAdmScreenFilter59List);
						Session[KEY_currPageIndex] = GetCurrPageIndex();
						ShowDirty(false); PopAdmScreenFilter59List(sender, e, false, ds.Tables["AdmScreenFilter"].Rows[0]["ScreenFilterId86"]);
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cScreenFilterId86.Text != string.Empty)
			{
				AdmScreenFilter59 ds = PrepAdmScreenFilterData(null,false);
				try
				{
					if (ds != null && (new AdminSystem()).DelData(59,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc))
					{
						cAdmScreenFilter59List.ClearSearch(); Session.Remove(KEY_dtAdmScreenFilter59List);
						ShowDirty(false); PopAdmScreenFilter59List(sender, e, false, null);
		...
		private AdmScreenFilter59 PrepAdmScreenFilterData(DataView dv, bool bAdd)
		{
			...
			if (bAdd) { dr["ScreenFilterId86"] = string.Empty; } else { dr["ScreenFilterId86"] = cScreenFilterId86.Text; }
			drType["ScreenFilterId86"] = "Numeric"; drDisp["ScreenFilterId86"] = "TextBox";
			try {dr["ScreenId86"] = cScreenId86.SelectedValue;} catch {}
			drType["ScreenId86"] = "Numeric"; drDisp["ScreenId86"] = "AutoComplete";
			try {dr["ScreenFilterName86"] = cScreenFilterName86.Text.Trim();} catch {}
			drType["ScreenFilterName86"] = "VarChar"; drDisp["ScreenFilterName86"] = "TextBox";
			try {dr["FilterClause86"] = cFilterClause86.Text;} catch {}
			drType["FilterClause86"] = "VarChar"; drDisp["FilterClause86"] = "MultiLine";
			try {dr["FilterOrder86"] = cFilterOrder86.Text.Trim();} catch {}
			drType["FilterOrder86"] = "Numeric"; drDisp["FilterOrder86"] = "TextBox";
			try {dr["ApplyToMst86"] = base.SetBool(cApplyToMst86.Checked);} catch {}
			...
			if (dv != null)
			{
				...
				if (bAdd)
				{
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmScreenFilterAdd"].Rows.Add(MakeColRow(ds.Tables["AdmScreenFilterAdd"].NewRow(), drv, true));
					}
				}
				else
				{
					...
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmScreenFilterUpd"].Rows.Add(MakeColRow(ds.Tables["AdmScreenFilterUpd"].NewRow(), drv, false));
					}
					...
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmScreenFilterDel"].Rows.Add(MakeColRow(ds.Tables["AdmScreenFilterDel"].NewRow(), drv, false));
					}
		...
		public bool CanAct(char typ) { return CanAct(typ, GetAuthRow(), cAdmScreenFilter59List.SelectedValue.ToString()); }
		...
		{
			dr["ScreenFilterId86"] = cScreenFilterId86.Text;
			DataTable dtAuth = GetAuthCol();
			if (dtAuth != null)
			{
				dr["ScreenFilterHlpId87"] = drv["ScreenFilterHlpId87"].ToString().Trim();
				dr["CultureId87"] = drv["CultureId87"];
				dr["FilterName87"] = drv["FilterName87"].ToString().Trim();
			}
			return dr;
		}
		...
		{
			if (cAdmScreenFilterGrid.EditIndex > -1)
			{
				cAdmScreenFilterGrid_OnItemUpdating(sender, new ListViewUpdateEventArgs(cAdmScreenFilterGrid.EditIndex));
			}
			return true;
		}

		protected void cAdmScreenFilterGrid_OnPreRender(object sender, System.EventArgs e)
		{
			System.Web.UI.WebControls.Image hi = null;
			hi = (System.Web.UI.WebControls.Image)cAdmScreenFilterGrid.FindControl("cScreenFilterHlpId87hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmScreenFilterGrid.FindControl("cCultureId87hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmScreenFilterGrid.FindControl("cFilterName87hi"); if (hi != null) { hi.Visible = false; }
			if (Session[KEY_lastSortImg] != null)
			{
				hi = (System.Web.UI.WebControls.Image)cAdmScreenFilterGrid.FindControl((string)Session[KEY_lastSortImg]);
				if (hi != null) { hi.ImageUrl = Utils.AddTilde((string)Session[KEY_lastSortUrl]); hi.Visible = true; }
			}
			IgnoreHeaderConfirm((LinkButton)cAdmScreenFilterGrid.FindControl("cScreenFilterHlpId87hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmScreenFilterGrid.FindControl("cCultureId87hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmScreenFilterGrid.FindControl("cFilterName87hl"));
		}

		private string GetButtonId(ListViewItem lvi)
		{
			string ButtonID = String.Empty;
			Control c = lvi.FindControl("cAdmScreenFilterGridEdit");
		...
		{
		    return cAdmScreenFilterGridDataPager.StartRowIndex / cAdmScreenFilterGridDataPager.PageSize;
		}

		private int GetTotalPages()
		{
		    return (int)Math.Ceiling((double)cAdmScreenFilterGridDataPager.TotalRowCount / cAdmScreenFilterGridDataPager.PageSize);
		}

		private int GetDataItemIndex(int editIndex)
		{
		    return cAdmScreenFilterGridDataPager.StartRowIndex + editIndex;
		}

		protected void cAdmScreenFilterGrid_OnLayoutCreated(object sender, EventArgs e)
		{
		    // Header:
		    LinkButton lb = null;
		    lb = cAdmScreenFilterGrid.FindControl("cScreenFilterHlpId87hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(6); lb.ToolTip = ColumnToolTip(6); lb.Parent.Visible = GridColumnVisible(6); }
		    lb = cAdmScreenFilterGrid.FindControl("cCultureId87hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(7); lb.ToolTip = ColumnToolTip(7); lb.Parent.Visible = GridColumnVisible(7); }
		    lb = cAdmScreenFilterGrid.FindControl("cFilterName87hl") as LinkButton;
			...
			if (dtAuthRow != null)
			{
				...
				{
		            lb = cAdmScreenFilterGrid.FindControl("cDeleteAllButton") as LinkButton; if (lb != null) { lb.Visible = false; }
				}
			}
		    // footer:
		    Label gc = null;
		    gc = cAdmScreenFilterGrid.FindControl("cScreenFilterHlpId87fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(6); }
		    gc = cAdmScreenFilterGrid.FindControl("cCultureId87fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(7); }
		    gc = cAdmScreenFilterGrid.FindControl("cFilterName87fl") as Label;
		...
		private void PreMsgPopup(string msg) { PreMsgPopup(msg,cAdmScreenFilter59List,null); }

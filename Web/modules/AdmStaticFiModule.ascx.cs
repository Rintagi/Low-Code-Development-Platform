		...
		private void getReport(string eExport)
		{
			try
			{
				...
				if (eExport == "TXT")
				{
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmStaticFi.csv"; Session["ExportStr"] = (Config.ExportExcelCSV ? "sep=\t\n": "") + sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
		...
		private void PopAdmStaticFi117List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			...
			try
			{
				try {dt = (new AdminSystem()).GetLis(117,"GetLisAdmStaticFi117",false,"Y",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				catch {dt = (new AdminSystem()).GetLis(117,"GetLisAdmStaticFi117",false,"N",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
		...
		private bool UpdateGridRow(object sender, CommandEventArgs e)
		{
			...
			{
				cAdmStaticFiGrid_OnItemUpdating(sender, new ListViewUpdateEventArgs(cAdmStaticFiGrid.EditIndex));
			}
			return true;
		}

...
namespace RO.Web
{
	public partial class AdmStaticFiModule : RO.Web.ModuleBase
	{

		private const string KEY_lastAddedRow = "Cache:lastAddedRow3_117";
		private const string KEY_lastSortOrd = "Cache:lastSortOrd3_117";
		private const string KEY_lastSortImg = "Cache:lastSortImg3_117";
		private const string KEY_lastSortCol = "Cache:lastSortCol3_117";
		private const string KEY_lastSortExp = "Cache:lastSortExp3_117";
		private const string KEY_lastSortUrl = "Cache:lastSortUrl3_117";
		private const string KEY_lastSortTog = "Cache:lastSortTog3_117";
		private const string KEY_lastImpPwdOvride = "Cache:lastImpPwdOvride3_117";
		private const string KEY_cntImpPwdOvride = "Cache:cntImpPwdOvride3_117";
		private const string KEY_currPageIndex = "Cache:currPageIndex3_117";
		private const string KEY_bHiImpVisible = "Cache:bHiImpVisible3_117";
		private const string KEY_bShImpVisible = "Cache:bShImpVisible3_117";
		private const string KEY_dtAdmStaticFiGrid = "Cache:dtAdmStaticFiGrid";
		private const string KEY_scrImport = "Cache:scrImport3_117";
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_117";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_117";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_117";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_117";
		private const string KEY_dtLabel = "Cache:dtLabel3_117";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_117";
		private const string KEY_dtScrCri = "Cache:dtScrCri3_117";
		private const string KEY_dsScrCriVal = "Cache:dsScrCriVal3_117";


		private const string KEY_dtSystems = "Cache:dtSystems3_117";
		private const string KEY_sysConnectionString = "Cache:sysConnectionString3_117";
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_117";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_117";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_117";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				DataTable dtMenuAccess = (new MenuSystem()).GetMenu(base.LUser.CultureId, 3, base.LImpr, LcSysConnString, LcAppPw,117, null, null);
				...
				Session.Remove(KEY_dtAdmStaticFiGrid);
				...
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 117, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmStaticFi117List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=117&typ=N&sys=3','','',''); return false;"; }
			...
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmStaticFiGrid];
				if (dt != null)
				{
		...
		private void InitializeComponent()
		{
			...
			if (LcSysConnString == null)
			{
				...
				{
					DataTable dt;
					dt = (new AdminSystem()).GetAuthRow(117,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
					...
					{
						cAdmStaticFiGrid.PreRender += new EventHandler(cAdmStaticFiGrid_OnPreRender);
					}
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
			cGridUploadBtn.Click += cStaticFiUrl262Upl_Click;
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(117,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(117,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void SetClientRule(ListViewDataItem ee, bool isEditItem)
		{
			...
			if (dvRul.Count > 0)
			{
				...
				{
					if (ee == null || drv["ScriptEvent"].ToString().Substring(0,2).ToLower() != "on" || (cAdmStaticFiGrid.EditIndex > -1 && GetDataItemIndex(cAdmStaticFiGrid.EditIndex) == ee.DataItemIndex))
					{
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(117,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(117,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,117,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(117,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(117,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(117,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				CheckAuthentication(false);
				DataView dv = null;
				try {dv = ((DataTable)Session[KEY_dtAdmStaticFiGrid]).DefaultView;}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(117,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					{
						if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["StaticFiId262"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["StaticFiUrl262"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						sb.Append(Environment.NewLine);
					}
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmStaticFi.xls"; Session["ExportStr"] = sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmStaticFi.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private string GetRtfGrid(DataView dv)
		{
			...
			try
			{
				...
				foreach (DataRowView drv in dv)
				{
					...
					sb.Append("\r\n");
					sb.Append(@"{");
					if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["StaticFiId262"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["StaticFiUrl262"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					sb.Append(@"}");
					sb.Append("\r\n");
					sb.Append(@"\pard \ql \li0\ri0\widctlpar\intbl\aspalpha\aspnum\adjustright\rin0\lin0 {");
					sb.Append(@"\insrsid2981395 \trowd \irow0\irowband0\lastrow \ts15\trgaph108\trleft-108\trbrdrt\brdrs\brdrw10 \trbrdrl\brdrs\brdrw10 \trbrdrb\brdrs\brdrw10 \trbrdrr\brdrs\brdrw10 \trbrdrh\brdrs\brdrw10 \trbrdrv\brdrs\brdrw10 ");
		...
		public void cStaticFiUrl262Upl_Click(object sender, System.EventArgs e)
		{
			if (cAdmStaticFiGrid.EditIndex > -1 && cAdmStaticFiGrid.Items.Count > cAdmStaticFiGrid.EditIndex && ((FileUpload)cAdmStaticFiGrid.EditItem.FindControl("cStaticFiUrl262Fi")).HasFile && ((FileUpload)cAdmStaticFiGrid.EditItem.FindControl("cStaticFiUrl262Fi")).PostedFile.FileName != string.Empty)
			{
				string pth = "~/dev//AdmStaticFi_" + LcSystemId.ToString() + "/";
				if (!Directory.Exists(Server.MapPath(pth))) { Directory.CreateDirectory(Server.MapPath(pth)); }
				string fname = pth + Regex.Replace((new FileInfo(((FileUpload)cAdmStaticFiGrid.EditItem.FindControl("cStaticFiUrl262Fi")).PostedFile.FileName)).Name, "[ #=+]", string.Empty);
				fname = fname.Replace(":","").Replace("..","");
				if (fname != string.Empty)
				{
					if (File.Exists(Server.MapPath(fname))) { (new FileInfo(Server.MapPath(fname))).Delete(); }
					((FileUpload)cAdmStaticFiGrid.EditItem.FindControl("cStaticFiUrl262Fi")).PostedFile.SaveAs(Server.MapPath(fname));
					((Panel)cAdmStaticFiGrid.EditItem.FindControl("cStaticFiUrl262Pan")).Visible = false; ((TextBox)cAdmStaticFiGrid.EditItem.FindControl("cStaticFiUrl262")).Visible = true;
					((TextBox)cAdmStaticFiGrid.EditItem.FindControl("cStaticFiUrl262")).Text = fname; ShowDirty(true); ((TextBox)cAdmStaticFiGrid.EditItem.FindControl("cStaticFiUrl262")).Focus();
				}
			}
		}

		public void cStaticFiUrl262Tgo_Click(object sender, System.Web.UI.ImageClickEventArgs e)
		{
			if (((TextBox)cAdmStaticFiGrid.EditItem.FindControl("cStaticFiUrl262")).Visible)
			{
				((Panel)cAdmStaticFiGrid.EditItem.FindControl("cStaticFiUrl262Pan")).Visible = true; ((TextBox)cAdmStaticFiGrid.EditItem.FindControl("cStaticFiUrl262")).Visible = false;
			}
			else
			{
				((Panel)cAdmStaticFiGrid.EditItem.FindControl("cStaticFiUrl262Pan")).Visible = false; ((TextBox)cAdmStaticFiGrid.EditItem.FindControl("cStaticFiUrl262")).Visible = true;
		...
		private void GetPageInfo()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastPageInfo(117, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 117, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			// *** Criteria Trigger (before) Web Rule starts here *** //
			DataTable dt = (DataTable)Session[KEY_dtAdmStaticFiGrid];
			if (cAdmStaticFiGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
			if (IsPostBack && cCriteria.Visible) ReBindCriteria(GetScrCriteria());
			UpdCriteria(true);
			PopAdmStaticFi117List(sender, e, false, null);
			}
		}
			...
			try
			{
				foreach (DataRowView drv in dvCri)
				{
					...
					if (drv["DisplayName"].ToString() == "ComboBox")
					{
						...
						(new AdminSystem()).MkGetScreenIn("117", drv["ScreenCriId"].ToString(), "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), LcAppDb, LcDesDb, drv["MultiDesignDb"].ToString(), LcSysConnString, LcAppPw);
						DataView dv = new DataView((new AdminSystem()).GetScreenIn("117", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), (new AdminSystem()).CountScrCri(drv["ScreenCriId"].ToString(), drv["MultiDesignDb"].ToString(), drv["MultiDesignDb"].ToString() == "N" ? LcSysConnString : (string)Session[KEY_sysConnectionString], LcAppPw), drv["RequiredValid"].ToString(), 0, string.Empty, drv["DisplayMode"].ToString() != "AutoListBox", val, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], LcAppPw));
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("117", LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmStaticFi117" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmStaticFi117" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			foreach (DataRowView drv in dvCri)
			{
			    if (drv["DisplayName"].ToString() == "ListBox")
			    {
					...
					if (cListBox != null)
					{
						...
						int TotalChoiceCnt = new DataView((new AdminSystem()).GetScreenIn("117", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), CriCnt, drv["RequiredValid"].ToString(), 0, string.Empty, true, string.Empty, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], LcAppPw)).Count;
						string selectedValues = string.Join(",", cListBox.Items.Cast<ListItem>().Where(x => x.Selected).Select(x => "'" + x.Value + "'").ToArray());
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("117", "AdmStaticFi117", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		private void PopAdmStaticFi117List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			DataTable dt = null;
			CheckAuthentication(false);
			...
			try
			{
				try {dt = (new AdminSystem()).GetLis(117,"GetLisAdmStaticFi117",false,"Y",0,(string)Session[KEY_sysConnectionString],LcAppPw,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				catch {dt = (new AdminSystem()).GetLis(117,"GetLisAdmStaticFi117",false,"N",0,(string)Session[KEY_sysConnectionString],LcAppPw,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				Session[KEY_dtAdmStaticFiGrid] = dt;
				cAdmStaticFiGridDataPager.PageSize = Int16.Parse(cPgSize.Text); GotoPage(0);
			    if (Session[KEY_lastSortCol] != null)
			    {
					cAdmStaticFiGrid_OnSorting(sender, new ListViewSortEventArgs((string)Session[KEY_lastSortExp], SortDirection.Ascending));
			    }
				else
			    {
					cAdmStaticFiGrid_DataBind(dt.DefaultView);
			    }
		...
		protected void cStaticFiUrl262_TextChanged(object sender, System.EventArgs e)
		{
			...
			if (cAdmStaticFiGrid.EditIndex > -1 && cAdmStaticFiGrid.Items.Count > cAdmStaticFiGrid.EditIndex)
			{
			}
		}
		...
		public void cFindButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dv != null)
			{
				if (cAdmStaticFiGrid.EditIndex < 0 || UpdateGridRow(cAdmStaticFiGrid, new CommandEventArgs("Save", "")))
				{
					...
					GotoPage(0); cAdmStaticFiGrid_DataBind(dv);
					if (GetCurrPageIndex() != (int)Session[KEY_currPageIndex] && (int)Session[KEY_currPageIndex] < GetTotalPages()) { GotoPage((int)Session[KEY_currPageIndex]); cAdmStaticFiGrid_DataBind(dv);}
		...
		public void cInsRowButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dt != null && (ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")))
			{
				...
				Session[KEY_lastAddedRow] = 0; Session[KEY_dtAdmStaticFiGrid] = dt; Session[KEY_currPageIndex] = 0; GotoPage(0);
				cAdmStaticFiGrid_DataBind(dt.DefaultView);
				cAdmStaticFiGrid_OnItemEditing(cAdmStaticFiGrid, new ListViewEditEventArgs(0));
		...
		public void cPgSizeButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cAdmStaticFiGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				...
				cAdmStaticFiGridDataPager.PageSize = int.Parse(cPgSize.Text); GotoPage(0);
				if (dt.Rows.Count <= 0 || (GetAuthRow().Rows[0]["AllowAdd"].ToString() == "Y" && dt.Rows.Count == 1)) {cAdmStaticFiGrid_DataBind(dt.DefaultView);} else {cFindButton_Click(sender, e);}
				try
				{
					(new AdminSystem()).UpdLastPageInfo(117, base.LUser.UsrId, cPgSize.Text, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		}
		...
		public void cShowImpButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cSchemaImage.Visible)
			{
				try
				{
					Session["ImportSchema"] = (new AdminSystem()).GetSchemaScrImp(117,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (cSchemaImage.Attributes["OnClick"] == null || cSchemaImage.Attributes["OnClick"].IndexOf("ImportSchema.aspx") < 0) {cSchemaImage.Attributes["OnClick"] += "SearchLink('ImportSchema.aspx?scm=S&key=117&csy=3','','',''); return false;";}
			}
			// *** System Button Click (after) Web Rule starts here *** //
		}
		...
		protected void ScrImport(bool bClear)
		{
			...
			DataTable dtAdmStaticFiGrid = (DataTable)Session[KEY_dtAdmStaticFiGrid];
			if (dti != null && dtAdmStaticFiGrid != null)
			{
				if (bClear)
				{
					dtAdmStaticFiGrid.Rows.Clear();
				}
				cFind.Text = string.Empty;
				int iExist = dtAdmStaticFiGrid.Rows.Count;
				iRow = dti.DefaultView.Count - iExist;
				if (!dti.Columns.Contains("_NewRow")) { dti.Columns.Add("_NewRow"); }
				Session[KEY_dtAdmStaticFiGrid] = dti;
				cAdmStaticFiGrid_DataBind(dti.DefaultView);
		...
		public void cImportButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cFNameO.Text != string.Empty && cWorkSheet.Items.Count > 0 && cWorkSheet.SelectedItem.Text != string.Empty && cStartRow.Text != string.Empty)
			{
				...
				try
				{
					if (cAdmStaticFiGrid.EditIndex < 0 || UpdateGridRow(cAdmStaticFiGrid, new CommandEventArgs("Save", "")))
					{
						Session[KEY_scrImport] = ImportFile(dtAdmStaticFiGrid.Copy(),cFNameO.Text,cWorkSheet.SelectedItem.Text,cStartRow.Text,Config.PathTmpImport + cFName.Text);
					}
		...
		private void GotoPage(int pageNo)
		{
			if (pageNo >= 0 && pageNo < GetTotalPages())
			{
				if (cAdmStaticFiGrid.EditIndex < 0 || UpdateGridRow(cAdmStaticFiGrid, new CommandEventArgs("Save", "")))
				{
					try { cAdmStaticFiGridDataPager.SetPageProperties(cAdmStaticFiGridDataPager.PageSize * pageNo, cAdmStaticFiGridDataPager.MaximumRows, false); cAdmStaticFiGrid_DataBind(null); }
					catch
					{
						try { cAdmStaticFiGridDataPager.SetPageProperties(0, cAdmStaticFiGridDataPager.MaximumRows, false); cAdmStaticFiGrid_DataBind(null); } catch {}
					}
				}
			}
		}

		protected void cAdmStaticFiGrid_OnItemCommand(object sender, ListViewCommandEventArgs e)
		{
		}

		private void cAdmStaticFiGrid_DataBind(DataView dvAdmStaticFiGrid)
		{
			if (dvAdmStaticFiGrid == null)
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmStaticFiGrid];
				dvAdmStaticFiGrid = dt != null ? dt.DefaultView : null;
			}
			if (dvAdmStaticFiGrid != null)
			{
				LcAuth = GetAuthCol().AsEnumerable().ToDictionary<DataRow,string>(dr=>dr["ColName"].ToString());
				cAdmStaticFiGrid.DataSource = dvAdmStaticFiGrid;
				cAdmStaticFiGrid.DataBind();
		...
		protected void cAdmStaticFiGrid_OnSorting(object sender, ListViewSortEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmStaticFiGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null)
			{
				if (cAdmStaticFiGrid.EditIndex < 0 || UpdateGridRow(cAdmStaticFiGrid, new CommandEventArgs("Save", "")))
				{
					...
					Session[KEY_dtAdmStaticFiGrid] = dt;
					cAdmStaticFiGrid_DataBind(dv);
				}
			}
		}

		protected void cAdmStaticFiGrid_OnPagePropertiesChanging(object sender, PagePropertiesChangingEventArgs e)
		{
		    cAdmStaticFiGridDataPager.SetPageProperties(e.StartRowIndex, e.MaximumRows, false);
		    DataTable dt = (DataTable)Session[KEY_dtAdmStaticFiGrid];
		    DataView dv = dt != null ? dt.DefaultView : null;
			cAdmStaticFiGrid_DataBind(dv); cAdmStaticFiGrid.EditIndex = -1;
		}

		private void GridChkPgDirty(ListViewItem lvi)
		{
				WebControl cc = null;
				cc = ((WebControl)lvi.FindControl("cStaticFiUrl262"));
				...
				cc = ((WebControl)lvi.FindControl("cStaticFiUrl262Tgo"));
		...
		protected void cAdmStaticFiGrid_OnItemDataBound(object sender, ListViewItemEventArgs e)
		{
			...
			DataView dvAdmStaticFiGrid = dt != null ? dt.DefaultView : null;
			if (cAdmStaticFiGrid.EditIndex > -1 && GetDataItemIndex(cAdmStaticFiGrid.EditIndex) == e.Item.DataItemIndex)
			{
				isEditItem = true;
			...
			if (dtAuthRow != null)
			{
				...
				{
					cc = e.Item.FindControl("cAdmStaticFiGridDelete"); if (cc != null) { cc.Visible = false; }
				}
				if (!AllowRowEdit(GetAuthRow(),dvAdmStaticFiGrid[e.Item.DataItemIndex]["StaticFiId262"].ToString()) || (Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N"))
				{
					cc = e.Item.FindControl("cAdmStaticFiGridEdit"); if (cc != null) { cc.Visible = false; }
				}
				else
				{
			        HtmlTableRow tr = e.Item.FindControl("cAdmStaticFiGridRow") as HtmlTableRow;
			        LinkButton lb = e.Item.FindControl("cAdmStaticFiGridEdit") as LinkButton;
		...
		protected void cStaticFiId262hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "0") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "0"; Session[KEY_lastSortExp] = "StaticFiId262";Session[KEY_lastSortImg] = "cStaticFiId262hi";
			cAdmStaticFiGrid_OnSorting(sender, new ListViewSortEventArgs("StaticFiId262", SortDirection.Ascending));
		}

		protected void cStaticFiUrl262hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "1") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "1"; Session[KEY_lastSortExp] = "StaticFiUrl262";Session[KEY_lastSortImg] = "cStaticFiUrl262hi";
			cAdmStaticFiGrid_OnSorting(sender, new ListViewSortEventArgs("StaticFiUrl262", SortDirection.Ascending));
		}

		protected void cAdmStaticFiGrid_OnItemEditing(object sender, ListViewEditEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmStaticFiGrid];
			if ((ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")) && dt != null && dt.DefaultView.Count > GetDataItemIndex(e.NewEditIndex))
			{
				cAdmStaticFiGrid.EditIndex = e.NewEditIndex;
				cAdmStaticFiGrid_DataBind(null);
				Control wc = null;
				string idx = Page.Request["__EVENTARGUMENT"];
				if (!string.IsNullOrEmpty(idx))
				{
				    Control ctrlToFocus = cAdmStaticFiGrid.EditItem.FindControl("c" + idx);
				    if (((WebControl)ctrlToFocus).Enabled) { wc = ctrlToFocus; }
				}
				if (wc == null) { wc = FindEditableControl(cAdmStaticFiGrid.EditItem); }
				if (wc != null)
             {
             ...
             MkMobileLabel(cAdmStaticFiGrid.EditItem);
		    }
		    else { e.Cancel = true; }
		}
		...
		{
		    Label ml = null;
		    ml = lvi.FindControl("cStaticFiId262ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(0); }
		    ml = lvi.FindControl("cStaticFiUrl262ml") as Label;
		...
		{
			TextBox tb = null;
			tb = (TextBox)lvi.FindControl("cStaticFiUrl262");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["StaticFiUrl262"] = tb.Text;} else {dr["StaticFiUrl262"] = System.DBNull.Value;}
			}
		    if (bInsert) { dt.Rows.InsertAt(dr, 0); }
			Session[KEY_dtAdmStaticFiGrid] = dt;
			cAdmStaticFiGrid.EditIndex = -1;
			cAdmStaticFiGrid_DataBind(dt.DefaultView);
		}

		protected void cAdmStaticFiGrid_OnItemUpdating(object sender, ListViewUpdateEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmStaticFiGrid];
			if (dt != null && dt.Rows.Count > 0)
			{
				DataRow dr = dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row;
			    GridFill(cAdmStaticFiGrid.EditItem, dt, dr, false);
			    dr["_NewRow"] = "N";
			    if (dt.Columns.Contains("_SortOrder")) dr["_SortOrder"] = null;
			}
		}

		protected void cAdmStaticFiGrid_OnItemCanceling(object sender, ListViewCancelEventArgs e)
		{
			PanelTop.Update();
			DataTable dt = (DataTable)Session[KEY_dtAdmStaticFiGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			...
			{
				cAdmStaticFiGrid_OnItemDeleting(sender, new ListViewDeleteEventArgs(e.ItemIndex));
			}
			if (dt.Columns.Contains("_SortOrder")) dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row["_SortOrder"] = null;
			cAdmStaticFiGrid.EditIndex = -1; cAdmStaticFiGrid_DataBind(null);
		}
		...
		protected void cDeleteAllButton_Click(object sender, System.EventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmStaticFiGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null && (cAdmStaticFiGrid.EditIndex < 0 || UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				GotoPage(0);
				while (dv.Count > 0) {dv.Delete(0);}
				Session[KEY_dtAdmStaticFiGrid] = dt;
				cAdmStaticFiGrid.EditIndex = -1; cAdmStaticFiGrid_DataBind(dv);
			}
		}

		protected void cAdmStaticFiGrid_OnItemDeleting(object sender, ListViewDeleteEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmStaticFiGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			DataRow dr = dv != null ? dv[GetDataItemIndex(e.ItemIndex)].Row : null;
			if (dv != null && (cAdmStaticFiGrid.EditIndex == e.ItemIndex || UpdateGridRow(sender, new CommandEventArgs("Save",""))))
			{
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmStaticFiGrid); PopAdmStaticFi117List(sender, e, false, null);
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			if (ValidPage() && UpdateGridRow(cAdmStaticFiGrid, new CommandEventArgs("Save", "")))
			{
				...
				if (ftr != string.Empty) {dv.RowFilter = ftr;}
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(117,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],LcAppPw,base.CPrj,base.CSrc)) {bValid7 = true;}
					if (bValid7)
					{
						Session[KEY_currPageIndex] = GetCurrPageIndex();
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmStaticFi117List(sender, e, false, null);
		...
		private AdmStaticFi117 PrepAdmStaticFiData(DataView dv, bool bAdd)
		{
			...
			if (dv != null)
			{
				...
				ds.Tables["AdmStaticFiDef"].Rows.Add(MakeDisRow(ds.Tables["AdmStaticFiDef"].NewRow()));
					...
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmStaticFiUpd"].Rows.Add(MakeColRow(ds.Tables["AdmStaticFiUpd"].NewRow(), drv, false));
					}
					dv.RowStateFilter = DataViewRowState.Added;
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmStaticFiAdd"].Rows.Add(MakeColRow(ds.Tables["AdmStaticFiAdd"].NewRow(), drv, true));
					}
					dv.RowStateFilter = DataViewRowState.Deleted;
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmStaticFiDel"].Rows.Add(MakeColRow(ds.Tables["AdmStaticFiDel"].NewRow(), drv, false));
					}
		...
		private DataRow MakeColRow(DataRow dr, DataRowView drv, bool bAdd)
		{
			...
			if (dtAuth != null)
			{
				dr["StaticFiId262"] = drv["StaticFiId262"].ToString().Trim();
		...
		{
			if (cAdmStaticFiGrid.EditIndex > -1)
			{
				TextBox pwd = null;				cAdmStaticFiGrid_OnItemUpdating(sender, new ListViewUpdateEventArgs(cAdmStaticFiGrid.EditIndex));
			}
			return true;
		}

		protected void cAdmStaticFiGrid_OnPreRender(object sender, System.EventArgs e)
		{
			System.Web.UI.WebControls.Image hi = null;
			hi = (System.Web.UI.WebControls.Image)cAdmStaticFiGrid.FindControl("cStaticFiId262hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmStaticFiGrid.FindControl("cStaticFiUrl262hi"); if (hi != null) { hi.Visible = false; }
			if (Session[KEY_lastSortImg] != null)
			{
				hi = (System.Web.UI.WebControls.Image)cAdmStaticFiGrid.FindControl((string)Session[KEY_lastSortImg]);
				if (hi != null) { hi.ImageUrl = Utils.AddTilde((string)Session[KEY_lastSortUrl]); hi.Visible = true; }
			}
			IgnoreHeaderConfirm((LinkButton)cAdmStaticFiGrid.FindControl("cStaticFiId262hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmStaticFiGrid.FindControl("cStaticFiUrl262hl"));
		}

		private string GetButtonId(ListViewItem lvi)
		{
			string ButtonID = String.Empty;
			Control c = lvi.FindControl("cAdmStaticFiGridEdit");
		...
		{
		    return cAdmStaticFiGridDataPager.StartRowIndex / cAdmStaticFiGridDataPager.PageSize;
		}

		private int GetTotalPages()
		{
		    return (int)Math.Ceiling((double)cAdmStaticFiGridDataPager.TotalRowCount / cAdmStaticFiGridDataPager.PageSize);
		}

		private int GetDataItemIndex(int editIndex)
		{
		    return cAdmStaticFiGridDataPager.StartRowIndex + editIndex;
		}

		protected void cAdmStaticFiGrid_OnLayoutCreated(object sender, EventArgs e)
		{
		    // Header:
		    LinkButton lb = null;
		    lb = cAdmStaticFiGrid.FindControl("cStaticFiId262hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(0); lb.ToolTip = ColumnToolTip(0); lb.Parent.Visible = GridColumnVisible(0); }
		    lb = cAdmStaticFiGrid.FindControl("cStaticFiUrl262hl") as LinkButton;
			...
			if (dtAuthRow != null)
			{
				...
				if (dr["AllowDel"].ToString() == "N" || (Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N"))
				{
		            lb = cAdmStaticFiGrid.FindControl("cDeleteAllButton") as LinkButton; if (lb != null) { lb.Visible = false; }
				}
			}
		    // footer:
		    Label gc = null;
		    gc = cAdmStaticFiGrid.FindControl("cStaticFiId262fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(0); }
		    gc = cAdmStaticFiGrid.FindControl("cStaticFiUrl262fl") as Label;

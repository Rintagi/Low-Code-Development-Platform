...
{
	public class AdmWizardRule50 : DataSet
	{
		public AdmWizardRule50()
		{
			this.Tables.Add(MakeColumns(new DataTable("AdmWizardRule")));
			this.DataSetName = "AdmWizardRule50";
			this.Namespace = "http://Rintagi.com/DataSet/AdmWizardRule50";
		}

		private DataTable MakeColumns(DataTable dt)
		{
			DataColumnCollection columns = dt.Columns;
			columns.Add("WizardRuleId73", typeof(string));
			columns.Add("RuleName73", typeof(string));
			columns.Add("RuleDescription73", typeof(string));
			columns.Add("RuleTypeId73", typeof(string));
			columns.Add("WizardId73", typeof(string));
			columns.Add("RuleOrder73", typeof(string));
			columns.Add("ProcedureName73", typeof(string));
			columns.Add("BeforeCRUD73", typeof(string));
			return dt;
		}
	}
}
...
namespace RO.Web
{
	public partial class AdmWizardRuleModule : RO.Web.ModuleBase
	{
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_50";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_50";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_50";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_50";
		private const string KEY_dtLabel = "Cache:dtLabel3_50";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_50";
		private const string KEY_dtScrCri = "Cache:dtScrCri3_50";
		private const string KEY_dsScrCriVal = "Cache:dsScrCriVal3_50";

		private const string KEY_dtRuleTypeId73 = "Cache:dtRuleTypeId73";
		private const string KEY_dtWizardId73 = "Cache:dtWizardId73";

		private const string KEY_dtSystems = "Cache:dtSystems3_50";
		private const string KEY_sysConnectionString = "Cache:sysConnectionString3_50";
		private const string KEY_dtAdmWizardRule50List = "Cache:dtAdmWizardRule50List";
		private const string KEY_bNewVisible = "Cache:bNewVisible3_50";
		private const string KEY_bNewSaveVisible = "Cache:bNewSaveVisible3_50";
		private const string KEY_bCopyVisible = "Cache:bCopyVisible3_50";
		private const string KEY_bCopySaveVisible = "Cache:bCopySaveVisible3_50";
		private const string KEY_bDeleteVisible = "Cache:bDeleteVisible3_50";
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_50";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_50";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_50";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				DataTable dtMenuAccess = (new MenuSystem()).GetMenu(base.LUser.CultureId, 3, base.LImpr, LcSysConnString, LcAppPw,50, null, null);
				...
				Session.Remove(KEY_dtAdmWizardRule50List);
				...
				Session.Remove(KEY_dtRuleTypeId73);
				Session.Remove(KEY_dtWizardId73);
				SetButtonHlp();
				GetSystems();
				SetColumnAuthority();
				GetGlobalFilter();
				...
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 50, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmWizardRule50List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=50&typ=N&sys=3','','',''); return false;"; }
		...
		private void SetSystem(byte SystemId)
		{
			...
			try
			{
				...
				if ((Config.DeployType == "DEV" || row["dbAppDatabase"].ToString() == base.CPrj.EntityCode + "View") && !(base.CPrj.EntityCode != "RO" && row["SysProgram"].ToString() == "Y") && (new AdminSystem()).IsRegenNeeded(string.Empty,50,0,0,LcSysConnString,LcAppPw))
				{
					(new GenScreensSystem()).CreateProgram(50, "Wizard Rule", row["dbAppDatabase"].ToString(), base.CPrj, base.CSrc, base.CTar, LcAppConnString, LcAppPw);
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(50,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(50,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(50,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(50,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,50,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(50,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(50,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(50,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		protected DataTable GetAuthRow()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthRow(50,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				try
				{
					try {dv = new DataView((new AdminSystem()).GetExp(50,"GetExpAdmWizardRule50","Y",(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
					catch {dv = new DataView((new AdminSystem()).GetExp(50,"GetExpAdmWizardRule50","N",(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(50,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					{
						if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["WizardRuleId73"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["RuleName73"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["RuleDescription73"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(drv["RuleTypeId73"].ToString() + (char)9 + drv["RuleTypeId73Text"].ToString() + (char)9);}
						if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append(drv["WizardId73"].ToString() + (char)9 + drv["WizardId73Text"].ToString() + (char)9);}
						if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["RuleOrder73"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["ProcedureName73"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[7]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["BeforeCRUD73"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						sb.Append(Environment.NewLine);
					}
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmWizardRule.csv"; Session["ExportStr"] = (Config.ExportExcelCSV ? "sep=\t\n": "") + sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmWizardRule.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private string GetRtfGrid(DataView dv)
		{
			...
			try
			{
				...
				foreach (DataRowView drv in dv)
				{
					...
					sb.Append("\r\n");
					sb.Append(@"{");
					if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["WizardRuleId73"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["RuleName73"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(drv["RuleDescription73"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(drv["RuleTypeId73Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append(drv["WizardId73Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["RuleOrder73"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append(drv["ProcedureName73"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[7]["ColExport"].ToString() == "Y") {sb.Append(drv["BeforeCRUD73"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					sb.Append(@"}");
					sb.Append("\r\n");
					sb.Append(@"\pard \ql \li0\ri0\widctlpar\intbl\aspalpha\aspnum\adjustright\rin0\lin0 {");
					sb.Append(@"\insrsid2981395 \trowd \irow0\irowband0\lastrow \ts15\trgaph108\trleft-108\trbrdrt\brdrs\brdrw10 \trbrdrl\brdrs\brdrw10 \trbrdrb\brdrs\brdrw10 \trbrdrr\brdrs\brdrw10 \trbrdrh\brdrs\brdrw10 \trbrdrv\brdrs\brdrw10 ");
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 50, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			...
			UpdCriteria(true);
			cAdmWizardRule50List.ClearSearch(); Session.Remove(KEY_dtAdmWizardRule50List);
			PopAdmWizardRule50List(sender, e, false, null);
		}
			...
			try
			{
				foreach (DataRowView drv in dvCri)
				{
					...
					if (drv["DisplayName"].ToString() == "ComboBox")
					{
						...
						(new AdminSystem()).MkGetScreenIn("50", drv["ScreenCriId"].ToString(), "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), LcAppDb, LcDesDb, drv["MultiDesignDb"].ToString(), LcSysConnString, LcAppPw);
						DataView dv = new DataView((new AdminSystem()).GetScreenIn("50", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), (new AdminSystem()).CountScrCri(drv["ScreenCriId"].ToString(), drv["MultiDesignDb"].ToString(), drv["MultiDesignDb"].ToString() == "N" ? LcSysConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)), drv["RequiredValid"].ToString(), 0, string.Empty, drv["DisplayMode"].ToString() != "AutoListBox", val, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)));
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("50", LcSysConnString, LcAppPw);
				}
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmWizardRule50" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmWizardRule50" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			foreach (DataRowView drv in dvCri)
			{
			    if (drv["DisplayName"].ToString() == "ListBox")
			    {
					...
					if (cListBox != null)
					{
						...
						int TotalChoiceCnt = new DataView((new AdminSystem()).GetScreenIn("50", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), CriCnt, drv["RequiredValid"].ToString(), 0, string.Empty, true, string.Empty, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId))).Count;
						string selectedValues = string.Join(",", cListBox.Items.Cast<ListItem>().Where(x => x.Selected).Select(x => "'" + x.Value + "'").ToArray());
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("50", "AdmWizardRule50", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		private void SetRuleTypeId73(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtRuleTypeId73];
			...
			if (ddl != null)
			{
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(50,"GetDdlRuleTypeId3S654",true,bAll,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					if (dv.Table.Columns.Contains("WizardRuleId"))
					{
						ss = "(WizardRuleId is null";
						if (string.IsNullOrEmpty(cAdmWizardRule50List.SelectedValue)) {ss = ss + ")";} else {ss = ss + " OR WizardRuleId = " + cAdmWizardRule50List.SelectedValue + ")";}
					}
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "RuleTypeId73 is not NULL"; }
					dv.RowFilter = ss;
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(50,"GetDdlRuleTypeId3S654",true,true,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtRuleTypeId73] = dv.Table;
				}
			}
		}

		private void SetWizardId73(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtWizardId73];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (ddl != null)
			{
				string ss = string.Empty;
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(50,"GetDdlWizardId3S645",true,bAll,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					...
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "WizardId73 is not NULL"; }
					dv.RowFilter = ss;
					ddl.DataSource = dv; ddl.DataBind();
					li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					else if (!bFirst)
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(50,"GetDdlWizardId3S645",true,true,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtWizardId73] = dv.Table;
				}
			}
		}

		private DataView GetAdmWizardRule50List()
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmWizardRule50List];
			if (dt == null)
			{
				CheckAuthentication(false);
				int filterId = 0;
				...
				try
				{
					try {dt = (new AdminSystem()).GetLis(50,"GetLisAdmWizardRule50",true,"Y",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
					catch {dt = (new AdminSystem()).GetLis(50,"GetLisAdmWizardRule50",true,"N",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				}
		...
		private void PopAdmWizardRule50List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			DataView dv = GetAdmWizardRule50List();
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			context["method"] = "GetLisAdmWizardRule50";
			context["mKey"] = "WizardRuleId73";
			context["mVal"] = "WizardRuleId73Text";
			context["mTip"] = "WizardRuleId73Text";
			context["mImg"] = "WizardRuleId73Text";
			context["ssd"] = Request.QueryString["ssd"];
			context["scr"] = "50";
			context["csy"] = "3";
			...
			cAdmWizardRule50List.AutoCompleteUrl = "AutoComplete.aspx/LisSuggests";
			cAdmWizardRule50List.DataContext = context;
			if (dv.Table == null) return;
			cAdmWizardRule50List.DataSource = dv;
			cAdmWizardRule50List.Visible = true;
			if (cAdmWizardRule50List.Items.Count <= 0) {cAdmWizardRule50List.Visible = false; cAdmWizardRule50List_SelectedIndexChanged(sender,e);}
			else
			{
				if (bPageLoad && !string.IsNullOrEmpty(Request.QueryString["key"]) && bUseCri.Value == string.Empty)
				{
					try {cAdmWizardRule50List.SelectByValue(Request.QueryString["key"],string.Empty,true);} catch {cAdmWizardRule50List.Items[0].Selected = true; cAdmWizardRule50List_SelectedIndexChanged(sender, e);}
				    cScreenSearch.Visible = false; cSystem.Visible = false; cEditButton.Visible = true; cSaveCloseButton.Visible = cSaveButton.Visible;
				}
				else
				{
					if (ID != null) {if (!cAdmWizardRule50List.SelectByValue(ID,string.Empty,true)) {ID = null;}}
					if (ID == null)
					{
						cAdmWizardRule50List_SelectedIndexChanged(sender, e);
					}
				}
			}
		}
		...
		private void SetColumnAuthority()
		{
			...
			{
				base.SetFoldBehavior(cWizardRuleId73, dtAuth.Rows[0], cWizardRuleId73P1, cWizardRuleId73Label, cWizardRuleId73P2, null, dtLabel.Rows[0], null, null, null);
				base.SetFoldBehavior(cRuleName73, dtAuth.Rows[1], cRuleName73P1, cRuleName73Label, cRuleName73P2, null, dtLabel.Rows[1], cRFVRuleName73, null, null);
				base.SetFoldBehavior(cRuleDescription73, dtAuth.Rows[2], cRuleDescription73P1, cRuleDescription73Label, cRuleDescription73P2, cRuleDescription73E, null, dtLabel.Rows[2], null, null, null);
				cRuleDescription73E.Attributes["label_id"] = cRuleDescription73Label.ClientID; cRuleDescription73E.Attributes["target_id"] = cRuleDescription73.ClientID;
				base.SetFoldBehavior(cRuleTypeId73, dtAuth.Rows[3], cRuleTypeId73P1, cRuleTypeId73Label, cRuleTypeId73P2, null, dtLabel.Rows[3], cRFVRuleTypeId73, null, null);
				base.SetFoldBehavior(cWizardId73, dtAuth.Rows[4], cWizardId73P1, cWizardId73Label, cWizardId73P2, null, dtLabel.Rows[4], cRFVWizardId73, null, null);
				base.SetFoldBehavior(cRuleOrder73, dtAuth.Rows[5], cRuleOrder73P1, cRuleOrder73Label, cRuleOrder73P2, null, dtLabel.Rows[5], cRFVRuleOrder73, null, null);
				base.SetFoldBehavior(cProcedureName73, dtAuth.Rows[6], cProcedureName73P1, cProcedureName73Label, cProcedureName73P2, null, dtLabel.Rows[6], cRFVProcedureName73, null, null);
				base.SetFoldBehavior(cBeforeCRUD73, dtAuth.Rows[7], cBeforeCRUD73P1, cBeforeCRUD73Label, cBeforeCRUD73P2, null, dtLabel.Rows[7], null, null, null);
			}
			if ((cRuleName73.Attributes["OnChange"] == null || cRuleName73.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cRuleName73.Visible && !cRuleName73.ReadOnly) {cRuleName73.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cRuleDescription73.Attributes["OnChange"] == null || cRuleDescription73.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cRuleDescription73.Visible && !cRuleDescription73.ReadOnly) {cRuleDescription73.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cRuleTypeId73.Attributes["OnChange"] == null || cRuleTypeId73.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cRuleTypeId73.Visible && cRuleTypeId73.Enabled) {cRuleTypeId73.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cWizardId73.Attributes["OnChange"] == null || cWizardId73.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cWizardId73.Visible && cWizardId73.Enabled) {cWizardId73.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cRuleOrder73.Attributes["OnChange"] == null || cRuleOrder73.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cRuleOrder73.Visible && !cRuleOrder73.ReadOnly) {cRuleOrder73.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cProcedureName73.Attributes["OnChange"] == null || cProcedureName73.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cProcedureName73.Visible && !cProcedureName73.ReadOnly) {cProcedureName73.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cBeforeCRUD73.Attributes["OnClick"] == null || cBeforeCRUD73.Attributes["OnClick"].IndexOf("ChkPgDirty") < 0) && cBeforeCRUD73.Visible && cBeforeCRUD73.Enabled) {cBeforeCRUD73.Attributes["OnClick"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty(); this.focus();";}
		}
		...
		private void ClearMaster(object sender, System.EventArgs e)
		{
			...
			if (dt.Rows[1]["ColVisible"].ToString() == "Y" && dt.Rows[1]["ColReadOnly"].ToString() != "Y") {cRuleName73.Text = string.Empty;}
			if (dt.Rows[2]["ColVisible"].ToString() == "Y" && dt.Rows[2]["ColReadOnly"].ToString() != "Y") {cRuleDescription73.Text = string.Empty;}
			if (dt.Rows[3]["ColVisible"].ToString() == "Y" && dt.Rows[3]["ColReadOnly"].ToString() != "Y") {SetRuleTypeId73(cRuleTypeId73,string.Empty);}
			if (dt.Rows[4]["ColVisible"].ToString() == "Y" && dt.Rows[4]["ColReadOnly"].ToString() != "Y") {SetWizardId73(cWizardId73,string.Empty);}
			if (dt.Rows[5]["ColVisible"].ToString() == "Y" && dt.Rows[5]["ColReadOnly"].ToString() != "Y") {cRuleOrder73.Text = string.Empty;}
			if (dt.Rows[6]["ColVisible"].ToString() == "Y" && dt.Rows[6]["ColReadOnly"].ToString() != "Y") {cProcedureName73.Text = string.Empty;}
			if (dt.Rows[7]["ColVisible"].ToString() == "Y" && dt.Rows[7]["ColReadOnly"].ToString() != "Y") {cBeforeCRUD73.Checked = base.GetBool("N");}
			// *** Default Value (Folder) Web Rule starts here *** //
		}

		private void InitMaster(object sender, System.EventArgs e)
		{
			cWizardRuleId73.Text = string.Empty;
			cRuleName73.Text = string.Empty;
			cRuleDescription73.Text = string.Empty;
			SetRuleTypeId73(cRuleTypeId73,string.Empty);
			SetWizardId73(cWizardId73,string.Empty);
			cRuleOrder73.Text = string.Empty;
			cProcedureName73.Text = string.Empty;
			cBeforeCRUD73.Checked = base.GetBool("N");
			// *** Default Value (Folder) Web Rule starts here *** //
		}
		protected void cAdmWizardRule50List_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmWizardRule50List_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmWizardRule50List_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			InitMaster(sender, e);      // Do this first to enable defaults for non-database columns.
			DataTable dt = null;
			try
			{
				dt = (new AdminSystem()).GetMstById("GetAdmWizardRule50ById",cAdmWizardRule50List.SelectedValue,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId));
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				if (dt.Rows.Count > 0)
				{
					cAdmWizardRule50List.SetDdVisible();
					DataRow dr = dt.Rows[0];
					try {cWizardRuleId73.Text = RO.Common3.Utils.fmNumeric("0",dr["WizardRuleId73"].ToString(),base.LUser.Culture);} catch {cWizardRuleId73.Text = string.Empty;}
					try {cRuleName73.Text = dr["RuleName73"].ToString();} catch {cRuleName73.Text = string.Empty;}
					try {cRuleDescription73.Text = dr["RuleDescription73"].ToString();} catch {cRuleDescription73.Text = string.Empty;}
					SetRuleTypeId73(cRuleTypeId73,dr["RuleTypeId73"].ToString());
					SetWizardId73(cWizardId73,dr["WizardId73"].ToString());
					try {cRuleOrder73.Text = RO.Common3.Utils.fmNumeric("0",dr["RuleOrder73"].ToString(),base.LUser.Culture);} catch {cRuleOrder73.Text = string.Empty;}
					try {cProcedureName73.Text = dr["ProcedureName73"].ToString();} catch {cProcedureName73.Text = string.Empty;}
					try {cBeforeCRUD73.Checked = base.GetBool(dr["BeforeCRUD73"].ToString());} catch {cBeforeCRUD73.Checked = false;}
				}
			}
			cButPanel.DataBind();
			ScriptManager.GetCurrent(Parent.Page).SetFocus(cAdmWizardRule50List.FocusID);
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cWizardRuleId73.Text == string.Empty) { cClearButton_Click(sender, new EventArgs()); ShowDirty(false); PanelTop.Update();}
			else { PopAdmWizardRule50List(sender, e, false, cWizardRuleId73.Text); }
			// *** System Button Click (After) Web Rule starts here *** //
		}
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmWizardRule50List); PopAdmWizardRule50List(sender, e, false, null);
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			if (ValidPage())
			{
				AdmWizardRule50 ds = PrepAdmWizardRuleData(null,cWizardRuleId73.Text == string.Empty);
				if (!string.IsNullOrEmpty(cAdmWizardRule50List.SelectedValue) && cWizardRuleId73.Text != cAdmWizardRule50List.SelectedValue)
				{
					PreMsgPopup("Primary key cannot be changed");
					return rtn;
				}
				if (string.IsNullOrEmpty(cAdmWizardRule50List.SelectedValue))	// Add
				{
					if (ds != null)
					{
						pid = (new AdminSystem()).AddData(50,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc,noTrans,commandTimeOut);
					}
					if (!string.IsNullOrEmpty(pid))
					{
						cAdmWizardRule50List.ClearSearch(); Session.Remove(KEY_dtAdmWizardRule50List);
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmWizardRule50List(sender, e, false, pid);
						rtn = GetScreenHlp().Rows[0]["AddMsg"].ToString();
					}
				}
				else {
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(50,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc,noTrans,commandTimeOut)) {bValid7 = true;}
					if (bValid7)
					{
						cAdmWizardRule50List.ClearSearch(); Session.Remove(KEY_dtAdmWizardRule50List);
						ShowDirty(false); PopAdmWizardRule50List(sender, e, false, ds.Tables["AdmWizardRule"].Rows[0]["WizardRuleId73"]);
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cWizardRuleId73.Text != string.Empty)
			{
				AdmWizardRule50 ds = PrepAdmWizardRuleData(null,false);
				try
				{
					if (ds != null && (new AdminSystem()).DelData(50,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc))
					{
						cAdmWizardRule50List.ClearSearch(); Session.Remove(KEY_dtAdmWizardRule50List);
						ShowDirty(false); PopAdmWizardRule50List(sender, e, false, null);
		...
		private AdmWizardRule50 PrepAdmWizardRuleData(DataView dv, bool bAdd)
		{
			AdmWizardRule50 ds = new AdmWizardRule50();
			DataRow dr = ds.Tables["AdmWizardRule"].NewRow();
			DataRow drType = ds.Tables["AdmWizardRule"].NewRow();
			DataRow drDisp = ds.Tables["AdmWizardRule"].NewRow();
			if (bAdd) { dr["WizardRuleId73"] = string.Empty; } else { dr["WizardRuleId73"] = cWizardRuleId73.Text; }
			drType["WizardRuleId73"] = "Numeric"; drDisp["WizardRuleId73"] = "TextBox";
			try {dr["RuleName73"] = cRuleName73.Text.Trim();} catch {}
			drType["RuleName73"] = "VarWChar"; drDisp["RuleName73"] = "TextBox";
			try {dr["RuleDescription73"] = cRuleDescription73.Text;} catch {}
			drType["RuleDescription73"] = "VarWChar"; drDisp["RuleDescription73"] = "MultiLine";
			try {dr["RuleTypeId73"] = cRuleTypeId73.SelectedValue;} catch {}
			drType["RuleTypeId73"] = "Numeric"; drDisp["RuleTypeId73"] = "DropDownList";
			try {dr["WizardId73"] = cWizardId73.SelectedValue;} catch {}
			drType["WizardId73"] = "Numeric"; drDisp["WizardId73"] = "DropDownList";
			try {dr["RuleOrder73"] = cRuleOrder73.Text.Trim();} catch {}
			drType["RuleOrder73"] = "Numeric"; drDisp["RuleOrder73"] = "TextBox";
			try {dr["ProcedureName73"] = cProcedureName73.Text.Trim();} catch {}
			drType["ProcedureName73"] = "VarChar"; drDisp["ProcedureName73"] = "TextBox";
			try {dr["BeforeCRUD73"] = base.SetBool(cBeforeCRUD73.Checked);} catch {}
			drType["BeforeCRUD73"] = "Char"; drDisp["BeforeCRUD73"] = "CheckBox";
			if (bAdd)
			{
			}
			ds.Tables["AdmWizardRule"].Rows.Add(dr); ds.Tables["AdmWizardRule"].Rows.Add(drType); ds.Tables["AdmWizardRule"].Rows.Add(drDisp);
			return ds;
		}

		public bool CanAct(char typ) { return CanAct(typ, GetAuthRow(), cAdmWizardRule50List.SelectedValue.ToString()); }
		...
		private bool ValidPage()
		{
			...
			DataTable dt = null;
			dt = (DataTable)Session[KEY_dtRuleTypeId73];
			if (dt != null && dt.Rows.Count <= 0)
			{
				bErrNow.Value = "Y"; PreMsgPopup("Value is expected for 'RuleTypeId', please investigate."); return false;
			}
			dt = (DataTable)Session[KEY_dtWizardId73];
			if (dt != null && dt.Rows.Count <= 0)
			{
				bErrNow.Value = "Y"; PreMsgPopup("Value is expected for 'WizardId', please investigate."); return false;
			}
			return true;
		}
		...
		private void PreMsgPopup(string msg) { PreMsgPopup(msg,cAdmWizardRule50List,null); }

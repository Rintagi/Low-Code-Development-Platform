...
\par }}");
		...
		private void PopAdmRptwizTyp97List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			...
			try
			{
				try {dt = (new AdminSystem()).GetLis(97,"GetLisAdmRptwizTyp97",false,"Y",0,(string)Session[KEY_sysConnectionString],LcAppPw,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				catch {dt = (new AdminSystem()).GetLis(97,"GetLisAdmRptwizTyp97",false,"N",0,(string)Session[KEY_sysConnectionString],LcAppPw,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			if (ValidPage() && UpdateGridRow(cAdmRptwizTypGrid, new CommandEventArgs("Save", "")))
			{
				...
				if (ftr != string.Empty) {dv.RowFilter = ftr;}
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(97,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],LcAppPw,base.CPrj,base.CSrc)) {bValid7 = true;}
					if (bValid7)
					{
		...
		private bool UpdateGridRow(object sender, CommandEventArgs e)
		{
			...
			{
				TextBox pwd = null;				cAdmRptwizTypGrid_OnItemUpdating(sender, new ListViewUpdateEventArgs(cAdmRptwizTypGrid.EditIndex));
			}
			return true;
		}

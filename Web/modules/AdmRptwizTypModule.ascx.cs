...
{
	public class AdmRptwizTyp97 : DataSet
	{
		public AdmRptwizTyp97()
		{
			this.Tables.Add(MakeDtlColumns(new DataTable("AdmRptwizTypDef")));
			this.Tables.Add(MakeDtlColumns(new DataTable("AdmRptwizTypAdd")));
			this.Tables.Add(MakeDtlColumns(new DataTable("AdmRptwizTypUpd")));
			this.Tables.Add(MakeDtlColumns(new DataTable("AdmRptwizTypDel")));
			this.DataSetName = "AdmRptwizTyp97";
			this.Namespace = "http://Rintagi.com/DataSet/AdmRptwizTyp97";
		}

		private DataTable MakeDtlColumns(DataTable dt)
		{
			DataColumnCollection columns = dt.Columns;
			columns.Add("RptwizTypId185", typeof(string));
			columns.Add("RptwizTypName185", typeof(string));
			return dt;
		}
	}
}
...
namespace RO.Web
{
	public partial class AdmRptwizTypModule : RO.Web.ModuleBase
	{

		private const string KEY_lastAddedRow = "Cache:lastAddedRow3_97";
		private const string KEY_lastSortOrd = "Cache:lastSortOrd3_97";
		private const string KEY_lastSortImg = "Cache:lastSortImg3_97";
		private const string KEY_lastSortCol = "Cache:lastSortCol3_97";
		private const string KEY_lastSortExp = "Cache:lastSortExp3_97";
		private const string KEY_lastSortUrl = "Cache:lastSortUrl3_97";
		private const string KEY_lastSortTog = "Cache:lastSortTog3_97";
		private const string KEY_lastImpPwdOvride = "Cache:lastImpPwdOvride3_97";
		private const string KEY_cntImpPwdOvride = "Cache:cntImpPwdOvride3_97";
		private const string KEY_currPageIndex = "Cache:currPageIndex3_97";
		private const string KEY_bHiImpVisible = "Cache:bHiImpVisible3_97";
		private const string KEY_bShImpVisible = "Cache:bShImpVisible3_97";
		private const string KEY_dtAdmRptwizTypGrid = "Cache:dtAdmRptwizTypGrid";
		private const string KEY_scrImport = "Cache:scrImport3_97";
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_97";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_97";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_97";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_97";
		private const string KEY_dtLabel = "Cache:dtLabel3_97";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_97";
		private const string KEY_dtScrCri = "Cache:dtScrCri3_97";
		private const string KEY_dsScrCriVal = "Cache:dsScrCriVal3_97";


		private const string KEY_dtSystems = "Cache:dtSystems3_97";
		private const string KEY_sysConnectionString = "Cache:sysConnectionString3_97";
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_97";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_97";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_97";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				DataTable dtMenuAccess = (new MenuSystem()).GetMenu(base.LUser.CultureId, 3, base.LImpr, LcSysConnString, LcAppPw,97, null, null);
				...
				Session.Remove(KEY_dtAdmRptwizTypGrid);
				...
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 97, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmRptwizTyp97List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=97&typ=N&sys=3','','',''); return false;"; }
			...
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmRptwizTypGrid];
				if (dt != null)
				{
		...
		private void InitializeComponent()
		{
			...
			if (LcSysConnString == null)
			{
				...
				{
					DataTable dt;
					dt = (new AdminSystem()).GetAuthRow(97,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
					...
					{
						cAdmRptwizTypGrid.PreRender += new EventHandler(cAdmRptwizTypGrid_OnPreRender);
					}
				}
		...
		private void SetSystem(byte SystemId)
		{
			...
			try
			{
				...
				if ((Config.DeployType == "DEV" || row["dbAppDatabase"].ToString() == base.CPrj.EntityCode + "View") && !(base.CPrj.EntityCode != "RO" && row["SysProgram"].ToString() == "Y") && (new AdminSystem()).IsRegenNeeded(string.Empty,97,0,0,LcSysConnString,LcAppPw))
				{
					(new GenScreensSystem()).CreateProgram(97, "Report Data Type", row["dbAppDatabase"].ToString(), base.CPrj, base.CSrc, base.CTar, LcAppConnString, LcAppPw);
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(97,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(97,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void SetClientRule(ListViewDataItem ee, bool isEditItem)
		{
			...
			if (dvRul.Count > 0)
			{
				...
				{
					if (ee == null || drv["ScriptEvent"].ToString().Substring(0,2).ToLower() != "on" || (cAdmRptwizTypGrid.EditIndex > -1 && GetDataItemIndex(cAdmRptwizTypGrid.EditIndex) == ee.DataItemIndex))
					{
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(97,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(97,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,97,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(97,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(97,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(97,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				CheckAuthentication(false);
				DataView dv = null;
				try {dv = ((DataTable)Session[KEY_dtAdmRptwizTypGrid]).DefaultView;}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(97,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					{
						if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["RptwizTypId185"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["RptwizTypName185"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						sb.Append(Environment.NewLine);
					}
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmRptwizTyp.csv"; Session["ExportStr"] = (Config.ExportExcelCSV ? "sep=\t\n": "") + sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmRptwizTyp.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private string GetRtfGrid(DataView dv)
		{
			...
			try
			{
				...
				foreach (DataRowView drv in dv)
				{
					...
					sb.Append("\r\n");
					sb.Append(@"{");
					if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["RptwizTypId185"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["RptwizTypName185"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					sb.Append(@"}");
					sb.Append("\r\n");
					sb.Append(@"\pard \ql \li0\ri0\widctlpar\intbl\aspalpha\aspnum\adjustright\rin0\lin0 {");
					sb.Append(@"\insrsid2981395 \trowd \irow0\irowband0\lastrow \ts15\trgaph108\trleft-108\trbrdrt\brdrs\brdrw10 \trbrdrl\brdrs\brdrw10 \trbrdrb\brdrs\brdrw10 \trbrdrr\brdrs\brdrw10 \trbrdrh\brdrs\brdrw10 \trbrdrv\brdrs\brdrw10 ");
		...
		private void GetPageInfo()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastPageInfo(97, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 97, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			// *** Criteria Trigger (before) Web Rule starts here *** //
			DataTable dt = (DataTable)Session[KEY_dtAdmRptwizTypGrid];
			if (cAdmRptwizTypGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
			if (IsPostBack && cCriteria.Visible) ReBindCriteria(GetScrCriteria());
			UpdCriteria(true);
			PopAdmRptwizTyp97List(sender, e, false, null);
			}
		}
			...
			try
			{
				foreach (DataRowView drv in dvCri)
				{
					...
					if (drv["DisplayName"].ToString() == "ComboBox")
					{
						...
						(new AdminSystem()).MkGetScreenIn("97", drv["ScreenCriId"].ToString(), "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), LcAppDb, LcDesDb, drv["MultiDesignDb"].ToString(), LcSysConnString, LcAppPw);
						DataView dv = new DataView((new AdminSystem()).GetScreenIn("97", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), (new AdminSystem()).CountScrCri(drv["ScreenCriId"].ToString(), drv["MultiDesignDb"].ToString(), drv["MultiDesignDb"].ToString() == "N" ? LcSysConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)), drv["RequiredValid"].ToString(), 0, string.Empty, drv["DisplayMode"].ToString() != "AutoListBox", val, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)));
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("97", LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmRptwizTyp97" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmRptwizTyp97" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			foreach (DataRowView drv in dvCri)
			{
			    if (drv["DisplayName"].ToString() == "ListBox")
			    {
					...
					if (cListBox != null)
					{
						...
						int TotalChoiceCnt = new DataView((new AdminSystem()).GetScreenIn("97", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), CriCnt, drv["RequiredValid"].ToString(), 0, string.Empty, true, string.Empty, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId))).Count;
						string selectedValues = string.Join(",", cListBox.Items.Cast<ListItem>().Where(x => x.Selected).Select(x => "'" + x.Value + "'").ToArray());
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("97", "AdmRptwizTyp97", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		private void PopAdmRptwizTyp97List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			DataTable dt = null;
			CheckAuthentication(false);
			...
			try
			{
				try {dt = (new AdminSystem()).GetLis(97,"GetLisAdmRptwizTyp97",false,"Y",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				catch {dt = (new AdminSystem()).GetLis(97,"GetLisAdmRptwizTyp97",false,"N",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				Session[KEY_dtAdmRptwizTypGrid] = dt;
				cAdmRptwizTypGridDataPager.PageSize = Int16.Parse(cPgSize.Text); GotoPage(0);
			    if (Session[KEY_lastSortCol] != null)
			    {
					cAdmRptwizTypGrid_OnSorting(sender, new ListViewSortEventArgs((string)Session[KEY_lastSortExp], SortDirection.Ascending));
			    }
				else
			    {
					cAdmRptwizTypGrid_DataBind(dt.DefaultView);
			    }
		...
		public void cFindButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dv != null)
			{
				if (cAdmRptwizTypGrid.EditIndex < 0 || UpdateGridRow(cAdmRptwizTypGrid, new CommandEventArgs("Save", "")))
				{
					...
					GotoPage(0); cAdmRptwizTypGrid_DataBind(dv);
					if (GetCurrPageIndex() != (int)Session[KEY_currPageIndex] && (int)Session[KEY_currPageIndex] < GetTotalPages()) { GotoPage((int)Session[KEY_currPageIndex]); cAdmRptwizTypGrid_DataBind(dv);}
		...
		public void cInsRowButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dt != null && (ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")))
			{
				...
				Session[KEY_lastAddedRow] = 0; Session[KEY_dtAdmRptwizTypGrid] = dt; Session[KEY_currPageIndex] = 0; GotoPage(0);
				cAdmRptwizTypGrid_DataBind(dt.DefaultView);
				cAdmRptwizTypGrid_OnItemEditing(cAdmRptwizTypGrid, new ListViewEditEventArgs(0));
		...
		public void cPgSizeButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cAdmRptwizTypGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				...
				cAdmRptwizTypGridDataPager.PageSize = int.Parse(cPgSize.Text); GotoPage(0);
				if (dt.Rows.Count <= 0 || (GetAuthRow().Rows[0]["AllowAdd"].ToString() == "Y" && dt.Rows.Count == 1)) {cAdmRptwizTypGrid_DataBind(dt.DefaultView);} else {cFindButton_Click(sender, e);}
				try
				{
					(new AdminSystem()).UpdLastPageInfo(97, base.LUser.UsrId, cPgSize.Text, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		}
		...
		public void cShowImpButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cSchemaImage.Visible)
			{
				try
				{
					Session["ImportSchema"] = (new AdminSystem()).GetSchemaScrImp(97,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (cSchemaImage.Attributes["OnClick"] == null || cSchemaImage.Attributes["OnClick"].IndexOf("ImportSchema.aspx") < 0) {cSchemaImage.Attributes["OnClick"] += "SearchLink('ImportSchema.aspx?scm=S&key=97&csy=3','','',''); return false;";}
			}
			// *** System Button Click (after) Web Rule starts here *** //
		}
		...
		protected void ScrImport(bool bClear)
		{
			...
			DataTable dtAdmRptwizTypGrid = (DataTable)Session[KEY_dtAdmRptwizTypGrid];
			if (dti != null && dtAdmRptwizTypGrid != null)
			{
				if (bClear)
				{
					dtAdmRptwizTypGrid.Rows.Clear();
				}
				cFind.Text = string.Empty;
				int iExist = dtAdmRptwizTypGrid.Rows.Count;
				iRow = dti.DefaultView.Count - iExist;
				if (!dti.Columns.Contains("_NewRow")) { dti.Columns.Add("_NewRow"); }
				Session[KEY_dtAdmRptwizTypGrid] = dti;
				cAdmRptwizTypGrid_DataBind(dti.DefaultView);
		...
		public void cImportButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cFNameO.Text != string.Empty && cWorkSheet.Items.Count > 0 && cWorkSheet.SelectedItem.Text != string.Empty && cStartRow.Text != string.Empty)
			{
				...
				try
				{
					if (cAdmRptwizTypGrid.EditIndex < 0 || UpdateGridRow(cAdmRptwizTypGrid, new CommandEventArgs("Save", "")))
					{
						Session[KEY_scrImport] = ImportFile(dtAdmRptwizTypGrid.Copy(),cFNameO.Text,cWorkSheet.SelectedItem.Text,cStartRow.Text,Config.PathTmpImport + cFName.Text);
					}
		...
		private DataTable ImportFile(DataTable dt, string fileName, string workSheet, string startRow, string fileFullName)
		{
			try
			{
				...
				{
					if (rows[iRow][1].ToString() == string.Empty) {idel = idel + 1;}
					else
					{
		...
		private void GotoPage(int pageNo)
		{
			if (pageNo >= 0 && pageNo < GetTotalPages())
			{
				if (cAdmRptwizTypGrid.EditIndex < 0 || UpdateGridRow(cAdmRptwizTypGrid, new CommandEventArgs("Save", "")))
				{
					try { cAdmRptwizTypGridDataPager.SetPageProperties(cAdmRptwizTypGridDataPager.PageSize * pageNo, cAdmRptwizTypGridDataPager.MaximumRows, false); cAdmRptwizTypGrid_DataBind(null); }
					catch
					{
						try { cAdmRptwizTypGridDataPager.SetPageProperties(0, cAdmRptwizTypGridDataPager.MaximumRows, false); cAdmRptwizTypGrid_DataBind(null); } catch {}
					}
				}
			}
		}

		protected void cAdmRptwizTypGrid_OnItemCommand(object sender, ListViewCommandEventArgs e)
		{
		}

		private void cAdmRptwizTypGrid_DataBind(DataView dvAdmRptwizTypGrid)
		{
			if (dvAdmRptwizTypGrid == null)
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmRptwizTypGrid];
				dvAdmRptwizTypGrid = dt != null ? dt.DefaultView : null;
			}
			if (dvAdmRptwizTypGrid != null)
			{
				LcAuth = GetAuthCol().AsEnumerable().ToDictionary<DataRow,string>(dr=>dr["ColName"].ToString());
				cAdmRptwizTypGrid.DataSource = dvAdmRptwizTypGrid;
				cAdmRptwizTypGrid.DataBind();
		...
		protected void cAdmRptwizTypGrid_OnSorting(object sender, ListViewSortEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmRptwizTypGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null)
			{
				if (cAdmRptwizTypGrid.EditIndex < 0 || UpdateGridRow(cAdmRptwizTypGrid, new CommandEventArgs("Save", "")))
				{
					...
					Session[KEY_dtAdmRptwizTypGrid] = dt;
					cAdmRptwizTypGrid_DataBind(dv);
				}
			}
		}

		protected void cAdmRptwizTypGrid_OnPagePropertiesChanging(object sender, PagePropertiesChangingEventArgs e)
		{
		    cAdmRptwizTypGridDataPager.SetPageProperties(e.StartRowIndex, e.MaximumRows, false);
		    DataTable dt = (DataTable)Session[KEY_dtAdmRptwizTypGrid];
		    DataView dv = dt != null ? dt.DefaultView : null;
			cAdmRptwizTypGrid_DataBind(dv); cAdmRptwizTypGrid.EditIndex = -1;
		}

		private void GridChkPgDirty(ListViewItem lvi)
		{
				WebControl cc = null;
				cc = ((WebControl)lvi.FindControl("cRptwizTypName185"));
		...
		protected void cAdmRptwizTypGrid_OnItemDataBound(object sender, ListViewItemEventArgs e)
		{
			// *** GridItemDataBound (before) Web Rule End *** //
			DataTable dt = (DataTable)Session[KEY_dtAdmRptwizTypGrid];
			bool isEditItem = false;
			DataView dvAdmRptwizTypGrid = dt != null ? dt.DefaultView : null;
			if (cAdmRptwizTypGrid.EditIndex > -1 && GetDataItemIndex(cAdmRptwizTypGrid.EditIndex) == e.Item.DataItemIndex)
			{
				isEditItem = true;
			...
			if (dtAuthRow != null)
			{
				...
				{
					cc = e.Item.FindControl("cAdmRptwizTypGridDelete"); if (cc != null) { cc.Visible = false; }
				}
				if (!AllowRowEdit(GetAuthRow(),dvAdmRptwizTypGrid[e.Item.DataItemIndex]["RptwizTypId185"].ToString()) || (Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N"))
				{
					cc = e.Item.FindControl("cAdmRptwizTypGridEdit"); if (cc != null) { cc.Visible = false; }
				}
				else
				{
			        HtmlTableRow tr = e.Item.FindControl("cAdmRptwizTypGridRow") as HtmlTableRow;
			        LinkButton lb = e.Item.FindControl("cAdmRptwizTypGridEdit") as LinkButton;
		...
		protected void cRptwizTypId185hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "0") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "0"; Session[KEY_lastSortExp] = "RptwizTypId185";Session[KEY_lastSortImg] = "cRptwizTypId185hi";
			cAdmRptwizTypGrid_OnSorting(sender, new ListViewSortEventArgs("RptwizTypId185", SortDirection.Ascending));
		}

		protected void cRptwizTypName185hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "1") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "1"; Session[KEY_lastSortExp] = "RptwizTypName185";Session[KEY_lastSortImg] = "cRptwizTypName185hi";
			cAdmRptwizTypGrid_OnSorting(sender, new ListViewSortEventArgs("RptwizTypName185", SortDirection.Ascending));
		}

		protected void cAdmRptwizTypGrid_OnItemEditing(object sender, ListViewEditEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmRptwizTypGrid];
			if ((ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")) && dt != null && dt.DefaultView.Count > GetDataItemIndex(e.NewEditIndex))
			{
				cAdmRptwizTypGrid.EditIndex = e.NewEditIndex;
				cAdmRptwizTypGrid_DataBind(null);
				Control wc = null;
				string idx = Page.Request["__EVENTARGUMENT"];
				if (!string.IsNullOrEmpty(idx))
				{
				    Control ctrlToFocus = cAdmRptwizTypGrid.EditItem.FindControl("c" + idx);
				    if (((WebControl)ctrlToFocus).Enabled) { wc = ctrlToFocus; }
				}
				if (wc == null) { wc = FindEditableControl(cAdmRptwizTypGrid.EditItem); }
				if (wc != null)
             {
             ...
             MkMobileLabel(cAdmRptwizTypGrid.EditItem);
		    }
		    else { e.Cancel = true; }
		}
		...
		{
		    Label ml = null;
		    ml = lvi.FindControl("cRptwizTypId185ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(0); }
		    ml = lvi.FindControl("cRptwizTypName185ml") as Label;
		...
		{
			TextBox tb = null;
			tb = (TextBox)lvi.FindControl("cRptwizTypName185");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["RptwizTypName185"] = tb.Text;} else {dr["RptwizTypName185"] = System.DBNull.Value;}
			}
		    if (bInsert) { dt.Rows.InsertAt(dr, 0); }
			Session[KEY_dtAdmRptwizTypGrid] = dt;
			cAdmRptwizTypGrid.EditIndex = -1;
			cAdmRptwizTypGrid_DataBind(dt.DefaultView);
		}

		protected void cAdmRptwizTypGrid_OnItemUpdating(object sender, ListViewUpdateEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmRptwizTypGrid];
			if (dt != null && dt.Rows.Count > 0)
			{
				DataRow dr = dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row;
			    GridFill(cAdmRptwizTypGrid.EditItem, dt, dr, false);
			    dr["_NewRow"] = "N";
			    if (dt.Columns.Contains("_SortOrder")) dr["_SortOrder"] = null;
			}
		}

		protected void cAdmRptwizTypGrid_OnItemCanceling(object sender, ListViewCancelEventArgs e)
		{
			PanelTop.Update();
			DataTable dt = (DataTable)Session[KEY_dtAdmRptwizTypGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			...
			{
				cAdmRptwizTypGrid_OnItemDeleting(sender, new ListViewDeleteEventArgs(e.ItemIndex));
			}
			if (dt.Columns.Contains("_SortOrder")) dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row["_SortOrder"] = null;
			cAdmRptwizTypGrid.EditIndex = -1; cAdmRptwizTypGrid_DataBind(null);
		}
		...
		protected void cDeleteAllButton_Click(object sender, System.EventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmRptwizTypGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null && (cAdmRptwizTypGrid.EditIndex < 0 || UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				GotoPage(0);
				while (dv.Count > 0) {dv.Delete(0);}
				Session[KEY_dtAdmRptwizTypGrid] = dt;
				cAdmRptwizTypGrid.EditIndex = -1; cAdmRptwizTypGrid_DataBind(dv);
			}
		}

		protected void cAdmRptwizTypGrid_OnItemDeleting(object sender, ListViewDeleteEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmRptwizTypGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			DataRow dr = dv != null ? dv[GetDataItemIndex(e.ItemIndex)].Row : null;
			if (dv != null && (cAdmRptwizTypGrid.EditIndex == e.ItemIndex || UpdateGridRow(sender, new CommandEventArgs("Save",""))))
			{
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmRptwizTypGrid); PopAdmRptwizTyp97List(sender, e, false, null);
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			// *** System Button Click (Before) Web Rule starts here *** //
			if (ValidPage() && UpdateGridRow(cAdmRptwizTypGrid, new CommandEventArgs("Save", "")))
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmRptwizTypGrid];
				DataView dv = dt != null ? dt.DefaultView : null;
				string ftr = string.Empty;
				if (dv.RowFilter != string.Empty) { ftr = dv.RowFilter; dv.RowFilter = string.Empty; }
				AdmRptwizTyp97 ds = PrepAdmRptwizTypData(dv,false);
				if (ftr != string.Empty) {dv.RowFilter = ftr;}
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(97,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc,noTrans,commandTimeOut)) {bValid7 = true;}
					if (bValid7)
					{
						Session[KEY_currPageIndex] = GetCurrPageIndex();
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmRptwizTyp97List(sender, e, false, null);
		...
		private AdmRptwizTyp97 PrepAdmRptwizTypData(DataView dv, bool bAdd)
		{
			AdmRptwizTyp97 ds = new AdmRptwizTyp97();
			if (dv != null)
			{
				ds.Tables["AdmRptwizTypDef"].Rows.Add(MakeTypRow(ds.Tables["AdmRptwizTypDef"].NewRow()));
				ds.Tables["AdmRptwizTypDef"].Rows.Add(MakeDisRow(ds.Tables["AdmRptwizTypDef"].NewRow()));
					dv.RowStateFilter = DataViewRowState.ModifiedCurrent;
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmRptwizTypUpd"].Rows.Add(MakeColRow(ds.Tables["AdmRptwizTypUpd"].NewRow(), drv, false));
					}
					dv.RowStateFilter = DataViewRowState.Added;
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmRptwizTypAdd"].Rows.Add(MakeColRow(ds.Tables["AdmRptwizTypAdd"].NewRow(), drv, true));
					}
					dv.RowStateFilter = DataViewRowState.Deleted;
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmRptwizTypDel"].Rows.Add(MakeColRow(ds.Tables["AdmRptwizTypDel"].NewRow(), drv, false));
					}
		...
		{
			dr["RptwizTypId185"] = System.Data.OleDb.OleDbType.Numeric.ToString();
			dr["RptwizTypName185"] = System.Data.OleDb.OleDbType.VarWChar.ToString();
			return dr;
		}

		private DataRow MakeDisRow(DataRow dr)
		{
			dr["RptwizTypId185"] = "TextBox";
			dr["RptwizTypName185"] = "TextBox";
			return dr;
		}
		...
		private DataRow MakeColRow(DataRow dr, DataRowView drv, bool bAdd)
		{
			...
			if (dtAuth != null)
			{
				dr["RptwizTypId185"] = drv["RptwizTypId185"].ToString().Trim();
				dr["RptwizTypName185"] = drv["RptwizTypName185"].ToString().Trim();
			}
			return dr;
		}
		...
		{
			if (cAdmRptwizTypGrid.EditIndex > -1)
			{
				cAdmRptwizTypGrid_OnItemUpdating(sender, new ListViewUpdateEventArgs(cAdmRptwizTypGrid.EditIndex));
			}
			return true;
		}

		protected void cAdmRptwizTypGrid_OnPreRender(object sender, System.EventArgs e)
		{
			System.Web.UI.WebControls.Image hi = null;
			hi = (System.Web.UI.WebControls.Image)cAdmRptwizTypGrid.FindControl("cRptwizTypId185hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmRptwizTypGrid.FindControl("cRptwizTypName185hi"); if (hi != null) { hi.Visible = false; }
			if (Session[KEY_lastSortImg] != null)
			{
				hi = (System.Web.UI.WebControls.Image)cAdmRptwizTypGrid.FindControl((string)Session[KEY_lastSortImg]);
				if (hi != null) { hi.ImageUrl = Utils.AddTilde((string)Session[KEY_lastSortUrl]); hi.Visible = true; }
			}
			IgnoreHeaderConfirm((LinkButton)cAdmRptwizTypGrid.FindControl("cRptwizTypId185hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmRptwizTypGrid.FindControl("cRptwizTypName185hl"));
		}

		private string GetButtonId(ListViewItem lvi)
		{
			string ButtonID = String.Empty;
			Control c = lvi.FindControl("cAdmRptwizTypGridEdit");
		...
		{
		    return cAdmRptwizTypGridDataPager.StartRowIndex / cAdmRptwizTypGridDataPager.PageSize;
		}

		private int GetTotalPages()
		{
		    return (int)Math.Ceiling((double)cAdmRptwizTypGridDataPager.TotalRowCount / cAdmRptwizTypGridDataPager.PageSize);
		}

		private int GetDataItemIndex(int editIndex)
		{
		    return cAdmRptwizTypGridDataPager.StartRowIndex + editIndex;
		}

		protected void cAdmRptwizTypGrid_OnLayoutCreated(object sender, EventArgs e)
		{
		    // Header:
		    LinkButton lb = null;
		    lb = cAdmRptwizTypGrid.FindControl("cRptwizTypId185hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(0); lb.ToolTip = ColumnToolTip(0); lb.Parent.Visible = GridColumnVisible(0); }
		    lb = cAdmRptwizTypGrid.FindControl("cRptwizTypName185hl") as LinkButton;
			...
			if (dtAuthRow != null)
			{
				...
				if (dr["AllowDel"].ToString() == "N" || (Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N"))
				{
		            lb = cAdmRptwizTypGrid.FindControl("cDeleteAllButton") as LinkButton; if (lb != null) { lb.Visible = false; }
				}
			}
		    // footer:
		    Label gc = null;
		    gc = cAdmRptwizTypGrid.FindControl("cRptwizTypId185fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(0); }
		    gc = cAdmRptwizTypGrid.FindControl("cRptwizTypName185fl") as Label;

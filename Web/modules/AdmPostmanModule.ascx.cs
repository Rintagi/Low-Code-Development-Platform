...
namespace RO.Web
{
	public partial class AdmPostmanModule : RO.Web.ModuleBase
	{
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_1028";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_1028";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_1028";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_1028";
		private const string KEY_dtLabel = "Cache:dtLabel3_1028";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_1028";
		private const string KEY_dtScrCri = "Cache:dtScrCri3_1028";
		private const string KEY_dsScrCriVal = "Cache:dsScrCriVal3_1028";

		private const string KEY_dtPostContentType = "Cache:dtPostContentType";

		private const string KEY_dtSystems = "Cache:dtSystems3_1028";
		private const string KEY_sysConnectionString = "Cache:sysConnectionString3_1028";
		private const string KEY_dtAdmPostman1028List = "Cache:dtAdmPostman1028List";
		private const string KEY_bNewVisible = "Cache:bNewVisible3_1028";
		private const string KEY_bNewSaveVisible = "Cache:bNewSaveVisible3_1028";
		private const string KEY_bCopyVisible = "Cache:bCopyVisible3_1028";
		private const string KEY_bCopySaveVisible = "Cache:bCopySaveVisible3_1028";
		private const string KEY_bDeleteVisible = "Cache:bDeleteVisible3_1028";
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_1028";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_1028";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_1028";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				DataTable dtMenuAccess = (new MenuSystem()).GetMenu(base.LUser.CultureId, 3, base.LImpr, LcSysConnString, LcAppPw,1028, null, null);
				...
				Session.Remove(KEY_dtAdmPostman1028List);
				...
				Session.Remove(KEY_dtPostContentType);
				SetButtonHlp();
				GetSystems();
				SetColumnAuthority();
				GetGlobalFilter();
				...
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 1028, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmPostman1028List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=1028&typ=N&sys=3','','',''); return false;"; }
		...
		private void SetSystem(byte SystemId)
		{
			...
			try
			{
				...
				if ((Config.DeployType == "DEV" || row["dbAppDatabase"].ToString() == base.CPrj.EntityCode + "View") && !(base.CPrj.EntityCode != "RO" && row["SysProgram"].ToString() == "Y") && (new AdminSystem()).IsRegenNeeded(string.Empty,1028,0,0,LcSysConnString,LcAppPw))
				{
					(new GenScreensSystem()).CreateProgram(1028, "Rintagi Postman", row["dbAppDatabase"].ToString(), base.CPrj, base.CSrc, base.CTar, LcAppConnString, LcAppPw);
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(1028,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(1028,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(1028,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(1028,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,1028,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(1028,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(1028,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(1028,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		protected DataTable GetAuthRow()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthRow(1028,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				try
				{
					try {dv = new DataView((new AdminSystem()).GetExp(1028,"GetExpAdmPostman1028","Y",null,null,filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
					catch {dv = new DataView((new AdminSystem()).GetExp(1028,"GetExpAdmPostman1028","N",null,null,filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(1028,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					{
						if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["SystemId1317"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["SystemName1317"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["SystemAbbr1317"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["ServerName1317"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						sb.Append(Environment.NewLine);
					}
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmPostman.csv"; Session["ExportStr"] = (Config.ExportExcelCSV ? "sep=\t\n": "") + sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmPostman.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private string GetRtfGrid(DataView dv)
		{
			...
			try
			{
				...
				foreach (DataRowView drv in dv)
				{
					...
					sb.Append("\r\n");
					sb.Append(@"{");
					if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["SystemId1317"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["SystemName1317"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(drv["SystemAbbr1317"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(drv["ServerName1317"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					sb.Append(@"}");
					sb.Append("\r\n");
					sb.Append(@"\pard \ql \li0\ri0\widctlpar\intbl\aspalpha\aspnum\adjustright\rin0\lin0 {");
					sb.Append(@"\insrsid2981395 \trowd \irow0\irowband0\lastrow \ts15\trgaph108\trleft-108\trbrdrt\brdrs\brdrw10 \trbrdrl\brdrs\brdrw10 \trbrdrb\brdrs\brdrw10 \trbrdrr\brdrs\brdrw10 \trbrdrh\brdrs\brdrw10 \trbrdrv\brdrs\brdrw10 ");
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 1028, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			...
			UpdCriteria(true);
			cAdmPostman1028List.ClearSearch(); Session.Remove(KEY_dtAdmPostman1028List);
			PopAdmPostman1028List(sender, e, false, null);
		}
			...
			try
			{
				foreach (DataRowView drv in dvCri)
				{
					...
					if (drv["DisplayName"].ToString() == "ComboBox")
					{
						...
						(new AdminSystem()).MkGetScreenIn("1028", drv["ScreenCriId"].ToString(), "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), LcAppDb, LcDesDb, drv["MultiDesignDb"].ToString(), LcSysConnString, LcAppPw);
						DataView dv = new DataView((new AdminSystem()).GetScreenIn("1028", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), (new AdminSystem()).CountScrCri(drv["ScreenCriId"].ToString(), drv["MultiDesignDb"].ToString(), drv["MultiDesignDb"].ToString() == "N" ? LcSysConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)), drv["RequiredValid"].ToString(), 0, string.Empty, drv["DisplayMode"].ToString() != "AutoListBox", val, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)));
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("1028", LcSysConnString, LcAppPw);
				}
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmPostman1028" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmPostman1028" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			foreach (DataRowView drv in dvCri)
			{
			    if (drv["DisplayName"].ToString() == "ListBox")
			    {
					...
					if (cListBox != null)
					{
						...
						int TotalChoiceCnt = new DataView((new AdminSystem()).GetScreenIn("1028", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), CriCnt, drv["RequiredValid"].ToString(), 0, string.Empty, true, string.Empty, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId))).Count;
						string selectedValues = string.Join(",", cListBox.Items.Cast<ListItem>().Where(x => x.Selected).Select(x => "'" + x.Value + "'").ToArray());
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("1028", "AdmPostman1028", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		private void SetPostContentType(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtPostContentType];
			...
			if (ddl != null)
			{
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(1028,"GetDdlPostContentType3S4423",true,bAll,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					if (dv.Table.Columns.Contains("SystemId"))
					{
						ss = "(SystemId is null";
						if (string.IsNullOrEmpty(cAdmPostman1028List.SelectedValue)) {ss = ss + ")";} else {ss = ss + " OR SystemId = " + cAdmPostman1028List.SelectedValue + ")";}
					}
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "PostContentType is not NULL"; }
					dv.RowFilter = ss;
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(1028,"GetDdlPostContentType3S4423",true,true,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtPostContentType] = dv.Table;
				}
			}
		}

		private DataView GetAdmPostman1028List()
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmPostman1028List];
			if (dt == null)
			{
				CheckAuthentication(false);
				int filterId = 0;
				...
				try
				{
					try {dt = (new AdminSystem()).GetLis(1028,"GetLisAdmPostman1028",true,"Y",0,null,null,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
					catch {dt = (new AdminSystem()).GetLis(1028,"GetLisAdmPostman1028",true,"N",0,null,null,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				}
		...
		private void PopAdmPostman1028List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			DataView dv = GetAdmPostman1028List();
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			cAdmPostman1028List.AutoCompleteUrl = "AutoComplete.aspx/LisSuggests";
			cAdmPostman1028List.DataContext = context;
			if (dv.Table == null) return;
			cAdmPostman1028List.DataSource = dv;
			cAdmPostman1028List.Visible = true;
			if (cAdmPostman1028List.Items.Count <= 0) {cAdmPostman1028List.Visible = false; cAdmPostman1028List_SelectedIndexChanged(sender,e);}
			else
			{
				if (bPageLoad && !string.IsNullOrEmpty(Request.QueryString["key"]) && bUseCri.Value == string.Empty)
				{
					try {cAdmPostman1028List.SelectByValue(Request.QueryString["key"],string.Empty,true);} catch {cAdmPostman1028List.Items[0].Selected = true; cAdmPostman1028List_SelectedIndexChanged(sender, e);}
				    cScreenSearch.Visible = false; cSystem.Visible = false; cEditButton.Visible = true; cSaveCloseButton.Visible = cSaveButton.Visible;
				}
				else
				{
					if (ID != null) {if (!cAdmPostman1028List.SelectByValue(ID,string.Empty,true)) {ID = null;}}
					if (ID == null)
					{
						cAdmPostman1028List_SelectedIndexChanged(sender, e);
					}
				}
			}
		}
		...
		private void SetColumnAuthority()
		{
			...
			if (dtAuth != null && dtLabel != null)
			{
				...
				base.SetFoldBehavior(cServerName1317, dtAuth.Rows[3], cServerName1317P1, cServerName1317Label, cServerName1317P2, null, dtLabel.Rows[3], null, null, null);
				base.SetFoldBehavior(cEndpointUrl, dtAuth.Rows[4], cEndpointUrlP1, cEndpointUrlLabel, cEndpointUrlP2, null, dtLabel.Rows[4], null, null, null);
				base.SetFoldBehavior(cMethod, dtAuth.Rows[5], cMethodP1, cMethodLabel, cMethodP2, null, dtLabel.Rows[5], null, null, null);
				base.SetFoldBehavior(cResult, dtAuth.Rows[6], cResultP1, cResultLabel, cResultP2, cResultE, null, dtLabel.Rows[6], null, null, null);
				cResultE.Attributes["label_id"] = cResultLabel.ClientID; cResultE.Attributes["target_id"] = cResult.ClientID;
				base.SetFoldBehavior(cOTPCode, dtAuth.Rows[7], cOTPCodeP1, cOTPCodeLabel, cOTPCodeP2, null, dtLabel.Rows[7], null, null, null);
				base.SetFoldBehavior(cSubmit, dtAuth.Rows[8], null, null, null, dtLabel.Rows[8], null, null, null);
				base.SetFoldBehavior(cQueryString, dtAuth.Rows[9], cQueryStringP1, cQueryStringLabel, cQueryStringP2, cQueryStringE, null, dtLabel.Rows[9], null, null, null);
				cQueryStringE.Attributes["label_id"] = cQueryStringLabel.ClientID; cQueryStringE.Attributes["target_id"] = cQueryString.ClientID;
				base.SetFoldBehavior(cQSUrlEncode, dtAuth.Rows[10], cQSUrlEncodeP1, cQSUrlEncodeLabel, cQSUrlEncodeP2, null, dtLabel.Rows[10], null, null, null);
				base.SetFoldBehavior(cBody, dtAuth.Rows[11], cBodyP1, cBodyLabel, cBodyP2, cBodyE, null, dtLabel.Rows[11], null, null, null);
				cBodyE.Attributes["label_id"] = cBodyLabel.ClientID; cBodyE.Attributes["target_id"] = cBody.ClientID;
				base.SetFoldBehavior(cPostContentType, dtAuth.Rows[12], cPostContentTypeP1, cPostContentTypeLabel, cPostContentTypeP2, null, dtLabel.Rows[12], null, null, null);
				base.SetFoldBehavior(cBodyUrlEncode, dtAuth.Rows[13], cBodyUrlEncodeP1, cBodyUrlEncodeLabel, cBodyUrlEncodeP2, null, dtLabel.Rows[13], null, null, null);
				base.SetFoldBehavior(cHeader, dtAuth.Rows[14], cHeaderP1, cHeaderLabel, cHeaderP2, cHeaderE, null, dtLabel.Rows[14], null, null, null);
				cHeaderE.Attributes["label_id"] = cHeaderLabel.ClientID; cHeaderE.Attributes["target_id"] = cHeader.ClientID;
				base.SetFoldBehavior(cCookie, dtAuth.Rows[15], cCookieP1, cCookieLabel, cCookieP2, cCookieE, null, dtLabel.Rows[15], null, null, null);
				cCookieE.Attributes["label_id"] = cCookieLabel.ClientID; cCookieE.Attributes["target_id"] = cCookie.ClientID;
			}
			...
			if ((cServerName1317.Attributes["OnChange"] == null || cServerName1317.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cServerName1317.Visible && !cServerName1317.ReadOnly) {cServerName1317.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cEndpointUrl.Attributes["OnChange"] == null || cEndpointUrl.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cEndpointUrl.Visible && !cEndpointUrl.ReadOnly) {cEndpointUrl.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cMethod.Attributes["OnChange"] == null || cMethod.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cMethod.Visible && !cMethod.ReadOnly) {cMethod.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cResult.Attributes["OnChange"] == null || cResult.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cResult.Visible && !cResult.ReadOnly) {cResult.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cOTPCode.Attributes["OnChange"] == null || cOTPCode.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cOTPCode.Visible && !cOTPCode.ReadOnly) {cOTPCode.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cQueryString.Attributes["OnChange"] == null || cQueryString.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cQueryString.Visible && !cQueryString.ReadOnly) {cQueryString.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cQSUrlEncode.Attributes["OnClick"] == null || cQSUrlEncode.Attributes["OnClick"].IndexOf("ChkPgDirty") < 0) && cQSUrlEncode.Visible && cQSUrlEncode.Enabled) {cQSUrlEncode.Attributes["OnClick"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty(); this.focus();";}
			if ((cBody.Attributes["OnChange"] == null || cBody.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cBody.Visible && !cBody.ReadOnly) {cBody.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cPostContentType.Attributes["OnChange"] == null || cPostContentType.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cPostContentType.Visible && cPostContentType.Enabled) {cPostContentType.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cBodyUrlEncode.Attributes["OnClick"] == null || cBodyUrlEncode.Attributes["OnClick"].IndexOf("ChkPgDirty") < 0) && cBodyUrlEncode.Visible && cBodyUrlEncode.Enabled) {cBodyUrlEncode.Attributes["OnClick"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty(); this.focus();";}
			if ((cHeader.Attributes["OnChange"] == null || cHeader.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cHeader.Visible && !cHeader.ReadOnly) {cHeader.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cCookie.Attributes["OnChange"] == null || cCookie.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cCookie.Visible && !cCookie.ReadOnly) {cCookie.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
		}
		...
		private void ClearMaster(object sender, System.EventArgs e)
		{
			...
			if (dt.Rows[1]["ColVisible"].ToString() == "Y" && dt.Rows[1]["ColReadOnly"].ToString() != "Y") {cSystemName1317.Text = string.Empty;}
			if (dt.Rows[2]["ColVisible"].ToString() == "Y" && dt.Rows[2]["ColReadOnly"].ToString() != "Y") {cSystemAbbr1317.Text = string.Empty;}
			if (dt.Rows[3]["ColVisible"].ToString() == "Y" && dt.Rows[3]["ColReadOnly"].ToString() != "Y") {cServerName1317.Text = string.Empty;}
			if (dt.Rows[4]["ColVisible"].ToString() == "Y" && dt.Rows[4]["ColReadOnly"].ToString() != "Y") {cEndpointUrl.Text = string.Empty;}
			if (dt.Rows[5]["ColVisible"].ToString() == "Y" && dt.Rows[5]["ColReadOnly"].ToString() != "Y") {cMethod.Text = "GET";}
			if (dt.Rows[6]["ColVisible"].ToString() == "Y" && dt.Rows[6]["ColReadOnly"].ToString() != "Y") {cResult.Text = string.Empty;}
			if (dt.Rows[7]["ColVisible"].ToString() == "Y" && dt.Rows[7]["ColReadOnly"].ToString() != "Y") {cOTPCode.Text = string.Empty;}
			if (dt.Rows[9]["ColVisible"].ToString() == "Y" && dt.Rows[9]["ColReadOnly"].ToString() != "Y") {cQueryString.Text = string.Empty;}
			if (dt.Rows[10]["ColVisible"].ToString() == "Y" && dt.Rows[10]["ColReadOnly"].ToString() != "Y") {cQSUrlEncode.Checked = base.GetBool("Y");}
			if (dt.Rows[11]["ColVisible"].ToString() == "Y" && dt.Rows[11]["ColReadOnly"].ToString() != "Y") {cBody.Text = string.Empty;}
			if (dt.Rows[12]["ColVisible"].ToString() == "Y" && dt.Rows[12]["ColReadOnly"].ToString() != "Y") {SetPostContentType(cPostContentType,string.Empty);}
			if (dt.Rows[13]["ColVisible"].ToString() == "Y" && dt.Rows[13]["ColReadOnly"].ToString() != "Y") {cBodyUrlEncode.Checked = base.GetBool("N");}
			if (dt.Rows[14]["ColVisible"].ToString() == "Y" && dt.Rows[14]["ColReadOnly"].ToString() != "Y") {cHeader.Text = string.Empty;}
			if (dt.Rows[15]["ColVisible"].ToString() == "Y" && dt.Rows[15]["ColReadOnly"].ToString() != "Y") {cCookie.Text = string.Empty;}
			// *** Default Value (Folder) Web Rule starts here *** //
		}

		private void InitMaster(object sender, System.EventArgs e)
		{
			cSystemId1317.Text = string.Empty;
			cSystemName1317.Text = string.Empty;
			cSystemAbbr1317.Text = string.Empty;
			cServerName1317.Text = string.Empty;
			cEndpointUrl.Text = string.Empty;
			cMethod.Text = "GET";
			cResult.Text = string.Empty;
			cOTPCode.Text = string.Empty;
			cQueryString.Text = string.Empty;
			cQSUrlEncode.Checked = base.GetBool("Y");
			cBody.Text = string.Empty;
			SetPostContentType(cPostContentType,string.Empty);
			cBodyUrlEncode.Checked = base.GetBool("N");
			cHeader.Text = string.Empty;
			cCookie.Text = string.Empty;
			// *** Default Value (Folder) Web Rule starts here *** //
		}
		protected void cAdmPostman1028List_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmPostman1028List_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmPostman1028List_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			InitMaster(sender, e);      // Do this first to enable defaults for non-database columns.
			DataTable dt = null;
			try
			{
				dt = (new AdminSystem()).GetMstById("GetAdmPostman1028ById",cAdmPostman1028List.SelectedValue,null,null);
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				if (dt.Rows.Count > 0)
				{
					cAdmPostman1028List.SetDdVisible();
					DataRow dr = dt.Rows[0];
					try {cSystemId1317.Text = RO.Common3.Utils.fmNumeric("0",dr["SystemId1317"].ToString(),base.LUser.Culture);} catch {cSystemId1317.Text = string.Empty;}
					try {cSystemName1317.Text = dr["SystemName1317"].ToString();} catch {cSystemName1317.Text = string.Empty;}
					try {cSystemAbbr1317.Text = dr["SystemAbbr1317"].ToString();} catch {cSystemAbbr1317.Text = string.Empty;}
					try {cServerName1317.Text = dr["ServerName1317"].ToString();} catch {cServerName1317.Text = string.Empty;}
				}
			}
			cButPanel.DataBind();
			ScriptManager.GetCurrent(Parent.Page).SetFocus(cAdmPostman1028List.FocusID);
			ShowDirty(false); PanelTop.Update();
			// *** List Selection (End of) Web Rule starts here *** //
		}

		protected void cSubmit_Click(object sender, System.EventArgs e)
		{
			//WebRule: Submit Request
            string url = cEndpointUrl.Text;
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cSystemId1317.Text == string.Empty) { cClearButton_Click(sender, new EventArgs()); ShowDirty(false); PanelTop.Update();}
			else { PopAdmPostman1028List(sender, e, false, cSystemId1317.Text); }
			// *** System Button Click (After) Web Rule starts here *** //
		}
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmPostman1028List); PopAdmPostman1028List(sender, e, false, null);
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			if (ValidPage())
			{
				AdmPostman1028 ds = PrepAdmPostmanData(null,cSystemId1317.Text == string.Empty);
				if (!string.IsNullOrEmpty(cAdmPostman1028List.SelectedValue) && cSystemId1317.Text != cAdmPostman1028List.SelectedValue)
				{
					PreMsgPopup("Primary key cannot be changed");
					return rtn;
				}
				if (string.IsNullOrEmpty(cAdmPostman1028List.SelectedValue))	// Add
				{
					if (ds != null)
					{
						pid = (new AdminSystem()).AddData(1028,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc,noTrans,commandTimeOut);
					}
					if (!string.IsNullOrEmpty(pid))
					{
						cAdmPostman1028List.ClearSearch(); Session.Remove(KEY_dtAdmPostman1028List);
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmPostman1028List(sender, e, false, pid);
						rtn = GetScreenHlp().Rows[0]["AddMsg"].ToString();
					}
				}
				else {
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(1028,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc,noTrans,commandTimeOut)) {bValid7 = true;}
					if (bValid7)
					{
						cAdmPostman1028List.ClearSearch(); Session.Remove(KEY_dtAdmPostman1028List);
						ShowDirty(false); PopAdmPostman1028List(sender, e, false, ds.Tables["AdmPostman"].Rows[0]["SystemId1317"]);
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cSystemId1317.Text != string.Empty)
			{
				AdmPostman1028 ds = PrepAdmPostmanData(null,false);
				try
				{
					if (ds != null && (new AdminSystem()).DelData(1028,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc))
					{
						cAdmPostman1028List.ClearSearch(); Session.Remove(KEY_dtAdmPostman1028List);
						ShowDirty(false); PopAdmPostman1028List(sender, e, false, null);
		...
		private AdmPostman1028 PrepAdmPostmanData(DataView dv, bool bAdd)
		{
		...
		public bool CanAct(char typ) { return CanAct(typ, GetAuthRow(), cAdmPostman1028List.SelectedValue.ToString()); }
		...
		private void PreMsgPopup(string msg) { PreMsgPopup(msg,cAdmPostman1028List,null); }

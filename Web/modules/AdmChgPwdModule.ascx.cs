		...
		private void getReport(string eExport)
		{
			try
			{
				...
				if (eExport == "TXT")
				{
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmChgPwd.csv"; Session["ExportStr"] = (Config.ExportExcelCSV ? "sep=\t\n": "") + sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			if (ValidPage())
			{
				...
				if (!string.IsNullOrEmpty(cAdmChgPwd71List.SelectedValue) && cUsrId1.Text != cAdmChgPwd71List.SelectedValue)
				{
				...
				if (string.IsNullOrEmpty(cAdmChgPwd71List.SelectedValue))	// Add
				{
					if (ds != null)
					{
						pid = (new AdminSystem()).AddData(71,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc,noTrans,commandTimeOut);
					}
					...
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(71,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc,noTrans,commandTimeOut)) {bValid7 = true;}
					if (bValid7)
					{
						cAdmChgPwd71List.ClearSearch(); Session.Remove(KEY_dtAdmChgPwd71List);

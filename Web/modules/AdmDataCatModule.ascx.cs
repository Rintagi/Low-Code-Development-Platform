...
namespace RO.Web
{
	public partial class AdmDataCatModule : RO.Web.ModuleBase
	{

		private const string KEY_lastAddedRow = "Cache:lastAddedRow3_96";
		private const string KEY_lastSortOrd = "Cache:lastSortOrd3_96";
		private const string KEY_lastSortImg = "Cache:lastSortImg3_96";
		private const string KEY_lastSortCol = "Cache:lastSortCol3_96";
		private const string KEY_lastSortExp = "Cache:lastSortExp3_96";
		private const string KEY_lastSortUrl = "Cache:lastSortUrl3_96";
		private const string KEY_lastSortTog = "Cache:lastSortTog3_96";
		private const string KEY_lastImpPwdOvride = "Cache:lastImpPwdOvride3_96";
		private const string KEY_cntImpPwdOvride = "Cache:cntImpPwdOvride3_96";
		private const string KEY_currPageIndex = "Cache:currPageIndex3_96";
		private const string KEY_bHiImpVisible = "Cache:bHiImpVisible3_96";
		private const string KEY_bShImpVisible = "Cache:bShImpVisible3_96";
		private const string KEY_dtAdmDataCatGrid = "Cache:dtAdmDataCatGrid";
		private const string KEY_scrImport = "Cache:scrImport3_96";
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_96";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_96";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_96";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_96";
		private const string KEY_dtLabel = "Cache:dtLabel3_96";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_96";
		private const string KEY_dtScrCri = "Cache:dtScrCri3_96";
		private const string KEY_dsScrCriVal = "Cache:dsScrCriVal3_96";

		private const string KEY_dtRptwizTypId181 = "Cache:dtRptwizTypId181";
		private const string KEY_dtTableId181 = "Cache:dtTableId181";
		private const string KEY_dtColumnId182 = "Cache:dtColumnId182";
		private const string KEY_dtDisplayModeId182 = "Cache:dtDisplayModeId182";

		private const string KEY_dtSystems = "Cache:dtSystems3_96";
		private const string KEY_sysConnectionString = "Cache:sysConnectionString3_96";
		private const string KEY_dtAdmDataCat96List = "Cache:dtAdmDataCat96List";
		private const string KEY_bNewVisible = "Cache:bNewVisible3_96";
		private const string KEY_bNewSaveVisible = "Cache:bNewSaveVisible3_96";
		private const string KEY_bCopyVisible = "Cache:bCopyVisible3_96";
		private const string KEY_bCopySaveVisible = "Cache:bCopySaveVisible3_96";
		private const string KEY_bDeleteVisible = "Cache:bDeleteVisible3_96";
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_96";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_96";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_96";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				DataTable dtMenuAccess = (new MenuSystem()).GetMenu(base.LUser.CultureId, 3, base.LImpr, LcSysConnString, LcAppPw,96, null, null);
				...
				Session.Remove(KEY_dtRptwizTypId181);
				Session.Remove(KEY_dtTableId181);
				Session.Remove(KEY_dtColumnId182);
				Session.Remove(KEY_dtDisplayModeId182);
				SetButtonHlp();
				GetSystems();
				SetColumnAuthority();
				GetGlobalFilter();
				...
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 96, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmDataCat96List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=96&typ=N&sys=3','','',''); return false;"; }
				cRptwizTypId181Search_Script();
		...
		private void InitializeComponent()
		{
			...
			if (LcSysConnString == null)
			{
				...
				{
					DataTable dt;
					dt = (new AdminSystem()).GetAuthRow(96,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
					...
					{
						cAdmDataCatGrid.PreRender += new EventHandler(cAdmDataCatGrid_OnPreRender);
					}
				}
		...
		private void SetSystem(byte SystemId)
		{
			...
			try
			{
				...
				if ((Config.DeployType == "DEV" || row["dbAppDatabase"].ToString() == base.CPrj.EntityCode + "View") && !(base.CPrj.EntityCode != "RO" && row["SysProgram"].ToString() == "Y") && (new AdminSystem()).IsRegenNeeded(string.Empty,96,0,0,LcSysConnString,LcAppPw))
				{
					(new GenScreensSystem()).CreateProgram(96, "Report Data Category", row["dbAppDatabase"].ToString(), base.CPrj, base.CSrc, base.CTar, LcAppConnString, LcAppPw);
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(96,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(96,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void SetClientRule(ListViewDataItem ee, bool isEditItem)
		{
			...
			if (dvRul.Count > 0)
			{
				...
				{
					if (ee == null || drv["ScriptEvent"].ToString().Substring(0,2).ToLower() != "on" || (cAdmDataCatGrid.EditIndex > -1 && GetDataItemIndex(cAdmDataCatGrid.EditIndex) == ee.DataItemIndex))
					{
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(96,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(96,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,96,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(96,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(96,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(96,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				try
				{
					try {dv = new DataView((new AdminSystem()).GetExp(96,"GetExpAdmDataCat96","Y",(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
					catch {dv = new DataView((new AdminSystem()).GetExp(96,"GetExpAdmDataCat96","N",(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(96,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					{
						if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["RptwizCatId181"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["RptwizTypId181"].ToString() + (char)9 + drv["RptwizTypId181Text"].ToString() + (char)9);}
						if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["RptwizCatName181"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["CatDescription181"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append(drv["TableId181"].ToString() + (char)9 + drv["TableId181Text"].ToString() + (char)9);}
						if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["RptwizCatDtlId182"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[7]["ColExport"].ToString() == "Y") {sb.Append(drv["ColumnId182"].ToString() + (char)9 + drv["ColumnId182Text"].ToString() + (char)9);}
						if (dtAu.Rows[8]["ColExport"].ToString() == "Y") {sb.Append(drv["DisplayModeId182"].ToString() + (char)9 + drv["DisplayModeId182Text"].ToString() + (char)9);}
						if (dtAu.Rows[9]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["ColumnSize182"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[10]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["RowSize182"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[11]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["DdlKeyColNm182"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[12]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["DdlRefColNm182"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[13]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["RegClause182"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[14]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["StoredProc182"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						sb.Append(Environment.NewLine);
					}
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmDataCat.csv"; Session["ExportStr"] = (Config.ExportExcelCSV ? "sep=\t\n": "") + sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmDataCat.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private string GetRtfGrid(DataView dv)
		{
			...
			try
			{
				...
				foreach (DataRowView drv in dv)
				{
					...
					sb.Append("\r\n");
					sb.Append(@"{");
					if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["RptwizCatId181"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["RptwizTypId181Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(drv["RptwizCatName181"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(drv["CatDescription181"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append(drv["TableId181Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["RptwizCatDtlId182"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[7]["ColExport"].ToString() == "Y") {sb.Append(drv["ColumnId182Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[8]["ColExport"].ToString() == "Y") {sb.Append(drv["DisplayModeId182Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[9]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["ColumnSize182"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[10]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["RowSize182"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[11]["ColExport"].ToString() == "Y") {sb.Append(drv["DdlKeyColNm182"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[12]["ColExport"].ToString() == "Y") {sb.Append(drv["DdlRefColNm182"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[13]["ColExport"].ToString() == "Y") {sb.Append(drv["RegClause182"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[14]["ColExport"].ToString() == "Y") {sb.Append(drv["StoredProc182"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					sb.Append(@"}");
					sb.Append("\r\n");
					sb.Append(@"\pard \ql \li0\ri0\widctlpar\intbl\aspalpha\aspnum\adjustright\rin0\lin0 {");
					sb.Append(@"\insrsid2981395 \trowd \irow0\irowband0\lastrow \ts15\trgaph108\trleft-108\trbrdrt\brdrs\brdrw10 \trbrdrl\brdrs\brdrw10 \trbrdrb\brdrs\brdrw10 \trbrdrr\brdrs\brdrw10 \trbrdrh\brdrs\brdrw10 \trbrdrv\brdrs\brdrw10 ");
		...
		public void cRptwizTypId181Search_Click(object sender, System.Web.UI.ImageClickEventArgs e)
		{
			RoboCoder.WebControls.ComboBox cc = cRptwizTypId181; cc.SetTbVisible(); Session["CtrlAdmRptwizTyp"] = cc.FocusID;
		}

		public void cRptwizTypId181Search_Script()
		{
				ImageButton ib = cRptwizTypId181Search;
				if (ib != null)
				{
			    	TextBox pp = cRptwizCatId181;
					RoboCoder.WebControls.ComboBox cc = cRptwizTypId181;
					string ss = "&dsp=ComboBox"; if (cSystem.Visible) {ss = ss + "&sys=" + cSystemId.SelectedValue;} else {ss = ss + "&sys=3";}
					ib.Attributes["onclick"] = "SearchLink('AdmRptwizTyp.aspx?col=RptWizTypId" + ss + "&typ=N" + "','" + cc.FocusID + "','',''); return false;";
				}
		}

		public void cTableId181Search_Click(object sender, System.Web.UI.ImageClickEventArgs e)
		{
			RoboCoder.WebControls.ComboBox cc = cTableId181; cc.SetTbVisible(); Session["CtrlAdmDbTable"] = cc.FocusID;
		}

		public void cTableId181Search_Script()
		{
				ImageButton ib = cTableId181Search;
				if (ib != null)
				{
			    	TextBox pp = cRptwizCatId181;
					RoboCoder.WebControls.ComboBox cc = cTableId181;
					string ss = "&dsp=ComboBox"; if (cSystem.Visible) {ss = ss + "&sys=" + cSystemId.SelectedValue;} else {ss = ss + "&sys=3";}
					ib.Attributes["onclick"] = "SearchLink('AdmDbTable.aspx?col=TableId" + ss + "&typ=N" + "','" + cc.FocusID + "','',''); return false;";
				}
		}

		public void cSampleImage181Tgo_Click(object sender, System.Web.UI.ImageClickEventArgs e)
		{
			if (cSampleImage181.Visible)
			{
				cSampleImage181Pan.Visible = true; cSampleImage181.Visible = false;
			}
			else
			{
				cSampleImage181Pan.Visible = false; cSampleImage181.Visible = true;
			}
		}
		...
		private void GetPageInfo()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastPageInfo(96, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 96, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			// *** Criteria Trigger (before) Web Rule starts here *** //
			DataTable dt = (DataTable)Session[KEY_dtAdmDataCatGrid];
			if (cAdmDataCatGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
			...
			PopAdmDataCat96List(sender, e, false, null);
			}
		}
			...
			try
			{
				foreach (DataRowView drv in dvCri)
				{
					...
					if (drv["DisplayName"].ToString() == "ComboBox")
					{
						...
						(new AdminSystem()).MkGetScreenIn("96", drv["ScreenCriId"].ToString(), "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), LcAppDb, LcDesDb, drv["MultiDesignDb"].ToString(), LcSysConnString, LcAppPw);
						DataView dv = new DataView((new AdminSystem()).GetScreenIn("96", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), (new AdminSystem()).CountScrCri(drv["ScreenCriId"].ToString(), drv["MultiDesignDb"].ToString(), drv["MultiDesignDb"].ToString() == "N" ? LcSysConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)), drv["RequiredValid"].ToString(), 0, string.Empty, drv["DisplayMode"].ToString() != "AutoListBox", val, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)));
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("96", LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmDataCat96" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmDataCat96" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			foreach (DataRowView drv in dvCri)
			{
			    if (drv["DisplayName"].ToString() == "ListBox")
			    {
					...
					if (cListBox != null)
					{
						...
						int TotalChoiceCnt = new DataView((new AdminSystem()).GetScreenIn("96", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), CriCnt, drv["RequiredValid"].ToString(), 0, string.Empty, true, string.Empty, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId))).Count;
						string selectedValues = string.Join(",", cListBox.Items.Cast<ListItem>().Where(x => x.Selected).Select(x => "'" + x.Value + "'").ToArray());
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("96", "AdmDataCat96", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		protected void cbRptwizTypId181(object sender, System.EventArgs e)
		{
			SetRptwizTypId181((RoboCoder.WebControls.ComboBox)sender,string.Empty);
		}

		private void SetRptwizTypId181(RoboCoder.WebControls.ComboBox ddl, string keyId)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(96,"GetDdlRptwizTypId3S1671",true,false,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
					if (dv.Table.Columns.Contains("RptwizCatId"))
					{
						context["pMKeyColID"] = cAdmDataCat96List.ClientID;
						context["pMKeyCol"] = "RptwizCatId";
						string ss = "(RptwizCatId is null";
						if (string.IsNullOrEmpty(cAdmDataCat96List.SelectedValue)) {ss = ss + ")";} else {ss = ss + " OR RptwizCatId = " + cAdmDataCat96List.SelectedValue + ")";}
						dv.RowFilter = ss;
					}
					ddl.DataSource = dv; Session[KEY_dtRptwizTypId181] = dv.Table;
				    try { ddl.SelectByValue(keyId,string.Empty,true); } catch { try { ddl.SelectedIndex = 0; } catch { } }
				}
			}
		}

		protected void cbTableId181(object sender, System.EventArgs e)
		{
			SetTableId181((RoboCoder.WebControls.ComboBox)sender,string.Empty);
		}

		private void SetTableId181(RoboCoder.WebControls.ComboBox ddl, string keyId)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(96,"GetDdlTableId3S1663",true,false,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
					...
					ddl.DataSource = dv; Session[KEY_dtTableId181] = dv.Table;
				    try { ddl.SelectByValue(keyId,string.Empty,true); } catch { try { ddl.SelectedIndex = 0; } catch { } }
				}
			}
		}

		protected void cbColumnId182(object sender, System.EventArgs e)
		{
			SetColumnId182((RoboCoder.WebControls.ComboBox)sender,string.Empty,cTableId181.SelectedValue,cAdmDataCatGrid.EditItem);
		}

		private void SetColumnId182(RoboCoder.WebControls.ComboBox ddl, string keyId, string filtr, ListViewItem li)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			context["isSys"] = "N";
			context["conn"] = KEY_sysConnectionString;
			context["refColCID"] = cTableId181.ClientID;
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(96,"GetDdlColumnId3S1667",true,false,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
					...
					ddl.DataSource = dv; Session[KEY_dtColumnId182] = dv.Table;
		...
		private void SetDisplayModeId182(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtDisplayModeId182];
			...
			if (ddl != null)
			{
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(96,"GetDdlDisplayModeId3S1843",true,bAll,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					if (dv.Table.Columns.Contains("RptwizCatId"))
					{
						ss = "(RptwizCatId is null";
						if (string.IsNullOrEmpty(cAdmDataCat96List.SelectedValue)) {ss = ss + ")";} else {ss = ss + " OR RptwizCatId = " + cAdmDataCat96List.SelectedValue + ")";}
					}
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "DisplayModeId182 is not NULL"; }
					dv.RowFilter = ss;
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(96,"GetDdlDisplayModeId3S1843",true,true,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtDisplayModeId182] = dv.Table;
				}
			}
		}

		private DataView GetAdmDataCat96List()
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmDataCat96List];
			if (dt == null)
			{
				CheckAuthentication(false);
				int filterId = 0;
				...
				try
				{
					try {dt = (new AdminSystem()).GetLis(96,"GetLisAdmDataCat96",true,"Y",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
					catch {dt = (new AdminSystem()).GetLis(96,"GetLisAdmDataCat96",true,"N",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				}
		...
		private void PopAdmDataCat96List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			DataView dv = GetAdmDataCat96List();
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			context["isSys"] = "N";
			context["conn"] = KEY_sysConnectionString;
			cAdmDataCat96List.AutoCompleteUrl = "AutoComplete.aspx/LisSuggests";
			cAdmDataCat96List.DataContext = context;
			if (dv.Table == null) return;
			cAdmDataCat96List.DataSource = dv;
			cAdmDataCat96List.Visible = true;
			if (cAdmDataCat96List.Items.Count <= 0) {cAdmDataCat96List.Visible = false; cAdmDataCat96List_SelectedIndexChanged(sender,e);}
			else
			{
				if (bPageLoad && !string.IsNullOrEmpty(Request.QueryString["key"]) && bUseCri.Value == string.Empty)
				{
					try {cAdmDataCat96List.SelectByValue(Request.QueryString["key"],string.Empty,true);} catch {cAdmDataCat96List.Items[0].Selected = true; cAdmDataCat96List_SelectedIndexChanged(sender, e);}
				    cScreenSearch.Visible = false; cSystem.Visible = false; cEditButton.Visible = true; cSaveCloseButton.Visible = cSaveButton.Visible;
				}
				else
				{
					if (ID != null) {if (!cAdmDataCat96List.SelectByValue(ID,string.Empty,true)) {ID = null;}}
					if (ID == null)
					{
						cAdmDataCat96List_SelectedIndexChanged(sender, e);
					}
				}
			}
		}
		...
		private void SetColumnAuthority()
		{
			...
			{
				base.SetFoldBehavior(cRptwizCatId181, dtAuth.Rows[0], cRptwizCatId181P1, cRptwizCatId181Label, cRptwizCatId181P2, null, dtLabel.Rows[0], null, null, null);
				base.SetFoldBehavior(cRptwizTypId181, dtAuth.Rows[1], cRptwizTypId181P1, cRptwizTypId181Label, cRptwizTypId181P2, null, dtLabel.Rows[1], cRFVRptwizTypId181, null, null);
				SetRptwizTypId181(cRptwizTypId181,string.Empty);
				base.SetFoldBehavior(cRptwizCatName181, dtAuth.Rows[2], cRptwizCatName181P1, cRptwizCatName181Label, cRptwizCatName181P2, null, dtLabel.Rows[2], cRFVRptwizCatName181, null, null);
				base.SetFoldBehavior(cCatDescription181, dtAuth.Rows[3], cCatDescription181P1, cCatDescription181Label, cCatDescription181P2, cCatDescription181E, null, dtLabel.Rows[3], cRFVCatDescription181, null, null);
				...
				base.SetFoldBehavior(cTableId181, dtAuth.Rows[4], cTableId181P1, cTableId181Label, cTableId181P2, null, dtLabel.Rows[4], cRFVTableId181, null, null);
				SetTableId181(cTableId181,string.Empty);
				base.SetFoldBehavior(cSampleImage181, dtAuth.Rows[5], cSampleImage181P1, cSampleImage181Label, cSampleImage181P2, cSampleImage181Tgo, cSampleImage181Pan, dtLabel.Rows[5], null, null, null);
			}
			if ((cRptwizTypId181.Attributes["OnChange"] == null || cRptwizTypId181.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cRptwizTypId181.Visible && cRptwizTypId181.Enabled) {cRptwizTypId181.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if (cRptwizTypId181Search.Attributes["OnClick"] == null || cRptwizTypId181Search.Attributes["OnClick"].IndexOf("_bConfirm") < 0) {cRptwizTypId181Search.Attributes["OnClick"] += "document.getElementById('" + bConfirm.ClientID + "').value='N';";}
			if ((cRptwizCatName181.Attributes["OnChange"] == null || cRptwizCatName181.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cRptwizCatName181.Visible && !cRptwizCatName181.ReadOnly) {cRptwizCatName181.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cCatDescription181.Attributes["OnChange"] == null || cCatDescription181.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cCatDescription181.Visible && !cCatDescription181.ReadOnly) {cCatDescription181.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cTableId181.Attributes["OnChange"] == null || cTableId181.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cTableId181.Visible && cTableId181.Enabled) {cTableId181.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();document.getElementById('" + bConfirm.ClientID + "').value='N';";}
			if (cTableId181Search.Attributes["OnClick"] == null || cTableId181Search.Attributes["OnClick"].IndexOf("_bConfirm") < 0) {cTableId181Search.Attributes["OnClick"] += "document.getElementById('" + bConfirm.ClientID + "').value='N';";}
			if ((cSampleImage181.Attributes["OnChange"] == null || cSampleImage181.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cSampleImage181.Visible && cSampleImage181.Enabled) {cSampleImage181.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();document.getElementById('" + bConfirm.ClientID + "').value='N';";}
		}
		...
		private void ClearMaster(object sender, System.EventArgs e)
		{
			...
			if (dt.Rows[1]["ColVisible"].ToString() == "Y" && dt.Rows[1]["ColReadOnly"].ToString() != "Y") {cRptwizTypId181.ClearSearch();}
			if (dt.Rows[2]["ColVisible"].ToString() == "Y" && dt.Rows[2]["ColReadOnly"].ToString() != "Y") {cRptwizCatName181.Text = string.Empty;}
			if (dt.Rows[3]["ColVisible"].ToString() == "Y" && dt.Rows[3]["ColReadOnly"].ToString() != "Y") {cCatDescription181.Text = string.Empty;}
			if (dt.Rows[4]["ColVisible"].ToString() == "Y" && dt.Rows[4]["ColReadOnly"].ToString() != "Y") {cTableId181.ClearSearch();}
			if (dt.Rows[5]["ColVisible"].ToString() == "Y" && dt.Rows[5]["ColReadOnly"].ToString() != "Y") {cSampleImage181.ImageUrl = "~/images/DefaultImg.png"; }
			// *** Default Value (Folder) Web Rule starts here *** //
		}

		private void InitMaster(object sender, System.EventArgs e)
		{
			cRptwizCatId181.Text = string.Empty;
			cRptwizTypId181.ClearSearch();
			cRptwizCatName181.Text = string.Empty;
			cCatDescription181.Text = string.Empty;
			cTableId181.ClearSearch();
			cSampleImage181.ImageUrl = "~/images/DefaultImg.png";
			// *** Default Value (Folder) Web Rule starts here *** //
		}
		protected void cAdmDataCat96List_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmDataCat96List_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmDataCat96List_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			InitMaster(sender, e);      // Do this first to enable defaults for non-database columns.
			DataTable dt = null;
			try
			{
				dt = (new AdminSystem()).GetMstById("GetAdmDataCat96ById",cAdmDataCat96List.SelectedValue,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId));
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				if (dt.Rows.Count > 0)
				{
					cAdmDataCat96List.SetDdVisible();
					DataRow dr = dt.Rows[0];
					try {cRptwizCatId181.Text = RO.Common3.Utils.fmNumeric("0",dr["RptwizCatId181"].ToString(),base.LUser.Culture);} catch {cRptwizCatId181.Text = string.Empty;}
					SetRptwizTypId181(cRptwizTypId181,dr["RptwizTypId181"].ToString());
					cRptwizTypId181Search_Script();
					try {cRptwizCatName181.Text = dr["RptwizCatName181"].ToString();} catch {cRptwizCatName181.Text = string.Empty;}
			...
			DataTable dtAdmDataCatGrid = null;
			int filterId = 0; if (Utils.IsInt(cFilterId.SelectedValue)) { filterId = int.Parse(cFilterId.SelectedValue); }
			try
			{
				dtAdmDataCatGrid = (new AdminSystem()).GetDtlById(96,"GetAdmDataCat96DtlById",cAdmDataCat96List.SelectedValue,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,base.LImpr,base.LCurr);
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (!dtAdmDataCatGrid.Columns.Contains("_NewRow")) { dtAdmDataCatGrid.Columns.Add("_NewRow"); }
			Session[KEY_dtAdmDataCatGrid] = dtAdmDataCatGrid;
			cAdmDataCatGrid.EditIndex = -1;
			cAdmDataCatGridDataPager.PageSize = Int16.Parse(cPgSize.Text); GotoPage(0);
			if (Session[KEY_lastSortCol] != null)
			{
				cAdmDataCatGrid_OnSorting(sender, new ListViewSortEventArgs((string)Session[KEY_lastSortExp], SortDirection.Ascending));
			}
			else if (dtAdmDataCatGrid.Rows.Count <= 0 || (!((GetAuthRow().Rows[0]["AllowUpd"].ToString() == "N" || GetAuthRow().Rows[0]["ViewOnly"].ToString() == "G") && GetAuthRow().Rows[0]["AllowAdd"].ToString() == "N") && !(Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N") && dtAdmDataCatGrid.Rows.Count == 1))
			{
				cAdmDataCatGrid_DataBind(dtAdmDataCatGrid.DefaultView);
			}
			cFindButton_Click(sender, e);
			cNaviPanel.Visible = true; cImportPwdPanel.Visible = false; Session.Remove(KEY_scrImport);
			ScriptManager.GetCurrent(Parent.Page).SetFocus(cAdmDataCat96List.FocusID);
			ShowDirty(false); PanelTop.Update();
			cSampleImage181Fi.Attributes["onchange"] = "if('" + cRptwizCatId181.Text + "'==''){PopDialog('','Please save the record first before upload','');}else{sendFile(this.files[0],'" + GetUrlWithQSHash("UpLoad.aspx?key=" + cRptwizCatId181.Text + "&tbl=dbo.RptwizCat&knm=RptwizCatId&col=SampleImage&hgt=135&sys=" + base.LCurr.SystemId.ToString()) + "',refreshUploadCallback(this,'" + cSampleImage181.ClientID + "')); return false;}";
			cSampleImage181Del.Attributes["onclick"] = "sendFile('','" + GetUrlWithQSHash("UpLoad.aspx?del=true&key=" + cRptwizCatId181.Text + "&tbl=dbo.RptwizCat&knm=RptwizCatId&col=SampleImage&hgt=135&sys=" + base.LCurr.SystemId.ToString()) + "',refreshUploadCallback(this,'" + cSampleImage181.ClientID + "'));return false;";
			// *** List Selection (End of) Web Rule starts here *** //
		}

		protected void cRptwizTypId181_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cRptwizTypId181_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cRptwizTypId181_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			...
			TextBox pwd = null;
			if (cRptwizTypId181.Items.Count > 0 && cRptwizTypId181.DataSource != null)
			{
				DataView dv = (DataView) cRptwizTypId181.DataSource; dv.RowFilter = string.Empty;
				DataRowView dr = cRptwizTypId181.DataSetIndex >= 0 && cRptwizTypId181.DataSetIndex < dv.Count ? dv[cRptwizTypId181.DataSetIndex] : dv[0];
				cRptwizTypId181Search_Script();
		...
		protected void cTableId181_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cTableId181_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cTableId181_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			// *** On Change/ On Click Web Rule starts here *** //
			EnableValidators(true); // Do not remove; Need to reenable after postback, especially in the grid.
			TextBox pwd = null;
			if (cTableId181.Items.Count > 0 && cTableId181.DataSource != null)
			{
				DataView dv = (DataView) cTableId181.DataSource; dv.RowFilter = string.Empty;
				DataRowView dr = cTableId181.DataSetIndex >= 0 && cTableId181.DataSetIndex < dv.Count ? dv[cTableId181.DataSetIndex] : dv[0];
				cTableId181Search_Script();
			if (pwd != null) {ScriptManager.GetCurrent(Parent.Page).SetFocus(pwd.ClientID);} else {ScriptManager.GetCurrent(Parent.Page).SetFocus(SenderFocusId(sender));}
			}
		}

		protected void cSampleImage181_Click(object sender, System.Web.UI.ImageClickEventArgs e)
		{
			// *** On Change/ On Click Web Rule starts here *** //
			EnableValidators(true); // Do not remove; Need to reenable after postback, especially in the grid.
		}
		...
		public void cFindButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dv != null)
			{
				...
				if ((cc != null && cc.ID.Equals("cAdmDataCat96List")) || cAdmDataCatGrid.EditIndex < 0 || UpdateGridRow(cAdmDataCatGrid, new CommandEventArgs("Save", "")))
				{
					...
					if (GetCurrPageIndex() != (int)Session[KEY_currPageIndex] && (int)Session[KEY_currPageIndex] < GetTotalPages()) { GotoPage((int)Session[KEY_currPageIndex]); cAdmDataCatGrid_DataBind(dv);}
		...
		public void cInsRowButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dt != null && (ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")))
			{
				...
				Session[KEY_lastAddedRow] = 0; Session[KEY_dtAdmDataCatGrid] = dt; Session[KEY_currPageIndex] = 0; GotoPage(0);
				cAdmDataCatGrid_DataBind(dt.DefaultView);
				cAdmDataCatGrid_OnItemEditing(cAdmDataCatGrid, new ListViewEditEventArgs(0));
		...
		public void cPgSizeButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cAdmDataCatGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				...
				cAdmDataCatGridDataPager.PageSize = int.Parse(cPgSize.Text); GotoPage(0);
				if (dt.Rows.Count <= 0 || (!((GetAuthRow().Rows[0]["AllowUpd"].ToString() == "N" || GetAuthRow().Rows[0]["ViewOnly"].ToString() == "G") && GetAuthRow().Rows[0]["AllowAdd"].ToString() == "N") && !(Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N") && dt.Rows.Count == 1)) {cAdmDataCatGrid_DataBind(dt.DefaultView);} else {cFindButton_Click(sender, e);}
				try
				{
					(new AdminSystem()).UpdLastPageInfo(96, base.LUser.UsrId, cPgSize.Text, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		}
		...
		public void cShowImpButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cSchemaImage.Visible)
			{
				try
				{
					Session["ImportSchema"] = (new AdminSystem()).GetSchemaScrImp(96,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (cSchemaImage.Attributes["OnClick"] == null || cSchemaImage.Attributes["OnClick"].IndexOf("ImportSchema.aspx") < 0) {cSchemaImage.Attributes["OnClick"] += "SearchLink('ImportSchema.aspx?scm=S&key=96&csy=3','','',''); return false;";}
			}
			// *** System Button Click (after) Web Rule starts here *** //
		}
		...
		private void ScrImportDdl(DataView dvd, DataRowView drv, bool bComboBox, string PKey, string CKey, string CNam, int MaxLen, string MatchCd, bool bAllowNulls)
		{
			if (dvd != null)
			{
				...
				{
					if (string.IsNullOrEmpty(cAdmDataCat96List.SelectedValue))
					{
						dvd.RowFilter = "(" + PKey + " is null)";
					}
					else
					{
						dvd.RowFilter = "(" + PKey + " is null OR " + PKey + " = " + cAdmDataCat96List.SelectedValue + ")";
					}
				}
		...
		protected void ScrImport(bool bClear)
		{
			...
			DataTable dtAdmDataCatGrid = (DataTable)Session[KEY_dtAdmDataCatGrid];
			if (dti != null && dtAdmDataCatGrid != null)
			{
				if (bClear)
				{
					dtAdmDataCatGrid.Rows.Clear();
				}
				cFind.Text = string.Empty;
				int iExist = dtAdmDataCatGrid.Rows.Count;
				...
				foreach (DataRowView drv in dti.DefaultView)
				{
					...
					{
						DataTable dtd;
						dtd = jj==iExist ? null : (DataTable)Session[KEY_dtColumnId182];
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(96,"GetDdlColumnId3S1667",true,true,0,string.Empty,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
							Session[KEY_dtColumnId182] = dtd;
						}
						ScrImportDdl(dtd.DefaultView, drv, true, "RptwizCatId", "ColumnId182", "ColumnId182Text", 50, "2", false);
						dtd = (DataTable)Session[KEY_dtDisplayModeId182];
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(96,"GetDdlDisplayModeId3S1843",true,true,0,string.Empty,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
							Session[KEY_dtDisplayModeId182] = dtd;
						}
						ScrImportDdl(dtd.DefaultView, drv, false, "RptwizCatId", "DisplayModeId182", "DisplayModeId182Text", 20, "2", true);
					}
					jj = jj + 1;
				}
				iRow = dti.DefaultView.Count - iExist;
				if (!dti.Columns.Contains("_NewRow")) { dti.Columns.Add("_NewRow"); }
				Session[KEY_dtAdmDataCatGrid] = dti;
				cAdmDataCatGrid_DataBind(dti.DefaultView);
		...
		public void cImportButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cFNameO.Text != string.Empty && cWorkSheet.Items.Count > 0 && cWorkSheet.SelectedItem.Text != string.Empty && cStartRow.Text != string.Empty)
			{
				...
				try
				{
					if (cAdmDataCatGrid.EditIndex < 0 || UpdateGridRow(cAdmDataCatGrid, new CommandEventArgs("Save", "")))
					{
						Session[KEY_scrImport] = ImportFile(dtAdmDataCatGrid.Copy(),cFNameO.Text,cWorkSheet.SelectedItem.Text,cStartRow.Text,Config.PathTmpImport + cFName.Text);
					}
		...
		private void GotoPage(int pageNo)
		{
			if (pageNo >= 0 && pageNo < GetTotalPages())
			{
				if (cAdmDataCatGrid.EditIndex < 0 || UpdateGridRow(cAdmDataCatGrid, new CommandEventArgs("Save", "")))
				{
					try { cAdmDataCatGridDataPager.SetPageProperties(cAdmDataCatGridDataPager.PageSize * pageNo, cAdmDataCatGridDataPager.MaximumRows, false); cAdmDataCatGrid_DataBind(null); }
					catch
					{
						try { cAdmDataCatGridDataPager.SetPageProperties(0, cAdmDataCatGridDataPager.MaximumRows, false); cAdmDataCatGrid_DataBind(null); } catch {}
					}
				}
			}
		}

		protected void cAdmDataCatGrid_OnItemCommand(object sender, ListViewCommandEventArgs e)
		{
		}

		private void cAdmDataCatGrid_DataBind(DataView dvAdmDataCatGrid)
		{
			if (dvAdmDataCatGrid == null)
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmDataCatGrid];
				dvAdmDataCatGrid = dt != null ? dt.DefaultView : null;
			}
			if (dvAdmDataCatGrid != null)
			{
				LcAuth = GetAuthCol().AsEnumerable().ToDictionary<DataRow,string>(dr=>dr["ColName"].ToString());
				cAdmDataCatGrid.DataSource = dvAdmDataCatGrid;
				cAdmDataCatGrid.DataBind();
			...
			if (cFindFilter.Items.Count <= 0)
			{
				...
				{
					if (ii >= 6 && !string.IsNullOrEmpty(dr["ColumnHeader"].ToString()) && !string.IsNullOrEmpty(dr["TableId"].ToString()) && dtAuth.Rows[ii]["ColVisible"].ToString() == "Y")
					{
						li = new ListItem();
		...
		protected void cAdmDataCatGrid_OnSorting(object sender, ListViewSortEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmDataCatGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null)
			{
				if (cAdmDataCatGrid.EditIndex < 0 || UpdateGridRow(cAdmDataCatGrid, new CommandEventArgs("Save", "")))
				{
					...
					Session[KEY_dtAdmDataCatGrid] = dt;
					cAdmDataCatGrid_DataBind(dv);
				}
			}
		}

		protected void cAdmDataCatGrid_OnPagePropertiesChanging(object sender, PagePropertiesChangingEventArgs e)
		{
		    cAdmDataCatGridDataPager.SetPageProperties(e.StartRowIndex, e.MaximumRows, false);
		    DataTable dt = (DataTable)Session[KEY_dtAdmDataCatGrid];
		    DataView dv = dt != null ? dt.DefaultView : null;
			cAdmDataCatGrid_DataBind(dv); cAdmDataCatGrid.EditIndex = -1;
		}

		private void GridChkPgDirty(ListViewItem lvi)
		{
				WebControl cc = null; System.Web.UI.WebControls.Image ib = null;
				cc = ((WebControl)lvi.FindControl("cColumnId182"));
				...
				cc = ((WebControl)lvi.FindControl("cDisplayModeId182"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cColumnSize182"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cRowSize182"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cDdlKeyColNm182"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cDdlRefColNm182"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				cc = ((WebControl)lvi.FindControl("cRegClause182"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				ib = ((System.Web.UI.WebControls.Image)lvi.FindControl("cRegClause182E"));
				ib.Attributes["target_id"] = cc.ClientID;
				cc = ((WebControl)lvi.FindControl("cStoredProc182"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
				ib = ((System.Web.UI.WebControls.Image)lvi.FindControl("cStoredProc182E"));
				ib.Attributes["target_id"] = cc.ClientID;
		}

		protected void cAdmDataCatGrid_OnItemDataBound(object sender, ListViewItemEventArgs e)
		{
			...
			{
				isEditItem = true;
				base.SetGridEnabled(e.Item, GetAuthCol(), GetLabel(), 6);
				SetColumnId182((RoboCoder.WebControls.ComboBox)e.Item.FindControl("cColumnId182"),dvAdmDataCatGrid[e.Item.DataItemIndex]["ColumnId182"].ToString(),cTableId181.SelectedValue, e.Item);
				SetDisplayModeId182((DropDownList)e.Item.FindControl("cDisplayModeId182"),dvAdmDataCatGrid[e.Item.DataItemIndex]["DisplayModeId182"].ToString());
			...
			if (dtAuthRow != null)
			{
				...
				{
					cc = e.Item.FindControl("cAdmDataCatGridDelete"); if (cc != null) { cc.Visible = false; }
					cc = e.Item.FindControl("cAdmDataCatGridEdit"); if (cc != null) { cc.Visible = false; }
				}
				else
				{
			        HtmlTableRow tr = e.Item.FindControl("cAdmDataCatGridRow") as HtmlTableRow;
			        LinkButton lb = e.Item.FindControl("cAdmDataCatGridEdit") as LinkButton;
		...
		protected void cRptwizCatDtlId182hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "0") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "0"; Session[KEY_lastSortExp] = "RptwizCatDtlId182";Session[KEY_lastSortImg] = "cRptwizCatDtlId182hi";
			cAdmDataCatGrid_OnSorting(sender, new ListViewSortEventArgs("RptwizCatDtlId182", SortDirection.Ascending));
		}

		protected void cColumnId182hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "1") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "1"; Session[KEY_lastSortExp] = "ColumnId182Text";Session[KEY_lastSortImg] = "cColumnId182hi";
			cAdmDataCatGrid_OnSorting(sender, new ListViewSortEventArgs("ColumnId182Text", SortDirection.Ascending));
		}

		protected void cDisplayModeId182hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "2") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "2"; Session[KEY_lastSortExp] = "DisplayModeId182Text";Session[KEY_lastSortImg] = "cDisplayModeId182hi";
			cAdmDataCatGrid_OnSorting(sender, new ListViewSortEventArgs("DisplayModeId182Text", SortDirection.Ascending));
		}

		protected void cColumnSize182hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "3") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "3"; Session[KEY_lastSortExp] = "ColumnSize182";Session[KEY_lastSortImg] = "cColumnSize182hi";
			cAdmDataCatGrid_OnSorting(sender, new ListViewSortEventArgs("ColumnSize182", SortDirection.Ascending));
		}

		protected void cRowSize182hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "4") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "4"; Session[KEY_lastSortExp] = "RowSize182";Session[KEY_lastSortImg] = "cRowSize182hi";
			cAdmDataCatGrid_OnSorting(sender, new ListViewSortEventArgs("RowSize182", SortDirection.Ascending));
		}

		protected void cDdlKeyColNm182hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "5") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "5"; Session[KEY_lastSortExp] = "DdlKeyColNm182";Session[KEY_lastSortImg] = "cDdlKeyColNm182hi";
			cAdmDataCatGrid_OnSorting(sender, new ListViewSortEventArgs("DdlKeyColNm182", SortDirection.Ascending));
		}

		protected void cDdlRefColNm182hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "6") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "6"; Session[KEY_lastSortExp] = "DdlRefColNm182";Session[KEY_lastSortImg] = "cDdlRefColNm182hi";
			cAdmDataCatGrid_OnSorting(sender, new ListViewSortEventArgs("DdlRefColNm182", SortDirection.Ascending));
		}

		protected void cRegClause182hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "7") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "7"; Session[KEY_lastSortExp] = "RegClause182";Session[KEY_lastSortImg] = "cRegClause182hi";
			cAdmDataCatGrid_OnSorting(sender, new ListViewSortEventArgs("RegClause182", SortDirection.Ascending));
		}

		protected void cStoredProc182hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "8") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "8"; Session[KEY_lastSortExp] = "StoredProc182";Session[KEY_lastSortImg] = "cStoredProc182hi";
			cAdmDataCatGrid_OnSorting(sender, new ListViewSortEventArgs("StoredProc182", SortDirection.Ascending));
		}

		protected void cAdmDataCatGrid_OnItemEditing(object sender, ListViewEditEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmDataCatGrid];
			if ((ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")) && dt != null && dt.DefaultView.Count > GetDataItemIndex(e.NewEditIndex))
			{
				cAdmDataCatGrid.EditIndex = e.NewEditIndex;
				cAdmDataCatGrid_DataBind(null);
				Control wc = null;
				string idx = Page.Request["__EVENTARGUMENT"];
				if (!string.IsNullOrEmpty(idx))
				{
				    Control ctrlToFocus = cAdmDataCatGrid.EditItem.FindControl("c" + idx);
				    if (((WebControl)ctrlToFocus).Enabled) { wc = ctrlToFocus; }
				}
				if (wc == null) { wc = FindEditableControl(cAdmDataCatGrid.EditItem); }
				if (wc != null)
             {
             ...
             MkMobileLabel(cAdmDataCatGrid.EditItem);
		    }
		    else { e.Cancel = true; }
		}
		...
		{
		    Label ml = null;
		    ml = lvi.FindControl("cRptwizCatDtlId182ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(6); }
		    ml = lvi.FindControl("cColumnId182ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(7); }
		    ml = lvi.FindControl("cDisplayModeId182ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(8); }
		    ml = lvi.FindControl("cColumnSize182ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(9); }
		    ml = lvi.FindControl("cRowSize182ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(10); }
		    ml = lvi.FindControl("cDdlKeyColNm182ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(11); }
		    ml = lvi.FindControl("cDdlRefColNm182ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(12); }
		    ml = lvi.FindControl("cRegClause182ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(13); }
		    ml = lvi.FindControl("cStoredProc182ml") as Label;
		...
		protected void GridFill(ListViewItem lvi, DataTable dt, DataRow dr, bool bInsert)
		{
			...
			cbb = (RoboCoder.WebControls.ComboBox)lvi.FindControl("cColumnId182");
			if (cbb != null)
			{
				if (cbb.SelectedValue != string.Empty)
				{
					dr["ColumnId182"] = cbb.SelectedValue;
					dr["ColumnId182Text"] = cbb.SelectedText;
				}
				else
				{
					dr["ColumnId182"] = System.DBNull.Value;
					dr["ColumnId182Text"] = System.DBNull.Value;
				}
			}
			DropDownList ddl = null;
			ddl = (DropDownList)lvi.FindControl("cDisplayModeId182");
			if (ddl != null)
			{
				if (ddl.SelectedIndex >= 0 && ddl.SelectedValue.ToString() != string.Empty)
				{
					dr["DisplayModeId182"] = ddl.SelectedValue;
					dr["DisplayModeId182Text"] = ddl.SelectedItem.Text;
				}
				else
				{
					dr["DisplayModeId182"] = System.DBNull.Value;
					dr["DisplayModeId182Text"] = System.DBNull.Value;
				}
			}
			TextBox tb = null;
			tb = (TextBox)lvi.FindControl("cColumnSize182");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["ColumnSize182"] = tb.Text;} else {dr["ColumnSize182"] = System.DBNull.Value;}
			}
			tb = (TextBox)lvi.FindControl("cRowSize182");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["RowSize182"] = tb.Text;} else {dr["RowSize182"] = System.DBNull.Value;}
			}
			tb = (TextBox)lvi.FindControl("cDdlKeyColNm182");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["DdlKeyColNm182"] = tb.Text;} else {dr["DdlKeyColNm182"] = System.DBNull.Value;}
			}
			tb = (TextBox)lvi.FindControl("cDdlRefColNm182");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["DdlRefColNm182"] = tb.Text;} else {dr["DdlRefColNm182"] = System.DBNull.Value;}
			}
			tb = (TextBox)lvi.FindControl("cRegClause182");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["RegClause182"] = tb.Text;} else {dr["RegClause182"] = System.DBNull.Value;}
			}
			tb = (TextBox)lvi.FindControl("cStoredProc182");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["StoredProc182"] = tb.Text;} else {dr["StoredProc182"] = System.DBNull.Value;}
			}
		...
		protected void cAdmDataCatGrid_OnItemUpdating(object sender, ListViewUpdateEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmDataCatGrid];
			if (dt != null && dt.Rows.Count > 0)
			{
				DataRow dr = dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row;
			    GridFill(cAdmDataCatGrid.EditItem, dt, dr, false);
			    dr["_NewRow"] = "N";
			    if (dt.Columns.Contains("_SortOrder")) dr["_SortOrder"] = null;
			}
		}

		protected void cAdmDataCatGrid_OnItemCanceling(object sender, ListViewCancelEventArgs e)
		{
			PanelTop.Update();
			DataTable dt = (DataTable)Session[KEY_dtAdmDataCatGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			...
			{
				cAdmDataCatGrid_OnItemDeleting(sender, new ListViewDeleteEventArgs(e.ItemIndex));
			}
			if (dt.Columns.Contains("_SortOrder")) dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row["_SortOrder"] = null;
			cAdmDataCatGrid.EditIndex = -1; cAdmDataCatGrid_DataBind(null);
		}
		...
		protected void cDeleteAllButton_Click(object sender, System.EventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmDataCatGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null && (cAdmDataCatGrid.EditIndex < 0 || UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				GotoPage(0);
				while (dv.Count > 0) {dv.Delete(0);}
				Session[KEY_dtAdmDataCatGrid] = dt;
				cAdmDataCatGrid.EditIndex = -1; cAdmDataCatGrid_DataBind(dv);
			}
		}

		protected void cAdmDataCatGrid_OnItemDeleting(object sender, ListViewDeleteEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmDataCatGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			DataRow dr = dv != null ? dv[GetDataItemIndex(e.ItemIndex)].Row : null;
			if (dv != null && (cAdmDataCatGrid.EditIndex == e.ItemIndex || UpdateGridRow(sender, new CommandEventArgs("Save",""))))
			{
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cRptwizCatId181.Text == string.Empty) { cClearButton_Click(sender, new EventArgs()); ShowDirty(false); PanelTop.Update();}
			else { PopAdmDataCat96List(sender, e, false, cRptwizCatId181.Text); }
			// *** System Button Click (After) Web Rule starts here *** //
		}
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmDataCat96List); PopAdmDataCat96List(sender, e, false, null);
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			if (ValidPage() && (cAdmDataCatGrid.EditIndex < 0 || UpdateGridRow(cAdmDataCatGrid, new CommandEventArgs("Save", ""))))
			{
				...
				string ftr = string.Empty;
				if (dv.RowFilter != string.Empty) { ftr = dv.RowFilter; dv.RowFilter = string.Empty; }
				AdmDataCat96 ds = PrepAdmDataCatData(dv,cRptwizCatId181.Text == string.Empty);
				if (ftr != string.Empty) {dv.RowFilter = ftr;}
				if (!string.IsNullOrEmpty(cAdmDataCat96List.SelectedValue) && cRptwizCatId181.Text != cAdmDataCat96List.SelectedValue)
				{
					PreMsgPopup("Primary key cannot be changed");
					return rtn;
				}
				if (string.IsNullOrEmpty(cAdmDataCat96List.SelectedValue))	// Add
				{
					if (ds != null)
					{
					...
					{
						cAdmDataCat96List.ClearSearch(); Session.Remove(KEY_dtAdmDataCat96List);
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmDataCat96List(sender, e, false, pid);
					...
					if (bValid7)
					{
						...
						ShowDirty(false); PopAdmDataCat96List(sender, e, false, ds.Tables["AdmDataCat"].Rows[0]["RptwizCatId181"]);
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cRptwizCatId181.Text != string.Empty)
			{
				AdmDataCat96 ds = PrepAdmDataCatData(null,false);
				try
				{
					if (ds != null && (new AdminSystem()).DelData(96,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc))
					{
						cAdmDataCat96List.ClearSearch(); Session.Remove(KEY_dtAdmDataCat96List);
						ShowDirty(false); PopAdmDataCat96List(sender, e, false, null);
		...
		private AdmDataCat96 PrepAdmDataCatData(DataView dv, bool bAdd)
		{
			...
			if (bAdd) { dr["RptwizCatId181"] = string.Empty; } else { dr["RptwizCatId181"] = cRptwizCatId181.Text; }
			drType["RptwizCatId181"] = "Numeric"; drDisp["RptwizCatId181"] = "TextBox";
			try {dr["RptwizTypId181"] = cRptwizTypId181.SelectedValue;} catch {}
			drType["RptwizTypId181"] = "Numeric"; drDisp["RptwizTypId181"] = "AutoComplete";
			try {dr["RptwizCatName181"] = cRptwizCatName181.Text.Trim();} catch {}
			drType["RptwizCatName181"] = "VarWChar"; drDisp["RptwizCatName181"] = "TextBox";
			try {dr["CatDescription181"] = cCatDescription181.Text;} catch {}
			drType["CatDescription181"] = "VarWChar"; drDisp["CatDescription181"] = "MultiLine";
			try {dr["TableId181"] = cTableId181.SelectedValue;} catch {}
			...
			if (dv != null)
			{
				...
				if (bAdd)
				{
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmDataCatAdd"].Rows.Add(MakeColRow(ds.Tables["AdmDataCatAdd"].NewRow(), drv, true));
					}
				}
				else
				{
					...
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmDataCatUpd"].Rows.Add(MakeColRow(ds.Tables["AdmDataCatUpd"].NewRow(), drv, false));
					}
					...
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmDataCatDel"].Rows.Add(MakeColRow(ds.Tables["AdmDataCatDel"].NewRow(), drv, false));
					}
		...
		public bool CanAct(char typ) { return CanAct(typ, GetAuthRow(), cAdmDataCat96List.SelectedValue.ToString()); }
		...
		private bool ValidPage()
		{
			...
			DataTable dt = null;
			dt = (DataTable)Session[KEY_dtRptwizTypId181];
			if (dt != null && dt.Rows.Count <= 0)
			{
				bErrNow.Value = "Y"; PreMsgPopup("Value is expected for 'RptwizTypId', please investigate."); return false;
			}
			dt = (DataTable)Session[KEY_dtTableId181];
			if (dt != null && dt.Rows.Count <= 0)
			{
				bErrNow.Value = "Y"; PreMsgPopup("Value is expected for 'TableId', please investigate."); return false;
			}
			return true;
		}
		...
		{
			dr["RptwizCatId181"] = cRptwizCatId181.Text;
			DataTable dtAuth = GetAuthCol();
			if (dtAuth != null)
			{
				dr["RptwizCatDtlId182"] = drv["RptwizCatDtlId182"].ToString().Trim();
				dr["ColumnId182"] = drv["ColumnId182"];
				dr["DisplayModeId182"] = drv["DisplayModeId182"];
				if (bAdd && dtAuth.Rows[8]["ColReadOnly"].ToString() == "Y" && dr["DisplayModeId182"].ToString() == string.Empty) {dr["DisplayModeId182"] = System.DBNull.Value;}
				dr["ColumnSize182"] = drv["ColumnSize182"].ToString().Trim();
				if (bAdd && dtAuth.Rows[9]["ColReadOnly"].ToString() == "Y" && dr["ColumnSize182"].ToString() == string.Empty) {dr["ColumnSize182"] = System.DBNull.Value;}
				dr["RowSize182"] = drv["RowSize182"].ToString().Trim();
				if (bAdd && dtAuth.Rows[10]["ColReadOnly"].ToString() == "Y" && dr["RowSize182"].ToString() == string.Empty) {dr["RowSize182"] = System.DBNull.Value;}
				dr["DdlKeyColNm182"] = drv["DdlKeyColNm182"].ToString().Trim();
				if (bAdd && dtAuth.Rows[11]["ColReadOnly"].ToString() == "Y" && dr["DdlKeyColNm182"].ToString() == string.Empty) {dr["DdlKeyColNm182"] = System.DBNull.Value;}
				dr["DdlRefColNm182"] = drv["DdlRefColNm182"].ToString().Trim();
				if (bAdd && dtAuth.Rows[12]["ColReadOnly"].ToString() == "Y" && dr["DdlRefColNm182"].ToString() == string.Empty) {dr["DdlRefColNm182"] = System.DBNull.Value;}
				dr["RegClause182"] = drv["RegClause182"];
				if (bAdd && dtAuth.Rows[13]["ColReadOnly"].ToString() == "Y" && dr["RegClause182"].ToString() == string.Empty) {dr["RegClause182"] = System.DBNull.Value;}
				dr["StoredProc182"] = drv["StoredProc182"];
				if (bAdd && dtAuth.Rows[14]["ColReadOnly"].ToString() == "Y" && dr["StoredProc182"].ToString() == string.Empty) {dr["StoredProc182"] = System.DBNull.Value;}
			}
			return dr;
		}
		...
		{
			if (cAdmDataCatGrid.EditIndex > -1)
			{
				cAdmDataCatGrid_OnItemUpdating(sender, new ListViewUpdateEventArgs(cAdmDataCatGrid.EditIndex));
			}
			return true;
		}

		protected void cAdmDataCatGrid_OnPreRender(object sender, System.EventArgs e)
		{
			System.Web.UI.WebControls.Image hi = null;
			hi = (System.Web.UI.WebControls.Image)cAdmDataCatGrid.FindControl("cRptwizCatDtlId182hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmDataCatGrid.FindControl("cColumnId182hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmDataCatGrid.FindControl("cDisplayModeId182hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmDataCatGrid.FindControl("cColumnSize182hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmDataCatGrid.FindControl("cRowSize182hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmDataCatGrid.FindControl("cDdlKeyColNm182hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmDataCatGrid.FindControl("cDdlRefColNm182hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmDataCatGrid.FindControl("cRegClause182hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmDataCatGrid.FindControl("cStoredProc182hi"); if (hi != null) { hi.Visible = false; }
			if (Session[KEY_lastSortImg] != null)
			{
				hi = (System.Web.UI.WebControls.Image)cAdmDataCatGrid.FindControl((string)Session[KEY_lastSortImg]);
				if (hi != null) { hi.ImageUrl = Utils.AddTilde((string)Session[KEY_lastSortUrl]); hi.Visible = true; }
			}
			IgnoreHeaderConfirm((LinkButton)cAdmDataCatGrid.FindControl("cRptwizCatDtlId182hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmDataCatGrid.FindControl("cColumnId182hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmDataCatGrid.FindControl("cDisplayModeId182hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmDataCatGrid.FindControl("cColumnSize182hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmDataCatGrid.FindControl("cRowSize182hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmDataCatGrid.FindControl("cDdlKeyColNm182hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmDataCatGrid.FindControl("cDdlRefColNm182hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmDataCatGrid.FindControl("cRegClause182hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmDataCatGrid.FindControl("cStoredProc182hl"));
		}

		private string GetButtonId(ListViewItem lvi)
		{
			string ButtonID = String.Empty;
			Control c = lvi.FindControl("cAdmDataCatGridEdit");
		...
		protected void cAdmDataCatGrid_OnLayoutCreated(object sender, EventArgs e)
		{
			...
			if (dtAuthRow != null)
			{
				...
				{
		            lb = cAdmDataCatGrid.FindControl("cDeleteAllButton") as LinkButton; if (lb != null) { lb.Visible = false; }
				}
			}
		    // footer:
		    Label gc = null;
		    gc = cAdmDataCatGrid.FindControl("cRptwizCatDtlId182fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(6); }
		    gc = cAdmDataCatGrid.FindControl("cColumnId182fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(7); }
		    gc = cAdmDataCatGrid.FindControl("cDisplayModeId182fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(8); }
		    gc = cAdmDataCatGrid.FindControl("cColumnSize182fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(9); }
		    gc = cAdmDataCatGrid.FindControl("cRowSize182fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(10); }
		    gc = cAdmDataCatGrid.FindControl("cDdlKeyColNm182fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(11); }
		    gc = cAdmDataCatGrid.FindControl("cDdlRefColNm182fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(12); }
		    gc = cAdmDataCatGrid.FindControl("cRegClause182fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(13); }
		    gc = cAdmDataCatGrid.FindControl("cStoredProc182fl") as Label;
		...
		private void PreMsgPopup(string msg) { PreMsgPopup(msg,cAdmDataCat96List,null); }

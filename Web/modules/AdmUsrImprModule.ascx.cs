...
namespace RO.Web
{
	public partial class AdmUsrImprModule : RO.Web.ModuleBase
	{
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_66";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_66";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_66";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_66";
		private const string KEY_dtLabel = "Cache:dtLabel3_66";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_66";
		private const string KEY_dtScrCri = "Cache:dtScrCri3_66";
		private const string KEY_dsScrCriVal = "Cache:dsScrCriVal3_66";

		private const string KEY_dtUsrId95 = "Cache:dtUsrId95";
		private const string KEY_dtImprUsrId95 = "Cache:dtImprUsrId95";
		private const string KEY_dtInputBy95 = "Cache:dtInputBy95";
		private const string KEY_dtModifiedBy95 = "Cache:dtModifiedBy95";
		private const string KEY_dtTestCulture95 = "Cache:dtTestCulture95";

		private const string KEY_dtSystems = "Cache:dtSystems3_66";
		private const string KEY_sysConnectionString = "Cache:sysConnectionString3_66";
		private const string KEY_dtAdmUsrImpr66List = "Cache:dtAdmUsrImpr66List";
		private const string KEY_bNewVisible = "Cache:bNewVisible3_66";
		private const string KEY_bNewSaveVisible = "Cache:bNewSaveVisible3_66";
		private const string KEY_bCopyVisible = "Cache:bCopyVisible3_66";
		private const string KEY_bCopySaveVisible = "Cache:bCopySaveVisible3_66";
		private const string KEY_bDeleteVisible = "Cache:bDeleteVisible3_66";
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_66";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_66";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_66";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				DataTable dtMenuAccess = (new MenuSystem()).GetMenu(base.LUser.CultureId, 3, base.LImpr, LcSysConnString, LcAppPw,66, null, null);
				...
				Session.Remove(KEY_dtAdmUsrImpr66List);
				...
				Session.Remove(KEY_dtUsrId95);
				Session.Remove(KEY_dtImprUsrId95);
				Session.Remove(KEY_dtInputBy95);
				Session.Remove(KEY_dtModifiedBy95);
				Session.Remove(KEY_dtTestCulture95);
				SetButtonHlp();
				GetSystems();
				SetColumnAuthority();
				GetGlobalFilter();
				...
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 66, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmUsrImpr66List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=66&typ=N&sys=3','','',''); return false;"; }
		...
		private void SetSystem(byte SystemId)
		{
			...
			try
			{
				...
				if ((Config.DeployType == "DEV" || row["dbAppDatabase"].ToString() == base.CPrj.EntityCode + "View") && !(base.CPrj.EntityCode != "RO" && row["SysProgram"].ToString() == "Y") && (new AdminSystem()).IsRegenNeeded(string.Empty,66,0,0,LcSysConnString,LcAppPw))
				{
					(new GenScreensSystem()).CreateProgram(66, "User Impersonation", row["dbAppDatabase"].ToString(), base.CPrj, base.CSrc, base.CTar, LcAppConnString, LcAppPw);
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(66,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(66,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(66,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(66,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,66,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(66,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(66,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(66,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		protected DataTable GetAuthRow()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthRow(66,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				try
				{
					try {dv = new DataView((new AdminSystem()).GetExp(66,"GetExpAdmUsrImpr66","Y",null,null,filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
					catch {dv = new DataView((new AdminSystem()).GetExp(66,"GetExpAdmUsrImpr66","N",null,null,filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(66,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					{
						if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["UsrImprId95"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["UsrId95"].ToString() + (char)9 + drv["UsrId95Text"].ToString() + (char)9);}
						if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(drv["ImprUsrId95"].ToString() + (char)9 + drv["ImprUsrId95Text"].ToString() + (char)9);}
						if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["FailedAttempt1"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append(drv["InputBy95"].ToString() + (char)9 + drv["InputBy95Text"].ToString() + (char)9);}
						if (dtAu.Rows[7]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmShortDateTimeUTC(drv["InputOn95"].ToString(),base.LUser.Culture,CurrTimeZoneInfo()) + (char)9);}
						if (dtAu.Rows[8]["ColExport"].ToString() == "Y") {sb.Append(drv["ModifiedBy95"].ToString() + (char)9 + drv["ModifiedBy95Text"].ToString() + (char)9);}
						if (dtAu.Rows[9]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmShortDateTimeUTC(drv["ModifiedOn95"].ToString(),base.LUser.Culture,CurrTimeZoneInfo()) + (char)9);}
						if (dtAu.Rows[10]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["TestCulture95"].ToString().Replace("\"","\"\"") + "\"" + (char)9 + "\"" + drv["TestCulture95Text"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[11]["ColExport"].ToString() == "Y") {sb.Append(base.fxCurrency(drv["TestCurrency95"].ToString(),drv["TestCulture95"].ToString(),drv["TestCulture95"].ToString()) + (char)9);}
						if (dtAu.Rows[12]["ColExport"].ToString() == "Y") {sb.Append(drv["SignOff95"].ToString() + (char)9);}
						sb.Append(Environment.NewLine);
					}
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmUsrImpr.csv"; Session["ExportStr"] = (Config.ExportExcelCSV ? "sep=\t\n": "") + sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmUsrImpr.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private string GetRtfGrid(DataView dv)
		{
			...
			try
			{
				...
				foreach (DataRowView drv in dv)
				{
					...
					sb.Append("\r\n");
					sb.Append(@"{");
					if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["UsrImprId95"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["UsrId95Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(drv["ImprUsrId95Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["FailedAttempt1"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append(drv["InputBy95Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[7]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmShortDateTimeUTC(drv["InputOn95"].ToString(),base.LUser.Culture,CurrTimeZoneInfo()) + @"\cell ");}
					if (dtAu.Rows[8]["ColExport"].ToString() == "Y") {sb.Append(drv["ModifiedBy95Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[9]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmShortDateTimeUTC(drv["ModifiedOn95"].ToString(),base.LUser.Culture,CurrTimeZoneInfo()) + @"\cell ");}
					if (dtAu.Rows[10]["ColExport"].ToString() == "Y") {sb.Append(drv["TestCulture95Text"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[11]["ColExport"].ToString() == "Y") {sb.Append(base.fxCurrency(drv["TestCurrency95"].ToString(),drv["TestCulture95"].ToString(),drv["TestCulture95"].ToString()) + @"\cell ");}
					if (dtAu.Rows[12]["ColExport"].ToString() == "Y") {sb.Append(drv["SignOff95"].ToString() + @"\cell ");}
					sb.Append(@"}");
					sb.Append("\r\n");
					sb.Append(@"\pard \ql \li0\ri0\widctlpar\intbl\aspalpha\aspnum\adjustright\rin0\lin0 {");
					sb.Append(@"\insrsid2981395 \trowd \irow0\irowband0\lastrow \ts15\trgaph108\trleft-108\trbrdrt\brdrs\brdrw10 \trbrdrl\brdrs\brdrw10 \trbrdrb\brdrs\brdrw10 \trbrdrr\brdrs\brdrw10 \trbrdrh\brdrs\brdrw10 \trbrdrv\brdrs\brdrw10 ");
		...
		public void cUsrId95Search_Click(object sender, System.Web.UI.ImageClickEventArgs e)
		{
			RoboCoder.WebControls.ComboBox cc = cUsrId95; cc.SetTbVisible(); Session["CtrlAdmUsr"] = cc.FocusID;
		}

		public void cUsrId95Search_Script()
		{
				ImageButton ib = cUsrId95Search;
				if (ib != null)
				{
			    	TextBox pp = cUsrImprId95;
					RoboCoder.WebControls.ComboBox cc = cUsrId95;
					string ss = "&dsp=ComboBox"; if (cSystem.Visible) {ss = ss + "&sys=" + cSystemId.SelectedValue;} else {ss = ss + "&sys=3";}
					ib.Attributes["onclick"] = "SearchLink('AdmUsr.aspx?col=UsrId&enb=N" + ss + "&typ=N" + "','" + cc.FocusID + "','',''); return false;";
				}
		}

		public void cImprUsrId95Search_Click(object sender, System.Web.UI.ImageClickEventArgs e)
		{
			RoboCoder.WebControls.ComboBox cc = cImprUsrId95; cc.SetTbVisible(); Session["CtrlAdmUsr"] = cc.FocusID;
		}

		public void cImprUsrId95Search_Script()
		{
				ImageButton ib = cImprUsrId95Search;
				if (ib != null)
				{
			    	TextBox pp = cUsrImprId95;
					RoboCoder.WebControls.ComboBox cc = cImprUsrId95;
					string ss = "&dsp=ComboBox"; if (cSystem.Visible) {ss = ss + "&sys=" + cSystemId.SelectedValue;} else {ss = ss + "&sys=3";}
					ib.Attributes["onclick"] = "SearchLink('AdmUsr.aspx?col=UsrId&enb=N" + ss + "&typ=N" + "','" + cc.FocusID + "','',''); return false;";
				}
		}
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 66, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			...
			PopAdmUsrImpr66List(sender, e, false, null);
		}
			...
			try
			{
				foreach (DataRowView drv in dvCri)
				{
					...
					if (drv["DisplayName"].ToString() == "ComboBox")
					{
						...
						(new AdminSystem()).MkGetScreenIn("66", drv["ScreenCriId"].ToString(), "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), LcAppDb, LcDesDb, drv["MultiDesignDb"].ToString(), LcSysConnString, LcAppPw);
						DataView dv = new DataView((new AdminSystem()).GetScreenIn("66", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), (new AdminSystem()).CountScrCri(drv["ScreenCriId"].ToString(), drv["MultiDesignDb"].ToString(), drv["MultiDesignDb"].ToString() == "N" ? LcSysConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)), drv["RequiredValid"].ToString(), 0, string.Empty, drv["DisplayMode"].ToString() != "AutoListBox", val, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)));
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("66", LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmUsrImpr66" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmUsrImpr66" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			foreach (DataRowView drv in dvCri)
			{
			    if (drv["DisplayName"].ToString() == "ListBox")
			    {
					...
					if (cListBox != null)
					{
						...
						int TotalChoiceCnt = new DataView((new AdminSystem()).GetScreenIn("66", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), CriCnt, drv["RequiredValid"].ToString(), 0, string.Empty, true, string.Empty, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId))).Count;
						string selectedValues = string.Join(",", cListBox.Items.Cast<ListItem>().Where(x => x.Selected).Select(x => "'" + x.Value + "'").ToArray());
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("66", "AdmUsrImpr66", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		protected void cbUsrId95(object sender, System.EventArgs e)
		{
			SetUsrId95((RoboCoder.WebControls.ComboBox)sender,string.Empty);
		}

		private void SetUsrId95(RoboCoder.WebControls.ComboBox ddl, string keyId)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(66,"GetDdlUsrId3S986",true,false,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
					if (dv.Table.Columns.Contains("UsrImprId"))
					{
						context["pMKeyColID"] = cAdmUsrImpr66List.ClientID;
						context["pMKeyCol"] = "UsrImprId";
						string ss = "(UsrImprId is null";
						if (string.IsNullOrEmpty(cAdmUsrImpr66List.SelectedValue)) {ss = ss + ")";} else {ss = ss + " OR UsrImprId = " + cAdmUsrImpr66List.SelectedValue + ")";}
						dv.RowFilter = ss;
					}
					ddl.DataSource = dv; Session[KEY_dtUsrId95] = dv.Table;
				    try { ddl.SelectByValue(keyId,string.Empty,true); } catch { try { ddl.SelectedIndex = 0; } catch { } }
				}
			}
		}

		protected void cbImprUsrId95(object sender, System.EventArgs e)
		{
			SetImprUsrId95((RoboCoder.WebControls.ComboBox)sender,string.Empty);
		}

		private void SetImprUsrId95(RoboCoder.WebControls.ComboBox ddl, string keyId)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(66,"GetDdlImprUsrId3S1002",true,false,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
					...
					ddl.DataSource = dv; Session[KEY_dtImprUsrId95] = dv.Table;
				    try { ddl.SelectByValue(keyId,string.Empty,true); } catch { try { ddl.SelectedIndex = 0; } catch { } }
				}
			}
		}

		private void SetInputBy95(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtInputBy95];
			...
			if (ddl != null)
			{
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(66,"GetDdlInputBy3S3360",true,bAll,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					if (dv.Table.Columns.Contains("UsrImprId"))
					{
						ss = "(UsrImprId is null";
						if (string.IsNullOrEmpty(cAdmUsrImpr66List.SelectedValue)) {ss = ss + ")";} else {ss = ss + " OR UsrImprId = " + cAdmUsrImpr66List.SelectedValue + ")";}
					}
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "InputBy95 is not NULL"; }
					dv.RowFilter = ss;
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(66,"GetDdlInputBy3S3360",true,true,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtInputBy95] = dv.Table;
				}
			}
		}

		private void SetModifiedBy95(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtModifiedBy95];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (ddl != null)
			{
				string ss = string.Empty;
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(66,"GetDdlModifiedBy3S3362",true,bAll,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					...
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "ModifiedBy95 is not NULL"; }
					dv.RowFilter = ss;
					ddl.DataSource = dv; ddl.DataBind();
					li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					else if (!bFirst)
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(66,"GetDdlModifiedBy3S3362",true,true,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtModifiedBy95] = dv.Table;
				}
			}
		}

		protected void cbTestCulture95(object sender, System.EventArgs e)
		{
			SetTestCulture95((RoboCoder.WebControls.ComboBox)sender,string.Empty);
		}

		private void SetTestCulture95(RoboCoder.WebControls.ComboBox ddl, string keyId)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(66,"GetDdlTestCulture3S3076",true,false,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
					...
					ddl.DataSource = dv; Session[KEY_dtTestCulture95] = dv.Table;
				    try { ddl.SelectByValue(keyId,string.Empty,true); } catch { try { ddl.SelectedIndex = 0; } catch { } }
				}
			}
		}

		private DataView GetAdmUsrImpr66List()
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmUsrImpr66List];
			if (dt == null)
			{
				CheckAuthentication(false);
				int filterId = 0;
				...
				try
				{
					try {dt = (new AdminSystem()).GetLis(66,"GetLisAdmUsrImpr66",true,"Y",0,null,null,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
					catch {dt = (new AdminSystem()).GetLis(66,"GetLisAdmUsrImpr66",true,"N",0,null,null,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				}
		...
		private void PopAdmUsrImpr66List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			DataView dv = GetAdmUsrImpr66List();
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			context["isSys"] = "Y";
			context["conn"] = string.Empty;
			cAdmUsrImpr66List.AutoCompleteUrl = "AutoComplete.aspx/LisSuggests";
			cAdmUsrImpr66List.DataContext = context;
			if (dv.Table == null) return;
			cAdmUsrImpr66List.DataSource = dv;
			cAdmUsrImpr66List.Visible = true;
			if (cAdmUsrImpr66List.Items.Count <= 0) {cAdmUsrImpr66List.Visible = false; cAdmUsrImpr66List_SelectedIndexChanged(sender,e);}
			else
			{
				if (bPageLoad && !string.IsNullOrEmpty(Request.QueryString["key"]) && bUseCri.Value == string.Empty)
				{
					try {cAdmUsrImpr66List.SelectByValue(Request.QueryString["key"],string.Empty,true);} catch {cAdmUsrImpr66List.Items[0].Selected = true; cAdmUsrImpr66List_SelectedIndexChanged(sender, e);}
				    cScreenSearch.Visible = false; cSystem.Visible = false; cEditButton.Visible = true; cSaveCloseButton.Visible = cSaveButton.Visible;
				}
				else
				{
					if (ID != null) {if (!cAdmUsrImpr66List.SelectByValue(ID,string.Empty,true)) {ID = null;}}
					if (ID == null)
					{
						cAdmUsrImpr66List_SelectedIndexChanged(sender, e);
					}
				}
			}
		}
		...
		private void SetColumnAuthority()
		{
			...
			{
				base.SetFoldBehavior(cUsrImprId95, dtAuth.Rows[0], cUsrImprId95P1, cUsrImprId95Label, cUsrImprId95P2, null, dtLabel.Rows[0], null, null, null);
				base.SetFoldBehavior(cUsrId95, dtAuth.Rows[1], cUsrId95P1, cUsrId95Label, cUsrId95P2, null, dtLabel.Rows[1], cRFVUsrId95, null, null);
				SetUsrId95(cUsrId95,string.Empty);
				base.SetFoldBehavior(cUPicMed1, dtAuth.Rows[2], cUPicMed1P1, cUPicMed1Label, cUPicMed1P2, null, dtLabel.Rows[2], null, null, null);
				base.SetFoldBehavior(cImprUsrId95, dtAuth.Rows[3], cImprUsrId95P1, cImprUsrId95Label, cImprUsrId95P2, null, dtLabel.Rows[3], cRFVImprUsrId95, null, null);
				SetImprUsrId95(cImprUsrId95,string.Empty);
				base.SetFoldBehavior(cIPicMed1, dtAuth.Rows[4], cIPicMed1P1, cIPicMed1Label, cIPicMed1P2, null, dtLabel.Rows[4], null, null, null);
				base.SetFoldBehavior(cFailedAttempt1, dtAuth.Rows[5], cFailedAttempt1P1, cFailedAttempt1Label, cFailedAttempt1P2, null, dtLabel.Rows[5], null, null, null);
				base.SetFoldBehavior(cInputBy95, dtAuth.Rows[6], cInputBy95P1, cInputBy95Label, cInputBy95P2, null, dtLabel.Rows[6], null, null, null);
				base.SetFoldBehavior(cInputOn95, dtAuth.Rows[7], cInputOn95P1, cInputOn95Label, cInputOn95P2, null, dtLabel.Rows[7], null, null, null);
				base.SetFoldBehavior(cModifiedBy95, dtAuth.Rows[8], cModifiedBy95P1, cModifiedBy95Label, cModifiedBy95P2, null, dtLabel.Rows[8], null, null, null);
				base.SetFoldBehavior(cModifiedOn95, dtAuth.Rows[9], cModifiedOn95P1, cModifiedOn95Label, cModifiedOn95P2, null, dtLabel.Rows[9], null, null, null);
				base.SetFoldBehavior(cTestCulture95, dtAuth.Rows[10], cTestCulture95P1, cTestCulture95Label, cTestCulture95P2, null, dtLabel.Rows[10], null, null, null);
				SetTestCulture95(cTestCulture95,string.Empty);
				base.SetFoldBehavior(cTestCurrency95, dtAuth.Rows[11], cTestCurrency95P1, cTestCurrency95Label, cTestCurrency95P2, null, dtLabel.Rows[11], null, null, null);
				base.SetFoldBehavior(cSignOff95, dtAuth.Rows[12], cSignOff95P1, cSignOff95Label, cSignOff95P2, null, dtLabel.Rows[12], null, null, null);
cSignOff95.HintMessage = ColumnWatermark(12);
			}
			if ((cUsrId95.Attributes["OnChange"] == null || cUsrId95.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cUsrId95.Visible && cUsrId95.Enabled) {cUsrId95.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();document.getElementById('" + bConfirm.ClientID + "').value='N';";}
			if (cUsrId95Search.Attributes["OnClick"] == null || cUsrId95Search.Attributes["OnClick"].IndexOf("_bConfirm") < 0) {cUsrId95Search.Attributes["OnClick"] += "document.getElementById('" + bConfirm.ClientID + "').value='N';";}
			if ((cUPicMed1.Attributes["OnChange"] == null || cUPicMed1.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cUPicMed1.Visible && cUPicMed1.Enabled) {cUPicMed1.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();document.getElementById('" + bConfirm.ClientID + "').value='N';";}
			if ((cImprUsrId95.Attributes["OnChange"] == null || cImprUsrId95.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cImprUsrId95.Visible && cImprUsrId95.Enabled) {cImprUsrId95.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();document.getElementById('" + bConfirm.ClientID + "').value='N';";}
			if (cImprUsrId95Search.Attributes["OnClick"] == null || cImprUsrId95Search.Attributes["OnClick"].IndexOf("_bConfirm") < 0) {cImprUsrId95Search.Attributes["OnClick"] += "document.getElementById('" + bConfirm.ClientID + "').value='N';";}
			if ((cIPicMed1.Attributes["OnChange"] == null || cIPicMed1.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cIPicMed1.Visible && cIPicMed1.Enabled) {cIPicMed1.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();document.getElementById('" + bConfirm.ClientID + "').value='N';";}
			...
			if ((cInputBy95.Attributes["OnChange"] == null || cInputBy95.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cInputBy95.Visible && cInputBy95.Enabled) {cInputBy95.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cInputOn95.Attributes["OnChange"] == null || cInputOn95.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cInputOn95.Visible && !cInputOn95.ReadOnly) {cInputOn95.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cModifiedBy95.Attributes["OnChange"] == null || cModifiedBy95.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cModifiedBy95.Visible && cModifiedBy95.Enabled) {cModifiedBy95.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cModifiedOn95.Attributes["OnChange"] == null || cModifiedOn95.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cModifiedOn95.Visible && !cModifiedOn95.ReadOnly) {cModifiedOn95.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cTestCulture95.Attributes["OnChange"] == null || cTestCulture95.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cTestCulture95.Visible && cTestCulture95.Enabled) {cTestCulture95.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cTestCurrency95.Attributes["OnChange"] == null || cTestCurrency95.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cTestCurrency95.Visible && !cTestCurrency95.ReadOnly) {cTestCurrency95.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cSignOff95.Attributes["OnChange"] == null || cSignOff95.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cSignOff95.Visible && cSignOff95.Enabled) {cSignOff95.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
		}
		...
		private void ClearMaster(object sender, System.EventArgs e)
		{
			...
			if (dt.Rows[1]["ColVisible"].ToString() == "Y" && dt.Rows[1]["ColReadOnly"].ToString() != "Y") {cUsrId95.ClearSearch();}
			if (dt.Rows[3]["ColVisible"].ToString() == "Y" && dt.Rows[3]["ColReadOnly"].ToString() != "Y") {cImprUsrId95.ClearSearch();}
			if (dt.Rows[6]["ColVisible"].ToString() == "Y" && dt.Rows[6]["ColReadOnly"].ToString() != "Y") {SetInputBy95(cInputBy95,string.Empty);}
			if (dt.Rows[7]["ColVisible"].ToString() == "Y" && dt.Rows[7]["ColReadOnly"].ToString() != "Y") {cInputOn95.Text = string.Empty;}
			if (dt.Rows[8]["ColVisible"].ToString() == "Y" && dt.Rows[8]["ColReadOnly"].ToString() != "Y") {SetModifiedBy95(cModifiedBy95,string.Empty);}
			if (dt.Rows[9]["ColVisible"].ToString() == "Y" && dt.Rows[9]["ColReadOnly"].ToString() != "Y") {cModifiedOn95.Text = string.Empty;}
			if (dt.Rows[10]["ColVisible"].ToString() == "Y" && dt.Rows[10]["ColReadOnly"].ToString() != "Y") {cTestCulture95.ClearSearch();}
			if (dt.Rows[11]["ColVisible"].ToString() == "Y" && dt.Rows[11]["ColReadOnly"].ToString() != "Y") {cTestCurrency95.Text = string.Empty;}
			if (dt.Rows[12]["ColVisible"].ToString() == "Y" && dt.Rows[12]["ColReadOnly"].ToString() != "Y") {cSignOff95.ClearSignature();}
			// *** Default Value (Folder) Web Rule starts here *** //
		}

		private void InitMaster(object sender, System.EventArgs e)
		{
			cUsrImprId95.Text = string.Empty;
			cUsrId95.ClearSearch();

			cImprUsrId95.ClearSearch();


			SetInputBy95(cInputBy95,string.Empty);
			cInputOn95.Text = string.Empty;
			SetModifiedBy95(cModifiedBy95,string.Empty);
			cModifiedOn95.Text = string.Empty;
			cTestCulture95.ClearSearch();
			cTestCurrency95.Text = string.Empty;
			cSignOff95.ClearSignature();
			// *** Default Value (Folder) Web Rule starts here *** //
		}
		protected void cAdmUsrImpr66List_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmUsrImpr66List_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmUsrImpr66List_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			InitMaster(sender, e);      // Do this first to enable defaults for non-database columns.
			DataTable dt = null;
			try
			{
				dt = (new AdminSystem()).GetMstById("GetAdmUsrImpr66ById",cAdmUsrImpr66List.SelectedValue,null,null);
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				if (dt.Rows.Count > 0)
				{
					cAdmUsrImpr66List.SetDdVisible();
					DataRow dr = dt.Rows[0];
					try {cUsrImprId95.Text = RO.Common3.Utils.fmNumeric("0",dr["UsrImprId95"].ToString(),base.LUser.Culture);} catch {cUsrImprId95.Text = string.Empty;}
					SetUsrId95(cUsrId95,dr["UsrId95"].ToString()); cUsrId95_SelectedIndexChanged(cUsrId95, new EventArgs());
					cUsrId95Search_Script();
					SetImprUsrId95(cImprUsrId95,dr["ImprUsrId95"].ToString()); cImprUsrId95_SelectedIndexChanged(cImprUsrId95, new EventArgs());
					cImprUsrId95Search_Script();
					SetInputBy95(cInputBy95,dr["InputBy95"].ToString());
					try {cInputOn95.Text = RO.Common3.Utils.fmShortDateTimeUTC(dr["InputOn95"].ToString(),base.LUser.Culture,CurrTimeZoneInfo());} catch {cInputOn95.Text = string.Empty;}
					SetModifiedBy95(cModifiedBy95,dr["ModifiedBy95"].ToString());
					try {cModifiedOn95.Text = RO.Common3.Utils.fmShortDateTimeUTC(dr["ModifiedOn95"].ToString(),base.LUser.Culture,CurrTimeZoneInfo());} catch {cModifiedOn95.Text = string.Empty;}
					SetTestCulture95(cTestCulture95,dr["TestCulture95"].ToString());
					try {cTestCurrency95.Text = base.fxCurrency(dr["TestCurrency95"].ToString(),dr["TestCulture95"].ToString(),dr["TestCulture95"].ToString());} catch {cTestCurrency95.Text = string.Empty;}
					try {cSignOff95.SignatureData = dr["SignOff95"] as byte[];} catch {cSignOff95.SignatureData = null;}
				}
			}
			cButPanel.DataBind();
			ScriptManager.GetCurrent(Parent.Page).SetFocus(cAdmUsrImpr66List.FocusID);
			ShowDirty(false); PanelTop.Update();
			// *** List Selection (End of) Web Rule starts here *** //
		}

		protected void cUsrId95_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cUsrId95_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cUsrId95_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			...
			TextBox pwd = null;
			if (cUsrId95.Items.Count > 0 && cUsrId95.DataSource != null)
			{
		...
		protected void cUPicMed1_Click(object sender, System.Web.UI.ImageClickEventArgs e)
		{
			// *** On Change/ On Click Web Rule starts here *** //
			EnableValidators(true); // Do not remove; Need to reenable after postback, especially in the grid.
		}

		protected void cImprUsrId95_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cImprUsrId95_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cImprUsrId95_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			// *** On Change/ On Click Web Rule starts here *** //
			EnableValidators(true); // Do not remove; Need to reenable after postback, especially in the grid.
			TextBox pwd = null;
			if (cImprUsrId95.Items.Count > 0 && cImprUsrId95.DataSource != null)
			{
		...
		protected void cIPicMed1_Click(object sender, System.Web.UI.ImageClickEventArgs e)
		{
			// *** On Change/ On Click Web Rule starts here *** //
			EnableValidators(true); // Do not remove; Need to reenable after postback, especially in the grid.
		}
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cUsrImprId95.Text == string.Empty) { cClearButton_Click(sender, new EventArgs()); ShowDirty(false); PanelTop.Update();}
			else { PopAdmUsrImpr66List(sender, e, false, cUsrImprId95.Text); }
			// *** System Button Click (After) Web Rule starts here *** //
		}
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmUsrImpr66List); PopAdmUsrImpr66List(sender, e, false, null);
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			if (ValidPage())
			{
				AdmUsrImpr66 ds = PrepAdmUsrImprData(null,cUsrImprId95.Text == string.Empty);
				if (!string.IsNullOrEmpty(cAdmUsrImpr66List.SelectedValue) && cUsrImprId95.Text != cAdmUsrImpr66List.SelectedValue)
				{
					PreMsgPopup("Primary key cannot be changed");
					return rtn;
				}
				if (string.IsNullOrEmpty(cAdmUsrImpr66List.SelectedValue))	// Add
				{
					if (ds != null)
					{
					...
					{
						cAdmUsrImpr66List.ClearSearch(); Session.Remove(KEY_dtAdmUsrImpr66List);
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmUsrImpr66List(sender, e, false, pid);
					...
					{
						cAdmUsrImpr66List.ClearSearch(); Session.Remove(KEY_dtAdmUsrImpr66List);
						ShowDirty(false); PopAdmUsrImpr66List(sender, e, false, ds.Tables["AdmUsrImpr"].Rows[0]["UsrImprId95"]);
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cUsrImprId95.Text != string.Empty)
			{
				AdmUsrImpr66 ds = PrepAdmUsrImprData(null,false);
				try
				{
					if (ds != null && (new AdminSystem()).DelData(66,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc))
					{
						cAdmUsrImpr66List.ClearSearch(); Session.Remove(KEY_dtAdmUsrImpr66List);
						ShowDirty(false); PopAdmUsrImpr66List(sender, e, false, null);
		...
		private AdmUsrImpr66 PrepAdmUsrImprData(DataView dv, bool bAdd)
		{
			...
			if (bAdd) { dr["UsrImprId95"] = string.Empty; } else { dr["UsrImprId95"] = cUsrImprId95.Text; }
			drType["UsrImprId95"] = "Numeric"; drDisp["UsrImprId95"] = "TextBox";
			try {dr["UsrId95"] = cUsrId95.SelectedValue;} catch {}
			drType["UsrId95"] = "Numeric"; drDisp["UsrId95"] = "AutoComplete";
			try {dr["ImprUsrId95"] = cImprUsrId95.SelectedValue;} catch {}
			drType["ImprUsrId95"] = "Numeric"; drDisp["ImprUsrId95"] = "AutoComplete";
			try { dr["InputBy95"] = cInputBy95.SelectedValue == string.Empty && bAdd ? base.LUser.UsrId.ToString().ToString(): cInputBy95.SelectedValue;} catch {};
			drType["InputBy95"] = "Numeric"; drDisp["InputBy95"] = "DropDownList";
			try {dr["InputOn95"] = base.ToIntDateTime(cInputOn95.Text,true,true);} catch {}
			drType["InputOn95"] = "DBTimeStamp"; drDisp["InputOn95"] = "ShortDateTimeUTC";
			try {dr["ModifiedBy95"] = base.LUser.UsrId.ToString();} catch {};
			drType["ModifiedBy95"] = "Numeric"; drDisp["ModifiedBy95"] = "DropDownList";
			try {dr["TestCulture95"] = cTestCulture95.SelectedValue;} catch {}
			drType["TestCulture95"] = "VarChar"; drDisp["TestCulture95"] = "AutoComplete";
			try {dr["TestCurrency95"] = Decimal.Parse(cTestCurrency95.Text.Trim(), System.Globalization.NumberStyles.Currency, new System.Globalization.CultureInfo(cTestCulture95.SelectedValue)).ToString();} catch {}
			drType["TestCurrency95"] = "Currency"; drDisp["TestCurrency95"] = "Currency";
			try {dr["SignOff95"] = Convert.ToBase64String(cSignOff95.SignatureData);} catch {}
		...
		public bool CanAct(char typ) { return CanAct(typ, GetAuthRow(), cAdmUsrImpr66List.SelectedValue.ToString()); }
		...
		private bool ValidPage()
		{
			...
			DataTable dt = null;
			dt = (DataTable)Session[KEY_dtUsrId95];
			if (dt != null && dt.Rows.Count <= 0)
			{
				bErrNow.Value = "Y"; PreMsgPopup("Value is expected for 'UsrId', please investigate."); return false;
			}
			dt = (DataTable)Session[KEY_dtImprUsrId95];
			if (dt != null && dt.Rows.Count <= 0)
			{
				bErrNow.Value = "Y"; PreMsgPopup("Value is expected for 'ImprUsrId', please investigate."); return false;
			}
			return true;
		}
		...
		private void PreMsgPopup(string msg) { PreMsgPopup(msg,cAdmUsrImpr66List,null); }

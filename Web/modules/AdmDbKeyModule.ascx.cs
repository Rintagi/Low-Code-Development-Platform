...
{
	public class AdmDbKey15 : DataSet
	{
		public AdmDbKey15()
		{
			this.Tables.Add(MakeColumns(new DataTable("AdmDbKey")));
			this.DataSetName = "AdmDbKey15";
			this.Namespace = "http://Rintagi.com/DataSet/AdmDbKey15";
		}

		private DataTable MakeColumns(DataTable dt)
		{
			DataColumnCollection columns = dt.Columns;
			columns.Add("KeyId20", typeof(string));
			columns.Add("KeyName20", typeof(string));
			columns.Add("TableId20", typeof(string));
			columns.Add("ColumnId20", typeof(string));
			columns.Add("RefTableId20", typeof(string));
			columns.Add("RefColumnId20", typeof(string));
			return dt;
		}
	}
}
...
namespace RO.Web
{
	public partial class AdmDbKeyModule : RO.Web.ModuleBase
	{
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_15";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_15";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_15";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_15";
		private const string KEY_dtLabel = "Cache:dtLabel3_15";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_15";
		private const string KEY_dtScrCri = "Cache:dtScrCri3_15";
		private const string KEY_dsScrCriVal = "Cache:dsScrCriVal3_15";

		private const string KEY_dtTableId20 = "Cache:dtTableId20";
		private const string KEY_dtColumnId20 = "Cache:dtColumnId20";
		private const string KEY_dtRefTableId20 = "Cache:dtRefTableId20";
		private const string KEY_dtRefColumnId20 = "Cache:dtRefColumnId20";

		private const string KEY_dtSystems = "Cache:dtSystems3_15";
		private const string KEY_sysConnectionString = "Cache:sysConnectionString3_15";
		private const string KEY_dtAdmDbKey15List = "Cache:dtAdmDbKey15List";
		private const string KEY_bNewVisible = "Cache:bNewVisible3_15";
		private const string KEY_bNewSaveVisible = "Cache:bNewSaveVisible3_15";
		private const string KEY_bCopyVisible = "Cache:bCopyVisible3_15";
		private const string KEY_bCopySaveVisible = "Cache:bCopySaveVisible3_15";
		private const string KEY_bDeleteVisible = "Cache:bDeleteVisible3_15";
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_15";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_15";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_15";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				DataTable dtMenuAccess = (new MenuSystem()).GetMenu(base.LUser.CultureId, 3, base.LImpr, LcSysConnString, LcAppPw,15, null, null);
				...
				Session.Remove(KEY_dtAdmDbKey15List);
				...
				Session.Remove(KEY_dtTableId20);
				Session.Remove(KEY_dtColumnId20);
				Session.Remove(KEY_dtRefTableId20);
				Session.Remove(KEY_dtRefColumnId20);
				SetButtonHlp();
				GetSystems();
				SetColumnAuthority();
				GetGlobalFilter();
				...
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 15, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmDbKey15List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=15&typ=N&sys=3','','',''); return false;"; }
		...
		private void SetSystem(byte SystemId)
		{
			...
			try
			{
				...
				if ((Config.DeployType == "DEV" || row["dbAppDatabase"].ToString() == base.CPrj.EntityCode + "View") && !(base.CPrj.EntityCode != "RO" && row["SysProgram"].ToString() == "Y") && (new AdminSystem()).IsRegenNeeded(string.Empty,15,0,0,LcSysConnString,LcAppPw))
				{
					(new GenScreensSystem()).CreateProgram(15, "Foreign Key", row["dbAppDatabase"].ToString(), base.CPrj, base.CSrc, base.CTar, LcAppConnString, LcAppPw);
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(15,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(15,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(15,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(15,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,15,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(15,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(15,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(15,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		protected DataTable GetAuthRow()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthRow(15,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				try
				{
					try {dv = new DataView((new AdminSystem()).GetExp(15,"GetExpAdmDbKey15","Y",(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
					catch {dv = new DataView((new AdminSystem()).GetExp(15,"GetExpAdmDbKey15","N",(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(15,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					{
						if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["KeyId20"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["KeyName20"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(drv["TableId20"].ToString() + (char)9 + drv["TableId20Text"].ToString() + (char)9);}
						if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(drv["ColumnId20"].ToString() + (char)9 + drv["ColumnId20Text"].ToString() + (char)9);}
						if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append(drv["RefTableId20"].ToString() + (char)9 + drv["RefTableId20Text"].ToString() + (char)9);}
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmDbKey.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private string GetRtfGrid(DataView dv)
		{
			...
			try
			{
				...
				foreach (DataRowView drv in dv)
				{
					...
					sb.Append("\r\n");
					sb.Append(@"{");
					if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["KeyId20"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["KeyName20"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(drv["TableId20Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(drv["ColumnId20Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append(drv["RefTableId20Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append(drv["RefColumnId20Text"].ToString() + @"\cell ");}
					sb.Append(@"}");
					sb.Append("\r\n");
					sb.Append(@"\pard \ql \li0\ri0\widctlpar\intbl\aspalpha\aspnum\adjustright\rin0\lin0 {");
					sb.Append(@"\insrsid2981395 \trowd \irow0\irowband0\lastrow \ts15\trgaph108\trleft-108\trbrdrt\brdrs\brdrw10 \trbrdrl\brdrs\brdrw10 \trbrdrb\brdrs\brdrw10 \trbrdrr\brdrs\brdrw10 \trbrdrh\brdrs\brdrw10 \trbrdrv\brdrs\brdrw10 ");
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 15, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			...
			UpdCriteria(true);
			cAdmDbKey15List.ClearSearch(); Session.Remove(KEY_dtAdmDbKey15List);
			PopAdmDbKey15List(sender, e, false, null);
		}
			...
			try
			{
				foreach (DataRowView drv in dvCri)
				{
					...
					if (drv["DisplayName"].ToString() == "ComboBox")
					{
						...
						(new AdminSystem()).MkGetScreenIn("15", drv["ScreenCriId"].ToString(), "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), LcAppDb, LcDesDb, drv["MultiDesignDb"].ToString(), LcSysConnString, LcAppPw);
						DataView dv = new DataView((new AdminSystem()).GetScreenIn("15", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), (new AdminSystem()).CountScrCri(drv["ScreenCriId"].ToString(), drv["MultiDesignDb"].ToString(), drv["MultiDesignDb"].ToString() == "N" ? LcSysConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)), drv["RequiredValid"].ToString(), 0, string.Empty, drv["DisplayMode"].ToString() != "AutoListBox", val, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)));
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("15", LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmDbKey15" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmDbKey15" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			foreach (DataRowView drv in dvCri)
			{
			    if (drv["DisplayName"].ToString() == "ListBox")
			    {
					...
					if (cListBox != null)
					{
						...
						int TotalChoiceCnt = new DataView((new AdminSystem()).GetScreenIn("15", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), CriCnt, drv["RequiredValid"].ToString(), 0, string.Empty, true, string.Empty, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId))).Count;
						string selectedValues = string.Join(",", cListBox.Items.Cast<ListItem>().Where(x => x.Selected).Select(x => "'" + x.Value + "'").ToArray());
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("15", "AdmDbKey15", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		protected void cbTableId20(object sender, System.EventArgs e)
		{
			SetTableId20((RoboCoder.WebControls.ComboBox)sender,string.Empty);
		}

		private void SetTableId20(RoboCoder.WebControls.ComboBox ddl, string keyId)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			context["method"] = "GetDdlTableId3S166";
			context["addnew"] = "Y";
			context["mKey"] = "TableId20";
			context["mVal"] = "TableId20Text";
			context["mTip"] = "TableId20Text";
			context["mImg"] = "TableId20Text";
			context["ssd"] = Request.QueryString["ssd"];
			context["scr"] = "15";
			context["csy"] = "3";
			context["filter"] = Utils.IsInt(cFilterId.SelectedValue)? cFilterId.SelectedValue : "0";
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(15,"GetDdlTableId3S166",true,false,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
					if (dv.Table.Columns.Contains("KeyId"))
					{
						context["pMKeyColID"] = cAdmDbKey15List.ClientID;
						context["pMKeyCol"] = "KeyId";
						string ss = "(KeyId is null";
						if (string.IsNullOrEmpty(cAdmDbKey15List.SelectedValue)) {ss = ss + ")";} else {ss = ss + " OR KeyId = " + cAdmDbKey15List.SelectedValue + ")";}
						dv.RowFilter = ss;
					}
					ddl.DataSource = dv; Session[KEY_dtTableId20] = dv.Table;
				    try { ddl.SelectByValue(keyId,string.Empty,true); } catch { try { ddl.SelectedIndex = 0; } catch { } }
				}
			}
		}

		protected void cbColumnId20(object sender, System.EventArgs e)
		{
			SetColumnId20((RoboCoder.WebControls.ComboBox)sender,string.Empty,cTableId20.SelectedValue);
		}

		private void SetColumnId20(RoboCoder.WebControls.ComboBox ddl, string keyId, string filtr)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			context["method"] = "GetDdlColumnId3S167";
			context["addnew"] = "Y";
			context["mKey"] = "ColumnId20";
			context["mVal"] = "ColumnId20Text";
			context["mTip"] = "ColumnId20Text";
			context["mImg"] = "ColumnId20Text";
			context["ssd"] = Request.QueryString["ssd"];
			context["scr"] = "15";
			context["csy"] = "3";
			context["filter"] = Utils.IsInt(cFilterId.SelectedValue)? cFilterId.SelectedValue : "0";
			context["isSys"] = "N";
			context["conn"] = KEY_sysConnectionString;
			context["refColCID"] = cTableId20.ClientID;
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(15,"GetDdlColumnId3S167",true,false,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
					...
					ddl.DataSource = dv; Session[KEY_dtColumnId20] = dv.Table;
		...
		protected void cbRefTableId20(object sender, System.EventArgs e)
		{
			SetRefTableId20((RoboCoder.WebControls.ComboBox)sender,string.Empty);
		}

		private void SetRefTableId20(RoboCoder.WebControls.ComboBox ddl, string keyId)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			context["method"] = "GetDdlRefTableId3S168";
			context["addnew"] = "Y";
			context["mKey"] = "RefTableId20";
			context["mVal"] = "RefTableId20Text";
			context["mTip"] = "RefTableId20Text";
			context["mImg"] = "RefTableId20Text";
			context["ssd"] = Request.QueryString["ssd"];
			context["scr"] = "15";
			context["csy"] = "3";
			context["filter"] = Utils.IsInt(cFilterId.SelectedValue)? cFilterId.SelectedValue : "0";
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(15,"GetDdlRefTableId3S168",true,false,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
					...
					ddl.DataSource = dv; Session[KEY_dtRefTableId20] = dv.Table;
				    try { ddl.SelectByValue(keyId,string.Empty,true); } catch { try { ddl.SelectedIndex = 0; } catch { } }
				}
			}
		}

		protected void cbRefColumnId20(object sender, System.EventArgs e)
		{
			SetRefColumnId20((RoboCoder.WebControls.ComboBox)sender,string.Empty,cRefTableId20.SelectedValue);
		}

		private void SetRefColumnId20(RoboCoder.WebControls.ComboBox ddl, string keyId, string filtr)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			context["method"] = "GetDdlRefColumnId3S169";
			context["addnew"] = "Y";
			context["mKey"] = "RefColumnId20";
			context["mVal"] = "RefColumnId20Text";
			context["mTip"] = "RefColumnId20Text";
			context["mImg"] = "RefColumnId20Text";
			context["ssd"] = Request.QueryString["ssd"];
			context["scr"] = "15";
			context["csy"] = "3";
			context["filter"] = Utils.IsInt(cFilterId.SelectedValue)? cFilterId.SelectedValue : "0";
			context["isSys"] = "N";
			context["conn"] = KEY_sysConnectionString;
			context["refColCID"] = cRefTableId20.ClientID;
			context["refCol"] = "TableId";
			context["refColDataType"] = "Int";
			ddl.AutoCompleteUrl = "AutoComplete.aspx/DdlSuggests";
			ddl.DataContext = context;
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(15,"GetDdlRefColumnId3S169",true,false,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
					...
					ddl.DataSource = dv; Session[KEY_dtRefColumnId20] = dv.Table;
				    try { ddl.SelectByValue(keyId,filtr != string.Empty ? "(TableId is null OR TableId = " + filtr + ")" : string.Empty,true); } catch { try { ddl.SelectedIndex = 0; } catch { } }
				}
			}
		}

		private DataView GetAdmDbKey15List()
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmDbKey15List];
			if (dt == null)
			{
				CheckAuthentication(false);
				int filterId = 0;
				...
				try
				{
					try {dt = (new AdminSystem()).GetLis(15,"GetLisAdmDbKey15",true,"Y",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
					catch {dt = (new AdminSystem()).GetLis(15,"GetLisAdmDbKey15",true,"N",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				}
		...
		private void PopAdmDbKey15List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			DataView dv = GetAdmDbKey15List();
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			context["method"] = "GetLisAdmDbKey15";
			context["mKey"] = "KeyId20";
			context["mVal"] = "KeyId20Text";
			context["mTip"] = "KeyId20Text";
			context["mImg"] = "KeyId20Text";
			context["ssd"] = Request.QueryString["ssd"];
			context["scr"] = "15";
			context["csy"] = "3";
			...
			context["isSys"] = "N";
			context["conn"] = KEY_sysConnectionString;
			cAdmDbKey15List.AutoCompleteUrl = "AutoComplete.aspx/LisSuggests";
			cAdmDbKey15List.DataContext = context;
			if (dv.Table == null) return;
			cAdmDbKey15List.DataSource = dv;
			cAdmDbKey15List.Visible = true;
			if (cAdmDbKey15List.Items.Count <= 0) {cAdmDbKey15List.Visible = false; cAdmDbKey15List_SelectedIndexChanged(sender,e);}
			else
			{
				if (bPageLoad && !string.IsNullOrEmpty(Request.QueryString["key"]) && bUseCri.Value == string.Empty)
				{
					try {cAdmDbKey15List.SelectByValue(Request.QueryString["key"],string.Empty,true);} catch {cAdmDbKey15List.Items[0].Selected = true; cAdmDbKey15List_SelectedIndexChanged(sender, e);}
				    cScreenSearch.Visible = false; cSystem.Visible = false; cEditButton.Visible = true; cSaveCloseButton.Visible = cSaveButton.Visible;
				}
				else
				{
					if (ID != null) {if (!cAdmDbKey15List.SelectByValue(ID,string.Empty,true)) {ID = null;}}
					if (ID == null)
					{
						cAdmDbKey15List_SelectedIndexChanged(sender, e);
					}
				}
			}
		}
		...
		private void SetColumnAuthority()
		{
			...
			{
				base.SetFoldBehavior(cKeyId20, dtAuth.Rows[0], cKeyId20P1, cKeyId20Label, cKeyId20P2, null, dtLabel.Rows[0], null, null, null);
				base.SetFoldBehavior(cKeyName20, dtAuth.Rows[1], cKeyName20P1, cKeyName20Label, cKeyName20P2, null, dtLabel.Rows[1], cRFVKeyName20, null, null);
				base.SetFoldBehavior(cTableId20, dtAuth.Rows[2], cTableId20P1, cTableId20Label, cTableId20P2, null, dtLabel.Rows[2], cRFVTableId20, null, null);
				SetTableId20(cTableId20,string.Empty);
				base.SetFoldBehavior(cColumnId20, dtAuth.Rows[3], cColumnId20P1, cColumnId20Label, cColumnId20P2, null, dtLabel.Rows[3], cRFVColumnId20, null, null);
				SetColumnId20(cColumnId20,string.Empty,string.Empty);
				base.SetFoldBehavior(cRefTableId20, dtAuth.Rows[4], cRefTableId20P1, cRefTableId20Label, cRefTableId20P2, null, dtLabel.Rows[4], null, null, null);
				SetRefTableId20(cRefTableId20,string.Empty);
				base.SetFoldBehavior(cRefColumnId20, dtAuth.Rows[5], cRefColumnId20P1, cRefColumnId20Label, cRefColumnId20P2, null, dtLabel.Rows[5], cRFVRefColumnId20, null, null);
				SetRefColumnId20(cRefColumnId20,string.Empty,string.Empty);
			}
			if ((cKeyName20.Attributes["OnChange"] == null || cKeyName20.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cKeyName20.Visible && !cKeyName20.ReadOnly) {cKeyName20.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cTableId20.Attributes["OnChange"] == null || cTableId20.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cTableId20.Visible && cTableId20.Enabled) {cTableId20.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();document.getElementById('" + bConfirm.ClientID + "').value='N';";}
			if ((cColumnId20.Attributes["OnChange"] == null || cColumnId20.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cColumnId20.Visible && cColumnId20.Enabled) {cColumnId20.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cRefTableId20.Attributes["OnChange"] == null || cRefTableId20.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cRefTableId20.Visible && cRefTableId20.Enabled) {cRefTableId20.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();document.getElementById('" + bConfirm.ClientID + "').value='N';";}
			if ((cRefColumnId20.Attributes["OnChange"] == null || cRefColumnId20.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cRefColumnId20.Visible && cRefColumnId20.Enabled) {cRefColumnId20.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
		}
		...
		private void ClearMaster(object sender, System.EventArgs e)
		{
			...
			if (dt.Rows[1]["ColVisible"].ToString() == "Y" && dt.Rows[1]["ColReadOnly"].ToString() != "Y") {cKeyName20.Text = string.Empty;}
			if (dt.Rows[2]["ColVisible"].ToString() == "Y" && dt.Rows[2]["ColReadOnly"].ToString() != "Y") {cTableId20.ClearSearch();}
			if (dt.Rows[3]["ColVisible"].ToString() == "Y" && dt.Rows[3]["ColReadOnly"].ToString() != "Y") {cColumnId20.ClearSearch();}
			if (dt.Rows[4]["ColVisible"].ToString() == "Y" && dt.Rows[4]["ColReadOnly"].ToString() != "Y") {cRefTableId20.ClearSearch();}
			if (dt.Rows[5]["ColVisible"].ToString() == "Y" && dt.Rows[5]["ColReadOnly"].ToString() != "Y") {cRefColumnId20.ClearSearch();}
			// *** Default Value (Folder) Web Rule starts here *** //
		}

		private void InitMaster(object sender, System.EventArgs e)
		{
			cKeyId20.Text = string.Empty;
			cKeyName20.Text = string.Empty;
			cTableId20.ClearSearch();
			cColumnId20.ClearSearch();
			cRefTableId20.ClearSearch();
			cRefColumnId20.ClearSearch();
			// *** Default Value (Folder) Web Rule starts here *** //
		}
		protected void cAdmDbKey15List_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmDbKey15List_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmDbKey15List_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			InitMaster(sender, e);      // Do this first to enable defaults for non-database columns.
			DataTable dt = null;
			try
			{
				dt = (new AdminSystem()).GetMstById("GetAdmDbKey15ById",cAdmDbKey15List.SelectedValue,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId));
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				if (dt.Rows.Count > 0)
				{
					cAdmDbKey15List.SetDdVisible();
					DataRow dr = dt.Rows[0];
					try {cKeyId20.Text = RO.Common3.Utils.fmNumeric("0",dr["KeyId20"].ToString(),base.LUser.Culture);} catch {cKeyId20.Text = string.Empty;}
					try {cKeyName20.Text = dr["KeyName20"].ToString();} catch {cKeyName20.Text = string.Empty;}
					SetTableId20(cTableId20,dr["TableId20"].ToString()); cTableId20_SelectedIndexChanged(cTableId20, new EventArgs());
					SetColumnId20(cColumnId20,dr["ColumnId20"].ToString(),cTableId20.SelectedValue);
					SetRefTableId20(cRefTableId20,dr["RefTableId20"].ToString()); cRefTableId20_SelectedIndexChanged(cRefTableId20, new EventArgs());
					SetRefColumnId20(cRefColumnId20,dr["RefColumnId20"].ToString(),cRefTableId20.SelectedValue);
				}
			}
			cButPanel.DataBind();
			ScriptManager.GetCurrent(Parent.Page).SetFocus(cAdmDbKey15List.FocusID);
			ShowDirty(false); PanelTop.Update();
			// *** List Selection (End of) Web Rule starts here *** //
		}

		protected void cTableId20_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cTableId20_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cTableId20_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			...
			TextBox pwd = null;
			if (cTableId20.Items.Count > 0 && cTableId20.DataSource != null)
			{
				DataView dv = (DataView) cTableId20.DataSource; dv.RowFilter = string.Empty;
				DataRowView dr = cTableId20.DataSetIndex >= 0 && cTableId20.DataSetIndex < dv.Count ? dv[cTableId20.DataSetIndex] : dv[0];
				if (cColumnId20.Mode!="A") cColumnId20.ClearSearch();				else SetColumnId20(cColumnId20,cColumnId20.SelectedValue,cTableId20.SelectedValue);
		...
		protected void cRefTableId20_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cRefTableId20_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cRefTableId20_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			// *** On Change/ On Click Web Rule starts here *** //
			EnableValidators(true); // Do not remove; Need to reenable after postback, especially in the grid.
			TextBox pwd = null;
			if (cRefTableId20.Items.Count > 0 && cRefTableId20.DataSource != null)
			{
				DataView dv = (DataView) cRefTableId20.DataSource; dv.RowFilter = string.Empty;
				DataRowView dr = cRefTableId20.DataSetIndex >= 0 && cRefTableId20.DataSetIndex < dv.Count ? dv[cRefTableId20.DataSetIndex] : dv[0];
				if (cRefColumnId20.Mode!="A") cRefColumnId20.ClearSearch();				else SetRefColumnId20(cRefColumnId20,cRefColumnId20.SelectedValue,cRefTableId20.SelectedValue);
			if (pwd != null) {ScriptManager.GetCurrent(Parent.Page).SetFocus(pwd.ClientID);} else {ScriptManager.GetCurrent(Parent.Page).SetFocus(SenderFocusId(sender));}
			}
		}
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cKeyId20.Text == string.Empty) { cClearButton_Click(sender, new EventArgs()); ShowDirty(false); PanelTop.Update();}
			else { PopAdmDbKey15List(sender, e, false, cKeyId20.Text); }
			// *** System Button Click (After) Web Rule starts here *** //
		}
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmDbKey15List); PopAdmDbKey15List(sender, e, false, null);
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			if (ValidPage())
			{
				AdmDbKey15 ds = PrepAdmDbKeyData(null,cKeyId20.Text == string.Empty);
				if (!string.IsNullOrEmpty(cAdmDbKey15List.SelectedValue) && cKeyId20.Text != cAdmDbKey15List.SelectedValue)
				{
					PreMsgPopup("Primary key cannot be changed");
					return rtn;
				}
				if (string.IsNullOrEmpty(cAdmDbKey15List.SelectedValue))	// Add
				{
					if (ds != null)
					{
						pid = (new AdminSystem()).AddData(15,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc,noTrans,commandTimeOut);
					}
					if (!string.IsNullOrEmpty(pid))
					{
						cAdmDbKey15List.ClearSearch(); Session.Remove(KEY_dtAdmDbKey15List);
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmDbKey15List(sender, e, false, pid);
						rtn = GetScreenHlp().Rows[0]["AddMsg"].ToString();
					}
				}
				else {
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(15,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc,noTrans,commandTimeOut)) {bValid7 = true;}
					if (bValid7)
					{
						cAdmDbKey15List.ClearSearch(); Session.Remove(KEY_dtAdmDbKey15List);
						ShowDirty(false); PopAdmDbKey15List(sender, e, false, ds.Tables["AdmDbKey"].Rows[0]["KeyId20"]);
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cKeyId20.Text != string.Empty)
			{
				AdmDbKey15 ds = PrepAdmDbKeyData(null,false);
				try
				{
					if (ds != null && (new AdminSystem()).DelData(15,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc))
					{
						cAdmDbKey15List.ClearSearch(); Session.Remove(KEY_dtAdmDbKey15List);
						ShowDirty(false); PopAdmDbKey15List(sender, e, false, null);
		...
		private AdmDbKey15 PrepAdmDbKeyData(DataView dv, bool bAdd)
		{
			AdmDbKey15 ds = new AdmDbKey15();
			DataRow dr = ds.Tables["AdmDbKey"].NewRow();
			DataRow drType = ds.Tables["AdmDbKey"].NewRow();
			DataRow drDisp = ds.Tables["AdmDbKey"].NewRow();
			if (bAdd) { dr["KeyId20"] = string.Empty; } else { dr["KeyId20"] = cKeyId20.Text; }
			drType["KeyId20"] = "Numeric"; drDisp["KeyId20"] = "TextBox";
			try {dr["KeyName20"] = cKeyName20.Text.Trim();} catch {}
			drType["KeyName20"] = "VarChar"; drDisp["KeyName20"] = "TextBox";
			try {dr["TableId20"] = cTableId20.SelectedValue;} catch {}
			drType["TableId20"] = "Numeric"; drDisp["TableId20"] = "AutoComplete";
			try {dr["ColumnId20"] = cColumnId20.SelectedValue;} catch {}
			drType["ColumnId20"] = "Numeric"; drDisp["ColumnId20"] = "AutoComplete";
			try {dr["RefTableId20"] = cRefTableId20.SelectedValue;} catch {}
			drType["RefTableId20"] = "Numeric"; drDisp["RefTableId20"] = "AutoComplete";
			try {dr["RefColumnId20"] = cRefColumnId20.SelectedValue;} catch {}
			drType["RefColumnId20"] = "Numeric"; drDisp["RefColumnId20"] = "AutoComplete";
			if (bAdd)
			{
			}
			ds.Tables["AdmDbKey"].Rows.Add(dr); ds.Tables["AdmDbKey"].Rows.Add(drType); ds.Tables["AdmDbKey"].Rows.Add(drDisp);
			return ds;
		}

		public bool CanAct(char typ) { return CanAct(typ, GetAuthRow(), cAdmDbKey15List.SelectedValue.ToString()); }
		...
		private bool ValidPage()
		{
			...
			DataTable dt = null;
			dt = (DataTable)Session[KEY_dtTableId20];
			if (dt != null && dt.Rows.Count <= 0)
			{
				bErrNow.Value = "Y"; PreMsgPopup("Value is expected for 'TableId', please investigate."); return false;
			}
			dt = (DataTable)Session[KEY_dtColumnId20];
			if (dt != null && dt.Rows.Count <= 0)
			{
				bErrNow.Value = "Y"; PreMsgPopup("Value is expected for 'ColumnId', please investigate."); return false;
			}
			dt = (DataTable)Session[KEY_dtRefColumnId20];
			if (dt != null && dt.Rows.Count <= 0)
			{
				bErrNow.Value = "Y"; PreMsgPopup("Value is expected for 'RefColumnId', please investigate."); return false;
			}
			return true;
		}
		...
		private void PreMsgPopup(string msg) { PreMsgPopup(msg,cAdmDbKey15List,null); }

...
namespace RO.Web
{
	public partial class AdmTbdRuleModule : RO.Web.ModuleBase
	{
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_113";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_113";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_113";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_113";
		private const string KEY_dtLabel = "Cache:dtLabel3_113";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_113";
		private const string KEY_dtScrCri = "Cache:dtScrCri3_113";
		private const string KEY_dsScrCriVal = "Cache:dsScrCriVal3_113";

		private const string KEY_dtScreenId254 = "Cache:dtScreenId254";

		private const string KEY_dtSystems = "Cache:dtSystems3_113";
		private const string KEY_sysConnectionString = "Cache:sysConnectionString3_113";
		private const string KEY_dtAdmTbdRule113List = "Cache:dtAdmTbdRule113List";
		private const string KEY_bNewVisible = "Cache:bNewVisible3_113";
		private const string KEY_bNewSaveVisible = "Cache:bNewSaveVisible3_113";
		private const string KEY_bCopyVisible = "Cache:bCopyVisible3_113";
		private const string KEY_bCopySaveVisible = "Cache:bCopySaveVisible3_113";
		private const string KEY_bDeleteVisible = "Cache:bDeleteVisible3_113";
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_113";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_113";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_113";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				DataTable dtMenuAccess = (new MenuSystem()).GetMenu(base.LUser.CultureId, 3, base.LImpr, LcSysConnString, LcAppPw,113, null, null);
				...
				Session.Remove(KEY_dtAdmTbdRule113List);
				...
				Session.Remove(KEY_dtScreenId254);
				SetButtonHlp();
				GetSystems();
				SetColumnAuthority();
				GetGlobalFilter();
				...
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 113, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmTbdRule113List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=113&typ=N&sys=3','','',''); return false;"; }
		...
		private void SetSystem(byte SystemId)
		{
			...
			try
			{
				...
				if ((Config.DeployType == "DEV" || row["dbAppDatabase"].ToString() == base.CPrj.EntityCode + "View") && !(base.CPrj.EntityCode != "RO" && row["SysProgram"].ToString() == "Y") && (new AdminSystem()).IsRegenNeeded(string.Empty,113,0,0,LcSysConnString,LcAppPw))
				{
					(new GenScreensSystem()).CreateProgram(113, "Rules To Be Done", row["dbAppDatabase"].ToString(), base.CPrj, base.CSrc, base.CTar, LcAppConnString, LcAppPw);
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(113,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(113,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(113,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(113,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,113,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(113,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(113,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(113,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		protected DataTable GetAuthRow()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthRow(113,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				try
				{
					try {dv = new DataView((new AdminSystem()).GetExp(113,"GetExpAdmTbdRule113","Y",(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
					catch {dv = new DataView((new AdminSystem()).GetExp(113,"GetExpAdmTbdRule113","N",(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(113,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					{
						if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["TbdRuleId254"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["ScreenId254"].ToString() + (char)9 + drv["ScreenId254Text"].ToString() + (char)9);}
						if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["TbdRuleName254"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["TbdRuleDesc254"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						sb.Append(Environment.NewLine);
					}
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmTbdRule.csv"; Session["ExportStr"] = (Config.ExportExcelCSV ? "sep=\t\n": "") + sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmTbdRule.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private string GetRtfGrid(DataView dv)
		{
			...
			try
			{
				...
				foreach (DataRowView drv in dv)
				{
					...
					sb.Append("\r\n");
					sb.Append(@"{");
					if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["TbdRuleId254"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["ScreenId254Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(drv["TbdRuleName254"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(drv["TbdRuleDesc254"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					sb.Append(@"}");
					sb.Append("\r\n");
					sb.Append(@"\pard \ql \li0\ri0\widctlpar\intbl\aspalpha\aspnum\adjustright\rin0\lin0 {");
					sb.Append(@"\insrsid2981395 \trowd \irow0\irowband0\lastrow \ts15\trgaph108\trleft-108\trbrdrt\brdrs\brdrw10 \trbrdrl\brdrs\brdrw10 \trbrdrb\brdrs\brdrw10 \trbrdrr\brdrs\brdrw10 \trbrdrh\brdrs\brdrw10 \trbrdrv\brdrs\brdrw10 ");
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 113, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			...
			UpdCriteria(true);
			cAdmTbdRule113List.ClearSearch(); Session.Remove(KEY_dtAdmTbdRule113List);
			PopAdmTbdRule113List(sender, e, false, null);
		}
			...
			try
			{
				foreach (DataRowView drv in dvCri)
				{
					...
					if (drv["DisplayName"].ToString() == "ComboBox")
					{
						...
						(new AdminSystem()).MkGetScreenIn("113", drv["ScreenCriId"].ToString(), "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), LcAppDb, LcDesDb, drv["MultiDesignDb"].ToString(), LcSysConnString, LcAppPw);
						DataView dv = new DataView((new AdminSystem()).GetScreenIn("113", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), (new AdminSystem()).CountScrCri(drv["ScreenCriId"].ToString(), drv["MultiDesignDb"].ToString(), drv["MultiDesignDb"].ToString() == "N" ? LcSysConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)), drv["RequiredValid"].ToString(), 0, string.Empty, drv["DisplayMode"].ToString() != "AutoListBox", val, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)));
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("113", LcSysConnString, LcAppPw);
				}
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmTbdRule113" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmTbdRule113" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			foreach (DataRowView drv in dvCri)
			{
			    if (drv["DisplayName"].ToString() == "ListBox")
			    {
					...
					if (cListBox != null)
					{
						...
						int TotalChoiceCnt = new DataView((new AdminSystem()).GetScreenIn("113", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), CriCnt, drv["RequiredValid"].ToString(), 0, string.Empty, true, string.Empty, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId))).Count;
						string selectedValues = string.Join(",", cListBox.Items.Cast<ListItem>().Where(x => x.Selected).Select(x => "'" + x.Value + "'").ToArray());
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("113", "AdmTbdRule113", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		protected void cbScreenId254(object sender, System.EventArgs e)
		{
			SetScreenId254((RoboCoder.WebControls.ComboBox)sender,string.Empty);
		}

		private void SetScreenId254(RoboCoder.WebControls.ComboBox ddl, string keyId)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(113,"GetDdlScreenId3S1977",true,false,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
					if (dv.Table.Columns.Contains("TbdRuleId"))
					{
						context["pMKeyColID"] = cAdmTbdRule113List.ClientID;
						context["pMKeyCol"] = "TbdRuleId";
						string ss = "(TbdRuleId is null";
						if (string.IsNullOrEmpty(cAdmTbdRule113List.SelectedValue)) {ss = ss + ")";} else {ss = ss + " OR TbdRuleId = " + cAdmTbdRule113List.SelectedValue + ")";}
						dv.RowFilter = ss;
					}
					ddl.DataSource = dv; Session[KEY_dtScreenId254] = dv.Table;
				    try { ddl.SelectByValue(keyId,string.Empty,true); } catch { try { ddl.SelectedIndex = 0; } catch { } }
				}
			}
		}

		private DataView GetAdmTbdRule113List()
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmTbdRule113List];
			if (dt == null)
			{
				CheckAuthentication(false);
				int filterId = 0;
				...
				try
				{
					try {dt = (new AdminSystem()).GetLis(113,"GetLisAdmTbdRule113",true,"Y",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
					catch {dt = (new AdminSystem()).GetLis(113,"GetLisAdmTbdRule113",true,"N",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				}
		...
		private void PopAdmTbdRule113List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			DataView dv = GetAdmTbdRule113List();
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			context["isSys"] = "N";
			context["conn"] = KEY_sysConnectionString;
			cAdmTbdRule113List.AutoCompleteUrl = "AutoComplete.aspx/LisSuggests";
			cAdmTbdRule113List.DataContext = context;
			if (dv.Table == null) return;
			cAdmTbdRule113List.DataSource = dv;
			cAdmTbdRule113List.Visible = true;
			if (cAdmTbdRule113List.Items.Count <= 0) {cAdmTbdRule113List.Visible = false; cAdmTbdRule113List_SelectedIndexChanged(sender,e);}
			else
			{
				if (bPageLoad && !string.IsNullOrEmpty(Request.QueryString["key"]) && bUseCri.Value == string.Empty)
				{
					try {cAdmTbdRule113List.SelectByValue(Request.QueryString["key"],string.Empty,true);} catch {cAdmTbdRule113List.Items[0].Selected = true; cAdmTbdRule113List_SelectedIndexChanged(sender, e);}
				    cScreenSearch.Visible = false; cSystem.Visible = false; cEditButton.Visible = true; cSaveCloseButton.Visible = cSaveButton.Visible;
				}
				else
				{
					if (ID != null) {if (!cAdmTbdRule113List.SelectByValue(ID,string.Empty,true)) {ID = null;}}
					if (ID == null)
					{
						cAdmTbdRule113List_SelectedIndexChanged(sender, e);
					}
				}
			}
		}
		...
		private void SetColumnAuthority()
		{
			...
			{
				base.SetFoldBehavior(cTbdRuleId254, dtAuth.Rows[0], cTbdRuleId254P1, cTbdRuleId254Label, cTbdRuleId254P2, null, dtLabel.Rows[0], null, null, null);
				base.SetFoldBehavior(cScreenId254, dtAuth.Rows[1], cScreenId254P1, cScreenId254Label, cScreenId254P2, null, dtLabel.Rows[1], cRFVScreenId254, null, null);
				SetScreenId254(cScreenId254,string.Empty);
				base.SetFoldBehavior(cTbdRuleName254, dtAuth.Rows[2], cTbdRuleName254P1, cTbdRuleName254Label, cTbdRuleName254P2, null, dtLabel.Rows[2], cRFVTbdRuleName254, null, null);
				base.SetFoldBehavior(cTbdRuleDesc254, dtAuth.Rows[3], cTbdRuleDesc254P1, cTbdRuleDesc254Label, cTbdRuleDesc254P2, cTbdRuleDesc254E, null, dtLabel.Rows[3], null, null, null);
				cTbdRuleDesc254E.Attributes["label_id"] = cTbdRuleDesc254Label.ClientID; cTbdRuleDesc254E.Attributes["target_id"] = cTbdRuleDesc254.ClientID;
			}
			if ((cScreenId254.Attributes["OnChange"] == null || cScreenId254.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cScreenId254.Visible && cScreenId254.Enabled) {cScreenId254.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cTbdRuleName254.Attributes["OnChange"] == null || cTbdRuleName254.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cTbdRuleName254.Visible && !cTbdRuleName254.ReadOnly) {cTbdRuleName254.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cTbdRuleDesc254.Attributes["OnChange"] == null || cTbdRuleDesc254.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cTbdRuleDesc254.Visible && !cTbdRuleDesc254.ReadOnly) {cTbdRuleDesc254.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
		}
		...
		private void ClearMaster(object sender, System.EventArgs e)
		{
			...
			if (dt.Rows[1]["ColVisible"].ToString() == "Y" && dt.Rows[1]["ColReadOnly"].ToString() != "Y") {cScreenId254.ClearSearch();}
			if (dt.Rows[2]["ColVisible"].ToString() == "Y" && dt.Rows[2]["ColReadOnly"].ToString() != "Y") {cTbdRuleName254.Text = string.Empty;}
			if (dt.Rows[3]["ColVisible"].ToString() == "Y" && dt.Rows[3]["ColReadOnly"].ToString() != "Y") {cTbdRuleDesc254.Text = string.Empty;}
			// *** Default Value (Folder) Web Rule starts here *** //
		}

		private void InitMaster(object sender, System.EventArgs e)
		{
			cTbdRuleId254.Text = string.Empty;
			cScreenId254.ClearSearch();
			cTbdRuleName254.Text = string.Empty;
			cTbdRuleDesc254.Text = string.Empty;
			// *** Default Value (Folder) Web Rule starts here *** //
		}
		protected void cAdmTbdRule113List_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmTbdRule113List_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmTbdRule113List_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			InitMaster(sender, e);      // Do this first to enable defaults for non-database columns.
			DataTable dt = null;
			try
			{
				dt = (new AdminSystem()).GetMstById("GetAdmTbdRule113ById",cAdmTbdRule113List.SelectedValue,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId));
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				if (dt.Rows.Count > 0)
				{
					cAdmTbdRule113List.SetDdVisible();
					DataRow dr = dt.Rows[0];
					try {cTbdRuleId254.Text = RO.Common3.Utils.fmNumeric("0",dr["TbdRuleId254"].ToString(),base.LUser.Culture);} catch {cTbdRuleId254.Text = string.Empty;}
					SetScreenId254(cScreenId254,dr["ScreenId254"].ToString());
					try {cTbdRuleName254.Text = dr["TbdRuleName254"].ToString();} catch {cTbdRuleName254.Text = string.Empty;}
					try {cTbdRuleDesc254.Text = dr["TbdRuleDesc254"].ToString();} catch {cTbdRuleDesc254.Text = string.Empty;}
				}
			}
			cButPanel.DataBind();
			ScriptManager.GetCurrent(Parent.Page).SetFocus(cAdmTbdRule113List.FocusID);
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cTbdRuleId254.Text == string.Empty) { cClearButton_Click(sender, new EventArgs()); ShowDirty(false); PanelTop.Update();}
			else { PopAdmTbdRule113List(sender, e, false, cTbdRuleId254.Text); }
			// *** System Button Click (After) Web Rule starts here *** //
		}
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmTbdRule113List); PopAdmTbdRule113List(sender, e, false, null);
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			if (ValidPage())
			{
				AdmTbdRule113 ds = PrepAdmTbdRuleData(null,cTbdRuleId254.Text == string.Empty);
				if (!string.IsNullOrEmpty(cAdmTbdRule113List.SelectedValue) && cTbdRuleId254.Text != cAdmTbdRule113List.SelectedValue)
				{
					PreMsgPopup("Primary key cannot be changed");
					return rtn;
				}
				if (string.IsNullOrEmpty(cAdmTbdRule113List.SelectedValue))	// Add
				{
					if (ds != null)
					{
						pid = (new AdminSystem()).AddData(113,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc,noTrans,commandTimeOut);
					}
					if (!string.IsNullOrEmpty(pid))
					{
						cAdmTbdRule113List.ClearSearch(); Session.Remove(KEY_dtAdmTbdRule113List);
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmTbdRule113List(sender, e, false, pid);
						rtn = GetScreenHlp().Rows[0]["AddMsg"].ToString();
					}
				}
				else {
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(113,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc,noTrans,commandTimeOut)) {bValid7 = true;}
					if (bValid7)
					{
						cAdmTbdRule113List.ClearSearch(); Session.Remove(KEY_dtAdmTbdRule113List);
						ShowDirty(false); PopAdmTbdRule113List(sender, e, false, ds.Tables["AdmTbdRule"].Rows[0]["TbdRuleId254"]);
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cTbdRuleId254.Text != string.Empty)
			{
				AdmTbdRule113 ds = PrepAdmTbdRuleData(null,false);
				try
				{
					if (ds != null && (new AdminSystem()).DelData(113,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc))
					{
						cAdmTbdRule113List.ClearSearch(); Session.Remove(KEY_dtAdmTbdRule113List);
						ShowDirty(false); PopAdmTbdRule113List(sender, e, false, null);
		...
		private AdmTbdRule113 PrepAdmTbdRuleData(DataView dv, bool bAdd)
		{
			...
			if (bAdd) { dr["TbdRuleId254"] = string.Empty; } else { dr["TbdRuleId254"] = cTbdRuleId254.Text; }
			drType["TbdRuleId254"] = "Numeric"; drDisp["TbdRuleId254"] = "TextBox";
			try {dr["ScreenId254"] = cScreenId254.SelectedValue;} catch {}
			drType["ScreenId254"] = "Numeric"; drDisp["ScreenId254"] = "AutoComplete";
			try {dr["TbdRuleName254"] = cTbdRuleName254.Text.Trim();} catch {}
			drType["TbdRuleName254"] = "VarWChar"; drDisp["TbdRuleName254"] = "TextBox";
			try {dr["TbdRuleDesc254"] = cTbdRuleDesc254.Text;} catch {}
		...
		public bool CanAct(char typ) { return CanAct(typ, GetAuthRow(), cAdmTbdRule113List.SelectedValue.ToString()); }
		...
		private bool ValidPage()
		{
			...
			DataTable dt = null;
			dt = (DataTable)Session[KEY_dtScreenId254];
			if (dt != null && dt.Rows.Count <= 0)
			{
		...
		private void PreMsgPopup(string msg) { PreMsgPopup(msg,cAdmTbdRule113List,null); }

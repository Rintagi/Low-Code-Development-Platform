...
namespace RO.Web
{
	public partial class AdmOvrideModule : RO.Web.ModuleBase
	{

		private const string KEY_lastAddedRow = "Cache:lastAddedRow3_78";
		private const string KEY_lastSortOrd = "Cache:lastSortOrd3_78";
		private const string KEY_lastSortImg = "Cache:lastSortImg3_78";
		private const string KEY_lastSortCol = "Cache:lastSortCol3_78";
		private const string KEY_lastSortExp = "Cache:lastSortExp3_78";
		private const string KEY_lastSortUrl = "Cache:lastSortUrl3_78";
		private const string KEY_lastSortTog = "Cache:lastSortTog3_78";
		private const string KEY_lastImpPwdOvride = "Cache:lastImpPwdOvride3_78";
		private const string KEY_cntImpPwdOvride = "Cache:cntImpPwdOvride3_78";
		private const string KEY_currPageIndex = "Cache:currPageIndex3_78";
		private const string KEY_bHiImpVisible = "Cache:bHiImpVisible3_78";
		private const string KEY_bShImpVisible = "Cache:bShImpVisible3_78";
		private const string KEY_dtAdmOvrideGrid = "Cache:dtAdmOvrideGrid";
		private const string KEY_scrImport = "Cache:scrImport3_78";
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_78";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_78";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_78";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_78";
		private const string KEY_dtLabel = "Cache:dtLabel3_78";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_78";
		private const string KEY_dtScrCri = "Cache:dtScrCri3_78";
		private const string KEY_dsScrCriVal = "Cache:dsScrCriVal3_78";

		private const string KEY_dtUsrGroupId123 = "Cache:dtUsrGroupId123";

		private const string KEY_dtSystems = "Cache:dtSystems3_78";
		private const string KEY_sysConnectionString = "Cache:sysConnectionString3_78";
		private const string KEY_dtAdmOvride78List = "Cache:dtAdmOvride78List";
		private const string KEY_bNewVisible = "Cache:bNewVisible3_78";
		private const string KEY_bNewSaveVisible = "Cache:bNewSaveVisible3_78";
		private const string KEY_bCopyVisible = "Cache:bCopyVisible3_78";
		private const string KEY_bCopySaveVisible = "Cache:bCopySaveVisible3_78";
		private const string KEY_bDeleteVisible = "Cache:bDeleteVisible3_78";
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_78";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_78";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_78";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				DataTable dtMenuAccess = (new MenuSystem()).GetMenu(base.LUser.CultureId, 3, base.LImpr, LcSysConnString, LcAppPw,78, null, null);
				...
				Session.Remove(KEY_dtUsrGroupId123);
				SetButtonHlp();
				GetSystems();
				SetColumnAuthority();
				GetGlobalFilter();
				...
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 78, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmOvride78List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=78&typ=N&sys=3','','',''); return false;"; }
			...
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmOvrideGrid];
				if (dt != null)
				{
		...
		private void InitializeComponent()
		{
			...
			if (LcSysConnString == null)
			{
				...
				{
					DataTable dt;
					dt = (new AdminSystem()).GetAuthRow(78,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
					...
					{
						cAdmOvrideGrid.PreRender += new EventHandler(cAdmOvrideGrid_OnPreRender);
					}
				}
		...
		private void SetSystem(byte SystemId)
		{
			...
			try
			{
				...
				if ((Config.DeployType == "DEV" || row["dbAppDatabase"].ToString() == base.CPrj.EntityCode + "View") && !(base.CPrj.EntityCode != "RO" && row["SysProgram"].ToString() == "Y") && (new AdminSystem()).IsRegenNeeded(string.Empty,78,0,0,LcSysConnString,LcAppPw))
				{
					(new GenScreensSystem()).CreateProgram(78, "Password Override Type", row["dbAppDatabase"].ToString(), base.CPrj, base.CSrc, base.CTar, LcAppConnString, LcAppPw);
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(78,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(78,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void SetClientRule(ListViewDataItem ee, bool isEditItem)
		{
			...
			if (dvRul.Count > 0)
			{
				...
				{
					if (ee == null || drv["ScriptEvent"].ToString().Substring(0,2).ToLower() != "on" || (cAdmOvrideGrid.EditIndex > -1 && GetDataItemIndex(cAdmOvrideGrid.EditIndex) == ee.DataItemIndex))
					{
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(78,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(78,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,78,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(78,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(78,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(78,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				try
				{
					try {dv = new DataView((new AdminSystem()).GetExp(78,"GetExpAdmOvride78","Y",null,null,filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
					catch {dv = new DataView((new AdminSystem()).GetExp(78,"GetExpAdmOvride78","N",null,null,filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(78,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					{
						if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["OvrideId122"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["OvrideName122"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["PromptAlways122"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["PromptModal122"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["OvrideGrpId123"].ToString(),base.LUser.Culture) + (char)9);}
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmOvride.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private string GetRtfGrid(DataView dv)
		{
			...
			try
			{
				...
				foreach (DataRowView drv in dv)
				{
					...
					sb.Append("\r\n");
					sb.Append(@"{");
					if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["OvrideId122"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["OvrideName122"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(drv["PromptAlways122"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(drv["PromptModal122"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["OvrideGrpId123"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append(drv["UsrGroupId123Text"].ToString() + @"\cell ");}
					sb.Append(@"}");
					sb.Append("\r\n");
					sb.Append(@"\pard \ql \li0\ri0\widctlpar\intbl\aspalpha\aspnum\adjustright\rin0\lin0 {");
					sb.Append(@"\insrsid2981395 \trowd \irow0\irowband0\lastrow \ts15\trgaph108\trleft-108\trbrdrt\brdrs\brdrw10 \trbrdrl\brdrs\brdrw10 \trbrdrb\brdrs\brdrw10 \trbrdrr\brdrs\brdrw10 \trbrdrh\brdrs\brdrw10 \trbrdrv\brdrs\brdrw10 ");
		...
		private void GetPageInfo()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastPageInfo(78, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 78, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			// *** Criteria Trigger (before) Web Rule starts here *** //
			DataTable dt = (DataTable)Session[KEY_dtAdmOvrideGrid];
			if (cAdmOvrideGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
			if (IsPostBack && cCriteria.Visible) ReBindCriteria(GetScrCriteria());
			UpdCriteria(true);
			cAdmOvride78List.ClearSearch(); Session.Remove(KEY_dtAdmOvride78List);
			PopAdmOvride78List(sender, e, false, null);
			}
		}
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("78", LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmOvride78" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmOvride78" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("78", "AdmOvride78", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		private void SetUsrGroupId123(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtUsrGroupId123];
			...
			if (ddl != null)
			{
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(78,"GetDdlUsrGroupId3S1246",true,bAll,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					if (dv.Table.Columns.Contains("OvrideId"))
					{
						ss = "(OvrideId is null";
						if (string.IsNullOrEmpty(cAdmOvride78List.SelectedValue)) {ss = ss + ")";} else {ss = ss + " OR OvrideId = " + cAdmOvride78List.SelectedValue + ")";}
					}
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "UsrGroupId123 is not NULL"; }
					dv.RowFilter = ss;
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(78,"GetDdlUsrGroupId3S1246",true,true,0,keyId,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtUsrGroupId123] = dv.Table;
				}
			}
		}

		private DataView GetAdmOvride78List()
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmOvride78List];
			if (dt == null)
			{
				CheckAuthentication(false);
				int filterId = 0;
				...
				try
				{
					try {dt = (new AdminSystem()).GetLis(78,"GetLisAdmOvride78",true,"Y",0,null,null,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
					catch {dt = (new AdminSystem()).GetLis(78,"GetLisAdmOvride78",true,"N",0,null,null,filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				}
		...
		private void PopAdmOvride78List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			DataView dv = GetAdmOvride78List();
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			cAdmOvride78List.AutoCompleteUrl = "AutoComplete.aspx/LisSuggests";
			cAdmOvride78List.DataContext = context;
			if (dv.Table == null) return;
			cAdmOvride78List.DataSource = dv;
			cAdmOvride78List.Visible = true;
			if (cAdmOvride78List.Items.Count <= 0) {cAdmOvride78List.Visible = false; cAdmOvride78List_SelectedIndexChanged(sender,e);}
			else
			{
				if (bPageLoad && !string.IsNullOrEmpty(Request.QueryString["key"]) && bUseCri.Value == string.Empty)
				{
					try {cAdmOvride78List.SelectByValue(Request.QueryString["key"],string.Empty,true);} catch {cAdmOvride78List.Items[0].Selected = true; cAdmOvride78List_SelectedIndexChanged(sender, e);}
				    cScreenSearch.Visible = false; cSystem.Visible = false; cEditButton.Visible = true; cSaveCloseButton.Visible = cSaveButton.Visible;
				}
				else
				{
					if (ID != null) {if (!cAdmOvride78List.SelectByValue(ID,string.Empty,true)) {ID = null;}}
					if (ID == null)
					{
						cAdmOvride78List_SelectedIndexChanged(sender, e);
					}
				}
			}
		}
		...
		private void SetColumnAuthority()
		{
			...
			{
				base.SetFoldBehavior(cOvrideId122, dtAuth.Rows[0], cOvrideId122P1, cOvrideId122Label, cOvrideId122P2, null, dtLabel.Rows[0], null, null, null);
				base.SetFoldBehavior(cOvrideName122, dtAuth.Rows[1], cOvrideName122P1, cOvrideName122Label, cOvrideName122P2, null, dtLabel.Rows[1], cRFVOvrideName122, null, null);
				base.SetFoldBehavior(cPromptAlways122, dtAuth.Rows[2], cPromptAlways122P1, cPromptAlways122Label, cPromptAlways122P2, null, dtLabel.Rows[2], null, null, null);
				base.SetFoldBehavior(cPromptModal122, dtAuth.Rows[3], cPromptModal122P1, cPromptModal122Label, cPromptModal122P2, null, dtLabel.Rows[3], null, null, null);
			}
			if ((cOvrideName122.Attributes["OnChange"] == null || cOvrideName122.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cOvrideName122.Visible && !cOvrideName122.ReadOnly) {cOvrideName122.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cPromptAlways122.Attributes["OnClick"] == null || cPromptAlways122.Attributes["OnClick"].IndexOf("ChkPgDirty") < 0) && cPromptAlways122.Visible && cPromptAlways122.Enabled) {cPromptAlways122.Attributes["OnClick"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty(); this.focus();";}
			if ((cPromptModal122.Attributes["OnClick"] == null || cPromptModal122.Attributes["OnClick"].IndexOf("ChkPgDirty") < 0) && cPromptModal122.Visible && cPromptModal122.Enabled) {cPromptModal122.Attributes["OnClick"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty(); this.focus();";}
		}
		...
		private void ClearMaster(object sender, System.EventArgs e)
		{
			...
			if (dt.Rows[1]["ColVisible"].ToString() == "Y" && dt.Rows[1]["ColReadOnly"].ToString() != "Y") {cOvrideName122.Text = string.Empty;}
			if (dt.Rows[2]["ColVisible"].ToString() == "Y" && dt.Rows[2]["ColReadOnly"].ToString() != "Y") {cPromptAlways122.Checked = base.GetBool("N");}
			if (dt.Rows[3]["ColVisible"].ToString() == "Y" && dt.Rows[3]["ColReadOnly"].ToString() != "Y") {cPromptModal122.Checked = base.GetBool("N");}
			// *** Default Value (Folder) Web Rule starts here *** //
		}

		private void InitMaster(object sender, System.EventArgs e)
		{
			cOvrideId122.Text = string.Empty;
			cOvrideName122.Text = string.Empty;
			cPromptAlways122.Checked = base.GetBool("N");
			cPromptModal122.Checked = base.GetBool("N");
			// *** Default Value (Folder) Web Rule starts here *** //
		}
		protected void cAdmOvride78List_TextChanged(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmOvride78List_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmOvride78List_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			InitMaster(sender, e);      // Do this first to enable defaults for non-database columns.
			DataTable dt = null;
			try
			{
				dt = (new AdminSystem()).GetMstById("GetAdmOvride78ById",cAdmOvride78List.SelectedValue,null,null);
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				if (dt.Rows.Count > 0)
				{
					cAdmOvride78List.SetDdVisible();
					DataRow dr = dt.Rows[0];
					try {cOvrideId122.Text = RO.Common3.Utils.fmNumeric("0",dr["OvrideId122"].ToString(),base.LUser.Culture);} catch {cOvrideId122.Text = string.Empty;}
					try {cOvrideName122.Text = dr["OvrideName122"].ToString();} catch {cOvrideName122.Text = string.Empty;}
					try {cPromptAlways122.Checked = base.GetBool(dr["PromptAlways122"].ToString());} catch {cPromptAlways122.Checked = false;}
					try {cPromptModal122.Checked = base.GetBool(dr["PromptModal122"].ToString());} catch {cPromptModal122.Checked = false;}
				}
			}
			cButPanel.DataBind(); if (!cSaveButton.Visible) { cInsRowButton.Visible = false; }
			DataTable dtAdmOvrideGrid = null;
			int filterId = 0; if (Utils.IsInt(cFilterId.SelectedValue)) { filterId = int.Parse(cFilterId.SelectedValue); }
			try
			{
				dtAdmOvrideGrid = (new AdminSystem()).GetDtlById(78,"GetAdmOvride78DtlById",cAdmOvride78List.SelectedValue,null,null,filterId,base.LImpr,base.LCurr);
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (!dtAdmOvrideGrid.Columns.Contains("_NewRow")) { dtAdmOvrideGrid.Columns.Add("_NewRow"); }
			Session[KEY_dtAdmOvrideGrid] = dtAdmOvrideGrid;
			cAdmOvrideGrid.EditIndex = -1;
			cAdmOvrideGridDataPager.PageSize = Int16.Parse(cPgSize.Text); GotoPage(0);
			if (Session[KEY_lastSortCol] != null)
			{
				cAdmOvrideGrid_OnSorting(sender, new ListViewSortEventArgs((string)Session[KEY_lastSortExp], SortDirection.Ascending));
			}
			else if (dtAdmOvrideGrid.Rows.Count <= 0 || (!((GetAuthRow().Rows[0]["AllowUpd"].ToString() == "N" || GetAuthRow().Rows[0]["ViewOnly"].ToString() == "G") && GetAuthRow().Rows[0]["AllowAdd"].ToString() == "N") && !(Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N") && dtAdmOvrideGrid.Rows.Count == 1))
			{
				cAdmOvrideGrid_DataBind(dtAdmOvrideGrid.DefaultView);
			}
			cFindButton_Click(sender, e);
			cNaviPanel.Visible = true; cImportPwdPanel.Visible = false; Session.Remove(KEY_scrImport);
			ScriptManager.GetCurrent(Parent.Page).SetFocus(cAdmOvride78List.FocusID);
		...
		public void cFindButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dv != null)
			{
				...
				if ((cc != null && cc.ID.Equals("cAdmOvride78List")) || cAdmOvrideGrid.EditIndex < 0 || UpdateGridRow(cAdmOvrideGrid, new CommandEventArgs("Save", "")))
				{
					...
					GotoPage(0); cAdmOvrideGrid_DataBind(dv);
					if (GetCurrPageIndex() != (int)Session[KEY_currPageIndex] && (int)Session[KEY_currPageIndex] < GetTotalPages()) { GotoPage((int)Session[KEY_currPageIndex]); cAdmOvrideGrid_DataBind(dv);}
		...
		public void cInsRowButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dt != null && (ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")))
			{
				...
				Session[KEY_lastAddedRow] = 0; Session[KEY_dtAdmOvrideGrid] = dt; Session[KEY_currPageIndex] = 0; GotoPage(0);
				cAdmOvrideGrid_DataBind(dt.DefaultView);
				cAdmOvrideGrid_OnItemEditing(cAdmOvrideGrid, new ListViewEditEventArgs(0));
		...
		public void cPgSizeButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cAdmOvrideGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				...
				cAdmOvrideGridDataPager.PageSize = int.Parse(cPgSize.Text); GotoPage(0);
				if (dt.Rows.Count <= 0 || (!((GetAuthRow().Rows[0]["AllowUpd"].ToString() == "N" || GetAuthRow().Rows[0]["ViewOnly"].ToString() == "G") && GetAuthRow().Rows[0]["AllowAdd"].ToString() == "N") && !(Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N") && dt.Rows.Count == 1)) {cAdmOvrideGrid_DataBind(dt.DefaultView);} else {cFindButton_Click(sender, e);}
				try
				{
					(new AdminSystem()).UpdLastPageInfo(78, base.LUser.UsrId, cPgSize.Text, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		}
		...
		public void cShowImpButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cSchemaImage.Visible)
			{
				try
				{
					Session["ImportSchema"] = (new AdminSystem()).GetSchemaScrImp(78,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (cSchemaImage.Attributes["OnClick"] == null || cSchemaImage.Attributes["OnClick"].IndexOf("ImportSchema.aspx") < 0) {cSchemaImage.Attributes["OnClick"] += "SearchLink('ImportSchema.aspx?scm=S&key=78&csy=3','','',''); return false;";}
			}
			// *** System Button Click (after) Web Rule starts here *** //
		}
		...
		private void ScrImportDdl(DataView dvd, DataRowView drv, bool bComboBox, string PKey, string CKey, string CNam, int MaxLen, string MatchCd, bool bAllowNulls)
		{
			if (dvd != null)
			{
				...
				{
					if (string.IsNullOrEmpty(cAdmOvride78List.SelectedValue))
					{
						dvd.RowFilter = "(" + PKey + " is null)";
					}
					else
					{
						dvd.RowFilter = "(" + PKey + " is null OR " + PKey + " = " + cAdmOvride78List.SelectedValue + ")";
					}
				}
		...
		protected void ScrImport(bool bClear)
		{
			...
			DataTable dtAdmOvrideGrid = (DataTable)Session[KEY_dtAdmOvrideGrid];
			if (dti != null && dtAdmOvrideGrid != null)
			{
				if (bClear)
				{
					dtAdmOvrideGrid.Rows.Clear();
				}
				cFind.Text = string.Empty;
				int iExist = dtAdmOvrideGrid.Rows.Count;
				...
				foreach (DataRowView drv in dti.DefaultView)
				{
					...
					{
						DataTable dtd;
						dtd = (DataTable)Session[KEY_dtUsrGroupId123];
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(78,"GetDdlUsrGroupId3S1246",true,true,0,string.Empty,LcAppConnString,LcAppPw,string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
							Session[KEY_dtUsrGroupId123] = dtd;
						}
						ScrImportDdl(dtd.DefaultView, drv, false, "OvrideId", "UsrGroupId123", "UsrGroupId123Text", 50, "2", false);
					}
					jj = jj + 1;
				}
				iRow = dti.DefaultView.Count - iExist;
				if (!dti.Columns.Contains("_NewRow")) { dti.Columns.Add("_NewRow"); }
				Session[KEY_dtAdmOvrideGrid] = dti;
				cAdmOvrideGrid_DataBind(dti.DefaultView);
		...
		public void cImportButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cFNameO.Text != string.Empty && cWorkSheet.Items.Count > 0 && cWorkSheet.SelectedItem.Text != string.Empty && cStartRow.Text != string.Empty)
			{
				...
				try
				{
					if (cAdmOvrideGrid.EditIndex < 0 || UpdateGridRow(cAdmOvrideGrid, new CommandEventArgs("Save", "")))
					{
						Session[KEY_scrImport] = ImportFile(dtAdmOvrideGrid.Copy(),cFNameO.Text,cWorkSheet.SelectedItem.Text,cStartRow.Text,Config.PathTmpImport + cFName.Text);
					}
		...
		private void GotoPage(int pageNo)
		{
			if (pageNo >= 0 && pageNo < GetTotalPages())
			{
				if (cAdmOvrideGrid.EditIndex < 0 || UpdateGridRow(cAdmOvrideGrid, new CommandEventArgs("Save", "")))
				{
					try { cAdmOvrideGridDataPager.SetPageProperties(cAdmOvrideGridDataPager.PageSize * pageNo, cAdmOvrideGridDataPager.MaximumRows, false); cAdmOvrideGrid_DataBind(null); }
					catch
					{
						try { cAdmOvrideGridDataPager.SetPageProperties(0, cAdmOvrideGridDataPager.MaximumRows, false); cAdmOvrideGrid_DataBind(null); } catch {}
					}
				}
			}
		}

		protected void cAdmOvrideGrid_OnItemCommand(object sender, ListViewCommandEventArgs e)
		{
		}

		private void cAdmOvrideGrid_DataBind(DataView dvAdmOvrideGrid)
		{
			if (dvAdmOvrideGrid == null)
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmOvrideGrid];
				dvAdmOvrideGrid = dt != null ? dt.DefaultView : null;
			}
			if (dvAdmOvrideGrid != null)
			{
				LcAuth = GetAuthCol().AsEnumerable().ToDictionary<DataRow,string>(dr=>dr["ColName"].ToString());
				cAdmOvrideGrid.DataSource = dvAdmOvrideGrid;
				cAdmOvrideGrid.DataBind();
		...
		protected void cAdmOvrideGrid_OnSorting(object sender, ListViewSortEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmOvrideGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null)
			{
				if (cAdmOvrideGrid.EditIndex < 0 || UpdateGridRow(cAdmOvrideGrid, new CommandEventArgs("Save", "")))
				{
					...
					Session[KEY_dtAdmOvrideGrid] = dt;
					cAdmOvrideGrid_DataBind(dv);
				}
			}
		}

		protected void cAdmOvrideGrid_OnPagePropertiesChanging(object sender, PagePropertiesChangingEventArgs e)
		{
		    cAdmOvrideGridDataPager.SetPageProperties(e.StartRowIndex, e.MaximumRows, false);
		    DataTable dt = (DataTable)Session[KEY_dtAdmOvrideGrid];
		    DataView dv = dt != null ? dt.DefaultView : null;
			cAdmOvrideGrid_DataBind(dv); cAdmOvrideGrid.EditIndex = -1;
		}

		private void GridChkPgDirty(ListViewItem lvi)
		{
				WebControl cc = null;
				cc = ((WebControl)lvi.FindControl("cUsrGroupId123"));
		...
		protected void cAdmOvrideGrid_OnItemDataBound(object sender, ListViewItemEventArgs e)
		{
			// *** GridItemDataBound (before) Web Rule End *** //
			DataTable dt = (DataTable)Session[KEY_dtAdmOvrideGrid];
			bool isEditItem = false;
			DataView dvAdmOvrideGrid = dt != null ? dt.DefaultView : null;
			if (cAdmOvrideGrid.EditIndex > -1 && GetDataItemIndex(cAdmOvrideGrid.EditIndex) == e.Item.DataItemIndex)
			{
				isEditItem = true;
				base.SetGridEnabled(e.Item, GetAuthCol(), GetLabel(), 4);
				SetUsrGroupId123((DropDownList)e.Item.FindControl("cUsrGroupId123"),dvAdmOvrideGrid[e.Item.DataItemIndex]["UsrGroupId123"].ToString());
			...
			if (dtAuthRow != null)
			{
				...
				{
					cc = e.Item.FindControl("cAdmOvrideGridDelete"); if (cc != null) { cc.Visible = false; }
					cc = e.Item.FindControl("cAdmOvrideGridEdit"); if (cc != null) { cc.Visible = false; }
				}
				else
				{
			        HtmlTableRow tr = e.Item.FindControl("cAdmOvrideGridRow") as HtmlTableRow;
			        LinkButton lb = e.Item.FindControl("cAdmOvrideGridEdit") as LinkButton;
		...
		protected void cOvrideGrpId123hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "0") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "0"; Session[KEY_lastSortExp] = "OvrideGrpId123";Session[KEY_lastSortImg] = "cOvrideGrpId123hi";
			cAdmOvrideGrid_OnSorting(sender, new ListViewSortEventArgs("OvrideGrpId123", SortDirection.Ascending));
		}

		protected void cUsrGroupId123hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "1") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "1"; Session[KEY_lastSortExp] = "UsrGroupId123Text";Session[KEY_lastSortImg] = "cUsrGroupId123hi";
			cAdmOvrideGrid_OnSorting(sender, new ListViewSortEventArgs("UsrGroupId123Text", SortDirection.Ascending));
		}

		protected void cAdmOvrideGrid_OnItemEditing(object sender, ListViewEditEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmOvrideGrid];
			if ((ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")) && dt != null && dt.DefaultView.Count > GetDataItemIndex(e.NewEditIndex))
			{
				cAdmOvrideGrid.EditIndex = e.NewEditIndex;
				cAdmOvrideGrid_DataBind(null);
				Control wc = null;
				string idx = Page.Request["__EVENTARGUMENT"];
				if (!string.IsNullOrEmpty(idx))
				{
				    Control ctrlToFocus = cAdmOvrideGrid.EditItem.FindControl("c" + idx);
				    if (((WebControl)ctrlToFocus).Enabled) { wc = ctrlToFocus; }
				}
				if (wc == null) { wc = FindEditableControl(cAdmOvrideGrid.EditItem); }
				if (wc != null)
             {
             ...
             MkMobileLabel(cAdmOvrideGrid.EditItem);
		    }
		    else { e.Cancel = true; }
		}
		...
		{
		    Label ml = null;
		    ml = lvi.FindControl("cOvrideGrpId123ml") as Label;
		    if (ml != null) { ml.Text = ColumnHeaderText(4); }
		    ml = lvi.FindControl("cUsrGroupId123ml") as Label;
		...
		protected void GridFill(ListViewItem lvi, DataTable dt, DataRow dr, bool bInsert)
		{
			...
			ddl = (DropDownList)lvi.FindControl("cUsrGroupId123");
			if (ddl != null)
			{
				if (ddl.SelectedIndex >= 0 && ddl.SelectedValue.ToString() != string.Empty)
				{
					dr["UsrGroupId123"] = ddl.SelectedValue;
					dr["UsrGroupId123Text"] = ddl.SelectedItem.Text;
				}
				else
				{
					dr["UsrGroupId123"] = System.DBNull.Value;
					dr["UsrGroupId123Text"] = System.DBNull.Value;
				}
			}
		...
		protected void cAdmOvrideGrid_OnItemUpdating(object sender, ListViewUpdateEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmOvrideGrid];
			if (dt != null && dt.Rows.Count > 0)
			{
				DataRow dr = dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row;
			    GridFill(cAdmOvrideGrid.EditItem, dt, dr, false);
			    dr["_NewRow"] = "N";
			    if (dt.Columns.Contains("_SortOrder")) dr["_SortOrder"] = null;
			}
		}

		protected void cAdmOvrideGrid_OnItemCanceling(object sender, ListViewCancelEventArgs e)
		{
			PanelTop.Update();
			DataTable dt = (DataTable)Session[KEY_dtAdmOvrideGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			...
			{
				cAdmOvrideGrid_OnItemDeleting(sender, new ListViewDeleteEventArgs(e.ItemIndex));
			}
			if (dt.Columns.Contains("_SortOrder")) dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row["_SortOrder"] = null;
			cAdmOvrideGrid.EditIndex = -1; cAdmOvrideGrid_DataBind(null);
		}
		...
		protected void cDeleteAllButton_Click(object sender, System.EventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmOvrideGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null && (cAdmOvrideGrid.EditIndex < 0 || UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				GotoPage(0);
				while (dv.Count > 0) {dv.Delete(0);}
				Session[KEY_dtAdmOvrideGrid] = dt;
				cAdmOvrideGrid.EditIndex = -1; cAdmOvrideGrid_DataBind(dv);
			}
		}

		protected void cAdmOvrideGrid_OnItemDeleting(object sender, ListViewDeleteEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmOvrideGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			DataRow dr = dv != null ? dv[GetDataItemIndex(e.ItemIndex)].Row : null;
			if (dv != null && (cAdmOvrideGrid.EditIndex == e.ItemIndex || UpdateGridRow(sender, new CommandEventArgs("Save",""))))
			{
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cOvrideId122.Text == string.Empty) { cClearButton_Click(sender, new EventArgs()); ShowDirty(false); PanelTop.Update();}
			else { PopAdmOvride78List(sender, e, false, cOvrideId122.Text); }
			// *** System Button Click (After) Web Rule starts here *** //
		}
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmOvride78List); PopAdmOvride78List(sender, e, false, null);
		...
		private string SaveDb(object sender, System.EventArgs e)
		{
			...
			if (ValidPage() && (cAdmOvrideGrid.EditIndex < 0 || UpdateGridRow(cAdmOvrideGrid, new CommandEventArgs("Save", ""))))
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmOvrideGrid];
				DataView dv = dt != null ? dt.DefaultView : null;
				string ftr = string.Empty;
				if (dv.RowFilter != string.Empty) { ftr = dv.RowFilter; dv.RowFilter = string.Empty; }
				AdmOvride78 ds = PrepAdmOvrideData(dv,cOvrideId122.Text == string.Empty);
				if (ftr != string.Empty) {dv.RowFilter = ftr;}
				if (!string.IsNullOrEmpty(cAdmOvride78List.SelectedValue) && cOvrideId122.Text != cAdmOvride78List.SelectedValue)
				{
					PreMsgPopup("Primary key cannot be changed");
					return rtn;
				}
				if (string.IsNullOrEmpty(cAdmOvride78List.SelectedValue))	// Add
				{
					if (ds != null)
					{
						pid = (new AdminSystem()).AddData(78,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc,noTrans,commandTimeOut);
					}
					if (!string.IsNullOrEmpty(pid))
					{
						cAdmOvride78List.ClearSearch(); Session.Remove(KEY_dtAdmOvride78List);
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmOvride78List(sender, e, false, pid);
						rtn = GetScreenHlp().Rows[0]["AddMsg"].ToString();
					}
				}
				else {
					bool bValid7 = false;
					if (ds != null && (new AdminSystem()).UpdData(78,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc,noTrans,commandTimeOut)) {bValid7 = true;}
					if (bValid7)
					{
						cAdmOvride78List.ClearSearch(); Session.Remove(KEY_dtAdmOvride78List);
						Session[KEY_currPageIndex] = GetCurrPageIndex();
						ShowDirty(false); PopAdmOvride78List(sender, e, false, ds.Tables["AdmOvride"].Rows[0]["OvrideId122"]);
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cOvrideId122.Text != string.Empty)
			{
				AdmOvride78 ds = PrepAdmOvrideData(null,false);
				try
				{
					if (ds != null && (new AdminSystem()).DelData(78,false,base.LUser,base.LImpr,base.LCurr,ds,null,null,base.CPrj,base.CSrc))
					{
						cAdmOvride78List.ClearSearch(); Session.Remove(KEY_dtAdmOvride78List);
						ShowDirty(false); PopAdmOvride78List(sender, e, false, null);
		...
		private AdmOvride78 PrepAdmOvrideData(DataView dv, bool bAdd)
		{
			...
			if (bAdd) { dr["OvrideId122"] = string.Empty; } else { dr["OvrideId122"] = cOvrideId122.Text; }
			drType["OvrideId122"] = "Numeric"; drDisp["OvrideId122"] = "TextBox";
			try {dr["OvrideName122"] = cOvrideName122.Text.Trim();} catch {}
			drType["OvrideName122"] = "VarWChar"; drDisp["OvrideName122"] = "TextBox";
			try {dr["PromptAlways122"] = base.SetBool(cPromptAlways122.Checked);} catch {}
			drType["PromptAlways122"] = "Char"; drDisp["PromptAlways122"] = "CheckBox";
			try {dr["PromptModal122"] = base.SetBool(cPromptModal122.Checked);} catch {}
			...
			if (dv != null)
			{
				...
				if (bAdd)
				{
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmOvrideAdd"].Rows.Add(MakeColRow(ds.Tables["AdmOvrideAdd"].NewRow(), drv, true));
					}
				}
				else
				{
					...
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmOvrideUpd"].Rows.Add(MakeColRow(ds.Tables["AdmOvrideUpd"].NewRow(), drv, false));
					}
					...
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmOvrideDel"].Rows.Add(MakeColRow(ds.Tables["AdmOvrideDel"].NewRow(), drv, false));
					}
		...
		public bool CanAct(char typ) { return CanAct(typ, GetAuthRow(), cAdmOvride78List.SelectedValue.ToString()); }
		...
		private bool ValidPage()
		{
			...
			DataTable dt = null;
			dt = (DataTable)Session[KEY_dtUsrGroupId123];
			if (dt != null && dt.Rows.Count <= 0)
			{
				bErrNow.Value = "Y"; PreMsgPopup("Value is expected for 'UsrGroupId', please investigate."); return false;
			}
			return true;
		}
		...
		{
			dr["OvrideId122"] = System.Data.OleDb.OleDbType.Numeric.ToString();
			dr["OvrideGrpId123"] = System.Data.OleDb.OleDbType.Numeric.ToString();
		...
		{
			dr["OvrideId122"] = "TextBox";
			dr["OvrideGrpId123"] = "TextBox";
		...
		{
			dr["OvrideId122"] = cOvrideId122.Text;
			DataTable dtAuth = GetAuthCol();
			if (dtAuth != null)
			{
				dr["OvrideGrpId123"] = drv["OvrideGrpId123"].ToString().Trim();
		...
		{
			if (cAdmOvrideGrid.EditIndex > -1)
			{
				cAdmOvrideGrid_OnItemUpdating(sender, new ListViewUpdateEventArgs(cAdmOvrideGrid.EditIndex));
			}
			return true;
		}

		protected void cAdmOvrideGrid_OnPreRender(object sender, System.EventArgs e)
		{
			System.Web.UI.WebControls.Image hi = null;
			hi = (System.Web.UI.WebControls.Image)cAdmOvrideGrid.FindControl("cOvrideGrpId123hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmOvrideGrid.FindControl("cUsrGroupId123hi"); if (hi != null) { hi.Visible = false; }
			if (Session[KEY_lastSortImg] != null)
			{
				hi = (System.Web.UI.WebControls.Image)cAdmOvrideGrid.FindControl((string)Session[KEY_lastSortImg]);
				if (hi != null) { hi.ImageUrl = Utils.AddTilde((string)Session[KEY_lastSortUrl]); hi.Visible = true; }
			}
			IgnoreHeaderConfirm((LinkButton)cAdmOvrideGrid.FindControl("cOvrideGrpId123hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmOvrideGrid.FindControl("cUsrGroupId123hl"));
		}

		private string GetButtonId(ListViewItem lvi)
		{
			string ButtonID = String.Empty;
			Control c = lvi.FindControl("cAdmOvrideGridEdit");
		...
		{
		    return cAdmOvrideGridDataPager.StartRowIndex / cAdmOvrideGridDataPager.PageSize;
		}

		private int GetTotalPages()
		{
		    return (int)Math.Ceiling((double)cAdmOvrideGridDataPager.TotalRowCount / cAdmOvrideGridDataPager.PageSize);
		}

		private int GetDataItemIndex(int editIndex)
		{
		    return cAdmOvrideGridDataPager.StartRowIndex + editIndex;
		}

		protected void cAdmOvrideGrid_OnLayoutCreated(object sender, EventArgs e)
		{
		    // Header:
		    LinkButton lb = null;
		    lb = cAdmOvrideGrid.FindControl("cOvrideGrpId123hl") as LinkButton;
		    if (lb != null) { lb.Text = ColumnHeaderText(4); lb.ToolTip = ColumnToolTip(4); lb.Parent.Visible = GridColumnVisible(4); }
		    lb = cAdmOvrideGrid.FindControl("cUsrGroupId123hl") as LinkButton;
			...
			if (dtAuthRow != null)
			{
				...
				{
		            lb = cAdmOvrideGrid.FindControl("cDeleteAllButton") as LinkButton; if (lb != null) { lb.Visible = false; }
				}
			}
		    // footer:
		    Label gc = null;
		    gc = cAdmOvrideGrid.FindControl("cOvrideGrpId123fl") as Label;
		    if (gc != null) { gc.Parent.Visible = GridColumnVisible(4); }
		    gc = cAdmOvrideGrid.FindControl("cUsrGroupId123fl") as Label;
		...
		private void PreMsgPopup(string msg) { PreMsgPopup(msg,cAdmOvride78List,null); }

...
namespace RO.Web
{
	public partial class AdmWizardObjModule : RO.Web.ModuleBase
	{

		private const string KEY_lastAddedRow = "Cache:lastAddedRow3_49";
		private const string KEY_lastSortOrd = "Cache:lastSortOrd3_49";
		private const string KEY_lastSortImg = "Cache:lastSortImg3_49";
		private const string KEY_lastSortCol = "Cache:lastSortCol3_49";
		private const string KEY_lastSortExp = "Cache:lastSortExp3_49";
		private const string KEY_lastSortUrl = "Cache:lastSortUrl3_49";
		private const string KEY_lastSortTog = "Cache:lastSortTog3_49";
		private const string KEY_lastImpPwdOvride = "Cache:lastImpPwdOvride3_49";
		private const string KEY_cntImpPwdOvride = "Cache:cntImpPwdOvride3_49";
		private const string KEY_currPageIndex = "Cache:currPageIndex3_49";
		private const string KEY_bHiImpVisible = "Cache:bHiImpVisible3_49";
		private const string KEY_bShImpVisible = "Cache:bShImpVisible3_49";
		private const string KEY_dtAdmWizardObjGrid = "Cache:dtAdmWizardObjGrid";
		private const string KEY_scrImport = "Cache:scrImport3_49";
		private const string KEY_dtScreenHlp = "Cache:dtScreenHlp3_49";
		private const string KEY_dtClientRule = "Cache:dtClientRule3_49";
		private const string KEY_dtAuthCol = "Cache:dtAuthCol3_49";
		private const string KEY_dtAuthRow = "Cache:dtAuthRow3_49";
		private const string KEY_dtLabel = "Cache:dtLabel3_49";
		private const string KEY_dtCriHlp = "Cache:dtCriHlp3_49";
		private const string KEY_dtScrCri = "Cache:dtScrCri3_49";
		private const string KEY_dsScrCriVal = "Cache:dsScrCriVal3_49";

		private const string KEY_dtWizardTypeId71 = "Cache:dtWizardTypeId71";
		private const string KEY_dtMasterTableId71 = "Cache:dtMasterTableId71";
		private const string KEY_dtColumnId72 = "Cache:dtColumnId72";

		private const string KEY_dtSystems = "Cache:dtSystems3_49";
		private const string KEY_sysConnectionString = "Cache:sysConnectionString3_49";
		private const string KEY_dtAdmWizardObj49List = "Cache:dtAdmWizardObj49List";
		private const string KEY_bNewVisible = "Cache:bNewVisible3_49";
		private const string KEY_bNewSaveVisible = "Cache:bNewSaveVisible3_49";
		private const string KEY_bCopyVisible = "Cache:bCopyVisible3_49";
		private const string KEY_bCopySaveVisible = "Cache:bCopySaveVisible3_49";
		private const string KEY_bDeleteVisible = "Cache:bDeleteVisible3_49";
		private const string KEY_bUndoAllVisible = "Cache:bUndoAllVisible3_49";
		private const string KEY_bUpdateVisible = "Cache:bUpdateVisible3_49";

		private const string KEY_bClCriVisible = "Cache:bClCriVisible3_49";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				DataTable dtMenuAccess = (new MenuSystem()).GetMenu(base.LUser.CultureId, 3, base.LImpr, LcSysConnString, LcAppPw,49, null, null);
				...
				Session.Remove(KEY_dtWizardTypeId71);
				Session.Remove(KEY_dtMasterTableId71);
				Session.Remove(KEY_dtColumnId72);
				SetButtonHlp();
				GetSystems();
				SetColumnAuthority();
				GetGlobalFilter();
				...
				try
				{
					(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ScreenTitle"].ToString(), 49, 0, 0, string.Empty, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				// *** Criteria Trigger (before) Web Rule starts here *** //
				PopAdmWizardObj49List(sender, e, true, null);
				if (cAuditButton.Attributes["OnClick"] == null || cAuditButton.Attributes["OnClick"].IndexOf("AdmScrAudit.aspx") < 0) { cAuditButton.Attributes["OnClick"] += "SearchLink('AdmScrAudit.aspx?cri0=49&typ=N&sys=3','','',''); return false;"; }
		...
		private void InitializeComponent()
		{
			...
			if (LcSysConnString == null)
			{
				...
				{
					DataTable dt;
					dt = (new AdminSystem()).GetAuthRow(49,base.LImpr.RowAuthoritys,LcSysConnString,LcAppPw);
					...
					{
						cAdmWizardObjGrid.PreRender += new EventHandler(cAdmWizardObjGrid_OnPreRender);
					}
				}
		...
		private void SetSystem(byte SystemId)
		{
			...
			try
			{
				...
				if ((Config.DeployType == "DEV" || row["dbAppDatabase"].ToString() == base.CPrj.EntityCode + "View") && !(base.CPrj.EntityCode != "RO" && row["SysProgram"].ToString() == "Y") && (new AdminSystem()).IsRegenNeeded(string.Empty,49,0,0,LcSysConnString,LcAppPw))
				{
					(new GenScreensSystem()).CreateProgram(49, "Wizard Object", row["dbAppDatabase"].ToString(), base.CPrj, base.CSrc, base.CTar, LcAppConnString, LcAppPw);
		...
		private void SetButtonHlp()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetButtonHlp(49,0,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				try
				{
					dtRul = (new AdminSystem()).GetClientRule(49,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void SetClientRule(ListViewDataItem ee, bool isEditItem)
		{
			...
			if (dvRul.Count > 0)
			{
				...
				{
					if (ee == null || drv["ScriptEvent"].ToString().Substring(0,2).ToLower() != "on" || (cAdmWizardObjGrid.EditIndex > -1 && GetDataItemIndex(cAdmWizardObjGrid.EditIndex) == ee.DataItemIndex))
					{
		...
		private DataTable GetScreenCriHlp()
		{
			...
			if (dtCriHlp == null)
			{
				...
				try
				{
					dtCriHlp = (new AdminSystem()).GetScreenCriHlp(49,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetScreenHlp()
		{
			...
			if (dtHlp == null)
			{
				...
				try
				{
					dtHlp = (new AdminSystem()).GetScreenHlp(49,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void GetGlobalFilter()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetGlobalFilter(base.LUser.UsrId,49,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null && dt.Rows.Count > 0)
				{
		...
		private void GetScreenFilter()
		{
			try
			{
				DataTable dt = (new AdminSystem()).GetScreenFilter(49,base.LUser.CultureId,LcSysConnString,LcAppPw);
				if (dt != null)
				{
		...
		private DataTable GetLabel()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetScreenLabel(49,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private DataTable GetAuthCol()
		{
			...
			if (dt == null)
			{
				try
				{
					dt = (new AdminSystem()).GetAuthCol(49,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		...
		private void getReport(string eExport)
		{
			try
			{
				...
				try
				{
					try {dv = new DataView((new AdminSystem()).GetExp(49,"GetExpAdmWizardObj49","Y",(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
					catch {dv = new DataView((new AdminSystem()).GetExp(49,"GetExpAdmWizardObj49","N",(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false)));}
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (eExport == "TXT")
				{
					DataTable dtAu;
					dtAu = (new AdminSystem()).GetAuthExp(49,base.LUser.CultureId,base.LImpr,base.LCurr,LcSysConnString,LcAppPw);
					if (dtAu != null)
					{
					...
					{
						if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["WizardId71"].ToString(),base.LUser.Culture) + (char)9);}
						if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["WizardTypeId71"].ToString() + (char)9 + drv["WizardTypeId71Text"].ToString() + (char)9);}
						if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(drv["MasterTableId71"].ToString() + (char)9 + drv["MasterTableId71Text"].ToString() + (char)9);}
						if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["WizardTitle71"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["ProgramName71"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append("\"" + drv["DefWorkSheet71"].ToString().Replace("\"","\"\"") + "\"" + (char)9);}
						if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["DefStartRow71"].ToString(),base.LUser.Culture) + (char)9);}
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmWizardObj.csv"; Session["ExportStr"] = (Config.ExportExcelCSV ? "sep=\t\n": "") + sb.Replace("\r\n","\n");
				}
				else if (eExport == "RTF")
				{
...
\par }}");
					...
					bExpNow.Value = "Y"; Session["ExportFnm"] = "AdmWizardObj.rtf"; Session["ExportStr"] = sb;
				}
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
		}
		...
		private string GetRtfGrid(DataView dv)
		{
			...
			try
			{
				...
				foreach (DataRowView drv in dv)
				{
					...
					sb.Append("\r\n");
					sb.Append(@"{");
					if (dtAu.Rows[0]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["WizardId71"].ToString(),base.LUser.Culture) + @"\cell ");}
					if (dtAu.Rows[1]["ColExport"].ToString() == "Y") {sb.Append(drv["WizardTypeId71Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[2]["ColExport"].ToString() == "Y") {sb.Append(drv["MasterTableId71Text"].ToString() + @"\cell ");}
					if (dtAu.Rows[3]["ColExport"].ToString() == "Y") {sb.Append(drv["WizardTitle71"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[4]["ColExport"].ToString() == "Y") {sb.Append(drv["ProgramName71"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[5]["ColExport"].ToString() == "Y") {sb.Append(drv["DefWorkSheet71"].ToString().Replace("\r\n",@"\par ") + @"\cell ");}
					if (dtAu.Rows[6]["ColExport"].ToString() == "Y") {sb.Append(RO.Common3.Utils.fmNumeric("0",drv["DefStartRow71"].ToString(),base.LUser.Culture) + @"\cell ");}
		...
		private void GetPageInfo()
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastPageInfo(49, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		private void GetCriteria(DataView dvCri)
		{
			...
			{
				DataTable dt;
				dt = (new AdminSystem()).GetLastCriteria(int.Parse(dvCri.Count.ToString()) + 1, 49, 0, base.LUser.UsrId, LcSysConnString, LcAppPw);
		...
		protected void cCriButton_Click(object sender, System.EventArgs e)
		{
			// *** Criteria Trigger (before) Web Rule starts here *** //
			DataTable dt = (DataTable)Session[KEY_dtAdmWizardObjGrid];
			if (cAdmWizardObjGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
			...
			PopAdmWizardObj49List(sender, e, false, null);
			}
		}
			...
			try
			{
				foreach (DataRowView drv in dvCri)
				{
					...
					if (drv["DisplayName"].ToString() == "ComboBox")
					{
						...
						(new AdminSystem()).MkGetScreenIn("49", drv["ScreenCriId"].ToString(), "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), LcAppDb, LcDesDb, drv["MultiDesignDb"].ToString(), LcSysConnString, LcAppPw);
						DataView dv = new DataView((new AdminSystem()).GetScreenIn("49", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), (new AdminSystem()).CountScrCri(drv["ScreenCriId"].ToString(), drv["MultiDesignDb"].ToString(), drv["MultiDesignDb"].ToString() == "N" ? LcSysConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)), drv["RequiredValid"].ToString(), 0, string.Empty, drv["DisplayMode"].ToString() != "AutoListBox", val, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId)));
		...
		private DataView GetScrCriteria()
		{
		    ...
		    if (dtScrCri == null)
		    {
				try
				{
					dtScrCri = (new AdminSystem()).GetScrCriteria("49", LcSysConnString, LcAppPw);
				}
		...
		private void MakeScrGrpInp(DataRowView drv)
		{
		    ...
		    else if (drv["DisplayName"].ToString() == "ListBox")
		    {
		        ...
		        if (drv["DisplayMode"].ToString() == "AutoListBox")
		        {
		            ...
		            context["refCol"] = wcFtr != null ? drv["DdlFtrColumnName"].ToString() : null;
		            context["refColDataType"] = wcFtr != null ? drv["DdlFtrDataType"].ToString() : null;
		            Session["AdmWizardObj49" + cListBox.ID] = context;
		            cListBox.Attributes["ac_context"] = "AdmWizardObj49" + cListBox.ID;
		...
		private DataSet UpdCriteria(bool bUpdate)
		{
			...
			foreach (DataRowView drv in dvCri)
			{
			    if (drv["DisplayName"].ToString() == "ListBox")
			    {
					...
					if (cListBox != null)
					{
						...
						int TotalChoiceCnt = new DataView((new AdminSystem()).GetScreenIn("49", "GetDdl" + drv["ColumnName"].ToString() + LCurr.SystemId.ToString() + "C" + drv["ScreenCriId"].ToString(), CriCnt, drv["RequiredValid"].ToString(), 0, string.Empty, true, string.Empty, base.LImpr, base.LCurr, drv["MultiDesignDb"].ToString() == "N" ? LcAppConnString : (string)Session[KEY_sysConnectionString], base.AppPwd(LCurr.DbId))).Count;
						string selectedValues = string.Join(",", cListBox.Items.Cast<ListItem>().Where(x => x.Selected).Select(x => "'" + x.Value + "'").ToArray());
			...
			if (bUpdate) 
			{
				try
				{
					(new AdminSystem()).UpdScrCriteria("49", "AdmWizardObj49", dvCri, base.LUser.UsrId, cCriteria.Visible, ds, LcAppConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		...
		private void SetWizardTypeId71(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtWizardTypeId71];
			...
			if (ddl != null)
			{
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(49,"GetDdlWizardTypeId3S634",true,bAll,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					if (dv.Table.Columns.Contains("WizardId"))
					{
						ss = "(WizardId is null";
						if (string.IsNullOrEmpty(cAdmWizardObj49List.SelectedValue)) {ss = ss + ")";} else {ss = ss + " OR WizardId = " + cAdmWizardObj49List.SelectedValue + ")";}
					}
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "WizardTypeId71 is not NULL"; }
					dv.RowFilter = ss;
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(49,"GetDdlWizardTypeId3S634",true,true,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtWizardTypeId71] = dv.Table;
				}
			}
		}

		private void SetMasterTableId71(DropDownList ddl, string keyId)
		{
			DataTable dt = (DataTable)Session[KEY_dtMasterTableId71];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (ddl != null)
			{
				string ss = string.Empty;
				...
				if (dv == null)
				{
					...
					try
					{
						dv = new DataView((new AdminSystem()).GetDdl(49,"GetDdlMasterTableId3S635",true,bAll,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
					}
					catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				}
				if (dv != null)
				{
					...
					if (!string.IsNullOrEmpty(keyId) && !ddl.Enabled) { ss = ss + (string.IsNullOrEmpty(ss) ? string.Empty : " AND ") + "MasterTableId71 is not NULL"; }
					dv.RowFilter = ss;
					ddl.DataSource = dv; ddl.DataBind();
					li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					else if (!bFirst)
					...
					if (dv.Count <= 1 && keyId != string.Empty)	// In case invalid value casued by copy action.
					{
						try
						{
							dv = new DataView((new AdminSystem()).GetDdl(49,"GetDdlMasterTableId3S635",true,true,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
						}
						catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
						dv.RowFilter = ss; ddl.DataSource = dv; ddl.DataBind();
						li = ddl.Items.FindByValue(keyId); if (li != null) {li.Selected = true;}
					}
					Session[KEY_dtMasterTableId71] = dv.Table;
				}
			}
		}

		protected void cbColumnId72(object sender, System.EventArgs e)
		{
			SetColumnId72((RoboCoder.WebControls.ComboBox)sender,string.Empty,cAdmWizardObjGrid.EditItem);
		}

		private void SetColumnId72(RoboCoder.WebControls.ComboBox ddl, string keyId, ListViewItem li)
		{
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			if (ddl != null)
			{
				...
				try
				{
					dv = new DataView((new AdminSystem()).GetDdl(49,"GetDdlColumnId3S642",true,false,0,keyId,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr));
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (dv != null)
				{
					if (dv.Table.Columns.Contains("WizardId"))
					{
						context["pMKeyColID"] = cAdmWizardObj49List.ClientID;
						context["pMKeyCol"] = "WizardId";
						string ss = "(WizardId is null";
						if (string.IsNullOrEmpty(cAdmWizardObj49List.SelectedValue)) {ss = ss + ")";} else {ss = ss + " OR WizardId = " + cAdmWizardObj49List.SelectedValue + ")";}
						dv.RowFilter = ss;
					}
					ddl.DataSource = dv; Session[KEY_dtColumnId72] = dv.Table;
				    try { ddl.SelectByValue(keyId,string.Empty,true); } catch { try { ddl.SelectedIndex = 0; } catch { } }
				}
			}
		}

		private DataView GetAdmWizardObj49List()
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmWizardObj49List];
			if (dt == null)
			{
				CheckAuthentication(false);
				int filterId = 0;
				...
				try
				{
					try {dt = (new AdminSystem()).GetLis(49,"GetLisAdmWizardObj49",true,"Y",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
					catch {dt = (new AdminSystem()).GetLis(49,"GetLisAdmWizardObj49",true,"N",0,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,key,string.Empty,GetScrCriteria(),base.LImpr,base.LCurr,UpdCriteria(false));}
				}
		...
		private void PopAdmWizardObj49List(object sender, System.EventArgs e, bool bPageLoad, object ID)
		{
			DataView dv = GetAdmWizardObj49List();
			System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
			...
			context["isSys"] = "N";
			context["conn"] = KEY_sysConnectionString;
			cAdmWizardObj49List.AutoCompleteUrl = "AutoComplete.aspx/LisSuggests";
			cAdmWizardObj49List.DataContext = context;
			if (dv.Table == null) return;
			cAdmWizardObj49List.DataSource = dv;
			cAdmWizardObj49List.Visible = true;
			if (cAdmWizardObj49List.Items.Count <= 0) {cAdmWizardObj49List.Visible = false; cAdmWizardObj49List_SelectedIndexChanged(sender,e);}
			else
			{
				if (bPageLoad && !string.IsNullOrEmpty(Request.QueryString["key"]) && bUseCri.Value == string.Empty)
				{
					try {cAdmWizardObj49List.SelectByValue(Request.QueryString["key"],string.Empty,true);} catch {cAdmWizardObj49List.Items[0].Selected = true; cAdmWizardObj49List_SelectedIndexChanged(sender, e);}
				    cScreenSearch.Visible = false; cSystem.Visible = false; cEditButton.Visible = true; cSaveCloseButton.Visible = cSaveButton.Visible;
				}
				else
				{
					if (ID != null) {if (!cAdmWizardObj49List.SelectByValue(ID,string.Empty,true)) {ID = null;}}
					if (ID == null)
					{
						cAdmWizardObj49List_SelectedIndexChanged(sender, e);
					}
				}
			}
		}
		...
		private void SetColumnAuthority()
		{
			...
			{
				base.SetFoldBehavior(cWizardId71, dtAuth.Rows[0], cWizardId71P1, cWizardId71Label, cWizardId71P2, null, dtLabel.Rows[0], null, null, null);
				base.SetFoldBehavior(cWizardTypeId71, dtAuth.Rows[1], cWizardTypeId71P1, cWizardTypeId71Label, cWizardTypeId71P2, null, dtLabel.Rows[1], cRFVWizardTypeId71, null, null);
				base.SetFoldBehavior(cMasterTableId71, dtAuth.Rows[2], cMasterTableId71P1, cMasterTableId71Label, cMasterTableId71P2, null, dtLabel.Rows[2], cRFVMasterTableId71, null, null);
				base.SetFoldBehavior(cWizardTitle71, dtAuth.Rows[3], cWizardTitle71P1, cWizardTitle71Label, cWizardTitle71P2, null, dtLabel.Rows[3], cRFVWizardTitle71, null, null);
				base.SetFoldBehavior(cProgramName71, dtAuth.Rows[4], cProgramName71P1, cProgramName71Label, cProgramName71P2, null, dtLabel.Rows[4], cRFVProgramName71, null, null);
				base.SetFoldBehavior(cDefWorkSheet71, dtAuth.Rows[5], cDefWorkSheet71P1, cDefWorkSheet71Label, cDefWorkSheet71P2, null, dtLabel.Rows[5], cRFVDefWorkSheet71, null, null);
				base.SetFoldBehavior(cDefStartRow71, dtAuth.Rows[6], cDefStartRow71P1, cDefStartRow71Label, cDefStartRow71P2, null, dtLabel.Rows[6], cRFVDefStartRow71, null, cRVDefStartRow71);
			...
			if ((cWizardTitle71.Attributes["OnChange"] == null || cWizardTitle71.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cWizardTitle71.Visible && !cWizardTitle71.ReadOnly) {cWizardTitle71.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cProgramName71.Attributes["OnChange"] == null || cProgramName71.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cProgramName71.Visible && !cProgramName71.ReadOnly) {cProgramName71.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cDefWorkSheet71.Attributes["OnChange"] == null || cDefWorkSheet71.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cDefWorkSheet71.Visible && !cDefWorkSheet71.ReadOnly) {cDefWorkSheet71.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
			if ((cDefStartRow71.Attributes["OnChange"] == null || cDefStartRow71.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cDefStartRow71.Visible && !cDefStartRow71.ReadOnly) {cDefStartRow71.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
		...
		private void ClearMaster(object sender, System.EventArgs e)
		{
			...
			if (dt.Rows[1]["ColVisible"].ToString() == "Y" && dt.Rows[1]["ColReadOnly"].ToString() != "Y") {SetWizardTypeId71(cWizardTypeId71,string.Empty);}
			if (dt.Rows[2]["ColVisible"].ToString() == "Y" && dt.Rows[2]["ColReadOnly"].ToString() != "Y") {SetMasterTableId71(cMasterTableId71,string.Empty);}
			if (dt.Rows[3]["ColVisible"].ToString() == "Y" && dt.Rows[3]["ColReadOnly"].ToString() != "Y") {cWizardTitle71.Text = string.Empty;}
			if (dt.Rows[4]["ColVisible"].ToString() == "Y" && dt.Rows[4]["ColReadOnly"].ToString() != "Y") {cProgramName71.Text = string.Empty;}
			if (dt.Rows[5]["ColVisible"].ToString() == "Y" && dt.Rows[5]["ColReadOnly"].ToString() != "Y") {cDefWorkSheet71.Text = "Sheet1";}
			if (dt.Rows[6]["ColVisible"].ToString() == "Y" && dt.Rows[6]["ColReadOnly"].ToString() != "Y") {cDefStartRow71.Text = "2";}
		...
		{
			cWizardId71.Text = string.Empty;
			SetWizardTypeId71(cWizardTypeId71,string.Empty);
			SetMasterTableId71(cMasterTableId71,string.Empty);
			cWizardTitle71.Text = string.Empty;
			cProgramName71.Text = string.Empty;
			cDefWorkSheet71.Text = "Sheet1";
			cDefStartRow71.Text = "2";
		...
		protected void cAdmWizardObj49List_DDFindClick(object sender, System.EventArgs e)
		{
			ScriptManager.GetCurrent(Parent.Page).SetFocus(((RoboCoder.WebControls.ComboBox)sender).FocusID);
		}

		protected void cAdmWizardObj49List_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			InitMaster(sender, e);      // Do this first to enable defaults for non-database columns.
			DataTable dt = null;
			try
			{
				dt = (new AdminSystem()).GetMstById("GetAdmWizardObj49ById",cAdmWizardObj49List.SelectedValue,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId));
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (dt != null)
			{
				...
				if (dt.Rows.Count > 0)
				{
					cAdmWizardObj49List.SetDdVisible();
					DataRow dr = dt.Rows[0];
					try {cWizardId71.Text = RO.Common3.Utils.fmNumeric("0",dr["WizardId71"].ToString(),base.LUser.Culture);} catch {cWizardId71.Text = string.Empty;}
					SetWizardTypeId71(cWizardTypeId71,dr["WizardTypeId71"].ToString());
					SetMasterTableId71(cMasterTableId71,dr["MasterTableId71"].ToString());
					try {cWizardTitle71.Text = dr["WizardTitle71"].ToString();} catch {cWizardTitle71.Text = string.Empty;}
					try {cProgramName71.Text = dr["ProgramName71"].ToString();} catch {cProgramName71.Text = string.Empty;}
					try {cDefWorkSheet71.Text = dr["DefWorkSheet71"].ToString();} catch {cDefWorkSheet71.Text = string.Empty;}
					try {cDefStartRow71.Text = RO.Common3.Utils.fmNumeric("0",dr["DefStartRow71"].ToString(),base.LUser.Culture);} catch {cDefStartRow71.Text = string.Empty;}
			...
			DataTable dtAdmWizardObjGrid = null;
			int filterId = 0; if (Utils.IsInt(cFilterId.SelectedValue)) { filterId = int.Parse(cFilterId.SelectedValue); }
			try
			{
				dtAdmWizardObjGrid = (new AdminSystem()).GetDtlById(49,"GetAdmWizardObj49DtlById",cAdmWizardObj49List.SelectedValue,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),filterId,base.LImpr,base.LCurr);
			}
			catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
			if (!dtAdmWizardObjGrid.Columns.Contains("_NewRow")) { dtAdmWizardObjGrid.Columns.Add("_NewRow"); }
			Session[KEY_dtAdmWizardObjGrid] = dtAdmWizardObjGrid;
			cAdmWizardObjGrid.EditIndex = -1;
			cAdmWizardObjGridDataPager.PageSize = Int16.Parse(cPgSize.Text); GotoPage(0);
			if (Session[KEY_lastSortCol] != null)
			{
				cAdmWizardObjGrid_OnSorting(sender, new ListViewSortEventArgs((string)Session[KEY_lastSortExp], SortDirection.Ascending));
			}
			else if (dtAdmWizardObjGrid.Rows.Count <= 0 || (!((GetAuthRow().Rows[0]["AllowUpd"].ToString() == "N" || GetAuthRow().Rows[0]["ViewOnly"].ToString() == "G") && GetAuthRow().Rows[0]["AllowAdd"].ToString() == "N") && !(Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N") && dtAdmWizardObjGrid.Rows.Count == 1))
			{
				cAdmWizardObjGrid_DataBind(dtAdmWizardObjGrid.DefaultView);
			}
			cFindButton_Click(sender, e);
			cNaviPanel.Visible = true; cImportPwdPanel.Visible = false; Session.Remove(KEY_scrImport);
			ScriptManager.GetCurrent(Parent.Page).SetFocus(cAdmWizardObj49List.FocusID);
		...
		public void cFindButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dv != null)
			{
				...
				if ((cc != null && cc.ID.Equals("cAdmWizardObj49List")) || cAdmWizardObjGrid.EditIndex < 0 || UpdateGridRow(cAdmWizardObjGrid, new CommandEventArgs("Save", "")))
				{
					...
					if (GetCurrPageIndex() != (int)Session[KEY_currPageIndex] && (int)Session[KEY_currPageIndex] < GetTotalPages()) { GotoPage((int)Session[KEY_currPageIndex]); cAdmWizardObjGrid_DataBind(dv);}
		...
		public void cInsRowButton_Click(object sender, System.EventArgs e)
		{
			...
			if (dt != null && (ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")))
			{
				...
				Session[KEY_lastAddedRow] = 0; Session[KEY_dtAdmWizardObjGrid] = dt; Session[KEY_currPageIndex] = 0; GotoPage(0);
				cAdmWizardObjGrid_DataBind(dt.DefaultView);
				cAdmWizardObjGrid_OnItemEditing(cAdmWizardObjGrid, new ListViewEditEventArgs(0));
		...
		public void cPgSizeButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cAdmWizardObjGrid.EditIndex < 0 || (dt != null && UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				...
				cAdmWizardObjGridDataPager.PageSize = int.Parse(cPgSize.Text); GotoPage(0);
				if (dt.Rows.Count <= 0 || (!((GetAuthRow().Rows[0]["AllowUpd"].ToString() == "N" || GetAuthRow().Rows[0]["ViewOnly"].ToString() == "G") && GetAuthRow().Rows[0]["AllowAdd"].ToString() == "N") && !(Request.QueryString["enb"] != null && Request.QueryString["enb"] == "N") && dt.Rows.Count == 1)) {cAdmWizardObjGrid_DataBind(dt.DefaultView);} else {cFindButton_Click(sender, e);}
				try
				{
					(new AdminSystem()).UpdLastPageInfo(49, base.LUser.UsrId, cPgSize.Text, LcSysConnString, LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); }
			}
		}
		...
		public void cShowImpButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cSchemaImage.Visible)
			{
				try
				{
					Session["ImportSchema"] = (new AdminSystem()).GetSchemaScrImp(49,base.LUser.CultureId,LcSysConnString,LcAppPw);
				}
				catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
				if (cSchemaImage.Attributes["OnClick"] == null || cSchemaImage.Attributes["OnClick"].IndexOf("ImportSchema.aspx") < 0) {cSchemaImage.Attributes["OnClick"] += "SearchLink('ImportSchema.aspx?scm=S&key=49&csy=3','','',''); return false;";}
			}
			// *** System Button Click (after) Web Rule starts here *** //
		}
		...
		private void ScrImportDdl(DataView dvd, DataRowView drv, bool bComboBox, string PKey, string CKey, string CNam, int MaxLen, string MatchCd, bool bAllowNulls)
		{
			if (dvd != null)
			{
				...
				{
					if (string.IsNullOrEmpty(cAdmWizardObj49List.SelectedValue))
					{
						dvd.RowFilter = "(" + PKey + " is null)";
					}
					else
					{
						dvd.RowFilter = "(" + PKey + " is null OR " + PKey + " = " + cAdmWizardObj49List.SelectedValue + ")";
					}
				}
		...
		protected void ScrImport(bool bClear)
		{
			...
			DataTable dtAdmWizardObjGrid = (DataTable)Session[KEY_dtAdmWizardObjGrid];
			if (dti != null && dtAdmWizardObjGrid != null)
			{
				if (bClear)
				{
					dtAdmWizardObjGrid.Rows.Clear();
				}
				cFind.Text = string.Empty;
				int iExist = dtAdmWizardObjGrid.Rows.Count;
				...
				foreach (DataRowView drv in dti.DefaultView)
				{
					...
					{
						DataTable dtd;
						dtd = jj==iExist ? null : (DataTable)Session[KEY_dtColumnId72];
						if (dtd == null)
						{
							try
							{
								dtd = (new AdminSystem()).GetDdl(49,"GetDdlColumnId3S642",true,true,0,string.Empty,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),string.Empty,base.LImpr,base.LCurr);
							}
							catch (Exception err) { bErrNow.Value = "Y"; PreMsgPopup(err.Message); return; }
							Session[KEY_dtColumnId72] = dtd;
						}
						ScrImportDdl(dtd.DefaultView, drv, true, "WizardId", "ColumnId72", "ColumnId72Text", 100, "2", true);
					}
					jj = jj + 1;
				}
				iRow = dti.DefaultView.Count - iExist;
				if (!dti.Columns.Contains("_NewRow")) { dti.Columns.Add("_NewRow"); }
				Session[KEY_dtAdmWizardObjGrid] = dti;
				cAdmWizardObjGrid_DataBind(dti.DefaultView);
		...
		public void cImportButton_Click(object sender, System.EventArgs e)
		{
			...
			if (cFNameO.Text != string.Empty && cWorkSheet.Items.Count > 0 && cWorkSheet.SelectedItem.Text != string.Empty && cStartRow.Text != string.Empty)
			{
				...
				try
				{
					if (cAdmWizardObjGrid.EditIndex < 0 || UpdateGridRow(cAdmWizardObjGrid, new CommandEventArgs("Save", "")))
					{
						Session[KEY_scrImport] = ImportFile(dtAdmWizardObjGrid.Copy(),cFNameO.Text,cWorkSheet.SelectedItem.Text,cStartRow.Text,Config.PathTmpImport + cFName.Text);
					}
		...
		private DataTable ImportFile(DataTable dt, string fileName, string workSheet, string startRow, string fileFullName)
		{
			try
			{
				...
				{
					if (rows[iRow][3].ToString() == string.Empty) {idel = idel + 1;}
					else
					{
		...
		private void GotoPage(int pageNo)
		{
			if (pageNo >= 0 && pageNo < GetTotalPages())
			{
				if (cAdmWizardObjGrid.EditIndex < 0 || UpdateGridRow(cAdmWizardObjGrid, new CommandEventArgs("Save", "")))
				{
					try { cAdmWizardObjGridDataPager.SetPageProperties(cAdmWizardObjGridDataPager.PageSize * pageNo, cAdmWizardObjGridDataPager.MaximumRows, false); cAdmWizardObjGrid_DataBind(null); }
					catch
					{
						try { cAdmWizardObjGridDataPager.SetPageProperties(0, cAdmWizardObjGridDataPager.MaximumRows, false); cAdmWizardObjGrid_DataBind(null); } catch {}
					}
				}
			}
		}

		protected void cAdmWizardObjGrid_OnItemCommand(object sender, ListViewCommandEventArgs e)
		{
		}

		private void cAdmWizardObjGrid_DataBind(DataView dvAdmWizardObjGrid)
		{
			if (dvAdmWizardObjGrid == null)
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmWizardObjGrid];
				dvAdmWizardObjGrid = dt != null ? dt.DefaultView : null;
			}
			if (dvAdmWizardObjGrid != null)
			{
				LcAuth = GetAuthCol().AsEnumerable().ToDictionary<DataRow,string>(dr=>dr["ColName"].ToString());
				cAdmWizardObjGrid.DataSource = dvAdmWizardObjGrid;
				cAdmWizardObjGrid.DataBind();
		...
		protected void cAdmWizardObjGrid_OnSorting(object sender, ListViewSortEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmWizardObjGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null)
			{
				if (cAdmWizardObjGrid.EditIndex < 0 || UpdateGridRow(cAdmWizardObjGrid, new CommandEventArgs("Save", "")))
				{
					...
					Session[KEY_dtAdmWizardObjGrid] = dt;
					cAdmWizardObjGrid_DataBind(dv);
				}
			}
		}

		protected void cAdmWizardObjGrid_OnPagePropertiesChanging(object sender, PagePropertiesChangingEventArgs e)
		{
		    cAdmWizardObjGridDataPager.SetPageProperties(e.StartRowIndex, e.MaximumRows, false);
		    DataTable dt = (DataTable)Session[KEY_dtAdmWizardObjGrid];
		    DataView dv = dt != null ? dt.DefaultView : null;
			cAdmWizardObjGrid_DataBind(dv); cAdmWizardObjGrid.EditIndex = -1;
		}

		private void GridChkPgDirty(ListViewItem lvi)
		{
				WebControl cc = null;
				cc = ((WebControl)lvi.FindControl("cColumnId72"));
				...
				cc = ((WebControl)lvi.FindControl("cTabIndex72"));
				if ((cc.Attributes["OnChange"] == null || cc.Attributes["OnChange"].IndexOf("ChkPgDirty") < 0) && cc.Visible && cc.Enabled) {cc.Attributes["OnChange"] += "document.getElementById('" + bPgDirty.ClientID + "').value='Y'; ChkPgDirty();";}
		}

		protected void cAdmWizardObjGrid_OnItemDataBound(object sender, ListViewItemEventArgs e)
		{
			...
			if (dtAuthRow != null)
			{
				...
				{
					cc = e.Item.FindControl("cAdmWizardObjGridDelete"); if (cc != null) { cc.Visible = false; }
					cc = e.Item.FindControl("cAdmWizardObjGridEdit"); if (cc != null) { cc.Visible = false; }
				}
				else
				{
			        HtmlTableRow tr = e.Item.FindControl("cAdmWizardObjGridRow") as HtmlTableRow;
			        LinkButton lb = e.Item.FindControl("cAdmWizardObjGridEdit") as LinkButton;
		...
		protected void cWizardObjId72hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "0") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "0"; Session[KEY_lastSortExp] = "WizardObjId72";Session[KEY_lastSortImg] = "cWizardObjId72hi";
			cAdmWizardObjGrid_OnSorting(sender, new ListViewSortEventArgs("WizardObjId72", SortDirection.Ascending));
		}

		protected void cColumnId72hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "1") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "1"; Session[KEY_lastSortExp] = "ColumnId72Text";Session[KEY_lastSortImg] = "cColumnId72hi";
			cAdmWizardObjGrid_OnSorting(sender, new ListViewSortEventArgs("ColumnId72Text", SortDirection.Ascending));
		}

		protected void cTabIndex72hl_Click(object sender, System.EventArgs e)
		{
			if (Session[KEY_lastSortCol] == null || (string)Session[KEY_lastSortCol] != "2") { Session.Remove(KEY_lastSortUrl); }
			Session[KEY_lastSortTog] = "Y"; Session[KEY_lastSortCol] = "2"; Session[KEY_lastSortExp] = "TabIndex72";Session[KEY_lastSortImg] = "cTabIndex72hi";
			cAdmWizardObjGrid_OnSorting(sender, new ListViewSortEventArgs("TabIndex72", SortDirection.Ascending));
		}

		protected void cAdmWizardObjGrid_OnItemEditing(object sender, ListViewEditEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmWizardObjGrid];
			if ((ValidPage()) && UpdateGridRow(sender, new CommandEventArgs("Save", "")) && dt != null && dt.DefaultView.Count > GetDataItemIndex(e.NewEditIndex))
			{
				cAdmWizardObjGrid.EditIndex = e.NewEditIndex;
				cAdmWizardObjGrid_DataBind(null);
				Control wc = null;
				string idx = Page.Request["__EVENTARGUMENT"];
				if (!string.IsNullOrEmpty(idx))
				{
				    Control ctrlToFocus = cAdmWizardObjGrid.EditItem.FindControl("c" + idx);
				    if (((WebControl)ctrlToFocus).Enabled) { wc = ctrlToFocus; }
				}
				if (wc == null) { wc = FindEditableControl(cAdmWizardObjGrid.EditItem); }
				if (wc != null)
             {
             ...
             MkMobileLabel(cAdmWizardObjGrid.EditItem);
		    }
		    else { e.Cancel = true; }
		}
		...
		protected void GridFill(ListViewItem lvi, DataTable dt, DataRow dr, bool bInsert)
		{
			...
			cbb = (RoboCoder.WebControls.ComboBox)lvi.FindControl("cColumnId72");
			if (cbb != null)
			{
				if (cbb.SelectedValue != string.Empty)
				{
					dr["ColumnId72"] = cbb.SelectedValue;
					dr["ColumnId72Text"] = cbb.SelectedText;
				}
				else
				{
					dr["ColumnId72"] = System.DBNull.Value;
					dr["ColumnId72Text"] = System.DBNull.Value;
				}
			}
			TextBox tb = null;
			tb = (TextBox)lvi.FindControl("cTabIndex72");
			if (tb != null)
			{
				if (tb.Text != string.Empty) {dr["TabIndex72"] = tb.Text;} else {dr["TabIndex72"] = System.DBNull.Value;}
			}
		...
		protected void cAdmWizardObjGrid_OnItemUpdating(object sender, ListViewUpdateEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmWizardObjGrid];
			if (dt != null && dt.Rows.Count > 0)
			{
				DataRow dr = dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row;
			    GridFill(cAdmWizardObjGrid.EditItem, dt, dr, false);
			    dr["_NewRow"] = "N";
			    if (dt.Columns.Contains("_SortOrder")) dr["_SortOrder"] = null;
			}
		}

		protected void cAdmWizardObjGrid_OnItemCanceling(object sender, ListViewCancelEventArgs e)
		{
			PanelTop.Update();
			DataTable dt = (DataTable)Session[KEY_dtAdmWizardObjGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			...
			{
				cAdmWizardObjGrid_OnItemDeleting(sender, new ListViewDeleteEventArgs(e.ItemIndex));
			}
			if (dt.Columns.Contains("_SortOrder")) dt.DefaultView[GetDataItemIndex(e.ItemIndex)].Row["_SortOrder"] = null;
			cAdmWizardObjGrid.EditIndex = -1; cAdmWizardObjGrid_DataBind(null);
		}
		...
		protected void cDeleteAllButton_Click(object sender, System.EventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmWizardObjGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			if (dv != null && (cAdmWizardObjGrid.EditIndex < 0 || UpdateGridRow(sender, new CommandEventArgs("Save", ""))))
			{
				GotoPage(0);
				while (dv.Count > 0) {dv.Delete(0);}
				Session[KEY_dtAdmWizardObjGrid] = dt;
				cAdmWizardObjGrid.EditIndex = -1; cAdmWizardObjGrid_DataBind(dv);
			}
		}

		protected void cAdmWizardObjGrid_OnItemDeleting(object sender, ListViewDeleteEventArgs e)
		{
			DataTable dt = (DataTable)Session[KEY_dtAdmWizardObjGrid];
			DataView dv = dt != null ? dt.DefaultView : null;
			DataRow dr = dv != null ? dv[GetDataItemIndex(e.ItemIndex)].Row : null;
			if (dv != null && (cAdmWizardObjGrid.EditIndex == e.ItemIndex || UpdateGridRow(sender, new CommandEventArgs("Save",""))))
			{
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cWizardId71.Text == string.Empty) { cClearButton_Click(sender, new EventArgs()); ShowDirty(false); PanelTop.Update();}
			else { PopAdmWizardObj49List(sender, e, false, cWizardId71.Text); }
			// *** System Button Click (After) Web Rule starts here *** //
		}
		...
		public void cEditButton_Click(object sender, System.EventArgs e)
		{
			...
			Session.Remove(KEY_dtAdmWizardObj49List); PopAdmWizardObj49List(sender, e, false, null);
...
bool bRegenNeeded = false;
			...
			if (ValidPage() && (cAdmWizardObjGrid.EditIndex < 0 || UpdateGridRow(cAdmWizardObjGrid, new CommandEventArgs("Save", ""))))
			{
				DataTable dt = (DataTable)Session[KEY_dtAdmWizardObjGrid];
				DataView dv = dt != null ? dt.DefaultView : null;
				string ftr = string.Empty;
				if (dv.RowFilter != string.Empty) { ftr = dv.RowFilter; dv.RowFilter = string.Empty; }
				AdmWizardObj49 ds = PrepAdmWizardObjData(dv,cWizardId71.Text == string.Empty);
				if (ftr != string.Empty) {dv.RowFilter = ftr;}
				if (!string.IsNullOrEmpty(cAdmWizardObj49List.SelectedValue) && cWizardId71.Text != cAdmWizardObj49List.SelectedValue)
				{
					PreMsgPopup("Primary key cannot be changed");
					return rtn;
				}
				if (string.IsNullOrEmpty(cAdmWizardObj49List.SelectedValue))	// Add
				{
					if (ds != null)
					{
					...
					{
						cAdmWizardObj49List.ClearSearch(); Session.Remove(KEY_dtAdmWizardObj49List);
						ShowDirty(false); bUseCri.Value = "Y"; PopAdmWizardObj49List(sender, e, false, pid);
					...
					if (bValid7)
					{
						...
						ShowDirty(false); PopAdmWizardObj49List(sender, e, false, ds.Tables["AdmWizardObj"].Rows[0]["WizardId71"]);
						rtn = GetScreenHlp().Rows[0]["UpdMsg"].ToString();
					}
				}
			}
			//WebRule: Create program upon program name change or add wizard
           if (bRegenNeeded || pid != string.Empty)		// ProgramName changed or a new wizard has been added successfully.
			{
				try
				{
		...
		{
			// *** System Button Click (Before) Web Rule starts here *** //
			if (cWizardId71.Text != string.Empty)
			{
				AdmWizardObj49 ds = PrepAdmWizardObjData(null,false);
				try
				{
					if (ds != null && (new AdminSystem()).DelData(49,false,base.LUser,base.LImpr,base.LCurr,ds,(string)Session[KEY_sysConnectionString],base.AppPwd(base.LCurr.DbId),base.CPrj,base.CSrc))
					{
						cAdmWizardObj49List.ClearSearch(); Session.Remove(KEY_dtAdmWizardObj49List);
						ShowDirty(false); PopAdmWizardObj49List(sender, e, false, null);
		...
		private AdmWizardObj49 PrepAdmWizardObjData(DataView dv, bool bAdd)
		{
			...
			if (bAdd) { dr["WizardId71"] = string.Empty; } else { dr["WizardId71"] = cWizardId71.Text; }
			drType["WizardId71"] = "Numeric"; drDisp["WizardId71"] = "TextBox";
			try {dr["WizardTypeId71"] = cWizardTypeId71.SelectedValue;} catch {}
			drType["WizardTypeId71"] = "Numeric"; drDisp["WizardTypeId71"] = "DropDownList";
			try {dr["MasterTableId71"] = cMasterTableId71.SelectedValue;} catch {}
			drType["MasterTableId71"] = "Numeric"; drDisp["MasterTableId71"] = "DropDownList";
			try {dr["WizardTitle71"] = cWizardTitle71.Text.Trim();} catch {}
			drType["WizardTitle71"] = "VarWChar"; drDisp["WizardTitle71"] = "TextBox";
			try {dr["ProgramName71"] = cProgramName71.Text.Trim();} catch {}
			drType["ProgramName71"] = "VarChar"; drDisp["ProgramName71"] = "TextBox";
			try {dr["DefWorkSheet71"] = cDefWorkSheet71.Text.Trim();} catch {}
			drType["DefWorkSheet71"] = "VarWChar"; drDisp["DefWorkSheet71"] = "TextBox";
			try {dr["DefStartRow71"] = cDefStartRow71.Text.Trim();} catch {}
			drType["DefStartRow71"] = "Numeric"; drDisp["DefStartRow71"] = "TextBox";
			try {dr["DefOverwrite71"] = base.SetBool(cDefOverwrite71.Checked);} catch {}
			drType["DefOverwrite71"] = "Char"; drDisp["DefOverwrite71"] = "CheckBox";
			try {dr["OvwrReadonly71"] = base.SetBool(cOvwrReadonly71.Checked);} catch {}
			...
			if (dv != null)
			{
				...
				if (bAdd)
				{
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmWizardObjAdd"].Rows.Add(MakeColRow(ds.Tables["AdmWizardObjAdd"].NewRow(), drv, true));
					}
				}
				else
				{
					...
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmWizardObjUpd"].Rows.Add(MakeColRow(ds.Tables["AdmWizardObjUpd"].NewRow(), drv, false));
					}
					...
					foreach (DataRowView drv in dv)
					{
						ds.Tables["AdmWizardObjDel"].Rows.Add(MakeColRow(ds.Tables["AdmWizardObjDel"].NewRow(), drv, false));
					}
		...
		public bool CanAct(char typ) { return CanAct(typ, GetAuthRow(), cAdmWizardObj49List.SelectedValue.ToString()); }
		...
		private bool ValidPage()
		{
			...
			DataTable dt = null;
			dt = (DataTable)Session[KEY_dtWizardTypeId71];
			if (dt != null && dt.Rows.Count <= 0)
			{
				bErrNow.Value = "Y"; PreMsgPopup("Value is expected for 'WizardTypeId', please investigate."); return false;
			}
			dt = (DataTable)Session[KEY_dtMasterTableId71];
			if (dt != null && dt.Rows.Count <= 0)
			{
		...
		{
			dr["WizardId71"] = cWizardId71.Text;
			DataTable dtAuth = GetAuthCol();
			if (dtAuth != null)
			{
		...
		{
			if (cAdmWizardObjGrid.EditIndex > -1)
			{
				cAdmWizardObjGrid_OnItemUpdating(sender, new ListViewUpdateEventArgs(cAdmWizardObjGrid.EditIndex));
			}
			return true;
		}

		protected void cAdmWizardObjGrid_OnPreRender(object sender, System.EventArgs e)
		{
			System.Web.UI.WebControls.Image hi = null;
			hi = (System.Web.UI.WebControls.Image)cAdmWizardObjGrid.FindControl("cWizardObjId72hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmWizardObjGrid.FindControl("cColumnId72hi"); if (hi != null) { hi.Visible = false; }
			hi = (System.Web.UI.WebControls.Image)cAdmWizardObjGrid.FindControl("cTabIndex72hi"); if (hi != null) { hi.Visible = false; }
			if (Session[KEY_lastSortImg] != null)
			{
				hi = (System.Web.UI.WebControls.Image)cAdmWizardObjGrid.FindControl((string)Session[KEY_lastSortImg]);
				if (hi != null) { hi.ImageUrl = Utils.AddTilde((string)Session[KEY_lastSortUrl]); hi.Visible = true; }
			}
			IgnoreHeaderConfirm((LinkButton)cAdmWizardObjGrid.FindControl("cWizardObjId72hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmWizardObjGrid.FindControl("cColumnId72hl"));
			IgnoreHeaderConfirm((LinkButton)cAdmWizardObjGrid.FindControl("cTabIndex72hl"));
		}

		private string GetButtonId(ListViewItem lvi)
		{
			string ButtonID = String.Empty;
			Control c = lvi.FindControl("cAdmWizardObjGridEdit");
		...
		{
		    return cAdmWizardObjGridDataPager.StartRowIndex / cAdmWizardObjGridDataPager.PageSize;
		}

		private int GetTotalPages()
		{
		    return (int)Math.Ceiling((double)cAdmWizardObjGridDataPager.TotalRowCount / cAdmWizardObjGridDataPager.PageSize);
		}

		private int GetDataItemIndex(int editIndex)
		{
		    return cAdmWizardObjGridDataPager.StartRowIndex + editIndex;
		}

		protected void cAdmWizardObjGrid_OnLayoutCreated(object sender, EventArgs e)
		{
		    // Header:
		    LinkButton lb = null;
			...
			if (dtAuthRow != null)
			{
				...
				{
		            lb = cAdmWizardObjGrid.FindControl("cDeleteAllButton") as LinkButton; if (lb != null) { lb.Visible = false; }
				}
			}
		    // footer:
		    Label gc = null;
		...
		private void PreMsgPopup(string msg) { PreMsgPopup(msg,cAdmWizardObj49List,null); }

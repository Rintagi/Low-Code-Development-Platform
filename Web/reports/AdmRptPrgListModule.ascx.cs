...
{
	public class DsAdmRptPrgListIn : DataSet
	{
		public DsAdmRptPrgListIn()
		{
			this.Tables.Add(MakeColumns(new DataTable("DtAdmRptPrgListIn")));
			this.DataSetName = "DsAdmRptPrgListIn";
			this.Namespace = "http://Rintagi.com/DataSet/DsAdmRptPrgListIn";
		}
...
namespace RO.Web
{
	public partial class AdmRptPrgListModule : RO.Web.ModuleBase
	{

		public ReportDocument rp = new ReportDocument();
		private const string KEY_dtReportSct = "Cache:dtReportSct58";
		private const string KEY_dtReportItem = "Cache:dtReportItem58";
		private const string KEY_dtReportHlp = "Cache:dtReportHlp58";
		private const string KEY_dtCri = "Cache:dtCri58";
		private const string KEY_dtClientRule = "Cache:dtClientRule58";
		private const string KEY_dtPrinters = "Cache:dtPrinters58";
		private const string KEY_bClCriVisible = "Cache:bClCriVisible58";
		private const string KEY_bShCriVisible = "Cache:bShCriVisible58";
		private enum exportTo {TXT, RTF, XML, XLS, PDF, DOC, VIEW};
		private string LcSysConnString;
		private string LcAppConnString;
		private string LcAppPw;

		public AdmRptPrgListModule()
		{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			rp.Load(Server.MapPath(@"reports/AdmRptPrgListReport.rpt"));
			if (!IsPostBack)
			{
				DataTable dtMenuAccess = (new MenuSystem()).GetMenu(base.LUser.CultureId,3, base.LImpr, LcSysConnString, LcAppPw, null, 58, null);
				...
				else
				{
					...
					{
						(new AdminSystem()).UpdLastCriteria(0,58,base.LUser.UsrId,Int32.Parse(Request.QueryString["Cri"+ii.ToString()]),Request.QueryString["Val"+ii.ToString()],LcSysConnString,LcAppPw);
				...
				dt = (new AdminSystem()).GetLastCriteria(2,0,58,base.LUser.UsrId,LcSysConnString,LcAppPw);
				...
				dv = new DataView((new AdminSystem()).GetIn(58,"GetIn58CultureId",(new SqlReportSystem()).CountRptCri("24",LcSysConnString,LcAppPw),"Y",base.LImpr,base.LCurr,LcAppConnString,LcAppPw));
				cCultureId.DataSource = dv;
				if (true)
				{
					System.Collections.Generic.Dictionary<string, string> context = new System.Collections.Generic.Dictionary<string, string>();
					context["method"] = "58CultureId";
					context["addnew"] = "Y";
					context["sp"] = "58CultureId";
				...
				dv = new DataView((new AdminSystem()).GetIn(58,"GetIn58SystemId",(new SqlReportSystem()).CountRptCri("23",LcSysConnString,LcAppPw),"Y",base.LImpr,base.LCurr,LcAppConnString,LcAppPw));
				...
				SetClientRule();
				(new AdminSystem()).LogUsage(base.LUser.UsrId, string.Empty, dtHlp.Rows[0]["ReportTitle"].ToString(), 0, 58, 0, string.Empty, LcSysConnString, LcAppPw);
				if (bBatchPrint)
				{
		...
		private void SetSystem(byte SystemId)
		{
			...
			try
			{
				...
				if ((Config.DeployType == "DEV" || row["dbAppDatabase"].ToString() == base.CPrj.EntityCode + "View") && !(base.CPrj.EntityCode != "RO" && row["SysProgram"].ToString() == "Y") && (new AdminSystem()).IsRegenNeeded(string.Empty,0,58,0,LcSysConnString,LcAppPw))
				{
					(new GenReportsSystem()).CreateProgram(string.Empty,58, "Program Listing", row["dbAppDatabase"].ToString(), base.CPrj, base.CSrc, base.CTar, LcAppConnString, LcAppPw);
		...
		{
			DataTable dt;
			dt = (new AdminSystem()).GetButtonHlp(0,58,0,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetClientRule()
		{
			...
			if (dtRul == null)
			{
				...
				dtRul = (new AdminSystem()).GetClientRule(0,58,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetReportCriHlp()
		{
			...
			{
				CheckAuthentication(false);
				dtCri = (new AdminSystem()).GetReportCriHlp(58,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		private DataTable GetReportHlp()
		{
			...
			{
				CheckAuthentication(false);
			    dtHlp = (new AdminSystem()).GetReportHlp(58,base.LUser.CultureId,LcSysConnString,LcAppPw);
		...
		{
			return ((new SqlReportSystem()).GetReportCriteria(string.Empty,"58",LcSysConnString,LcAppPw)).DefaultView;
		}

		private DsAdmRptPrgListIn UpdCriteria(bool bUpdate)
		{
			DsAdmRptPrgListIn ds = new DsAdmRptPrgListIn();
			...
			ds.Tables["DtAdmRptPrgListIn"].Rows.Add(dr);
			if (bUpdate) {(new AdminSystem()).UpdRptDt(58,"UpdAdmRptPrgList",base.LUser.UsrId,ds,GetRptCriteria(),LcAppConnString,LcAppPw);}
			return ds;
		}
		...
		private DataView GetReportItem()
		{
			...
			{
				CheckAuthentication(false);
				dtItem = (new AdminSystem()).GetReportItem(58,LcSysConnString,LcAppPw);
		...
		private void getReport(bool sendToPrinter, exportTo eExport)
		{
			...
			DataTable dt = (new AdminSystem()).GetRptDt(58,"GetAdmRptPrgList",base.LImpr,base.LCurr,UpdCriteria(false),GetRptCriteria(),LcAppConnString,LcAppPw,false,false,false,Config.CommandTimeOut);
			CovertRptUTC(dt);
			...
			if (eExport == exportTo.TXT)
			{
				...
				if (dt.Columns.Contains("QId")) {sb.Append("Menu Order" + (char)9);}
				if (dt.Columns.Contains("MenuText")) {sb.Append("Menu Label" + (char)9);}
				if (dt.Columns.Contains("ProgramType")) {sb.Append("Program Type" + (char)9);}
				if (dt.Columns.Contains("ProgramName")) {sb.Append("Program Name" + (char)9);}
				if (dt.Columns.Contains("Everyone")) {sb.Append("Everyone Access" + (char)9);}
				if (dt.Columns.Contains("ClientTierList")) {sb.Append("Client Tier List" + (char)9);}
				if (dt.Columns.Contains("RuleTierList")) {sb.Append("Rule Tier List" + (char)9);}
				if (dt.Columns.Contains("DataTierList")) {sb.Append("Data Tier List" + (char)9);}
				if (dt.Columns.Contains("ClientRule")) {sb.Append("Client Rule" + (char)9);}
				if (dt.Columns.Contains("WebRule")) {sb.Append("Web Rule" + (char)9);}
				if (dt.Columns.Contains("ServerRule")) {sb.Append("Server Rule" + (char)9);}
				...
				foreach (DataRowView drv in dv)
				{
					...
					if (dt.Columns.Contains("QId")) {sb.Append(drv["QId"].ToString() + (char)9);}
					if (dt.Columns.Contains("MenuText")) {sb.Append(drv["MenuText"].ToString() + (char)9);}
					if (dt.Columns.Contains("ProgramType")) {sb.Append(drv["ProgramType"].ToString() + (char)9);}
					if (dt.Columns.Contains("ProgramName")) {sb.Append(drv["ProgramName"].ToString() + (char)9);}
					if (dt.Columns.Contains("Everyone")) {sb.Append(drv["Everyone"].ToString() + (char)9);}
					if (dt.Columns.Contains("ClientTierList")) {sb.Append(drv["ClientTierList"].ToString() + (char)9);}
					if (dt.Columns.Contains("RuleTierList")) {sb.Append(drv["RuleTierList"].ToString() + (char)9);}
					if (dt.Columns.Contains("DataTierList")) {sb.Append(drv["DataTierList"].ToString() + (char)9);}
					if (dt.Columns.Contains("ClientRule")) {sb.Append(drv["ClientRule"].ToString() + (char)9);}
					if (dt.Columns.Contains("WebRule")) {sb.Append(drv["WebRule"].ToString() + (char)9);}
					if (dt.Columns.Contains("ServerRule")) {sb.Append(drv["ServerRule"].ToString() + (char)9);}
					sb.Append(Environment.NewLine);
				}

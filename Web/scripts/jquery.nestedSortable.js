(function( factory ) {
	...
	$.widget("js.nestedSortable", $.extend({}, $.ui.sortable.prototype, {
		...
		options: {
			...
			branchClass: 'js-nestedSortable-branch',
			collapsedClass: 'js-nestedSortable-collapsed',
			disableNestingClass: 'js-nestedSortable-no-nesting',
			errorClass: 'js-nestedSortable-error',
			expandedClass: 'js-nestedSortable-expanded',
			hoveringClass: 'js-nestedSortable-hovering',
			leafClass: 'js-nestedSortable-leaf',
			disabledClass: 'js-nestedSortable-disabled'
		},

		_create: function() {
			this.element.data('ui-sortable', this.element.data('js-nestedSortable'));

			// js - prevent browser from freezing if the HTML is not correct
			if (!this.element.is(this.options.listType))
				throw new Error('nestedSortable: Please check that the listType option is set to your actual list type');

			// js - force 'intersect' tolerance method if we have a tree with expanding/collapsing functionality
			...
			// js - prepare the tree by applying the right classes (the CSS is responsible for actual hide/show functionality)
		...
		_mouseDrag: function(event) {
			...
			// js - find the top offset before rearrangement,
			...
			// js - check and reset hovering state at each cycle
			this.hovering = this.hovering ? this.hovering : null;
			this.mouseentered = this.mouseentered ? this.mouseentered : false;

			// js - let's start caching some variables
			...
			for (i = this.items.length - 1; i >= 0; i--) {
				...
				if (itemElement !== this.currentItem[0] &&
					...
					// js - we are intersecting an element: trigger the mouseenter event and store this state
					if (!this.mouseentered) {
						$(itemElement).mouseenter();
						this.mouseentered = true;
					}

					// js - if the element has children and they are hidden, show them after a delay (CSS responsible)
					...
					// js - rearrange the elements and reset timeouts and hovering state
					if (this.options.tolerance == "pointer" || this._intersectsWithSides(item)) {
						...
						// js - do not switch container if it's a root item and 'protectRoot' is true
			...
			// js - to find the previous sibling in the list, keep backtracking until we hit a valid list item.
			...
			// js - to find the next sibling in the list, keep stepping forward until we hit a valid list item.
			...
			// js - if the item is moved to the left, send it one level up but only if it's at the bottom of the list
			if (parentItem != null
				&& nextItem == null
			...
			// js - if the item is below a sibling and is moved to the right, make it a child of that sibling
		        ...
		        // js - if this item is being moved from the top, add it to the top of the list.
		        if (previousTopOffset && (previousTopOffset <= previousItem.offset().top)) {
		        	previousItem.children(o.listType).prepend(this.placeholder);
		        }
		        // js - otherwise, add it to the bottom of the list.
		        else {
		...
		_mouseStop: function(event, noPropagation) {
			...
			// js - if the item is in a position not allowed, send it back
			...
			// js - clear the hovering timeout, just to be sure
		...
		// js - this function is slightly modified to make it easier to hover over a collapsed element and have it expand
		...
		_isAllowed: function(parentItem, level, levels) {
			...
			// js - is the root protected?
			// js - are we nesting too deep?
	...
	$.js.nestedSortable.prototype.options = $.extend({}, $.ui.sortable.prototype.options, $.js.nestedSortable.prototype.options);
}));
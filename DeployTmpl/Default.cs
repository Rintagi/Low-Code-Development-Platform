...
namespace Install
{
	public partial class Default : Form
	{
		...
		public Default()
		{
            ...
            copyright.Text = "V" + assemblyVersion +  " © 1999-" + DateTime.Now.Year.ToString() + " robocoder corporation. All rights reserved.";
            item.SetVersion(lvVersion);
			txtOldNS.Text = item.GetOldNS();
			txtWebServer.Text = Environment.MachineName;
		...
		private void InitializeTiers(string NmSpace, string WebServer)
		{
			if (txtOldNS.Text == "RO") 
            { 
                label8.Text = "Rintagi Installer";
                this.Text = "Rintagi " + item.GetInsType() + " Installer";
            } 
            else 
            { 
                label8.Text = "Application Installer";
                this.Text = item.GetOldNS() + " " + item.GetInsType() + " Installer";
            }
            string oldBasePath = "C:\\inetpub\\wwwroot\\";
            NmSpace = (string.IsNullOrEmpty(NmSpace) ? (txtOldNS.Text == "RO" ? "ZZ" : txtOldNS.Text) : NmSpace).ToUpper();
		...
		private string ValidateControls(bool bBackup)
		{
            ...
            else if (!cbNew.Checked && Directory.Exists(txtRuleTier.Text + "\\UsrRules") && item.GetInsType().Contains("PDT") && !bBackup) { return string.Format("Rule tier/Project Root {0} found though you are installing a production package.", txtRuleTier.Text); }
            else if (gbRule.Visible && txtRuleTier.Text == string.Empty && !item.GetInsType().Contains("PDT")) { return "Please enter the rule tier/project root directory and try again."; }
            else if (gbRule.Visible && !cbNew.Checked && !string.IsNullOrEmpty(txtRuleTier.Text) && !Directory.Exists(txtRuleTier.Text) && !item.GetInsType().Contains("PDT")) { return "Please enter a valid rule tier/project root directory and try again."; }
		...
		private void rbPtyUpgrade_CheckedChanged(object sender, EventArgs e)
		{
			...
			else
			{
                if (txtOldNS.Text == "RO")
                {
                    ...
                    txtNewNS.Text = "RO"; txtNewNS.Enabled = true;
                }
                else
                {
		...
		{
            if (txtNewNS.Text != string.Empty) { 
                txtNewNS.Text = txtNewNS.Text.ToUpper(); InitializeTiers(txtNewNS.Text, txtWebServer.Text);
                txtNewNS.SelectionStart = txtNewNS.Text.Length;
            }
		}
        ...
        private void control_MouseHover(object sender, EventArgs e)
        {
            Dictionary<Control,string> toolTipMsg = new Dictionary<Control,string>{
                ...
                {txtUserName,"database login for this app, must have create database right for new installation. use dummy and password dummy if no db access needed"},
                ...
                {txtBkPath1,@"client side backup path, must be accessible by the credential running this program can be \\SERVER\ShareName. leave blank to skip client backup"},
                {txtBkPath2,@"server side backup path(where SQL server is running, can be different from the client side), must be accessible by credential running SQL server, not this program. can be \\SERVER\ShareName. leave blank to skip server backup"},
            };
            try {

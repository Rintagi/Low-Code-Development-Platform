...
namespace OpenAI.Files
{
    ...
    public sealed class FilesEndpoint : BaseEndPoint
    {
        ...
        /// <inheritdoc />
        protected override string Root { get { return "files"; } }
        ...
        public async Task<IReadOnlyList<FileResponse>> ListFilesAsync(string purpose = null, CancellationToken cancellationToken = default(CancellationToken))
        {
            Dictionary<string, string> query = null;

            if (!string.IsNullOrWhiteSpace(purpose))
            {
                query = new Dictionary<string, string> { { "purpose", purpose } };
            }
            ...
            return Newtonsoft.Json.JsonConvert.DeserializeObject<FilesList>(resultAsString, OpenAIClient.jsonSerializationOptions).Files;
        }
        ...
        public async Task<FileResponse> UploadFileAsync(string filePath, string purpose, CancellationToken cancellationToken = default(CancellationToken))
        { return await UploadFileAsync(new FileUploadRequest(filePath, purpose), cancellationToken).ConfigureAwait(false);}
        ...
        /// <param name="cancellationToken">Optional, <see cref="CancellationToken"/>.</param>
        /// <returns><see cref="FileResponse"/>.</returns>
        public async Task<FileResponse> UploadFileAsync(FileUploadRequest request, CancellationToken cancellationToken = default(CancellationToken))
        {
            var fileData = new MemoryStream();
            var content = new MultipartFormDataContent();
            await request.File.CopyToAsync(fileData, 4096, cancellationToken).ConfigureAwait(false);
            ...
            return Newtonsoft.Json.JsonConvert.DeserializeObject<FileResponse>(responseAsString, OpenAIClient.jsonSerializationOptions);
        }
        ...
        public async Task<bool> DeleteFileAsync(string fileId, CancellationToken cancellationToken = default(CancellationToken))
        {
            Func<int, Task<bool>> InternalDeleteFileAsync = null;
            
            InternalDeleteFileAsync = async (int attempt) =>
            {
                var response = await client.client.DeleteAsync(GetUrl(string.Format("/{0}", fileId)), cancellationToken).ConfigureAwait(false);
        ...
        /// <param name="cancellationToken">Optional, <see cref="CancellationToken"/>.</param>
        /// <returns><see cref="FileResponse"/></returns>
        public async Task<FileResponse> GetFileInfoAsync(string fileId, CancellationToken cancellationToken = default(CancellationToken))
        {
            var response = await client.client.GetAsync(GetUrl(string.Format("/{0}", fileId)), cancellationToken).ConfigureAwait(false);
            var responseAsString = await response.ReadAsStringAsync(EnableDebug, cancellationToken).ConfigureAwait(false);
            return Newtonsoft.Json.JsonConvert.DeserializeObject<FileResponse>(responseAsString, OpenAIClient.jsonSerializationOptions);
        }
        ...
        public async Task<string> DownloadFileAsync(string fileId, string directory, bool deleteCachedFile = false, CancellationToken cancellationToken = default(CancellationToken))
        {
        ...
        /// <param name="cancellationToken">Optional, <see cref="CancellationToken"/></param>
        /// <returns>The full path of the downloaded file.</returns>
        public async Task<string> DownloadFileAsync(FileResponse fileData, string directory, bool deleteCachedFile = false, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            var response = await client.client.GetStreamAsync(GetUrl(string.Format("/{0}/content", fileData.Id))).ConfigureAwait(false);

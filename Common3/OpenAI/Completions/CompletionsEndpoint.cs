...
namespace OpenAI.Completions
{
    ...
    public sealed class CompletionsEndpoint : BaseEndPoint
    {
        ...
        /// <inheritdoc />
        protected override string Root { get { return "completions"; } }
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            var request = new CompletionRequest(
                string.IsNullOrWhiteSpace(model) ? (string) Model.Davinci : model,
                prompt,
                prompts,
                suffix,
                maxTokens,
        ...
        /// Look in its <see cref="CompletionResponse.Completions"/> property for the completions.
        /// </returns>
        public async Task<CompletionResponse> CreateCompletionAsync(CompletionRequest completionRequest, CancellationToken cancellationToken = default(CancellationToken))
        {
            completionRequest.Stream = false;
            var jsonContent = Newtonsoft.Json.JsonConvert.SerializeObject(completionRequest, OpenAIClient.jsonSerializationOptions).ToJsonStringContent(EnableDebug);
        ...
        /// <param name="resultHandler">An action to be called as each new result arrives.</param>
        /// <param name="cancellationToken">Optional, <see cref="CancellationToken"/>.</param>
        public async Task StreamCompletionAsync(CompletionRequest completionRequest, Action<CompletionResponse> resultHandler, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            while (true)
            {
                string streamData = await reader.ReadLineAsync();
        ...
        /// for more details on how to consume an async enumerable.
        /// </returns>
        //public async IAsyncEnumerable<CompletionResponse> StreamCompletionEnumerableAsync(CompletionRequest completionRequest, [EnumeratorCancellation] CancellationToken cancellationToken = default(CancellationToken))
        //{
        //    completionRequest.Stream = true;
        //    var jsonContent = Newtonsoft.Json.JsonConvert.SerializeObject(completionRequest, OpenAIClient.jsonSerializationOptions).ToJsonStringContent(EnableDebug);

...
namespace OpenAI
{
    ...
    public sealed class OpenAIAuthentication
    {
        ...
        /// </summary>
        public string ApiKey { get { return authInfo.ApiKey; } }
        ...
        /// </summary>
        public string OrganizationId { get { return authInfo.OrganizationId; }}
        ...
        public static implicit operator OpenAIAuthentication(string key) { return new OpenAIAuthentication(key); }
        ...
        public static OpenAIAuthentication Default
        {
            get
            {
                ...
                var auth = LoadFromDirectory() ??
                           ...
                           LoadFromEnv();
                if (auth == null) throw new UnauthorizedAccessException("Failed to load a valid API Key!");
                cachedDefault = auth; 
                return auth;
            }
        ...
        { return LoadFromDirectory(Path.GetDirectoryName(path), Path.GetFileName(path), false); }
        ...
        public static OpenAIAuthentication LoadFromDirectory(string directory = null, string filename = ".openai", bool searchUp = true)
        {
            ...
            while (authInfo == null && currentDirectory.Parent != null)
            {
                ...
                if (File.Exists(filePath))
                {
                    try
                    {
                        authInfo = Newtonsoft.Json.JsonConvert.DeserializeObject<AuthInfo>(File.ReadAllText(filePath));
                        break;
                    }
                    catch (Exception)
                    {

namespace RO.Access3.Odbc
{
	...
	public class DeployAccess : DeployAccessBase, IDisposable
	{
		...
		public override void BackupDb(string dbProviderCd, string connStr, string pwd, string dbName, string bkFile)
		{
			...
			OdbcConnection cn =  new OdbcConnection(Config.ConvertOleDbConnStrToOdbcConnStr(connStr + DecryptString(pwd)));
			cn.Open();
			OdbcCommand cmd = new OdbcCommand(ss, cn);
		...
		public override void OnlineDb(string connStr, string pwd, string dbName)
		{
			...
			OdbcConnection cn =  new OdbcConnection(Config.ConvertOleDbConnStrToOdbcConnStr(connStr + DecryptString(pwd)));
			cn.Open();
			OdbcCommand cmd = new OdbcCommand("ONLINE DATABASE " + dbName, cn);
			cmd.CommandType = CommandType.Text;
		...
		public override void RestoreWaDb(string dbProviderCd, string connStr, string pwd, string waDb, string waFile, string waPath)
		{
			...
			if (dbProviderCd == "S")
			{
				cmd = new OdbcCommand("LOAD DATABASE " + waDb + " FROM '" + waFile + "'", cn);
			}
			else	//Ms Windows only for now, not Linux
			{
				cmd = new OdbcCommand("RESTORE FILELISTONLY FROM DISK = '" + waFile +"'", cn);
				cmd.CommandType = CommandType.Text;
				...
				cn.Close();
				cn.Open();
				cmd = new OdbcCommand(restoreCmd, cn);
			}
			cmd.CommandType = CommandType.Text;
			cmd.CommandTimeout = 10000;
			try
		...
		public override string GenRestoreScript(string dbProviderCd, string connStr, string pwd, string dbName, string iFileAbs, string rsFileRel, string dbPath)
		{
			...
			else	//Ms Windows only for now, not Linux
			{
				...
				OdbcConnection cn =  new OdbcConnection(Config.ConvertOleDbConnStrToOdbcConnStr(connStr + DecryptString(pwd)));
				cn.Open();
				OdbcCommand cmd = new OdbcCommand("RESTORE FILELISTONLY FROM DISK = '" + iFileAbs +"'", cn);
				cmd.CommandType = CommandType.Text;
				da.SelectCommand = TransformCmd(cmd);
				DataTable dt = new DataTable();
				da.Fill(dt);
		...
		public override void DropDb(string dbProviderCd, string connStr, string pwd, string dbName)
		{
			...
			OdbcConnection cn =  new OdbcConnection(Config.ConvertOleDbConnStrToOdbcConnStr(connStr + DecryptString(pwd)));
			cn.Open();
			OdbcCommand cmd = new OdbcCommand("DROP DATABASE " + dbName, cn);
			cmd.CommandType = CommandType.Text;
			try
			{
				TransformCmd(cmd).ExecuteNonQuery();
		...
		public override void TruncateTable(string connStr, string pwd, string tableName)
		{
			...
			OdbcConnection cn =  new OdbcConnection(Config.ConvertOleDbConnStrToOdbcConnStr(connStr + DecryptString(pwd)));
			cn.Open();
			OdbcCommand cmd = new OdbcCommand("DELETE FROM " + tableName, cn);
			cmd.CommandType = CommandType.Text;
			try
			{
				TransformCmd(cmd).ExecuteNonQuery();
        ...
        public override void MkDBOwner(string connStr, string usr, string pwd, string dbName)
        {
            ...
            string ss = ""
                        ...
                        + " END"; OdbcCommand cmd = new OdbcCommand(ss, cn);
            cmd.CommandType = CommandType.Text;
            cmd.CommandTimeout = 4000;
            try
            {
		...
		public override DataTable GetReleaseInf(int releaseId)
		{
			...
			cn.Open();
			OdbcCommand cmd = new OdbcCommand("GetReleaseInf", cn);
			cmd.CommandType = CommandType.StoredProcedure;
			cmd.Parameters.Add("@ReleaseId", OdbcType.Numeric).Value = releaseId == -1 ? (object) DBNull.Value : (object) releaseId;
			da.SelectCommand = TransformCmd(cmd);
			DataTable dt = new DataTable();
			da.Fill(dt);
		...
		public override DataTable GetReleaseDtl(int releaseId)
		{
			...
			OdbcConnection cn = new OdbcConnection(Config.ConvertOleDbConnStrToOdbcConnStr(GetDesConnStr()));
			cn.Open();
			OdbcCommand cmd = new OdbcCommand("GetReleaseDtl", cn);
			cmd.CommandType = CommandType.StoredProcedure;
		...
		public override DataTable GetReleaseVer(int ReleaseId, string EntityCode)
		{
			...
			OdbcConnection cn = new OdbcConnection(Config.ConvertOleDbConnStrToOdbcConnStr(GetDesConnStr()));
			cn.Open();
			OdbcCommand cmd = new OdbcCommand("GetReleaseVer", cn);
			cmd.CommandType = CommandType.StoredProcedure;
			cmd.Parameters.Add("@ReleaseId", OdbcType.Numeric).Value = ReleaseId;
			cmd.Parameters.Add("@EntityCode", OdbcType.VarChar).Value = EntityCode;
			da.SelectCommand = TransformCmd(cmd);
			DataTable dt = new DataTable();
			da.Fill(dt);
			return dt;
		...
		public override DataTable GetYrReadme(int ReleaseId, string EntityCode)
		{
			...
			OdbcConnection cn = new OdbcConnection(Config.ConvertOleDbConnStrToOdbcConnStr(GetDesConnStr()));
			cn.Open();
			OdbcCommand cmd = new OdbcCommand("GetYrReadme", cn);
			cmd.CommandType = CommandType.StoredProcedure;
			cmd.Parameters.Add("@ReleaseId", OdbcType.Numeric).Value = ReleaseId;
			cmd.Parameters.Add("@EntityCode", OdbcType.VarChar).Value = EntityCode;
			da.SelectCommand = TransformCmd(cmd);
        ...
        {
            using (OdbcCommand cmd = new OdbcCommand(sql, new OdbcConnection(Config.ConvertOleDbConnStrToOdbcConnStr(connStr + DecryptString(pwd)))))
            {
                DataTable dt = new DataTable();
                da.Fill(dt);
                return dt;
        ...
        {
            using (OdbcConnection cn = new OdbcConnection(Config.ConvertOleDbConnStrToOdbcConnStr(connStr + DecryptString(pwd))))
            {
                cn.Open();
...
", col[1].Trim(), col[0].Trim(), desDBName);
                    OdbcTransaction tr = cn.BeginTransaction();
                    OdbcCommand cmd = new OdbcCommand(updateCmd, cn,tr);
                    cmd.CommandType = CommandType.Text;
                    try
                    {
                        TransformCmd(cmd).ExecuteNonQuery();
                ...
                foreach (string f in systemId)
                {
                    ...
                    da.SelectCommand = new OdbcCommand("SELECT * FROM " + col[0], cn);
                    DataTable dt = new DataTable();
                    da.Fill(dt);
                    foreach (DataRow dr in dt.Rows)
                    {
                        ...
                        try
                        {
                            ...
                            OdbcTransaction tr = cn.BeginTransaction();
                            OdbcCommand updCmd = new OdbcCommand(string.Format(@"update {0} set {1}={2} WHERE {3} = {4} ", col[0], col[2], newSystemId.ToString(), col[1], key.ToString()), cn, tr);
                            try
                            {

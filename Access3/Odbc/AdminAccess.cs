                ...
                if (drv[firingEvent].ToString() == "Y" && drv["BeforeCRUD"].ToString() == beforeCRUD && (bDeferError || !bHasErr))
                {
                    try
                    {
                        ...
                        while (parameterNames.Length > 0)
                        {
                            ...
                            {
                                cmd.Parameters.Add("@" + param, GetOdbcType(Utils.PopFirstWord(parameterTypes, (char)44).Trim())).Value = System.DBNull.Value;
                            }
                            else
                            {
                                cmd.Parameters.Add("@" + param, GetOdbcType(Utils.PopFirstWord(parameterTypes, (char)44).Trim())).Value = val;
                            }
                        }

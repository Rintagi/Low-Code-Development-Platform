...
const initState = {
    ...
    Label: {
        ...
        PwdHlpMsgLabel:"Minimum 8 characters and at least one digit, one capital, one lower case and one symbol",
        ...
        authService.login(username, password).then(
            (data) => {
                if (data.status === "success") {
                    ...
                    authService.getToken(data.accessCode).then(
                        data => {
            ...
            (error) => {
                console.log(error);
                dispatchWithNotification(dispatch, { type: LOGIN.FAILED, payload: error });
            }
        );
    };
}
...
export function logout() {
    return (dispatch) => {
        ...
        authService.logout().then(
            ...
            .catch(error => {
                dispatchWithNotification(dispatch, { type: LOGOUT.FAILED, payload: error });
            })
    }
}
...
export function getCurrentUser(silent = false) {
    return (dispatch) => {
        ...
        if (!authService.getRefreshToken().refresh_token) return;

...
import * as AdmRulTierService from '../../services/AdmRulTierService';
...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmRulTier || {}).initialized) {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmRulTier || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).RuleTierId196) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    // if (!AdmRulTierState.initialized || AdmRulTierState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmRulTierReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmRulTierReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmRulTierState.ScreenCriteria || {}
    ...
    const currMst = AdmRulTierState.Mst;
    const currDtl = AdmRulTierState.EditDtl;
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.RuleTierId196));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmRulTierReduxObj.QuickFilterDdlToSelectList(AdmRulTierState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmRulTierState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmRulTierState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).RuleTierId196)}
                    ...
                    <Formik
                      ...
                      key={AdmRulTierState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmRulTierState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmRulTierState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmRulTierReduxObj.LoadInitPage.bind(AdmRulTierReduxObj) },
    { LoadSearchList: AdmRulTierReduxObj.LoadSearchList.bind(AdmRulTierReduxObj) },
    { SelectMst: AdmRulTierReduxObj.SelectMst.bind(AdmRulTierReduxObj) },
    { DelMst: AdmRulTierReduxObj.DelMst.bind(AdmRulTierReduxObj) },
    { AddMst: AdmRulTierReduxObj.AddMst.bind(AdmRulTierReduxObj) },
    { AddDtl: AdmRulTierReduxObj.AddDtl.bind(AdmRulTierReduxObj) },
    { changeMstListFilterVisibility: AdmRulTierReduxObj.ChangeMstListFilterVisibility.bind(AdmRulTierReduxObj) },
    { SetScreenCriteria: AdmRulTierReduxObj.SetScreenCriteria.bind(AdmRulTierReduxObj) },

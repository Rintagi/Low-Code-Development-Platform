...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmDataCat || {}).initialized) {
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).RptwizCatId181) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    const screenDdlSelectors = AdmDataCatReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmDataCatReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmDataCatState.ScreenCriteria || {}
    ...
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.RptwizCatId181));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmDataCatReduxObj.QuickFilterDdlToSelectList(AdmDataCatState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmDataCatState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmDataCatState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).RptwizCatId181)}
                    ...
                    <Formik
                      ...
                      key={AdmDataCatState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmDataCatState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmDataCatState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmDataCatReduxObj.LoadInitPage.bind(AdmDataCatReduxObj) },
    { LoadSearchList: AdmDataCatReduxObj.LoadSearchList.bind(AdmDataCatReduxObj) },
    { SelectMst: AdmDataCatReduxObj.SelectMst.bind(AdmDataCatReduxObj) },
    ...
    { changeMstListFilterVisibility: AdmDataCatReduxObj.ChangeMstListFilterVisibility.bind(AdmDataCatReduxObj) },
    { SetScreenCriteria: AdmDataCatReduxObj.SetScreenCriteria.bind(AdmDataCatReduxObj) },

...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmWizardObj || {}).initialized) {
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).WizardId71) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    const screenDdlSelectors = AdmWizardObjReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmWizardObjReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmWizardObjState.ScreenCriteria || {}
    ...
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.WizardId71));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmWizardObjReduxObj.QuickFilterDdlToSelectList(AdmWizardObjState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmWizardObjState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmWizardObjState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).WizardId71)}
                    ...
                    <Formik
                      ...
                      key={AdmWizardObjState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmWizardObjState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmWizardObjState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmWizardObjReduxObj.LoadInitPage.bind(AdmWizardObjReduxObj) },
    { LoadSearchList: AdmWizardObjReduxObj.LoadSearchList.bind(AdmWizardObjReduxObj) },
    { SelectMst: AdmWizardObjReduxObj.SelectMst.bind(AdmWizardObjReduxObj) },
    ...
    { changeMstListFilterVisibility: AdmWizardObjReduxObj.ChangeMstListFilterVisibility.bind(AdmWizardObjReduxObj) },
    { SetScreenCriteria: AdmWizardObjReduxObj.SetScreenCriteria.bind(AdmWizardObjReduxObj) },

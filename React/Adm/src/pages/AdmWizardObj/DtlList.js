...
import AdmWizardObjReduxObj from '../../redux/AdmWizardObj';
...
class DtlList extends RintagiScreen {
  ...
  CopyRow({ mst, dtlId, dtl, ...rest }) {
    ...
    return function (evt) {
      evt.preventDefault();
      const currDtlId = dtlId || dtl.WizardObjId72;
      const copyFn = () => {
        if (currDtlId) {
          this.props.AddDtl(mst.WizardId71, currDtlId);
          const isMobileView = window.matchMedia('(max-width: 1099px)').matches;
          const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
          if (useMobileView) {
            const naviBar = getNaviBar('DtlList', mst, {}, this.props.AdmWizardObj.Label);
      ...
      const deleteFn = () => {
        ...
        if (currDtlId) {
          this.props.SavePage(
            ...
            this.props.AdmWizardObj.Mst,
            [
              {
                WizardObjId72: currDtlId,
                _mode: 'delete',
              }
            ],
            {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmWizardObj || {}).Buttons || {};
  ...
  render() {
    ...
    //if (!AdmWizardObjState.initialized || AdmWizardObjState.page_loading) return null;
    ...
    const itemList = AdmWizardObjReduxObj.DtlListToSelectList(AdmWizardObjState, 'WizardObjId72');
    ...
    const currMst = AdmWizardObjState.Mst;
    const currDtl = AdmWizardObjState.EditDtl;
    const dtlFilter = AdmWizardObjState.DtlFilter;
    const filterList = AdmWizardObjState.DtlFilter.FilterColumnDdl.map((v, i) => ({ key: v.ColName, label: v.ColumnHeader, value: v.ColName, pos: i }))
                        ...
                        resetForm
                          ...
                          <div>
                                      ...
                                      <ButtonGroup className='btn-group--icons'>
                                        ...
                                        </Button>
                                        {
                                          dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                            if ((!activeSelectionVisible && v.buttonType !== 'InsRow') || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).WizardId71)) return null;
                                            return (
                                              <Button
                                                key={v.tid}
                                        ...
                                        {activeSelectionVisible &&
                                          dropdownMenuButtonList
                                            .filter(v => !v.expose)
                                            .filter(v => (!this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).WizardId71,(currDtl || {}).WizardObjId72)))
                                            .length > 0 &&
                                        ...
                                        <DropdownMenu right className={`dropdown__menu dropdown-options`}>
                                          {
                                            ...
                                            dropdownMenuButtonList.filter(v => !v.expose).map(v => {
                                              if ((!activeSelectionVisible && v.buttonType !== 'InsRow') || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).WizardId71,(currDtl || {}).WizardObjId72)) return null;
                                              return (
                            ...
                            <Form className='form'>
                                ...
                                </div>
                                {
                                  itemList.slice(0, dtlFilter.Limit).map(
                                    (obj, i) => {
                                      ...
                                      return (
                                        <div className='form__form-group-narrow list-divider' key={obj.WizardObjId72 || ('_' + i)}>
                                          <div className='form__form-group-field'>
                                            <label className='radio-btn radio-btn--button margin-narrow'>
                                              <Field
                                                key={obj.WizardObjId72 || ('_' + i)}
                                                className='radio-btn__radio'
                                                name='expenseItem'
                                                type='radio'
                                                value={values.name}
                                                onClick={this.SelectDtlListRow(naviSelectBar, currMst, obj, i, 'WizardId71', 'WizardObjId72')}
                                              ...
                                              </span>
                                              {
                                                rowMenuButtonList
                                                  ...
                                                  .map((v, i) => {
                                                    return (
                                                      <button type='button' onClick={this.ScreenButtonAction[v.buttonType]({ naviBar: naviSelectBar, mst: currMst, useMobileView, dtlId: obj.WizardObjId72 })} className={`${v.exposedClassName}`}><i className={`${v.iconClassName}`}></i></button>
                                                    )
                                                  })
                                              }
                                              {
                                                ...
                                                <UncontrolledDropdown className={`btn-row-dropdown`}>
                                                  ...
                                                  <DropdownMenu right className={`dropdown__menu dropdown-options`}>
                                                    {rowMenuButtonList
                                                      .filter(v => !v.expose || useMobileView)
                                                      .map((v) => {
                                                        if (this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).WizardId71,obj.WizardObjId72)) return null;
                                                        return <DropdownItem key={v.tid} onClick={this.ScreenButtonAction[v.buttonType]({ naviBar: naviSelectBar, mst: currMst, useMobileView, dtlId: obj.WizardObjId72 })} className={`${v.className}`}><i className={`${v.iconClassName} mr-10`}></i>{v.label}</DropdownItem>
                                                      })
                                                    }
                                                  </DropdownMenu>
                                                </UncontrolledDropdown>
                              ...
                              <Button onClick={this.props.ViewMoreDtl} className={`${viewMoreClass} btn btn-view-more-blue account__btn`} type='button'>{strFormat(IncrementMsg, this.props.AdmWizardObj.DtlFilter.PageSize)}<br /><i className='fa fa-arrow-down'></i></Button>
            ...
            {!useMobileView && <Col xs={6}>
              ...
              {!activeSelectionVisible &&
                ...
                <div className='empty-block'>
                  ...
                  <p className='create-new-message'>{NoDetailMsg}. <span className='link-imitation' onClick={this.AddNewDtl({ naviBar, mstId: currMst.WizardId71 })}>{AddDetailMsg}</span></p>
                </div>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { SelectDtl: AdmWizardObjReduxObj.SelectDtl.bind(AdmWizardObjReduxObj) },
    { SavePage: AdmWizardObjReduxObj.SavePage.bind(AdmWizardObjReduxObj) },
    { AddDtl: AdmWizardObjReduxObj.AddDtl.bind(AdmWizardObjReduxObj) },
    { ChangeDtlListFilterVisibility: AdmWizardObjReduxObj.ChangeDtlListFilterVisibility.bind(AdmWizardObjReduxObj) },
    { ChangeDtlListFilter: AdmWizardObjReduxObj.ChangeDtlListFilter.bind(AdmWizardObjReduxObj) },
    { ViewMoreDtl: AdmWizardObjReduxObj.ViewMoreDtl.bind(AdmWizardObjReduxObj) },
    { setTitle: setTitle },

...
class DtlRecord extends RintagiScreen {
  ...
  ColumnId72InputChange() {
    const _this = this; 
    return function (name, v) { 
      const filterBy = ''; 
      _this.props.SearchColumnId72(v, filterBy);
    } 
  }
  ...
  ValidatePage(values) {
    ...
    if (!values.cTabIndex72) { errors.cTabIndex72 = (columnLabel.TabIndex72 || {}).ErrMessage; }
    return errors;
  }
    ...
    return function (evt) {
      ...
      const copyFn = () => {
        if (currDtlId) {
          ...
          if (useMobileView) {
            const naviBar = getNaviBar('MstRecord', mst, {}, this.props.AdmWizardObj.Label);
          ...
          <div className='account'>
            <div className='account__wrapper account-col'>
              <div className='account__card shadow-box rad-4'>
                {/* {!useMobileView && this.constructor.ShowSpinner(this.props.AdmWizardObj) && <div className='panel__refresh'><LoadingIcon /></div>} */}
                ...
                <Formik
                  initialValues={{
                    ...
                    cTabIndex72: formatContent(currDtl.TabIndex72 || '', 'TextBox'),
                  }}
                              ...
                              <ButtonToolbar className='f-right'>
                                <UncontrolledDropdown>
                                  <ButtonGroup className='btn-group--icons'>
                                    ...
                                    {
                                      dropdownMenuButtonList.filter(v => !v.expose && !this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).WizardId71, currDtl.WizardObjId72)).length > 0 &&
                                  ...
                                  <div className='form__form-group'>
                                    ...
                                    {((true && this.constructor.ShowSpinner(AdmWizardObjState)) && <Skeleton height='36px' />) ||
                                      <div className='form__form-group-field'>
                                        <AutoCompleteField
                                          name='cColumnId72'
                                          onChange={this.FieldChange(setFieldValue, setFieldTouched, 'cColumnId72', false)}
                                          onBlur={this.FieldChange(setFieldValue, setFieldTouched, 'cColumnId72', true)}
                                          onInputChange={this.ColumnId72InputChange()}
                                          value={values.cColumnId72}
                                          defaultSelected={ColumnId72List.filter(obj => { return obj.key === ColumnId72 })}
                                          options={ColumnId72List}
                                          filterBy={this.AutoCompleteFilterBy}
                                          disabled={(authCol.ColumnId72 || {}).readonly ? true : false} />
                                      </div>
                                    }
                                    {errors.cColumnId72 && touched.cColumnId72 && <span className='form__form-group-error'>{errors.cColumnId72}</span>}
                                  </div>
                                </Col>
                              }
                                    ...
                                    {((true && this.constructor.ShowSpinner(AdmWizardObjState)) && <Skeleton height='36px' />) ||
                                      <div className='form__form-group-field'>
                                        <Field
                                          type='text'
                                          name='cTabIndex72'
                                          disabled={(authCol.TabIndex72 || {}).readonly ? 'disabled' : ''} />
                                      </div>
                                    }
                                    {errors.cTabIndex72 && touched.cTabIndex72 && <span className='form__form-group-error'>{errors.cTabIndex72}</span>}
                                  </div>
                                </Col>
                              }
                            </Row>
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { SearchColumnId72: AdmWizardObjReduxObj.SearchActions.SearchColumnId72.bind(AdmWizardObjReduxObj) },
    { setTitle: setTitle },

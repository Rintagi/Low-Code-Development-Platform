...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmDatTier || {}).initialized) {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmDatTier || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).DataTierId195) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    // if (!AdmDatTierState.initialized || AdmDatTierState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmDatTierReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmDatTierReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmDatTierState.ScreenCriteria || {}
    ...
    const currMst = AdmDatTierState.Mst;
    const currDtl = AdmDatTierState.EditDtl;
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.DataTierId195));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmDatTierReduxObj.QuickFilterDdlToSelectList(AdmDatTierState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmDatTierState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmDatTierState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).DataTierId195)}
                    ...
                    <Formik
                      ...
                      key={AdmDatTierState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmDatTierState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmDatTierState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmDatTierReduxObj.LoadInitPage.bind(AdmDatTierReduxObj) },
    { LoadSearchList: AdmDatTierReduxObj.LoadSearchList.bind(AdmDatTierReduxObj) },
    { SelectMst: AdmDatTierReduxObj.SelectMst.bind(AdmDatTierReduxObj) },
    { DelMst: AdmDatTierReduxObj.DelMst.bind(AdmDatTierReduxObj) },
    { AddMst: AdmDatTierReduxObj.AddMst.bind(AdmDatTierReduxObj) },
    { AddDtl: AdmDatTierReduxObj.AddDtl.bind(AdmDatTierReduxObj) },
    { changeMstListFilterVisibility: AdmDatTierReduxObj.ChangeMstListFilterVisibility.bind(AdmDatTierReduxObj) },
    { SetScreenCriteria: AdmDatTierReduxObj.SetScreenCriteria.bind(AdmDatTierReduxObj) },

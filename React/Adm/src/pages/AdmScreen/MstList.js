...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmScreen || {}).initialized) {
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).ScreenId15) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    const screenDdlSelectors = AdmScreenReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmScreenReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmScreenState.ScreenCriteria || {}
    ...
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.ScreenId15));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmScreenReduxObj.QuickFilterDdlToSelectList(AdmScreenState);
    ...
    const CriScreenId10List = screenCriDdlSelectors.ScreenId10(AdmScreenState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmScreenState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmScreenState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).ScreenId15)}
                    ...
                    <Formik
                      ...
                      key={AdmScreenState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmScreenState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmScreenState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmScreenReduxObj.LoadInitPage.bind(AdmScreenReduxObj) },
    { LoadSearchList: AdmScreenReduxObj.LoadSearchList.bind(AdmScreenReduxObj) },
    { SelectMst: AdmScreenReduxObj.SelectMst.bind(AdmScreenReduxObj) },
    ...
    { changeMstListFilterVisibility: AdmScreenReduxObj.ChangeMstListFilterVisibility.bind(AdmScreenReduxObj) },
    { SetScreenCriteria: AdmScreenReduxObj.SetScreenCriteria.bind(AdmScreenReduxObj) },
    { SearchCriScreenId10: AdmScreenReduxObj.SearchActions.SearchCriScreenId10.bind(AdmScreenReduxObj) },
    { setTitle: setTitle },

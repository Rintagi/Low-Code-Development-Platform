...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmClnTier || {}).initialized) {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmClnTier || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).ClientTierId194) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    // if (!AdmClnTierState.initialized || AdmClnTierState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmClnTierReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmClnTierReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmClnTierState.ScreenCriteria || {}
    ...
    const currMst = AdmClnTierState.Mst;
    const currDtl = AdmClnTierState.EditDtl;
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.ClientTierId194));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmClnTierReduxObj.QuickFilterDdlToSelectList(AdmClnTierState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmClnTierState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmClnTierState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).ClientTierId194)}
                    ...
                    <Formik
                      ...
                      key={AdmClnTierState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmClnTierState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmClnTierState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmClnTierReduxObj.LoadInitPage.bind(AdmClnTierReduxObj) },
    { LoadSearchList: AdmClnTierReduxObj.LoadSearchList.bind(AdmClnTierReduxObj) },
    { SelectMst: AdmClnTierReduxObj.SelectMst.bind(AdmClnTierReduxObj) },
    { DelMst: AdmClnTierReduxObj.DelMst.bind(AdmClnTierReduxObj) },
    { AddMst: AdmClnTierReduxObj.AddMst.bind(AdmClnTierReduxObj) },
    { AddDtl: AdmClnTierReduxObj.AddDtl.bind(AdmClnTierReduxObj) },
    { changeMstListFilterVisibility: AdmClnTierReduxObj.ChangeMstListFilterVisibility.bind(AdmClnTierReduxObj) },
    { SetScreenCriteria: AdmClnTierReduxObj.SetScreenCriteria.bind(AdmClnTierReduxObj) },

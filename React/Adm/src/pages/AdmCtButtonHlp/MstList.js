...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmCtButtonHlp || {}).initialized) {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmCtButtonHlp || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).ButtonHlpId131) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    // if (!AdmCtButtonHlpState.initialized || AdmCtButtonHlpState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmCtButtonHlpReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmCtButtonHlpReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmCtButtonHlpState.ScreenCriteria || {}
    ...
    const currMst = AdmCtButtonHlpState.Mst;
    const currDtl = AdmCtButtonHlpState.EditDtl;
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.ButtonHlpId131));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmCtButtonHlpReduxObj.QuickFilterDdlToSelectList(AdmCtButtonHlpState);
    ...
    const CriCultureId10List = screenCriDdlSelectors.CultureId10(AdmCtButtonHlpState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmCtButtonHlpState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmCtButtonHlpState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).ButtonHlpId131)}
                    ...
                    <Formik
                      ...
                      key={AdmCtButtonHlpState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmCtButtonHlpState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmCtButtonHlpState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmCtButtonHlpReduxObj.LoadInitPage.bind(AdmCtButtonHlpReduxObj) },
    { LoadSearchList: AdmCtButtonHlpReduxObj.LoadSearchList.bind(AdmCtButtonHlpReduxObj) },
    { SelectMst: AdmCtButtonHlpReduxObj.SelectMst.bind(AdmCtButtonHlpReduxObj) },
    ...
    { AddDtl: AdmCtButtonHlpReduxObj.AddDtl.bind(AdmCtButtonHlpReduxObj) },
    { changeMstListFilterVisibility: AdmCtButtonHlpReduxObj.ChangeMstListFilterVisibility.bind(AdmCtButtonHlpReduxObj) },
    { SetScreenCriteria: AdmCtButtonHlpReduxObj.SetScreenCriteria.bind(AdmCtButtonHlpReduxObj) },
    { SearchCriCultureId10: AdmCtButtonHlpReduxObj.SearchActions.SearchCriCultureId10.bind(AdmCtButtonHlpReduxObj) },

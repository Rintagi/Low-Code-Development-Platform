...
import * as AdmCronJobService from '../../services/AdmCronJobService';
...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmCronJob || {}).initialized) {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmCronJob || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).CronJobId264) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    // if (!AdmCronJobState.initialized || AdmCronJobState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmCronJobReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmCronJobReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmCronJobState.ScreenCriteria || {}
    ...
    const currMst = AdmCronJobState.Mst;
    const currDtl = AdmCronJobState.EditDtl;
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.CronJobId264));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmCronJobReduxObj.QuickFilterDdlToSelectList(AdmCronJobState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmCronJobState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmCronJobState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).CronJobId264)}
                    ...
                    <Formik
                      ...
                      key={AdmCronJobState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmCronJobState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmCronJobState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmCronJobReduxObj.LoadInitPage.bind(AdmCronJobReduxObj) },
    { LoadSearchList: AdmCronJobReduxObj.LoadSearchList.bind(AdmCronJobReduxObj) },
    { SelectMst: AdmCronJobReduxObj.SelectMst.bind(AdmCronJobReduxObj) },
    { DelMst: AdmCronJobReduxObj.DelMst.bind(AdmCronJobReduxObj) },
    { AddMst: AdmCronJobReduxObj.AddMst.bind(AdmCronJobReduxObj) },
    { AddDtl: AdmCronJobReduxObj.AddDtl.bind(AdmCronJobReduxObj) },
    { changeMstListFilterVisibility: AdmCronJobReduxObj.ChangeMstListFilterVisibility.bind(AdmCronJobReduxObj) },
    { SetScreenCriteria: AdmCronJobReduxObj.SetScreenCriteria.bind(AdmCronJobReduxObj) },

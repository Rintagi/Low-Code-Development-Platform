...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmScreenFilter || {}).initialized) {
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).ScreenFilterId86) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    const screenDdlSelectors = AdmScreenFilterReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmScreenFilterReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmScreenFilterState.ScreenCriteria || {}
    ...
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.ScreenFilterId86));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmScreenFilterReduxObj.QuickFilterDdlToSelectList(AdmScreenFilterState);
    ...
    const CriScreenId10List = screenCriDdlSelectors.ScreenId10(AdmScreenFilterState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmScreenFilterState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmScreenFilterState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).ScreenFilterId86)}
                    ...
                    <Formik
                      ...
                      key={AdmScreenFilterState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmScreenFilterState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmScreenFilterState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmScreenFilterReduxObj.LoadInitPage.bind(AdmScreenFilterReduxObj) },
    { LoadSearchList: AdmScreenFilterReduxObj.LoadSearchList.bind(AdmScreenFilterReduxObj) },
    { SelectMst: AdmScreenFilterReduxObj.SelectMst.bind(AdmScreenFilterReduxObj) },
    ...
    { changeMstListFilterVisibility: AdmScreenFilterReduxObj.ChangeMstListFilterVisibility.bind(AdmScreenFilterReduxObj) },
    { SetScreenCriteria: AdmScreenFilterReduxObj.SetScreenCriteria.bind(AdmScreenFilterReduxObj) },
    { SearchCriScreenId10: AdmScreenFilterReduxObj.SearchActions.SearchCriScreenId10.bind(AdmScreenFilterReduxObj) },

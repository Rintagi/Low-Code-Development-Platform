...
class MstRecord extends RintagiScreen {
  ...
  ResetFromGitRepo({ submitForm, ScreenButton, naviBar, redirectTo, onSuccess }) {
    return function (evt) {
      this.OnClickColumeName = 'ResetFromGitRepo';
      //Enter Custom Code here, eg: submitForm();
      evt.preventDefault();
    }.bind(this);
  }
  CreateReactBase({ submitForm, ScreenButton, naviBar, redirectTo, onSuccess }) {
    return function (evt) {
      this.OnClickColumeName = 'CreateReactBase';
      //Enter Custom Code here, eg: submitForm();
      evt.preventDefault();
    }.bind(this);
  }
  RemoveReactBase({ submitForm, ScreenButton, naviBar, redirectTo, onSuccess }) {
    return function (evt) {
      this.OnClickColumeName = 'RemoveReactBase';
      //Enter Custom Code here, eg: submitForm();
      evt.preventDefault();
    }.bind(this);
  }
  PublishReactToSite({ submitForm, ScreenButton, naviBar, redirectTo, onSuccess }) {
    return function (evt) {
      this.OnClickColumeName = 'PublishReactToSite';
      //Enter Custom Code here, eg: submitForm();
      evt.preventDefault();
    }.bind(this);
  }
    ...
    return function (evt) {
      ...
      const copyFn = () => {
        if (fromMstId) {
          ...
          if (useMobileView) {
            const naviBar = getNaviBar('MstRecord', {}, {}, this.props.AdmSystems.Label);
  ...
  render() {
    ...
    const naviBar = getNaviBar('MstRecord', currMst, currDtl, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.SystemId45));
          ...
          <div className='account'>
            <div className='account__wrapper account-col'>
              <div className='account__card shadow-box rad-4'>
                ...
                <Formik
                  initialValues={{
                    cSystemId45: formatContent(SystemId45 || '', 'TextBox'),
                    cServerName45: formatContent(ServerName45 || '', 'TextBox'),
                    cSystemName45: formatContent(SystemName45 || '', 'TextBox'),
                    cSystemAbbr45: formatContent(SystemAbbr45 || '', 'TextBox'),
                    cSysProgram45: SysProgram45 === 'Y',
                    cActive45: Active45 === 'Y',
                    cdbAppProvider45: formatContent(dbAppProvider45 || '', 'TextBox'),
                    cdbAppServer45: formatContent(dbAppServer45 || '', 'TextBox'),
                    cdbAppDatabase45: formatContent(dbAppDatabase45 || '', 'TextBox'),
                    cdbDesDatabase45: formatContent(dbDesDatabase45 || '', 'TextBox'),
                    cdbAppUserId45: formatContent(dbAppUserId45 || '', 'TextBox'),
                    cdbAppPassword45: formatContent(dbAppPassword45 || '', 'TextBox'),
                    cdbX01Provider45: formatContent(dbX01Provider45 || '', 'TextBox'),
                    cdbX01Server45: formatContent(dbX01Server45 || '', 'TextBox'),
                    cdbX01Database45: formatContent(dbX01Database45 || '', 'TextBox'),
                    cdbX01UserId45: formatContent(dbX01UserId45 || '', 'TextBox'),
                    cdbX01Password45: formatContent(dbX01Password45 || '', 'TextBox'),
                    cdbX01Extra45: formatContent(dbX01Extra45 || '', 'TextBox'),
                    cAdminEmail45: formatContent(AdminEmail45 || '', 'TextBox'),
                    cAdminPhone45: formatContent(AdminPhone45 || '', 'TextBox'),
                    cCustServEmail45: formatContent(CustServEmail45 || '', 'TextBox'),
                    cCustServPhone45: formatContent(CustServPhone45 || '', 'TextBox'),
                    cCustServFax45: formatContent(CustServFax45 || '', 'TextBox'),
                    cWebAddress45: formatContent(WebAddress45 || '', 'TextBox'),
                            ...
                            <Row>
                              ...
                              {(authCol.FromSystemId || {}).visible &&
                                <Col lg={6} xl={6}>
                                  <div className='form__form-group'>
                                    {((true && this.constructor.ShowSpinner(AdmSystemsState)) && <Skeleton height='20px' />) ||
                                      <label className='form__form-group-label'>{(columnLabel.FromSystemId || {}).ColumnHeader} {(columnLabel.FromSystemId || {}).ToolTip &&
                                        (<ControlledPopover id={(columnLabel.FromSystemId || {}).ColumnName} className='sticky-icon pt-0 lh-23' message={(columnLabel.FromSystemId || {}).ToolTip} />
                                        )}
                                      </label>
                                    }
                                    {((true && this.constructor.ShowSpinner(AdmSystemsState)) && <Skeleton height='36px' />) ||
                                      <div className='form__form-group-field'>
                                        <Field
                                          type='text'
                                          name='cFromSystemId'
                                          disabled={(authCol.FromSystemId || {}).readonly ? 'disabled' : ''} />
                                      </div>
                                    }
                                    {errors.cFromSystemId && touched.cFromSystemId && <span className='form__form-group-error'>{errors.cFromSystemId}</span>}
                                  </div>
                                </Col>
                              }
                              {(authCol.UpdBaseSystemId || {}).visible &&
                                <Col lg={12} xl={12}>
                                  <div className='form__form-group'>
                                    <label className='checkbox-btn checkbox-btn--colored-click'>
                                      <Field
                                        className='checkbox-btn__checkbox'
                                        type='checkbox'
                                        name='cUpdBaseSystemId'
                                        onChange={handleChange}
                                        defaultChecked={values.cUpdBaseSystemId}
                                        disabled={(authCol.UpdBaseSystemId || {}).readonly || !(authCol.UpdBaseSystemId || {}).visible}
                                      />
                                      <span className='checkbox-btn__checkbox-custom'><CheckIcon /></span>
                                      <span className='checkbox-btn__label'>{(columnLabel.UpdBaseSystemId || {}).ColumnHeader}</span>
                                    </label>
                                    {(columnLabel.UpdBaseSystemId || {}).ToolTip &&
                                      (<ControlledPopover id={(columnLabel.UpdBaseSystemId || {}).ColumnName} className='sticky-icon pt-0 lh-23' message={(columnLabel.UpdBaseSystemId || {}).ToolTip} />
                                      )}
                                  </div>
                                </Col>
                              }
                              {(authCol.UpdRefSystemId || {}).visible &&
                                <Col lg={12} xl={12}>
                                  <div className='form__form-group'>
                                    <label className='checkbox-btn checkbox-btn--colored-click'>
                                      <Field
                                        className='checkbox-btn__checkbox'
                                        type='checkbox'
                                        name='cUpdRefSystemId'
                                        onChange={handleChange}
                                        defaultChecked={values.cUpdRefSystemId}
                                        disabled={(authCol.UpdRefSystemId || {}).readonly || !(authCol.UpdRefSystemId || {}).visible}
                                      />
                                      <span className='checkbox-btn__checkbox-custom'><CheckIcon /></span>
                                      <span className='checkbox-btn__label'>{(columnLabel.UpdRefSystemId || {}).ColumnHeader}</span>
                                    </label>

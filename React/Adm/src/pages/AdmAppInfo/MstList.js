...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmAppInfo || {}).initialized) {
  ...
  RefreshSearchList(values, { setErrors, resetForm, setValues /* setValues and other goodies */ }) {
    ...
    const refreshFn = (() => {
      this.props.SetScreenCriteria(values.search,
        {
          VersionDt10: (values.cCriVersionDt10) ? values.cCriVersionDt10 : '',
        },
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmAppInfo || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).AppInfoId135) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    // if (!AdmAppInfoState.initialized || AdmAppInfoState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmAppInfoReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmAppInfoReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmAppInfoState.ScreenCriteria || {}
    ...
    const currMst = AdmAppInfoState.Mst;
    const currDtl = AdmAppInfoState.EditDtl;
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.AppInfoId135));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmAppInfoReduxObj.QuickFilterDdlToSelectList(AdmAppInfoState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmAppInfoState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmAppInfoState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).AppInfoId135)}
                            ...
                            <Form className='form'>
                              <div className='form__form-group'>
                                <div className={`form__form-group filter-padding ${filterVisibility}`} key={screenCriteria.key}>
                                  <Row className='mb-5'>
                                    <Col xs={12} md={12}>
                                      <label className='form__form-group-label filter-label'>{(screenCriteria.VersionDt10 || {}).ColumnHeader}</label>
                                      <div className='form__form-group-field filter-form-border'>
                                        <Field
                                          type='text'
                                          name='cCriVersionDt10'
                                          value={values.cCriVersionDt10}
                                          onBlur={this.SearchFilterTextValueChange(handleSubmit, setFieldValue, 'text', 'cCriVersionDt10')}
                                        />
                                      </div>
                                    </Col>
                                  </Row>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmAppInfoState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmAppInfoState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmAppInfoReduxObj.LoadInitPage.bind(AdmAppInfoReduxObj) },
    { LoadSearchList: AdmAppInfoReduxObj.LoadSearchList.bind(AdmAppInfoReduxObj) },
    { SelectMst: AdmAppInfoReduxObj.SelectMst.bind(AdmAppInfoReduxObj) },
    { DelMst: AdmAppInfoReduxObj.DelMst.bind(AdmAppInfoReduxObj) },
    { AddMst: AdmAppInfoReduxObj.AddMst.bind(AdmAppInfoReduxObj) },
    { AddDtl: AdmAppInfoReduxObj.AddDtl.bind(AdmAppInfoReduxObj) },
    { changeMstListFilterVisibility: AdmAppInfoReduxObj.ChangeMstListFilterVisibility.bind(AdmAppInfoReduxObj) },
    { SetScreenCriteria: AdmAppInfoReduxObj.SetScreenCriteria.bind(AdmAppInfoReduxObj) },

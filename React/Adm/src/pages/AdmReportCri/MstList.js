...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmReportCri || {}).initialized) {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmReportCri || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).ReportCriId97) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    // if (!AdmReportCriState.initialized || AdmReportCriState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmReportCriReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmReportCriReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmReportCriState.ScreenCriteria || {}
    ...
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.ReportCriId97));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmReportCriReduxObj.QuickFilterDdlToSelectList(AdmReportCriState);
    ...
    const CriReportId10List = screenCriDdlSelectors.ReportId10(AdmReportCriState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmReportCriState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmReportCriState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).ReportCriId97)}
                    ...
                    <Formik
                      ...
                      key={AdmReportCriState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmReportCriState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmReportCriState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmReportCriReduxObj.LoadInitPage.bind(AdmReportCriReduxObj) },
    { LoadSearchList: AdmReportCriReduxObj.LoadSearchList.bind(AdmReportCriReduxObj) },
    { SelectMst: AdmReportCriReduxObj.SelectMst.bind(AdmReportCriReduxObj) },
    ...
    { changeMstListFilterVisibility: AdmReportCriReduxObj.ChangeMstListFilterVisibility.bind(AdmReportCriReduxObj) },
    { SetScreenCriteria: AdmReportCriReduxObj.SetScreenCriteria.bind(AdmReportCriReduxObj) },
    { SearchCriReportId10: AdmReportCriReduxObj.SearchActions.SearchCriReportId10.bind(AdmReportCriReduxObj) },

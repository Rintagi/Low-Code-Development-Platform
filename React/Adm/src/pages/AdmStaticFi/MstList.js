...
import AdmStaticFiReduxObj, { ShowMstFilterApplied } from '../../redux/AdmStaticFi';
import * as AdmStaticFiService from '../../services/AdmStaticFiService';
...
class MstList extends RintagiScreen {
  constructor(props) {
    super(props);
    this.GetReduxState = () => (this.props.AdmStaticFi || {});
    this.hasChangedContent = false;
    this.titleSet = false;
    this.MstKeyColumnName = 'StaticFiId262';
    ...
    if (lastAppUrl && !(this.props.AdmStaticFi || {}).initialized) {
  ...
  CopyHdr({ naviBar, ScreenButton, useMobileView, mst, mstId }) {
    const AdmStaticFiState = this.props.AdmStaticFi || {};
    const auxSystemLabels = AdmStaticFiState.SystemLabel || {};
    return function (evt) {
      evt.preventDefault();
      const fromMstId = mstId || mst.StaticFiId262;
      const copyFn = () => {
  ...
  DelMst({ naviBar, ScreenButton, mst, mstId }) {
    ...
    return this.Prompt({
      okFn: function (evt) {
        const fromMstId = mstId || mst.StaticFiId262;
        this.props.DelMst(this.props.AdmStaticFi, fromMstId);
      }.bind(this),
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmStaticFi || {}).Buttons || {};
  ...
  componentDidMount() {
    ...
    if (!(this.props.AdmStaticFi || {}).AuthCol || true) {
  ...
  componentDidUpdate(prevProps, prevStates) {
    const currReduxScreenState = this.props.AdmStaticFi || {};
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).StaticFiId262) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    if (AdmStaticFiState.access_denied) {
      return <Redirect to='/error' />;
    }

    // if (!AdmStaticFiState.initialized || AdmStaticFiState.page_loading) return null;
    const screenHlp = AdmStaticFiState.ScreenHlp;
    ...
    const screenButtons = AdmStaticFiReduxObj.GetScreenButtons(AdmStaticFiState);
    const screenDdlSelectors = AdmStaticFiReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmStaticFiReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmStaticFiState.ScreenCriteria || {}
    const selectList = AdmStaticFiReduxObj.SearchListToSelectList(AdmStaticFiState);
    const itemList = AdmStaticFiState.Dtl || [];
    const auxLabels = AdmStaticFiState.Label || {};
    const auxSystemLabels = AdmStaticFiState.SystemLabel || {};
    const columnLabel = AdmStaticFiState.ColumnLabel || {};
    const currMst = AdmStaticFiState.Mst;
    const currDtl = AdmStaticFiState.EditDtl;
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.StaticFiId262));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmStaticFiReduxObj.QuickFilterDdlToSelectList(AdmStaticFiState);
    const screenFilterSelected = screenFilterList.filter(obj => { return obj.key === screenCriteria.FilterId });
    const authRow = (AdmStaticFiState.AuthRow || [])[0] || {};
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmStaticFiState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmStaticFiState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).StaticFiId262)}
                    ...
                    <Formik
                      ...
                      key={AdmStaticFiState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmStaticFiState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmStaticFiState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
                                    ...
                                    <Row className='btn-bottom-row'>
                                      {
                                        bottomButtonList
                                          .filter(v => v.expose)
                                          .map((v, i, a) => {
                                            if (this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).StaticFiId262)) return null;
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    { LoadPage: AdmStaticFiReduxObj.LoadPage.bind(AdmStaticFiReduxObj) },
    { LoadInitPage: AdmStaticFiReduxObj.LoadInitPage.bind(AdmStaticFiReduxObj) },
    { LoadSearchList: AdmStaticFiReduxObj.LoadSearchList.bind(AdmStaticFiReduxObj) },
    { SelectMst: AdmStaticFiReduxObj.SelectMst.bind(AdmStaticFiReduxObj) },
    { DelMst: AdmStaticFiReduxObj.DelMst.bind(AdmStaticFiReduxObj) },
    { AddMst: AdmStaticFiReduxObj.AddMst.bind(AdmStaticFiReduxObj) },
    { AddDtl: AdmStaticFiReduxObj.AddDtl.bind(AdmStaticFiReduxObj) },
    { changeMstListFilterVisibility: AdmStaticFiReduxObj.ChangeMstListFilterVisibility.bind(AdmStaticFiReduxObj) },
    { SetScreenCriteria: AdmStaticFiReduxObj.SetScreenCriteria.bind(AdmStaticFiReduxObj) },

...
class MstRecord extends RintagiScreen {
  ...
  SavePage(values, { setSubmitting, setErrors, resetForm, setFieldValue, setValues }) {
    const errors = [];
    const currMst = (this.props.AdmStaticFi || {}).Mst || {};
    ...
    return function (evt) {
      ...
      const copyFn = () => {
        if (fromMstId) {
          ...
          this.props.AdmStaticFi.Mst.Posted64 = 'N';
          if (useMobileView) {
            const naviBar = getNaviBar('MstRecord', {}, {}, this.props.AdmStaticFi.Label);
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const nextReduxScreenState = nextProps.AdmStaticFi || {};
  ...
  render() {
    ...
    const authCol = this.GetAuthCol(AdmStaticFiState);
    const authRow = (AdmStaticFiState.AuthRow || [])[0] || {};
    const currMst = ((this.props.AdmStaticFi || {}).Mst || {});
    const currDtl = ((this.props.AdmStaticFi || {}).EditDtl || {});
    const naviBar = getNaviBar('MstRecord', currMst, currDtl, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.StaticFiId262));
          ...
          <div className='account'>
            <div className='account__wrapper account-col'>
              <div className='account__card shadow-box rad-4'>
                {/* {!useMobileView && this.constructor.ShowSpinner(AdmStaticFiState) && <div className='panel__refresh'></div>} */}
                    ...
                    submitForm
                      ...
                      <div>
                          ...
                          <Row>
                            ...
                            <Col xs={useMobileView ? 3 : 4}>
                              <ButtonToolbar className='f-right'>
                                {(this.constructor.ShowSpinner(AdmStaticFiState) && <Skeleton height='40px' />) ||
                                  <UncontrolledDropdown>
                                    <ButtonGroup className='btn-group--icons'>
                                      <i className={dirty ? 'fa fa-exclamation exclamation-icon' : ''}></i>
                                      {
                                        dropdownMenuButtonList.filter(v => !v.expose && !this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).StaticFiId262)).length > 0 &&
                        ...
                        <Form className='form'> {/* this line equals to <form className='form' onSubmit={handleSubmit} */}
                          ...
                          <div className='w-100'>
                            <Row>
                              {(authCol.StaticFiId262 || {}).visible &&
                                <Col lg={6} xl={6}>
                                  <div className='form__form-group'>
                                    {((true && this.constructor.ShowSpinner(AdmStaticFiState)) && <Skeleton height='20px' />) ||
                                      <label className='form__form-group-label'>{(columnLabel.StaticFiId262 || {}).ColumnHeader} {(columnLabel.StaticFiId262 || {}).ToolTip &&
                                        (<ControlledPopover id={(columnLabel.StaticFiId262 || {}).ColumnName} className='sticky-icon pt-0 lh-23' message={(columnLabel.StaticFiId262 || {}).ToolTip} />
                                        )}
                                      </label>
                                    }
                                    {((true && this.constructor.ShowSpinner(AdmStaticFiState)) && <Skeleton height='36px' />) ||
                                      <div className='form__form-group-field'>
                                        <Field
                                          type='text'
                                          name='cStaticFiId262'
                                          disabled={(authCol.StaticFiId262 || {}).readonly ? 'disabled' : ''} />
                                      </div>
                                    }
                                    {errors.cStaticFiId262 && touched.cStaticFiId262 && <span className='form__form-group-error'>{errors.cStaticFiId262}</span>}
                                  </div>
                                </Col>
                              }
                                ...
                                <Row>
                                  {
                                    bottomButtonList
                                      ...
                                      .map((v, i, a) => {
                                        if (this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).StaticFiId262)) return null;
                                        const buttonCount = a.length - a.filter(x => this.ActionSuppressed(authRow, x.buttonType, (currMst || {}).StaticFiId262));
                                        const colWidth = parseInt(12 / buttonCount, 10);
                                        const lastBtn = i === (a.length - 1);
                                        const outlineProperty = lastBtn ? false : true;
                                        return (
                                          <Col key={v.tid || v.order} xs={colWidth} sm={colWidth} className='btn-bottom-column' >
                                            {(this.constructor.ShowSpinner(AdmStaticFiState) && <Skeleton height='43px' />) ||
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { SavePage: AdmStaticFiReduxObj.SavePage.bind(AdmStaticFiReduxObj) },

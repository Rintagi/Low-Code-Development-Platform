...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmLabelVw || {}).initialized) {
  ...
  CriCultureId10InputChange() {
    return function (name, v) {
      this.props.SearchCriCultureId10(v);
    }.bind(this);
  }
  ...
  RefreshSearchList(values, { setErrors, resetForm, setValues /* setValues and other goodies */ }) {
    ...
    const refreshFn = (() => {
      this.props.SetScreenCriteria(values.search,
        {
          CultureId10: (values.cCriCultureId10) ? values.cCriCultureId10.value : '',
          LabelCat20: (values.cCriLabelCat20) ? values.cCriLabelCat20 : '',
        },
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmLabelVw || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).LabelId265) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    // if (!AdmLabelVwState.initialized || AdmLabelVwState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmLabelVwReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmLabelVwReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmLabelVwState.ScreenCriteria || {}
    ...
    const currMst = AdmLabelVwState.Mst;
    const currDtl = AdmLabelVwState.EditDtl;
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.LabelId265));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmLabelVwReduxObj.QuickFilterDdlToSelectList(AdmLabelVwState);
    ...
    const CriCultureId10List = screenCriDdlSelectors.CultureId10(AdmLabelVwState);
    const CriCultureId10Selected = CriCultureId10List.filter(obj => { return obj.key === screenCriteria.CultureId10.LastCriteria });
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmLabelVwState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmLabelVwState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).LabelId265)}
                    ...
                    <Formik
                      initialValues={{
                        cCriCultureId10: CriCultureId10Selected[0],
                        cCriLabelCat20: ((screenCriteria || {}).LabelCat20 || {}).LastCriteria,
                        search: screenCriteria.SearchStr || '',
                        cFilterId: (screenFilterSelected.length > 0 ? screenFilterSelected[0] : screenFilterList[0])
                      }}
                      key={AdmLabelVwState.searchListVersion}
                            ...
                            <Form className='form'>
                              <div className='form__form-group'>
                                <div className={`form__form-group filter-padding ${filterVisibility}`} key={screenCriteria.key}>
                                  <Row className='mb-5'>
                                    <Col xs={12} md={12}>
                                      <label className='form__form-group-label filter-label'>{(screenCriteria.CultureId10 || {}).ColumnHeader}</label>
                                      <div className='form__form-group-field filter-form-border'>
                                        <AutoCompleteField
                                          name='cCriCultureId10'
                                          onChange={this.SearchFilterValueChange(handleSubmit, setFieldValue, 'autocomplete', 'cCriCultureId10')}
                                          onInputChange={this.CriCultureId10InputChange()}
                                          value={values.cCriCultureId10}
                                          defaultSelected={CriCultureId10Selected}
                                          options={CriCultureId10List}
                                        />
                                      </div>
                                    </Col>
                                    <Col xs={12} md={12}>
                                      <label className='form__form-group-label filter-label'>{(screenCriteria.LabelCat20 || {}).ColumnHeader}</label>
                                      <div className='form__form-group-field filter-form-border'>
                                        <Field
                                          type='text'
                                          name='cCriLabelCat20'
                                          value={values.cCriLabelCat20}
                                          onBlur={this.SearchFilterTextValueChange(handleSubmit, setFieldValue, 'text', 'cCriLabelCat20')}
                                        />
                                      </div>
                                    </Col>
                                  </Row>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmLabelVwState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmLabelVwState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmLabelVwReduxObj.LoadInitPage.bind(AdmLabelVwReduxObj) },
    { LoadSearchList: AdmLabelVwReduxObj.LoadSearchList.bind(AdmLabelVwReduxObj) },
    { SelectMst: AdmLabelVwReduxObj.SelectMst.bind(AdmLabelVwReduxObj) },
    { DelMst: AdmLabelVwReduxObj.DelMst.bind(AdmLabelVwReduxObj) },
    { AddMst: AdmLabelVwReduxObj.AddMst.bind(AdmLabelVwReduxObj) },
    { AddDtl: AdmLabelVwReduxObj.AddDtl.bind(AdmLabelVwReduxObj) },
    { changeMstListFilterVisibility: AdmLabelVwReduxObj.ChangeMstListFilterVisibility.bind(AdmLabelVwReduxObj) },
    { SetScreenCriteria: AdmLabelVwReduxObj.SetScreenCriteria.bind(AdmLabelVwReduxObj) },
    { SearchCriCultureId10: AdmLabelVwReduxObj.SearchActions.SearchCriCultureId10.bind(AdmLabelVwReduxObj) },

...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmMenuDrg || {}).initialized) {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmMenuDrg || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).MenuId39) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    // if (!AdmMenuDrgState.initialized || AdmMenuDrgState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmMenuDrgReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmMenuDrgReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmMenuDrgState.ScreenCriteria || {}
    ...
    const currMst = AdmMenuDrgState.Mst;
    const currDtl = AdmMenuDrgState.EditDtl;
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.MenuId39));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmMenuDrgReduxObj.QuickFilterDdlToSelectList(AdmMenuDrgState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmMenuDrgState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmMenuDrgState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).MenuId39)}
                    ...
                    <Formik
                      ...
                      key={AdmMenuDrgState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmMenuDrgState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmMenuDrgState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmMenuDrgReduxObj.LoadInitPage.bind(AdmMenuDrgReduxObj) },
    { LoadSearchList: AdmMenuDrgReduxObj.LoadSearchList.bind(AdmMenuDrgReduxObj) },
    { SelectMst: AdmMenuDrgReduxObj.SelectMst.bind(AdmMenuDrgReduxObj) },
    { DelMst: AdmMenuDrgReduxObj.DelMst.bind(AdmMenuDrgReduxObj) },
    { AddMst: AdmMenuDrgReduxObj.AddMst.bind(AdmMenuDrgReduxObj) },
    { AddDtl: AdmMenuDrgReduxObj.AddDtl.bind(AdmMenuDrgReduxObj) },
    { changeMstListFilterVisibility: AdmMenuDrgReduxObj.ChangeMstListFilterVisibility.bind(AdmMenuDrgReduxObj) },
    { SetScreenCriteria: AdmMenuDrgReduxObj.SetScreenCriteria.bind(AdmMenuDrgReduxObj) },

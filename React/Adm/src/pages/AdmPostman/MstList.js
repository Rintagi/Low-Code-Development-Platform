...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmPostman || {}).initialized) {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmPostman || {}).Buttons || {};
  ...
  render() {
    ...
    // if (!AdmPostmanState.initialized || AdmPostmanState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmPostmanReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmPostmanReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmPostmanState.ScreenCriteria || {}
    ...
    const currMst = AdmPostmanState.Mst;
    const currDtl = AdmPostmanState.EditDtl;
    ...
    const screenFilterList = AdmPostmanReduxObj.QuickFilterDdlToSelectList(AdmPostmanState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmPostmanState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmPostmanState) && <i className='filter-applied'></i>}
                    ...
                    <Formik
                      ...
                      key={AdmPostmanState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmPostmanState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmPostmanState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmPostmanReduxObj.LoadInitPage.bind(AdmPostmanReduxObj) },
    { LoadSearchList: AdmPostmanReduxObj.LoadSearchList.bind(AdmPostmanReduxObj) },
    { SelectMst: AdmPostmanReduxObj.SelectMst.bind(AdmPostmanReduxObj) },
    { DelMst: AdmPostmanReduxObj.DelMst.bind(AdmPostmanReduxObj) },
    { AddMst: AdmPostmanReduxObj.AddMst.bind(AdmPostmanReduxObj) },
    { AddDtl: AdmPostmanReduxObj.AddDtl.bind(AdmPostmanReduxObj) },
    { changeMstListFilterVisibility: AdmPostmanReduxObj.ChangeMstListFilterVisibility.bind(AdmPostmanReduxObj) },
    { SetScreenCriteria: AdmPostmanReduxObj.SetScreenCriteria.bind(AdmPostmanReduxObj) },

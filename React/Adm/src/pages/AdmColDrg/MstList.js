...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmColDrg || {}).initialized) {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmColDrg || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).ScreenObjId14) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    // if (!AdmColDrgState.initialized || AdmColDrgState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmColDrgReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmColDrgReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmColDrgState.ScreenCriteria || {}
    ...
    const currMst = AdmColDrgState.Mst;
    const currDtl = AdmColDrgState.EditDtl;
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.ScreenObjId14));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmColDrgReduxObj.QuickFilterDdlToSelectList(AdmColDrgState);
    ...
    const CriScreenId10List = screenCriDdlSelectors.ScreenId10(AdmColDrgState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmColDrgState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmColDrgState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).ScreenObjId14)}
                    ...
                    <Formik
                      ...
                      key={AdmColDrgState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmColDrgState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmColDrgState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmColDrgReduxObj.LoadInitPage.bind(AdmColDrgReduxObj) },
    { LoadSearchList: AdmColDrgReduxObj.LoadSearchList.bind(AdmColDrgReduxObj) },
    { SelectMst: AdmColDrgReduxObj.SelectMst.bind(AdmColDrgReduxObj) },
    { DelMst: AdmColDrgReduxObj.DelMst.bind(AdmColDrgReduxObj) },
    { AddMst: AdmColDrgReduxObj.AddMst.bind(AdmColDrgReduxObj) },
    { AddDtl: AdmColDrgReduxObj.AddDtl.bind(AdmColDrgReduxObj) },
    { changeMstListFilterVisibility: AdmColDrgReduxObj.ChangeMstListFilterVisibility.bind(AdmColDrgReduxObj) },
    { SetScreenCriteria: AdmColDrgReduxObj.SetScreenCriteria.bind(AdmColDrgReduxObj) },
    { SearchCriScreenId10: AdmColDrgReduxObj.SearchActions.SearchCriScreenId10.bind(AdmColDrgReduxObj) },

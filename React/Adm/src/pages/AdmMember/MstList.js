...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmMember || {}).initialized) {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmMember || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).SredMebrId274) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    // if (!AdmMemberState.initialized || AdmMemberState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmMemberReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmMemberReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmMemberState.ScreenCriteria || {}
    ...
    const currMst = AdmMemberState.Mst;
    const currDtl = AdmMemberState.EditDtl;
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.SredMebrId274));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmMemberReduxObj.QuickFilterDdlToSelectList(AdmMemberState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmMemberState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmMemberState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).SredMebrId274)}
                    ...
                    <Formik
                      ...
                      key={AdmMemberState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmMemberState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmMemberState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmMemberReduxObj.LoadInitPage.bind(AdmMemberReduxObj) },
    { LoadSearchList: AdmMemberReduxObj.LoadSearchList.bind(AdmMemberReduxObj) },
    { SelectMst: AdmMemberReduxObj.SelectMst.bind(AdmMemberReduxObj) },
    { DelMst: AdmMemberReduxObj.DelMst.bind(AdmMemberReduxObj) },
    { AddMst: AdmMemberReduxObj.AddMst.bind(AdmMemberReduxObj) },
    { AddDtl: AdmMemberReduxObj.AddDtl.bind(AdmMemberReduxObj) },
    { changeMstListFilterVisibility: AdmMemberReduxObj.ChangeMstListFilterVisibility.bind(AdmMemberReduxObj) },
    { SetScreenCriteria: AdmMemberReduxObj.SetScreenCriteria.bind(AdmMemberReduxObj) },

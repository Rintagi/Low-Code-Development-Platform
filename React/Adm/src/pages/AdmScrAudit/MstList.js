...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmScrAudit || {}).initialized) {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmScrAudit || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).ScrAuditId1300) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    // if (!AdmScrAuditState.initialized || AdmScrAuditState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmScrAuditReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmScrAuditReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmScrAuditState.ScreenCriteria || {}
    ...
    const currMst = AdmScrAuditState.Mst;
    const currDtl = AdmScrAuditState.EditDtl;
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.ScrAuditId1300));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmScrAuditReduxObj.QuickFilterDdlToSelectList(AdmScrAuditState);
    ...
    const CriScreenId10List = screenCriDdlSelectors.ScreenId10(AdmScrAuditState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmScrAuditState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmScrAuditState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).ScrAuditId1300)}
                    ...
                    <Formik
                      ...
                      key={AdmScrAuditState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmScrAuditState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmScrAuditState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmScrAuditReduxObj.LoadInitPage.bind(AdmScrAuditReduxObj) },
    { LoadSearchList: AdmScrAuditReduxObj.LoadSearchList.bind(AdmScrAuditReduxObj) },
    { SelectMst: AdmScrAuditReduxObj.SelectMst.bind(AdmScrAuditReduxObj) },
    { DelMst: AdmScrAuditReduxObj.DelMst.bind(AdmScrAuditReduxObj) },
    { AddMst: AdmScrAuditReduxObj.AddMst.bind(AdmScrAuditReduxObj) },
    { AddDtl: AdmScrAuditReduxObj.AddDtl.bind(AdmScrAuditReduxObj) },
    { changeMstListFilterVisibility: AdmScrAuditReduxObj.ChangeMstListFilterVisibility.bind(AdmScrAuditReduxObj) },
    { SetScreenCriteria: AdmScrAuditReduxObj.SetScreenCriteria.bind(AdmScrAuditReduxObj) },
    { SearchCriScreenId10: AdmScrAuditReduxObj.SearchActions.SearchCriScreenId10.bind(AdmScrAuditReduxObj) },

...
class DtlList extends RintagiScreen {
    ...
    return function (evt) {
      ...
      const copyFn = () => {
        if (currDtlId) {
          ...
          if (useMobileView) {
            const naviBar = getNaviBar('DtlList', mst, {}, this.props.AdmRowOvrd.Label);
  ...
  render() {
    ...
    const itemList = AdmRowOvrdReduxObj.DtlListToSelectList(AdmRowOvrdState, 'RowOvrdPrmId239');
    ...
    const dtlFilter = AdmRowOvrdState.DtlFilter;
    const filterList = AdmRowOvrdState.DtlFilter.FilterColumnDdl.map((v, i) => ({ key: v.ColName, label: v.ColumnHeader, value: v.ColName, pos: i }))
                        ...
                        resetForm
                          ...
                          <div>
                                      ...
                                      <ButtonGroup className='btn-group--icons'>
                                        ...
                                        </Button>
                                        {
                                          dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                            if ((!activeSelectionVisible && v.buttonType !== 'InsRow') || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).RowOvrdId238)) return null;
                                            return (
                                              <Button
                                                key={v.tid}
                                        ...
                                        {activeSelectionVisible &&
                                          dropdownMenuButtonList
                                            .filter(v => !v.expose)
                                            .filter(v => (!this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).RowOvrdId238,(currDtl || {}).RowOvrdPrmId239)))
                                            .length > 0 &&
                                        ...
                                        <DropdownMenu right className={`dropdown__menu dropdown-options`}>
                                          {
                                            ...
                                            dropdownMenuButtonList.filter(v => !v.expose).map(v => {
                                              if ((!activeSelectionVisible && v.buttonType !== 'InsRow') || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).RowOvrdId238,(currDtl || {}).RowOvrdPrmId239)) return null;
                                              return (
                            ...
                            <Form className='form'>
                                ...
                                </div>
                                {
                                  itemList.slice(0, dtlFilter.Limit).map(
                                    (obj, i) => {
                                      ...
                                      return (
                                        <div className='form__form-group-narrow list-divider' key={obj.RowOvrdPrmId239 || ('_' + i)}>
                                          <div className='form__form-group-field'>
                                            <label className='radio-btn radio-btn--button margin-narrow'>
                                              <Field
                                                key={obj.RowOvrdPrmId239 || ('_' + i)}
                                                className='radio-btn__radio'
                                                name='expenseItem'
                                                type='radio'
                                                value={values.name}
                                                onClick={this.SelectDtlListRow(naviSelectBar, currMst, obj, i, 'RowOvrdId238', 'RowOvrdPrmId239')}
                                              ...
                                              </span>
                                              {
                                                rowMenuButtonList
                                                  ...
                                                  .map((v, i) => {
                                                    return (
                                                      <button type='button' onClick={this.ScreenButtonAction[v.buttonType]({ naviBar: naviSelectBar, mst: currMst, useMobileView, dtlId: obj.RowOvrdPrmId239 })} className={`${v.exposedClassName}`}><i className={`${v.iconClassName}`}></i></button>
                                                    )
                                                  })
                                              }
                                              {
                                                ...
                                                <UncontrolledDropdown className={`btn-row-dropdown`}>
                                                  ...
                                                  <DropdownMenu right className={`dropdown__menu dropdown-options`}>
                                                    {rowMenuButtonList
                                                      .filter(v => !v.expose || useMobileView)
                                                      .map((v) => {
                                                        if (this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).RowOvrdId238,obj.RowOvrdPrmId239)) return null;
                                                        return <DropdownItem key={v.tid} onClick={this.ScreenButtonAction[v.buttonType]({ naviBar: naviSelectBar, mst: currMst, useMobileView, dtlId: obj.RowOvrdPrmId239 })} className={`${v.className}`}><i className={`${v.iconClassName} mr-10`}></i>{v.label}</DropdownItem>
                                                      })
                                                    }
                                                  </DropdownMenu>
                                                </UncontrolledDropdown>
                              ...
                              <Button onClick={this.props.ViewMoreDtl} className={`${viewMoreClass} btn btn-view-more-blue account__btn`} type='button'>{strFormat(IncrementMsg, this.props.AdmRowOvrd.DtlFilter.PageSize)}<br /><i className='fa fa-arrow-down'></i></Button>
            ...
            {!useMobileView && <Col xs={6}>
              ...
              {!activeSelectionVisible &&
                ...
                <div className='empty-block'>
                  ...
                  <p className='create-new-message'>{NoDetailMsg}. <span className='link-imitation' onClick={this.AddNewDtl({ naviBar, mstId: currMst.RowOvrdId238 })}>{AddDetailMsg}</span></p>
                </div>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { SelectDtl: AdmRowOvrdReduxObj.SelectDtl.bind(AdmRowOvrdReduxObj) },
    { SavePage: AdmRowOvrdReduxObj.SavePage.bind(AdmRowOvrdReduxObj) },
    { AddDtl: AdmRowOvrdReduxObj.AddDtl.bind(AdmRowOvrdReduxObj) },
    { ChangeDtlListFilterVisibility: AdmRowOvrdReduxObj.ChangeDtlListFilterVisibility.bind(AdmRowOvrdReduxObj) },
    { ChangeDtlListFilter: AdmRowOvrdReduxObj.ChangeDtlListFilter.bind(AdmRowOvrdReduxObj) },
    { ViewMoreDtl: AdmRowOvrdReduxObj.ViewMoreDtl.bind(AdmRowOvrdReduxObj) },
    { setTitle: setTitle },

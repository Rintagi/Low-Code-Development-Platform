...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmRowOvrd || {}).initialized) {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmRowOvrd || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).RowOvrdId238) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    // if (!AdmRowOvrdState.initialized || AdmRowOvrdState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmRowOvrdReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmRowOvrdReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmRowOvrdState.ScreenCriteria || {}
    ...
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.RowOvrdId238));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmRowOvrdReduxObj.QuickFilterDdlToSelectList(AdmRowOvrdState);
    ...
    const CriScreenId10List = screenCriDdlSelectors.ScreenId10(AdmRowOvrdState);
    const CriScreenId10Selected = CriScreenId10List.filter(obj => { return obj.key === screenCriteria.ScreenId10.LastCriteria });
    const CriReportId20List = screenCriDdlSelectors.ReportId20(AdmRowOvrdState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmRowOvrdState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmRowOvrdState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).RowOvrdId238)}
                    ...
                    <Formik
                      ...
                      key={AdmRowOvrdState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmRowOvrdState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmRowOvrdState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmRowOvrdReduxObj.LoadInitPage.bind(AdmRowOvrdReduxObj) },
    { LoadSearchList: AdmRowOvrdReduxObj.LoadSearchList.bind(AdmRowOvrdReduxObj) },
    { SelectMst: AdmRowOvrdReduxObj.SelectMst.bind(AdmRowOvrdReduxObj) },
    ...
    { changeMstListFilterVisibility: AdmRowOvrdReduxObj.ChangeMstListFilterVisibility.bind(AdmRowOvrdReduxObj) },
    { SetScreenCriteria: AdmRowOvrdReduxObj.SetScreenCriteria.bind(AdmRowOvrdReduxObj) },
    { SearchCriScreenId10: AdmRowOvrdReduxObj.SearchActions.SearchCriScreenId10.bind(AdmRowOvrdReduxObj) },
    { SearchCriReportId20: AdmRowOvrdReduxObj.SearchActions.SearchCriReportId20.bind(AdmRowOvrdReduxObj) },

...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmAuthCol || {}).initialized) {
  ...
  DelMst({ naviBar, ScreenButton, mst, mstId }) {
    ...
    return this.Prompt({
      okFn: function (evt) {
        const fromMstId = mstId || mst.ScreenObjId14;
        this.props.DelMst(this.props.AdmAuthCol, fromMstId);
      }.bind(this),
  ...
  render() {
    ...
    const screenDdlSelectors = AdmAuthColReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmAuthColReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmAuthColState.ScreenCriteria || {}
    const selectList = AdmAuthColReduxObj.SearchListToSelectList(AdmAuthColState);
    const itemList = AdmAuthColState.Dtl || [];
    ...
    const screenFilterList = AdmAuthColReduxObj.QuickFilterDdlToSelectList(AdmAuthColState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmAuthColState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmAuthColState) && <i className='filter-applied'></i>}
                    ...
                    <Formik
                      ...
                      key={AdmAuthColState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmAuthColState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmAuthColState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmAuthColReduxObj.LoadInitPage.bind(AdmAuthColReduxObj) },
    { LoadSearchList: AdmAuthColReduxObj.LoadSearchList.bind(AdmAuthColReduxObj) },
    { SelectMst: AdmAuthColReduxObj.SelectMst.bind(AdmAuthColReduxObj) },
    { DelMst: AdmAuthColReduxObj.DelMst.bind(AdmAuthColReduxObj) },
    { AddMst: AdmAuthColReduxObj.AddMst.bind(AdmAuthColReduxObj) },
    { AddDtl: AdmAuthColReduxObj.AddDtl.bind(AdmAuthColReduxObj) },
    { changeMstListFilterVisibility: AdmAuthColReduxObj.ChangeMstListFilterVisibility.bind(AdmAuthColReduxObj) },
    { SetScreenCriteria: AdmAuthColReduxObj.SetScreenCriteria.bind(AdmAuthColReduxObj) },

...
import AdmAuthColReduxObj from '../../redux/AdmAuthCol';
...
class DtlList extends RintagiScreen {
    ...
    return function (evt) {
      ...
      const copyFn = () => {
        if (currDtlId) {
          ...
          if (useMobileView) {
            const naviBar = getNaviBar('DtlList', mst, {}, this.props.AdmAuthCol.Label);
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmAuthCol || {}).Buttons || {};
  ...
  render() {
    ...
    //if (!AdmAuthColState.initialized || AdmAuthColState.page_loading) return null;
    ...
    const itemList = AdmAuthColReduxObj.DtlListToSelectList(AdmAuthColState, 'ColOvrdId241');
    const selectList = AdmAuthColReduxObj.SearchListToSelectList(AdmAuthColState);
    ...
    const dtlFilter = AdmAuthColState.DtlFilter;
    const filterList = AdmAuthColState.DtlFilter.FilterColumnDdl.map((v, i) => ({ key: v.ColName, label: v.ColumnHeader, value: v.ColName, pos: i }))
                        ...
                        resetForm
                          ...
                          <div>
                                      ...
                                      <ButtonGroup className='btn-group--icons'>
                                        ...
                                        {activeSelectionVisible &&
                                          dropdownMenuButtonList
                                            .filter(v => !v.expose)
                                            .filter(v => (!this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).ScreenObjId14,(currDtl || {}).ColOvrdId241)))
                                            .length > 0 &&
                                        ...
                                        <DropdownMenu right className={`dropdown__menu dropdown-options`}>
                                          {
                                            ...
                                            dropdownMenuButtonList.filter(v => !v.expose).map(v => {
                                              if ((!activeSelectionVisible && v.buttonType !== 'InsRow') || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).ScreenObjId14,(currDtl || {}).ColOvrdId241)) return null;
                                              return (
                            ...
                            <Form className='form'>
                                ...
                                </div>
                                {
                                  itemList.slice(0, dtlFilter.Limit).map(
                                    (obj, i) => {
                                      ...
                                      return (
                                        <div className='form__form-group-narrow list-divider' key={obj.ColOvrdId241 || ('_' + i)}>
                                          <div className='form__form-group-field'>
                                            <label className='radio-btn radio-btn--button margin-narrow'>
                                              <Field
                                                key={obj.ColOvrdId241 || ('_' + i)}
                                                className='radio-btn__radio'
                                                name='expenseItem'
                                                type='radio'
                                                value={values.name}
                                                onClick={this.SelectDtlListRow(naviSelectBar, currMst, obj, i, 'ScreenObjId14', 'ColOvrdId241')}
                                              ...
                                              </span>
                                              {
                                                rowMenuButtonList
                                                  ...
                                                  .map((v, i) => {
                                                    return (
                                                      <button type='button' onClick={this.ScreenButtonAction[v.buttonType]({ naviBar: naviSelectBar, mst: currMst, useMobileView, dtlId: obj.ColOvrdId241 })} className={`${v.exposedClassName}`}><i className={`${v.iconClassName}`}></i></button>
                                                    )
                                                  })
                                              }
                                              {
                                                ...
                                                <UncontrolledDropdown className={`btn-row-dropdown`}>
                                                  ...
                                                  <DropdownMenu right className={`dropdown__menu dropdown-options`}>
                                                    {rowMenuButtonList
                                                      .filter(v => !v.expose || useMobileView)
                                                      .map((v) => {
                                                        if (this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).ScreenObjId14,obj.ColOvrdId241)) return null;
                                                        return <DropdownItem key={v.tid} onClick={this.ScreenButtonAction[v.buttonType]({ naviBar: naviSelectBar, mst: currMst, useMobileView, dtlId: obj.ColOvrdId241 })} className={`${v.className}`}><i className={`${v.iconClassName} mr-10`}></i>{v.label}</DropdownItem>
                                                      })
                                                    }
                                                  </DropdownMenu>
                                                </UncontrolledDropdown>
                              ...
                              <Button onClick={this.props.ViewMoreDtl} className={`${viewMoreClass} btn btn-view-more-blue account__btn`} type='button'>{strFormat(IncrementMsg, this.props.AdmAuthCol.DtlFilter.PageSize)}<br /><i className='fa fa-arrow-down'></i></Button>
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { SelectDtl: AdmAuthColReduxObj.SelectDtl.bind(AdmAuthColReduxObj) },
    { SavePage: AdmAuthColReduxObj.SavePage.bind(AdmAuthColReduxObj) },
    { AddDtl: AdmAuthColReduxObj.AddDtl.bind(AdmAuthColReduxObj) },
    { ChangeDtlListFilterVisibility: AdmAuthColReduxObj.ChangeDtlListFilterVisibility.bind(AdmAuthColReduxObj) },
    { ChangeDtlListFilter: AdmAuthColReduxObj.ChangeDtlListFilter.bind(AdmAuthColReduxObj) },
    { ViewMoreDtl: AdmAuthColReduxObj.ViewMoreDtl.bind(AdmAuthColReduxObj) },
    { setTitle: setTitle },

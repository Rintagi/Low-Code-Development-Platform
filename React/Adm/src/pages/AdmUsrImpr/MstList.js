...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmUsrImpr || {}).initialized) {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmUsrImpr || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).UsrImprId95) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    // if (!AdmUsrImprState.initialized || AdmUsrImprState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmUsrImprReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmUsrImprReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmUsrImprState.ScreenCriteria || {}
    ...
    const currMst = AdmUsrImprState.Mst;
    const currDtl = AdmUsrImprState.EditDtl;
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.UsrImprId95));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmUsrImprReduxObj.QuickFilterDdlToSelectList(AdmUsrImprState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmUsrImprState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmUsrImprState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).UsrImprId95)}
                    ...
                    <Formik
                      ...
                      key={AdmUsrImprState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmUsrImprState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmUsrImprState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmUsrImprReduxObj.LoadInitPage.bind(AdmUsrImprReduxObj) },
    { LoadSearchList: AdmUsrImprReduxObj.LoadSearchList.bind(AdmUsrImprReduxObj) },
    { SelectMst: AdmUsrImprReduxObj.SelectMst.bind(AdmUsrImprReduxObj) },
    { DelMst: AdmUsrImprReduxObj.DelMst.bind(AdmUsrImprReduxObj) },
    { AddMst: AdmUsrImprReduxObj.AddMst.bind(AdmUsrImprReduxObj) },
    { AddDtl: AdmUsrImprReduxObj.AddDtl.bind(AdmUsrImprReduxObj) },
    { changeMstListFilterVisibility: AdmUsrImprReduxObj.ChangeMstListFilterVisibility.bind(AdmUsrImprReduxObj) },
    { SetScreenCriteria: AdmUsrImprReduxObj.SetScreenCriteria.bind(AdmUsrImprReduxObj) },

...
class MstRecord extends RintagiScreen {
  ...
  PaypalPayoutBtn({ submitForm, ScreenButton, naviBar, redirectTo, onSuccess }) {
    return function (evt) {
      this.OnClickColumeName = 'PaypalPayoutBtn';
      //Enter Custom Code here, eg: submitForm();
      evt.preventDefault();
    }.bind(this);
  }
  PaypalCCBtn({ submitForm, ScreenButton, naviBar, redirectTo, onSuccess }) {
    return function (evt) {
      this.OnClickColumeName = 'PaypalCCBtn';
      //Enter Custom Code here, eg: submitForm();
      evt.preventDefault();
    }.bind(this);
  }
    ...
    return function (evt) {
      ...
      const copyFn = () => {
        if (fromMstId) {
          ...
          if (useMobileView) {
            const naviBar = getNaviBar('MstRecord', {}, {}, this.props.AdmPayment.Label);
              ...
              <div className='account__card shadow-box rad-4'>
                ...
                <Formik
                  initialValues={{
                    ...
                    cDescription: formatContent(Description || '', 'TextBox'),
                    cItemDescription: formatContent(ItemDescription || '', 'TextBox'),
                    cCurrency: formatContent(Currency || '', 'TextBox'),
                    cAmt: formatContent(Amt || '', 'TextBox'),
                    cTaxAmt: formatContent(TaxAmt || '', 'TextBox'),
                    cPayTo: formatContent(PayTo || '', 'TextBox'),
                    cPayFrom: formatContent(PayFrom || '', 'TextBox'),
                    cCCNbr: formatContent(CCNbr || '', 'TextBox'),
                    cCCExpiryMonth: formatContent(CCExpiryMonth || '', 'TextBox'),
                    cCCType: formatContent(CCType || '', 'TextBox'),
                    cCCExpiryYear: formatContent(CCExpiryYear || '', 'TextBox'),
                    cCCCVV: formatContent(CCCVV || '', 'TextBox'),
                  }}

...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmSredTime || {}).initialized) {
  ...
  CriMemberId10InputChange() {
    return function (name, v) {
      this.props.SearchCriMemberId10(v);
    }.bind(this);
  }
  ...
  RefreshSearchList(values, { setErrors, resetForm, setValues /* setValues and other goodies */ }) {
    ...
    const refreshFn = (() => {
      this.props.SetScreenCriteria(values.search,
        {
          MemberId10: (values.cCriMemberId10) ? values.cCriMemberId10.value : '',
          SredTimeDt20: (values.cCriSredTimeDt20) ? values.cCriSredTimeDt20 : '',
          SredTimeDt30: (values.cCriSredTimeDt30) ? values.cCriSredTimeDt30 : '',
        },
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmSredTime || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).SredTimeId272) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    // if (!AdmSredTimeState.initialized || AdmSredTimeState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmSredTimeReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmSredTimeReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmSredTimeState.ScreenCriteria || {}
    ...
    const currMst = AdmSredTimeState.Mst;
    const currDtl = AdmSredTimeState.EditDtl;
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.SredTimeId272));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmSredTimeReduxObj.QuickFilterDdlToSelectList(AdmSredTimeState);
    ...
    const CriMemberId10List = screenCriDdlSelectors.MemberId10(AdmSredTimeState);
    const CriMemberId10Selected = CriMemberId10List.filter(obj => { return obj.key === screenCriteria.MemberId10.LastCriteria });
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmSredTimeState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmSredTimeState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).SredTimeId272)}
                    ...
                    <Formik
                      initialValues={{
                        cCriMemberId10: CriMemberId10Selected[0],
                        cCriSredTimeDt20: ((screenCriteria || {}).SredTimeDt20 || {}).LastCriteria,
                        cCriSredTimeDt30: ((screenCriteria || {}).SredTimeDt30 || {}).LastCriteria,
                        search: screenCriteria.SearchStr || '',
                        cFilterId: (screenFilterSelected.length > 0 ? screenFilterSelected[0] : screenFilterList[0])
                      }}
                      key={AdmSredTimeState.searchListVersion}
                            ...
                            <Form className='form'>
                              <div className='form__form-group'>
                                <div className={`form__form-group filter-padding ${filterVisibility}`} key={screenCriteria.key}>
                                  <Row className='mb-5'>
                                    <Col xs={12} md={12}>
                                      <label className='form__form-group-label filter-label'>{(screenCriteria.MemberId10 || {}).ColumnHeader}</label>
                                      <div className='form__form-group-field filter-form-border'>
                                        <AutoCompleteField
                                          name='cCriMemberId10'
                                          onChange={this.SearchFilterValueChange(handleSubmit, setFieldValue, 'autocomplete', 'cCriMemberId10')}
                                          onInputChange={this.CriMemberId10InputChange()}
                                          value={values.cCriMemberId10}
                                          defaultSelected={CriMemberId10Selected}
                                          options={CriMemberId10List}
                                        />
                                      </div>
                                    </Col>
                                    <Col xs={12} md={12}>
                                      <label className='form__form-group-label filter-label'>{(screenCriteria.SredTimeDt20 || {}).ColumnHeader}</label>
                                      <div className='form__form-group-field filter-form-border'>
                                        <Field
                                          type='text'
                                          name='cCriSredTimeDt20'
                                          value={values.cCriSredTimeDt20}
                                          onBlur={this.SearchFilterTextValueChange(handleSubmit, setFieldValue, 'text', 'cCriSredTimeDt20')}
                                        />
                                      </div>
                                    </Col>
                                    <Col xs={12} md={12}>
                                      <label className='form__form-group-label filter-label'>{(screenCriteria.SredTimeDt30 || {}).ColumnHeader}</label>
                                      <div className='form__form-group-field filter-form-border'>
                                        <Field
                                          type='text'
                                          name='cCriSredTimeDt30'
                                          value={values.cCriSredTimeDt30}
                                          onBlur={this.SearchFilterTextValueChange(handleSubmit, setFieldValue, 'text', 'cCriSredTimeDt30')}
                                        />
                                      </div>
                                    </Col>
                                  </Row>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmSredTimeState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmSredTimeState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmSredTimeReduxObj.LoadInitPage.bind(AdmSredTimeReduxObj) },
    { LoadSearchList: AdmSredTimeReduxObj.LoadSearchList.bind(AdmSredTimeReduxObj) },
    { SelectMst: AdmSredTimeReduxObj.SelectMst.bind(AdmSredTimeReduxObj) },
    { DelMst: AdmSredTimeReduxObj.DelMst.bind(AdmSredTimeReduxObj) },
    { AddMst: AdmSredTimeReduxObj.AddMst.bind(AdmSredTimeReduxObj) },
    { AddDtl: AdmSredTimeReduxObj.AddDtl.bind(AdmSredTimeReduxObj) },
    { changeMstListFilterVisibility: AdmSredTimeReduxObj.ChangeMstListFilterVisibility.bind(AdmSredTimeReduxObj) },
    { SetScreenCriteria: AdmSredTimeReduxObj.SetScreenCriteria.bind(AdmSredTimeReduxObj) },
    { SearchCriMemberId10: AdmSredTimeReduxObj.SearchActions.SearchCriMemberId10.bind(AdmSredTimeReduxObj) },

...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmLicense || {}).initialized) {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmLicense || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).SystemId1317) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    // if (!AdmLicenseState.initialized || AdmLicenseState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmLicenseReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmLicenseReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmLicenseState.ScreenCriteria || {}
    ...
    const currMst = AdmLicenseState.Mst;
    const currDtl = AdmLicenseState.EditDtl;
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.SystemId1317));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmLicenseReduxObj.QuickFilterDdlToSelectList(AdmLicenseState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmLicenseState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmLicenseState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).SystemId1317)}
                    ...
                    <Formik
                      ...
                      key={AdmLicenseState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmLicenseState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmLicenseState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmLicenseReduxObj.LoadInitPage.bind(AdmLicenseReduxObj) },
    { LoadSearchList: AdmLicenseReduxObj.LoadSearchList.bind(AdmLicenseReduxObj) },
    { SelectMst: AdmLicenseReduxObj.SelectMst.bind(AdmLicenseReduxObj) },
    { DelMst: AdmLicenseReduxObj.DelMst.bind(AdmLicenseReduxObj) },
    { AddMst: AdmLicenseReduxObj.AddMst.bind(AdmLicenseReduxObj) },
    { AddDtl: AdmLicenseReduxObj.AddDtl.bind(AdmLicenseReduxObj) },
    { changeMstListFilterVisibility: AdmLicenseReduxObj.ChangeMstListFilterVisibility.bind(AdmLicenseReduxObj) },
    { SetScreenCriteria: AdmLicenseReduxObj.SetScreenCriteria.bind(AdmLicenseReduxObj) },

...
class MstRecord extends RintagiScreen {
  ...
  Generate({ submitForm, ScreenButton, naviBar, redirectTo, onSuccess }) {
    return function (evt) {
      this.OnClickColumeName = 'Generate';
      //Enter Custom Code here, eg: submitForm();
      evt.preventDefault();
    }.bind(this);
  }
                          ...
                          <div className='w-100'>
                            <Row>
                                  ...
                                  <div className='d-block'>
                                    {(authCol.launch || {}).visible &&
                                      <Button color='secondary' size='sm' className='admin-ap-post-btn mb-10'
                                        ...
                                        {auxLabels.launch || (columnLabel.launch || {}).ColumnName}
                                      </Button>}
                                  </div>
                                </div>
                              </Col>
                              <Col lg={6} xl={6}>
                                <div className='form__form-group'>
                                  <div className='d-block'>
                                    {(authCol.Generate || {}).visible &&
                                      <Button color='secondary' size='sm' className='admin-ap-post-btn mb-10'
                                        disabled={(authCol.Generate || {}).readonly || !(authCol.Generate || {}).visible}
                                        onClick={this.Generate({ naviBar, submitForm, currMst })} >
                                        {auxLabels.Generate || (columnLabel.Generate || {}).ColumnName}
                                      </Button>}
                                  </div>
                                </div>
                              </Col>

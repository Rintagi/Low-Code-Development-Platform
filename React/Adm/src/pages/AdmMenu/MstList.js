...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmMenu || {}).initialized) {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmMenu || {}).Buttons || {};
  ...
  render() {
    ...
    // if (!AdmMenuState.initialized || AdmMenuState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmMenuReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmMenuReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmMenuState.ScreenCriteria || {}
    ...
    const currMst = AdmMenuState.Mst;
    const currDtl = AdmMenuState.EditDtl;
    ...
    const screenFilterList = AdmMenuReduxObj.QuickFilterDdlToSelectList(AdmMenuState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmMenuState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmMenuState) && <i className='filter-applied'></i>}
                    ...
                    <Formik
                      ...
                      key={AdmMenuState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmMenuState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmMenuState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmMenuReduxObj.LoadInitPage.bind(AdmMenuReduxObj) },
    { LoadSearchList: AdmMenuReduxObj.LoadSearchList.bind(AdmMenuReduxObj) },
    { SelectMst: AdmMenuReduxObj.SelectMst.bind(AdmMenuReduxObj) },
    { DelMst: AdmMenuReduxObj.DelMst.bind(AdmMenuReduxObj) },
    { AddMst: AdmMenuReduxObj.AddMst.bind(AdmMenuReduxObj) },
    { AddDtl: AdmMenuReduxObj.AddDtl.bind(AdmMenuReduxObj) },
    { changeMstListFilterVisibility: AdmMenuReduxObj.ChangeMstListFilterVisibility.bind(AdmMenuReduxObj) },
    { SetScreenCriteria: AdmMenuReduxObj.SetScreenCriteria.bind(AdmMenuReduxObj) },

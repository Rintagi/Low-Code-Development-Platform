...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmWebRule || {}).initialized) {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmWebRule || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).WebRuleId128) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    // if (!AdmWebRuleState.initialized || AdmWebRuleState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmWebRuleReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmWebRuleReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmWebRuleState.ScreenCriteria || {}
    ...
    const currMst = AdmWebRuleState.Mst;
    const currDtl = AdmWebRuleState.EditDtl;
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.WebRuleId128));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmWebRuleReduxObj.QuickFilterDdlToSelectList(AdmWebRuleState);
    ...
    const CriScreenId10List = screenCriDdlSelectors.ScreenId10(AdmWebRuleState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmWebRuleState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmWebRuleState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).WebRuleId128)}
                    ...
                    <Formik
                      ...
                      key={AdmWebRuleState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmWebRuleState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmWebRuleState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmWebRuleReduxObj.LoadInitPage.bind(AdmWebRuleReduxObj) },
    { LoadSearchList: AdmWebRuleReduxObj.LoadSearchList.bind(AdmWebRuleReduxObj) },
    { SelectMst: AdmWebRuleReduxObj.SelectMst.bind(AdmWebRuleReduxObj) },
    { DelMst: AdmWebRuleReduxObj.DelMst.bind(AdmWebRuleReduxObj) },
    { AddMst: AdmWebRuleReduxObj.AddMst.bind(AdmWebRuleReduxObj) },
    { AddDtl: AdmWebRuleReduxObj.AddDtl.bind(AdmWebRuleReduxObj) },

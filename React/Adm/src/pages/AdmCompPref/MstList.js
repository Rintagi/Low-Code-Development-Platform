...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmCompPref || {}).initialized) {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmCompPref || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).CompPrefId1329) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    // if (!AdmCompPrefState.initialized || AdmCompPrefState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmCompPrefReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmCompPrefReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmCompPrefState.ScreenCriteria || {}
    ...
    const currMst = AdmCompPrefState.Mst;
    const currDtl = AdmCompPrefState.EditDtl;
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.CompPrefId1329));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmCompPrefReduxObj.QuickFilterDdlToSelectList(AdmCompPrefState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmCompPrefState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmCompPrefState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).CompPrefId1329)}
                    ...
                    <Formik
                      ...
                      key={AdmCompPrefState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmCompPrefState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmCompPrefState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmCompPrefReduxObj.LoadInitPage.bind(AdmCompPrefReduxObj) },
    { LoadSearchList: AdmCompPrefReduxObj.LoadSearchList.bind(AdmCompPrefReduxObj) },
    { SelectMst: AdmCompPrefReduxObj.SelectMst.bind(AdmCompPrefReduxObj) },
    { DelMst: AdmCompPrefReduxObj.DelMst.bind(AdmCompPrefReduxObj) },
    { AddMst: AdmCompPrefReduxObj.AddMst.bind(AdmCompPrefReduxObj) },
    { AddDtl: AdmCompPrefReduxObj.AddDtl.bind(AdmCompPrefReduxObj) },
    { changeMstListFilterVisibility: AdmCompPrefReduxObj.ChangeMstListFilterVisibility.bind(AdmCompPrefReduxObj) },
    { SetScreenCriteria: AdmCompPrefReduxObj.SetScreenCriteria.bind(AdmCompPrefReduxObj) },

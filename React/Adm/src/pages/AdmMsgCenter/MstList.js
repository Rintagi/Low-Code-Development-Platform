...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmMsgCenter || {}).initialized) {
  ...
  CopyHdr({ naviBar, ScreenButton, useMobileView, mst, mstId }) {
    ...
    return function (evt) {
      evt.preventDefault();
      const fromMstId = mstId || mst.MsgId146;
      const copyFn = () => {
  ...
  DelMst({ naviBar, ScreenButton, mst, mstId }) {
    ...
    return this.Prompt({
      okFn: function (evt) {
        const fromMstId = mstId || mst.MsgId146;
        this.props.DelMst(this.props.AdmMsgCenter, fromMstId);
      }.bind(this),
  ...
  RefreshSearchList(values, { setErrors, resetForm, setValues /* setValues and other goodies */ }) {
    ...
    const refreshFn = (() => {
      this.props.SetScreenCriteria(values.search,
        {
          MsgId10: (values.cCriMsgId10) ? values.cCriMsgId10 : '',
          MsgSource20: (values.cCriMsgSource20) ? values.cCriMsgSource20 : '',
          MsgName30: (values.cCriMsgName30) ? values.cCriMsgName30 : '',
        },
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmMsgCenter || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).MsgId146) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    // if (!AdmMsgCenterState.initialized || AdmMsgCenterState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmMsgCenterReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmMsgCenterReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmMsgCenterState.ScreenCriteria || {}
    const selectList = AdmMsgCenterReduxObj.SearchListToSelectList(AdmMsgCenterState);
    const itemList = AdmMsgCenterState.Dtl || [];
    ...
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.MsgId146));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmMsgCenterReduxObj.QuickFilterDdlToSelectList(AdmMsgCenterState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmMsgCenterState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmMsgCenterState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).MsgId146)}
                    ...
                    <Formik
                      initialValues={{
                        cCriMsgId10: ((screenCriteria || {}).MsgId10 || {}).LastCriteria,
                        cCriMsgSource20: ((screenCriteria || {}).MsgSource20 || {}).LastCriteria,
                        cCriMsgName30: ((screenCriteria || {}).MsgName30 || {}).LastCriteria,
                        search: screenCriteria.SearchStr || '',
                        cFilterId: (screenFilterSelected.length > 0 ? screenFilterSelected[0] : screenFilterList[0])
                      }}
                      key={AdmMsgCenterState.searchListVersion}
                            ...
                            <Form className='form'>
                              <div className='form__form-group'>
                                <div className={`form__form-group filter-padding ${filterVisibility}`} key={screenCriteria.key}>
                                  <Row className='mb-5'>
                                    <Col xs={12} md={12}>
                                      <label className='form__form-group-label filter-label'>{(screenCriteria.MsgId10 || {}).ColumnHeader}</label>
                                      <div className='form__form-group-field filter-form-border'>
                                        <Field
                                          type='text'
                                          name='cCriMsgId10'
                                          value={values.cCriMsgId10}
                                          onBlur={this.SearchFilterTextValueChange(handleSubmit, setFieldValue, 'text', 'cCriMsgId10')}
                                        />
                                      </div>
                                    </Col>
                                    <Col xs={12} md={12}>
                                      <label className='form__form-group-label filter-label'>{(screenCriteria.MsgSource20 || {}).ColumnHeader}</label>
                                      <div className='form__form-group-field filter-form-border'>
                                        <Field
                                          type='text'
                                          name='cCriMsgSource20'
                                          value={values.cCriMsgSource20}
                                          onBlur={this.SearchFilterTextValueChange(handleSubmit, setFieldValue, 'text', 'cCriMsgSource20')}
                                        />
                                      </div>
                                    </Col>
                                    <Col xs={12} md={12}>
                                      <label className='form__form-group-label filter-label'>{(screenCriteria.MsgName30 || {}).ColumnHeader}</label>
                                      <div className='form__form-group-field filter-form-border'>
                                        <Field
                                          type='text'
                                          name='cCriMsgName30'
                                          value={values.cCriMsgName30}
                                          onBlur={this.SearchFilterTextValueChange(handleSubmit, setFieldValue, 'text', 'cCriMsgName30')}
                                        />
                                      </div>
                                    </Col>
                                  </Row>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmMsgCenterState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmMsgCenterState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
                                    ...
                                    <Row className='btn-bottom-row'>
                                      {
                                        bottomButtonList
                                          .filter(v => v.expose)
                                          .map((v, i, a) => {
                                            if (this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).MsgId146)) return null;
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmMsgCenterReduxObj.LoadInitPage.bind(AdmMsgCenterReduxObj) },
    { LoadSearchList: AdmMsgCenterReduxObj.LoadSearchList.bind(AdmMsgCenterReduxObj) },
    { SelectMst: AdmMsgCenterReduxObj.SelectMst.bind(AdmMsgCenterReduxObj) },
    { DelMst: AdmMsgCenterReduxObj.DelMst.bind(AdmMsgCenterReduxObj) },
    { AddMst: AdmMsgCenterReduxObj.AddMst.bind(AdmMsgCenterReduxObj) },
    { AddDtl: AdmMsgCenterReduxObj.AddDtl.bind(AdmMsgCenterReduxObj) },
    { changeMstListFilterVisibility: AdmMsgCenterReduxObj.ChangeMstListFilterVisibility.bind(AdmMsgCenterReduxObj) },
    { SetScreenCriteria: AdmMsgCenterReduxObj.SetScreenCriteria.bind(AdmMsgCenterReduxObj) },

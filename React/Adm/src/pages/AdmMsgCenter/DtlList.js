...
import AdmMsgCenterReduxObj from '../../redux/AdmMsgCenter';
...
class DtlList extends RintagiScreen {
    ...
    return function (evt) {
      ...
      const copyFn = () => {
        if (currDtlId) {
          ...
          if (useMobileView) {
            const naviBar = getNaviBar('DtlList', mst, {}, this.props.AdmMsgCenter.Label);
  ...
  render() {
    ...
    const itemList = AdmMsgCenterReduxObj.DtlListToSelectList(AdmMsgCenterState, 'MsgCenterId147');
    ...
    const dtlFilter = AdmMsgCenterState.DtlFilter;
    const filterList = AdmMsgCenterState.DtlFilter.FilterColumnDdl.map((v, i) => ({ key: v.ColName, label: v.ColumnHeader, value: v.ColName, pos: i }))
                        ...
                        resetForm
                          ...
                          <div>
                                      ...
                                      <ButtonGroup className='btn-group--icons'>
                                        ...
                                        </Button>
                                        {
                                          dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                            if ((!activeSelectionVisible && v.buttonType !== 'InsRow') || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).MsgId146)) return null;
                                            return (
                                              <Button
                                                key={v.tid}
                                        ...
                                        {activeSelectionVisible &&
                                          dropdownMenuButtonList
                                            .filter(v => !v.expose)
                                            .filter(v => (!this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).MsgId146,(currDtl || {}).MsgCenterId147)))
                                            .length > 0 &&
                                        ...
                                        <DropdownMenu right className={`dropdown__menu dropdown-options`}>
                                          {
                                            ...
                                            dropdownMenuButtonList.filter(v => !v.expose).map(v => {
                                              if ((!activeSelectionVisible && v.buttonType !== 'InsRow') || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).MsgId146,(currDtl || {}).MsgCenterId147)) return null;
                                              return (
                            ...
                            <Form className='form'>
                                ...
                                </div>
                                {
                                  itemList.slice(0, dtlFilter.Limit).map(
                                    (obj, i) => {
                                      ...
                                      return (
                                        <div className='form__form-group-narrow list-divider' key={obj.MsgCenterId147 || ('_' + i)}>
                                          <div className='form__form-group-field'>
                                            <label className='radio-btn radio-btn--button margin-narrow'>
                                              <Field
                                                key={obj.MsgCenterId147 || ('_' + i)}
                                                className='radio-btn__radio'
                                                name='expenseItem'
                                                type='radio'
                                                value={values.name}
                                                onClick={this.SelectDtlListRow(naviSelectBar, currMst, obj, i, 'MsgId146', 'MsgCenterId147')}
                                              ...
                                              </span>
                                              {
                                                rowMenuButtonList
                                                  ...
                                                  .map((v, i) => {
                                                    return (
                                                      <button type='button' onClick={this.ScreenButtonAction[v.buttonType]({ naviBar: naviSelectBar, mst: currMst, useMobileView, dtlId: obj.MsgCenterId147 })} className={`${v.exposedClassName}`}><i className={`${v.iconClassName}`}></i></button>
                                                    )
                                                  })
                                              }
                                              {
                                                ...
                                                <UncontrolledDropdown className={`btn-row-dropdown`}>
                                                  ...
                                                  <DropdownMenu right className={`dropdown__menu dropdown-options`}>
                                                    {rowMenuButtonList
                                                      .filter(v => !v.expose || useMobileView)
                                                      .map((v) => {
                                                        if (this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).MsgId146,obj.MsgCenterId147)) return null;
                                                        return <DropdownItem key={v.tid} onClick={this.ScreenButtonAction[v.buttonType]({ naviBar: naviSelectBar, mst: currMst, useMobileView, dtlId: obj.MsgCenterId147 })} className={`${v.className}`}><i className={`${v.iconClassName} mr-10`}></i>{v.label}</DropdownItem>
                                                      })
                                                    }
                                                  </DropdownMenu>
                                                </UncontrolledDropdown>
                              ...
                              <Button onClick={this.props.ViewMoreDtl} className={`${viewMoreClass} btn btn-view-more-blue account__btn`} type='button'>{strFormat(IncrementMsg, this.props.AdmMsgCenter.DtlFilter.PageSize)}<br /><i className='fa fa-arrow-down'></i></Button>
            ...
            {!useMobileView && <Col xs={6}>
              ...
              {!activeSelectionVisible &&
                ...
                <div className='empty-block'>
                  ...
                  <p className='create-new-message'>{NoDetailMsg}. <span className='link-imitation' onClick={this.AddNewDtl({ naviBar, mstId: currMst.MsgId146 })}>{AddDetailMsg}</span></p>
                </div>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { SelectDtl: AdmMsgCenterReduxObj.SelectDtl.bind(AdmMsgCenterReduxObj) },
    ...
    { ChangeDtlListFilterVisibility: AdmMsgCenterReduxObj.ChangeDtlListFilterVisibility.bind(AdmMsgCenterReduxObj) },
    { ChangeDtlListFilter: AdmMsgCenterReduxObj.ChangeDtlListFilter.bind(AdmMsgCenterReduxObj) },
    { ViewMoreDtl: AdmMsgCenterReduxObj.ViewMoreDtl.bind(AdmMsgCenterReduxObj) },
    { setTitle: setTitle },

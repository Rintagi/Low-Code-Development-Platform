...
import * as AdmSignupService from '../../services/AdmSignupService';
...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmSignup || {}).initialized) {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmSignup || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).UsrId270) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    // if (!AdmSignupState.initialized || AdmSignupState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmSignupReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmSignupReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmSignupState.ScreenCriteria || {}
    ...
    const currMst = AdmSignupState.Mst;
    const currDtl = AdmSignupState.EditDtl;
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.UsrId270));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmSignupReduxObj.QuickFilterDdlToSelectList(AdmSignupState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmSignupState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmSignupState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).UsrId270)}
                    ...
                    <Formik
                      ...
                      key={AdmSignupState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmSignupState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmSignupState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmSignupReduxObj.LoadInitPage.bind(AdmSignupReduxObj) },
    { LoadSearchList: AdmSignupReduxObj.LoadSearchList.bind(AdmSignupReduxObj) },
    { SelectMst: AdmSignupReduxObj.SelectMst.bind(AdmSignupReduxObj) },
    { DelMst: AdmSignupReduxObj.DelMst.bind(AdmSignupReduxObj) },
    { AddMst: AdmSignupReduxObj.AddMst.bind(AdmSignupReduxObj) },
    { AddDtl: AdmSignupReduxObj.AddDtl.bind(AdmSignupReduxObj) },
    { changeMstListFilterVisibility: AdmSignupReduxObj.ChangeMstListFilterVisibility.bind(AdmSignupReduxObj) },
    { SetScreenCriteria: AdmSignupReduxObj.SetScreenCriteria.bind(AdmSignupReduxObj) },

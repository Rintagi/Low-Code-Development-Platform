...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmCtCulture || {}).initialized) {
  ...
  RefreshSearchList(values, { setErrors, resetForm, setValues /* setValues and other goodies */ }) {
    ...
    const refreshFn = (() => {
      this.props.SetScreenCriteria(values.search,
        {
          ToTranslate10: (values.cCriToTranslate10) ? values.cCriToTranslate10 : '',
          EnNumberRule20: (values.cCriEnNumberRule20) ? values.cCriEnNumberRule20.value : '',
        },
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmCtCulture || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).CultureTypeId8) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    // if (!AdmCtCultureState.initialized || AdmCtCultureState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmCtCultureReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmCtCultureReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmCtCultureState.ScreenCriteria || {}
    ...
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.CultureTypeId8));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmCtCultureReduxObj.QuickFilterDdlToSelectList(AdmCtCultureState);
    ...
    const CriEnNumberRule20List = screenCriDdlSelectors.EnNumberRule20(AdmCtCultureState);
    const CriEnNumberRule20Selected = CriEnNumberRule20List.filter(obj => { return obj.key === screenCriteria.EnNumberRule20.LastCriteria });
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmCtCultureState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmCtCultureState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).CultureTypeId8)}
                    ...
                    <Formik
                      initialValues={{
                        cCriToTranslate10: ((screenCriteria || {}).ToTranslate10 || {}).LastCriteria,
                        cCriEnNumberRule20: CriEnNumberRule20Selected[0],
                        search: screenCriteria.SearchStr || '',
                        cFilterId: (screenFilterSelected.length > 0 ? screenFilterSelected[0] : screenFilterList[0])
                      }}
                      key={AdmCtCultureState.searchListVersion}
                            ...
                            <Form className='form'>
                              <div className='form__form-group'>
                                <div className={`form__form-group filter-padding ${filterVisibility}`} key={screenCriteria.key}>
                                  <Row className='mb-5'>
                                    <Col xs={12} md={12}>
                                      <label className='form__form-group-label filter-label'>{(screenCriteria.ToTranslate10 || {}).ColumnHeader}</label>
                                    ...
                                    <Col xs={12} md={12}>
                                      <label className='form__form-group-label filter-label'>{(screenCriteria.EnNumberRule20 || {}).ColumnHeader}</label>
                                      <div className='form__form-group-field filter-form-border'>
                                        <DropdownField
                                          name='cCriEnNumberRule20'
                                          onChange={this.SearchFilterValueChange(handleSubmit, setFieldValue, 'ddl', 'cCriEnNumberRule20')}
                                          value={values.cCriEnNumberRule20}
                                          options={CriEnNumberRule20List}
                                          placeholder=''
                                        />
                                      </div>
                                    </Col>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmCtCultureState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmCtCultureState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmCtCultureReduxObj.LoadInitPage.bind(AdmCtCultureReduxObj) },
    { LoadSearchList: AdmCtCultureReduxObj.LoadSearchList.bind(AdmCtCultureReduxObj) },
    { SelectMst: AdmCtCultureReduxObj.SelectMst.bind(AdmCtCultureReduxObj) },
    ...
    { changeMstListFilterVisibility: AdmCtCultureReduxObj.ChangeMstListFilterVisibility.bind(AdmCtCultureReduxObj) },
    { SetScreenCriteria: AdmCtCultureReduxObj.SetScreenCriteria.bind(AdmCtCultureReduxObj) },

...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmReportObj || {}).initialized) {
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).ReportObjId23) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    const screenDdlSelectors = AdmReportObjReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmReportObjReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmReportObjState.ScreenCriteria || {}
    ...
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.ReportObjId23));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmReportObjReduxObj.QuickFilterDdlToSelectList(AdmReportObjState);
    ...
    const CriReportId10List = screenCriDdlSelectors.ReportId10(AdmReportObjState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmReportObjState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmReportObjState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).ReportObjId23)}
                    ...
                    <Formik
                      ...
                      key={AdmReportObjState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmReportObjState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmReportObjState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmReportObjReduxObj.LoadInitPage.bind(AdmReportObjReduxObj) },
    { LoadSearchList: AdmReportObjReduxObj.LoadSearchList.bind(AdmReportObjReduxObj) },
    { SelectMst: AdmReportObjReduxObj.SelectMst.bind(AdmReportObjReduxObj) },
    ...
    { changeMstListFilterVisibility: AdmReportObjReduxObj.ChangeMstListFilterVisibility.bind(AdmReportObjReduxObj) },
    { SetScreenCriteria: AdmReportObjReduxObj.SetScreenCriteria.bind(AdmReportObjReduxObj) },

...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmDbKey || {}).initialized) {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmDbKey || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).KeyId20) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    // if (!AdmDbKeyState.initialized || AdmDbKeyState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmDbKeyReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmDbKeyReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmDbKeyState.ScreenCriteria || {}
    ...
    const currMst = AdmDbKeyState.Mst;
    const currDtl = AdmDbKeyState.EditDtl;
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.KeyId20));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmDbKeyReduxObj.QuickFilterDdlToSelectList(AdmDbKeyState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmDbKeyState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmDbKeyState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).KeyId20)}
                    ...
                    <Formik
                      ...
                      key={AdmDbKeyState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmDbKeyState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmDbKeyState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmDbKeyReduxObj.LoadInitPage.bind(AdmDbKeyReduxObj) },
    { LoadSearchList: AdmDbKeyReduxObj.LoadSearchList.bind(AdmDbKeyReduxObj) },
    { SelectMst: AdmDbKeyReduxObj.SelectMst.bind(AdmDbKeyReduxObj) },
    { DelMst: AdmDbKeyReduxObj.DelMst.bind(AdmDbKeyReduxObj) },
    { AddMst: AdmDbKeyReduxObj.AddMst.bind(AdmDbKeyReduxObj) },
    { AddDtl: AdmDbKeyReduxObj.AddDtl.bind(AdmDbKeyReduxObj) },
    { changeMstListFilterVisibility: AdmDbKeyReduxObj.ChangeMstListFilterVisibility.bind(AdmDbKeyReduxObj) },
    { SetScreenCriteria: AdmDbKeyReduxObj.SetScreenCriteria.bind(AdmDbKeyReduxObj) },

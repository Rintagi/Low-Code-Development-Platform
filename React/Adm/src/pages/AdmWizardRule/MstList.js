...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmWizardRule || {}).initialized) {
  ...
  RefreshSearchList(values, { setErrors, resetForm, setValues /* setValues and other goodies */ }) {
    ...
    const refreshFn = (() => {
      this.props.SetScreenCriteria(values.search,
        {
          WizardId10: (values.cCriWizardId10) ? values.cCriWizardId10.value : '',
        },
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmWizardRule || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).WizardRuleId73) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    // if (!AdmWizardRuleState.initialized || AdmWizardRuleState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmWizardRuleReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmWizardRuleReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmWizardRuleState.ScreenCriteria || {}
    ...
    const currMst = AdmWizardRuleState.Mst;
    const currDtl = AdmWizardRuleState.EditDtl;
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.WizardRuleId73));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmWizardRuleReduxObj.QuickFilterDdlToSelectList(AdmWizardRuleState);
    ...
    const CriWizardId10List = screenCriDdlSelectors.WizardId10(AdmWizardRuleState);
    const CriWizardId10Selected = CriWizardId10List.filter(obj => { return obj.key === screenCriteria.WizardId10.LastCriteria });
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmWizardRuleState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmWizardRuleState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).WizardRuleId73)}
                            ...
                            <Form className='form'>
                              <div className='form__form-group'>
                                <div className={`form__form-group filter-padding ${filterVisibility}`} key={screenCriteria.key}>
                                  <Row className='mb-5'>
                                    <Col xs={12} md={12}>
                                      <label className='form__form-group-label filter-label'>{(screenCriteria.WizardId10 || {}).ColumnHeader}</label>
                                      <div className='form__form-group-field filter-form-border'>
                                        <DropdownField
                                          name='cCriWizardId10'
                                          onChange={this.SearchFilterValueChange(handleSubmit, setFieldValue, 'ddl', 'cCriWizardId10')}
                                          value={values.cCriWizardId10}
                                          options={CriWizardId10List}
                                          placeholder=''
                                        />
                                      </div>
                                    </Col>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmWizardRuleState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmWizardRuleState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmWizardRuleReduxObj.LoadInitPage.bind(AdmWizardRuleReduxObj) },
    { LoadSearchList: AdmWizardRuleReduxObj.LoadSearchList.bind(AdmWizardRuleReduxObj) },
    { SelectMst: AdmWizardRuleReduxObj.SelectMst.bind(AdmWizardRuleReduxObj) },
    ...
    { AddDtl: AdmWizardRuleReduxObj.AddDtl.bind(AdmWizardRuleReduxObj) },
    { changeMstListFilterVisibility: AdmWizardRuleReduxObj.ChangeMstListFilterVisibility.bind(AdmWizardRuleReduxObj) },
    { SetScreenCriteria: AdmWizardRuleReduxObj.SetScreenCriteria.bind(AdmWizardRuleReduxObj) },

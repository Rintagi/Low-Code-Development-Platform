...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmScrAuditDtl || {}).initialized) {
  ...
  RefreshSearchList(values, { setErrors, resetForm, setValues /* setValues and other goodies */ }) {
    ...
    const refreshFn = (() => {
      this.props.SetScreenCriteria(values.search,
        {
          ScrAuditId10: (values.cCriScrAuditId10) ? values.cCriScrAuditId10 : '',
        },
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmScrAuditDtl || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).ScrAuditDtlId1301) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    // if (!AdmScrAuditDtlState.initialized || AdmScrAuditDtlState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmScrAuditDtlReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmScrAuditDtlReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmScrAuditDtlState.ScreenCriteria || {}
    ...
    const currMst = AdmScrAuditDtlState.Mst;
    const currDtl = AdmScrAuditDtlState.EditDtl;
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.ScrAuditDtlId1301));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmScrAuditDtlReduxObj.QuickFilterDdlToSelectList(AdmScrAuditDtlState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmScrAuditDtlState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmScrAuditDtlState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).ScrAuditDtlId1301)}
                            ...
                            <Form className='form'>
                              <div className='form__form-group'>
                                <div className={`form__form-group filter-padding ${filterVisibility}`} key={screenCriteria.key}>
                                  <Row className='mb-5'>
                                    <Col xs={12} md={12}>
                                      <label className='form__form-group-label filter-label'>{(screenCriteria.ScrAuditId10 || {}).ColumnHeader}</label>
                                      <div className='form__form-group-field filter-form-border'>
                                        <Field
                                          type='text'
                                          name='cCriScrAuditId10'
                                          value={values.cCriScrAuditId10}
                                          onBlur={this.SearchFilterTextValueChange(handleSubmit, setFieldValue, 'text', 'cCriScrAuditId10')}
                                        />
                                      </div>
                                    </Col>
                                  </Row>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmScrAuditDtlState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmScrAuditDtlState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmScrAuditDtlReduxObj.LoadInitPage.bind(AdmScrAuditDtlReduxObj) },
    { LoadSearchList: AdmScrAuditDtlReduxObj.LoadSearchList.bind(AdmScrAuditDtlReduxObj) },
    { SelectMst: AdmScrAuditDtlReduxObj.SelectMst.bind(AdmScrAuditDtlReduxObj) },
    ...
    { AddDtl: AdmScrAuditDtlReduxObj.AddDtl.bind(AdmScrAuditDtlReduxObj) },
    { changeMstListFilterVisibility: AdmScrAuditDtlReduxObj.ChangeMstListFilterVisibility.bind(AdmScrAuditDtlReduxObj) },
    { SetScreenCriteria: AdmScrAuditDtlReduxObj.SetScreenCriteria.bind(AdmScrAuditDtlReduxObj) },

...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmCtDisplayType || {}).initialized) {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmCtDisplayType || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).TypeId18) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    // if (!AdmCtDisplayTypeState.initialized || AdmCtDisplayTypeState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmCtDisplayTypeReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmCtDisplayTypeReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmCtDisplayTypeState.ScreenCriteria || {}
    ...
    const currMst = AdmCtDisplayTypeState.Mst;
    const currDtl = AdmCtDisplayTypeState.EditDtl;
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.TypeId18));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmCtDisplayTypeReduxObj.QuickFilterDdlToSelectList(AdmCtDisplayTypeState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmCtDisplayTypeState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmCtDisplayTypeState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).TypeId18)}
                    ...
                    <Formik
                      ...
                      key={AdmCtDisplayTypeState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmCtDisplayTypeState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmCtDisplayTypeState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmCtDisplayTypeReduxObj.LoadInitPage.bind(AdmCtDisplayTypeReduxObj) },
    { LoadSearchList: AdmCtDisplayTypeReduxObj.LoadSearchList.bind(AdmCtDisplayTypeReduxObj) },
    { SelectMst: AdmCtDisplayTypeReduxObj.SelectMst.bind(AdmCtDisplayTypeReduxObj) },
    ...
    { AddDtl: AdmCtDisplayTypeReduxObj.AddDtl.bind(AdmCtDisplayTypeReduxObj) },
    { changeMstListFilterVisibility: AdmCtDisplayTypeReduxObj.ChangeMstListFilterVisibility.bind(AdmCtDisplayTypeReduxObj) },
    { SetScreenCriteria: AdmCtDisplayTypeReduxObj.SetScreenCriteria.bind(AdmCtDisplayTypeReduxObj) },

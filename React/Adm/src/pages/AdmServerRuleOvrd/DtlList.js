...
import AdmServerRuleOvrdReduxObj from '../../redux/AdmServerRuleOvrd';
...
class DtlList extends RintagiScreen {
    ...
    return function (evt) {
      ...
      const copyFn = () => {
        if (currDtlId) {
          ...
          if (useMobileView) {
            const naviBar = getNaviBar('DtlList', mst, {}, this.props.AdmServerRuleOvrd.Label);
      ...
      const deleteFn = () => {
        ...
        if (currDtlId) {
          this.props.SavePage(
            ...
            [
              {
                ServerRuledOvrdPrmId1321: currDtlId,
                _mode: 'delete',
              }
            ],
            {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmServerRuleOvrd || {}).Buttons || {};
  ...
  render() {
    ...
    //if (!AdmServerRuleOvrdState.initialized || AdmServerRuleOvrdState.page_loading) return null;
    ...
    const itemList = AdmServerRuleOvrdReduxObj.DtlListToSelectList(AdmServerRuleOvrdState, 'ServerRuledOvrdPrmId1321');
    ...
    const dtlFilter = AdmServerRuleOvrdState.DtlFilter;
    const filterList = AdmServerRuleOvrdState.DtlFilter.FilterColumnDdl.map((v, i) => ({ key: v.ColName, label: v.ColumnHeader, value: v.ColName, pos: i }))
                        ...
                        resetForm
                          ...
                          <div>
                                      ...
                                      <ButtonGroup className='btn-group--icons'>
                                        ...
                                        </Button>
                                        {
                                          dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                            if ((!activeSelectionVisible && v.buttonType !== 'InsRow') || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).AtServerRuleOvrdId1322)) return null;
                                            return (
                                              <Button
                                                key={v.tid}
                                        ...
                                        {activeSelectionVisible &&
                                          dropdownMenuButtonList
                                            .filter(v => !v.expose)
                                            .filter(v => (!this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).AtServerRuleOvrdId1322, (currDtl || {}).ServerRuledOvrdPrmId1321)))
                                            .length > 0 &&
                                        ...
                                        <DropdownMenu right className={`dropdown__menu dropdown-options`}>
                                          {
                                            ...
                                            dropdownMenuButtonList.filter(v => !v.expose).map(v => {
                                              if ((!activeSelectionVisible && v.buttonType !== 'InsRow') || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).AtServerRuleOvrdId1322, (currDtl || {}).ServerRuledOvrdPrmId1321)) return null;
                                              return (
                            ...
                            <Form className='form'>
                                ...
                                </div>
                                {
                                  itemList.slice(0, dtlFilter.Limit).map(
                                    (obj, i) => {
                                      ...
                                      return (
                                        <div className='form__form-group-narrow list-divider' key={obj.ServerRuledOvrdPrmId1321 || ('_' + i)}>
                                          <div className='form__form-group-field'>
                                            <label className='radio-btn radio-btn--button margin-narrow'>
                                              <Field
                                                key={obj.ServerRuledOvrdPrmId1321 || ('_' + i)}
                                                className='radio-btn__radio'
                                                name='expenseItem'
                                                type='radio'
                                                value={values.name}
                                                onClick={this.SelectDtlListRow(naviSelectBar, currMst, obj, i, 'AtServerRuleOvrdId1322', 'ServerRuledOvrdPrmId1321')}
                                              ...
                                              </span>
                                              {
                                                rowMenuButtonList
                                                  ...
                                                  .map((v, i) => {
                                                    return (
                                                      <button type='button' onClick={this.ScreenButtonAction[v.buttonType]({ naviBar: naviSelectBar, mst: currMst, useMobileView, dtlId: obj.ServerRuledOvrdPrmId1321 })} className={`${v.exposedClassName}`}><i className={`${v.iconClassName}`}></i></button>
                                                    )
                                                  })
                                              }
                                              {
                                                ...
                                                <UncontrolledDropdown className={`btn-row-dropdown`}>
                                                  ...
                                                  <DropdownMenu right className={`dropdown__menu dropdown-options`}>
                                                    {rowMenuButtonList
                                                      .filter(v => !v.expose || useMobileView)
                                                      .map((v) => {
                                                        if (this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).AtServerRuleOvrdId1322, obj.ServerRuledOvrdPrmId1321)) return null;
                                                        return <DropdownItem key={v.tid} onClick={this.ScreenButtonAction[v.buttonType]({ naviBar: naviSelectBar, mst: currMst, useMobileView, dtlId: obj.ServerRuledOvrdPrmId1321 })} className={`${v.className}`}><i className={`${v.iconClassName} mr-10`}></i>{v.label}</DropdownItem>
                                                      })
                                                    }
                                                  </DropdownMenu>
                                                </UncontrolledDropdown>
                              ...
                              <Button onClick={this.props.ViewMoreDtl} className={`${viewMoreClass} btn btn-view-more-blue account__btn`} type='button'>{strFormat(IncrementMsg, this.props.AdmServerRuleOvrd.DtlFilter.PageSize)}<br /><i className='fa fa-arrow-down'></i></Button>
            ...
            {!useMobileView && <Col xs={6}>
              ...
              {!activeSelectionVisible &&
                ...
                <div className='empty-block'>
                  ...
                  <p className='create-new-message'>{NoDetailMsg}. <span className='link-imitation' onClick={this.AddNewDtl({ naviBar, mstId: currMst.AtServerRuleOvrdId1322 })}>{AddDetailMsg}</span></p>
                </div>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { SelectDtl: AdmServerRuleOvrdReduxObj.SelectDtl.bind(AdmServerRuleOvrdReduxObj) },
    ...
    { ChangeDtlListFilterVisibility: AdmServerRuleOvrdReduxObj.ChangeDtlListFilterVisibility.bind(AdmServerRuleOvrdReduxObj) },
    { ChangeDtlListFilter: AdmServerRuleOvrdReduxObj.ChangeDtlListFilter.bind(AdmServerRuleOvrdReduxObj) },
    { ViewMoreDtl: AdmServerRuleOvrdReduxObj.ViewMoreDtl.bind(AdmServerRuleOvrdReduxObj) },
    { setTitle: setTitle },

...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmServerRuleOvrd || {}).initialized) {
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).AtServerRuleOvrdId1322) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    const screenDdlSelectors = AdmServerRuleOvrdReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmServerRuleOvrdReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmServerRuleOvrdState.ScreenCriteria || {}
    ...
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.AtServerRuleOvrdId1322));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmServerRuleOvrdReduxObj.QuickFilterDdlToSelectList(AdmServerRuleOvrdState);
    ...
    const CriScreenId10List = screenCriDdlSelectors.ScreenId10(AdmServerRuleOvrdState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmServerRuleOvrdState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmServerRuleOvrdState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).AtServerRuleOvrdId1322)}
                    ...
                    <Formik
                      ...
                      key={AdmServerRuleOvrdState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmServerRuleOvrdState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmServerRuleOvrdState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmServerRuleOvrdReduxObj.LoadInitPage.bind(AdmServerRuleOvrdReduxObj) },
    { LoadSearchList: AdmServerRuleOvrdReduxObj.LoadSearchList.bind(AdmServerRuleOvrdReduxObj) },
    { SelectMst: AdmServerRuleOvrdReduxObj.SelectMst.bind(AdmServerRuleOvrdReduxObj) },
    ...
    { changeMstListFilterVisibility: AdmServerRuleOvrdReduxObj.ChangeMstListFilterVisibility.bind(AdmServerRuleOvrdReduxObj) },
    { SetScreenCriteria: AdmServerRuleOvrdReduxObj.SetScreenCriteria.bind(AdmServerRuleOvrdReduxObj) },
    { SearchCriScreenId10: AdmServerRuleOvrdReduxObj.SearchActions.SearchCriScreenId10.bind(AdmServerRuleOvrdReduxObj) },

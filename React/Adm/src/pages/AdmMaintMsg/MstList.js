...
import AdmMaintMsgReduxObj, { ShowMstFilterApplied } from '../../redux/AdmMaintMsg';
import * as AdmMaintMsgService from '../../services/AdmMaintMsgService';
...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmMaintMsg || {}).initialized) {
  ...
  CopyHdr({ naviBar, ScreenButton, useMobileView, mst, mstId }) {
    const AdmMaintMsgState = this.props.AdmMaintMsg || {};
    const auxSystemLabels = AdmMaintMsgState.SystemLabel || {};
    return function (evt) {
      evt.preventDefault();
  ...
  DelMst({ naviBar, ScreenButton, mst, mstId }) {
    ...
    return this.Prompt({
      okFn: function (evt) {
        const fromMstId = mstId || mst.MaintMsgId233;
        this.props.DelMst(this.props.AdmMaintMsg, fromMstId);
      }.bind(this),
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmMaintMsg || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    const currReduxScreenState = this.props.AdmMaintMsg || {};
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).MaintMsgId233) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    if (AdmMaintMsgState.access_denied) {
      return <Redirect to='/error' />;
    }

    // if (!AdmMaintMsgState.initialized || AdmMaintMsgState.page_loading) return null;
    const screenHlp = AdmMaintMsgState.ScreenHlp;
    ...
    const screenDdlSelectors = AdmMaintMsgReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmMaintMsgReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmMaintMsgState.ScreenCriteria || {}
    const selectList = AdmMaintMsgReduxObj.SearchListToSelectList(AdmMaintMsgState);
    const itemList = AdmMaintMsgState.Dtl || [];
    const auxLabels = AdmMaintMsgState.Label || {};
    const auxSystemLabels = AdmMaintMsgState.SystemLabel || {};
    const columnLabel = AdmMaintMsgState.ColumnLabel || {};
    const currMst = AdmMaintMsgState.Mst;
    const currDtl = AdmMaintMsgState.EditDtl;
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.MaintMsgId233));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmMaintMsgReduxObj.QuickFilterDdlToSelectList(AdmMaintMsgState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmMaintMsgState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmMaintMsgState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).MaintMsgId233)}
                    ...
                    <Formik
                      ...
                      key={AdmMaintMsgState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmMaintMsgState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmMaintMsgState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
                                    ...
                                    <Row className='btn-bottom-row'>
                                      {
                                        bottomButtonList
                                          .filter(v => v.expose)
                                          .map((v, i, a) => {
                                            if (this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).MaintMsgId233)) return null;
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    { LoadPage: AdmMaintMsgReduxObj.LoadPage.bind(AdmMaintMsgReduxObj) },
    { LoadInitPage: AdmMaintMsgReduxObj.LoadInitPage.bind(AdmMaintMsgReduxObj) },
    { LoadSearchList: AdmMaintMsgReduxObj.LoadSearchList.bind(AdmMaintMsgReduxObj) },
    { SelectMst: AdmMaintMsgReduxObj.SelectMst.bind(AdmMaintMsgReduxObj) },
    { DelMst: AdmMaintMsgReduxObj.DelMst.bind(AdmMaintMsgReduxObj) },
    { AddMst: AdmMaintMsgReduxObj.AddMst.bind(AdmMaintMsgReduxObj) },
    { AddDtl: AdmMaintMsgReduxObj.AddDtl.bind(AdmMaintMsgReduxObj) },
    { changeMstListFilterVisibility: AdmMaintMsgReduxObj.ChangeMstListFilterVisibility.bind(AdmMaintMsgReduxObj) },
    { SetScreenCriteria: AdmMaintMsgReduxObj.SetScreenCriteria.bind(AdmMaintMsgReduxObj) },

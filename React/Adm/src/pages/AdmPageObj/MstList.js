...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmPageObj || {}).initialized) {
  ...
  RefreshSearchList(values, { setErrors, resetForm, setValues /* setValues and other goodies */ }) {
    ...
    const refreshFn = (() => {
      this.props.SetScreenCriteria(values.search,
        {
          SectionCd10: (values.cCriSectionCd10) ? values.cCriSectionCd10.value : '',
        },
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).PageObjId1277) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    const screenDdlSelectors = AdmPageObjReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmPageObjReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmPageObjState.ScreenCriteria || {}
    ...
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.PageObjId1277));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmPageObjReduxObj.QuickFilterDdlToSelectList(AdmPageObjState);
    ...
    const CriSectionCd10List = screenCriDdlSelectors.SectionCd10(AdmPageObjState);
    const CriSectionCd10Selected = CriSectionCd10List.filter(obj => { return obj.key === screenCriteria.SectionCd10.LastCriteria });
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmPageObjState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmPageObjState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).PageObjId1277)}
                            ...
                            <Form className='form'>
                              <div className='form__form-group'>
                                <div className={`form__form-group filter-padding ${filterVisibility}`} key={screenCriteria.key}>
                                  <Row className='mb-5'>
                                    <Col xs={12} md={12}>
                                      <label className='form__form-group-label filter-label'>{(screenCriteria.SectionCd10 || {}).ColumnHeader}</label>
                                      <div className='form__form-group-field filter-form-border'>
                                        <DropdownField
                                          name='cCriSectionCd10'
                                          onChange={this.SearchFilterValueChange(handleSubmit, setFieldValue, 'ddl', 'cCriSectionCd10')}
                                          value={values.cCriSectionCd10}
                                          options={CriSectionCd10List}
                                          placeholder=''
                                        />
                                      </div>
                                    </Col>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmPageObjState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmPageObjState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmPageObjReduxObj.LoadInitPage.bind(AdmPageObjReduxObj) },
    { LoadSearchList: AdmPageObjReduxObj.LoadSearchList.bind(AdmPageObjReduxObj) },
    { SelectMst: AdmPageObjReduxObj.SelectMst.bind(AdmPageObjReduxObj) },
    ...
    { changeMstListFilterVisibility: AdmPageObjReduxObj.ChangeMstListFilterVisibility.bind(AdmPageObjReduxObj) },
    { SetScreenCriteria: AdmPageObjReduxObj.SetScreenCriteria.bind(AdmPageObjReduxObj) },

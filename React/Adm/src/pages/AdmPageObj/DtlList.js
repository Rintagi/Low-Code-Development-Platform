...
import AdmPageObjReduxObj from '../../redux/AdmPageObj';
...
class DtlList extends RintagiScreen {
  constructor(props) {
    ...
    this.DtlKeyColumnName = 'PageLnkId1278';
  ...
  CopyRow({ mst, dtlId, dtl, ...rest }) {
    ...
    return function (evt) {
      evt.preventDefault();
      const currDtlId = dtlId || dtl.PageLnkId1278;
      const copyFn = () => {
        if (currDtlId) {
          this.props.AddDtl(mst.PageObjId1277, currDtlId);
          const isMobileView = window.matchMedia('(max-width: 1099px)').matches;
          const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
          if (useMobileView) {
            const naviBar = getNaviBar('DtlList', mst, {}, this.props.AdmPageObj.Label);
      ...
      const deleteFn = () => {
        ...
        if (currDtlId) {
          this.props.SavePage(
            ...
            this.props.AdmPageObj.Mst,
            [
              {
                PageLnkId1278: currDtlId,
                _mode: 'delete',
              }
            ],
            {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmPageObj || {}).Buttons || {};
  ...
  render() {
    ...
    //if (!AdmPageObjState.initialized || AdmPageObjState.page_loading) return null;
    ...
    const itemList = AdmPageObjReduxObj.DtlListToSelectList(AdmPageObjState, 'PageLnkId1278');
    ...
    const currMst = AdmPageObjState.Mst;
    const currDtl = AdmPageObjState.EditDtl;
    const dtlFilter = AdmPageObjState.DtlFilter;
    const filterList = AdmPageObjState.DtlFilter.FilterColumnDdl.map((v, i) => ({ key: v.ColName, label: v.ColumnHeader, value: v.ColName, pos: i }))
                        ...
                        resetForm
                          ...
                          <div>
                                      ...
                                      <ButtonGroup className='btn-group--icons'>
                                        ...
                                        </Button>
                                        {
                                          dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                            if ((!activeSelectionVisible && v.buttonType !== 'InsRow') || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).PageObjId1277)) return null;
                                            return (
                                              <Button
                                                key={v.tid}
                                        ...
                                        {activeSelectionVisible &&
                                          dropdownMenuButtonList
                                            .filter(v => !v.expose)
                                            .filter(v => (!this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).PageObjId1277,(currDtl || {}).PageLnkId1278)))
                                            .length > 0 &&
                                        ...
                                        <DropdownMenu right className={`dropdown__menu dropdown-options`}>
                                          {
                                            ...
                                            dropdownMenuButtonList.filter(v => !v.expose).map(v => {
                                              if ((!activeSelectionVisible && v.buttonType !== 'InsRow') || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).PageObjId1277,(currDtl || {}).PageLnkId1278)) return null;
                                              return (
                            ...
                            <Form className='form'>
                                ...
                                </div>
                                {
                                  itemList.slice(0, dtlFilter.Limit).map(
                                    (obj, i) => {
                                      ...
                                      return (
                                        <div className='form__form-group-narrow list-divider' key={obj.PageLnkId1278 || ('_' + i)}>
                                          <div className='form__form-group-field'>
                                            <label className='radio-btn radio-btn--button margin-narrow'>
                                              <Field
                                                key={obj.PageLnkId1278 || ('_' + i)}
                                                className='radio-btn__radio'
                                                name='expenseItem'
                                                type='radio'
                                                value={values.name}
                                                onClick={this.SelectDtlListRow(naviSelectBar, currMst, obj, i, 'PageObjId1277', 'PageLnkId1278')}
                                              ...
                                              </span>
                                              {
                                                rowMenuButtonList
                                                  ...
                                                  .map((v, i) => {
                                                    return (
                                                      <button type='button' onClick={this.ScreenButtonAction[v.buttonType]({ naviBar: naviSelectBar, mst: currMst, useMobileView, dtlId: obj.PageLnkId1278 })} className={`${v.exposedClassName}`}><i className={`${v.iconClassName}`}></i></button>
                                                    )
                                                  })
                                              }
                                              {
                                                ...
                                                <UncontrolledDropdown className={`btn-row-dropdown`}>
                                                  ...
                                                  <DropdownMenu right className={`dropdown__menu dropdown-options`}>
                                                    {rowMenuButtonList
                                                      .filter(v => !v.expose || useMobileView)
                                                      .map((v) => {
                                                        if (this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).PageObjId1277,obj.PageLnkId1278)) return null;
                                                        return <DropdownItem key={v.tid} onClick={this.ScreenButtonAction[v.buttonType]({ naviBar: naviSelectBar, mst: currMst, useMobileView, dtlId: obj.PageLnkId1278 })} className={`${v.className}`}><i className={`${v.iconClassName} mr-10`}></i>{v.label}</DropdownItem>
                                                      })
                                                    }
                                                  </DropdownMenu>
                                                </UncontrolledDropdown>
                              ...
                              <Button onClick={this.props.ViewMoreDtl} className={`${viewMoreClass} btn btn-view-more-blue account__btn`} type='button'>{strFormat(IncrementMsg, this.props.AdmPageObj.DtlFilter.PageSize)}<br /><i className='fa fa-arrow-down'></i></Button>
            ...
            {!useMobileView && <Col xs={6}>
              ...
              {!activeSelectionVisible &&
                ...
                <div className='empty-block'>
                  ...
                  <p className='create-new-message'>{NoDetailMsg}. <span className='link-imitation' onClick={this.AddNewDtl({ naviBar, mstId: currMst.PageObjId1277 })}>{AddDetailMsg}</span></p>
                </div>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { SelectDtl: AdmPageObjReduxObj.SelectDtl.bind(AdmPageObjReduxObj) },
    { SavePage: AdmPageObjReduxObj.SavePage.bind(AdmPageObjReduxObj) },
    { AddDtl: AdmPageObjReduxObj.AddDtl.bind(AdmPageObjReduxObj) },
    { ChangeDtlListFilterVisibility: AdmPageObjReduxObj.ChangeDtlListFilterVisibility.bind(AdmPageObjReduxObj) },
    { ChangeDtlListFilter: AdmPageObjReduxObj.ChangeDtlListFilter.bind(AdmPageObjReduxObj) },
    { ViewMoreDtl: AdmPageObjReduxObj.ViewMoreDtl.bind(AdmPageObjReduxObj) },
    { setTitle: setTitle },

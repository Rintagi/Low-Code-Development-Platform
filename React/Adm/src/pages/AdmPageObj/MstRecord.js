...
class MstRecord extends RintagiScreen {
  ...
  LinkTypeCd1277InputChange() { const _this = this; return function (name, v) { const filterBy = ''; _this.props.SearchLinkTypeCd1277(v, filterBy); } }
  BtnDefault({ submitForm, ScreenButton, naviBar, redirectTo, onSuccess }) {
    return function (evt) {
      this.OnClickColumeName = 'BtnDefault';
      //Enter Custom Code here, eg: submitForm();
      evt.preventDefault();
    }.bind(this);
  }
  BtnHeader({ submitForm, ScreenButton, naviBar, redirectTo, onSuccess }) {
    return function (evt) {
      this.OnClickColumeName = 'BtnHeader';
      //Enter Custom Code here, eg: submitForm();
      evt.preventDefault();
    }.bind(this);
  }
  BtnFooter({ submitForm, ScreenButton, naviBar, redirectTo, onSuccess }) {
    return function (evt) {
      this.OnClickColumeName = 'BtnFooter';
      //Enter Custom Code here, eg: submitForm();
      evt.preventDefault();
    }.bind(this);
  }
  BtnSidebar({ submitForm, ScreenButton, naviBar, redirectTo, onSuccess }) {
    return function (evt) {
      this.OnClickColumeName = 'BtnSidebar';
      //Enter Custom Code here, eg: submitForm();
      evt.preventDefault();
    }.bind(this);
  }
    ...
    return function (evt) {
      ...
      const copyFn = () => {
        if (fromMstId) {
          ...
          if (useMobileView) {
            const naviBar = getNaviBar('MstRecord', {}, {}, this.props.AdmPageObj.Label);
  ...
  render() {
    ...
    const naviBar = getNaviBar('MstRecord', currMst, currDtl, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.PageObjId1277));
          ...
          <div className='account'>
            <div className='account__wrapper account-col'>
              <div className='account__card shadow-box rad-4'>
                ...
                <Formik
                  initialValues={{
                    cPageObjId1277: formatContent(PageObjId1277 || '', 'TextBox'),
                                  ...
                                  <div className='form__form-group'>
                                    ...
                                    {((true && this.constructor.ShowSpinner(AdmPageObjState)) && <Skeleton height='36px' />) ||
                                      <div className='form__form-group-field'>
                                        <DropdownField
                                          name='cSectionCd1277'
                                          onChange={this.DropdownChangeV1(setFieldValue, setFieldTouched, 'cSectionCd1277')}

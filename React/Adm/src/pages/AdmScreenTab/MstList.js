...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmScreenTab || {}).initialized) {
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).ScreenTabId19) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    const screenDdlSelectors = AdmScreenTabReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmScreenTabReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmScreenTabState.ScreenCriteria || {}
    ...
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.ScreenTabId19));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmScreenTabReduxObj.QuickFilterDdlToSelectList(AdmScreenTabState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmScreenTabState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmScreenTabState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).ScreenTabId19)}
                    ...
                    <Formik
                      ...
                      key={AdmScreenTabState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmScreenTabState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmScreenTabState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmScreenTabReduxObj.LoadInitPage.bind(AdmScreenTabReduxObj) },
    { LoadSearchList: AdmScreenTabReduxObj.LoadSearchList.bind(AdmScreenTabReduxObj) },
    { SelectMst: AdmScreenTabReduxObj.SelectMst.bind(AdmScreenTabReduxObj) },
    ...
    { changeMstListFilterVisibility: AdmScreenTabReduxObj.ChangeMstListFilterVisibility.bind(AdmScreenTabReduxObj) },
    { SetScreenCriteria: AdmScreenTabReduxObj.SetScreenCriteria.bind(AdmScreenTabReduxObj) },

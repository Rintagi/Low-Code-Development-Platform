...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmTemplate || {}).initialized) {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmTemplate || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).TemplateId79) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    // if (!AdmTemplateState.initialized || AdmTemplateState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmTemplateReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmTemplateReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmTemplateState.ScreenCriteria || {}
    ...
    const currMst = AdmTemplateState.Mst;
    const currDtl = AdmTemplateState.EditDtl;
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.TemplateId79));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmTemplateReduxObj.QuickFilterDdlToSelectList(AdmTemplateState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmTemplateState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmTemplateState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).TemplateId79)}
                    ...
                    <Formik
                      ...
                      key={AdmTemplateState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmTemplateState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmTemplateState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmTemplateReduxObj.LoadInitPage.bind(AdmTemplateReduxObj) },
    { LoadSearchList: AdmTemplateReduxObj.LoadSearchList.bind(AdmTemplateReduxObj) },
    { SelectMst: AdmTemplateReduxObj.SelectMst.bind(AdmTemplateReduxObj) },
    { DelMst: AdmTemplateReduxObj.DelMst.bind(AdmTemplateReduxObj) },
    { AddMst: AdmTemplateReduxObj.AddMst.bind(AdmTemplateReduxObj) },
    { AddDtl: AdmTemplateReduxObj.AddDtl.bind(AdmTemplateReduxObj) },
    { changeMstListFilterVisibility: AdmTemplateReduxObj.ChangeMstListFilterVisibility.bind(AdmTemplateReduxObj) },
    { SetScreenCriteria: AdmTemplateReduxObj.SetScreenCriteria.bind(AdmTemplateReduxObj) },

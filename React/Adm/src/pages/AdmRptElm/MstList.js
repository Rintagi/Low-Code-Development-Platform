...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmRptElm || {}).initialized) {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmRptElm || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).RptElmId160) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    // if (!AdmRptElmState.initialized || AdmRptElmState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmRptElmReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmRptElmReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmRptElmState.ScreenCriteria || {}
    ...
    const currMst = AdmRptElmState.Mst;
    const currDtl = AdmRptElmState.EditDtl;
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.RptElmId160));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmRptElmReduxObj.QuickFilterDdlToSelectList(AdmRptElmState);
    ...
    const CriReportId10List = screenCriDdlSelectors.ReportId10(AdmRptElmState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmRptElmState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmRptElmState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).RptElmId160)}
                    ...
                    <Formik
                      ...
                      key={AdmRptElmState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmRptElmState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmRptElmState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmRptElmReduxObj.LoadInitPage.bind(AdmRptElmReduxObj) },
    { LoadSearchList: AdmRptElmReduxObj.LoadSearchList.bind(AdmRptElmReduxObj) },
    { SelectMst: AdmRptElmReduxObj.SelectMst.bind(AdmRptElmReduxObj) },
    { DelMst: AdmRptElmReduxObj.DelMst.bind(AdmRptElmReduxObj) },
    { AddMst: AdmRptElmReduxObj.AddMst.bind(AdmRptElmReduxObj) },
    { AddDtl: AdmRptElmReduxObj.AddDtl.bind(AdmRptElmReduxObj) },
    { changeMstListFilterVisibility: AdmRptElmReduxObj.ChangeMstListFilterVisibility.bind(AdmRptElmReduxObj) },
    { SetScreenCriteria: AdmRptElmReduxObj.SetScreenCriteria.bind(AdmRptElmReduxObj) },
    { SearchCriReportId10: AdmRptElmReduxObj.SearchActions.SearchCriReportId10.bind(AdmRptElmReduxObj) },

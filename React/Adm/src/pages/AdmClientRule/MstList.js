...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmClientRule || {}).initialized) {
  ...
  CriReportId20InputChange() {
    return function (name, v) {
      this.props.SearchCriReportId20(v);
    }.bind(this);
  }
  CriCultureId30InputChange() {
    return function (name, v) {
      this.props.SearchCriCultureId30(v);
    }.bind(this);
  }
      ...
      this.props.SetScreenCriteria(values.search,
        {
          ...
          ReportId20: (values.cCriReportId20) ? values.cCriReportId20.value : '',
          CultureId30: (values.cCriCultureId30) ? values.cCriCultureId30.value : '',
        },
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmClientRule || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).ClientRuleId127) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    // if (!AdmClientRuleState.initialized || AdmClientRuleState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmClientRuleReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmClientRuleReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmClientRuleState.ScreenCriteria || {}
    ...
    const currMst = AdmClientRuleState.Mst;
    const currDtl = AdmClientRuleState.EditDtl;
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.ClientRuleId127));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmClientRuleReduxObj.QuickFilterDdlToSelectList(AdmClientRuleState);
    ...
    const CriScreenId10List = screenCriDdlSelectors.ScreenId10(AdmClientRuleState);
    const CriScreenId10Selected = CriScreenId10List.filter(obj => { return obj.key === screenCriteria.ScreenId10.LastCriteria });
    const CriReportId20List = screenCriDdlSelectors.ReportId20(AdmClientRuleState);
    const CriReportId20Selected = CriReportId20List.filter(obj => { return obj.key === screenCriteria.ReportId20.LastCriteria });
    const CriCultureId30List = screenCriDdlSelectors.CultureId30(AdmClientRuleState);
    const CriCultureId30Selected = CriCultureId30List.filter(obj => { return obj.key === screenCriteria.CultureId30.LastCriteria });
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmClientRuleState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmClientRuleState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).ClientRuleId127)}
                    ...
                    <Formik
                      initialValues={{
                        ...
                        cCriReportId20: CriReportId20Selected[0],
                        cCriCultureId30: CriCultureId30Selected[0],
                        search: screenCriteria.SearchStr || '',
                        cFilterId: (screenFilterSelected.length > 0 ? screenFilterSelected[0] : screenFilterList[0])
                      }}
                      key={AdmClientRuleState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              <div className='form__form-group'>
                                <div className={`form__form-group filter-padding ${filterVisibility}`} key={screenCriteria.key}>
                                  <Row className='mb-5'>
                                    ...
                                    <Col xs={12} md={12}>
                                      <label className='form__form-group-label filter-label'>{(screenCriteria.ReportId20 || {}).ColumnHeader}</label>
                                      <div className='form__form-group-field filter-form-border'>
                                        <AutoCompleteField
                                          name='cCriReportId20'
                                          onChange={this.SearchFilterValueChange(handleSubmit, setFieldValue, 'autocomplete', 'cCriReportId20')}
                                          onInputChange={this.CriReportId20InputChange()}
                                          value={values.cCriReportId20}
                                          defaultSelected={CriReportId20Selected}
                                          options={CriReportId20List}
                                        />
                                      </div>
                                    </Col>
                                    <Col xs={12} md={12}>
                                      <label className='form__form-group-label filter-label'>{(screenCriteria.CultureId30 || {}).ColumnHeader}</label>
                                      <div className='form__form-group-field filter-form-border'>
                                        <AutoCompleteField
                                          name='cCriCultureId30'
                                          onChange={this.SearchFilterValueChange(handleSubmit, setFieldValue, 'autocomplete', 'cCriCultureId30')}
                                          onInputChange={this.CriCultureId30InputChange()}
                                          value={values.cCriCultureId30}
                                          defaultSelected={CriCultureId30Selected}
                                          options={CriCultureId30List}
                                        />
                                      </div>
                                    </Col>
                                  </Row>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmClientRuleState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmClientRuleState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmClientRuleReduxObj.LoadInitPage.bind(AdmClientRuleReduxObj) },
    { LoadSearchList: AdmClientRuleReduxObj.LoadSearchList.bind(AdmClientRuleReduxObj) },
    { SelectMst: AdmClientRuleReduxObj.SelectMst.bind(AdmClientRuleReduxObj) },
    ...
    { AddDtl: AdmClientRuleReduxObj.AddDtl.bind(AdmClientRuleReduxObj) },
    { changeMstListFilterVisibility: AdmClientRuleReduxObj.ChangeMstListFilterVisibility.bind(AdmClientRuleReduxObj) },
    { SetScreenCriteria: AdmClientRuleReduxObj.SetScreenCriteria.bind(AdmClientRuleReduxObj) },
    { SearchCriScreenId10: AdmClientRuleReduxObj.SearchActions.SearchCriScreenId10.bind(AdmClientRuleReduxObj) },
    { SearchCriReportId20: AdmClientRuleReduxObj.SearchActions.SearchCriReportId20.bind(AdmClientRuleReduxObj) },
    { SearchCriCultureId30: AdmClientRuleReduxObj.SearchActions.SearchCriCultureId30.bind(AdmClientRuleReduxObj) },
    { setTitle: setTitle },

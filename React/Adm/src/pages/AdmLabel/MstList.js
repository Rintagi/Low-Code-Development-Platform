...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmLabel || {}).initialized) {
      ...
      this.props.SetScreenCriteria(values.search,
        {
          ...
          LabelKey30: (values.cCriLabelKey30) ? values.cCriLabelKey30 : '',
          LabelText40: (values.cCriLabelText40) ? values.cCriLabelText40 : '',
        },
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmLabel || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).LabelId215) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    // if (!AdmLabelState.initialized || AdmLabelState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmLabelReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmLabelReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmLabelState.ScreenCriteria || {}
    ...
    const currMst = AdmLabelState.Mst;
    const currDtl = AdmLabelState.EditDtl;
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.LabelId215));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmLabelReduxObj.QuickFilterDdlToSelectList(AdmLabelState);
    ...
    const CriCultureId10List = screenCriDdlSelectors.CultureId10(AdmLabelState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmLabelState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmLabelState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).LabelId215)}
                    ...
                    <Formik
                      initialValues={{
                        ...
                        cCriLabelKey30: ((screenCriteria || {}).LabelKey30 || {}).LastCriteria,
                        cCriLabelText40: ((screenCriteria || {}).LabelText40 || {}).LastCriteria,
                        search: screenCriteria.SearchStr || '',
                        cFilterId: (screenFilterSelected.length > 0 ? screenFilterSelected[0] : screenFilterList[0])
                      }}
                      key={AdmLabelState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              <div className='form__form-group'>
                                <div className={`form__form-group filter-padding ${filterVisibility}`} key={screenCriteria.key}>
                                  <Row className='mb-5'>
                                    ...
                                    <Col xs={12} md={12}>
                                      <label className='form__form-group-label filter-label'>{(screenCriteria.LabelKey30 || {}).ColumnHeader}</label>
                                      <div className='form__form-group-field filter-form-border'>
                                        <Field
                                          type='text'
                                          name='cCriLabelKey30'
                                          value={values.cCriLabelKey30}
                                          onBlur={this.SearchFilterTextValueChange(handleSubmit, setFieldValue, 'text', 'cCriLabelKey30')}
                                        />
                                      </div>
                                    </Col>
                                    <Col xs={12} md={12}>
                                      <label className='form__form-group-label filter-label'>{(screenCriteria.LabelText40 || {}).ColumnHeader}</label>
                                      <div className='form__form-group-field filter-form-border'>
                                        <Field
                                          type='text'
                                          name='cCriLabelText40'
                                          value={values.cCriLabelText40}
                                          onBlur={this.SearchFilterTextValueChange(handleSubmit, setFieldValue, 'text', 'cCriLabelText40')}
                                        />
                                      </div>
                                    </Col>
                                  </Row>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmLabelState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmLabelState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmLabelReduxObj.LoadInitPage.bind(AdmLabelReduxObj) },
    { LoadSearchList: AdmLabelReduxObj.LoadSearchList.bind(AdmLabelReduxObj) },
    { SelectMst: AdmLabelReduxObj.SelectMst.bind(AdmLabelReduxObj) },
    { DelMst: AdmLabelReduxObj.DelMst.bind(AdmLabelReduxObj) },
    { AddMst: AdmLabelReduxObj.AddMst.bind(AdmLabelReduxObj) },
    { AddDtl: AdmLabelReduxObj.AddDtl.bind(AdmLabelReduxObj) },
    { changeMstListFilterVisibility: AdmLabelReduxObj.ChangeMstListFilterVisibility.bind(AdmLabelReduxObj) },
    { SetScreenCriteria: AdmLabelReduxObj.SetScreenCriteria.bind(AdmLabelReduxObj) },
    { SearchCriCultureId10: AdmLabelReduxObj.SearchActions.SearchCriCultureId10.bind(AdmLabelReduxObj) },
    { setTitle: setTitle },

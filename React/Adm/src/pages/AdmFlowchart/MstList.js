...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmFlowchart || {}).initialized) {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmFlowchart || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).ChartId1325) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    // if (!AdmFlowchartState.initialized || AdmFlowchartState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmFlowchartReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmFlowchartReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmFlowchartState.ScreenCriteria || {}
    ...
    const currMst = AdmFlowchartState.Mst;
    const currDtl = AdmFlowchartState.EditDtl;
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.ChartId1325));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmFlowchartReduxObj.QuickFilterDdlToSelectList(AdmFlowchartState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmFlowchartState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmFlowchartState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).ChartId1325)}
                    ...
                    <Formik
                      ...
                      key={AdmFlowchartState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmFlowchartState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmFlowchartState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmFlowchartReduxObj.LoadInitPage.bind(AdmFlowchartReduxObj) },
    { LoadSearchList: AdmFlowchartReduxObj.LoadSearchList.bind(AdmFlowchartReduxObj) },
    { SelectMst: AdmFlowchartReduxObj.SelectMst.bind(AdmFlowchartReduxObj) },

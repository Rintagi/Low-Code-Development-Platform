...
import * as AdmMenuOptService from '../../services/AdmMenuOptService';
...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmMenuOpt || {}).initialized) {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmMenuOpt || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).MenuOptId248) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    // if (!AdmMenuOptState.initialized || AdmMenuOptState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmMenuOptReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmMenuOptReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmMenuOptState.ScreenCriteria || {}
    ...
    const currMst = AdmMenuOptState.Mst;
    const currDtl = AdmMenuOptState.EditDtl;
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.MenuOptId248));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmMenuOptReduxObj.QuickFilterDdlToSelectList(AdmMenuOptState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmMenuOptState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmMenuOptState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).MenuOptId248)}
                    ...
                    <Formik
                      ...
                      key={AdmMenuOptState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmMenuOptState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmMenuOptState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmMenuOptReduxObj.LoadInitPage.bind(AdmMenuOptReduxObj) },
    { LoadSearchList: AdmMenuOptReduxObj.LoadSearchList.bind(AdmMenuOptReduxObj) },
    { SelectMst: AdmMenuOptReduxObj.SelectMst.bind(AdmMenuOptReduxObj) },
    { DelMst: AdmMenuOptReduxObj.DelMst.bind(AdmMenuOptReduxObj) },
    { AddMst: AdmMenuOptReduxObj.AddMst.bind(AdmMenuOptReduxObj) },
    { AddDtl: AdmMenuOptReduxObj.AddDtl.bind(AdmMenuOptReduxObj) },
    { changeMstListFilterVisibility: AdmMenuOptReduxObj.ChangeMstListFilterVisibility.bind(AdmMenuOptReduxObj) },
    { SetScreenCriteria: AdmMenuOptReduxObj.SetScreenCriteria.bind(AdmMenuOptReduxObj) },

...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmUsrPref || {}).initialized) {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmUsrPref || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).UsrPrefId93) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    // if (!AdmUsrPrefState.initialized || AdmUsrPrefState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmUsrPrefReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmUsrPrefReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmUsrPrefState.ScreenCriteria || {}
    ...
    const currMst = AdmUsrPrefState.Mst;
    const currDtl = AdmUsrPrefState.EditDtl;
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.UsrPrefId93));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmUsrPrefReduxObj.QuickFilterDdlToSelectList(AdmUsrPrefState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmUsrPrefState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmUsrPrefState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).UsrPrefId93)}
                    ...
                    <Formik
                      ...
                      key={AdmUsrPrefState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmUsrPrefState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmUsrPrefState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmUsrPrefReduxObj.LoadInitPage.bind(AdmUsrPrefReduxObj) },
    { LoadSearchList: AdmUsrPrefReduxObj.LoadSearchList.bind(AdmUsrPrefReduxObj) },
    { SelectMst: AdmUsrPrefReduxObj.SelectMst.bind(AdmUsrPrefReduxObj) },
    { DelMst: AdmUsrPrefReduxObj.DelMst.bind(AdmUsrPrefReduxObj) },
    { AddMst: AdmUsrPrefReduxObj.AddMst.bind(AdmUsrPrefReduxObj) },
    { AddDtl: AdmUsrPrefReduxObj.AddDtl.bind(AdmUsrPrefReduxObj) },
    { changeMstListFilterVisibility: AdmUsrPrefReduxObj.ChangeMstListFilterVisibility.bind(AdmUsrPrefReduxObj) },
    { SetScreenCriteria: AdmUsrPrefReduxObj.SetScreenCriteria.bind(AdmUsrPrefReduxObj) },

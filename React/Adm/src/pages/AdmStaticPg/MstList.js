...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmStaticPg || {}).initialized) {
  ...
  RefreshSearchList(values, { setErrors, resetForm, setValues /* setValues and other goodies */ }) {
    ...
    const refreshFn = (() => {
      this.props.SetScreenCriteria(values.search,
        {
          StaticPgId10: (values.cCriStaticPgId10) ? values.cCriStaticPgId10 : '',
          StaticMeta20: (values.cCriStaticMeta20) ? values.cCriStaticMeta20 : '',
        },
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmStaticPg || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).StaticPgId259) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    // if (!AdmStaticPgState.initialized || AdmStaticPgState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmStaticPgReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmStaticPgReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmStaticPgState.ScreenCriteria || {}
    ...
    const currMst = AdmStaticPgState.Mst;
    const currDtl = AdmStaticPgState.EditDtl;
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.StaticPgId259));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmStaticPgReduxObj.QuickFilterDdlToSelectList(AdmStaticPgState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmStaticPgState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmStaticPgState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).StaticPgId259)}
                    ...
                    <Formik
                      initialValues={{
                        cCriStaticPgId10: ((screenCriteria || {}).StaticPgId10 || {}).LastCriteria,
                        cCriStaticMeta20: ((screenCriteria || {}).StaticMeta20 || {}).LastCriteria,
                        search: screenCriteria.SearchStr || '',
                        cFilterId: (screenFilterSelected.length > 0 ? screenFilterSelected[0] : screenFilterList[0])
                      }}
                      key={AdmStaticPgState.searchListVersion}
                            ...
                            <Form className='form'>
                              <div className='form__form-group'>
                                <div className={`form__form-group filter-padding ${filterVisibility}`} key={screenCriteria.key}>
                                  <Row className='mb-5'>
                                    <Col xs={12} md={12}>
                                      <label className='form__form-group-label filter-label'>{(screenCriteria.StaticPgId10 || {}).ColumnHeader}</label>
                                      <div className='form__form-group-field filter-form-border'>
                                        <Field
                                          type='text'
                                          name='cCriStaticPgId10'
                                          value={values.cCriStaticPgId10}
                                          onBlur={this.SearchFilterTextValueChange(handleSubmit, setFieldValue, 'text', 'cCriStaticPgId10')}
                                        />
                                      </div>
                                    </Col>
                                    <Col xs={12} md={12}>
                                      <label className='form__form-group-label filter-label'>{(screenCriteria.StaticMeta20 || {}).ColumnHeader}</label>
                                      <div className='form__form-group-field filter-form-border'>
                                        <Field
                                          type='text'
                                          name='cCriStaticMeta20'
                                          value={values.cCriStaticMeta20}
                                          onBlur={this.SearchFilterTextValueChange(handleSubmit, setFieldValue, 'text', 'cCriStaticMeta20')}
                                        />
                                      </div>
                                    </Col>
                                  </Row>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmStaticPgState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmStaticPgState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmStaticPgReduxObj.LoadInitPage.bind(AdmStaticPgReduxObj) },
    { LoadSearchList: AdmStaticPgReduxObj.LoadSearchList.bind(AdmStaticPgReduxObj) },
    { SelectMst: AdmStaticPgReduxObj.SelectMst.bind(AdmStaticPgReduxObj) },
    { DelMst: AdmStaticPgReduxObj.DelMst.bind(AdmStaticPgReduxObj) },
    { AddMst: AdmStaticPgReduxObj.AddMst.bind(AdmStaticPgReduxObj) },
    { AddDtl: AdmStaticPgReduxObj.AddDtl.bind(AdmStaticPgReduxObj) },
    { changeMstListFilterVisibility: AdmStaticPgReduxObj.ChangeMstListFilterVisibility.bind(AdmStaticPgReduxObj) },
    { SetScreenCriteria: AdmStaticPgReduxObj.SetScreenCriteria.bind(AdmStaticPgReduxObj) },

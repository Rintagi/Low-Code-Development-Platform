...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmAtRowAuth || {}).initialized) {
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).RowAuthId236) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    const screenDdlSelectors = AdmAtRowAuthReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmAtRowAuthReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmAtRowAuthState.ScreenCriteria || {}
    ...
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.RowAuthId236));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmAtRowAuthReduxObj.QuickFilterDdlToSelectList(AdmAtRowAuthState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmAtRowAuthState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmAtRowAuthState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).RowAuthId236)}
                    ...
                    <Formik
                      ...
                      key={AdmAtRowAuthState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmAtRowAuthState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmAtRowAuthState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmAtRowAuthReduxObj.LoadInitPage.bind(AdmAtRowAuthReduxObj) },
    { LoadSearchList: AdmAtRowAuthReduxObj.LoadSearchList.bind(AdmAtRowAuthReduxObj) },
    { SelectMst: AdmAtRowAuthReduxObj.SelectMst.bind(AdmAtRowAuthReduxObj) },
    ...
    { changeMstListFilterVisibility: AdmAtRowAuthReduxObj.ChangeMstListFilterVisibility.bind(AdmAtRowAuthReduxObj) },
    { SetScreenCriteria: AdmAtRowAuthReduxObj.SetScreenCriteria.bind(AdmAtRowAuthReduxObj) },

...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmUsrGroup || {}).initialized) {
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).UsrGroupId7) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    const screenDdlSelectors = AdmUsrGroupReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmUsrGroupReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmUsrGroupState.ScreenCriteria || {}
    ...
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.UsrGroupId7));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmUsrGroupReduxObj.QuickFilterDdlToSelectList(AdmUsrGroupState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmUsrGroupState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmUsrGroupState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).UsrGroupId7)}
                    ...
                    <Formik
                      ...
                      key={AdmUsrGroupState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmUsrGroupState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmUsrGroupState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmUsrGroupReduxObj.LoadInitPage.bind(AdmUsrGroupReduxObj) },
    { LoadSearchList: AdmUsrGroupReduxObj.LoadSearchList.bind(AdmUsrGroupReduxObj) },
    { SelectMst: AdmUsrGroupReduxObj.SelectMst.bind(AdmUsrGroupReduxObj) },
    ...
    { changeMstListFilterVisibility: AdmUsrGroupReduxObj.ChangeMstListFilterVisibility.bind(AdmUsrGroupReduxObj) },
    { SetScreenCriteria: AdmUsrGroupReduxObj.SetScreenCriteria.bind(AdmUsrGroupReduxObj) },

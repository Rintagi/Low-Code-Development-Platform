...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmRptwizTyp || {}).initialized) {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmRptwizTyp || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).RptwizTypId185) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    // if (!AdmRptwizTypState.initialized || AdmRptwizTypState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmRptwizTypReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmRptwizTypReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmRptwizTypState.ScreenCriteria || {}
    ...
    const currMst = AdmRptwizTypState.Mst;
    const currDtl = AdmRptwizTypState.EditDtl;
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.RptwizTypId185));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmRptwizTypReduxObj.QuickFilterDdlToSelectList(AdmRptwizTypState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmRptwizTypState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmRptwizTypState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).RptwizTypId185)}
                    ...
                    <Formik
                      ...
                      key={AdmRptwizTypState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmRptwizTypState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmRptwizTypState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmRptwizTypReduxObj.LoadInitPage.bind(AdmRptwizTypReduxObj) },
    { LoadSearchList: AdmRptwizTypReduxObj.LoadSearchList.bind(AdmRptwizTypReduxObj) },
    { SelectMst: AdmRptwizTypReduxObj.SelectMst.bind(AdmRptwizTypReduxObj) },
    { DelMst: AdmRptwizTypReduxObj.DelMst.bind(AdmRptwizTypReduxObj) },
    { AddMst: AdmRptwizTypReduxObj.AddMst.bind(AdmRptwizTypReduxObj) },
    { AddDtl: AdmRptwizTypReduxObj.AddDtl.bind(AdmRptwizTypReduxObj) },
    { changeMstListFilterVisibility: AdmRptwizTypReduxObj.ChangeMstListFilterVisibility.bind(AdmRptwizTypReduxObj) },
    { SetScreenCriteria: AdmRptwizTypReduxObj.SetScreenCriteria.bind(AdmRptwizTypReduxObj) },

...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmRptCha || {}).initialized) {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmRptCha || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).RptChaId206) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    // if (!AdmRptChaState.initialized || AdmRptChaState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmRptChaReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmRptChaReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmRptChaState.ScreenCriteria || {}
    ...
    const currMst = AdmRptChaState.Mst;
    const currDtl = AdmRptChaState.EditDtl;
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.RptChaId206));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmRptChaReduxObj.QuickFilterDdlToSelectList(AdmRptChaState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmRptChaState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmRptChaState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).RptChaId206)}
                    ...
                    <Formik
                      ...
                      key={AdmRptChaState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmRptChaState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmRptChaState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmRptChaReduxObj.LoadInitPage.bind(AdmRptChaReduxObj) },
    { LoadSearchList: AdmRptChaReduxObj.LoadSearchList.bind(AdmRptChaReduxObj) },
    { SelectMst: AdmRptChaReduxObj.SelectMst.bind(AdmRptChaReduxObj) },
    { DelMst: AdmRptChaReduxObj.DelMst.bind(AdmRptChaReduxObj) },
    { AddMst: AdmRptChaReduxObj.AddMst.bind(AdmRptChaReduxObj) },
    { AddDtl: AdmRptChaReduxObj.AddDtl.bind(AdmRptChaReduxObj) },
    { changeMstListFilterVisibility: AdmRptChaReduxObj.ChangeMstListFilterVisibility.bind(AdmRptChaReduxObj) },
    { SetScreenCriteria: AdmRptChaReduxObj.SetScreenCriteria.bind(AdmRptChaReduxObj) },

...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmServerRule || {}).initialized) {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmServerRule || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).ServerRuleId24) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    // if (!AdmServerRuleState.initialized || AdmServerRuleState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmServerRuleReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmServerRuleReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmServerRuleState.ScreenCriteria || {}
    ...
    const currMst = AdmServerRuleState.Mst;
    const currDtl = AdmServerRuleState.EditDtl;
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.ServerRuleId24));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmServerRuleReduxObj.QuickFilterDdlToSelectList(AdmServerRuleState);
    ...
    const CriScreenId10List = screenCriDdlSelectors.ScreenId10(AdmServerRuleState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmServerRuleState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmServerRuleState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).ServerRuleId24)}
                    ...
                    <Formik
                      ...
                      key={AdmServerRuleState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmServerRuleState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmServerRuleState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmServerRuleReduxObj.LoadInitPage.bind(AdmServerRuleReduxObj) },
    { LoadSearchList: AdmServerRuleReduxObj.LoadSearchList.bind(AdmServerRuleReduxObj) },
    { SelectMst: AdmServerRuleReduxObj.SelectMst.bind(AdmServerRuleReduxObj) },
    ...
    { AddDtl: AdmServerRuleReduxObj.AddDtl.bind(AdmServerRuleReduxObj) },
    { changeMstListFilterVisibility: AdmServerRuleReduxObj.ChangeMstListFilterVisibility.bind(AdmServerRuleReduxObj) },
    { SetScreenCriteria: AdmServerRuleReduxObj.SetScreenCriteria.bind(AdmServerRuleReduxObj) },
    { SearchCriScreenId10: AdmServerRuleReduxObj.SearchActions.SearchCriScreenId10.bind(AdmServerRuleReduxObj) },
    { setTitle: setTitle },

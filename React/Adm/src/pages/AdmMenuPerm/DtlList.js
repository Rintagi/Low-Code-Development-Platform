...
import AdmMenuPermReduxObj from '../../redux/AdmMenuPerm';

class DtlList extends RintagiScreen {
  constructor(props) {
    super(props);
    this.GetReduxState = ()=> (this.props.AdmMenuPerm || {});
    this.titleSet = false;
    this.hasChangedContent = false;
    this.SystemName = 'FintruX';
    this.MstKeyColumnName = 'MenuId39';
    this.DtlKeyColumnName = 'MenuPrmId231';
  ...
  CopyRow({ mst, dtlId, dtl, ...rest }) {
    const AdmMenuPermState = this.props.AdmMenuPerm || {};
    const auxSystemLabels = AdmMenuPermState.SystemLabel || {};
    return function (evt) {
      evt.preventDefault();
      const currDtlId = dtlId || dtl.MenuPrmId231;
      const copyFn = () => {
        if (currDtlId) {
          ...
          if (useMobileView) {
            const naviBar = getNaviBar('DtlList', mst, {}, this.props.AdmMenuPerm.Label);
  ...
  DelDtl({ mst, dtlId, dtl }) {
    ...
    return function (evt) {
      ...
      const deleteFn = () => {
        ...
        if (currDtlId) {
          this.props.SavePage(
            this.props.AdmMenuPerm,
            this.props.AdmMenuPerm.Mst,
            [
              {
                MenuPrmId231: currDtlId,
                _mode: 'delete',
              }
            ],
            {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmMenuPerm || {}).Buttons || {};
  ...
  componentDidMount() {
    ...
    if (!(this.props.AdmMenuPerm || {}).AuthCol || true)
  ...
  componentDidUpdate(prevProps, prevStates) {
    const currReduxScreenState = this.props.AdmMenuPerm || {};
  ...
  render() {
    ...
    if (AdmMenuPermState.access_denied) {
      return <Redirect to='/error' />;
    }
    //if (!AdmMenuPermState.initialized || AdmMenuPermState.page_loading) return null;
    const screenHlp = AdmMenuPermState.ScreenHlp;
    ...
    const screenButtons = AdmMenuPermReduxObj.GetScreenButtons(AdmMenuPermState) || {};
    const itemList = AdmMenuPermReduxObj.DtlListToSelectList(AdmMenuPermState, 'MenuPrmId231');
    const selectList = AdmMenuPermReduxObj.SearchListToSelectList(AdmMenuPermState);
    const selectedMst = (selectList || []).filter(v=>v.isSelected)[0] || {};
    const auxLabels = AdmMenuPermState.Label || {};
    const auxSystemLabels = AdmMenuPermState.SystemLabel || {};
    const columnLabel = AdmMenuPermState.ColumnLabel;
    ...
    const currMst = AdmMenuPermState.Mst;
    const currDtl = AdmMenuPermState.EditDtl;
    const dtlFilter = AdmMenuPermState.DtlFilter;
    const filterList = AdmMenuPermState.DtlFilter.FilterColumnDdl.map((v, i) => ({ key: v.ColName, label: v.ColumnHeader, value: v.ColName, pos: i }))
    const naviBar = getNaviBar('DtlList', currMst, currDtl, screenButtons);
    const authRow = (AdmMenuPermState.AuthRow || [])[0] || {};
                        ...
                        resetForm
                          ...
                          <div>
                                      ...
                                      <ButtonGroup className='btn-group--icons'>
                                        ...
                                        </Button>
                                        {
                                          dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                            if ((!activeSelectionVisible && v.buttonType !== 'InsRow') || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).MenuId39)) return null;
                                            return (
                                              <Button
                                                key={v.tid}
                                        ...
                                        {activeSelectionVisible &&
                                          dropdownMenuButtonList
                                            .filter(v => !v.expose)
                                            .filter(v => (!this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).MenuId39,(currDtl || {}).MenuPrmId231)))
                                            .length > 0 &&
                                        ...
                                        <DropdownMenu right className={`dropdown__menu dropdown-options`}>
                                          {
                                            ...
                                            dropdownMenuButtonList.filter(v => !v.expose).map(v => {
                                              if ((!activeSelectionVisible && v.buttonType !== 'InsRow') || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).MenuId39,(currDtl || {}).MenuPrmId231)) return null;
                                              return (
                            ...
                            <Form className='form'>
                                ...
                                </div>
                                {
                                  itemList.slice(0, dtlFilter.Limit).map(
                                    (obj, i) => {
                                      ...
                                      return (
                                        <div className='form__form-group-narrow list-divider' key={obj.MenuPrmId231 || ('_' + i)}>
                                          <div className='form__form-group-field'>
                                            <label className='radio-btn radio-btn--button margin-narrow'>
                                              <Field
                                                key={obj.MenuPrmId231 || ('_' + i)}
                                                className='radio-btn__radio'
                                                name='expenseItem'
                                                type='radio'
                                                value={values.name}
                                                onClick={this.SelectDtlListRow(naviSelectBar, currMst, obj, i, 'MenuId39', 'MenuPrmId231')}
                                              ...
                                              </span>
                                              {
                                                rowMenuButtonList
                                                  ...
                                                  .map((v, i) => {
                                                    return (
                                                      <button type='button' onClick={this.ScreenButtonAction[v.buttonType]({ naviBar: naviSelectBar, mst: currMst, useMobileView, dtlId: obj.MenuPrmId231 })} className={`${v.exposedClassName}`}><i className={`${v.iconClassName}`}></i></button>
                                                    )
                                                  })
                                              }
                                              {
                                                ...
                                                <UncontrolledDropdown className={`btn-row-dropdown`}>
                                                  ...
                                                  <DropdownMenu right className={`dropdown__menu dropdown-options`}>
                                                    {rowMenuButtonList
                                                      .filter(v => !v.expose || useMobileView)
                                                      .map((v) => {
                                                        if (this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).MenuId39,obj.MenuPrmId231)) return null;
                                                        return <DropdownItem key={v.tid} onClick={this.ScreenButtonAction[v.buttonType]({ naviBar: naviSelectBar, mst: currMst, useMobileView, dtlId: obj.MenuPrmId231 })} className={`${v.className}`}><i className={`${v.iconClassName} mr-10`}></i>{v.label}</DropdownItem>
                                                      })
                                                    }
                                                  </DropdownMenu>
                                                </UncontrolledDropdown>
                              ...
                              <Button onClick={this.props.ViewMoreDtl} className={`${viewMoreClass} btn btn-view-more-blue account__btn`} type='button'>{strFormat(IncrementMsg, this.props.AdmMenuPerm.DtlFilter.PageSize)}<br /><i className='fa fa-arrow-down'></i></Button>
            ...
            {!useMobileView && <Col xs={6}>
              ...
              {!activeSelectionVisible &&
                ...
                <div className='empty-block'>
                  ...
                  <p className='create-new-message'>{NoDetailMsg}. <span className='link-imitation' onClick={this.AddNewDtl({ naviBar, mstId: currMst.MenuId39 })}>{AddDetailMsg}</span></p>
                </div>}
...
const mapStateToProps = (state) => ({
  user: state.user,
  error: state.error,
  AdmMenuPerm: state.AdmMenuPerm,
  global: state.global,
});

const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    { LoadPage: AdmMenuPermReduxObj.LoadPage.bind(AdmMenuPermReduxObj) },
    { SelectDtl: AdmMenuPermReduxObj.SelectDtl.bind(AdmMenuPermReduxObj) },
    { SavePage: AdmMenuPermReduxObj.SavePage.bind(AdmMenuPermReduxObj) },
    { AddDtl: AdmMenuPermReduxObj.AddDtl.bind(AdmMenuPermReduxObj) },
    { ChangeDtlListFilterVisibility: AdmMenuPermReduxObj.ChangeDtlListFilterVisibility.bind(AdmMenuPermReduxObj) },
    { ChangeDtlListFilter: AdmMenuPermReduxObj.ChangeDtlListFilter.bind(AdmMenuPermReduxObj) },
    { ViewMoreDtl: AdmMenuPermReduxObj.ViewMoreDtl.bind(AdmMenuPermReduxObj) },
    { setTitle: setTitle },

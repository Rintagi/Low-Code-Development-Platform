...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmMenuPerm || {}).initialized) {
  ...
  DelMst({ naviBar, ScreenButton, mst, mstId }) {
    ...
    return this.Prompt({
      okFn: function (evt) {
        const fromMstId = mstId || mst.MenuId39;
        this.props.DelMst(this.props.AdmMenuPerm, fromMstId);
      }.bind(this),
  ...
  render() {
    ...
    const screenDdlSelectors = AdmMenuPermReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmMenuPermReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmMenuPermState.ScreenCriteria || {}
    const selectList = AdmMenuPermReduxObj.SearchListToSelectList(AdmMenuPermState);
    const itemList = AdmMenuPermState.Dtl || [];
    ...
    const screenFilterList = AdmMenuPermReduxObj.QuickFilterDdlToSelectList(AdmMenuPermState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmMenuPermState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmMenuPermState) && <i className='filter-applied'></i>}
                    ...
                    <Formik
                      ...
                      key={AdmMenuPermState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmMenuPermState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmMenuPermState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmMenuPermReduxObj.LoadInitPage.bind(AdmMenuPermReduxObj) },
    { LoadSearchList: AdmMenuPermReduxObj.LoadSearchList.bind(AdmMenuPermReduxObj) },
    { SelectMst: AdmMenuPermReduxObj.SelectMst.bind(AdmMenuPermReduxObj) },
    { DelMst: AdmMenuPermReduxObj.DelMst.bind(AdmMenuPermReduxObj) },
    { AddMst: AdmMenuPermReduxObj.AddMst.bind(AdmMenuPermReduxObj) },
    { AddDtl: AdmMenuPermReduxObj.AddDtl.bind(AdmMenuPermReduxObj) },
    { changeMstListFilterVisibility: AdmMenuPermReduxObj.ChangeMstListFilterVisibility.bind(AdmMenuPermReduxObj) },
    { SetScreenCriteria: AdmMenuPermReduxObj.SetScreenCriteria.bind(AdmMenuPermReduxObj) },

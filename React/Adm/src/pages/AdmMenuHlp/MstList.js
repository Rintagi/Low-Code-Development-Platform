...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmMenuHlp || {}).initialized) {
  ...
  DelMst({ naviBar, ScreenButton, mst, mstId }) {
    ...
    return this.Prompt({
      okFn: function (evt) {
        const fromMstId = mstId || mst.MenuId39;
        this.props.DelMst(this.props.AdmMenuHlp, fromMstId);
      }.bind(this),
  ...
  render() {
    ...
    const screenDdlSelectors = AdmMenuHlpReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmMenuHlpReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmMenuHlpState.ScreenCriteria || {}
    const selectList = AdmMenuHlpReduxObj.SearchListToSelectList(AdmMenuHlpState);
    const itemList = AdmMenuHlpState.Dtl || [];
    ...
    const screenFilterList = AdmMenuHlpReduxObj.QuickFilterDdlToSelectList(AdmMenuHlpState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmMenuHlpState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmMenuHlpState) && <i className='filter-applied'></i>}
                    ...
                    <Formik
                      ...
                      key={AdmMenuHlpState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmMenuHlpState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmMenuHlpState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmMenuHlpReduxObj.LoadInitPage.bind(AdmMenuHlpReduxObj) },
    { LoadSearchList: AdmMenuHlpReduxObj.LoadSearchList.bind(AdmMenuHlpReduxObj) },
    { SelectMst: AdmMenuHlpReduxObj.SelectMst.bind(AdmMenuHlpReduxObj) },
    { DelMst: AdmMenuHlpReduxObj.DelMst.bind(AdmMenuHlpReduxObj) },
    { AddMst: AdmMenuHlpReduxObj.AddMst.bind(AdmMenuHlpReduxObj) },
    { AddDtl: AdmMenuHlpReduxObj.AddDtl.bind(AdmMenuHlpReduxObj) },
    { changeMstListFilterVisibility: AdmMenuHlpReduxObj.ChangeMstListFilterVisibility.bind(AdmMenuHlpReduxObj) },
    { SetScreenCriteria: AdmMenuHlpReduxObj.SetScreenCriteria.bind(AdmMenuHlpReduxObj) },

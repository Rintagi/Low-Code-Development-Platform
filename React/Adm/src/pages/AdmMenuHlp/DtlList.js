...
import AdmMenuHlpReduxObj from '../../redux/AdmMenuHlp';
import * as AdmMenuHlpService from '../../services/AdmMenuHlpService';
...
class DtlList extends RintagiScreen {
    ...
    return function (evt) {
      ...
      const copyFn = () => {
        if (currDtlId) {
          ...
          if (useMobileView) {
            const naviBar = getNaviBar('DtlList', mst, {}, this.props.AdmMenuHlp.Label);
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmMenuHlp || {}).Buttons || {};
  ...
  render() {
    ...
    //if (!AdmMenuHlpState.initialized || AdmMenuHlpState.page_loading) return null;
    ...
    const itemList = AdmMenuHlpReduxObj.DtlListToSelectList(AdmMenuHlpState, 'MenuHlpId40');
    const selectList = AdmMenuHlpReduxObj.SearchListToSelectList(AdmMenuHlpState);
    ...
    const dtlFilter = AdmMenuHlpState.DtlFilter;
    const filterList = AdmMenuHlpState.DtlFilter.FilterColumnDdl.map((v, i) => ({ key: v.ColName, label: v.ColumnHeader, value: v.ColName, pos: i }))
                        ...
                        resetForm
                          ...
                          <div>
                                      ...
                                      <ButtonGroup className='btn-group--icons'>
                                        ...
                                        {activeSelectionVisible &&
                                          dropdownMenuButtonList
                                            .filter(v => !v.expose)
                                            .filter(v => (!this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).MenuId39, (currDtl || {}).MenuHlpId40)))
                                            .length > 0 &&
                                        ...
                                        <DropdownMenu right className={`dropdown__menu dropdown-options`}>
                                          {
                                            ...
                                            dropdownMenuButtonList.filter(v => !v.expose).map(v => {
                                              if ((!activeSelectionVisible && v.buttonType !== 'InsRow') || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).MenuId39, (currDtl || {}).MenuHlpId40)) return null;
                                              return (
                            ...
                            <Form className='form'>
                                ...
                                </div>
                                {
                                  itemList.slice(0, dtlFilter.Limit).map(
                                    (obj, i) => {
                                      ...
                                      return (
                                        <div className='form__form-group-narrow list-divider' key={obj.MenuHlpId40 || ('_' + i)}>
                                          <div className='form__form-group-field'>
                                            <label className='radio-btn radio-btn--button margin-narrow'>
                                              <Field
                                                key={obj.MenuHlpId40 || ('_' + i)}
                                                className='radio-btn__radio'
                                                name='expenseItem'
                                                type='radio'
                                                value={values.name}
                                                onClick={this.SelectDtlListRow(naviSelectBar, currMst, obj, i, 'MenuId39', 'MenuHlpId40')}
                                              ...
                                              </span>
                                              {
                                                rowMenuButtonList
                                                  ...
                                                  .map((v, i) => {
                                                    return (
                                                      <button type='button' onClick={this.ScreenButtonAction[v.buttonType]({ naviBar: naviSelectBar, mst: currMst, useMobileView, dtlId: obj.MenuHlpId40 })} className={`${v.exposedClassName}`}><i className={`${v.iconClassName}`}></i></button>
                                                    )
                                                  })
                                              }
                                              {
                                                ...
                                                <UncontrolledDropdown className={`btn-row-dropdown`}>
                                                  ...
                                                  <DropdownMenu right className={`dropdown__menu dropdown-options`}>
                                                    {rowMenuButtonList
                                                      .filter(v => !v.expose || useMobileView)
                                                      .map((v) => {
                                                        if (this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).MenuId39, obj.MenuHlpId40)) return null;
                                                        return <DropdownItem key={v.tid} onClick={this.ScreenButtonAction[v.buttonType]({ naviBar: naviSelectBar, mst: currMst, useMobileView, dtlId: obj.MenuHlpId40 })} className={`${v.className}`}><i className={`${v.iconClassName} mr-10`}></i>{v.label}</DropdownItem>
                                                      })
                                                    }
                                                  </DropdownMenu>
                                                </UncontrolledDropdown>
                              ...
                              <Button onClick={this.props.ViewMoreDtl} className={`${viewMoreClass} btn btn-view-more-blue account__btn`} type='button'>{strFormat(IncrementMsg, this.props.AdmMenuHlp.DtlFilter.PageSize)}<br /><i className='fa fa-arrow-down'></i></Button>
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { SelectDtl: AdmMenuHlpReduxObj.SelectDtl.bind(AdmMenuHlpReduxObj) },
    { SavePage: AdmMenuHlpReduxObj.SavePage.bind(AdmMenuHlpReduxObj) },
    { AddDtl: AdmMenuHlpReduxObj.AddDtl.bind(AdmMenuHlpReduxObj) },
    { ChangeDtlListFilterVisibility: AdmMenuHlpReduxObj.ChangeDtlListFilterVisibility.bind(AdmMenuHlpReduxObj) },
    { ChangeDtlListFilter: AdmMenuHlpReduxObj.ChangeDtlListFilter.bind(AdmMenuHlpReduxObj) },
    { ViewMoreDtl: AdmMenuHlpReduxObj.ViewMoreDtl.bind(AdmMenuHlpReduxObj) },
    { setTitle: setTitle },

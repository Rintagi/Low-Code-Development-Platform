...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmScreenCri || {}).initialized) {
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).ScreenCriId104) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    const screenDdlSelectors = AdmScreenCriReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmScreenCriReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmScreenCriState.ScreenCriteria || {}
    ...
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.ScreenCriId104));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmScreenCriReduxObj.QuickFilterDdlToSelectList(AdmScreenCriState);
    ...
    const CriScreenId10List = screenCriDdlSelectors.ScreenId10(AdmScreenCriState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmScreenCriState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmScreenCriState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).ScreenCriId104)}
                    ...
                    <Formik
                      ...
                      key={AdmScreenCriState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmScreenCriState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmScreenCriState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmScreenCriReduxObj.LoadInitPage.bind(AdmScreenCriReduxObj) },
    { LoadSearchList: AdmScreenCriReduxObj.LoadSearchList.bind(AdmScreenCriReduxObj) },
    { SelectMst: AdmScreenCriReduxObj.SelectMst.bind(AdmScreenCriReduxObj) },
    ...
    { changeMstListFilterVisibility: AdmScreenCriReduxObj.ChangeMstListFilterVisibility.bind(AdmScreenCriReduxObj) },
    { SetScreenCriteria: AdmScreenCriReduxObj.SetScreenCriteria.bind(AdmScreenCriReduxObj) },
    { SearchCriScreenId10: AdmScreenCriReduxObj.SearchActions.SearchCriScreenId10.bind(AdmScreenCriReduxObj) },
    { setTitle: setTitle },

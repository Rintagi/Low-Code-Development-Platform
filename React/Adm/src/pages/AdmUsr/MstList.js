...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmUsr || {}).initialized) {
  ...
  RefreshSearchList(values, { setErrors, resetForm, setValues /* setValues and other goodies */ }) {
    ...
    const refreshFn = (() => {
      this.props.SetScreenCriteria(values.search,
        {
          UsrEmail10: (values.cCriUsrEmail10) ? values.cCriUsrEmail10 : '',
          UsrName20: (values.cCriUsrName20) ? values.cCriUsrName20 : '',
          UsrGroupLs30: (values.cCriUsrGroupLs30) ? values.cCriUsrGroupLs30 : '',
        },
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmUsr || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).UsrId1) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    // if (!AdmUsrState.initialized || AdmUsrState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmUsrReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmUsrReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmUsrState.ScreenCriteria || {}
    ...
    const currMst = AdmUsrState.Mst;
    const currDtl = AdmUsrState.EditDtl;
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.UsrId1));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmUsrReduxObj.QuickFilterDdlToSelectList(AdmUsrState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmUsrState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmUsrState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).UsrId1)}
                    ...
                    <Formik
                      initialValues={{
                        cCriUsrEmail10: ((screenCriteria || {}).UsrEmail10 || {}).LastCriteria,
                        cCriUsrName20: ((screenCriteria || {}).UsrName20 || {}).LastCriteria,
                        cCriUsrGroupLs30: ((screenCriteria || {}).UsrGroupLs30 || {}).LastCriteria,
                        search: screenCriteria.SearchStr || '',
                        cFilterId: (screenFilterSelected.length > 0 ? screenFilterSelected[0] : screenFilterList[0])
                      }}
                      key={AdmUsrState.searchListVersion}
                            ...
                            <Form className='form'>
                              <div className='form__form-group'>
                                <div className={`form__form-group filter-padding ${filterVisibility}`} key={screenCriteria.key}>
                                  <Row className='mb-5'>
                                    <Col xs={12} md={12}>
                                      <label className='form__form-group-label filter-label'>{(screenCriteria.UsrEmail10 || {}).ColumnHeader}</label>
                                    ...
                                    <Col xs={12} md={12}>
                                      <label className='form__form-group-label filter-label'>{(screenCriteria.UsrName20 || {}).ColumnHeader}</label>
                                      <div className='form__form-group-field filter-form-border'>
                                        <Field
                                          type='text'
                                          name='cCriUsrName20'
                                    ...
                                    <Col xs={12} md={12}>
                                      <label className='form__form-group-label filter-label'>{(screenCriteria.UsrGroupLs30 || {}).ColumnHeader}</label>
                                      <div className='form__form-group-field filter-form-border'>
                                        <Field
                                          type='text'
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmUsrState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmUsrState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmUsrReduxObj.LoadInitPage.bind(AdmUsrReduxObj) },
    { LoadSearchList: AdmUsrReduxObj.LoadSearchList.bind(AdmUsrReduxObj) },
    { SelectMst: AdmUsrReduxObj.SelectMst.bind(AdmUsrReduxObj) },
    { DelMst: AdmUsrReduxObj.DelMst.bind(AdmUsrReduxObj) },
    { AddMst: AdmUsrReduxObj.AddMst.bind(AdmUsrReduxObj) },
    { AddDtl: AdmUsrReduxObj.AddDtl.bind(AdmUsrReduxObj) },
    { changeMstListFilterVisibility: AdmUsrReduxObj.ChangeMstListFilterVisibility.bind(AdmUsrReduxObj) },
    { SetScreenCriteria: AdmUsrReduxObj.SetScreenCriteria.bind(AdmUsrReduxObj) },

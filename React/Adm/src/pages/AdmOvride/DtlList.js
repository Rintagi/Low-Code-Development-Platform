...
import AdmOvrideReduxObj from '../../redux/AdmOvride';
...
class DtlList extends RintagiScreen {
  constructor(props) {
    ...
    this.DtlKeyColumnName = 'OvrideGrpId123';
  ...
  CopyRow({ mst, dtlId, dtl, ...rest }) {
    const AdmOvrideState = this.props.AdmOvride || {};
    const auxSystemLabels = AdmOvrideState.SystemLabel || {};
    return function (evt) {
      evt.preventDefault();
      const currDtlId = dtlId || dtl.OvrideGrpId123;
      const copyFn = () => {
        if (currDtlId) {
          this.props.AddDtl(mst.OvrideId122, currDtlId);
          const isMobileView = window.matchMedia('(max-width: 1099px)').matches;
          const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
          if (useMobileView) {
            const naviBar = getNaviBar('DtlList', mst, {}, this.props.AdmOvride.Label);
      ...
      const deleteFn = () => {
        ...
        if (currDtlId) {
          this.props.SavePage(
            ...
            this.props.AdmOvride.Mst,
            [
              {
                OvrideGrpId123: currDtlId,
                _mode: 'delete',
              }
            ],
            {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmOvride || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    const currReduxScreenState = this.props.AdmOvride || {};
  ...
  render() {
    ...
    if (AdmOvrideState.access_denied) {
      return <Redirect to='/error' />;
    }
    //if (!AdmOvrideState.initialized || AdmOvrideState.page_loading) return null;
    const screenHlp = AdmOvrideState.ScreenHlp;
    ...
    const itemList = AdmOvrideReduxObj.DtlListToSelectList(AdmOvrideState, 'OvrideGrpId123');
    const selectList = AdmOvrideReduxObj.SearchListToSelectList(AdmOvrideState);
    const selectedMst = (selectList || []).filter(v=>v.isSelected)[0] || {};
    const auxLabels = AdmOvrideState.Label || {};
    const auxSystemLabels = AdmOvrideState.SystemLabel || {};
    const columnLabel = AdmOvrideState.ColumnLabel;
    ...
    const currMst = AdmOvrideState.Mst;
    const currDtl = AdmOvrideState.EditDtl;
    const dtlFilter = AdmOvrideState.DtlFilter;
    const filterList = AdmOvrideState.DtlFilter.FilterColumnDdl.map((v, i) => ({ key: v.ColName, label: v.ColumnHeader, value: v.ColName, pos: i }))
                        ...
                        resetForm
                          ...
                          <div>
                                      ...
                                      <ButtonGroup className='btn-group--icons'>
                                        ...
                                        </Button>
                                        {
                                          dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                            if ((!activeSelectionVisible && v.buttonType !== 'InsRow') || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).OvrideId122)) return null;
                                            return (
                                              <Button
                                                key={v.tid}
                                        ...
                                        {activeSelectionVisible &&
                                          dropdownMenuButtonList
                                            .filter(v => !v.expose)
                                            .filter(v => (!this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).OvrideId122,(currDtl || {}).OvrideGrpId123)))
                                            .length > 0 &&
                                        ...
                                        <DropdownMenu right className={`dropdown__menu dropdown-options`}>
                                          {
                                            ...
                                            dropdownMenuButtonList.filter(v => !v.expose).map(v => {
                                              if ((!activeSelectionVisible && v.buttonType !== 'InsRow') || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).OvrideId122,(currDtl || {}).OvrideGrpId123)) return null;
                                              return (
                            ...
                            <Form className='form'>
                                ...
                                </div>
                                {
                                  itemList.slice(0, dtlFilter.Limit).map(
                                    (obj, i) => {
                                      ...
                                      return (
                                        <div className='form__form-group-narrow list-divider' key={obj.OvrideGrpId123 || ('_' + i)}>
                                          <div className='form__form-group-field'>
                                            <label className='radio-btn radio-btn--button margin-narrow'>
                                              <Field
                                                key={obj.OvrideGrpId123 || ('_' + i)}
                                                className='radio-btn__radio'
                                                name='expenseItem'
                                                type='radio'
                                                value={values.name}
                                                onClick={this.SelectDtlListRow(naviSelectBar, currMst, obj, i, 'OvrideId122', 'OvrideGrpId123')}
                                              ...
                                              </span>
                                              {
                                                rowMenuButtonList
                                                  ...
                                                  .map((v, i) => {
                                                    return (
                                                      <button type='button' onClick={this.ScreenButtonAction[v.buttonType]({ naviBar: naviSelectBar, mst: currMst, useMobileView, dtlId: obj.OvrideGrpId123 })} className={`${v.exposedClassName}`}><i className={`${v.iconClassName}`}></i></button>
                                                    )
                                                  })
                                              }
                                              {
                                                ...
                                                <UncontrolledDropdown className={`btn-row-dropdown`}>
                                                  ...
                                                  <DropdownMenu right className={`dropdown__menu dropdown-options`}>
                                                    {rowMenuButtonList
                                                      .filter(v => !v.expose || useMobileView)
                                                      .map((v) => {
                                                        if (this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).OvrideId122,obj.OvrideGrpId123)) return null;
                                                        return <DropdownItem key={v.tid} onClick={this.ScreenButtonAction[v.buttonType]({ naviBar: naviSelectBar, mst: currMst, useMobileView, dtlId: obj.OvrideGrpId123 })} className={`${v.className}`}><i className={`${v.iconClassName} mr-10`}></i>{v.label}</DropdownItem>
                                                      })
                                                    }
                                                  </DropdownMenu>
                                                </UncontrolledDropdown>
                              ...
                              <Button onClick={this.props.ViewMoreDtl} className={`${viewMoreClass} btn btn-view-more-blue account__btn`} type='button'>{strFormat(IncrementMsg, this.props.AdmOvride.DtlFilter.PageSize)}<br /><i className='fa fa-arrow-down'></i></Button>
            ...
            {!useMobileView && <Col xs={6}>
              ...
              {!activeSelectionVisible &&
                ...
                <div className='empty-block'>
                  ...
                  <p className='create-new-message'>{NoDetailMsg}. <span className='link-imitation' onClick={this.AddNewDtl({ naviBar, mstId: currMst.OvrideId122 })}>{AddDetailMsg}</span></p>
                </div>}
...
const mapStateToProps = (state) => ({
  user: state.user,
  error: state.error,
  AdmOvride: state.AdmOvride,
  global: state.global,
});

const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    { LoadPage: AdmOvrideReduxObj.LoadPage.bind(AdmOvrideReduxObj) },
    { SelectDtl: AdmOvrideReduxObj.SelectDtl.bind(AdmOvrideReduxObj) },
    { SavePage: AdmOvrideReduxObj.SavePage.bind(AdmOvrideReduxObj) },
    { AddDtl: AdmOvrideReduxObj.AddDtl.bind(AdmOvrideReduxObj) },
    { ChangeDtlListFilterVisibility: AdmOvrideReduxObj.ChangeDtlListFilterVisibility.bind(AdmOvrideReduxObj) },
    { ChangeDtlListFilter: AdmOvrideReduxObj.ChangeDtlListFilter.bind(AdmOvrideReduxObj) },
    { ViewMoreDtl: AdmOvrideReduxObj.ViewMoreDtl.bind(AdmOvrideReduxObj) },
    { setTitle: setTitle },

...
import AdmOvrideReduxObj, { ShowMstFilterApplied } from '../../redux/AdmOvride';
...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmOvride || {}).initialized) {
  ...
  DelMst({ naviBar, ScreenButton, mst, mstId }) {
    ...
    return this.Prompt({
      okFn: function (evt) {
        const fromMstId = mstId || mst.OvrideId122;
        this.props.DelMst(this.props.AdmOvride, fromMstId);
      }.bind(this),
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).OvrideId122) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    const screenDdlSelectors = AdmOvrideReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmOvrideReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmOvrideState.ScreenCriteria || {}
    ...
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.OvrideId122));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmOvrideReduxObj.QuickFilterDdlToSelectList(AdmOvrideState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmOvrideState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmOvrideState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).OvrideId122)}
                    ...
                    <Formik
                      ...
                      key={AdmOvrideState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmOvrideState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmOvrideState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
                                    ...
                                    <Row className='btn-bottom-row'>
                                      {
                                        bottomButtonList
                                          .filter(v => v.expose)
                                          .map((v, i, a) => {
                                            if (this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).OvrideId122)) return null;
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmOvrideReduxObj.LoadInitPage.bind(AdmOvrideReduxObj) },
    { LoadSearchList: AdmOvrideReduxObj.LoadSearchList.bind(AdmOvrideReduxObj) },
    { SelectMst: AdmOvrideReduxObj.SelectMst.bind(AdmOvrideReduxObj) },
    ...
    { changeMstListFilterVisibility: AdmOvrideReduxObj.ChangeMstListFilterVisibility.bind(AdmOvrideReduxObj) },
    { SetScreenCriteria: AdmOvrideReduxObj.SetScreenCriteria.bind(AdmOvrideReduxObj) },

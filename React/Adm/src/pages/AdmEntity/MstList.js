...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmEntity || {}).initialized) {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmEntity || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).EntityId199) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    // if (!AdmEntityState.initialized || AdmEntityState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmEntityReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmEntityReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmEntityState.ScreenCriteria || {}
    ...
    const currMst = AdmEntityState.Mst;
    const currDtl = AdmEntityState.EditDtl;
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.EntityId199));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmEntityReduxObj.QuickFilterDdlToSelectList(AdmEntityState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmEntityState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmEntityState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).EntityId199)}
                    ...
                    <Formik
                      ...
                      key={AdmEntityState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmEntityState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmEntityState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmEntityReduxObj.LoadInitPage.bind(AdmEntityReduxObj) },
    { LoadSearchList: AdmEntityReduxObj.LoadSearchList.bind(AdmEntityReduxObj) },
    { SelectMst: AdmEntityReduxObj.SelectMst.bind(AdmEntityReduxObj) },
    { DelMst: AdmEntityReduxObj.DelMst.bind(AdmEntityReduxObj) },
    { AddMst: AdmEntityReduxObj.AddMst.bind(AdmEntityReduxObj) },
    { AddDtl: AdmEntityReduxObj.AddDtl.bind(AdmEntityReduxObj) },
    { changeMstListFilterVisibility: AdmEntityReduxObj.ChangeMstListFilterVisibility.bind(AdmEntityReduxObj) },
    { SetScreenCriteria: AdmEntityReduxObj.SetScreenCriteria.bind(AdmEntityReduxObj) },

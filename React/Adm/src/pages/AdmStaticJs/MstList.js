...
import AdmStaticJsReduxObj, { ShowMstFilterApplied } from '../../redux/AdmStaticJs';
import * as AdmStaticJsService from '../../services/AdmStaticJsService';
...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmStaticJs || {}).initialized) {
  ...
  CopyHdr({ naviBar, ScreenButton, useMobileView, mst, mstId }) {
    const AdmStaticJsState = this.props.AdmStaticJs || {};
    const auxSystemLabels = AdmStaticJsState.SystemLabel || {};
    return function (evt) {
      evt.preventDefault();
  ...
  DelMst({ naviBar, ScreenButton, mst, mstId }) {
    ...
    return this.Prompt({
      okFn: function (evt) {
        const fromMstId = mstId || mst.StaticJsId261;
        this.props.DelMst(this.props.AdmStaticJs, fromMstId);
      }.bind(this),
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmStaticJs || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    const currReduxScreenState = this.props.AdmStaticJs || {};
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).StaticJsId261) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    if (AdmStaticJsState.access_denied) {
      return <Redirect to='/error' />;
    }

    // if (!AdmStaticJsState.initialized || AdmStaticJsState.page_loading) return null;
    const screenHlp = AdmStaticJsState.ScreenHlp;
    ...
    const screenDdlSelectors = AdmStaticJsReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmStaticJsReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmStaticJsState.ScreenCriteria || {}
    const selectList = AdmStaticJsReduxObj.SearchListToSelectList(AdmStaticJsState);
    const itemList = AdmStaticJsState.Dtl || [];
    const auxLabels = AdmStaticJsState.Label || {};
    const auxSystemLabels = AdmStaticJsState.SystemLabel || {};
    const columnLabel = AdmStaticJsState.ColumnLabel || {};
    const currMst = AdmStaticJsState.Mst;
    const currDtl = AdmStaticJsState.EditDtl;
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.StaticJsId261));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmStaticJsReduxObj.QuickFilterDdlToSelectList(AdmStaticJsState);
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmStaticJsState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmStaticJsState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).StaticJsId261)}
                    ...
                    <Formik
                      ...
                      key={AdmStaticJsState.searchListVersion}
                        ...
                        setFieldTouched
                          ...
                          <div>
                            <Form className='form'>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmStaticJsState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmStaticJsState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
                                    ...
                                    <Row className='btn-bottom-row'>
                                      {
                                        bottomButtonList
                                          .filter(v => v.expose)
                                          .map((v, i, a) => {
                                            if (this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).StaticJsId261)) return null;
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    { LoadPage: AdmStaticJsReduxObj.LoadPage.bind(AdmStaticJsReduxObj) },
    { LoadInitPage: AdmStaticJsReduxObj.LoadInitPage.bind(AdmStaticJsReduxObj) },
    { LoadSearchList: AdmStaticJsReduxObj.LoadSearchList.bind(AdmStaticJsReduxObj) },
    { SelectMst: AdmStaticJsReduxObj.SelectMst.bind(AdmStaticJsReduxObj) },
    { DelMst: AdmStaticJsReduxObj.DelMst.bind(AdmStaticJsReduxObj) },
    { AddMst: AdmStaticJsReduxObj.AddMst.bind(AdmStaticJsReduxObj) },
    { AddDtl: AdmStaticJsReduxObj.AddDtl.bind(AdmStaticJsReduxObj) },
    { changeMstListFilterVisibility: AdmStaticJsReduxObj.ChangeMstListFilterVisibility.bind(AdmStaticJsReduxObj) },
    { SetScreenCriteria: AdmStaticJsReduxObj.SetScreenCriteria.bind(AdmStaticJsReduxObj) },

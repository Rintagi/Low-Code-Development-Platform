...
class MstList extends RintagiScreen {
  constructor(props) {
    ...
    if (lastAppUrl && !(this.props.AdmRptStyle || {}).initialized) {
  ...
  RefreshSearchList(values, { setErrors, resetForm, setValues /* setValues and other goodies */ }) {
    ...
    const refreshFn = (() => {
      this.props.SetScreenCriteria(values.search,
        {
          DefaultCd10: (values.cCriDefaultCd10) ? values.cCriDefaultCd10.value : '',
        },
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmRptStyle || {}).Buttons || {};
  ...
  componentDidUpdate(prevProps, prevStates) {
    ...
    if (prevStates.key !== (currReduxScreenState.Mst || {}).RptStyleId167) {
      const isMobileView = this.state.isMobile;
      const useMobileView = (isMobileView && !(this.props.user || {}).desktopView);
  ...
  render() {
    ...
    // if (!AdmRptStyleState.initialized || AdmRptStyleState.page_loading) return null;
    ...
    const screenDdlSelectors = AdmRptStyleReduxObj.ScreenDdlSelectors;
    const screenCriDdlSelectors = AdmRptStyleReduxObj.ScreenCriDdlSelectors;
    const screenCriteria = AdmRptStyleState.ScreenCriteria || {}
    ...
    const currMst = AdmRptStyleState.Mst;
    const currDtl = AdmRptStyleState.EditDtl;
    const naviBar = getNaviBar('MstList', currMst, {}, screenButtons).filter(v => ((v.type !== 'DtlRecord' && v.type !== 'DtlList') || currMst.RptStyleId167));
    const naviSelectBar = getNaviBar('MstList', {}, {}, screenButtons);
    const screenFilterList = AdmRptStyleReduxObj.QuickFilterDdlToSelectList(AdmRptStyleState);
    ...
    const CriDefaultCd10List = screenCriDdlSelectors.DefaultCd10(AdmRptStyleState);
    const CriDefaultCd10Selected = CriDefaultCd10List.filter(obj => { return obj.key === screenCriteria.DefaultCd10.LastCriteria });
              ...
              <div className='account'>
                <div className='account__wrapper account-col'>
                  <div className='account__card rad-4'>
                    {/* {this.constructor.ShowSpinner(AdmRptStyleState,'MstList') && siteSpinner} */}
                            ...
                            <UncontrolledDropdown>
                              <ButtonGroup className='btn-group--icons'>
                                <Button className={`mw-50 ${filterBtnStyle}`} onClick={this.props.changeMstListFilterVisibility} outline>
                                  <i className={`fa fa-filter icon-holder ${filterActive}`}></i>{ShowMstFilterApplied(AdmRptStyleState) && <i className='filter-applied'></i>}
                                  ...
                                  dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                    return (
                                      <Button
                                        ...
                                        // disabled={!activeSelectionVisible || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).RptStyleId167)}
                            ...
                            <Form className='form'>
                              <div className='form__form-group'>
                                <div className={`form__form-group filter-padding ${filterVisibility}`} key={screenCriteria.key}>
                                  <Row className='mb-5'>
                                    <Col xs={12} md={12}>
                                      <label className='form__form-group-label filter-label'>{(screenCriteria.DefaultCd10 || {}).ColumnHeader}</label>
                                      <div className='form__form-group-field filter-form-border'>
                                        <DropdownField
                                          name='cCriDefaultCd10'
                                          onChange={this.SearchFilterValueChange(handleSubmit, setFieldValue, 'ddl', 'cCriDefaultCd10')}
                                          value={values.cCriDefaultCd10}
                                          options={CriDefaultCd10List}
                                          placeholder=''
                                        />
                                      </div>
                                    </Col>
                              ...
                              {this.constructor.ShowMoreMstBtn(AdmRptStyleState) && <Button className={`btn btn-view-more-blue account__btn ${noMargin}`} onClick={this.ShowMoreSearchList} type='button'>{strFormat(IncrementMsg, AdmRptStyleState.ScreenCriteria.Increment)}<br /><i className='fa fa-arrow-down'></i></Button>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { LoadInitPage: AdmRptStyleReduxObj.LoadInitPage.bind(AdmRptStyleReduxObj) },
    { LoadSearchList: AdmRptStyleReduxObj.LoadSearchList.bind(AdmRptStyleReduxObj) },
    { SelectMst: AdmRptStyleReduxObj.SelectMst.bind(AdmRptStyleReduxObj) },
    { DelMst: AdmRptStyleReduxObj.DelMst.bind(AdmRptStyleReduxObj) },
    { AddMst: AdmRptStyleReduxObj.AddMst.bind(AdmRptStyleReduxObj) },
    { AddDtl: AdmRptStyleReduxObj.AddDtl.bind(AdmRptStyleReduxObj) },
    { changeMstListFilterVisibility: AdmRptStyleReduxObj.ChangeMstListFilterVisibility.bind(AdmRptStyleReduxObj) },
    { SetScreenCriteria: AdmRptStyleReduxObj.SetScreenCriteria.bind(AdmRptStyleReduxObj) },

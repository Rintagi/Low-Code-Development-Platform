...
import AdmDbTableReduxObj from '../../redux/AdmDbTable';
...
class DtlList extends RintagiScreen {
    ...
    return function (evt) {
      ...
      const copyFn = () => {
        if (currDtlId) {
          ...
          if (useMobileView) {
            const naviBar = getNaviBar('DtlList', mst, {}, this.props.AdmDbTable.Label);
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const buttons = (nextProps.AdmDbTable || {}).Buttons || {};
  ...
  render() {
    ...
    //if (!AdmDbTableState.initialized || AdmDbTableState.page_loading) return null;
    ...
    const itemList = AdmDbTableReduxObj.DtlListToSelectList(AdmDbTableState, 'ColumnId5');
    ...
    const dtlFilter = AdmDbTableState.DtlFilter;
    const filterList = AdmDbTableState.DtlFilter.FilterColumnDdl.map((v, i) => ({ key: v.ColName, label: v.ColumnHeader, value: v.ColName, pos: i }))
                        ...
                        resetForm
                          ...
                          <div>
                                      ...
                                      <ButtonGroup className='btn-group--icons'>
                                        ...
                                        </Button>
                                        {
                                          dropdownMenuButtonList.filter(v => v.expose).map(v => {
                                            if ((!activeSelectionVisible && v.buttonType !== 'InsRow') || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).TableId3)) return null;
                                            return (
                                              <Button
                                                key={v.tid}
                                        ...
                                        {activeSelectionVisible &&
                                          dropdownMenuButtonList
                                            .filter(v => !v.expose)
                                            .filter(v => (!this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).TableId3,(currDtl || {}).ColumnId5)))
                                            .length > 0 &&
                                        ...
                                        <DropdownMenu right className={`dropdown__menu dropdown-options`}>
                                          {
                                            ...
                                            dropdownMenuButtonList.filter(v => !v.expose).map(v => {
                                              if ((!activeSelectionVisible && v.buttonType !== 'InsRow') || this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).TableId3,(currDtl || {}).ColumnId5)) return null;
                                              return (
                            ...
                            <Form className='form'>
                                ...
                                </div>
                                {
                                  itemList.slice(0, dtlFilter.Limit).map(
                                    (obj, i) => {
                                      ...
                                      return (
                                        <div className='form__form-group-narrow list-divider' key={obj.ColumnId5 || ('_' + i)}>
                                          <div className='form__form-group-field'>
                                            <label className='radio-btn radio-btn--button margin-narrow'>
                                              <Field
                                                key={obj.ColumnId5 || ('_' + i)}
                                                className='radio-btn__radio'
                                                name='expenseItem'
                                                type='radio'
                                                value={values.name}
                                                onClick={this.SelectDtlListRow(naviSelectBar, currMst, obj, i, 'TableId3', 'ColumnId5')}
                                              ...
                                              </span>
                                              {
                                                rowMenuButtonList
                                                  ...
                                                  .map((v, i) => {
                                                    return (
                                                      <button type='button' onClick={this.ScreenButtonAction[v.buttonType]({ naviBar: naviSelectBar, mst: currMst, useMobileView, dtlId: obj.ColumnId5 })} className={`${v.exposedClassName}`}><i className={`${v.iconClassName}`}></i></button>
                                                    )
                                                  })
                                              }
                                              {
                                                ...
                                                <UncontrolledDropdown className={`btn-row-dropdown`}>
                                                  ...
                                                  <DropdownMenu right className={`dropdown__menu dropdown-options`}>
                                                    {rowMenuButtonList
                                                      .filter(v => !v.expose || useMobileView)
                                                      .map((v) => {
                                                        if (this.ActionSuppressed(authRow, v.buttonType, (currMst || {}).TableId3,obj.ColumnId5)) return null;
                                                        return <DropdownItem key={v.tid} onClick={this.ScreenButtonAction[v.buttonType]({ naviBar: naviSelectBar, mst: currMst, useMobileView, dtlId: obj.ColumnId5 })} className={`${v.className}`}><i className={`${v.iconClassName} mr-10`}></i>{v.label}</DropdownItem>
                                                      })
                                                    }
                                                  </DropdownMenu>
                                                </UncontrolledDropdown>
                              ...
                              <Button onClick={this.props.ViewMoreDtl} className={`${viewMoreClass} btn btn-view-more-blue account__btn`} type='button'>{strFormat(IncrementMsg, this.props.AdmDbTable.DtlFilter.PageSize)}<br /><i className='fa fa-arrow-down'></i></Button>
            ...
            {!useMobileView && <Col xs={6}>
              ...
              {!activeSelectionVisible &&
                ...
                <div className='empty-block'>
                  ...
                  <p className='create-new-message'>{NoDetailMsg}. <span className='link-imitation' onClick={this.AddNewDtl({ naviBar, mstId: currMst.TableId3 })}>{AddDetailMsg}</span></p>
                </div>}
...
const mapDispatchToProps = (dispatch) => (
  bindActionCreators(Object.assign({},
    ...
    { SelectDtl: AdmDbTableReduxObj.SelectDtl.bind(AdmDbTableReduxObj) },
    { SavePage: AdmDbTableReduxObj.SavePage.bind(AdmDbTableReduxObj) },
    { AddDtl: AdmDbTableReduxObj.AddDtl.bind(AdmDbTableReduxObj) },
    { ChangeDtlListFilterVisibility: AdmDbTableReduxObj.ChangeDtlListFilterVisibility.bind(AdmDbTableReduxObj) },
    { ChangeDtlListFilter: AdmDbTableReduxObj.ChangeDtlListFilter.bind(AdmDbTableReduxObj) },
    { ViewMoreDtl: AdmDbTableReduxObj.ViewMoreDtl.bind(AdmDbTableReduxObj) },
    { setTitle: setTitle },
